...
$sql_party=mysqli_query($link,"select * from tbl_partymaser where p_id='$txtparty'");
	...
	$dh="Stock_Transfer-Plant".$a."_From_".$_REQUEST['sdate']."_To_".$_REQUEST['edate'];
	$datahead = array($dh);
	$datahead1 = array("Stock Transfer-Plant" .$a);
	$datahead2 = array("Stock Transfer Plant Wise Report: Period  From ",$_REQUEST['sdate'],"  To ",$_REQUEST['edate']);
	$data1 = array();
...
function cleanData(&$str)
 ...
 $datatitle2 = array("","","","","","","","","PP","Moist %","Germ %","","","","");
	$datatitle3 = array("#","Date of Arrival","crop","Variety","Lot No.","NoB","Qty"," Stage","QC","","","QC Status","DoT","GOT Status","DoGR");
...
$sql_binn=mysqli_query($link,"select binname from tbl_bin where binid='".$row_sloc['binid']."' and whid='".$row_sloc['whid']."' and plantcode='".$plantcode."' and plantcode='".$plantcode."'") or die(mysqli_error($link));
...
if($tot_arr_home > 0)			
{
$data1[$d]=array($d,$trdate,$crop,$variety,$lotno,$bags,$qty,$stage,$vk,$moist,$qc,$got,$tdate12,$got1,$tdate13); 
$d++;
}
}
}
