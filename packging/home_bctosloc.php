...
$sql_tbl=mysqli_query($link,"select * from tbl_btslmain where plantcode='$plantcode' and btsl_logid='".$logid."' and btsl_trtyp='bctosloc' and btsl_tflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tbl_btslmain where btsl_logid='".$logid."' and btsl_trtyp='bctosloc' and btsl_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
<title>Packaging - Transaction - Barcode to SLOC Linking - Home</title>
...
<!--<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#1dbe03" style="border-collapse:collapse">
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_btslmain where plantcode='$plantcode' and btsl_tflg=1 LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_btslmain where plantcode='$plantcode' and btsl_tflg=1";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_btslmain where btsl_tflg=1 LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
//$tot_arr_home=0;

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_btslmain where btsl_tflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="600" bordercolor="#1dbe03" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
              ...
              <td width="65" align="center" valign="middle" class="smalltblheading">Details</td>
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$subtbltot=0;
	$sql_tbl_sub=mysqli_query($link,"select * from tbl_btslsub where plantcode='$plantcode' and btsl_id='".$arrival_id."'") or die(mysqli_error($link));
	$subtbltot=mysqli_num_rows($sql_tbl_sub);
	
	
	$wareh=""; $binn=""; $subbinn=""; $sloc=""; 
	$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='$plantcode' and plantcode='$plantcode' and whid='".$row_arr_home['btsl_wh']."' order by perticulars") or die(mysqli_error($link));
...
<tr class="Light">
	...
	<td align="center" valign="middle" class="smalltbltext">Details</td>
