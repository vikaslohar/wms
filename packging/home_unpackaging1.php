...
$sql_tbl=mysqli_query($link,"select * from tbl_unpkg where plantcode='$plantcode' and unpkg_logid='".$logid."' and unpkg_tflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['unpkg_id'];	
	$s_sub="delete from tbl_unpkgsub where unpkg_id='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
}
	
	$s_sub="delete from tbl_unpkg where unpkg_logid='".$logid."' and unpkg_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_unpackaging1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
...
<title>Packaging - Transaction - Unpackaging - Home</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Unpackaging</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_unpacking.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
</table></td>
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#1dbe03" style="border-collapse:collapse">
  ...
  <?php
  	$sdate2=$_REQUEST['sdate'];
	$edate2=$_REQUEST['edate'];
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_unpkg where plantcode='$plantcode' and unpkg_tflg=1 and unpkg_date>='$tdate' and unpkg_date<='$pdate' order by unpkg_id DESC LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_unpkg where plantcode='$plantcode' and unpkg_tflg=1 and unpkg_date>='$tdate' and unpkg_date<='$pdate'";
...
$pagination = "";
	...
	{	
		...
		//previous button
		if ($page > 1) 
			$pagination.= "<a href=\"$targetpage?page=$prev&sdate=$sdate2&edate=$edate2\">&laquo; Previous </a>";
		else
		...
		{	
			for ($counter = 1; $counter <= $lastpage; $counter++)
			{
				if ($counter == $page)
					...
					$pagination.= "<a href=\"$targetpage?page=$counter&sdate=$sdate2&edate=$edate2\"> $counter </a>";					
			}
		}
		elseif($lastpage > 5 + ($adjacents * 2))	//enough pages to hide some
		{
			...
			if($page < 1 + ($adjacents * 2))		
			{
				...
				$pagination.= "...";
				$pagination.= "<a href=\"$targetpage?page=$lpm1&sdate=$sdate2&edate=$edate2\"> $lpm1 </a>";
				$pagination.= "<a href=\"$targetpage?page=$lastpage&sdate=$sdate2&edate=$edate2\"> $lastpage </a>";		
			}
			//in middle; hide some front and some back
			elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
			{
				$pagination.= "<a href=\"$targetpage?page=1&sdate=$sdate2&edate=$edate2\"> 1 </a>";
				$pagination.= "<a href=\"$targetpage?page=2&sdate=$sdate2&edate=$edate2\"> 2 </a>";
				$pagination.= "...";
		...
		//next button
		if ($page < $counter - 1) 
			$pagination.= "<a href=\"$targetpage?page=$next&sdate=$sdate2&edate=$edate2\"> Next &raquo;</a>";
		else
...
$sql_arr_home=mysqli_query($link,"select * from tbl_unpkg where unpkg_tflg=1 and unpkg_date>='$tdate' and unpkg_date<='$pdate' order by unpkg_id DESC LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
//$tot_arr_home=0;

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_unpkg where unpkg_tflg=1 and unpkg_date>='$tdate' and unpkg_date<='$pdate'");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Unpackaged List</td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="850" bordercolor="#1dbe03" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="74" align="center" valign="middle" class="smalltblheading">Date</td>
              <td width="82" align="center" valign="middle" class="smalltblheading">Barcode</td>
              <td width="100" align="center" valign="middle" class="smalltblheading">Crop</td>
              ...
              <td width="65" align="center" valign="middle" class="smalltblheading">Packaging Type</td>
              <td width="80" align="center" valign="middle" class="smalltblheading">NoP</td>
              <td width="80" align="center" valign="middle" class="smalltblheading">Qty</td>
             </tr>
<?php
//$srno=1;
if($tot_arr_home > 0)
...
{
	$trdate=$row_arr_home['unpkg_date'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;
	
	$lrole=$row_arr_home['unpkg_logid'];
	$arrival_id=$row_arr_home['unpkg_id'];
	
	$lotno=""; $bags1=""; $qty1=""; $upstyp=""; $ep="";
	$sql_tbl_sub=mysqli_query($link,"select * from tbl_unpkgsub where plantcode='$plantcode' and unpkg_id='".$arrival_id."'") or die(mysqli_error($link));
	$subtbltot=mysqli_num_rows($sql_tbl_sub);
	while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
	{
	
	$quer3=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropid='".$row_tbl_sub['unpkgsub_crop']."' order by cropname Asc");
	$noticia = mysqli_fetch_array($quer3);
	$crop=$noticia['cropname'];
	
	$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$row_tbl_sub['unpkgsub_variety']."'  order by popularname Asc"); 
	$noticia_item = mysqli_fetch_array($quer4);
	$variety=$noticia_item['popularname'];
//echo $row_tbl_sub['pnpslipsub_pqty'];
$aq=explode(".",$row_tbl_sub['unpkgsub_nop']);
if($aq[1]==000){$onob=$aq[0];}else{$onob=$row_tbl_sub['unpkgsub_nop'];}

$an=explode(".",$row_tbl_sub['unpkgsub_qty']);
if($an[1]==000){$oqty=$an[0];}else{$oqty=$row_tbl_sub['unpkgsub_qty'];}

		if($lotno!="")
		{
			$lotno=$lotno."<br>".$row_tbl_sub['unpkgsub_lotno'];
		}
		else
		{
			$lotno=$row_tbl_sub['unpkgsub_lotno'];
		}
		if($bags1!="")
		{
		$bags1=$bags1."<br>".$onob;
		...
		$pktp=$row_tbl_sub['unpkgsub_mptype'];
		if($pktp=="PACKSMC")$pktp="SMC";
		if($pktp=="PACKLMC")$pktp="LMC";
		if($pktp=="PACKMMC")$pktp="MMC";
		if($ep!="")
		{
		$ep=$ep."<br>".$pktp;
		}
		...
		if($upstyp!="")
		{
		$upstyp=$upstyp."<br>".$row_tbl_sub['unpkgsub_ups'];
		}
		else
		{
		$upstyp=$row_tbl_sub['unpkgsub_ups'];
		}
	}
if($srno%2!=0)
{
...
<tr class="Light">
	...
	<td align="center" valign="middle" class="smalltbltext"><?php echo $row_arr_home['unpkg_barcode'];?></td>
...
<tr >
<td valign="top" align="right"><a href="home_unpackaging.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:Pointer;" /></a></td>
</tr>
</table>
