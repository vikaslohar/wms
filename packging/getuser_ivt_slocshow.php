...
$sql_month24=mysqli_query($link,"SELECT max(SUBSTRING(ivts_nlotno,15,2)) FROM tbl_ivtsub where plantcode='$plantcode' and SUBSTRING(ivts_nlotno,1,13)='$abc'")or die("Error:".mysqli_error($link));
...
$sql_issue24=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where plantcode='$plantcode' and lotldg_crop='".$a."' and lotldg_variety='".$b."' and lotldg_lotno='".$f."' and lotldg_qc!='Fail' and lotldg_got!='Fail'") or die(mysqli_error($link));
 while($row_issue24=mysqli_fetch_array($sql_issue24))
 { 

$sql_issue=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_subbinid='".$row_issue24['lotldg_subbinid']."' and lotldg_binid='".$row_issue24['lotldg_binid']."' and lotldg_whid='".$row_issue24['lotldg_whid']."' and lotldg_crop='".$a."' and lotldg_variety='".$b."' and lotldg_lotno='".$f."' and lotldg_qc!='Fail' and lotldg_got!='Fail'") or die(mysqli_error($link));
$trtr=mysqli_num_rows($sql_issue);
$row_issue=mysqli_fetch_array($sql_issue);

$sql_issue1=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='$plantcode' and lotldg_id='".$row_issue[0]."' and lotldg_variety='".$b."' and lotldg_lotno='".$f."'") or die(mysqli_error($link));
...
$sql_issue=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where plantcode='$plantcode' and lotldg_lotno='".$f."' ") or die(mysqli_error($link));

 while($row_issue=mysqli_fetch_array($sql_issue))
 { 

$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_lotno='".$f."' ") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 
...
$tqty=$difq1;
  ...
  <td width="56" align="center" valign="middle" class="smalltblheading" ><?php echo $wareh?></td>
	...
	<td width="54" align="center" valign="middle" class="smalltblheading"><input type="text" name="txttrnob<?php echo $srno2?>" id="txttrnob<?php echo $srno2?>" class="smalltbltext" value="" size="6" onkeypress="return isNumberKey(event)" onchange="nobchk(this.value,'<?php echo $srno2?>');" /></td>
   <td width="61" align="center" valign="middle" class="smalltblheading" ><input type="text" name="txttrqty<?php echo $srno2?>" id="txttrqty<?php echo $srno2?>" class="smalltbltext" value="" size="9"  onkeypress="return isNumberKey1(event)" onchange="qtychk(this.value,'<?php echo $srno2?>');" /></td>
