...
$sql_tbl=mysqli_query($link,"select * from tbl_ivtmain where plantcode='$plantcode' and ivt_logid='".$logid."' and ivt_tflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['ivt_id'];	
	
	$s_sub="delete from tbl_ivtsub where ivt_id='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_ivtsub_sub where ivt_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
	$s_sub_sub2="delete from tbl_ivtsub_sub2 where ivt_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub2) or die(mysqli_error($link));
	
}

	$s_sub="delete from tbl_ivtmain where ivt_logid='".$logid."' and ivt_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_ivt1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='home_ivt.php'</script>";
		}
	}
...
<title>Packaging -  Transaction - Lot Merger - Home</title>
...
{
winHandle=window.open('ivt_note.php?itmid='+tid,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); }

}

function openslocpopprint24(tid)
{
winHandle=window.open('merger_note.php?itmid='+tid,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); }
...
cellpadding="0">
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <td width="801" class="Mainheading" height="25"><table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
        <tr >
          <td width="810" height="25">&nbsp;Transaction - IVT</td>
        </tr>
	    </table></td>
...
<tr height="15" class="tbltitle" >
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_ivt.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
	  </tr>
	  </table></td></tr>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_ivtmain where plantcode='$plantcode' and ivt_tflg=1 order by ivt_date desc,ivt_code desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_ivtmain where plantcode='$plantcode' and ivt_tflg=1 order by ivt_date desc,ivt_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_mergermain where mergerm_tflg=1 order by mergerm_date desc,mergerm_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_mergermain where mergerm_tflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
if($tot_arr_home > 0)
{
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Inter Variety Transfer</td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="950" bordercolor="#1dbe03" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="8%" align="center" valign="middle" class="smalltblheading">IVT Date</td>
              <td width="13%" align="center" valign="middle" class="smalltblheading">Crop</td>
              <td width="15%" align="center" valign="middle" class="smalltblheading">Production Variety</td>
              <td width="15%" align="center" valign="middle" class="smalltblheading">Transfer From Variety</td>
			  <td width="15%" align="center" valign="middle" class="smalltblheading">Transfer To Variety</td>
              <td width="7%" align="center" valign="middle" class="smalltblheading">No. of Lots Transfered</td>
              <td width="9%" align="center" valign="middle" class="smalltblheading">Tatal Qty Transfered</td>
              <td width="5%" align="center" valign="middle" class="smalltblheading">Output</td>
              </tr>
<?php
//$srno=1;
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$trdate=$row_arr_home['ivt_date'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;	
	
	$trid=$row_arr_home['ivt_id'];
	$drole=$row_arr_home['ivt_logid'];
	$yrid=$row_arr_home['ivt_yearid'];
	
	$dq=explode(".",$row_arr_home['mergerm_qty']);
	if($dq[1]==000){$mqty=$dq[0];}else{$mqty=$row_arr_home['mergerm_qty'];}

$classqry=mysqli_query($link,"select cropid, cropname from tblcrop where cropid='".$row_arr_home['ivt_crop']."' order by cropname") or die(mysqli_error($link));
$noticia_class=mysqli_fetch_array($classqry);

$itemqry=mysqli_query($link,"select varietyid, popularname from tblvariety where varietyid='".$row_arr_home['ivt_pvariety']."' and actstatus='Active'") or die(mysqli_error($link));
$noticia_item=mysqli_fetch_array($itemqry);	

$itemqry2=mysqli_query($link,"select varietyid, popularname from tblvariety where varietyid='".$row_arr_home['ivt_trfromvariety']."' and actstatus='Active'") or die(mysqli_error($link));
$noticia_item2=mysqli_fetch_array($itemqry2);	

$itemqry3=mysqli_query($link,"select varietyid, popularname from tblvariety where varietyid='".$row_arr_home['ivt_trtovariety']."' and actstatus='Active'") or die(mysqli_error($link));
$noticia_item3=mysqli_fetch_array($itemqry3);	

$extlotno=""; $extnob=""; $extqty1=0; $extqty="";
$sql_eindent_sub=mysqli_query($link,"select * from tbl_ivtsub where plantcode='$plantcode' and ivt_id=$trid") or die(mysqli_error($link));
while($row_eindent_sub=mysqli_fetch_array($sql_eindent_sub))
{
$extlotno++;
$extnob=$extnob+$row_eindent_sub['ivts_nob']; 

$dq=explode(".",$row_eindent_sub['ivts_qty']);
if($dq[1]==000){$extqty1=$dq[0];}else{$extqty1=$row_eindent_sub['ivts_qty'];}

$extqty=$extqty+$extqty1;
...
<tr class="Light">
	...
	<td width="9%" align="center" valign="middle" class="smalltbltext"><a href="add_ivt_view.php?p_id=<?php echo $trid;?>"><?php echo "VT".$row_arr_home['ivt_code']."/".$row_arr_home['ivt_yearid']."/".$drole;?></a></td>
	<td width="8%" align="center" valign="middle" class="smalltbltext"><?php echo $trdate;?></td>
	<td width="13%" align="center" valign="middle" class="smalltbltext"><?php echo $noticia_class['cropname'];?></td>
	<td width="15%" align="center" valign="middle" class="smalltbltext"><?php echo $noticia_item['popularname'];?></td>
	<td width="15%" align="center" valign="middle" class="smalltbltext"><?php echo $noticia_item2['popularname'];?></td>
	<td width="15%" align="center" valign="middle" class="smalltbltext"><?php echo $noticia_item3['popularname']?></td>
	<td width="7%" align="center" valign="middle" class="smalltbltext"><?php echo $extlotno?></td>
	<td width="9%" align="center" valign="middle" class="smalltbltext"><?php echo $extqty;?></td>
	<td width="5%" align="center" valign="middle" class="smalltbltext"><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $trid;?>');">IVTN</a></td>
</tr>
<?php
}
else
