<?php
	...
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		...
		$nopinmp=$row_arrsub['pnpslipsub_mptnop'];
		...
		$a="";
		$sql_barcode2=mysqli_query($link,"Select * from tbl_barcodestmp where bar_lotno='".$packlotno."' and bar_psrn='".$drefno."'") or die(mysqli_error($link));
		...
		while($row_arrsubsub2=mysqli_fetch_array($sql_arrsubsub2))
		{
			...
			while($row_issuetbl22=mysqli_fetch_array($sql_issuetbl22))
			{
				...
				$srflg2=$row_issuetbl22['lotldg_srflg'];
				  ...
				  $sql_ins_main22="insert into tbl_lot_ldg (yearcode,lotldg_trtype, lotldg_trid, lotldg_trdate, lotldg_lotno, lotldg_crop, lotldg_variety, lotldg_whid, lotldg_binid, lotldg_subbinid, lotldg_opbags, lotldg_opqty, lotldg_trbags, lotldg_trqty, lotldg_balbags, lotldg_balqty, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_got, lotldg_qctestdate, lotldg_gottestdate, orlot, lotldg_gs, lotldg_resverstatus, lotldg_revcomment, lotldg_genpurity, lotldg_srtyp, lotldg_srflg, plantcode) values('$yearcode2','NSTPNPSLIPSUC', '$pid', '$arrival_date', '$packlotno', '$crop', '$variety', '$whid2', '$binid2', '$subbinid2', '$onob2', '$oqty2', '$nob12', '$qty12', '$balups2', '$balqty2', '$sstage2', '$sstatus2', '$moist2', '$gemp2', '$vchk2', '$got12', '$qc2', '$gotstatus2', '$qctestdate2', '$gottestdate2', '$orlot2', '$gs2', '$resverstatus2', '$revcomment2', '$geneticpurity2', '$srtype', '$srflg', '$plantcode')";
				//exit;
				...
				$sql_qc1=mysqli_query($link,"Select Max(tid) from tbl_qctest where plantcode='$plantcode' and oldlot='".$orlot2."'") or die(mysqli_error($link));
				...
				//}		
					...
					$sql_got1=mysqli_query($link,"Select Max(gottest_tid) from tbl_gottest  where plantcode='$plantcode' and gottest_oldlot='".$orlot2."'") or die(mysqli_error($link));
					...
					if($got1=="UT" && $qc2!="UT")
					{
						$sql_sub_sub13="insert into tbl_qctest(pp, moist, got, lotno, srdate, crop,  variety, sampleno, trstage, qc, state, gs, oldlot, spdate, dosdate, bflg, testdate, qcflg, qcstatus,gotsmpdflg,yearid,logid)values('$vchk2', '$moist2', '$got1', '$packlotno', '".$row_qc['srdate']."', '$crop', '$variety', '$ncode2', '$sstage2', '$qc2', '$state2',1,'$orlot2', '".$row_qc['spdate']."', '".$row_qc['dosdate']."',1,'".$row_qc['testdate']."',1, '$qc2',1,'$yrco','$logid')";
		...
		while($row_arrsubsub3=mysqli_fetch_array($sql_arrsubsub3))
		{
			...
			foreach($zxcd as $bstlval)
			{
				...
				$sql_qc1=mysqli_query($link,"Select Max(tid) from tbl_qctest where plantcode='$plantcode' and oldlot='".$orlot3."'") or die(mysqli_error($link));
				$row_qc1=mysqli_fetch_array($sql_qc1);
				...
				//}		
					...
					$sql_got1=mysqli_query($link,"Select Max(gottest_tid) from tbl_gottest  where plantcode='$plantcode' and gottest_oldlot='".$orlot3."'") or die(mysqli_error($link));
					$row_got1=mysqli_fetch_array($sql_got1);
					$sql_got=mysqli_query($link,"Select * from tbl_gottest where plantcode='$plantcode' and gottest_tid='".$row_got1[0]."'") or die(mysqli_error($link));
					$row_got=mysqli_fetch_array($sql_got);
					...
					{
						$sql_sub_sub12="insert into tbl_qctest(spdate, testdate, gotdate, dosdate, pp, moist, got, qc, variety, crop, gemp, srdate, qcstatus, gotstatus, sampleno, aflg, bflg, cflg, qcflg, gotflg, gsflg, gs, gotrefno, gotauth, doswdate, gotsmpdflg, stsno, qcrefno, genpurity, lotno, oldlot, yearid, logid, state, trstage) values('".$row_qc['spdate']."','".$row_qc['testdate']."','".$row_qc['gotdate']."','".$row_qc['dosdate']."','".$row_qc['pp']."','".$row_qc['moist']."','".$row_qc['got']."','".$row_qc['qc']."','".$row_qc['variety']."','".$row_qc['crop']."','".$row_qc['gemp']."','".$row_qc['srdate']."','".$row_qc['qcstatus']."','".$row_qc['gotstatus']."','".$row_qc['sampleno']."','".$row_qc['aflg']."','".$row_qc['bflg']."','".$row_qc['cflg']."','".$row_qc['qcflg']."','".$row_qc['gotflg']."','".$row_qc['gsflg']."','".$row_qc['gs']."','".$row_qc['gotrefno']."','".$row_qc['gotauth']."','".$row_qc['doswdate']."','".$row_qc['gotsmpdflg']."','".$row_qc['stsno']."','".$row_qc['qcrefno']."','".$row_qc['genpurity']."','".$packlotno."','".$orlot3."','$yrco','$logid', '$state3', '$sstage3')";
						mysqli_query($link,$sql_sub_sub12) or die(mysqli_error($link));
					}*/
		...
		$sql_code1="SELECT MAX(pnpslipmain_tcode) FROM tbl_pnpslipmain where pnpslipmain_ttype='NST Packing Slip' ORDER BY pnpslipmain_tcode DESC";
	$res_code1=mysqli_query($link,$sql_code1)or die(mysqli_error($link));
	 ...
	 //exit;
	echo "<script>window.location='home_nstpackslip.php'</script>";	
	}
?>
...
<title>Packaging -Transaction - NST Packing Slip- Preview</title>
...
function openslocpopprint()
{
	...
	if(document.frmaddDepartment.txtitem.value!="")
	{
		...
		winHandle=window.open('nstpackslip_print.php?itmid='+itm,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
		if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - NST Packing slip - Preview </td>
	    </tr></table></td>
