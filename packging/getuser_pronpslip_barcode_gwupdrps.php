<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if (($handle = fopen($filepath, "r")) !== FALSE) 
		{
			...
			$sqlmainbarc=mysqli_query($link,"select Distinct (wb_mpbarcode) from tbl_wbqrcode where wb_rpstrid='".$tid."' and wb_mpblinkflg=1 ") or die(mysqli_error($link));
...
//print_r($data);
					...
					{
//echo "select Distinct (wb_mpbarcode) from tbl_wbqrcode where wb_mpbarcode='".trim($data[7])."' and wb_rpstrid='".$tid."' "."<br />";
						$sql_mainbarc=mysqli_query($link,"select Distinct (wb_mpbarcode) from tbl_wbqrcode where wb_mpbarcode='".trim($data[9])."' and wb_rpstrid='".$tid."' ") or die(mysqli_error($link));
					 	$a_mainbarc=mysqli_num_rows($sql_mainbarc);
						if($a_mainbarc > 0)
						{
						 	$str="UPDATE tbl_wbqrcode SET wb_mpgrosswt='".trim($data[8])."' where wb_mpbarcode='".trim($data[9])."' and wb_mpgrosswt!='".trim($data[8])."' and wb_rpstrid='".$tid."'  ";
							//echo $str."<br>";
	...
	//echo $cnt;
		//exit;
		$sqlmainbarc2=mysqli_query($link,"select Distinct (wb_mpbarcode) from tbl_wbqrcode where wb_rpstrid='".$tid."' and wb_mpblinkflg=1 and wb_mpgrosswt>0 ") or die(mysqli_error($link));
