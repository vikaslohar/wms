...
$sss="";
$qr="select distinct lotno from tbl_lot_ldg_pack where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and packtype='$ups' and balqty > 0 $sss ";
...
{
$flg=0; $flg1=0;
$sql_is=mysqli_query($link,"select distinct subbinid from tbl_lot_ldg_pack where plantcode='$plantcode' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotno='".$row2['lotno']."' and packtype='$ups' group by subbinid, binid order by lotdgp_id asc") or die(mysqli_error($link));
$xt=mysqli_num_rows($sql_is);
 while($row_is=mysqli_fetch_array($sql_is))
 { 
$qr1="select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotno='".$row2['lotno']."' and packtype='$ups'  and subbinid='".$row_is['subbinid']."' order by lotdgp_id desc";

$sql_tbl_sub1=mysqli_query($link,$qr1) or die(mysqli_error($link));
while ($row1=mysqli_fetch_array($sql_tbl_sub1))
{
$qr1="select * from tbl_lot_ldg_pack where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotdgp_id='".$row1[0]."' and packtype='$ups' and balqty>0 ";
...
//echo $row['lotno']."-".$row['lotldg_rvflg']."-".$flg."<br>";
