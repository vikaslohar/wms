...
$sss="";
$qr="select distinct(lotldg_lotno) from tbl_lot_ldg where plantcode='$plantcode' lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$stage."' and lotldg_balqty > 0 and lotldg_mergerflg=0 $sss ";
...
$sql_subbin=mysqli_query($link,"select distinct lotldg_subbinid from tbl_lot_ldg where plantcode='$plantcode' and lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_balqty>0 and lotldg_mergerflg=0 order by lotldg_id desc ") or die(mysqli_error($link));
...
//echo "select max(lotldg_id) from tbl_lot_ldg where lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_subbinid='".$row_subbin['lotldg_subbinid']."' and lotldg_mergerflg=0 order by lotldg_id desc ";
 $sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_subbinid='".$row_subbin['lotldg_subbinid']."' and lotldg_mergerflg=0 order by lotldg_id desc ") or die(mysqli_error($link));
   $row_is1=mysqli_fetch_array($sql_is1);
   $sql_istb12=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='$plantcode' and lotldg_id='".$row_is1[0]."' and lotldg_balqty >= 0 and lotldg_mergerflg=0 order by lotldg_id desc") or die(mysqli_error($link)); 	
...
if($row_issuetbl24['lotldg_sstage']=="Raw")
{
	...
	{
		$sql_arrsub=mysqli_query($link,"Select * from tblarrival_sub where plantcode='$plantcode' and lotno='".$row['lotldg_lotno']."'") or die(mysqli_error($link));
