...
$qr="select distinct lotno from tbl_lot_ldg_pack where  plantcode='$plantcode' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and packtype='$ups' $sss  order by lotdgp_id desc";
...
{
	$nsflg=0; 
	$sql_issueg2=mysqli_query($link,"select min(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' and lotno='".$row['lotno']."' and trtype='NSTPNPSLIP'") or die(mysqli_error($link));
	...
	while($row245=mysqli_fetch_array($sql_issuetblg2))
	{	
		if($row245['trtype']!="NSTPNPSLIP")$nsflg++;
	}
	$flg=0; $flg1=0; $a=''; $c=''; $ups=''; $extqty=''; $extnob='';
		...
		while($row_mpns=mysqli_fetch_array($sql_mpns))
		{
			...
			for ($i=0; $i<count($lotarr); $i++)
			{
				if($a==$lotarr[$i] && $ups==$upsarr)
				{
					...
					$qtyns=$qtyns+($ptp*$noparr[$i]); $nompns=$nompn+$ct; 
				}
			}
...
<td align="right"  valign="middle" class="tbltext"><input type="checkbox" name="loc" value="<?php echo $a;?>" onclick="clk(this.value);" />&nbsp;</td>
<td align="left"  valign="middle" class="tbltext" >&nbsp;<?php echo $a;?></td>
</tr>
<?php
}
else
