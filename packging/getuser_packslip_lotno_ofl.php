...
$arrlots="";
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_oflpnpslipsub where plantcode='$plantcode' ") or die(mysqli_error($link));
		...
		$sql_tbl_main=mysqli_query($link,"select * from tbl_oflpnpslipmain where plantcode='$plantcode' and pnpslipmain_tflag='0' and logid!='$logid'") or die(mysqli_error($link));
		$tot_arrmain=mysqli_num_rows($sql_tbl_main);
		if($tot_arrmain > 0)
		{
			while($row_tbl_main=mysqli_fetch_array($sql_tbl_main))
			{
				$sql_tbl_sub=mysqli_query($link,"select * from tbl_oflpnpslipsub where plantcode='$plantcode' and pnpslipmain_id='".$row_tbl_main['pnpslipmain_id']."'") or die(mysqli_error($link));
				$tot_arrsub=mysqli_num_rows($sql_tbl_sub);
				if($tot_arrsub > 0)
				{
					while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
