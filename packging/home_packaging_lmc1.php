...
/*if(!isset($_GET['page'])) { 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_packaging where plantcode='$plantcode' and packaging_type='LMC' and packaging_tdate>='$tdate' and packaging_tdate<='$pdate' and packaging_tflg=1 order by packaging_tcode desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_packaging where plantcode='$plantcode' and packaging_type='LMC' and packaging_tdate>='$tdate' and packaging_tdate<='$pdate' and packaging_tflg=1 order by packaging_tcode desc";
...
$pagination = "";
	if($lastpage > 1)
	{	
		$pagination .= "<div class=\"pagination\" align=\"right\" style=\"width:965px\">";
		//previous button
		if ($page > 1) 
			$pagination.= " <a href=\"$targetpage?page=$prev&sdate=$sdate&edate=$edate\">« previous </a> ";
		else
		...
		//next button
		if ($page < $counter - 1) 
			$pagination.= " <a href=\"$targetpage?page=$next&sdate=$sdate&edate=$edate\"> next »</a> ";
		else
			$pagination.= " <span class=\"disabled\"> next »</span> ";
		$pagination.= "</div>\n";		
	}
	 $srno=($page-1)*$limit+1;
/*$sql_arr_home=mysqli_query($link,"select * from tbl_packaging where packaging_type='LMC' and packaging_tdate>='$tdate' and packaging_tdate<='$pdate' and packaging_tflg=1 order by packaging_tcode desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
//$tot_arr_home=0;

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_packaging where packaging_type='LMC' and packaging_tdate>='$tdate' and packaging_tdate<='$pdate' and packaging_tflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
...
?>
<?php echo $pagination?>
<table align="center" width="965" cellpadding="5" cellspacing="5" border="0" >
<tr >
<td valign="top" align="right"><a href="home_packaging_lmc.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:Pointer;" /></a></td>
</tr>
</table>
