<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
	   // exit;
		...
		$sql="update tbl_psunpp2c set unp_date='$dt', unp_tcode='$tcode', unp_p2ctype='$ptptype', unp_crop='$crop', unp_variety='$variety', unp_lotno='$lotno', unp_orlot='$orlot', unp_newlotno='$txtnewlot', unp_ups='$ups', unp_logid='$logid', unp_yearcode='$yearid_id' where unp_id='$pid'";
		
		if(mysqli_query($link,$sql) or die(mysqli_error($link)))
		{
			$p_id=$pid;
			$s_del="delete from tbl_psunpp2c_sub where unp_id='$p_id'";
			...
			for($i=1; $i<=$sno; $i++)
			{
				...
				$sql_sub="insert into tbl_psunpp2c_sub (unp_id, unp_wh, unp_bin, unp_sbin, unp_onop, unp_oqty, unp_nop, unp_qty, unp_bnop, unp_bqty, plantcode)values('$p_id','$wh2', '$bin2', '$sbin2', '$onob2', '$oqty2', '$rnob2', '$rqty2', '$balnob2', '$balqty2', '$plantcode')";
				mysqli_query($link,$sql_sub) or die(mysqli_error($link));
			}
			$s_del2="delete from tbl_psunpp2c_sub2 where unp_id='$p_id'";
			...
			$sql_sub2="insert into tbl_psunpp2c_sub2 (unp_id, unp_wh, unp_bin, unp_sbin, unp_nop, unp_qty, plantcode)values('$p_id','$wh1', '$bin1', '$sbin1', '$nob1', '$qty1', '$plantcode')";
			...
			if($qty21!="" && $qty21>0)
			{
				$sql_sub21="insert into tbl_psunpp2c_sub2 (unp_id, unp_wh, unp_bin, unp_sbin, unp_nop, unp_qty, plantcode)values('$p_id','$wh21', '$bin21', '$sbin21', '$nob21', '$qty21', '$plantcode')";
...
<title>Packaging - Transaction - Unpacking - P2C</title>
...
function chknop(nopval, sno)
{
	...
	if(parseInt(nopval) > 0)
	{
		...
		document.getElementById(qtys).value=Math.round((parseFloat(nopval)*parseFloat(pt))*1000)/1000;
		document.getElementById(qtys).value=parseFloat(document.getElementById(qtys).value).toFixed(3);
...
<td align="left"  valign="middle" class="tblheading"   >&nbsp;<input type="radio" name="ptptyp" class="tbltext" value="entire" onclick="chklot(this.value);"<?php if($tflg>0) echo "disabled"; else { if($row_tbl['unp_p2ctype']=="entire") echo "checked"; }?> />Entire&nbsp;&nbsp;<input type="radio" name="ptptyp" class="tbltext" value="partial" onclick="chklot(this.value);"  <?php if($row_tbl['unp_p2ctype']=="partial") echo "checked";?>  />Partial&nbsp;<font color="#FF0000">*</font>&nbsp;<input type="hidden" name="ptptype" value="<?php echo $row_tbl['unp_p2ctype'];?>" /></td>
<td align="left"  valign="middle" class="tblheading" colspan="2" id="batchchk">&nbsp;<?php if($row_tbl['unp_p2ctype']=="partial") echo "Batch No. Generated - <font color=red>YES</font>"; else if($row_tbl['unp_p2ctype']=="entire") echo "Batch No. Generated - <font color=red>NO</font>"; else echo ""; ?></td>
...
$sql_sub=mysqli_query($link,"Select * from tbl_psunpp2c_sub where plantcode='$plantcode' and unp_id='$tid' and unp_wh='".$row_issuetbl2['whid']."' and unp_bin='".$row_issuetbl2['binid']."' and unp_sbin='".$row_issuetbl2['subbinid']."'") or die(mysqli_error($link));
...
<tr class="Light" height="20">
	...
	<td width="107" align="center" valign="middle" class="tblheading"><input type="text" name="nop<?php echo $sno;?>" id="nop_<?php echo $sno;?>" value="<?php echo $nop;?>" size="6" maxlength="6" onchange="chknop(this.value,'<?php echo $sno;?>')" <?php if($row_tbl['unp_p2ctype']=="entire") echo "readonly='true' style='background-color:#CCCCCC'";?> /></td>
	<td width="107" align="center" valign="middle" class="tblheading"><input type="text" name="qty<?php echo $sno;?>" id="qty_<?php echo $sno;?>" value="<?php echo $qty;?>" size="6" maxlength="6" onchange="chkqty(this.value,'<?php echo $sno;?>')"  <?php if($row_tbl['unp_p2ctype']=="entire") echo "readonly='true' style='background-color:#CCCCCC'";?> /></td>
