...
session_start();
	...
	if($txtupsdc!="ALL")
	{
		$sqlrr=mysqli_query($link,"select distinct packtype from tbl_lot_ldg_pack where plantcode='$plantcode' and plantcode='$plantcode' and packtype='".$txtupsdc."' and lotldg_crop IN ($cp) and lotldg_trdate>='$sdt' and lotldg_trdate<='$edt' and trtype='PNPSLIP' order by packtype asc") or die(mysqli_error($link));
	}
	else
	{
		$sqlrr=mysqli_query($link,"select distinct packtype from tbl_lot_ldg_pack where plantcode='$plantcode' and plantcode='$plantcode' and lotldg_trdate>='$sdt' and lotldg_trdate<='$edt' and lotldg_crop IN ($cp) and trtype='PNPSLIP' order by packtype Asc") or die(mysqli_error($link));
	}
