<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if(isset($_POST['txtpacklinetype'])) { $txtpacklinetype = $_POST['txtpacklinetype']; }
		if(isset($_POST['txtplinecode'])) { $txtplinecode = $_POST['txtplinecode']; } else { $txtplinecode = ''; }
		if(isset($_POST['txtpacklblnos'])) { $txtpacklblnos = $_POST['txtpacklblnos']; } else { $txtpacklblnos = 0; }
		if(isset($_POST['txtmanualtype'])) { $txtmanualtype = $_POST['txtmanualtype']; }
		
		$zz=str_split($txtpacklblnos);
		$lblchar=$zz[0].$zz[1].$zz[2];
		$lblnm=$zz[3].$zz[4].$zz[5].$zz[6].$zz[7].$zz[8].$zz[9];
		
		
		
		$prdt=date("Y-m-d h:m:s");
		...
		*/
		...
		if($z1 == 0)
		{
		   $sql_main="insert into tbl_pnpslipmain(pnpslipmain_code, pnpslipmain_date, pnpslipsetup_date, pnpslipmain_proslipno,  pnpslipmain_crop, pnpslipmain_variety, pnpslipmain_stage, pnpslipmain_promachcode, pnpslipmain_proopr, pnpslipmain_treattype, pnpslipmain_ttype, logid, yearid, plantcode, pnpslipmain_trtype, pnpslipmain_macopr, pnpslipmain_packlinetype, pnpslipmain_packlinecode, pnpslipmain_manualtype) values ('$txtid','$tdate1','$tdate1','$txtpsrn','$txtcrop','$txtvariety','$txtstage', '$txtpromech', '$txtoprname', '$txttreattyp', '$ttype', '$logid', '$yearid_id', '$plantcode',  'wb', '$txtoprname', '$txtpacklinetype', '$txtplinecode', '$txtmanualtype')";
		if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
		{
		$mainid=mysqli_insert_id($link);
		
		 $sql_sub="insert into tbl_pnpslipsub (pnpslipsub_lotno, pnpslipmain_id, pnpslipsub_onob, pnpslipsub_oqty, pnpslipsub_processtype, pnpslipsub_pnob, pnpslipsub_pqty,  pnpslipsub_connob, pnpslipsub_conqty, pnpslipsub_rm, pnpslipsub_im, pnpslipsub_pl, pnpslipsub_rpl, pnpslipsub_tlqty, pnpslipsub_tlper, pnpslipsub_packtype, pnpslipsub_availpnob, pnpslipsub_availpqty, pnpslipsub_pickpqty, pnpslipsub_packloss, pnpslipsub_packcc, pnpslipsub_packqty, pnpslipsub_balcnob, pnpslipsub_balcqty, pnpslipsub_ups, pnpslipsub_upsid, pnpslipsub_remarks, pnpslipsub_sstatus, pnpslipsub_orlot, pnpslipsub_valperiod, pnpslipsub_valupto, pnpslipsub_qc, pnpslipsub_qcdot, pnpslipsub_qcdttype, pnpslipsub_sltyp, pnpslipsub_plotno, plantcode, pnpslipsub_wtmp, pnpslipsub_wbinmp, pnpslipsub_wbnop, pnpslipsub_wbwt, pnpslipsub_upsmrp, pnpslipsub_gmsmrp, pnpslipsub_pouchmrp, pnpslipsub_gmmrp, pnpslipsub_pouchccqty) values ('$txtlot1', '$mainid', '$txtonob', '$txtoqty', '$protyp', '$pnob', '$pqty', '$txtconnob', '$txtconqty', '$txtconrem', '$txtconim', '$pl', '$rpl', '$txtconloss', '$txtconper', '$pcktype', '$avlnobpck', '$avlqtypck', '$picqtyp', '$pckloss', '$ccloss', '$balpck', '$balcnob', '$balcqty', '$upss', '$upsidno', '$txtremarks', '$softstatus', '$orlot', '$validityperiod', '$tdate3', '$qcstatus', '$tdate4', '$qcdttype', '$slocssyncs24', '$txtplotno', '$plantcode', '$wtmp', '$nowbinmp', '$nopinwb', '$wbwts', '$mrpups', '$mrpgms', '$mrpups', '$mrpgms', '$pchccqty')";
		...
		$p_id=$z1;
		echo "<script>window.location='add_packslipwb_preview.php?p_id=$p_id'</script>";	
	}
...
function Bagschk1(qtyval1,srno)
{
	...
	var extnob="txtextnob"+srno;
	
	/*else*/ 
	document.getElementById('pchccqty').value="";
	...
	if(txtpacktype=="Standard")
	{
		for( var i=1; i<=sno; i++)
		{
			var fetchk="fetchk_"+i;
			document.getElementById(fetchk).checked=false;
		}
	}
	else
	{
	...
	if(qtyval1=="" || qtyval1<=0)
	{
		...
		document.getElementById(actqty).focus();
		return false;
	}
	...
	else
	{
		...
		{
			var fetchk="recqtyp"+i;
			var qqt=0;
			if(document.getElementById(fetchk).value!="")
			{qqt=parseFloat(document.getElementById(fetchk).value);}
			pckqty=parseFloat(pckqty)+parseFloat(qqt);
		}
		...
		*/
		...
		//alert(pckqty);
		var qtyg=parseFloat(pckqty);
		
		document.getElementById('txtbalnobp'+srno).value=
		document.getElementById('txtbalqtyp'+srno).value=parseFloat(document.getElementById('txtextqty'+srno).value)-parseFloat(qtyval1);
		
		
		document.getElementById('picqtyp').value=qtyg;
		document.getElementById('balpck').value=qtyg;
		document.getElementById(actnob).value=document.getElementById(extnob).value;
...
else if(document.frmaddDepartment.txtoprname.value=="")
{
 alert("Please Select Operator Name");
 return false;
}
...
{ 
	...
	if(document.frmaddDepartment.txtpacktype.value=="")
	{
		alert("Please Select Pack Type");
		//document.frmaddDepartment.txttreattyp.focus();
		f=1;
	...
	if(document.frmaddDepartment.txtpacklinetype.value=="")
	{
		alert("Please Select Pack Line Type");
		//document.frmaddDepartment.txttreattyp.focus();
		f=1;
		return false;
	}	
...
{ //alert(pkselval);
	if(document.frmaddDepartment.validityupto.value=="")
	{
		alert("Select Validity Period");
		document.frmaddDepartment.validityperiod.focus();
		document.getElementById('paceptyp1').checked=false;
		document.getElementById('paceptyp2').checked=false;
...
function clk(snoval,upsid)
{
	...
	else
	{
		...
		{
			var upsname="upsname_"+snoval;
			var upname="upname_"+snoval;
			//alert(upname);
			//alert(document.getElementById(upname).value);
			document.frmaddDepartment.upssize.value=snoval;
			document.frmaddDepartment.upsize.value=document.getElementById(upname).value;
		...
		if(document.frmaddDepartment.qcdtyp.value=="DoT")
		{
			if(valval==3)
			{
				...
				else
				{
					...
					{
						alert("Less than or equal 30 days Validity not allowed for Packing");
						document.frmaddDepartment.validityupto.value='';
						document.frmaddDepartment.valdays.value='';
					}
				}
...
function plselect(ptval)
{
	document.frmaddDepartment.txtpacklinetype.value=ptval;
	document.getElementById("postingsubsubtable").innerHTML='';
	document.frmaddDepartment.txtlot1.value='';
	//alert(ptval);
	if(ptval=="Manual")
	{
		...
		//document.getElementById('lblnums').innerHTML='&nbsp;<input type="hidden" name="txtpacklblnos" id="txtpacklblnos" value="" >';
		//document.frmaddDepartment.txtplinecode.value='';
		//document.frmaddDepartment.txtplinecode.selectedIndex=0;
		//document.frmaddDepartment.txtpacklblnos.value='';
	}
	else
	{
...
function mlselect(ptval)
{
	document.frmaddDepartment.txtmanualtype.value=ptval;
	document.getElementById("postingsubsubtable").innerHTML='';
	document.frmaddDepartment.txtlot1.value='';
}

function psschk(plval)
{
	document.getElementById('lblnums').innerHTML='&nbsp;<input type="hidden" name="txtpacklblnos" id="txtpacklblnos" value="" >';
	document.frmaddDepartment.txtpacklblnos.value='';
	showUser(plval,'lblnums','lblgen',plval,'','','','');
}
...
function updmerg(upval, snoval)
{
	...
	else
	{
		...
		document.getElementById(nopc).value=Math.floor(parseFloat(document.getElementById(nopc).value));
		document.getElementById(nopc).value=document.getElementById(nopc).value.toFixed(3);
		var nps=document.getElementById(nopc).value.split(".");
...
<tr class="tblsubtitle" height="20">
  <td colspan="8" align="center" class="tblheading">Add Packing Slip (Window Box) </td>
</tr>
...
<td width="319" align="left" valign="middle" class="smalltbltext">&nbsp;<input name="dopc" id="dopc" type="text" size="10" class="smalltbltext" tabindex="0" value="<?php echo date("d-m-Y");?>" readonly="true"  style="background-color:#CCCCCC" maxlength="10"/>&nbsp;<!--<a href="javascript:void(0)" onClick="datesel('dopc')" tabindex="6"><img src="../images/cal.gif" border="0" align="absmiddle" /></a>-->&nbsp;<font color="#FF0000">*</font></td>
...
<table align="center" border="1" width="970" cellspacing="0" cellpadding="0" bordercolor="#1dbe03" style="border-collapse:collapse" > 
  ...
  <tr class="Light" height="30">
 <?php
$quer3=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop  where croptype='Vegetable Crop' order by cropname Asc"); 
?>
...
<td width="165"  align="left" valign="middle" class="smalltbltext"  >&nbsp;Condition <input type="hidden" name="txtstage" value="Condition" /></td>
           </tr>
<?php
...
<tr class="Light" height="30">
<td align="right" width="151" valign="middle" class="smalltblheading" style="border-color:#1dbe03">&nbsp;Pack Type&nbsp;</td>
<td align="left" width="276" valign="middle" class="smalltbltext" style="border-color:#1dbe03">&nbsp;<input name="txtpktype" type="radio" class="smalltbltext" tabindex="" value="Standard" onclick="ptselect(this.value)" />&nbsp;Standard&nbsp;<input name="txtpktype" type="radio" class="smalltbltext" tabindex="" value="Non-standard" onclick="ptselect(this.value)" />&nbsp;Non-standard<font color="#FF0000">*</font>&nbsp;<input type="hidden" name="txtpacktype" value="" /></td>
<td align="right" width="210" valign="middle" class="smalltblheading" style="border-color:#1dbe03">&nbsp;Pack Line Type&nbsp;</td>
<td align="left" width="323" valign="middle" class="smalltbltext" style="border-color:#1dbe03">&nbsp;<input name="txtpltype" type="radio" class="smalltbltext" tabindex="" value="Manual" onclick="plselect(this.value)" />&nbsp;Manual&nbsp;<input name="txtpltype" type="radio" class="smalltbltext" tabindex="" value="Online" onclick="plselect(this.value)" />&nbsp;Online<font color="#FF0000">*</font>&nbsp;<input type="hidden" name="txtpacklinetype" value="" /></td>
</tr>
</table>
<div id="manualtable" style="display:none"> 
<table align="center" border="1" width="970" cellspacing="0" cellpadding="0" bordercolor="#1dbe03"  > 
<?php
$sql_sel="select * from tbl_rm_packmac WHERE plantcode='$plantcode' and packlinetype='Manual' order by packlinecode";
$res=mysqli_query($link,$sql_sel) or die (mysqli_error($link));
$total=mysqli_num_rows($res);
?>  
<tr class="Light" height="30">
<td align="right" width="150" valign="middle" class="smalltblheading" style="border-color:#1dbe03">&nbsp;Manual Type&nbsp;</td>
<td align="left" width="169" valign="middle" class="smalltbltext" style="border-color:#1dbe03">&nbsp;<input name="txtmltype" type="radio" class="smalltbltext" tabindex="" value="Manual" onclick="mlselect(this.value)" />&nbsp;Manual&nbsp;<input name="txtmltype" type="radio" class="smalltbltext" tabindex="" value="Machine" onclick="mlselect(this.value)" />&nbsp;Machine<font color="#FF0000">*</font>&nbsp;<input type="hidden" name="txtmanualtype" value="" /></td>
<!--<td align="right" width="106" valign="middle" class="smalltblheading" style="border-color:#1dbe03">&nbsp;Select Packline Code&nbsp;</td>
<td align="left" width="210" valign="middle" class="smalltbltext" style="border-color:#1dbe03">&nbsp;<select class="smalltbltext" name="txtplinecode" style="width:60px;" onChange="psschk(this.value);">
    <option value="" selected>Select</option>
 <?php while($noticia_item = mysqli_fetch_array($res)) { ?>
		<option value="<?php echo $noticia_item['packlinecode'];?>" />   
		<?php echo $noticia_item['packlinecode'];?>
		<?php } ?></select>&nbsp;<font color="#FF0000">*</font></td>
<td align="right" width="157" valign="middle" class="smalltblheading" style="border-color:#1dbe03">&nbsp;Start Label Number&nbsp;</td>
<td align="left" width="164" valign="middle" class="smalltbltext" style="border-color:#1dbe03" id="lblnums">&nbsp;<input type="hidden" name="txtpacklblnos" id="txtpacklblnos" value="" ></td>-->
</tr>
</table>
</div>
<table align="center" border="1" width="970" cellspacing="0" cellpadding="0" bordercolor="#1dbe03"  > 
<tr class="Light" height="30">
<td align="right" width="149" valign="middle" class="smalltblheading" style="border-color:#1dbe03">&nbsp;Lot No.&nbsp;</td>
<td align="left" width="330" valign="middle" class="smalltbltext" style="border-color:#1dbe03">&nbsp;<input name="txtlot1" type="text" size="20" class="smalltbltext" tabindex=""  maxlength="20"  value="" readonly="true" style="background-color:#CCCCCC" >&nbsp;<font color="#FF0000">*</font>&nbsp;<a href="Javascript:void(0);" onclick="openslocpop();">Select</a></td>
<td width="483" colspan="2" align="left" valign="middle" class="smalltblheading" style="border-color:#1dbe03">&nbsp;<a href="javascript:void(0);" onclick="getdetails();" >Get details</a></td>
</tr>
</table>
