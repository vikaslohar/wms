<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		while($row_arr=mysqli_fetch_array($sql_arr))
		{
			...
			$sql_arrsub=mysqli_query($link,"select * from tbl_rpspackaging_sub where plantcode='$plantcode' and packaging_id='".$pid."'") or die(mysqli_error($link));
			$a_arrsub=mysqli_num_rows($sql_arrsub);
			while($row_arrsub=mysqli_fetch_array($sql_arrsub))
			{
				
				
				$crop=$row_arr['packaging_crop'];
				$variety=$row_arr['packaging_variety'];
				$lotno=$row_arr['packaging_lotno'];
				$arrival_date=$row_arr['packaging_tdate'];
				$drefno=$row_arr['packaging_slipno'];
				$lotstage="Pack";
				$packtype=$row_arr['packaging_upssize'];
				...
				$sql_barcode2=mysqli_query($link,"Select distinct wb_mpbarcode from tbl_wbqrcode where wb_lotno='".$row_arr['packaging_lotno']."' and wb_rpstrid='".$pid."'") or die(mysqli_error($link));
				$tot_barcode2=mysqli_num_rows($sql_barcode2);
				if($tot_barcode2 > 0)
				{
					while($row_barcode2=mysqli_fetch_array($sql_barcode2))
					{
						if($row_barcode2['wb_mpbarcode']!='')
						{
							$sql_barcode1=mysqli_query($link,"select * from tbl_wbqrcode where wb_lotno='".$row_arr['packaging_lotno']."' and wb_rpstrid='".$pid."' and  wb_mpbarcode='".$row_barcode2['wb_mpbarcode']."'") or die(mysqli_error($link));
							$row_barcode1=mysqli_fetch_array($sql_barcode1); 
							
							
							$barcode=$row_barcode2['wb_mpbarcode'];
							$grosswt=$row_barcode1['wb_mpgrosswt'];
							$dt=$row_barcode1['wb_linkdate'];
							//$tim=$row_barcode2['bar_wttime'];
							$poprid='';
							$bar_crop=$row_arr['packaging_crop'];
							$bar_variety=$row_arr['packaging_variety'];
							$bar_ups=$row_arr['packaging_upssize'];
							$bar_dop=$row_arrsub['packagingsub_dop'];
							$bar_vdate=$row_barcode1['wb_linkdate'];
							$bar_netweight=$row_arrsub['packagingsub_wtmp'];
							if($a!="")
							$a=$a.",".$barcode;
							else
							$a=$barcode;
				...
				$sql_arr_subsub3=mysqli_query($link,"select * from tbl_rpspackagingsub_sub where plantcode='$plantcode' and packaging_id='".$pid."' and packagingsub_id='".$row_arrsub['packagingsub_id']."'") or die(mysqli_error($link));
				$a_sub_3=mysqli_num_rows($sql_arr_subsub3);
				if($a_sub_3 > 0)
				{
					while($row_arr_subsub3=mysqli_fetch_array($sql_arr_subsub3))
					{
					
						$nop12=$row_arr_subsub3['packagingsubsub_nopch'];
						$nomp2=$row_arr_subsub3['packagingsubsub_nomp'];
						$qty12=$row_arr_subsub3['packagingsubsub_totqty'];
						$balnop2=$row_arr_subsub3['packagingsubsub_nopch'];
						$balnomp2=$opnomp2+$row_arr_subsub3['packagingsubsub_nomp'];
						$balqty2=$row_arr_subsub3['packagingsubsub_totqty'];
						$whid2=$row_arr_subsub3['packagingsubsub_wh'];
						$binid2=$row_arr_subsub3['packagingsubsub_bin'];
						$subbinid2=$row_arr_subsub3['packagingsubsub_subbin'];
						$sstage2="Pack";
						$ltnop=$packnob-$packnop;
						$packnomp=$row_arr_subsub3['packagingsubsub_nomp'];
						$packnob=$row_arrsub['packagingsub_extnop'];
						$packqty=$row_arrsub['packagingsub_extqty'];
						
						$packnop=$row_arr_subsub3['packagingsubsub_nopch'];
						
						//$zxcv=$packnob-$packnop;
						$zxcv=$row_arr_subsub3['packagingsubsub_totpch'];
					...
					foreach($p1array as $pval1)
					{
						...
						if($pval1<>"")
						{
							...
							while($row_issuetbl22=mysqli_fetch_array($sql_issuetbl22))
							{
								...
								$sql_arrsubsub3=mysqli_query($link,"select * from tbl_rpspackagingsub_sub where plantcode='$plantcode' and packaging_id='".$pid."' and packagingsub_id='".$row_arrsub['packagingsub_id']."'") or die(mysqli_error($link));
		...
		$sql_code1="SELECT MAX(packaging_tcode) FROM tbl_rpspackaging  ORDER BY packaging_tcode DESC";
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Packing slip (Window Box) - Preview </td>
	    </tr></table></td>
...
<tr class="tblsubtitle" height="20">
	...
	<td width="7%" align="center" valign="middle" class="smalltblheading">NoP in WB</td>
	<td width="6%" align="center" valign="middle" class="smalltblheading">WBWT</td>
	<td width="5%" align="center" valign="middle" class="smalltblheading">NoWB in MP</td>
	<td width="5%" align="center" valign="middle" class="smalltblheading">MPWT</td>
	<td width="7%" align="center" valign="middle" class="smalltblheading">NoMP</td>
	<td width="7%" align="center" valign="middle" class="smalltblheading">Loose WB</td>
	<td width="7%" align="center" valign="middle" class="smalltblheading">Loose Pouches</td>
...
$nopinwb=$row_tbl_sub['packagingsub_wbnop']; 
$wbwt=$row_tbl_sub['packagingsub_wbwt']; 
$nowbinmp=$row_tbl_sub['packagingsub_wbinmp']; 
$wtmp=$row_tbl_sub['packagingsub_wtmp']; 


//$arrival_id=$row_tbl_sub['trid'];
$difq="";$difq1=""; $sloc=""; $sloc1=""; $cnt++;  $nomp=0; $losewb=0; $loosepch=0;
...
while($row_tbl_subsub=mysqli_fetch_array($sql_tbl_subsub))
{
	...
	$losewb=$row_tbl_subsub['packagingsubsub_loosewb'];  
	$loosepch=$row_tbl_subsub['packagingsubsub_nopch']; 
...
<tr class="Light" height="20">
	...
	<td align="center" valign="middle" class="smalltbltext"><?php echo $nopinwb;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $wbwt;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $nowbinmp;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $wtmp;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $nomp;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $losewb;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $loosepch;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php if($row_tbl['packaging_remarks']!=""){ ?><a href="Javascript:void(0)" title="<?php echo $row_tbl['packaging_remarks'];?>" onmouseover="<?php echo $row_tbl['packaging_remarks'];?>">Details</a><?php } ?></td>
	</tr>
  <?php
}
else
