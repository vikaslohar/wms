...
<?php	
	...
	$sqlarrhome2=mysqli_query($link,"select distinct(oldlot) from tbl_qctest where testdate='$sdate' and trstage='Pack' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
...
$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where testdate='$sdate' and trstage='Pack' and oldlot='".$rowarrhome['oldlot']."' order by crop asc, variety asc, oldlot asc, tid desc LIMIT 0,1") or die(mysqli_error($link));
...
$sql_arr_home2=mysqli_query($link,"select MAX(tid) from tbl_qctest where  sampleno='".$row_arr_home2['sampleno']."' and trstage='Pack' and testdate='$sdate' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
	$row_arr_home3=mysqli_fetch_array($sql_arr_home2);

$sql_arr_home3=mysqli_query($link,"select * from tbl_qctest where  tid='".$row_arr_home3[0]."' and sampleno='".$row_arr_home2['sampleno']."' and trstage='Pack' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
