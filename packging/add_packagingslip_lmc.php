<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
			...
			if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
			{
				...
				for($i=1; $i<=$sno; $i++)
				{
					...
					for($x=1; $x<=$ssno33; $x++)
					{
						...
						$extqtyhs24=trim($_POST["extqtyhs".$i."_".$x]);
						...
						if($nophs24>0)
						{
							$sql_subsub="insert into tbl_packagingsub_sub (packagingsub_id, packaging_id, packagingsubsub_lotno, packagingsubsub_upssize, extwh, extbin, extsubbin, packagingsubsub_extnop, packagingsubsub_extqty, packagingsubsub_nop, packagingsubsub_balpch, packagingsubsub_barcodes, packagingsubsub_remarks, plantcode) values ('$suid', '$mainid', '$ltno24', '$upssize', '$exwh24', '$exbin24', '$exsubbin24', '$extnophs24', '$extqtyhs24', '$nophs24', '$balnophs24', '$detmpbno24', '$txtremarks24', '$plantcode')";
							mysqli_query($link,$sql_subsub) or die(mysqli_error($link));
						}
					}	
				}	
				$sql_barcode="insert into tbl_barcodestmp (bar_tid, bar_subid, bar_barcodes, bar_lotno, bar_logid, bar_psrn, bar_grosswt, bar_wtdate, plantcode) values ('$mainid', '$suid', '$barcode', '$lotnno', '$logid', '$txtpsrn', '$weight', '$tdate2', '$plantcode')";
		...
		//exit;
		$p_id=$mainid;
		echo "<script>window.location='add_packagingsliplmc_preview.php?p_id=$p_id'</script>";	
		}
	}
...
{	
	...
	{	
		...
		alert(a);
		//showUser(a,'postingtable','mformlmc','','','','','');
...
{	
	...
	{	
		...
		var a=formPost(document.getElementById('mainform'));
		//alert(a);
		showUser(a,'postingtable','mformsubedtlmc','','','','','');
		}
	}
}
...
function openslocpop()
{
	if(document.frmaddDepartment.mptyp.value!="")
	{
		...
		//alert(variety);
		winHandle=window.open('getuser_packagingsliplmc_lotno.php?crop='+crop+'&variety='+variety+'&stage='+stage+'&tid='+tid+'&dop='+dop+'&ups='+ups,'WelCome','top=170,left=180,width=420,height=350,scrollbars=yes');
		if(winHandle==null){
...
{
//alert(edtrecid);
showUser(edtrecid,'postingsubtable','subformedtlmc','','','','','');
}
function getdetails(stage)
{
	...
	showUser(get,'postingsubsubtable','getlmc',crop,variety,tid,lotid,dsrn,stage);
...
function deleterec(v1,v2)
{
	...
	{
		showUser(v1,'postingtable','deletelmc',v2,'','','','');
	}
	else
	{
		return false;
...
function balnopcheck(balval, val12)
{
	if(parseInt(balval) > 0)
	{
		...
		if(parseFloat(nop) >= parseFloat(document.getElementById('wtmp_'+[val12]).value))
		{
...
function balqtcheck(balval, val12)
{
	if(parseFloat(balval) > 0)
	{
		...
		if(parseFloat(balval) >= parseFloat(document.getElementById('wtmp_'+[val12]).value))
		{
			alert("Qty cannot be greater than Master Pack Size");
			document.getElementById('pqty_'+[val12]).value="";
...
function pacpchk(pacpcval, snoval, sno33val)
{
	...
	document.getElementById(qtyphs).value=parseFloat(nop);
	...
	{
		var nophs2="nophs"+snoval+"_"+i;
		var tnps=document.getElementById(nophs2).value;
		if(tnps=="")tnps=0;
		//alert(tnps);
		tonop=parseInt(tonop)+parseInt(tnps);
		//alert(tonop);
	}
	//alert(document.getElementById(nophs).value);alert(document.getElementById(nomp).value);alert(tonop);
	if(parseInt(tonop)==0)
	{
...
function pacqtychk(pacpcval, snoval, sno33val)
{
	...
	//alert(document.getElementById(sno33).value);
	for (var i=1; i<=document.getElementById(sno33).value; i++)
	{
	...
	//alert(parseInt(tonop));alert(parseInt(document.getElementById(nomp).value));
	if(parseInt(tonop)>parseInt(document.getElementById(nomp).value))
	{
		alert("Total of NoP from SLOC cannot be more than NoP");
		document.getElementById(nophs).value="";
...
{ 	
	if(whno==1)
	{
		...
		//alert(tnop);alert(document.getElementById("wtnop_1").value);
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Packaging slip - LMC&nbsp;<input type="hidden" name="logid" value="<?php echo $logid?>" /></td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
<tr >
<td valign="top" align="right"><a href="home_packaging_lmc.php" tabindex="20"><img src="../images/cancel.gif" border="0"style="display:inline;cursor:Pointer;" onclick="return confirm('Do You wish to Cancel this Transaction?');" /></a>&nbsp;&nbsp;<input name="Submit" type="image" src="../images/preview.gif" alt="Submit Value"  border="0" style="display:inline;cursor:Pointer;" tabindex="" onClick="return mySubmit();">&nbsp;&nbsp;</td>
</tr>
</table>
