...
<title>PSW-Report Daily QC Result Report</title>
...
winHandle=window.open('daily_qc_report2.php?sdate='+sdate,'WelCome','top=20,left=80,width=1000,height=900,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   ...
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="813" height="30">&nbsp;Daily QC Result Report</td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
<?php	
	...
	$sqlarrhome2=mysqli_query($link,"select distinct(oldlot) from tbl_qctest where plantcode='$plantcode' and plantcode='$plantcode' and testdate='$sdate' and trstage='Pack' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
	$totarrhome2=mysqli_num_rows($sqlarrhome2);
?>
...
<table align="center" border="0" cellspacing="0" cellpadding="0" width="850" style="border-collapse:collapse">
   <!--	<tr height="25" >
    <td align="center" class="subheading" style="color:#303918; ">Lot Destination Report:</td>
  	...
  	<tr height="25">
    <td align="center" class="subheading" style="color:#303918; " colspan="3">Result Updated on: <?php echo $_GET['sdate'];?></td>
  	</tr>
  	<!--<tr height="25" >
...
if($totarrhome2 > 0)
{
while($rowarrhome=mysqli_fetch_array($sqlarrhome2))
{

$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where plantcode='$plantcode' and testdate='$sdate' and trstage='Pack' and oldlot='".$rowarrhome['oldlot']."' order by crop asc, variety asc, oldlot asc, tid desc LIMIT 0,1") or die(mysqli_error($link));
...
while($row_arr_home2=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_arr_home2=mysqli_query($link,"select MAX(tid) from tbl_qctest where plantcode='$plantcode' and sampleno='".$row_arr_home2['sampleno']."' and oldlot='".$rowarrhome['oldlot']."' and trstage='Pack' and testdate='$sdate' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
	$row_arr_home3=mysqli_fetch_array($sql_arr_home2);

$sql_arr_home3=mysqli_query($link,"select * from tbl_qctest where plantcode='$plantcode' and tid='".$row_arr_home3[0]."' and sampleno='".$row_arr_home2['sampleno']."' and trstage='Pack' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
...
{		
	...
	$sql_tbl_sub1=mysqli_query($link,"select distinct subbinid, lotldg_variety, lotldg_crop, whid, binid from tbl_lot_ldg_pack where plantcode='$plantcode' and lotno='".$row_arr_home['lotno']."' group by subbinid, lotldg_variety, lotno order by subbinid") or die(mysqli_error($link));
	$t=mysqli_num_rows($sql_tbl_sub1);
	while($row_tbl=mysqli_fetch_array($sql_tbl_sub1))
	{

$sql_tbl1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' and subbinid='".$row_tbl['subbinid']."' and lotno='".$row_arr_home['lotno']."'") or die(mysqli_error($link));  
$row_tbl1=mysqli_fetch_array($sql_tbl1);

$sql1=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='$plantcode' and lotdgp_id='".$row_tbl1[0]."' ")or die(mysqli_error($link));
...
<tr >
<td height="49" align="center" valign="top"><a href="daily_qc_report.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" />
