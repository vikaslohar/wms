<?php
	...
	//echo "Select * from tbl_barcodestmp where bar_lotno='".$lotno."' and bar_tid='".$tid."' and bar_subid='".$subtid."' and bar_logid='".$logid."' and bar_psrn='".$txtpsrn."'";
	$sql_barcode24=mysqli_query($link,"Select * from tbl_barcodestmp where bar_lotno='".$lotno."' and bar_tid='".$tid."' and bar_subid='".$subtid."' and bar_logid='".$logid."' and bar_psrn='".$txtpsrn."'") or die(mysqli_error($link));
echo	$tot_barcode24=mysqli_num_rows($sql_barcode24);
...
if($abrc!="")
{
	$sql_barcode=mysqli_query($link,"Select bar_barcode from tbl_barcodes where plantcode='$plantcode' and bar_barcode IN ($abrc)") or die(mysqli_error($link));
  ...
  <form id="mainform" name="from" method="post" action="<?php $_SERVER['PHP_SELF']; ?>" onSubmit="post_value();"   > 
	 ...
	 <input type="hidden" name="baryrcode" value="<?php echo $baryrcode;?>" />
	  ...
	  <input type="hidden" name="newbars" value="" />
	   ...
	   <input type="hidden" name="totbtsls" value="<?php echo $totbtsls;?>" />
