<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		else if($beseltyp=="onlineentrybased")
		{
			echo "<script>window.location='getuser_pronpslip_barcode4.php?totnomp=$totnomp&tid=$tid&lotno=$lotno&txtpsrn=$txtpsrn&subtid=$subtid&dval=$dval&dobg=$dobg&operatorcode=$operatorcode&wtmaccode=$wtmaccode'</script>";
		}
		else
		{
...
<tr class="Light" height="25">
<td width="50%" align="right" height="30" valign="middle" class="tblheading">Online Barcode Scanning Entry&nbsp;</td>
<td width="50%" align="left"  valign="middle">&nbsp;<input type="radio" name="besel" value="onlineentrybased" onClick="seltp(this.value);" /></td>
</tr>
<tr class="light" height="25">
...
<?php
$query=mysqli_query($link,"SELECT * FROM tbl_rm_wtopr") or die("Error: " . mysqli_error($link));
$numofrecords=mysqli_num_rows($query);


$query2=mysqli_query($link,"SELECT * FROM tbl_rm_wtmac ") or die("Error: " . mysqli_error($link));
