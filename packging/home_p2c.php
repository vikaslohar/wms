<?php
	...
	$sql_tbl=mysqli_query($link,"select * from tbl_psunpp2c where plantcode='$plantcode' and unp_logid='".$logid."' and unp_tflg=0") or die(mysqli_error($link));
	while($row_tbl=mysqli_fetch_array($sql_tbl))
	{
		$arrival_id=$row_tbl['unp_id'];	
		
		$s_sub="delete from tbl_psunpp2c_sub where unp_id='".$arrival_id."'";
		mysqli_query($link,$s_sub) or die(mysqli_error($link));
		
		$s_sub_sub="delete from tbl_psunpp2c_sub2 where unp_id='".$arrival_id."'";
		mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	}
	
	$s_sub="delete from tbl_psunpp2c where unp_logid='".$logid."' and unp_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
<title>Packaging - Transaction - Pack Seed Unpacking - Pack to Condition (P2C) - Home</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Pack Seed Unpacking - Pack to Condition (P2C)</td>
	    </tr></table></td>
...
<!--<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#1dbe03" style="border-collapse:collapse">
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_psunpp2c where plantcode='$plantcode' and unp_tflg=1 order by unp_id desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_psunpp2c where plantcode='$plantcode' and unp_tflg=1 order by unp_id desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_psunpp2c where unp_tflg=1 order by unp_id desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
//$tot_arr_home=0;

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_psunpp2c where unp_tflg=1 order by unp_id desc");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_sub2=mysqli_query($link,"Select * from tbl_psunpp2c_sub2 where plantcode='$plantcode' and unp_id='$arrival_id'") or die(mysqli_error($link));
	...
	if($tot_sub2 > 0)
	{
		...
		$nob=$row_sub2['unp_nop']; 
		$qty=$row_sub2['unp_qty'];
	
		$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='$plantcode' and whid='".$row_sub2['unp_wh']."' order by perticulars") or die(mysqli_error($link));
		$row_whouse=mysqli_fetch_array($sql_whouse);
		$wareh=$row_whouse['perticulars'];
		
		$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='$plantcode' and binid='".$row_sub2['unp_bin']."' and whid='".$row_sub2['unp_wh']."'") or die(mysqli_error($link));
		$row_binn=mysqli_fetch_array($sql_binn);
		$binn=$row_binn['binname'];
		
		$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='$plantcode' and sid='".$row_sub2['unp_sbin']."' and binid='".$row_sub2['unp_bin']."' and whid='".$row_sub2['unp_wh']."'") or die(mysqli_error($link));
	...
	$lotno=$row_arr_home['unp_newlotno'];
