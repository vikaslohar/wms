<?php
	...
	//exit;
	 ...
	 $sql_main="update tbl_rpspackaging set packaging_tflg=2 where packaging_id='$pid'";
...
<title>Packaging -Transaction - Re-Printing Packaging slip- Preview</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Re-Printing Packaging slip - Preview </td>
	    </tr></table></td>
...
$arrival_id=$row_tbl['packaging_id'];
	...
	$tdate2=$row_tbl['packaging_indentdate'];
...
<tr class="tblsubtitle" height="20">
  <td colspan="8" align="center" class="tblheading">Re-Printing Packaging slip</td>
</tr>
...
?>
 ...
 <?php
$sql_sel1="select * from tbl_rm_promac where plantcode='$plantcode' and promac_id='".$row_tbl['promachcode']."' order by promac_type";
...
$query_popr=mysqli_query($link,"SELECT * FROM tbl_rm_proopr where proopr_id='".$row_tbl['proopr']."'") or die("Error: " . mysqli_error($link));
...
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='$plantcode' and whid='".$row_tbl_sub['packagingsub_extwh']."' order by perticulars") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);
$wareh=$row_whouse['perticulars']."/";

$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='$plantcode' and binid='".$row_tbl_sub['packagingsub_extbin']."' and whid='".$row_tbl_sub['packagingsub_extwh']."'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname']."/";

$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='$plantcode' and sid='".$row_tbl_sub['packagingsub_extsubbin']."' and binid='".$row_tbl_sub['packagingsub_extbin']."' and whid='".$row_tbl_sub['packagingsub_extwh']."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
$subbinn=$row_subbinn['sname'];

$nomp=$row_tbl_sub['packagingsub_nomp']; 
$nop=$row_tbl_sub['packagingsub_nop']; 
$totp=$row_tbl_sub['packagingsub_nop']; 

$diq=explode(".",$row_tbl_sub['packagingsub_extqty']);
if($diq[1]==000){$totqty=$diq[0];}else{$totqty=$row_tbl_sub['packagingsub_extqty'];}
if($totqty > 0)
{
if($sloc!=""){
...
<td width="656" align="left"  valign="middle" class="smalltbltext">&nbsp;<?php echo $remarks;?></td>
</tr>
</table>
