...
if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
{
	...
	for ($i=1; $i<=$srno2; $i++)
	{
		...
		if(isset($_GET[$extslwhgx])) { $extslwhg1 = $_GET[$extslwhgx]; }
		if(isset($_GET[$extslbingx])) { $extslbing1 = $_GET[$extslbingx]; }
		if(isset($_GET[$extslsubbgx])) { $extslsubbg1 = $_GET[$extslsubbgx]; }
		if(isset($_GET[$txtextnobx])) { $txtextnob1 = $_GET[$txtextnobx]; }
		if(isset($_GET[$txtextqtyx])) { $txtextqty1 = $_GET[$txtextqtyx]; }
		if(isset($_GET[$recnobpx])) { $recnobp1 = $_GET[$recnobpx]; }
		if(isset($_GET[$recqtypx])) { $recqtyp1 = $_GET[$recqtypx]; }
		if(isset($_GET[$txtbalnobpx])) { $txtbalnobp1 = $_GET[$txtbalnobpx]; }
		if(isset($_GET[$txtbalqtypx])) { $txtbalqtyp1 = $_GET[$txtbalqtypx]; }
	...
	$sql_pnpsubbal="update tbl_pnpslipsub set pnpslipsub_bnob='$tbaln', pnpslipsub_bqty='$tbalq' where pnpslipsub_id='$suid' ";
...
$tid=$z1;
	...
	$sql_tbl=mysqli_query($link,"select * from tbl_pnpslipmain where pnpslipmain_id='".$tid."'") or die(mysqli_error($link));
...
<?php
$arrival_id;
$sql_tbl_sub=mysqli_query($link,"select * from tbl_pnpslipsub where  pnpslipmain_id='".$arrival_id."'") or die(mysqli_error($link));
...
$difq="";$difq1="";
$sloc=""; $sloc1=""; $cnt++; 
$sql_tbl_subsub=mysqli_query($link,"select * from tbl_pnpslipsubsub2 where pnpslipsub_id='".$row_tbl_sub['pnpslipsub_id']."' and pnpslipmain_id='".$arrival_id."' order by pnpslipsubsub_id asc") or die(mysqli_error($link));
