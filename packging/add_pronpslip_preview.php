<?php
	...
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		...
		$sql_barcode3=mysqli_query($link,"Select distinct pnpslipbar_subbinid from plantcode='$plantcode' and tbl_pnpslipbarcode where pnpslipbar_lotno='".$packlotno."' and pnpslipmain_id='".$pid."'") or die(mysqli_error($link));
		...
		if($tot_barcode3 > 0)
		{
			while($row_barcode3=mysqli_fetch_array($sql_barcode3))
			{
				...
				if($tot_barcode23 > 0)
				{
					...
					$sql_ups=mysqli_query($link,"select * from tblups where plantcode='$plantcode' and CONCAT(ups,' ',wt)='".$bar_ups."'") or die(mysqli_error($link));
		...
		while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
		{
			...
			$sql_issueg=mysqli_query($link,"select distinct lotldg_lotno from plantcode='$plantcode' and tbl_lot_ldg where lotldg_subbinid='".$subbinid."'") or die(mysqli_error($link));
		...
		while($row_arrsubsub2=mysqli_fetch_array($sql_arrsubsub2))
		{
			...
			while($row_issuetbl22=mysqli_fetch_array($sql_issuetbl22))
			{
				...
				else*/
				{
					...
					else if($got=="GOT-NR UT")
					{
						$got1="UT";
					}
					else if($got=="GOT-R UT")
					{
						$got1="UT";
					}
					else if($got=="GOT-NR UT")
		...
		while($row_arrsubsub3=mysqli_fetch_array($sql_arrsubsub3))
		{
			...
			foreach($b_arr as $bval1)
			{
				if($bval1<>"")
				{
					...
					if($balqty3>0)
					{
						$sql_mpmain=mysqli_query($link,"select mpmain_barcode from tbl_mpmain where plantcode='$plantcode' and mpmain_barcode='".$bval1."'")or die(mysqli_error($link));
