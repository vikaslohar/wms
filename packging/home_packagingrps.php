...
$sql_tbl=mysqli_query($link,"select * from tbl_rpspackaging where plantcode='$plantcode' and packaging_logid='".$logid."' and packaging_tflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['packaging_id'];	
	
	$s_sub="delete from tbl_rpspackaging_sub where packaging_id='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_rpspackagingsub_sub where packaging_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
}
	
	$s_sub="delete from tbl_rpspackaging where packaging_logid='".$logid."' and packaging_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	$s_sub="delete from tbl_btslmain where plantcode='$plantcode' and btsl_trtyp='packaging' and btsl_logid='$logid' and btsl_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));		
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_packagingrps1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
...
<title>Packaging - Transaction -Re-Printing Packaging slip - Home</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Re-Printing Packaging slip</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_packagingslip_rps.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
</table></td>
...
<!--<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#1dbe03" style="border-collapse:collapse">
  ...
  <?php
  $sql_arr_home=mysqli_query($link,"select * from tbl_rpspackaging where plantcode='$plantcode' and packaging_type='SMC' and packaging_tflg=2 order by packaging_tcode desc ") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Re-Printing Packaging slip - Pending List</td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="970" bordercolor="#1dbe03" style="border-collapse:collapse">
			...
			<td width="76" align="center" valign="middle" class="smalltblheading">Transaction Id</td>
            <td width="60" align="center" valign="middle" class="smalltblheading">Date</td>
            ...
            <td width="105"  align="center" valign="middle" class="smalltblheading">Lot No.</td>
			...
			<td width="11%" align="center" valign="middle" class="smalltblheading">No. of Pchs</td>
			<td width="35" align="center" valign="middle" class="smalltblheading">No. of MP</td>
			<td width="53" align="center" valign="middle" class="smalltblheading">Balance Pchs</td>
			<td width="58" align="center" valign="middle" class="smalltblheading">No. of Barcodes Attached</td>
           </tr>
...
{
	$trdate=$row_arr_home['packaging_tdate'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;
	
	$lrole=$row_arr_home['packaging_logid'];
		
	$arrival_id=$row_arr_home['packaging_id'];
	
	$sql_tblsub=mysqli_query($link,"select * from tbl_rpspackaging_sub where plantcode='$plantcode' and packaging_id='".$arrival_id."'") or die(mysqli_error($link));
	while($row_tblsub=mysqli_fetch_array($sql_tblsub))
	{
	
	$quer3=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropid='".$row_arr_home['packaging_crop']."' order by cropname Asc");
	$noticia = mysqli_fetch_array($quer3);
	$crop=$noticia['cropname'];
	
	$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$row_arr_home['packaging_variety']."'  order by popularname Asc"); 
	$noticia_item = mysqli_fetch_array($quer4);
	$variety=$noticia_item['popularname'];
	//echo $arrival_id;
	$lotno=""; $bags1=""; $qty1=""; $bags2=""; $qty2=""; $ep=""; $picqty=""; $pckloss=""; $cc=""; $pckqty=""; $upstyp=""; $nopch=""; $exqty=""; $expch=""; $upss=""; $nomps=""; $nopchs=""; $blpch=""; $nobarc=""; $difq="";$difq1="";
	$sloc=""; $sloc1=""; $cnt++; 
	$sql_tbl_subsub2=mysqli_query($link,"select * from tbl_rpspackaging_sub where plantcode='$plantcode' and packaging_id='".$arrival_id."' order by packagingsub_id asc") or die(mysqli_error($link));
	$subsubtbltot=mysqli_num_rows($sql_tbl_subsub2);
	while($row_tbl_subsub2=mysqli_fetch_array($sql_tbl_subsub2))
	{
	$lotno=$row_tbl_subsub2['packagingsub_lotno']; 
	$exqty=$row_tbl_subsub2['packagingsub_extqty']; 
	$expch=$row_tbl_subsub2['packagingsub_extnop']; 
	$upss=$row_tbl_subsub2['packagingsub_upssize']; 
	$nomps=$row_tbl_subsub2['packagingsub_nomp']; 
	$remarks=$row_tbl_subsub2['packagingsub_remarks']; 
	$blpch=$row_tbl_subsub2['packagingsub_balpch']; 
	$nobarc=$row_tbl_subsub2['packagingsub_barcodes']; 
...
<tr class="Light">
        ...
        <!-- <td align="center" valign="middle" class="smalltbltext"><a href="add_packagingslipsmc_view.php?p_id=<?php echo $arrival_id;?>"><?php echo "PS".$row_arr_home['packaging_tcode']."/".$row_arr_home['packaging_yearid']."/".$lrole;?></a></td>-->
		  <td align="center" valign="middle" class="smalltbltext"><?php echo "PS".$row_arr_home['packaging_tcode']."/".$row_arr_home['packaging_yearid']."/".$lrole;?></td>
         ...
         <td align="center" valign="middle" class="smalltbltext"><?php if($row_arr_home['packaging_wbactflag']==1){?><a href="add_rpspackaginsloc_slip.php?p_id=<?php echo $arrival_id;?>" >Update</a><?php } else {?>Pending<?php } ?></td>
...
<br />
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_rpspackaging where plantcode='$plantcode' and packaging_type='SMC' and packaging_tflg=1 order by packaging_tcode desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_rpspackaging where plantcode='$plantcode' and packaging_type='SMC' and packaging_tflg=1 order by packaging_tcode desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_rpspackaging where packaging_type='SMC' and packaging_tflg=1 order by packaging_tcode desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
//$tot_arr_home=0;

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_rpspackaging where packaging_type='SMC' and packaging_tflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
...
<table align="center" border="0" width="600" cellspacing="0" cellpadding="0" bordercolor="#4ea1e1" style="border-collapse:collapse" > 
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Re-Printing Packaging slip</td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="970" bordercolor="#1dbe03" style="border-collapse:collapse">
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$variety=$noticia_item['popularname'];
	//echo $arrival_id;
	$lotno=""; $bags1=""; $qty1=""; $bags2=""; $qty2=""; $ep=""; $picqty=""; $pckloss=""; $cc=""; $pckqty=""; $upstyp=""; $nopch=""; $exqty=""; $expch=""; $upss=""; $nomps=0; $nopchs=""; $blpch=""; $nobarc=""; $difq="";$difq1="";
	$sloc=""; $sloc1=""; $cnt++; $nb1=0; $qt1=0; $nb2=0; $qt2=0; $wareh=""; $binn=""; $subbinn=""; $wareh1=""; $binn1=""; $subbinn1="";
	$sql_tbl_subsub2=mysqli_query($link,"select * from tbl_rpspackagingsub_sub where plantcode='$plantcode' and packagingsub_id='".$row_tblsub['packagingsub_id']."' and packaging_id='".$arrival_id."' order by packagingsubsub_id asc") or die(mysqli_error($link));
	$subsubtbltot=mysqli_num_rows($sql_tbl_subsub2);
	while($row_tbl_subsub2=mysqli_fetch_array($sql_tbl_subsub2))
	{
	$lotno=$row_tblsub['packagingsub_lotno']; 
	$exqty=$row_tblsub['packagingsub_extqty']; 
	$expch=$row_tblsub['packagingsub_extnop']; 
	$upss=$row_arr_home['packaging_upssize']; 
	...
	$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='$plantcode' and whid='".$row_tbl_subsub2['packagingsubsub_wh']."' order by perticulars") or die(mysqli_error($link));
	$row_whouse=mysqli_fetch_array($sql_whouse);
	$wareh=$row_whouse['perticulars']."/";
	
	$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='$plantcode' and binid='".$row_tbl_subsub2['packagingsubsub_bin']."' and whid='".$row_tbl_subsub2['packagingsubsub_wh']."'") or die(mysqli_error($link));
	$row_binn=mysqli_fetch_array($sql_binn);
	$binn=$row_binn['binname']."/";
	
	$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='$plantcode' and sid='".$row_tbl_subsub2['packagingsubsub_subbin']."' and binid='".$row_tbl_subsub2['packagingsubsub_bin']."' and whid='".$row_tbl_subsub2['packagingsubsub_wh']."'") or die(mysqli_error($link));
	$row_subbinn=mysqli_fetch_array($sql_subbinn);
	$subbinn=$row_subbinn['sname'];
	
	$nomp=$row_tbl_subsub2['packagingsubsub_nomp']; 
	$nop=$row_tbl_subsub2['packagingsubsub_nopch']; 
	$totp=$row_tbl_subsub2['packagingsubsub_totpch']; 
	
	$diq=explode(".",$row_tbl_subsub2['packagingsubsub_totqty']);
	if($diq[1]==000){$totqty=$diq[0];}else{$totqty=$row_tbl_subsub2['packagingsubsub_totqty'];}
