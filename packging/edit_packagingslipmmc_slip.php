<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$tdate2=$tdate12[2]."-".$tdate12[1]."-".$tdate12[0];
   		...
   		$sql_main="update tbl_packaging set packaging_type='$ttype', packaging_tdate='$tdate1', packaging_code='$txtid',  packaging_date='$tdate2', packaging_slipno='$txtpsrn', packaging_remarks='$txtremarks', packaging_yearid='$yearid_id', packaging_logid='$logid' where packaging_id='$pid'";
		...
		echo "<script>window.location='add_packagingslipmmc_preview.php?p_id=$pid'</script>";	
		}
	}
...
function balnopcheck(balval, val12)
{
	...
	{
		if(parseInt(balval) > parseInt(document.getElementById('wtnop_'+[val12]).value))
		{
...
<?php
$upsize=""; $tonops=0;
$sql_tbl_sub24=mysqli_query($link,"select * from tbl_packagingsub_sub where plantcode='$plantcode' andpackagingsub_id='".$row_tblsub['packagingsub_id']."' and packaging_id='".$arrival_id."'") or die(mysqli_error($link));
