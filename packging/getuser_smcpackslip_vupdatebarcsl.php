<?php
	...
	if(isset($_POST['foccode'])) { $foccode = $_POST['foccode']; }
	if(isset($_POST['slocseldet'])) { $slocseldet = $_POST['slocseldet']; }
	$sbin="txtsubbg".$slocseldet;
	if(isset($_POST[$sbin])) { $ssubbin = $_POST[$sbin]; }
	...
	if(isset($_POST['lotno'])) { $lotno = $_POST['lotno']; }
	if(isset($_POST['barcval'])) { $barcval = $_POST['barcval']; }
	if(isset($_POST['slocssyncs'])) { $slocssyncs = $_POST['slocssyncs']; }
		//echo $foccode;
		$sql_code="SELECT MAX(btsl_tcode) FROM tbl_btslmain where btsl_yearcode='$yearid_id'  ORDER BY btsl_tcode DESC";
		...
		$dt=date("Y-m-d");
		$subbinquery=mysqli_query($link,"select * from tbl_subbin where plantcode='$plantcode' and sid='$ssubbin' order by sname") or die(mysqli_error($link));
		$rowsubbing = mysqli_fetch_array($subbinquery);
		$wh24=$rowsubbing['whid'];
		$bin24=$rowsubbing['binid'];
		$stge="Pack";
		$sql_btslm="Insert into tbl_btslmain (btsl_wh, btsl_bin, btsl_subbin, btsl_date, btsl_tcode, btsl_logid, btsl_yearcode, btsl_crop, btsl_variety, btsl_stage, btsl_trtyp, plantcode) values('$wh24', '$bin24', '$ssubbin', '$dt', '$tcode', '$logid', '$yearid_id', '$crop', '$variety', '$stge', 'packaging', '$plantcode')";
		if(mysqli_query($link,$sql_btslm) or die(mysqli_error($link)))
		{
			$ttrid=mysqli_insert_id($link);
			$zx=explode(",",$foccode);
			foreach($zx as $a)
			{
				if($a<>"")
				{
					$sql_btsls="Insert into tbl_btslsub (btsl_id, btslsub_barcode, plantcode) values('$ttrid', '$a', '$plantcode')";
	...
	$as2=explode(",",$barcval);
	$df2=explode(",",$foccode);
	$nm2=array_diff($as2,$df2);
	$nm12=array_unique($nm2);
	$barcval=implode(",",$nm12);
?>
<?php	
$conts24=0;
if($barcval!="")
...
<input type="hidden" name="conts" value="<?php echo $conts24?>">
...
$bid=$rowsubbinn['binid'];	
	$sql_tbl_bar=mysqli_query($link,"select * from tbl_btslmain where plantcode='$plantcode' and btsl_subbin='".$sbinval."'") or die(mysqli_error($link));
