...
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_packslip1_mu.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		 {?>
		 <script>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_pack_slip_mu.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onClick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
</table></td>
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#1dbe03" style="border-collapse:collapse">
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_pnpslipmain where plantcode='$plantcode' and pnpslipmain_ttype='Packing Slip MU' and pnpslipmain_date>='$tdate' and pnpslipmain_date<='$pdate' and pnpslipmain_tflag=1 order by pnpslipmain_tcode desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_pnpslipmain where plantcode='$plantcode' and pnpslipmain_ttype='Packing Slip MU' and pnpslipmain_date>='$tdate' and pnpslipmain_date<='$pdate' and pnpslipmain_tflag=1 order by pnpslipmain_tcode desc";
...
<tr class="Light">
         ...
         <td align="center" valign="middle" class="smalltbltext"><a href="add_packslip_mu_view.php?p_id=<?php echo $arrival_id;?>"><?php echo "PS".$row_arr_home['pnpslipmain_tcode']."/".$yearid_id."/".$lrole;?></a></td>
