...
{
	if($withreprint=="ALL")
	$sql_rr=mysqli_query($link,"select * from tbl_mpmain plantcode='$plantcode' and where mpmain_barcode='".$row_arr_home1['mpmain_barcode']."' and mpmain_date='$ndts' and (mpmain_trtype='PACKLMC' or mpmain_trtype='PACKMMC' or mpmain_trtype='PACKNLC') order by mpmain_barcode ASC") or die(mysqli_error($link));
	else
	$sql_rr=mysqli_query($link,"select * from tbl_mpmain plantcode='$plantcode' and where mpmain_barcode='".$row_arr_home1['mpmain_barcode']."' and mpmain_date='$ndts' and mpmain_trtype='$pcktype' order by mpmain_barcode ASC") or die(mysqli_error($link));
