...
$noparr=explode(",", $row_mpmain['mpmain_mptnop']);
...
for ($i=0; $i<count($lotarr); $i++)
{
	if($lotarr[$i]<>"")
	{
	$quer_crp=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropid='".$crparr[$i]."' order by cropname Asc");
	$noticiacrp = mysqli_fetch_array($quer_crp);
	$ecrp=$noticiacrp['cropname'];
	
	$quer_ver=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$verarr[$i]."' and actstatus='Active' order by popularname Asc"); 
	...
	if($dopc=="--" || $dopc=="0000-00-00" || $dopc=="- -")$dopc="";
	if($dov=="--" || $dov=="0000-00-00" || $dov=="- -")$dov="";
...
<tr class="Light" height="25">
	...
	<td width="68" align="center" class="smalltblheading"><?php echo $upsarr[$i]?></td>
...
$sql_dispsubsub=mysqli_query($link,"select * from tbl_dispsub_sub where plantcode='$plantcode' and dpss_barcode='".$a."' order by dpss_barcode") or die(mysqli_error($link));
$row_dispsubsub=mysqli_fetch_array($sql_dispsubsub);

$sql_disps=mysqli_query($link,"select * from tbl_disp_sub where plantcode='$plantcode' and disps_id='".$row_dispsubsub['disps_id']."' ") or die(mysqli_error($link));
$row_disps=mysqli_fetch_array($sql_disps);

$sql_dispm=mysqli_query($link,"select * from tbl_disp where plantcode='$plantcode' and disp_id='".$row_dispsubsub['disp_id']."' ") or die(mysqli_error($link));
$row_dispm=mysqli_fetch_array($sql_dispm);

$sql_partym=mysqli_query($link,"select * from tbl_partymaser where p_id='".$row_dispm['disp_party']."' ") or die(mysqli_error($link));
$row_partym=mysqli_fetch_array($sql_partym);

$sql_loc=mysqli_query($link,"select * from tblproductionlocation where productionlocationid='".$row_partym['location_id']."' ") or die(mysqli_error($link));
$row_loc=mysqli_fetch_array($sql_loc);

$tdt2=explode("-", $row_dispm['disp_dodc']);
$dodisp=$tdt2[2]."-".$tdt2[1]."-".$tdt2[0];

$orderrefno='';
$orderno=$row_disps['disps_ordno'];
$orderno2=$orderno.",";
$orderno3=explode(",",$orderno2);
foreach($orderno3 as $orderno4)
{
	if($orderno4<>"")	
	{
		$sql_orderm=mysqli_query($link,"select * from tbl_orderm where plantcode='$plantcode' and orderm_porderno='".$orderno4."' ") or die(mysqli_error($link));
		$row_orderm=mysqli_fetch_array($sql_orderm);
		if($orderrefno!="") {$orderrefno=$orderrefno.",".$row_orderm['orderm_partyrefno'];} else {$orderrefno=$row_orderm['orderm_partyrefno'];}
	}
}
?>
<table align="center" border="1" width="750" cellspacing="0" cellpadding="0" bordercolor="#1dbe03" style="border-collapse:collapse" > 
...
<tr class="Light" height="25">
	<td width="76" align="center" class="smalltblheading">Dispatch Date&nbsp;</td>
	
	<td width="208" align="center" class="smalltblheading">Party Name&nbsp;</td>
	<td width="149" align="center" class="smalltblheading">Location&nbsp;</td>
	<td width="188" align="center" class="smalltblheading">Order Number&nbsp;</td>
	<td width="117" align="center" class="smalltblheading">Order Ref No.&nbsp;</td>
	
	
</tr>
<tr class="Light" height="25">
	<td width="76" align="center" class="smalltbltext">&nbsp;<?php echo $dodisp?></td>
	<td width="208" align="center" class="smalltbltext">&nbsp;<?php echo $row_partym['business_name']?></td>
	
	<td width="149" align="center" class="smalltbltext">&nbsp;<?php echo $row_loc['productionlocation'].", ".$row_loc['state']?></td>
	<td align="center" class="smalltbltext" >&nbsp;<?php echo $orderno;?></td>
	
	<td width="117" align="center" class="smalltbltext">&nbsp;<?php echo $orderrefno?></td>
</tr>
</table>
<?php
}
