		...
		if (($handle = fopen($filepath, "r")) !== FALSE) 
		{
			...
			{ 
				...
				if($row>=2)
				{
					...
					{
						$dt=date("Y-m-d");
						$day_month_array=explode("/",$data[10]);
						$final_date=$day_month_array[2]."/".$day_month_array[1]."/".$day_month_array[0];
						
						$day_month_array=explode("/",$data[5]);
						$final_date2=$day_month_array[2]."/".$day_month_array[1]."/".$day_month_array[0];
						
						$day_month_array=explode("/",$data[6]);
						$final_date3=$day_month_array[2]."/".$day_month_array[1]."/".$day_month_array[0];
						
						$cr="";$var=""; $flg=0;
						
						$sql_barcode2=mysqli_query($link,"Select bar_barcodes from tbl_barcodestmp where plantcode='$plantcode' and bar_barcodes='".$data[1]."'") or die(mysqli_error($link));
						if($tot_barcode2=mysqli_num_rows($sql_barcode2) > 0)$flg++;
						$sql_barcode3=mysqli_query($link,"Select bar_barcode from tbl_barcodes where plantcode='$plantcode' and bar_barcode='".$data[1]."'") or die(mysqli_error($link));
						if($tot_barcode3=mysqli_num_rows($sql_barcode3) > 0)$flg1++;	
						$sql_barcode4=mysqli_query($link,"Select mpmain_barcode from tbl_mpmain where plantcode='$plantcode' and mpmain_barcode='".$data[1]."'") or die(mysqli_error($link));
						if($tot_barcode4=mysqli_num_rows($sql_barcode4) > 0)$flg1++;	
						
						$pcodes=explode(",", $plantcodes);
						$ycodes=explode(",", $yearcodes);
						$barcd1=implode(",", str_split($data[9]));
						$barcd=explode(",", $barcd1);
						if(!in_array($barcd[0], $pcodes))
						{
							$flg++;
						}
						if(!in_array($barcd[1],$ycodes))
						{
							$flg++;
						}
						/*if($lotno!==$data[3])
						...
						$wtmccode=$barcd[2].$barcd[3];
						$test_string=$barcd[2].$barcd[3].$barcd[4].$barcd[5].$barcd[6].$barcd[7].$barcd[8].$barcd[9].$barcd[10];
						if(preg_match('/[^0-9]/',$test_string))$flg++;
						//echo "  -  ".$flg."<br/>";
						if($flg == 0)
						{
							$sql_mainbarc=mysqli_query($link,"select bar_barcodes from tbl_barcodestmp where bar_barcodes='".$barcode."' ") or die(mysqli_error($link));
							$a_mainbarc=mysqli_num_rows($sql_mainbarc);
							if($a_mainbarc == 0)
							{
								$str="insert into tbl_barcodestmp (bar_barcodes, bar_lotno, bar_grosswt, bar_wtdate, bar_wttime, bar_tid, bar_subid, bar_logid, bar_yearid, bar_psrn, bar_wtmcode, bar_crop, bar_variety, bar_ups, bar_dop, bar_vdate, bar_netweight, bar_poprid, plantcode) values('". $data[9]."', '".$lotno."', '". $data[8]."', '".$final_date."', '".$data[11]."', '".$tid."', '".$subtid."', '".$logid."', '".$yearid_id."', '".$txtpsrn."', '".$wtmccode."', '".$data[1]."', '".$data[2]."', '".$data[3]."', '".$final_date2."', '".$final_date3."', '".$data[7]."', '".$operatorcode."', '$plantcode')";
								
								//$str="insert into tbl_barcodestmp (bar_barcodes, bar_lotno, bar_grosswt, bar_wtdate, bar_wttime, bar_tid, bar_subid, bar_poprid, bar_logid, bar_yearid, bar_psrn, bar_wtmcode) values('". $data[1]."', '".$lotno."', '". $data[2]."', '".$final_date."', '".$data[4]."', '".$tid."', '".$subtid."', '".$data[5]."', '".$logid."', '".$yearid_id."', '".$txtpsrn."', '".$data[7]."')";
								//echo $str."<br>";
		...
		if($cnt>0)
		{
			if($totnomp!=$cnt)
			{
				...
				foreach($asd as $val1)			
				{
					if($val1<>"")
					{
						$str="delete from tbl_barcodestmp where bar_id='".$val1."'";
