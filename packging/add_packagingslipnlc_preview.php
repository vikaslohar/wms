	...
	while($row_arr=mysqli_fetch_array($sql_arr))
	{
		...
		while($row_arrsub=mysqli_fetch_array($sql_arrsub))
		{
			...
			while($row_arrsubsub=mysqli_fetch_array($sql_arrsubsub))
			{
				...
				$packnomp=1;
				$wtmpn=$row_arrsubsub['packagingsubsub_wtmp'];
				...
				$sql_barcode2=mysqli_query($link,"Select * from tbl_barcodestmp where plantcode='$plantcode' and bar_lotno='".$lotno."' and bar_psrn='".$drefno."'") or die(mysqli_error($link));
				...
				if($tot_barcode2 > 0)
				{
					while($row_barcode2=mysqli_fetch_array($sql_barcode2))
					{
						...
						$sql_barcode="Insert into tbl_barcodes (bar_trid, bar_trtype, bar_subtrid, bar_lotno, bar_orlot, bar_barcode, bar_grosswt, bar_wtdate, bar_wttime, bar_poprid, logid, yearid, plantcode) values('$pid', 'PACKNLC', '$subtranid', '$lotno', '$oldlot2', '$barcode', '$grosswt', '$dt', '$tim', '$poprid', '$logid', '$yearid_id', '$plantcode')";
						//echo "<br/>";
					...
					if($pval1<>"")
					{
						...
						while($row_issuetbl22=mysqli_fetch_array($sql_issuetbl22))
						{
							...
							while($row_arrsubsub3=mysqli_fetch_array($sql_arrsubsub3))
							{
								...
								$balnop2=$row_arrsubsub3['packagingsubsub_totpch'];
								...
								$sql_ins_main231="insert into tbl_mpmain (mpmain_date, mpmain_trid, mpmain_trtype, mpmain_crop, mpmain_variety, mpmain_lotno, mpmain_upssize, mpmain_barcode, mpmain_wtmp, mpmain_mptnop, mpmain_opnop, mpmain_opqty, mpmain_nop, mpmain_qty, mpmain_balnop, mpmain_balqty, mpmain_wh, mpmain_bin, mpmain_subbin, mpmain_yearcode, mpmain_logid, mpmain_lotnop, plantcode) values('$arrival_date', '$pid', 'PACKNLC', '$crop', '$variety', '$lotno2', '$packtype', '$a', '$wtinmp', '$nopinmp', '0', '0', '$balnop2', '$balqty2', '$balnop2', '$balqty2', '$whid', '$binid', '$subbinid', '$yearcode', '$logid', '$ltnop', '$plantcode')";
...
<title>Packaging -Transaction - Packaging Slip NLC - Preview</title>
...
function openslocpopprint()
{
	if(document.frmaddDepartment.txtitem.value!="")
	{
		...
		winHandle=window.open('packagingslipnlc_print.php?itmid='+itm,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
		if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Packaging slip - NLC - Preview </td>
	    </tr></table></td>
...
<td width="240"  align="left" valign="middle" class="smalltblheading" id="tnopinmp">&nbsp;<?php echo $wtimp;?>&nbsp;Kgs.</td>
...
<tr class="tblsubtitle" height="20">
	...
	<td width="65" rowspan="2" align="center" valign="middle" class="smalltblheading">Qty</td>
	<td width="43" rowspan="2" align="center" valign="middle" class="smalltblheading">NoP</td>
