<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if(isset($_POST['paceptyp'])) { $pcktype = $_POST['paceptyp']; }
		
		
		if(isset($_POST['txtplotno'])) { $txtplotno = $_POST['txtplotno']; }
		if(isset($_POST['txtbalnobp1'])) { $txtbalnobp1 = $_POST['txtbalnobp1']; }
		if(isset($_POST['txtbalqtyp1'])) { $txtbalqtyp1 = $_POST['txtbalqtyp1']; }
		...
		$nopcx="nopc_".$i;
		if(isset($_POST[$nopcx])) { $nopc= $_POST[$nopcx]; }
		$mpckx="mpck_".$i;
		if(isset($_POST[$mpckx])) { $mpck= $_POST[$mpckx]; }
		$nompx="nomp_".$i;
		if(isset($_POST[$nompx])) { $nomp= $_POST[$nompx]; }
		$wtmpx="wtmp_".$i;
		if(isset($_POST[$wtmpx])) { $wtmp= $_POST[$wtmpx]; }
		$wtnopx="wtnop_".$i;
		if(isset($_POST[$wtnopx])) { $wtnop= $_POST[$wtnopx]; }
		$nowbx="nowb_".$i;
		if(isset($_POST[$nowbx])) { $nowb= $_POST[$nowbx]; }
		
		$upsname="upsname_".$i;
		if(isset($_POST[$upsname])) { $upss= $_POST[$upsname]; }
		...
		$sql_main="insert into tbl_oflpnpslipmain(pnpslipmain_date, pnpslipmain_crop, pnpslipmain_variety, pnpslipmain_stage, pnpslipmain_promachcode, pnpslipmain_proopr, pnpslipmain_treattype, pnpslipmain_ttype, logid, yearid, plantcode, pnpslipmain_macopr) values ('$tdate','$txtcrop','$txtvariety','$txtstage', '$txtpromech', '$txtoprname', '$txttreattyp', 'OFL Packing Slip', '$logid', '$yearid_id', '$plantcode', '$txtoprname')";
	//exit;
		if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
		{
			$mainid=mysqli_insert_id($link);
			
			$sql_sub="insert into tbl_oflpnpslipsub (pnpslipsub_lotno, pnpslipmain_id, pnpslipsub_onob, pnpslipsub_oqty, pnpslipsub_bnob, pnpslipsub_bqty, pnpslipsub_packtype, pnpslipsub_availpqty, pnpslipsub_pickpqty, pnpslipsub_packqty, pnpslipsub_balcnob, pnpslipsub_balcqty, pnpslipsub_ups, pnpslipsub_upsid, pnpslipsub_qty, pnpslipsub_nop, pnpslipsub_lblschar, pnpslipsub_lblsno, pnpslipsub_lbechar, pnpslipsub_lbeno, pnpslipsub_convtomp, pnpslipsub_nomp, pnpslipsub_balpouch, pnpslipsub_nobarcodes, pnpslipsub_remarks, pnpslipsub_sstatus, pnpslipsub_orlot, pnpslipsub_qc, pnpslipsub_qcdttype, pnpslipsub_sltyp, pnpslipsub_plotno, plantcode, pnpslipsub_wtmp, pnpslipsub_mptnop, pnpslipsub_connob, pnpslipsub_conqty, pnpslipsub_availpnob) values ('$txtlot1', '$mainid', '$txtonob', '$txtoqty', '$txtbalnobp1', '$txtbalqtyp1', '$pcktype', '$txtoqty', '$txtoqty', '$balpck', '$balcnob', '$balcqty', '$upss', '$upsidno', '$packqty', '$nopc', '$domcs', '$lbls', '$domce', '$lble', '$mpck', '$nomp', '$noofpcks', '$detmpbno', '$txtremarks', '$softstatus', '$orlot', '$qcstatus', '$qcdttype', '$slocssyncs24', '$txtplotno', '$plantcode', '$wtmp', '$wtnop', '$txtonob', '$txtoqty', '$txtonob')";
			if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
			{
				$suid=mysqli_insert_id($link);
				
				echo "<script>window.location='add_packslip_preview_ofl.php?p_id=$mainid'</script>";	
			}
		}
	}
?>
...
<title>Packaging - Transaction - OFL Packing slip</title>
...
//alert(variety);
winHandle=window.open('getuser_packslip_lotno_ofl.php?crop='+crop+'&variety='+variety+'&stage='+stage+'&tid='+tid,'WelCome','top=170,left=180,width=420,height=350,scrollbars=yes');
if(winHandle==null){
...
if(document.frmaddDepartment.txtlot1.value=="")
		...
		var dsrn="";
		var stage=document.frmaddDepartment.txtstage.value;
		document.frmaddDepartment.getDetailsInfo.value="Selected";
		//alert(tid);
		//alert(lotid);
		
		//document.getElementById("postingsubtable").style.display="block";
		showUser(get,'postingsubsubtable','getofl',crop,variety,tid,lotid,dsrn,stage);
...
function mpchk(val1, val12)
{
	...
	{*/
		if(document.getElementById('mpck_'+[val12]).checked == true)
		{
			...
			//alert((parseFloat(document.getElementById('packqty_'+[val12]).value)/parseFloat(document.getElementById('wtmp_'+[val12]).value))/100);
			var nomp=Math.floor(parseFloat(document.getElementById('packqty_'+[val12]).value)/parseFloat(document.getElementById('wtmp_'+[val12]).value));
			var balnop=parseInt(nomp)*parseInt(document.getElementById('wtnop_'+[val12]).value);
			var blnop=parseInt(document.getElementById('nopc_'+[val12]).value)-parseInt(balnop);
			//alert(nomp); alert(balnop); alert(blnop);
			if(parseInt(blnop)==parseInt(document.getElementById('wtnop_'+[val12]).value))
			{
				nomp=parseInt(nomp)+1;
				var balnop=parseInt(nomp)*parseInt(document.getElementById('wtnop_'+[val12]).value);
				var blnop=parseInt(document.getElementById('nopc_'+[val12]).value)-parseInt(balnop);
			}
			//alert(nomp); alert(balnop); alert(blnop);
			document.getElementById('nomp_'+[val12]).value=parseInt(nomp);
			
			
			//alert(document.getElementById('nopc_'+[val12]).value);
			//alert(balnop);
			document.getElementById('noofpacks_'+[val12]).value=parseInt(blnop);
...
{ 
	if(pkselval=="P")
	{
		...
		var c2 = (parseInt(cl) + 1).toString().padStart(2, '0');
		cltn=cltn+c2+"P";
...
function clk(snoval,upsid)
{
	...
	if(snoval>0)
	{
		...
		document.getElementById(a).value=document.getElementById('recqtyp1').value;
		...
		document.getElementById(a).value=xc;
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - OFL Packing slip&nbsp;<input type="hidden" name="logid" value="<?php echo $logid?>" /></td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
<tr class="tblsubtitle" height="20">
  <td colspan="8" align="center" class="tblheading">Add OFL Packing Slip </td>
</tr>
...
<td width="319" align="left" valign="middle" class="smalltbltext">&nbsp;<input name="dopc" id="dopc" type="text" size="10" class="smalltbltext" tabindex="0" value="<?php echo date("d-m-Y");?>" maxlength="10"/>&nbsp;<a href="javascript:void(0)" onClick="datesel('dopc')" tabindex="6"><img src="../images/cal.gif" border="0" align="absmiddle" /></a>&nbsp;<font color="#FF0000">*</font></td>
...
<td width="165"  align="left" valign="middle" class="smalltbltext"  >&nbsp;<!--<input type="hidden" name="txtstage" value="Condition" />--><select class="smalltbltext" name="txtstage" style="width:80px;" onChange="sschk1()">
...
<input type="hidden" name="getDetailsInfo" value="" />
...
<tr >
<td valign="top" align="right"><a href="home_packslip_ofl.php" tabindex="20"><img src="../images/cancel.gif" border="0"style="display:inline;cursor:Pointer;" onclick="return confirm('Do You wish to Cancel this Transaction?');" /></a>&nbsp;&nbsp;<input name="Submit" type="image" src="../images/preview.gif" alt="Submit Value"  border="0" style="display:inline;cursor:Pointer;" tabindex="" onClick="return mySubmit();">&nbsp;&nbsp;</td>
</tr>
</table>
