<?php
	...
	while($row_arrsubsub=mysqli_fetch_array($sql_arrsubsub))
	{
		...
		$packdop=$row_arr['packaging_date'];
		$drefno=$row_arr['packaging_slipno'];
		$lotstage="Pack";
		$packtype=$row_arrsubsub['packagingsubsub_upssize'];
		$wtmp=$row_arrsubsub['packagingsubsub_wtmp'];
		$wtnp=$row_arrsubsub['packagingsubsub_wtnop'];
		...
		if($tot_barcode2 > 0)
		{
			while($row_barcode2=mysqli_fetch_array($sql_barcode2))
			{
				...
				$sql_barcode="Insert into tbl_barcodes (bar_trid, bar_trtype, bar_subtrid, bar_lotno, bar_orlot, bar_barcode, bar_grosswt, bar_wtdate, bar_wttime, bar_poprid, logid, yearid, bar_crop, bar_variety, bar_ups, bar_dop, bar_vdate, bar_netweight, plantcode) values('$pid', 'PACKNMC', '$subtranid', '$lotno', '$oldlot2', '$barcode', '$grosswt', '$dt', '$tim', '$poprid', '$logid', '$yearid_id', '$crop', '$variety', '$packtype', '$packdop', '$bar_vdate', '$bar_netweight', '$plantcode')";
				//echo "<br/>";
			...
			foreach($p1array as $pval1)
			{
				...
				if($pval1<>"")
				{
					...
					while($row_issuetbl22=mysqli_fetch_array($sql_issuetbl22))
					{
						...
						$sql_arrsubsub3=mysqli_query($link,"select * from tbl_packagingsub_sub2 where packaging_id='".$pid."' and packagingsub_id='".$row_arrsub['packagingsub_id']."'") or die(mysqli_error($link));
						...
						while($row_arrsubsub3=mysqli_fetch_array($sql_arrsubsub3))
						{
							...
							foreach($b_arr as $bval1)
							{
								if($bval1<>"")
								{
									$sql_barcode24="update tbl_barcodes set bar_wtmp='$wtmp', bar_orlot='$orlot2' where bar_trid='$pid' and bar_subtrid='$subtranid' and bar_lotno='$lotno' and bar_barcode='$bval1'";
									...
									$sql_ins_main24="insert into tbl_mpmain (mpmain_date, mpmain_trid, mpmain_trtype, mpmain_crop, mpmain_variety, mpmain_lotno, mpmain_upssize, mpmain_barcode, mpmain_wtmp, mpmain_mptnop, mpmain_opnop, mpmain_opqty, mpmain_nop, mpmain_qty, mpmain_balnop, mpmain_balqty, mpmain_wh, mpmain_bin, mpmain_subbin, mpmain_yearcode, mpmain_logid, mpmain_lotnop, plantcode) values('$arrival_date', '$pid', 'PACKNMC', '$crop', '$variety', '$lotno2', '$packtype', '$bval1', '$wtinmp', '$nopinmp', '0', '0', '$balnop2', '$balqty2', '$balnop2', '$balqty2', '$whid', '$binid', '$subbinid', '$yearcode', '$logid', '$ltnop', '$plantcode')";
							...
							$barcodes2=$a;
							$sql_ins_main24="insert into tbl_mpmain (mpmain_date, mpmain_trid, mpmain_trtype, mpmain_crop, mpmain_variety, mpmain_lotno, mpmain_upssize, mpmain_barcode, mpmain_wtmp, mpmain_mptnop, mpmain_opnop, mpmain_opqty, mpmain_nop, mpmain_qty, mpmain_balnop, mpmain_balqty, mpmain_wh, mpmain_bin, mpmain_subbin, mpmain_yearcode, mpmain_logid) values('$arrival_date', '$pid', 'PACKNMC', '$crop', '$variety', '$lotno2', '$packtype', '$barcodes2', '$wtinmp', '$nopinmp', '0', '0', '$balnop2', '$balqty2', '$balnop2', '$balqty2', '$whid', '$binid', '$subbinid', '$yearcode', '$logid')";
							mysqli_query($link,$sql_ins_main24) or die(mysqli_error($link));*/
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Packaging slip - NMC - Preview </td>
	    </tr></table></td>
...
<tr class="tblsubtitle" height="20">
  <td colspan="8" align="center" class="tblheading">Packaging Slip - NMC</td>
</tr>
