...
{	
	...
	else
	{
		...
		window.location='getuser_packlip_lotno.php?crop='+val3+'&variety='+val4+'&sqq='+txtlot1+'&stage='+stage;
		return true;	
	}
}	
...
$arrlots="";
		...
		$sql_tbl_main=mysqli_query($link,"select * from tbl_pnpslipmain where plantcode='$plantcode' and pnpslipmain_tflag!='1' and logid!='$logid'") or die(mysqli_error($link));
		$tot_arrmain=mysqli_num_rows($sql_tbl_main);
		if($tot_arrmain > 0)
		{
			while($row_tbl_main=mysqli_fetch_array($sql_tbl_main))
			{
				$sql_tbl_sub=mysqli_query($link,"select * from tbl_pnpslipsub where plantcode='$plantcode' and pnpslipmain_id='".$row_tbl_main['pnpslipmain_id']."'") or die(mysqli_error($link));
				$tot_arrsub=mysqli_num_rows($sql_tbl_sub);
				if($tot_arrsub > 0)
				{
					while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
...
echo $qr="select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$stage."' and lotldg_mergerflg=0 $sss  order by lotldg_id desc";
...
<tr >
<td align="right" valign="baseline"><img src="../images/back.gif" border="0" onClick="window.close()" style="vertical-align:baseline; cursor:pointer;" />&nbsp;<input type="image" src="../images/update.gif" alt="Submit Value" border="0" style="display:inline;cursor:pointer; vertical-align:baseline" onclick="return mySubmit();"/>&nbsp;&nbsp;&nbsp;</td>
</tr>
</table>
...
$lotqry=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where plantcode='$plantcode' and lotldg_lotno='".$row['lotldg_lotno']."' ") or die(mysqli_error($link));
...
{ 
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_lotno='".$row['lotldg_lotno']."' ") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 
//echo "select * from tbl_lot_ldg where plantcode='$plantcode' and lotldg_id='".$row_issue1[0]."' and lotldg_balqty > 0";
...
if($qty > 0)
{
$sql_issueg1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_mergerflg=0") or die(mysqli_error($link));
$row_issueg1=mysqli_fetch_array($sql_issueg1); 
				
$sql_issuetblg=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='$plantcode' and lotldg_id='".$row_issueg1[0]."' and lotldg_mergerflg=0") or die(mysqli_error($link)); 
...
if($zz[0]=="GOT-NR")
{
	...
	if(($row_24['lotldg_qc']=="UT" || $row_24['lotldg_qc']=="RT") && ($row_24['lotldg_srflg']!=0 && $row_24['lotldg_srtyp']!="pack" && $row_24['lotldg_srtyp']!="dispatch"))
	{
		$flg++; 
	}
	if($row_24['lotldg_got']=="Fail" || $row_24['lotldg_qc']=="Fail")
...
else
{
	...
	if(($row_24['lotldg_got']=="UT" || $row_24['lotldg_got']=="RT") && ($row_24['lotldg_srflg']!=0 && $row_24['lotldg_srtyp']!="pack" && $row_24['lotldg_srtyp']!="dispatch"))
	{
		$flg++; 
	}
	if($row_24['lotldg_got']=="Fail" || $row_24['lotldg_qc']=="Fail")
...
$row_arr=mysqli_fetch_array($sql_arr);
	...
	if($tdate5!="")
	{
		...
		if($your_date<$now)
		{
			...
			//echo $flg;
			$sql_srf2=mysqli_query($link,"select max(softr_id) from tbl_softr_sub2 where plantcode='$plantcode' and softrsub_lotno='$lot'") or die(mysqli_error($link));
			if($tot_srf2=mysqli_num_rows($sql_srf2) > 0)
			{
				$row_srf2=mysqli_fetch_array($sql_srf2);
				if($row_srf2['softrsub_lotno']==$lot)
				{
					$sql_srf22=mysqli_query($link,"select * from tbl_softr2 where plantcode='$plantcode' and softr_id='".$row_srf2[0]."'") or die(mysqli_error($link));
					$row_srf22=mysqli_fetch_array($sql_srf22);
					$tdate15="";
					$tdate15=$row_srf22['softr_date'];
					$tyear15=substr($tdate15,0,4);
					$tmonth15=substr($tdate15,5,2);
					$tday15=substr($tdate15,8,2);
					$tdate15=$tday15."-".$tmonth15."-".$tyear15;
...
//echo $row_24['lotldg_lotno']." ".$flg."<br />";
if($flg==0)
{
if($srno%2!=0)
{
