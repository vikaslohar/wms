...
$sql_tbl=mysqli_query($link,"select * from tbl_packaging where plantcode='$plantcode' and packaging_logid='".$logid."' and packaging_type='LMC' and packaging_tflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['packaging_id'];	
	
	$s_sub="delete from tbl_packaging_sub where packaging_id='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_packagingsub_sub where packaging_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
	$s_sub_sub2="delete from tbl_packagingsub_sub2 where packaging_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub2) or die(mysqli_error($link));
	
}
	
	$s_sub="delete from tbl_packaging where packaging_logid='".$logid."' and packaging_type='LMC' and packaging_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_packaging_lmc1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
...
<title>Packaging - Transaction - Packaging Slip LMC - Home</title>
...
{
winHandle=window.open('barcode_details.php?itmid='+tid,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Packaging slip - LMC</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_packagingslip_lmc.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
</table></td>
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#1dbe03" style="border-collapse:collapse">
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_packaging where plantcode='$plantcode' and packaging_type='LMC' and packaging_tflg=1 order by packaging_tcode desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_packaging where plantcode='$plantcode' and packaging_type='LMC' and packaging_tflg=1 order by packaging_tcode desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_packaging where packaging_type='LMC' and packaging_tflg=1 order by packaging_tcode desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
//$tot_arr_home=0;

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_packaging where packaging_type='LMC' and packaging_tflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Packaing slip - LMC</td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="970" bordercolor="#1dbe03" style="border-collapse:collapse">
			...
			<td width="68" rowspan="2" align="center" valign="middle" class="smalltblheading">Transaction Id</td>
            <td width="51" rowspan="2" align="center" valign="middle" class="smalltblheading">Date</td>
            <td width="74" rowspan="2" align="center" valign="middle" class="smalltblheading">Crop</td>
            <td width="75" rowspan="2" align="center" valign="middle" class="smalltblheading">Variety</td>
			<td width="75" rowspan="2" align="center" valign="middle" class="smalltblheading">Barcode</td>
            <td width="105" rowspan="2" align="center" valign="middle" class="smalltblheading">Lot No.</td>
            <td width="75" rowspan="2" align="center" valign="middle" class="smalltblheading">UPS</td>
			<td colspan="2" align="center" valign="middle" class="smalltblheading">Existing</td>
			<td width="45" rowspan="2" align="center" valign="middle" class="smalltblheading">NoP</td>
			<td width="40" rowspan="2" align="center" valign="middle" class="smalltblheading">Qty</td>
			<td colspan="2" align="center" valign="middle" class="smalltblheading">Balance</td>
	...
	<td width="160" align="center" valign="middle" class="smalltblheading">SLOC | MP | Loose Pchs | Total Pchs | Total Qty</td>
</tr>
<?php
//$srno=1;
if($tot_arr_home > 0)
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_tblsub=mysqli_query($link,"select * from tbl_packaging_sub where plantcode='$plantcode' and packaging_id='".$arrival_id."'") or die(mysqli_error($link));
	while($row_tblsub=mysqli_fetch_array($sql_tblsub))
	{
		...
		$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$row_tblsub['packagingsub_variety']."'  order by popularname Asc"); 
		$noticia_item = mysqli_fetch_array($quer4);
		$variety=$noticia_item['popularname'];
		//echo $arrival_id;
		$lotno=""; $bags1=""; $qty1=""; $bags2=""; $qty2=""; $ep=""; $picqty=""; $pckloss=""; $cc=""; $pckqty=""; $upstyp=""; $nopch=""; $exqty=""; $expch=""; $upss=""; $nomps=""; $nopchs=""; $blpch=""; $nobarc=""; $difq="";$difq1=""; $nops=""; $blqtys=""; $qtys="";
		$sloc=""; $sloc1=""; $cnt++; 
		$sql_tbl_subsub2=mysqli_query($link,"select * from tbl_packagingsub_sub where plantcode='$plantcode' and packagingsub_id='".$row_tblsub['packagingsub_id']."' and packaging_id='".$arrival_id."' order by packagingsubsub_id asc") or die(mysqli_error($link));
		...
		while($row_tbl_subsub2=mysqli_fetch_array($sql_tbl_subsub2))
		{
			...
			if($exqty!="")
			$exqty=$exqty."<br/>".$row_tbl_subsub2['packagingsubsub_extqty']; 
			else
			$exqty=$row_tbl_subsub2['packagingsubsub_extqty']; 
			if($expch!="")
			$expch=$expch."<br/>".$row_tbl_subsub2['packagingsubsub_extnop']; 
			else
			$expch=$row_tbl_subsub2['packagingsubsub_extnop']; 
			
			if($nops!="")
			$nops=$nops."<br/>".$row_tbl_subsub2['packagingsubsub_nop']; 
			else
			$nops=$row_tbl_subsub2['packagingsubsub_nop'];
			
			if($blpch!="")
			$blpch=$blpch."<br/>".$row_tbl_subsub2['packagingsubsub_balpch']; 
			else
			$blpch=$row_tbl_subsub2['packagingsubsub_balpch']; 
			
			if($lotno!="")
			$lotno=$lotno."<br/>".$row_tbl_subsub2['packagingsubsub_lotno'];
			else
			...
			$qtyss=$row_tbl_subsub2['packagingsubsub_nop']*$ptp;
			$blqtyss=$row_tbl_subsub2['packagingsubsub_balpch']*$ptp;	
			...
			if($blqtys!="")
			$blqtys=$blqtys."<br/>".$blqtyss;
			else
			$blqtys=$blqtyss;
		}
		$sql_tbl_subsub=mysqli_query($link,"select * from tbl_packagingsub_sub2 where plantcode='$plantcode' and packagingsub_id='".$row_tblsub['packagingsub_id']."' and packaging_id='".$arrival_id."'  order by packagingsubsub_id asc") or die(mysqli_error($link));
		...
		$sql_mp=mysqli_query($link,"Select * from tbl_mpmain where plantcode='$plantcode' and mpmain_trid='$arrival_id' and mpmain_trtype='PACKLMC' ") or die (mysqli_error($link));
		$row_mp=mysqli_fetch_array($sql_mp);
		
		$barcd=$row_mp['mpmain_barcode'];	
...
<tr class="Light">
        ...
        <!-- <td align="center" valign="middle" class="smalltbltext"><a href="add_packagingslipsmc_view.php?p_id=<?php echo $arrival_id;?>"><?php echo "PS".$row_arr_home['packaging_tcode']."/".$row_arr_home['packaging_yearid']."/".$lrole;?></a></td>-->
		 ...
		 <td align="center" valign="middle" class="smalltbltext"><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $barcd;?>');"><?php echo $barcd?></a></td>
         ...
         <td align="center" valign="middle" class="smalltbltext"><?php echo $nops?></td>
...
<tr >
<td valign="top" align="right"><a href="home_packaging_sel.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:Pointer;" /></a></td>
</tr>
</table>
