...
$sql_tbl=mysqli_query($link,"select * from tbl_packaging where plantcode='$plantcode' and packaging_logid='".$logid."' and packaging_type='MMC' and packaging_tflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tbl_packaging where packaging_logid='".$logid."' and packaging_type='MMC' and packaging_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
			echo "<script>window.location='home_packaging_mmc1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
...
<title>Packaging - Transaction - Packaging Slip MMC - Home</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#1dbe03" style="border-bottom:solid; border-bottom-color:#1dbe03" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Packaging slip - MMC</td>
	    </tr></table></td>
...
<!--<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_packagingslip_mmc.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>-->
</tr>
</table></td>
...
$sql_arr_home=mysqli_query($link,"select * from tbl_packaging where plantcode='$plantcode' and packaging_type='MMC' and packaging_tflg=1 order by packaging_tcode desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
//$tot_arr_home=0;

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_packaging where plantcode='$plantcode' and packaging_type='MMC' and packaging_tflg=1");
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Packaing slip - MMC</td>
</tr>
</table>
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$lotno=$row_tbl_subsub['packagingsubsub_lotno'];
