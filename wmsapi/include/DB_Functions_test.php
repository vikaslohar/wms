...
//	return $stmt->num_rows;	
		...
		{ 
			...
			if (in_array($dpss_barcodetype, ['LMC','NLC','MMC'])) {
				...
				$lotno_string=implode(',', $lotno_array); <br />
				$orlot_array=array(); $lotdop_array=array();
				
				$stmt_pnpsub = $this->conn_ps->prepare("SELECT orlot, lotldg_dop FROM tbl_lot_ldg_pack where trtype='Dispatch' AND lotno in ($lotno_string) GROUP BY lotno order BY lotno ASC, lotdgp_id DESC");
				$stmt_pnpsub->bind_param("i", $pnpslipmain_id);
				$result_pnpsub=$stmt_pnpsub->execute();
				$stmt_pnpsub->store_result();
				if ($stmt_pnpsub->num_rows > 0) {
					while($stmt_pnpsub->bind_result($orlot, $lotldg_dop))
					{
