...
//	return $stmt->num_rows;	
		...
		{ 
			...
			if (in_array($dpss_barcodetype, ['LMC','NLC','MMC'])) {
				$comma_cropname=''; $comma_popularname=''; $comma_dpss_ups=''; $comma_orlot=''; $comma_dpss_qty=''; $comma_dpss_dov=''; $comma_dpss_qc=''; $comma_dpss_dot=''; $comma_lotldg_dop='';
				$stmt_sub = $this->conn_ps->prepare("SELECT 
				GROUP_CONCAT(crp.cropname) as comma_cropname,
				GROUP_CONCAT(vr.popularname) as comma_popularname,
				GROUP_CONCAT(dp.dpss_ups) as comma_dpss_ups,
				GROUP_CONCAT(lot.orlot) as comma_orlot,
				GROUP_CONCAT(dp.dpss_qty) as comma_dpss_qty,
				GROUP_CONCAT(dp.dpss_dov) as comma_dpss_dov,
				GROUP_CONCAT(dp.dpss_qc) as comma_dpss_qc,
				GROUP_CONCAT(dp.dpss_dot) as comma_dpss_dot,
				GROUP_CONCAT(lot.lotldg_dop) as comma_lotldg_dop
				...
				INNER JOIN tbl_lot_ldg_pack lot ON dp.dpss_lotno = lot.lotno AND dp.disp_id = lot.lotldg_id AND trtype='Dispatch'
				WHERE dp.dpss_barcode='$dpss_barcode' ");
				//$stmt->bind_param("i", $lasttrid); 
				$stmt_sub->execute();
				$stmt_sub->store_result();
				if ($stmt_sub->num_rows > 0) {
				
					$stmt_sub->bind_result($comma_cropname, $comma_popularname, $comma_dpss_ups, $comma_orlot, $comma_dpss_qty, $comma_dpss_dov, $comma_dpss_qc, $comma_dpss_dot, $comma_lotldg_dop);
...
//					WHERE d.wb_mpbarcode=$barcods";
					...
					dp.disp_id, dp.dpss_lotno, dp.dpss_qty, dp.dpss_grosswt, dp.dpss_dov, dp.dpss_qc, dp.dpss_dot, dp.dpss_ups, 	
					dm.disp_date, dm.disp_partytype, dm.disp_state, dm.disp_location, dm.disp_party, dm.disp_dodc, p.business_name, l.productionlocation, dp.dpss_barcodetype, d.wb_type
					FROM tbl_wbqrcode d
					LEFT JOIN tblcrop crp ON d.wb_crop = crp.cropid
					LEFT JOIN tblvariety vr ON d.wb_variety = vr.varietyid
					LEFT JOIN tbl_pnpslipsub pnp ON d.wb_lotno = pnp.pnpslipsub_plotno
					...
					$pnpslipsub_wbinmp=0; $dop=''; $disp_id=0; $dpss_lotno=''; $dpss_qty=''; $dpss_grosswt=''; $dpss_dov=''; $dpss_qc=''; $dpss_dot=''; $disp_date=''; $disp_partytype='';  $disp_state=''; $disp_location=''; $disp_party=''; $disp_dodc=''; $dpss_ups=''; $orlot=''; $dpss_barcodetype=''; $wb_mptype='';
					//$stmt_wbqrcode = $this->conn_ps->prepare("SELECT wb_intqrcode, wb_extqrcode, wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_mptype, wb_mpqrcode, wb_mpbarcode, wb_mpwt, wb_mpgrosswt  FROM tbl_wbqrcode WHERE wb_mpbarcode = ? ");
