...
function chkallzeros(elem)
{
	var numval=elem.value;
	if(numval<=0)
	{
		alert("Value as 0 (Zero) not allowed");
		elem.value="";
		elem.focus();
		return false;
	}
	if(numval.charAt(0)==0)
	{
		alert("Starting 0 (Zero) not allowed");
		elem.value="";
		elem.focus();
		return false;
	}
}
function chkallzeros1(elem)
{
	var numval=elem.value;
	if(numval<=0)
	{
...
function lengthRestriction(elem, minc)
{
	var uInput = elem.value;
	if(uInput.length >= minc)
	{
		return true;
	}
	else
...
function panRestriction(elem1,elem2)
{
	var uInput1 = elem1.value.toUpperCase();
	var uInput2 = elem2.value.toUpperCase();
	var z=uInput1.split("");
	var pan=z[0]+z[1]+z[2]+z[3]+z[4]+z[5]+z[6]+z[7]+z[8]+z[9];
