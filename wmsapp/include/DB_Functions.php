...
//	private $conn_vnr;
	...
	public function GetTrList($scode) {
		$plantcode = $this->getPlantcode($scode);
        $stmt = $this->conn_ps->prepare("SELECT arrival_id, arrival_code, disp_date, arrsetupflag, setuplogid, arrunldflag, unldlogid, arrtrflag, logid, arrtrnunldtype, dc_date, dcno FROM tblarrival_unld WHERE arrtrflag!=1 and arrtrnunldtype='online' AND plantcode='$plantcode' ORDER BY arrival_id DESC");
        //$stmt->bind_param("ss", $email, $password);
        $stmt->execute();
        $stmt->store_result();
