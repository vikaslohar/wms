...
//	private $conn_vnr;
	...
	public function GetTrList($scode) {
		$plantcode = $this->getPlantcode($scode);
        $stmt = $this->conn_ps->prepare("SELECT arrival_id, arrival_code, disp_date, arrsetupflag, setuplogid, arrunldflag, unldlogid, arrtrflag, logid FROM tblarrival_unld WHERE arrtrflag!=1 AND plantcode='$plantcode' ORDER BY arrival_id DESC");
        //$stmt->bind_param("ss", $email, $password);
        $stmt->execute();
        $stmt->store_result();
		$userSR = array(); $user24=array();
		$arrival_id=''; $arrival_code=''; $disp_date=''; $arrsetupflag=0; $setuplogid=''; $arrunldflag=0; $unldlogid=''; $arrtrflag=0; $logid='';
        if ($stmt->num_rows > 0) {
            // user existed 
			$stmt->bind_result($arrival_id, $arrival_code, $disp_date, $arrsetupflag, $setuplogid, $arrunldflag, $unldlogid, $arrtrflag, $logid);
			while($stmt->fetch())
			{
			if($disp_date==NULL){$disp_date='';} if($arrsetupflag==NULL){$arrsetupflag=0;} if($setuplogid==NULL){$setuplogid='';} if($arrunldflag==NULL){$arrunldflag=0;} if($unldlogid==NULL){$unldlogid='';} if($arrtrflag==NULL){$arrtrflag=0;} if($logid==NULL){$logid='';}
	...
	public function GetTranSetupInit($scode,$dispdate,$dcdate,$dcno) {
        ...
        $stmt = $this->conn_ps->prepare("SELECT MAX(arrival_code) FROM tblarrival_unld WHERE arrival_type = ? AND yearcode = ? AND plancode=?");
	...
	public function GetTranSetupLotIns($scode,$trid,$lotno,$nobdc,$qtydc,$tarewt,$arrsub_id,$setuptype) {
        ...
        if ($stmt->num_rows > 0) {
			...
			if ($stmt_lotimp->num_rows > 0) {
				...
				$pcode='';
				$plantcode = $this->getPlantcode($scode);
				$stmt_plant = $this->conn_ps->prepare("SELECT code  FROM tbl_parameters AND plantcode=? ");
				$stmt_plant->bind_param("s", $plantcode);
				$result_plant=$stmt_plant->execute();
				$stmt_plant->store_result();
				if ($stmt_plant->num_rows > 0) {
				...
				if($arrival_id==$trid)
				{
					$lotn=$plantcode.$lotno."/00000/00".$stage2; $orlot=$plantcode.$lotno."/00000/00";
...
public function GetTranSetupLotyrcodelist() {
	...
	public function GetTranFinSubBinchk($scode,$whname,$binname,$subbinname,$varietyname,$cropname,$trid) {
		...
		{ 
			$stmt_arss = $this->conn_ps->prepare("SELECT lotvariety, lotcrop  FROM tblarr_sloc_unld WHERE lotvariety != ? and subbin = ? and plantcode = ?");
			$stmt_arss->bind_param("sis", $popularname, $subbinid, $plantcode);
	...
	public function GetTranLotSubmit($scode,$trid,$lotno,$harvestdate,$geoindex,$gottype,$seedstatus,$moisture,$purity,$remark,$qcstatus,$leduration,$ledate,$arrstatus,$gotstatus,$stage,$whname,$binname,$subbinname,$whname1,$binname1,$subbinname1,$slocnob,$slocnob1,$slocqty,$slocqty1) {
		...
		if ($stmt_2->num_rows > 0) {
			...
			if ($stmt_lotimp->num_rows > 0) {
				...
				while($stmt_lotimp->fetch())
				{
					$stmt_arrss = $this->conn_ps->prepare("SELECT SUM(grosswt) FROM tblarrsub_sub_unld WHERE arrival_id = ? and arrsub_id = ? ");
					...
					$stmtsub->bind_param("sisssssssisissssiiii", $moisture, $gemp, $purity, $gottype, $qcstatus, $remark, $got1, $seedstatus, $qcstatus, $val, $harvestdate, $geoindex, $leduration, $ledate, $grosswt, $diff, $totnob, $diff1, $arrival_id, $arrsub_id);
	...
	//return false;
		...
		else
		{
			...
			$sqlcode=$this->conn_ps->prepare("SELECT MAX(arrival_code) FROM tblarrival where yearcode='".$yearcode."' and plantcode='$plantcode' and arrival_type='Fresh Seed with PDN' ORDER BY arr_code DESC");
								//$sql_code1->bind_param("s", $lotno);
			$result_sqlcode=$sqlcode->execute();
			$sqlcode->store_result();
			if ($sqlcode->num_rows == 0) 
			{
				$sqlcode->bind_result($arrival_code);
				$t_sqlcode=$arrival_code;
				$scode=$t_sqlcode+1;
			}
			else
			{
			...
			$sqlcode1=$this->conn_ps->prepare("SELECT MAX(arr_code) FROM tblarrival where yearcode='".$yearcode."' and plantcode='$plantcode' and arrival_type='Fresh Seed with PDN' ORDER BY arr_code DESC");
								//$sql_code1->bind_param("s", $lotno);
			$result_sqlcode1=$sqlcode1->execute();
			$sqlcode1->store_result();
			if ($sqlcode1->num_rows == 0) 
			{
				$sqlcode1->bind_result($arr_code);
				$t_sqlcode1=$arr_code;
				$scode1=$t_sqlcode1+1;
			}
			else
			...
			if($result_arrimain)
			{
				...
				$flg=2;
					...
					while($stmt_arsub->fetch())
					{
						...
						$stmt_arrsub = $this->conn_ps->prepare("insert into tblarrival_sub (arrival_id, organiser, pdnno, pdndate, spcodef, spcodem, lotcrop, lotvariety, ploc, pper, farmer, plotno, gi, harvestdate, got, qty, act, diff, qty1, act1, diff1, moisture, vchk, qc, remarks, sstage, sstatus, lotno, old, got1, sample, qcsample, orlot, gssample, prodtype, lotstate, leduration, leupto, plantcode) Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
						$stmt_arrsub->bind_param("isssssssssssisssssiiissssssssssssississ", $artrid, $organiser, $pdnno, $pdndate, $spcodef, $spcodem, $lotcrop, $lotvariety, $ploc, $pper, $farmer, $plotno, $gi, $harvestdate, $got, $qty, $act, $diff, $qty1, $act1, $diff1, $moisture, $vchk, $qc, $remarks, $sstage, $sstatus, $lotno, $old, $got1, $sample, $qcsample, $orlot, $gssample, $prodtype, $lotstate, $leduration, $leupto, $plantcode);
							...
							//return "SELECT arr_type, arr_tr_id, arr_id, whid, binid, subbin, rowid, qty, bags, balqty, balbags, lotcrop, lotvariety FROM tblarr_sloc_unld WHERE arr_tr_id = $trid AND arr_id = $arrsub_id ";
								...
								while($stmt_arsubsub->fetch())
								{
									...
									{  
										...
										$stmt_lotldg->bind_param("sssisssiiiisisisssisssssisiss", $yearcode, $lotno, $trtype, $artrid, $arrival_date, $lotcrop, $lotvariety, $whid, $binid, $subbin, $zero, $zero1, $bags, $qty, $balbags, $balqty, $stage, $moisture, $gemp, $vchk, $qc, $got, $sstatus, $orlot, $gssample, $got1, $leduration, $leupto, $plantcode);
								...
								if($qc=="UT")
								{
									...
									$sql_sub_sub1244->bind_param("sssssssssssssss", $vchk, $moisture, $lotno, $arrival_date, $lotcrop, $lotvariety, $ncode1, $stage, $qc, $state,$one ,$orlot, $yearcode, $arr_role, $plantcode);
								...
								if($got1=="UT")
								{
									...
									$sql_sub_sub1245->bind_param("sssssssssss", $got1, $lotno, $arrival_date, $lotcrop, $lotvariety, $ncode1, $stage, $orlot, $yearcode, $arr_role, $plantcode);
