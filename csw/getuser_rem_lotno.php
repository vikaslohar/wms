...
<?php 
$lotchk=array('DU02869/00000/00C');
if($sqq!="")
{
...
$sss="";
$qr="select distinct lotldg_lotno, lotldg_sstatus, lotldg_got1, lotldg_got, lotldg_srflg, lotldg_qc from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$tp."' and lotldg_balqty > 0 and lotldg_mergerflg=0 $sss ";
...
//echo $row['lotldg_lotno']."<br/>";
$ccnt=0;
$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_binid, lotldg_whid from tbl_lot_ldg where  plantcode='".$plantcode."' and lotldg_lotno='".$row['lotldg_lotno']."'  and lotldg_balqty > 0 and lotldg_mergerflg=0 group by lotldg_subbinid, lotldg_binid order by lotldg_id asc") or die(mysqli_error($link));
...
//echo $row_is1[0]."  ";
$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_id='".$row_is1[0]."' and lotldg_balqty > 0 and lotldg_mergerflg=0 order by lotldg_id asc") or die(mysqli_error($link)); 
...
if($trid!="" && $trid > 0)
{
$sqltbl=mysqli_query($link,"select * from tbl_cswrem where plantcode='".$plantcode."' and rswrem_id='".$trid."'") or die(mysqli_error($link));
$rowtbl=mysqli_fetch_array($sqltbl);
$tot=mysqli_num_rows($sqltbl);		
$arrival_id=$rowtbl['rswrem_id'];

$sqltblsub=mysqli_query($link,"select * from tbl_cswrem_sub where plantcode='".$plantcode."' and rswrem_id='".$arrival_id."'") or die(mysqli_error($link));
