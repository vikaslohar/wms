...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <?php
	...
	$crp="ALL"; $ver="ALL";
	$qry="select Distinct lotldg_crop, lotldg_variety from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_sstage='Condition' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_balqty > 0";
...
$sql_rr=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and lotldg_balqty > 0 and lotldg_sstage='Condition' and (lotldg_qc='Fail' OR lotldg_got='Fail') order by lotldg_id desc") or die(mysqli_error($link));
...
$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_id='".$row_is1[0]."' and lotldg_balqty > 0 and lotldg_sstage='Condition' and (lotldg_qc='Fail' OR lotldg_got='Fail') order by lotldg_id asc") or die(mysqli_error($link)); 
...
$totqty=0; $totnob=0; $totqc=""; $totdot=""; $totmost=""; $totgemp=""; $totgot=""; $reserve=""; $totsst=""; $sloc=""; $genpp=""; $dogr="";
	$sql_issue=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_lotno='".$row_arr_home['lotldg_lotno']."'  and lotldg_balqty > 0 and lotldg_sstage='Condition' and (lotldg_qc='Fail' OR lotldg_got='Fail') group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
...
$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_id='".$row_issue1[0]."' and lotldg_balqty > 0 and lotldg_sstage='Condition' and (lotldg_qc='Fail' OR lotldg_got='Fail') order by lotldg_id asc") or die(mysqli_error($link)); 
