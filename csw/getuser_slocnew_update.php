...
if(mysqli_query($link,$sql_in1) or die(mysqli_error($link)))
{
	...
	for($j=1; $j<=$ocnt; $j++)
	{
		...
		{
			$sql_in="insert into tbl_sloc_csw_sub(slocid, crop, variety, lotno, whid, binid, subbinid, opbags, opqty, balbags, balqty, bags, qty, rowid,plantcode) values('$mainid', '$txtcrop', '$txtvariety', '$txtlot1', '$txtwhg', '$txtbing', '$txtsubbg', '$exBagsg', '$exqtyg', '$balBagsg', '$balqtyg', '$txtslBagsg', '$txtslqtyg', '$rowid','$plantcode')";
...
{
$sql_in1="update tbl_sloc_csw set crop='$txtcrop', variety='$txtvariety', lotno='$txtlot1', yearcode='$yearid_id', stage='$tp',surole='$lgnid' where slid='".$trid."'";
...
if(mysqli_query($link,$sql_in1) or die(mysqli_error($link)))
{
	...
	for($j=1; $j<=$ocnt; $j++)
	{
		...
		if($txtslqtyg!="" && $txtsubbg!="")
		{
			$sql_in="insert into tbl_sloc_csw_sub(slocid, crop, variety, lotno, whid, binid, subbinid, opbags, opqty, balbags, balqty, bags, qty, rowid,plantcode) values('$mainid', '$txtcrop', '$txtvariety', '$txtlot1', '$txtwhg', '$txtbing', '$txtsubbg', '$exBagsg', '$exqtyg', '$balBagsg', '$balqtyg', '$txtslBagsg', '$txtslqtyg', '$rowidx','$plantcode')";
			mysqli_query($link,$sql_in) or die(mysqli_error($link));
		}
	}
/* if($god1==1)
