...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <?php
	...
	$crp="ALL"; $ver="ALL";
	$qry="select Distinct lotldg_crop, lotldg_variety from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_sstage='Condition' and lotldg_balqty > 0";

	if($crop!="ALL")
	{	
	$qry.=" and lotldg_crop='$crop' ";
		$sql_crp=mysqli_query($link,"select cropname from tblcrop where cropid='".$crop."'") or die(mysqli_error($link));
	...
	$qry.=" and lotldg_variety='$variety' ";
		$sql_var=mysqli_query($link,"select popularname from tblvariety where varietyid='".$variety."' ") or die(mysqli_error($link));
...
$sql_rr=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and lotldg_sstage='Condition' and lotldg_balqty > 0 order by lotldg_id desc") or die(mysqli_error($link));
...
while($row_rr=mysqli_fetch_array($sql_rr))
{
	...
	$sql_crop=mysqli_query($link,"select cropname from tblcrop where cropid='".$row_arr_home1['lotldg_crop']."'") or die(mysqli_error($link));
		$row31=mysqli_fetch_array($sql_crop);
		 $crop=$row31['cropname'];		
		$sql_variety=mysqli_query($link,"select popularname from tblvariety where varietyid='".$row_rr['lotldg_variety']."' ") or die(mysqli_error($link));
		...
		$ccnt=0;
$sql_arhome=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and lotldg_sstage='Condition' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
	while($row_arhome=mysqli_fetch_array($sql_arhome))
{  
	$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_lotno='".$row_arhome['lotldg_lotno']."' and lotldg_sstage='Condition' group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
...
$sql_istbl=mysqli_query($link,"select lotldg_id from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_id='".$row_is1[0]."' and lotldg_sstage='Condition' and lotldg_balqty > 0 order by lotldg_id asc") or die(mysqli_error($link)); 
...
<tr class="tblsubtitle" height="20">
		...
		<td width="120"  align="center" valign="middle" class="smalltblheading">Lot No.</td>
			...
			<?php if($slchk=="yes") { ?>
			<td width="154"  align="center" valign="middle" class="smalltblheading">SLOC</td>
			<?php } ?>
			<td width="45"  align="center" valign="middle" class="smalltblheading">QC Status</td>
			<td width="45" align="center" valign="middle" class="smalltblheading">Moist %</td>
			<td width="45" align="center" valign="middle" class="smalltblheading">Germ %</td>
			...
			<td width="88"  align="center" valign="middle" class="smalltblheading">Production Location</td>
			<td width="86"  align="center" valign="middle" class="smalltblheading">State</td>
			...
			<td width="2%" align="center" valign="middle" class="tblheading">GOT Grade</td>
			<?php if($slchk2=="yes") { ?>
			...
			<?php } ?>
			<td width="70"  align="center" valign="middle" class="smalltblheading">Treatment Schema</td>
			</tr>
...
$srno=0; $totalbags=0; $totalqty=0;
	$sql_arr_home=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."'  and lotldg_sstage='Condition' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
...
$totqty=0; $totnob=0; $totqc=""; $totdot=""; $totmost=""; $totgemp=""; $totgot=""; $reserve=""; $totsst=""; $sloc="";$txtdot=""; 
	$sql_issue=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_lotno='".$row_arr_home['lotldg_lotno']."'  and lotldg_sstage='Condition' group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));

 while($row_issue=mysqli_fetch_array($sql_issue))
 { 


$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_sstage='Condition' and lotldg_lotno='".$row_arr_home['lotldg_lotno']."' order by lotldg_id asc ") or die(mysqli_error($link));
...
$qry2="select * from tblarrival_sub where plantcode='".$plantcode."' and  lotcrop='".$crop."' and lotvariety='".$variety."' and orlot='".$lotn."'";
...
$sql_issuetbl6=mysqli_query($link,"select proslipmain_id from tbl_proslipsub where plantcode='".$plantcode."' and  proslipsub_orlot='".$lotn."' order by proslipsub_id desc") or die(mysqli_error($link)); 
...
if($harvesdt=="00-00-0000" || $harvesdt=="--")	$harvesdt="";
if($ardt=="00-00-0000" || $ardt=="--")	$ardt="";		
//echo $txtdot;
//echo $cnt;
if($cnt>0)
{
