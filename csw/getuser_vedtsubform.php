...
$sql_tbl_sub=mysqli_query($link,"select * from tbl_csw where plantcode='".$plantcode."' and  arrsub_id='".$a."'") or die(mysqli_error($link));
$row_tbl_sub=mysqli_fetch_array($sql_tbl_sub);
  $tot_tbl_sub=mysqli_num_rows($sql_tbl_sub);
$tid=$row_tbl_sub['arrival_id'];
$subtid=$a;

$sql_tbl=mysqli_query($link,"select * from tbl_csw_main where plantcode='".$plantcode."' and  arrival_id='".$tid."'") or die(mysqli_error($link));
...
$sql_qc=mysqli_query($link,"SELECT distinct(variety) FROM tbl_csw WHERE crop='".$row_tbl_sub['crop']."' and variety NOT RLIKE '^[-+0-9.E]+$'") or die(mysqli_error($link)); 
