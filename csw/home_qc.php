...
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tbl_csw where plantcode='".$plantcode."' and  arrival_id='".$arrival_id."'";
	...
	*/
	...
	$s_sub="delete from tbl_csw_main where plantcode='".$plantcode."' and  arr_role='".$logid."'  and arrtrflag=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_csw_main where plantcode='".$plantcode."' and arr_role='$logid' order by arr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_csw_main where plantcode='".$plantcode."' and arr_role='$logid' order by arr_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_csw_main where arr_role='$logid' order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_csw_main where arr_role='$logid' and arrtrflag=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
