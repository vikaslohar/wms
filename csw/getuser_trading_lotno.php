...
$arrlots="";
/**/$sql_arrival=mysqli_query($link,"select arrival_id from tbl_csw_main where plantcode='".$plantcode."' and  arrival_id='".$trid."'") or die(mysqli_error($link));
...
if($tot_arrival)
{
	...
	// $row_arrival['arrival_id']." ";
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_csw where plantcode='".$plantcode."' and  crop='".$crop."' and variety='".$variety."'  and arrival_id='".$row_arrival['arrival_id']."'") or die(mysqli_error($link));
...
//echo $arrlots;
$sql_tbl_sub=mysqli_query($link,"select distinct(lotldg_lotno)  from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$tp."' and lotldg_balqty > 0 and (lotldg_qc='OK' or lotldg_qc='NUT') $sss")or die(mysqli_error($link));
...
$ccnt=0;
$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_lotno='".$row['lotldg_lotno']."'  and lotldg_balqty > 0 group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));

 while($row_is=mysqli_fetch_array($sql_is))
 { 
$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_whid='".$row_is['lotldg_whid']."' and lotldg_lotno='".$row['lotldg_lotno']."' order by lotldg_id desc ") or die(mysqli_error($link));
$row_is1=mysqli_fetch_array($sql_is1); 

$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_id='".$row_is1[0]."' and lotldg_balqty > 0 order by lotldg_id desc") or die(mysqli_error($link)); 
