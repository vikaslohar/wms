...
session_start();
	...
	$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where plantcode='".$plantcode."' and testdate='$sdate' and trstage='Condition' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
...
$sql_arr_home2=mysqli_query($link,"select MAX(tid) from tbl_qctest where plantcode='".$plantcode."' and sampleno='".$row_arr_home2['sampleno']."' and trstage='Condition' and testdate='$sdate' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
	$row_arr_home3=mysqli_fetch_array($sql_arr_home2);

$sql_arr_home3=mysqli_query($link,"select * from tbl_qctest where plantcode='".$plantcode."' and tid='".$row_arr_home3[0]."' and sampleno='".$row_arr_home2['sampleno']."' and trstage='Condition' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
...
{	
	...
	$slups=0; $slqty=0; 
	$sql_tbl_sub1=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_variety, lotldg_crop, lotldg_whid, lotldg_binid from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_lotno='".$row_arr_home['lotno']."' group by lotldg_subbinid, lotldg_variety, lotldg_lotno order by lotldg_subbinid") or die(mysqli_error($link));
	$t=mysqli_num_rows($sql_tbl_sub1);
	while($row_tbl=mysqli_fetch_array($sql_tbl_sub1))
	{
	
	$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_lotno='".$row_arr_home['lotno']."'") or die(mysqli_error($link));  
$row_tbl1=mysqli_fetch_array($sql_tbl1);

$sql1=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_id='".$row_tbl1[0]."'")or die(mysqli_error($link));
