...
<title>CSW- Transaction -Qc Sampling Request</title>
...
/*$sql_arr_home=mysqli_query($link,"select * from tbl_csw_main arr_role='$logid'and arrtrflag=1") or die(mysqli_error($link));
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_csw_main  where plantcode='".$plantcode."' and  arrival_date >='$tdate' and arrival_date <='$pdate' and arrtrflag=1 order by arr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_csw_main  where plantcode='".$plantcode."' and  arrival_date >='$tdate' and arrival_date <='$pdate' and arrtrflag=1 order by arr_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_csw_main  where plantcode='".$plantcode."' and  arrival_date >='$tdate' and arrival_date <='$pdate' and arrtrflag=1 order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_csw_main where plantcode='".$plantcode."' and  arrival_date >='$tdate' and arrival_date <='$pdate'  and arrtrflag=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
...
<table align="center" border="0" width="600" cellspacing="0" cellpadding="0" bordercolor="#fa8283" style="border-collapse:collapse" > 
...
$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_lotno='".$row_tbl_sub['lotno']."' and lotldg_balqty > 0 order by lotldg_id desc") or die(mysqli_error($link));
while($row_arhome=mysqli_fetch_array($sql_arhome))
{  
$qc=$row_arhome['lotldg_qc'];
$zc=explode(" ", $row_arhome['lotldg_got1']);
if($row_arhome['lotldg_got']!="" && $row_arhome['lotldg_got']!="NULL")
