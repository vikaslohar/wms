...
<title>CSW-Report Daily QC Result Report</title>
...
<?php	
	...
	$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where testdate='$sdate' and trstage='Condition' AND plantcode='".$plantcode."' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
...
while($row_arr_home2=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_arr_home2=mysqli_query($link,"select MAX(tid) from tbl_qctest where sampleno='".$row_arr_home2['sampleno']."' AND plantcode='".$plantcode."' and trstage='Condition' and testdate='$sdate' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
	$row_arr_home3=mysqli_fetch_array($sql_arr_home2);

$sql_arr_home3=mysqli_query($link,"select * from tbl_qctest where tid='".$row_arr_home3[0]."' AND plantcode='".$plantcode."' and sampleno='".$row_arr_home2['sampleno']."' and trstage='Condition' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
