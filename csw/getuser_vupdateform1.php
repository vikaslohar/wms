...
if($z1 == 0)
{
   $sql_main="insert into tbl_drying(arr_code, dryingdate,crop, variety,stage,drefno,plantcode)values('$c','$tdate1','$o','$p','Condition','$txtdrefno','$plantcode')";
...
{
$sql_main="update tbl_drying set  dryingdate='$tdate1',crop='$o',variety='$p',stage='Condition'  where trid='$z1'";
if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=$z1;
 $sql_sub="insert into tbl_dryingsub (lotno,onob,oqty,nob1,qty1,adnob ,adqty,trid,plantcode)values('$txtlotno','$txtdisp','$txtqty','$recqty','$txtrecbagp','$txtdqtyp','$txtdbagp','$mainid','$plantcode')";
mysqli_query($link,$sql_sub) or die(mysqli_error($link));
}
}
...
$sql_tbl=mysqli_query($link,"select * from tbl_drying where  stage='Condition' and trid='".$tid."'") or die(mysqli_error($link));
...
$sql_qc=mysqli_query($link,"SELECT distinct(variety) FROM tbl_csw_main WHERE crop='".$row_tbl['crop']."' and variety NOT RLIKE '^[-+0-9.E]+$'") or die(mysqli_error($link)); 
$tt1=mysqli_num_rows($sql_qc);
$vv1 = mysqli_fetch_array($sql_qc);
echo $pp=$vv1['variety'];
$sql_month=mysqli_query($link,"select varietyid, popularname from tblvariety where cropname='".$row_tbl['crop']."' and actstatus='Active' order by popularname Asc")or die(mysqli_error($link));
$noticia = mysqli_fetch_array($sql_month);
 $tt=$noticia['popularname'];
	echo   $tot=mysqli_num_rows($sql_month);	
	 if($tot==0)
	 {
	 $vv=$vv1['variety'];
	 }
...
<td align="left"  valign="middle" class="tbltext" colspan="6" >&nbsp;<input type="text" name="sstage" readonly="true" style="background-color:#CCCCCC;"  size="32" value="<?php echo $row_tbl['drefno'];?>" />&nbsp;<font color="#FF0000">*</font>&nbsp;</td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="850" bordercolor="#fa8283" style="border-collapse:collapse">

  <?php
/*$sql_tbl=mysqli_query($link,"select * from tbl_drying where  trid='".$tid."'") or die(mysqli_error($link));
$row_tbl=mysqli_fetch_array($sql_tbl);
 $tot=mysqli_num_rows($sql_tbl);	*/	
