...
session_start();
	...
	$crp="ALL"; $ver="ALL";
	$qry="select Distinct lotldg_crop, lotldg_variety from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_sstage='Condition' and (lotldg_qc='Fail' OR lotldg_got='Fail')";
	...
	$dh="Crop_Variety_wise_Substandard_Raw_Seed_Report ".$dat;
	$datahead = array("Crop Variety wise Substandard Condition Seed Report  ".$dat);
	$filename=$dh.".xls";  
...
$sql_rr=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and lotldg_sstage='Condition' and (lotldg_qc='Fail' OR lotldg_got='Fail')") or die(mysqli_error($link));
...
$totalbags=0; $totalqty=0;
	...
	$datahead2[$cnt] = array("#","Lot No.","NoB","Qty","Qc Status","Moisture %","Germination %","DOT","GOT Status","Genetic Purity %","DOGR","Seed Status"); 
	...
	$sql_arr_home=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where  plantcode='".$plantcode."' and lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and lotldg_sstage='Condition' and (lotldg_qc='Fail' OR lotldg_got='Fail')  and lotldg_balqty > 0 group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
...
$totqty=0; $totnob=0; $totqc=""; $totdot=""; $totmost=""; $totgemp=""; $totgot=""; $reserve=""; $totsst=""; $sloc=""; $genpp=""; $dogr="";
	$sql_issue=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  plantcode='".$plantcode."' and lotldg_lotno='".$row_arr_home['lotldg_lotno']."' and lotldg_sstage='Condition' and (lotldg_qc='Fail' OR lotldg_got='Fail')  and lotldg_balqty > 0 group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
...
$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_id='".$row_issue1[0]."' and lotldg_sstage='Condition' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_balqty > 0 order by lotldg_id asc") or die(mysqli_error($link)); 
...
if($t > 0)
{
 ...
 //echo $row_issuetbl['lotldg_id']."<BR>";
  $cnt1++;
	...
	$lotn=$row_issuetbl['lotldg_lotno']; 
...
$datahead2[$cnt] = array("#","Lot No.","NoB","Qty","SLOC","Qc Status","Moisture %","Germination %","DOT","GOT Status","Genetic Purity %","DOGR","Seed Status"); 

$data1[$cnt][$d]=array($d,$lotn,$totnob,$totqty,$sloc,$totqc,$totmost,$totgemp,$txtdot,$totgot,$genpp,$dogr,$totsst);
$d++;
}
