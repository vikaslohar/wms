...
if($z1 == 0)
{
   $sql_main="insert into tbl_cswrem(rswrem_tcode, rswrem_date, logid,plantcode)values('$txtid','$tdate1','$logid','$plantcode')";
	if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
	{
		$mainid=mysqli_insert_id($link);
		$sql_sub="insert into tbl_cswrem_sub (rswrem_id, crop, variety, lotnumber,plantcode)values('$mainid', '$txtcrop', '$txtvariety', '$txtlotno_1','$plantcode')";
		...
		{	
			...
			for($i=1; $i<=$srno1;$i++)
			{
				...
				$balqty2=$_GET[$balqty];
				  ...
				  $sql_sub="insert into tbl_cswremsub_sub (rswremsub_id, rswrem_id, whid, binid, subbinid, opnob, opqty, remnob, remqty, balnob, balqty,plantcode)values('$subid','$mainid', '$wh2', '$bin2', '$sbin2', '$onob2', '$oqty2', '$rnob2', '$rqty2', '$balnob2', '$balqty2','$plantcode')";
...
{
	$sql_main="update tbl_cswrem set  rswrem_date='$tdate1', logid='$logid'  where rswrem_id='".$z1."'";
	if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
	{
		$mainid=$z1;
		$sql_sub="update tbl_cswrem_sub set rswrem_id='$mainid', crop='$txtcrop', variety='$txtvariety', lotnumber='$txtlotno_1' where rswremsub_id='".$subtrid."'";
			if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
			{	
				$subid=$subtrid;
				$sql_del=mysqli_query($link,"delete from tbl_cswremsub_sub where rswremsub_id='".$subtrid."'") or die(mysqli_error($link));
				for($i=1; $i<=$srno1;$i++)
				{
					$wh="wh_".$i;
					$bin="bin_".$i;
