...
<title>CSW- Transaction -Drying Slip</title>
...
/**/$sql_arr_home=mysqli_query($link,"select * from tbl_drying  where plantcode='".$plantcode."' and stage='RSW' and arrflg=1") or die(mysqli_error($link));
...
$sql_arr_home=mysqli_query($link,"select * from tbl_drying  where plantcode='".$plantcode."' and  dryingdate >='$tdate' and dryingdate <='$pdate' and arrflg=1 and stage='RSW' order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_drying where plantcode='".$plantcode."' and  dryingdate >='$tdate' and stage='RSW' and dryingdate <='$pdate'  and arrflg=1");
...
<table align="center" border="0" width="650" cellspacing="0" cellpadding="0" bordercolor="#fa8283" style="border-collapse:collapse" > 
