<?php
	...
	/*$sql_tbl=mysqli_query($link,"select * from tbl_sloc_csw where surole='".$logid."' and supflg=0") or die(mysqli_error($link));
	while($row_tbl=mysqli_fetch_array($sql_tbl))
	{
		$arrival_id=$row_tbl['slid'];	
		
		$s_sub="delete from tbl_sloc_csw_sub where slocid='".$arrival_id."'";
		mysqli_query($link,$s_sub) or die(mysqli_error($link));
	}
	
		$s_sub="delete from tbl_sloc_csw where surole='".$logid."' and supflg=0";
		mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	*/
	...
	{ 
 	 ...
 	 $sql_in="insert into tbl_sloc_csw(yearcode,code,issuedate,classification_id,items_id, noofbinsg, noofbinsd, itmtype, surole)values('$yearid_id','$code','$tdate','$classification','$item', '$slocnog', '$slocnod', '$itmtype', '$lgnid')";
							
	if(mysqli_query($link,$sql_in)or die(mysqli_error($link)))
	{
		$slid=mysqli_insert_id($link);
		if($itmtype=="good")
		{
		$flash=0;
		for($i=0; $i<$slocnog; $i++)
		{
			if($flash==0)
			{
				$sql_in1="insert into tbl_sloc_csw_sub (slocid, whid, binid, subbinid, Bags, qty, rowid) values ('$slid', '$wh1', '$bin1', '$subbin1', '$Bags1', '$qty1', '$rowid1')";
			}
			else if($flash==1)
			{
				$sql_in1="insert into tbl_sloc_csw_sub (slocid, whid, binid, subbinid, Bags, qty, rowid) values ('$slid', '$wh2', '$bin2', '$subbin2', '$Bags2', '$qty2', '$rowid2')";
			}
			else if($flash==2)
			{
				$sql_in1="insert into tbl_sloc_csw_sub (slocid, whid, binid, subbinid, Bags, qty, rowid) values ('$slid', '$wh3', '$bin3', '$subbin3', '$Bags3', '$qty3', '$rowid3')";
			}
		...
		for($i=0; $i<$slocnod; $i++)
		{
			if($flash==0)
			{
				$sql_in1="insert into tbl_sloc_csw_sub (slocid, whid, binid, subbinid, Bags, qty, rowid) values ('$slid', '$wh4', '$bin4', '$subbin4', '$Bags4', '$qty4', '$rowidd1')";
			}
			else if($flash==1)
			{
				$sql_in1="insert into tbl_sloc_csw_sub (slocid, whid, binid, subbinid, Bags, qty, rowid) values ('$slid', '$wh5', '$bin5', '$subbin5', '$Bags5', '$qty5', '$rowidd2')";
			}
			mysqli_query($link,$sql_in1)or die(mysqli_error($link));
			$flash++;
		}
...
//}
	...
	$a="TSU";
	$sql_code="SELECT MAX(code) FROM tbl_sloc_csw  where yearcode='$yearid_id' AND plantcode='".$plantcode."'   ORDER BY code DESC";
