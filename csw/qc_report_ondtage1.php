...
<?php	
	...
	if($itemid!="ALL")
	{
		...
		$qrymain="select distinct lotldg_crop from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_qctestdate<='".$edate."' and lotldg_crop='".$itemid."' ";
	}
	else
	{
		$qrymain="select distinct lotldg_crop from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_qctestdate<='".$edate."'  ";
	}
	
	
	if($ovariety!="ALL")
	{
		$qrymain.=" and lotldg_variety='".$ovariety."' ";
	}
	if($result!="ALL")	
	{
		$qrymain.=" and lotldg_qc='".$result."' ";
	}
	if($seedstage!="ALL")
	{
		$qrymain.=" and lotldg_sstage='".$seedstage."' ";
	}
	$qrymain.=" order by lotldg_variety asc,lotldg_qctestdate asc ";
	//echo $qry;
	$sql_arr_home1m=mysqli_query($link,$qrymain) or die(mysqli_error($link));
	$tot_arr_homem=mysqli_num_rows($sql_arr_home1m);
...
if($tot_arr_homem > 0)
{
while($row_arr_home1m=mysqli_fetch_array($sql_arr_home1m))
{
	
	
	$sql_class=mysqli_query($link,"select * from tblcrop where cropid='".$row_arr_home1m['lotldg_crop']."'") or die(mysqli_error($link));
	$row_class=mysqli_fetch_array($sql_class);
	$crop=$row_class['cropname'];	 
	
	$qry="select distinct lotldg_variety from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_qctestdate<='".$edate."' and lotldg_crop='".$row_arr_home1m['lotldg_crop']."' ";
	
	
	if($ovariety!="ALL")
	{
		$qry.=" and lotldg_variety='".$ovariety."' ";
	}
	if($result!="ALL")	
	{
...
{	
	...
	$quer2=mysqli_query($link,"SELECT  cropname,cropid FROM tblcrop where cropid='".$row_arr_home1m['lotldg_crop']."'"); 
	...
	$cont=0;
	$sql_arr_home244=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_qctestdate<='$edate' and lotldg_crop='".$row_arr_home1m['lotldg_crop']."'  and lotldg_variety='".$row_arr_home1['lotldg_variety']."' $reslt order by lotldg_variety asc, lotldg_qctestdate asc ") or die(mysqli_error($link));
	$t=mysqli_num_rows($sql_arr_home244);
	while($row_arr_home244=mysqli_fetch_array($sql_arr_home244))
	{
		//if($seedstage!="Pack")
		{
			$sql_tbl_sub1=mysqli_query($link,"select distinct lotldg_subbinid from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_lotno='".$row_arr_home244['lotldg_lotno']."' order by lotldg_subbinid") or die(mysqli_error($link));
			$t=mysqli_num_rows($sql_tbl_sub1);
			while($row_tbl=mysqli_fetch_array($sql_tbl_sub1))
			{$total_tbl=0;
				$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_lotno='".$row_arr_home244['lotldg_lotno']."'") or die(mysqli_error($link));  
		...
		//else
		{
			$sql_tbl_sub1=mysqli_query($link,"select distinct subbinid from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  lotno='".$row_arr_home244['lotldg_lotno']."' order by subbinid") or die(mysqli_error($link));
			$t=mysqli_num_rows($sql_tbl_sub1);
			while($row_tbl=mysqli_fetch_array($sql_tbl_sub1))
			{$total_tbl=0;
				$sql_tbl1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  subbinid='".$row_tbl['subbinid']."' and lotno='".$row_arr_home244['lotldg_lotno']."'") or die(mysqli_error($link));  
				$row_tbl1=mysqli_fetch_array($sql_tbl1);
...
<table  border="1" cellspacing="0" cellpadding="0" width="970" bordercolor="#fa8283" style="border-collapse:collapse" align="center">
...
<?php
$sql_arr_home2=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_qctestdate<='".$edate."' and lotldg_crop='".$row_arr_home1m['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' $reslt order by lotldg_variety asc, lotldg_qctestdate asc ") or die(mysqli_error($link));
$t=mysqli_num_rows($sql_arr_home2);
while($row_arr_home2=mysqli_fetch_array($sql_arr_home2))
{

$sql_arr_home3=mysqli_query($link,"select MAX(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_qctestdate<='".$edate."' and lotldg_crop='".$row_arr_home1m['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and lotldg_lotno='".$row_arr_home2['lotldg_lotno']."' $reslt order by lotldg_variety asc, lotldg_qctestdate asc  ") or die(mysqli_error($link));
$row_arr_home3=mysqli_fetch_array($sql_arr_home3);

$sql_arr_home=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_id='".$row_arr_home3[0]."'") or die(mysqli_error($link));
...
$crop=""; $variety=""; $lotno=""; $bags=""; $qty=0; $stage=""; $got=""; $qc=""; $sstatus=""; $loc1=""; $per=""; $qcresult="";
	$sql_tbl_sub1=mysqli_query($link,"select distinct lotldg_subbinid from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_lotno='".$row_arr_home2['lotldg_lotno']."'  order by lotldg_subbinid") or die(mysqli_error($link));
if(	 $t=mysqli_num_rows($sql_tbl_sub1) > 0)
{
	while($row_tbl=mysqli_fetch_array($sql_tbl_sub1))
	{	 
	
	$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_lotno='".$row_arr_home2['lotldg_lotno']."'") or die(mysqli_error($link));  
$row_tbl1=mysqli_fetch_array($sql_tbl1);
//echo "  ".$row_tbl1[0]."  ";
$sql1=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_id='".$row_tbl1[0]."' and lotldg_balqty > 0 ")or die(mysqli_error($link));
...
{
	$sql_tbl_sub1=mysqli_query($link,"select distinct subbinid from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  lotno='".$row_arr_home2['lotldg_lotno']."'  order by subbinid") or die(mysqli_error($link));
	while($row_tbl=mysqli_fetch_array($sql_tbl_sub1))
	{	 
	
	$sql_tbl1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  subbinid='".$row_tbl['subbinid']."' and lotno='".$row_arr_home2['lotldg_lotno']."'") or die(mysqli_error($link));  
$row_tbl1=mysqli_fetch_array($sql_tbl1);
//echo "  ".$row_tbl1[0]."  ";
$sql1=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  lotdgp_id='".$row_tbl1[0]."' and balqty > 0 ")or die(mysqli_error($link));
...
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='".$plantcode."' and  whid='".$row_tbl_sub['whid']."' order by perticulars") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);
$wareh=$row_whouse['perticulars']."/";

$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='".$plantcode."' and  binid='".$row_tbl_sub['binid']."' and whid='".$row_tbl_sub['whid']."'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname']."/";

$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='".$plantcode."' and  sid='".$row_tbl_sub['subbinid']."' and binid='".$row_tbl_sub['binid']."' and whid='".$row_tbl_sub['whid']."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
$subbinn=$row_subbinn['sname'];
