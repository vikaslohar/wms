...
if(isset($_GET['txt12']))
	...
	/*
	$quer_cn=mysqli_query($link,"SELECT * FROM tblvariety WHERE actstatus='Active' ");
		$row_cls=mysqli_fetch_array($quer_cn);
...
$sql_tbl=mysqli_query($link,"select * from tbl_drying where plantcode='".$plantcode."' and   stage='Condition' and trid='".$tid."'") or die(mysqli_error($link));
...
<td align="left"  valign="middle" class="tbltext" >&nbsp;<input type="text" name="txtcrop1" readonly="true" style="background-color:#CCCCCC;" value="<?php echo $noticia['cropname'];?>" size="32"  />&nbsp;<font color="#FF0000">*</font>&nbsp;<input type="hidden" name="txtcrop" value="<?php echo $noticia['cropid'];?>" /></td>
	  <?php
/*$quer3=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$row_tbl['variety']."' and actstatus='Active' order by popularname Asc"); 
