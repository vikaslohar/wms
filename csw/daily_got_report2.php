...
$sql_arr_home2=mysqli_query($link,"select MAX(tid) from tbl_qctest where sampleno='".$row_arr_home2['sampleno']."' AND plantcode='".$plantcode."' and gotdate='$sdate' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
	$row_arr_home3=mysqli_fetch_array($sql_arr_home2);

$sql_arr_home3=mysqli_query($link,"select * from tbl_qctest where tid='".$row_arr_home3[0]."' AND plantcode='".$plantcode."' and sampleno='".$row_arr_home2['sampleno']."' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
...
$stage=$row_arr_home['gemp'];	
	...
	$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' AND plantcode='".$plantcode."' and lotldg_lotno='".$row_arr_home['lotno']."'") or die(mysqli_error($link));
