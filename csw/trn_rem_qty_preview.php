<?php
	...
	$sql_arr=mysqli_query($link,"select * from tbl_cswrem where rswrem_id='".$pid."'") or die(mysqli_error($link));
	while($row_arr=mysqli_fetch_array($sql_arr))
	{
	$trdate=$row_arr['rswrem_date'];
	$arrival_id=$row_arr['rswrem_id'];
	
	$sql_arrsub=mysqli_query($link,"select * from tbl_cswrem_sub where rswrem_id='".$arrival_id."'") or die(mysqli_error($link));
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		$crop=$row_arrsub['crop'];
		$variety=$row_arrsub['variety'];
		$lot=$row_arrsub['lotnumber'];
		$sql_arrsub_sub=mysqli_query($link,"select * from tbl_cswremsub_sub where rswremsub_id='".$row_arrsub['rswremsub_id']."' and rswrem_id='".$row_arrsub['rswrem_id']."'") or die(mysqli_error($link));
		while($row_arrsub_sub=mysqli_fetch_array($sql_arrsub_sub))
		{
			...
			$qty=$row_arrsub_sub['remqty'];
				...
				$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$subbinid."' and lotldg_binid='".$binid."' and lotldg_whid='".$whid."' and lotldg_lotno='".$lot."'") or die(mysqli_error($link));
				...
				$sql_sub_sub="insert into tbl_lot_ldg (yearcode,lotldg_trtype, lotldg_trid, lotldg_lotno, orlot, lotldg_trdate, lotldg_crop, lotldg_variety, lotldg_whid, lotldg_binid, lotldg_subbinid, lotldg_opbags, lotldg_opqty, lotldg_trbags, lotldg_trqty, lotldg_balbags, lotldg_balqty, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_qctestdate, lotldg_gs, lotldg_resverstatus, lotldg_revcomment, lotldg_gottestdate, lotldg_got, lotldg_genpurity, lotldg_srtyp, lotldg_srflg,plantcode) values('$ycode','Qty-Rem', '$pid', '$lot', '$olot', '$trdate', '$crop', '$variety', '$whid', '$binid', '$subbinid', '$opups', '$opqty', '$ups', '$qty', '$balups', '$balqty', '$stage', '$status', '$moist', '$gemp', '$vchk', '$got1', '$qc', '$qcdate', '$gs', '$revstatus', '$revcoment', '$gotdate', '$got', '$geneticpurity', '$srtype', '$srflg','$plantcode')";
...
while($row_issueg=mysqli_fetch_array($sql_issueg))
 { 
	...
	$sql_code1="SELECT MAX(rswrem_code) FROM tbl_cswrem where plantcode='".$plantcode."' and  ORDER BY rswrem_code DESC";
	$res_code1=mysqli_query($link,$sql_code1)or die(mysqli_error($link));
  	...
  	$sql_main="update tbl_cswrem set rswrem_tflg=1, rswrem_code='$ncode1'  where rswrem_id ='$pid'";
...
<title>CSW -Transaction - Condition Seed Release - Preview</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#fa8283" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#fa8283" style="border-bottom:solid; border-bottom-color:#fa8283" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Condition Seed Release - Preview </td>
	    </tr></table></td>
...
$sql_tbl_subsub=mysqli_query($link,"select * from tbl_cswremsub_sub where plantcode='".$plantcode."' and rswremsub_id='".$row_tbl_sub['rswremsub_id']."' and rswrem_id='".$row_tbl_sub['rswrem_id']."'") or die(mysqli_error($link));
