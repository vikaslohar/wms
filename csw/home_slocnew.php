...
$sql_tbl=mysqli_query($link,"select * from tbl_sloc_csw where plantcode='".$plantcode."' and  surole='".$logid."' and supflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['slid'];	
	
	$s_sub="delete from tbl_sloc_csw_sub where plantcode='".$plantcode."' and  slocid='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
}

	$s_sub="delete from tbl_sloc_csw where plantcode='".$plantcode."' and  surole='".$logid."' and supflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#fa8283" >
	  ...
	  <table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#fa8283" style="border-bottom:solid; border-bottom-color:#fa8283" >
	    <tr >
	      <td width="810" height="25">&nbsp;Transaction - SLOC Updation</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_slocnew.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
...
$adjacents = 3;
 	...
 	$query = "SELECT COUNT(*) as num FROM tbl_sloc_csw where plantcode='".$plantcode."' and  supflg=1 and (stage='Condition' or stage='Condition Seed')";
	...
	/* Get data. */
	$sql = "select * from tbl_sloc_csw where plantcode='".$plantcode."' and  supflg=1 and (stage='Condition' or stage='Condition Seed') order by slid desc LIMIT $start, $limit";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_sloc_csw where plantcode='".$plantcode."' and  supflg=1 and stage='Condition' order by scode desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_sloc_csw where plantcode='".$plantcode."' and  supflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */

?>

<table align="center" border="0" width="700" cellspacing="0" cellpadding="0" bordercolor="#fa8283" style="border-collapse:collapse" > 
...
$itemqry1=mysqli_query($link,"select * from tblvariety where varietyid='".$row_arr_home['variety']."'  and vertype='PV'") or die(mysqli_error($link));
