<?php
	...
	{ 
		...
		$seedstage=trim($_POST['seedstage']);
		$ornob=trim($_POST['ornob']);
		$orqty=trim($_POST['orqty']);
		$batchnob=trim($_POST['batchnob']);
		$batchqty=trim($_POST['batchqty']);
		$batchorlot=trim($_POST['txtbatchno']);
		
		$wh1=trim($_POST['txtslwhg1']);
		$bin1=trim($_POST['txtslbing1']);
		$subbin1=trim($_POST['txtslsubbg1']);
		$Bags1=trim($_POST['txtslBagsg1']);
		$qty1=trim($_POST['txtslqtyg1']);
		
		if(isset($_GET['txtslwhg2'])) { $wh2=trim($_POST['txtslwhg2']); } else {$wh2=0;}
		if(isset($_GET['txtslbing2'])) { $bin2=trim($_POST['txtslbing2']); } else {$bin2=0;}
		if(isset($_GET['txtslsubbg2'])) { $subbin2=trim($_POST['txtslsubbg2']); } else {$subbin2=0;}
		if(isset($_GET['txtslBagsg2'])) { $Bags2=trim($_POST['txtslBagsg2']); } else {$Bags2=0;}
		if(isset($_GET['txtslqtyg2'])) { $qty2=trim($_POST['txtslqtyg2']); } else {$qty2=0;}
		$totnob=$Bags1+$Bags2;
		$totqty=$qty1+$qty2;
		
		$tdate=date("Y-m-d");
		
		if($seedstage=="Raw") { $batchlotno=$batchorlot."R";}
		else if($seedstage=="Condition") { $batchlotno=$batchorlot."C";}
		else { $batchlotno=$batchorlot."C";}

 	 $sql_in="insert into tbl_lotbatch(batch_date, batch_code, batch_crop, batch_variety, batch_lotno, batch_stage, batch_onob, batch_oqty, batch_batchlot, batch_orlot, batch_nob, batch_qty, batch_wh, batch_bin, batch_subbin, batch_slnob, batch_slqty, batch_wh1, batch_bin1, batch_subbin1, batch_slnob1, batch_slqty1, batch_tflg, batch_logid, batch_yearcode, plantcode)values('$tdate','$code','$txtcrop','$txtvariety','$txtlot1', '$seedstage', '$ornob', '$orqty', '$batchlotno', '$batchorlot', '$batchnob', '$batchqty', '$wh1', '$bin1', '$subbin1', '$Bags1', '$qty1', '$wh2', '$bin2', '$subbin2', '$Bags2', '$qty2', '1', '$logid', '$yearid_id', '$plantcode')";
	if(mysqli_query($link,$sql_in)or die(mysqli_error($link)))
	{
	//echo "<br />";		
		$sql_issue=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_binid, lotldg_whid  from tbl_lot_ldg where lotldg_crop='".$txtcrop."' and lotldg_variety='".$txtvariety."' and lotldg_lotno='".$txtlot1."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
		while ($row_issue=mysqli_fetch_array($sql_issue))
		{
			$pid=mysqli_insert_id($link);
			$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' AND plantcode='".$plantcode."' and lotldg_variety='".$txtvariety."' and lotldg_lotno='".$txtlot1."' ") or die(mysqli_error($link));
			...
			while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
			{
				...
				if($sstage=="")$sstage="Condition";
				
				 $sql_ins_main="insert into tbl_lot_ldg (yearcode,lotldg_trtype, lotldg_trid, lotldg_trdate, lotldg_lotno, lotldg_crop, lotldg_variety, lotldg_whid, lotldg_binid, lotldg_subbinid, lotldg_opbags, lotldg_opqty, lotldg_trbags, lotldg_trqty, lotldg_balbags, lotldg_balqty, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_got, lotldg_qctestdate, lotldg_gottestdate, orlot, lotldg_gs, lotldg_resverstatus, lotldg_revcomment, lotldg_genpurity, lotldg_srtyp, lotldg_srflg, lotldg_mergerflg, lotldg_unlistflg,plantcode) values('$ycode','BATCHLOTO', '$pid', '$tdate', '$lotno', '$txtcrop', '$txtvariety', '$whid', '$binid', '$subbinid', '$opups', '$opqty', '$tbag', '$tqty', '$balups', '$balqty', '$sstage', '$sstatus', '$moist', '$gemp', '$vchk', '$got1', '$qc', '$gotstatus', '$qctestdate', '$gottestdate', '$orlot', '$gs', '$resverstatus', '$revcomment', '$geneticpurity', '$srtyp', '$srflg', '$mergerflg', '$unlistflg','$plantcode')";
				mysqli_query($link,$sql_ins_main) or die(mysqli_error($link));
	//echo "<br />";					
			}	
		}	
			$sql_ins_main1="insert into tbl_lot_ldg (yearcode,lotldg_trtype, lotldg_trid, lotldg_trdate, lotldg_lotno, lotldg_crop, lotldg_variety, lotldg_whid, lotldg_binid, lotldg_subbinid, lotldg_opbags, lotldg_opqty, lotldg_trbags, lotldg_trqty, lotldg_balbags, lotldg_balqty, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_got, lotldg_qctestdate, lotldg_gottestdate, orlot, lotldg_gs, lotldg_resverstatus, lotldg_revcomment, lotldg_genpurity, lotldg_srtyp, lotldg_srflg, lotldg_mergerflg, lotldg_unlistflg,plantcode) values('$ycode','BATCHLOTC', '$pid', '$tdate', '$batchlotno', '$txtcrop', '$txtvariety', '$wh1', '$bin1', '$subbin1', '0', '0', '$Bags1', '$qty1', '$Bags1', '$qty1', '$sstage', '$sstatus', '$moist', '$gemp', '$vchk', '$got1', '$qc', '$gotstatus', '$qctestdate', '$gottestdate', '$batchorlot', '$gs', '$resverstatus', '$revcomment', '$geneticpurity', '$srtyp', '$srflg', '$mergerflg', '$unlistflg','$plantcode')";
			mysqli_query($link,$sql_ins_main1) or die(mysqli_error($link));
	//	echo "<br />";		
		if($qty2>0)	
		{
			$sql_ins_main2="insert into tbl_lot_ldg (yearcode,lotldg_trtype, lotldg_trid, lotldg_trdate, lotldg_lotno, lotldg_crop, lotldg_variety, lotldg_whid, lotldg_binid, lotldg_subbinid, lotldg_opbags, lotldg_opqty, lotldg_trbags, lotldg_trqty, lotldg_balbags, lotldg_balqty, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_got, lotldg_qctestdate, lotldg_gottestdate, orlot, lotldg_gs, lotldg_resverstatus, lotldg_revcomment, lotldg_genpurity, lotldg_srtyp, lotldg_srflg, lotldg_mergerflg, lotldg_unlistflg,plantcode) values('$ycode','BATCHLOTC', '$pid', '$tdate', '$batchlotno', '$txtcrop', '$txtvariety', '$wh2', '$bin2', '$subbin2', '0', '0', '$Bags2', '$qty2', '$Bags2', '$qty2', '$sstage', '$sstatus', '$moist', '$gemp', '$vchk', '$got1', '$qc', '$gotstatus', '$qctestdate', '$gottestdate', '$batchorlot', '$gs', '$resverstatus', '$revcomment', '$geneticpurity', '$srtyp', '$srflg', '$mergerflg', '$unlistflg','$plantcode')";
			mysqli_query($link,$sql_ins_main2) or die(mysqli_error($link));
		}
	//		echo "<br />";		
	//	exit;
			echo "<script>window.location='home_newbatch.php'</script>";	
	}
	//exit;	
}	
	$a="TSU";
	$sql_code="SELECT MAX(batch_code) FROM tbl_lotbatch  where batch_yearcode='$yearid_id' AND plantcode='".$plantcode."'   ORDER BY batch_code DESC";
...
{ 
	...
	if(document.frmaddDepartment.batchqty.value=="")
	{
		alert("Please enter Batch Lot Qty");
		return false;
	} 
	if(document.frmaddDepartment.txtbatchno.value=="")
	{
		alert("Please select Batch Conversion");
		return false;
	} 
	if(document.frmaddDepartment.txtslqtyg1.value=="")
	{
		alert("Please enter SLOC Qty");
		return false;
	} 
...
function pform()
{
	...
	if(document.frmaddDepartment.sloctyp.value=="")
	{
		alert("Please select Shift Stock To");
		return false;
	}
	if(document.frmaddDepartment.sloctyp.value=="Regular Warehouse")
	{
	...
	if(document.frmaddDepartment.txtlot1.value!="")
	{
		...
		var txtqtyg=document.frmaddDepartment.txtqtyg.value;
		txtqtyg=(Math.round(txtqtyg * 100) / 100).toFixed(2);
		qtyd=(Math.round(qtyd * 100) / 100).toFixed(2);
		...
		//alert(txtqtyg);alert(qtyd);
		if(parseFloat(txtqtyg) != parseFloat(qtyd))
		{
...
winHandle=window.open('getuser_batch_lotno.php?crop='+crop+'&variety='+variety,'WelCome','top=170,left=180,width=420,height=350,scrollbars=yes');
if(winHandle==null){
...
function chkvalidity(valval)
{
	if(valval!="")
	{
		if(valval=="E")
		{
			document.frmaddDepartment.txtbatchno.value=document.frmaddDepartment.lotnmo.value;
			document.frmaddDepartment.batchnob.value=parseInt(document.frmaddDepartment.ornob.value);
			document.frmaddDepartment.batchqty.value=parseFloat(document.frmaddDepartment.orqty.value);
			document.frmaddDepartment.batchnob.readOnly=true;
			document.frmaddDepartment.batchnob.style.backgroundColor='#ECECEC';
			document.frmaddDepartment.batchqty.readOnly=true;
			document.frmaddDepartment.batchqty.style.backgroundColor='#ECECEC';
			//alert(document.frmaddDepartment.oqty.value); alert(document.frmaddDepartment.batchqty.value); 
		}
		else if(valval=="P")
		{
			document.frmaddDepartment.txtbatchno.value=document.frmaddDepartment.lotnmb.value;
			document.frmaddDepartment.batchnob.value="";
			document.frmaddDepartment.batchqty.value="";
			document.frmaddDepartment.batchnob.readOnly=false;
			document.frmaddDepartment.batchnob.style.backgroundColor='#FFFFFF';
			document.frmaddDepartment.batchqty.readOnly=false;
			document.frmaddDepartment.batchqty.style.backgroundColor='#FFFFFF';
		}
		else
		{
			document.frmaddDepartment.txtbatchno.value="";
			document.frmaddDepartment.batchnob.value="";
			document.frmaddDepartment.batchqty.value="";
			document.frmaddDepartment.batchnob.readOnly=false;
			document.frmaddDepartment.batchnob.style.backgroundColor='#FFFFFF';
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#fa8283" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#fa8283" style="border-bottom:solid; border-bottom-color:#fa8283" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Lot Number Batch Generation </td>
	    </tr></table></td>
		...
		<form id="mainform" name="frmaddDepartment" method="post" action="<?php $_SERVER['PHP_SELF']; ?>" onSubmit="return mySubmit();" > 
	 	<input name="frm_action" value="submit" type="hidden"> 
	  	...
	  	<input type="hidden" name="txtdate" value="<?php echo date("d-m-Y");?>" />
	   	<input type="hidden" name="seedstage" value="Condition" />
...
<tr class="Light" height="25">
   <td width="378"  align="right"  valign="middle" class="tblheading">&nbsp;Crop&nbsp;</td>
...
<option value="" selected>--Select Crop--</option>
 ...
 <?php 
$itemqry=mysqli_query($link,"select varietyid, popularname from tblvariety where actstatus='Active' and vertype='PV'") or die(mysqli_error($link));
?>            
         <tr class="Dark" height="30">
<td align="right" valign="middle" class="tblheading">Variety&nbsp;</td>
<td width="566" align="left" valign="middle" class="tbltext" id="vitem"  colspan="3">&nbsp;<select class="tbltext" name="txtvariety" id="itm" style="width:170px;" >
<option value="" selected>--Select Variety--</option>
</select>&nbsp;<font color="#FF0000">*</font>&nbsp;</td>
</tr><input type="hidden" name="itmdchk" value="" />
 <tr class="Light" height="25">
            <td width="378" height="24"  align="right"  valign="middle" class="tblheading">Lot No.&nbsp;</td>
           <td align="left"  valign="middle" colspan="3"  class="tbltext">&nbsp;<input name="txtlot1" id="smt" type="text" class="tbltext" value="" size="20" maxlength="20" style="background-color:#CCCCCC" readonly="true"  />&nbsp;<font color="#FF0000">*</font>&nbsp;&nbsp;<a href="Javascript:void(0);" onclick="openslocpop();">Select</a></td>
         </tr>	
</table>
<br />
<div id="maindiv">
...
<tr >
<td valign="top" align="right"><a href="home_newbatch.php"><img src="../images/cancel.gif" border="0"  style="display:inline;cursor:pointer;" onclick="return confirm('Do You wish to Cancel this Transaction?');"/></a>&nbsp;<input name="Submit" type="image" src="../images/submit.gif" alt="Submit Value" onClick="return mySubmit();"  border="0" style="display:inline;cursor:pointer;" />&nbsp;&nbsp;</td>
</tr>
</table>
</td><td width="30"></td>
</tr>
