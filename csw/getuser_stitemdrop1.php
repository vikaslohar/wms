...
$sql_month=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_lotno='".$crop."' and lotldg_variety='".$b."' and lotldg_lotno='".$c."'and lotldg_sstage='".$tp."' order by lotldg_sstatus")or die(mysqli_error($link));
...
<?php
$sql_tbl_sub=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_lotno='".$c."'") or die(mysqli_error($link));
...
<td align="right"  valign="middle" class="tbltext">&nbsp;<input type="checkbox" name="sstatus"  readonly="true"<?php $p1_array=explode("/",$row_tbl_sub['lotldg_sstatus']);
//echo $row_tbl_sub['lotldg_sstatus']=="F";
			$i=0;
			$p1=array();
			foreach($p1_array as $val1)
				{
...
<td align="right"  valign="middle" class="tbltext" ><?php $p1_array=explode("/",$row_tbl_sub['lotldg_sstatus']);
 //echo $row_tbl_sub['lotldg_sstatus']=="Q";
			...
			foreach($p1_array as $val1)
				...
				if($i !=0) {?><input type="checkbox" name="sstatus"  readonly="true" checked="checked" disabled="disabled" value="Q" /><?php } else {?><input type="checkbox" name="sstatus"  readonly="true" value="Q" /><?php } ?>&nbsp;</td>
...
//}
?>
<!--<table align="center" border="1" cellspacing="0" cellpadding="0" width="574" bordercolor="#fa8283" style="border-collapse:collapse">
