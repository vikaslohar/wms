<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$p_id=$pid;
		$s_sub="delete from tbl_csw where arrival_id='".$pid."'";
		...
		foreach($pl as $val)
		{
			if($val<>"")
			{
				$sql_sub="insert into tbl_csw (crop, variety, pp, moist, gemp, got, arr_role, lotno , arrival_id, arrival_date, arr_code) values ('$txtcrop', '$txtvariety', '$chk1', '$chk2', '$chk3', '$chk4', '$logid', '$val', '$p_id', '$tdate1', '$dotage')";
...
<?php 
  $tid=$pid;
$sql_tbl=mysqli_query($link,"select * from tbl_csw_main where arrival_id='".$tid."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
...
$sql_tbl_sub2=mysqli_query($link,"select * from tbl_csw where arrival_id='".$arrival_id."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
$row_tbl_sub=mysqli_fetch_array($sql_tbl_sub2);
$sql_tbl_sub=mysqli_query($link,"select * from tbl_csw where arrival_id='".$arrival_id."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
...
<form id="mainform" name="frmaddDepartment" method="post" action="<?php $_SERVER['PHP_SELF']; ?>" onsubmit="return mySubmit();"   > 
	 ...
	 <input name="sstage" value="Condition" type="hidden"> 
...
<td width="234"  align="left" valign="middle" class="tbltext" colspan="3">&nbsp;Condition </td>
...
<?php
$sql_month=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where cropname='".$row_tbl_sub['crop']."' and actstatus='Active' order by popularname Asc"); 
?>
...
$sql_m=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='".$row_tbl_sub['crop']."' and lotldg_variety='".$row_tbl_sub['variety']."' AND plantcode='".$plantcode."' and lotldg_sstage='Condition'") or die(mysqli_error($link));
...
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$row_tbl_sub['crop']."' AND plantcode='".$plantcode."' and lotldg_variety='".$row_tbl_sub['variety']."' and lotldg_sstage='Condition' and lotldg_lotno='".$val."'") or die(mysqli_error($link));
$trtr=mysqli_num_rows($sql_issue);
 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' AND plantcode='".$plantcode."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_variety='".$row_tbl_sub['variety']."' and lotldg_sstage='Condition'  and lotldg_lotno='".$val."'") or die(mysqli_error($link));
...
{
$ltchk=0;
$sqltblsub=mysqli_query($link,"select * from tbl_csw where arrival_id='".$arrival_id."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
