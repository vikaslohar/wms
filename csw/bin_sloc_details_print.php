...
<?php
$srno=1;
$sql_tbl=mysqli_query($link,"select lotldg_subbinid, lotldg_variety, lotldg_crop,lotldg_lotno from tbl_lot_ldg where lotldg_whid='".$wid."' AND plantcode='".$plantcode."' and lotldg_binid='".$bid."' group by lotldg_subbinid, lotldg_variety, lotldg_lotno order by lotldg_subbinid") or die(mysqli_error($link));  
//echo $t=mysqli_num_rows($sql_tbl);
while($row_tbl=mysqli_fetch_array($sql_tbl))
{

$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_whid='".$wid."' and lotldg_binid='".$bid."' AND plantcode='".$plantcode."' and lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_variety='".$row_tbl['lotldg_variety']."' and lotldg_crop='".$row_tbl['lotldg_crop']."'  and lotldg_lotno='".$row_tbl['lotldg_lotno']."'") or die(mysqli_error($link));  
...
<?php
$srno=1;
$sql_tbl=mysqli_query($link,"select subbinid, lotldg_variety, lotldg_crop, lotno from tbl_lot_ldg_pack where whid='".$wid."' AND plantcode='".$plantcode."' and binid='".$bid."' group by subbinid, lotldg_variety, lotno order by subbinid") or die(mysqli_error($link));  
//echo $t=mysqli_num_rows($sql_tbl);
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
