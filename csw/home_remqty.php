...
$sql_tbl=mysqli_query($link,"select * from tbl_cswrem where plantcode='".$plantcode."' and  logid='$logid' and  rswrem_tflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	 $arrival_id=$row_tbl['rswrem_id'];	
	
	$s_sub="delete from tbl_cswrem_sub where plantcode='".$plantcode."' and  rswrem_id='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_cswremsub_sub where plantcode='".$plantcode."' and  rswrem_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
}

	$s_sub="delete from tbl_cswrem where plantcode='".$plantcode."' and  logid='$logid' and  rswrem_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
<title>CSW - Transaction - CSW - Condition Seed Release</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#fa8283" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#fa8283" style="border-bottom:solid; border-bottom-color:#fa8283" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Condition Seed Release</td>
	    </tr></table></td>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_cswrem where plantcode='".$plantcode."' and  rswrem_tflg=1  order by rswrem_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);

//$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_qctest where plantcode='".$plantcode."' and  bflg=1   and qcflg=0");
//$total_results2 = mysqli_fetch_array($total_results3);
//$total_results = $total_results2[0]; 

$query = "select * from tbl_cswrem where plantcode='".$plantcode."' and  rswrem_tflg=1  order by rswrem_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_cswrem where plantcode='".$plantcode."' and  rswrem_tflg=1  order by rswrem_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

 $total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_cswrem where plantcode='".$plantcode."' and  rswrem_tflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Condition Seed Release</td>
</tr>
</table>
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_tblsub_sub=mysqli_query($link,"select * from tbl_cswremsub_sub where plantcode='".$plantcode."' and rswrem_id='".$arrival_id."' and rswremsub_id='".$row_tbl_sub['rswremsub_id']."'") or die(mysqli_error($link));
...
<tr class="Light">
         <td width="3%" align="center" valign="middle" class="tblheading"><?php echo $srno;?></td>
		   <td width="8%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
		     <td width="13%" align="center" valign="middle" class="tblheading"><?php echo "CR".$row_arr_home['rswrem_code']."/".$yearid_id."/".$lgnid;?></td>
