<?php
	...
	$sql_arr=mysqli_query($link,"select * from tbl_csw_main where arrival_id='".$pid."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
	while($row_arr=mysqli_fetch_array($sql_arr))
	{
	
	$tdate1=$row_arr['arrival_date'];
	$ststus=$row_arr['status'];	
	$sql_arrsub=mysqli_query($link,"select * from tbl_csw where arrival_id='".$pid."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
	...
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		...
		$sql_sub_sub="insert into tbl_qctest(lotno, oldlot, crop, variety, srdate, sampleno, state, qc, trstage, yearid, logid,plantcode) values('$lotno', '$zx[0]', '$crop', '$variety', '$tdate1', '$ncode1', '$state', 'UT', 'Condition', '$yearid_id', '$logid','$plantcode')";
	...
	$sql_code="SELECT MAX(arr_code) FROM tbl_csw_main  WHERE plantcode='".$plantcode."' ORDER BY arr_code DESC";
	...
	$sql_main="update tbl_csw_main set arrtrflag=1 where arrival_id ='$pid'";
...
<title>CSW - Transaction - QC Sampling Request- Preview</title>
...
function openslocpopprint()
{
	...
	if(document.frmaddDepartment.txtitem.value!="")
	{
		...
		//alert("remarks");
		winHandle=window.open('qcsmp_print.php?itmid='+itm+'&txtvariety='+remarks,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
		if(winHandle==null){
...
$sql_tbl=mysqli_query($link,"select * from tbl_csw_main where arr_role='".$logid."'  and arrival_id='".$tid."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
$row_tbl=mysqli_fetch_array($sql_tbl);
 $tot=mysqli_num_rows($sql_tbl);		
$arrival_id=$row_tbl['arrival_id'];

$sqltblsub2=mysqli_query($link,"select * from tbl_csw where arrival_id='".$arrival_id."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
$rowtblsub=mysqli_fetch_array($sqltblsub2);
...
<td width="234"  align="left" valign="middle" class="tbltext">&nbsp;Condition Seed</td>


<td width="269" align="right" valign="middle" class="tblheading">&nbsp;Duration since last QC test&nbsp;</td>
<td width="179" align="left" valign="middle" class="tbltext">&nbsp;<?php echo "More than ".$rowtblsub['arr_code']." Days";?>&nbsp;</td>
</tr>
