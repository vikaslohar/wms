...
$lotn=$f;
$whtype='';
$whquery=mysqli_query($link,"select whid, perticulars, whtype from tbl_warehouse where plantcode='".$plantcode."' and whid='$whval' order by perticulars") or die(mysqli_error($link));
...
{ 
	...
	if($tpmflg==0)
	{
		...
		{	
			$tpflg=1;
			$sql_issuetblg2=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_id='".$sa."' and lotldg_balqty > 0") or die(mysqli_error($link));  
		...
		{	
			$tpflg=0;
			$s_good=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_subbinid='".$a."' ") or die(mysqli_error($link));
			//echo $r_good=mysqli_num_rows($s_good);
			while($row_issueg=mysqli_fetch_array($s_good))
			{ 
				$sql_lot=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where plantcode='".$plantcode."' and lotldg_subbinid='".$a."' and lotldg_variety='".$row_issueg['lotldg_variety']."'") or die(mysqli_error($link));
	...
	if($tpflg>0)$pflg=1;
		...
		$s_goodpack=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where plantcode='".$plantcode."' and subbinid='".$a."' and lotldg_variety!='".$b."'") or die(mysqli_error($link));
		...
		{ 
			...
			$sql_lotpack=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where plantcode='".$plantcode."' and subbinid='".$a."' and lotldg_variety='".$row_issuegpack['lotldg_variety']."'") or die(mysqli_error($link));
			while($row_lotpack=mysqli_fetch_array($sql_lotpack))
			{ 
			
				$sql_issueg1pack=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='".$plantcode."' and subbinid='".$a."' and lotldg_variety='".$row_issuegpack['lotldg_variety']."' and lotno='".$row_lotpack['lotno']."'") or die(mysqli_error($link));
				$row_issueg1pack=mysqli_fetch_array($sql_issueg1pack); 
				
				$sql_issuetblgpack=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='".$plantcode."' and lotdgp_id='".$row_issueg1pack[0]."' and balqty > 0") or die(mysqli_error($link)); 
		...
		{	
			$tpflg=1;
			$sql_issuetblg2pack=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='".$plantcode."' and lotdgp_id='".$sapack."' and balqty > 0") or die(mysqli_error($link));  
			$row_issuetblg_new2pack=mysqli_fetch_array($sql_issuetblg2pack);
			$sql_crop_new2pack=mysqli_query($link,"select * from tblcrop where plantcode='".$plantcode."' and cropid='".$row_issuetblg_new2pack['lotldg_crop']."'") or die(mysqli_error($link));
			...
			{		
				$sql_veriety2pack=mysqli_query($link,"select * from tblvariety where plantcode='".$plantcode."' and varietyid='".$row_issuetblg_new2pack['lotldg_variety']."' and actstatus='Active'") or die(mysqli_error($link));
		...
		{	
			...
			$s_goodpack=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where plantcode='".$plantcode."' and subbinid='".$a."' ") or die(mysqli_error($link));
			//$s_good=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where subbinid='".$a."' and lotldg_variety!='".$b."'") or die(mysqli_error($link));
			//echo $r_good=mysqli_num_rows($s_good);
			$cnt=0;$sapack='';
			while($row_issuegpack=mysqli_fetch_array($s_goodpack))
...
{ 	
		$sa="";$cnt=0; //echo $b."  ";
		$s_good=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_subbinid='".$a."' and lotldg_variety!='".$b."' and lotldg_crop='".$crpval."'") or die(mysqli_error($link));
		 $r_good=mysqli_num_rows($s_good);
		while($row_issueg=mysqli_fetch_array($s_good))
		{ 
			$sql_lot=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_subbinid='".$a."' and lotldg_variety='".$row_issueg['lotldg_variety']."' and lotldg_crop='".$crpval."'") or die(mysqli_error($link));
			while($row_lot=mysqli_fetch_array($sql_lot))
			{ 
				$sql_issueg1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_subbinid='".$a."' and lotldg_variety='".$row_issueg['lotldg_variety']."' and lotldg_lotno='".$row_lot['lotldg_lotno']."' and lotldg_crop='".$crpval."'") or die(mysqli_error($link));
				$row_issueg1=mysqli_fetch_array($sql_issueg1); 
				
				$sql_issuetblg=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_id='".$row_issueg1[0]."' and lotldg_balqty > 0 and lotldg_crop='".$crpval."'") or die(mysqli_error($link)); 
				$totno=mysqli_num_rows($sql_issuetblg);
		...
		{	
			$tpflg=1;
			$sql_issuetblg2=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_id='".$sa."' and lotldg_balqty > 0 and lotldg_crop='".$crpval."'") or die(mysqli_error($link));  
			$row_issuetblg_new2=mysqli_fetch_array($sql_issuetblg2);
			$sql_crop_new2=mysqli_query($link,"select * from tblcrop where cropid='".$row_issuetblg_new2['lotldg_crop']."'") or die(mysqli_error($link));
			$row_crop_new2=mysqli_fetch_array($sql_crop_new2);
	...
	if($tpflg>0)$pflg=1;
		...
		$s_goodpack=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  subbinid='".$a."' and lotldg_variety!='".$b."'  and lotldg_crop='".$crpval."'") or die(mysqli_error($link));
		...
		{ 
			...
			$sql_lotpack=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  subbinid='".$a."' and lotldg_variety='".$row_issuegpack['lotldg_variety']."' and lotldg_crop='".$crpval."'") or die(mysqli_error($link));
			while($row_lotpack=mysqli_fetch_array($sql_lotpack))
			{ 
			
				$sql_issueg1pack=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  subbinid='".$a."' and lotldg_variety='".$row_issuegpack['lotldg_variety']."' and lotno='".$row_lotpack['lotno']."' and lotldg_crop='".$crpval."'") or die(mysqli_error($link));
				$row_issueg1pack=mysqli_fetch_array($sql_issueg1pack); 
				
				$sql_issuetblgpack=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  lotdgp_id='".$row_issueg1pack[0]."' and balqty > 0 and lotldg_crop='".$crpval."'") or die(mysqli_error($link)); 
				$totnopack=mysqli_num_rows($sql_issuetblgpack);
		...
		{	
			...
			$sql_issuetblg2pack=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  lotdgp_id='".$sapack."' and balqty > 0 and lotldg_crop='".$crpval."'") or die(mysqli_error($link));  
			$row_issuetblg_new2pack=mysqli_fetch_array($sql_issuetblg2pack);
			$sql_crop_new2pack=mysqli_query($link,"select * from tblcrop where cropid='".$row_issuetblg_new2pack['lotldg_crop']."'") or die(mysqli_error($link));
			$row_crop_new2pack=mysqli_fetch_array($sql_crop_new2pack);
