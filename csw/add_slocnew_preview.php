<?php
	...
	{	
	//exit; 
		$sql_arr=mysqli_query($link,"select * from tbl_sloc_csw where slid='".$pid."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
		...
		$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' AND plantcode='".$plantcode."' and lotldg_variety='".$itemid."' and lotldg_lotno='".$lotno."' ") or die(mysqli_error($link));
...
{
	$sql_issueg1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$subbinid."' AND plantcode='".$plantcode."' and binid='".$row_issueg['binid']."' and whid='".$row_issueg['whid']."' and lotno='".$row_issueg['lotno']."'") or die(mysqli_error($link));
...
while($row_lot=mysqli_fetch_array($sql_lot))
{   
	$sql_issueg1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$subbinid."' and lotldg_binid='".$row_issueg['lotldg_binid']."' AND plantcode='".$plantcode."' and lotldg_whid='".$row_issueg['lotldg_whid']."' and lotldg_lotno='".$row_issueg['lotldg_lotno']."'") or die(mysqli_error($link));
	$row_issueg1=mysqli_fetch_array($sql_issueg1); 
	
	$sql_issuetblg=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issueg1[0]."' AND plantcode='".$plantcode."' and lotldg_balqty > 0") or die(mysqli_error($link)); 
	$totnog=mysqli_num_rows($sql_issuetblg);
	if($totnog > 0)
	{
	  $cntg++;
  ...
  if($cntg==0)
  {
	...
	mysqli_query($link,$sql_itmg) or die(mysqli_error($link));		
		...
		$sql_sloc_sub22=mysqli_query($link,"select * from tbl_sloc_csw_sub where slocid='".$pid."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
...
$cntg=0;
	...
	$s_chk=mysqli_query($link,"SELECT * FROM tbl_sloc_csw where yearcode='$yearid_id' AND plantcode='".$plantcode."'") or die (mysqli_error($link));
	$r_chk=mysqli_num_rows($s_chk);
	if($r_chk > 0)
	$sql_code="SELECT MAX(scode) FROM tbl_sloc_csw where yearcode='$yearid_id' AND plantcode='".$plantcode."'ORDER BY scode DESC";
	else
	$sql_code="SELECT MAX(scode) FROM tbl_sloc_csw WHERE plantcode='".$plantcode."' ORDER BY scode DESC";
	...
	$sql_main="update tbl_sloc_csw set supflg=1, scode=$code  where slid='".$pid."'";

	$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
	//exit;
 	
			echo "<script>window.location='select_slocnew_op.php?p_id=$pid'</script>";	
}
		

	
/*	$a="c";
	$sql_code="SELECT MAX(code) FROM tbl_sloc_csw where  yearcode='$yearid_id' ORDER BY code DESC";
	$res_code=mysqli_query($link,$sql_code)or die(mysqli_error($link));
...
<?php
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$c."' and lotldg_variety='".$f."' AND plantcode='".$plantcode."' and lotldg_lotno='".$a."'") or die(mysqli_error($link));
...
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' AND plantcode='".$plantcode."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_variety='".$f."'") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 
...
$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$row_issuetbl['lotldg_subbinid']."' and binid='".$row_issuetbl['lotldg_binid']."' AND plantcode='".$plantcode."' and whid='".$row_issuetbl['lotldg_whid']."'") or die(mysqli_error($link));
...
$sr=1;
$totups=0; $totqty=0;
$sql_sloc_sub=mysqli_query($link,"select * from tbl_sloc_csw_sub where slocid='".$pid."' AND plantcode='".$plantcode."' order by slocsubid") or die (mysqli_error($link));
...
$sql_binn=mysqli_query($link,"select binname from tbl_bin where binid='".$row_sloc_sub['binid']."' AND plantcode='".$plantcode."' and whid='".$row_sloc_sub['whid']."'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname'];

$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$row_sloc_sub['subbinid']."' AND plantcode='".$plantcode."' and binid='".$row_sloc_sub['binid']."' and whid='".$row_sloc_sub['whid']."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
$subbinn=$row_subbinn['sname'];
