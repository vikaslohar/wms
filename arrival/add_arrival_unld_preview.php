...
$sql_main="update tblarrival_unld set remarks='$remarks' where arrival_id = '$pid'";
$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
	...
	$sql_arr=mysqli_query($link,"select * from tblarrival_unld where arrival_id='".$pid."' ") or die(mysqli_error($link));
	while($row_arr=mysqli_fetch_array($sql_arr))
	{
		...
		$sql_code23="SELECT MAX(arrival_code) FROM tblarrival where yearcode='".$row_arr['yearcode']."' and plantcode='$plantcode' and arrival_type='Fresh Seed with PDN' ORDER BY arrival_code DESC";
		$res_code23=mysqli_query($link,$sql_code23)or die(mysqli_error($link));
		if(mysqli_num_rows($res_code23) > 0)
		{
			$row_code23=mysqli_fetch_row($res_code23);
			$t_code23=$row_code23['0'];
			$code23=$t_code23+1;
		}
		else
		{
		...
		$sql_main="insert into tblarrival (yearcode,arrival_type, arrival_code, arrival_date, grnno, dc_date, disp_date,  nolot, tmode, trans_name, trans_lorryrepno, trans_vehno, trans_paymode, courier_name, docket_no, pname_byhand, remarks, arr_role, dcno, plantcode)values('".$row_arr['yearcode']."', 'Fresh Seed with PDN', '".$code23."', '$tdate', '".$row_arr['dcno']."', '".$row_arr['dc_date']."','".$row_arr['disp_date']."', '".$row_arr['nolot']."', '".$row_arr['tmode']."', '".$row_arr['trans_name']."', '".$row_arr['trans_lorryrepno']."', '".$row_arr['trans_vehno']."', '".$row_arr['trans_paymode']."', '".$row_arr['courier_name']."', '".$row_arr['docket_no']."', '".$row_arr['pname_byhand']."', '".$row_arr['remarks']."', '".$row_arr['arr_role']."', '".$row_arr['dcno']."', '$plantcode')";
		...
		$sqlarrsub=mysqli_query($link,"select * from tblarrival_sub_unld where arrival_id='".$pid."' ") or die(mysqli_error($link));
			...
			if($variety11!="" && $variety11==$vrnew11)
			{
				...
				if($tot_spcdec > 0)
				{
					...
					if($row_spcdec['variety']!="")
					{
						...
						$sqltblarsub24="update tblarrival_sub_unld set lotvariety='$itemname' where arrival_id='".$pid."' and arrsub_id='".$rowarrsub['arrsub_id']."'";	
	...
	$sql_arrsub=mysqli_query($link,"select * from tblarrival_sub_unld where arrival_id='".$pid."' ") or die(mysqli_error($link));
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		$txtdcqty=$row_arrsub['qty'];
		$txtdcnob=$row_arrsub['act1'];
		
		$actnob=0; $actqty=0; $differencenob=0; $differenceqty=0;
		$sql_item=mysqli_query($link,"select * from tblarrsub_sub_unld where arrival_id='".$pid."' and arrsub_id='".$row_arrsub['arrsub_id']."' order by arrsubsub_id") or die(mysqli_error($link));
		...
		$txtactqty=$actqty;
		$txtactnob=$actnob;
		$differencenob=$actnob-$txtdcnob;
		$differenceqty=$actqty-$txtdcqty; 
		
		
		$sql_sub="insert into tblarrival_sub (arrival_id, organiser, pdnno, pdndate, spcodef, spcodem, lotcrop, lotvariety, ploc, pper, farmer, plotno, gi, harvestdate, got, qty, act, diff, qty1, act1, diff1, moisture, vchk, qc, remarks, sstage, sstatus, lotno, old, got1, sample,qcsample,orlot,gssample,prodtype, lotstate, leduration, leupto, prodgrade, plantcode) values('$mainid', '".$row_arrsub['organiser']."', '".$row_arrsub['pdnno']."', '".$row_arrsub['pdndate']."', '".$row_arrsub['spcodef']."', '".$row_arrsub['spcodem']."', '".$row_arrsub['lotcrop']."', '".$row_arrsub['lotvariety']."', '".$row_arrsub['ploc']."', '".$row_arrsub['pper']."', '".$row_arrsub['farmer']."', '".$row_arrsub['plotno']."', '".$row_arrsub['gi']."', '".$row_arrsub['harvestdate']."', '".$row_arrsub['got']."', '$txtdcqty', '$txtactqty', '$differenceqty', '$txtdcnob', '$txtactnob', '$differencenob', '".$row_arrsub['moisture']."', '".$row_arrsub['vchk']."', '".$row_arrsub['qc']."', '".$row_arrsub['remarks']."', '".$row_arrsub['sstage']."', '".$row_arrsub['sstatus']."', '".$row_arrsub['lotno']."', '".$row_arrsub['old']."', '".$row_arrsub['got1']."', '".$row_arrsub['sample']."','".$row_arrsub['qcsample']."','".$row_arrsub['orlot']."','".$row_arrsub['gssample']."','".$row_arrsub['prodtype']."','".$row_arrsub['lotstate']."','".$row_arrsub['leduration']."','".$row_arrsub['leupto']."','".$row_arrsub['prodgrade']."', '$plantcode')";
		...
		$wfsloc='';
		$sql_arrsub_sub=mysqli_query($link,"select * from tblarr_sloc_unld where arr_tr_id='".$pid."' and arr_id='".$row_arrsub['arrsub_id']."' and plantcode='$plantcode'") or die(mysqli_error($link));
		while($row_arrsub_sub=mysqli_fetch_array($sql_arrsub_sub))
		{
			...
			$sql_binn=mysqli_query($link,"select binname from tbl_bin where binid='".$binid."' and whid='".$whid."' and plantcode='$plantcode'") or die(mysqli_error($link));
			$row_binn=mysqli_fetch_array($sql_binn);
			$binn=$row_binn['binname']."/";
			
			$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$subbinid."' and binid='".$binid."' and whid='".$whid."' and plantcode='$plantcode'") or die(mysqli_error($link));
			...
			$sqlsubsubsloc="insert into tblarr_sloc (arr_type, arr_tr_id, arr_id, whid, binid, subbin, rowid, qty, bags, balqty, balbags, lotcrop, lotvariety, plantcode) values('Fresh Seed with PDN', '$mainid', '$subid', '$whid', '$binid', '$subbinid',  '$rowid1', '$qty', '$ups', '$qty', '$ups', '$crop', '$variety', '$plantcode')";
			mysqli_query($link,$sqlsubsubsloc) or die(mysqli_error($link));
		...
		$prodloc=$row_lotimp['lotploc']." ".$row_lotimp['lotstate'];
		...
		{
			$row_focusdbcode1=$res_focusdbcode1;
		...
		*/
		...
		$sqlsubsub13="insert into tbl_learchive (lea_lotno, lea_stage, lea_duration, lea_upto, lea_date, lea_module, lea_logid, plantcode) values( '$lotno' ,'$stage', '$leduration','$ledate', '$tdate', 'Arrival', '$logid', '$plantcode' )";
		...
		if($row_arrsub['qc']=="UT")
		{
			$sql_sub_sub1244="insert into tbl_qctest(pp, moist, lotno, srdate, crop, variety, sampleno, trstage, qc, state, gs, oldlot, yearid,logid, plantcode) values ('$vchk', '$moist', '$lotno', '$tdate', '$classid', '$itemid', '$ncode1', '$stage', '$qc', '$state',1 ,'$gln', '$yearid_id','$logid', '$plantcode')";
			mysqli_query($link,$sql_sub_sub1244) or die(mysqli_error($link));
		}
		if($got1=="UT")
		{
			$sql_sub_sub1245="insert into tbl_gottest(gottest_got, gottest_lotno, gottest_srdate, gottest_crop, gottest_variety, gottest_sampleno, gottest_trstage, gottest_oldlot, yearid, logid, plantcode) values ('$got1', '$lotno', '$tdate', '$classid', '$itemid', '$ncode1', '$stage', '$gln', '$yearid_id','$logid', '$plantcode')";
			mysqli_query($link,$sql_sub_sub1245) or die(mysqli_error($link));
		}
		//exit;
		$sql_sub_upd="update tbllotimp set lotimpflg=1, trid='".$mainid."' where lotnumber='".$oldlotno."'";
...
$fld="";		
$sql_tbl_arsub=mysqli_query($link,"select distinct organiser from tblarrival_sub where arrival_id='".$mainid."' and organiser!='$fld' and organiser!='NULL' and plantcode='$plantcode'") or die(mysqli_error($link));
$arsubtbltot=mysqli_num_rows($sql_tbl_arsub);

while($row_tbl_arsub=mysqli_fetch_array($sql_tbl_arsub))
{
$ncode=$ncode+1;
 $sqltblarsub="update tblarrival_sub set ncode='$ncode' where arrival_id='".$mainid."' and organiser='".$row_tbl_arsub['organiser']."' and plantcode='$plantcode' and (ncode='' or ncode='NULL' or ncode IS NULL)";
$cxcx=mysqli_query($link,$sqltblarsub) or die(mysqli_error($link));
}

$sql_tbl_arsub1=mysqli_query($link,"select distinct farmer from tblarrival_sub where arrival_id='".$mainid."' and plantcode='$plantcode' and (organiser='$fld' or organiser='NULL')") or die(mysqli_error($link));
$arsubtbltot1=mysqli_num_rows($sql_tbl_arsub1);
while($row_tbl_arsub1=mysqli_fetch_array($sql_tbl_arsub1))
{
$ncode=$ncode+1;
 $sqltblarsub1="update tblarrival_sub set ncode='$ncode' where arrival_id='".$mainid."' and farmer='".$row_tbl_arsub1['farmer']."' and plantcode='$plantcode' and (ncode='' or ncode='NULL' or ncode IS NULL)";
$cxcx1=mysqli_query($link,$sqltblarsub1) or die(mysqli_error($link));
}		
//echo "select farmer,ncode,lotcrop from tblarrival_sub where arrival_id='".$mainid."'";
$sql_tbl_arsubfrn2=mysqli_query($link,"select farmer,ncode,lotcrop from tblarrival_sub where arrival_id='".$mainid."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sqltblwfsub="update tbl_frn set wffrn_frnno='".$row_tbl_arsubfrn2['ncode']."', wffrn_narration='".$narration."' where wffrn_arrid='".$mainid."' and wffrn_farmername='".$row_tbl_arsubfrn2['farmer']."' ";
$cxwfcx=mysqli_query($connnew,$sqltblwfsub) or die(mysqli_error($connnew));

}	
//echo "<br />";	
 	$sql_main="update tblarrival set arrtrflag=1, arr_code=$code where arrival_id='$mainid'";
	$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
	
	$sql_main22="update tblarrival_unld set arrtrflag=1, arr_code=$code, unldarr_trid='$mainid' where arrival_id='$pid'";
	$a12345622=mysqli_query($link,$sql_main22) or die(mysqli_error($link));
//exit;
...
{ 
	...
	if(document.frmaddDepartment.xflg.value>0)
	{
		alert("You have not updated SLOC for One or more Lots");
		//document.frmaddDepartment.txtcla.focus();
		return false;
	}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" style="border-bottom:solid; border-bottom-color:#F1B01E" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Fresh Seed Arrival with PDN - Unloading - Preview</td>
	    </tr></table></td>
...
$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub_unld where arrival_id='".$arrival_id."'  order by arrsub_id") or die(mysqli_error($link));
...
if($tot_sloc=mysqli_num_rows($sql_sloc)==0)$xflg=1;
...
<td width="656" align="left"  valign="middle" class="smalltbltext">&nbsp;<?php echo $row_tbl['remarks'];?></td>
</tr>
</table>

<table align="center" width="950" cellpadding="5" cellspacing="5" border="0" >
<tr >
<td valign="top" align="right"><a href="add_arrival_unld.php?p_id=<?php echo $pid;?>"><img src="../images/edit.gif" border="0"style="display:inline;cursor:Pointer;" /></a>&nbsp;&nbsp;<!--<a href="Javascript:void(0)" onclick="openslocpopprint();"><img src="../images/printpreview.gif" border="0"style="display:inline;cursor:Pointer;" /></a>&nbsp;&nbsp;--><input name="Submit" type="image" src="../images/finalsubmit.gif" alt="Submit Value"  border="0" style="display:inline;cursor:Pointer;" tabindex="" onClick="return mySubmit();">&nbsp;&nbsp; <input type="hidden" name="xflg" value="<?php echo $xflg;?>" /></td>
</tr>
</table>
</td><td width="30"></td>
</tr>
