...
$sql_tbl=mysqli_query($link, "select * from tblarrival where arrival_id='".$trid."'") or die(mysqli_error($link));
$row_tbl=mysqli_fetch_array($sql_tbl);		
$arrival_id=$row_tbl['arrival_id'];
$truckno=$row_tbl['trans_vehno'];
//echo "select COUNT(*) from tblarrsub_sub_unld where arrival_id='".$arrival_id."' ";
$sql_class12=mysqli_query($link, "select COUNT(*) from tblarrsub_sub_unld where arrival_id='".$arrival_id."' order by grosswt ASC ") or die(mysqli_error($link));
$row_class12=mysqli_fetch_array($sql_class12);

$sql_class13=mysqli_query($link, "select sum( qty ) from tblarrival_sub where arrival_id='".$arrival_id."' ") or die(mysqli_error($link));
...
$sql_class1=mysqli_query($link, "select sum( grosswt ) , sum( tarewt ) , sum( netwt ) from tblarrsub_sub_unld where arrival_id='".$arrival_id."' and arrsub_id='".$row_tbl_sub['arrsub_id']."' order by grosswt ASC ") or die(mysqli_error($link));
...
$totalqty=$row_tbl_sub['qty1'];
$tareqty=$row_tbl_sub['tarewt'];
$netqty=$row_tbl_sub['act'];
$dcqty=$row_tbl_sub['qty'];
$diffqty=$netqty-$row_tbl_sub['qty'];
$diffqty=round($diffqty,2);

$dcbag=$row_tbl_sub['act1'];
$bagdiff=$row_tbl_sub['qty1']-$dcbag;
...
$pathtosave=$_SERVER['DOCUMENT_ROOT']."/wms-d/pdffiles/";
