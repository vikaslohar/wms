...
if($z1 == 0)
{
  $sql_main="insert into tblarrival_unld (yearcode,arrival_type, arrival_code, arrival_date, grnno, dc_date,disp_date,  nolot, tmode, trans_name, trans_lorryrepno, trans_vehno, trans_paymode, courier_name, docket_no, pname_byhand, remarks, arr_role, plantcode)values('$yearid_id', 'Fresh Seed with PDN', '$txtid', '$tdate', '$grn', '$ddate','$hdate12', '$noofpdn', '$txt1', '$txttname', '$txtlrn', '$txtvn', '$txt13', '$txtcname', '$txtdc', '$txtpname', '$txtremarks1', '$logid', '$plantcode')";

if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=mysqli_insert_id($link);

$sql_sub="insert into tblarrival_sub_unld (arrival_id, organiser, pdnno, pdndate, spcodef, spcodem, lotcrop, lotvariety, ploc, pper, farmer, plotno, gi, harvestdate, got, qty, act, diff, qty1, act1, diff1, moisture, vchk, qc, remarks, sstage, sstatus, lotno, old, got1, sample,qcsample,orlot,gssample,prodtype, lotstate, leduration, leupto, prodgrade, plantcode) values('$mainid', '$organi', '$txtpdno', '$pdate', '$spcodef', '$spcodem', '$txtcrop', '$txtvariety', '$txtloc', '$txtprod', '$txtfar', '$txtplot', '$gcode', '$hdate1', '$txtgstat', '$txtdcqty', '$txtactqty', '$txtdiffqty', '$txtdcnob', '$txtactnob', '$txtdiffnob', '$txtmoist', '$txtvisualck', '$qc', '$txtremarks', '$sstage', '$sstatus', '$glotno', '$txtlot1', '$gotstatus', '$gscheckbox','$qc2','$gln','$gs1','$productiontype','$statename','$leduration','$ledate','$prodctiongrade', '$plantcode')";
...
{
$sql_main="update tblarrival_unld set arrival_date='$tdate', grnno='$grn', dc_date='$ddate',disp_date='$hdate12',  nolot='$noofpdn', tmode='$txt1', trans_name='$txttname', trans_lorryrepno='$txtlrn', trans_vehno='$txtvn', trans_paymode='$txt13', courier_name='$txtcname', docket_no='$txtdc', pname_byhand='$txtpname', remarks='$txtremarks1', arr_role='$logid'  where arrival_id = '$z1'";
if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=$z1;
...
?>
  <?php
$sql_tbl=mysqli_query($link,"select * from tblarrival_unld where arr_role='".$logid."' and arrival_type='Fresh Seed with PDN' and arrival_id='".$tid."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_tbl=mysqli_fetch_array($sql_tbl);
 $tot=mysqli_num_rows($sql_tbl);		
$arrival_id=$row_tbl['arrival_id'];

$sqlarrsub=mysqli_query($link,"select * from tblarrival_sub_unld where arrival_id='".$arrival_id."' and plantcode='$plantcode' order by arrsub_id") or die(mysqli_error($link));
...
$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub_unld where arrival_id='".$arrival_id."' and plantcode='$plantcode'") or die(mysqli_error($link));
