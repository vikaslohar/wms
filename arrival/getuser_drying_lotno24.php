...
<title>Opening Stock - Transaction - Existing Lot selection</title>
...
{	
	...
	window.location='getuser_drying_lotno24.php?crop='+val3+'&variety='+val4+'&sqq='+txtlot1+'&tid='+tid;
	return true;	
	}
}	
...
$arrlots="";
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_opspa_sub where opspa_id='".$tid."' and plantcode='$plantcode'") or die(mysqli_error($link));
		...
		if($tot_arrsub > 0)
		{
			while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
			{
				if($arrlots!="")
				{
					$arrlots=$arrlots.",".$row_tbl_sub['orlot'];
				}
				else
				{
					  $arrlots=$row_tbl_sub['orlot'];
				}
			}
		}

$quer3=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropname='".$crop."' order by cropname Asc");
$noticia = mysqli_fetch_array($quer3);
$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where popularname='".$variety."' and actstatus='Active' and vertype='PV' order by popularname Asc"); 
$noticia_item = mysqli_fetch_array($quer4);

$aaaass="select distinct(orlot)  from tbl_lot_ldg where lotldg_crop='".$noticia['cropid']."' and lotldg_variety='".$noticia_item['varietyid']."' and plantcode='$plantcode' $sss";			
$sql_tbl_sub=mysqli_query($link,$aaaass)or die(mysqli_error($link));
...
//echo $ct;
 ...
 // $row['lotldg_lotno'];
 $sql_chk=mysqli_query($link,"select * from tbl_lot_ldg where orlot='".$row['orlot']."' and plantcode='$plantcode' and lotldg_sstage='Condition'") or die(mysqli_error($link));
  $tot_vhk=mysqli_num_rows($sql_chk);
 if($tot_vhk > 0)$ct++;
 //echo $ct;
 if($ct==0)
{ $cnt++;
...
<td align="right"  valign="middle" class="tbltext"><input type="radio" name="loc" value="<?php echo $row['orlot'];?>" onclick="clk(this.value);" />&nbsp;</td>
