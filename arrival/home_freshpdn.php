...
$sql_tbl=mysqli_query($link,"select * from tblarrival where arr_role='".$logid."' and plantcode='$plantcode' and  arrival_type='Fresh Seed with PDN' and arrtrflag=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tblarrival where arr_role='".$logid."' and plantcode='$plantcode' and arrival_type='Fresh Seed with PDN' and arrtrflag=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
function opensendemailpop(arrivalid,flg)
{
winHandle=window.open('selectmailids.php?pid='+arrivalid+'&flg='+flg,'WelCome','top=20,left=50,width=600,height=400,scrollbars=yes');
if(winHandle==null){
alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); }
...
function openpdfgen_new(tid)
{
var scode="AF"+tid;
winHandle=window.open('unloading_slip_mpdf2.php?scode='+scode+'&trid='+tid+'&heading=Complete','WelCome','top=20,left=50,width=50,height=50,scrollbars=yes');
//winHandle=window.open('../pdffiles/RawSeedsUnloadingSheet_AR'+tid+'.pdf','WelCome','top=20,left=50,width=850,height=750,scrollbars=yes');
if(winHandle==null){
alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); }
...
<!--- Table Place Holder --->
<!--<table align="center" border="1" cellspacing="0" cellpadding="0" width="900" bordercolor="#F1B01E" style="border-collapse:collapse">
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#F1B01E" style="border-collapse:collapse">
  ...
  <?php
 	...
 	$query = "SELECT COUNT(*) as num FROM tblarrival where arrival_type='Fresh Seed with PDN' and plantcode='$plantcode' and arrtrflag=1";
	...
	/* Get data. */
	$sql = "select * from tblarrival where arrival_type='Fresh Seed with PDN' and plantcode='$plantcode' and arrtrflag=1 order by arrival_date desc, arr_code desc LIMIT $start, $limit";
...
$sql_arr_home=mysqli_query($link,"select * from tblarrival where arrival_type='Fresh Seed with PDN' and arrtrflag=1 order by arrival_date desc, arr_code desc  LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tblarrival where arrival_type='Fresh Seed with PDN' and arrtrflag=1");
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$send_email=0;$send_email2=0;
	$sqltblsub=mysqli_query($link,"select * from tblarrival_unld where unldarr_trid='".$arrival_id."' and plantcode='$plantcode'") or die(mysqli_error($link));
	if($subttot=mysqli_num_rows($sqltblsub)>0)
	{
		$rowtblsub=mysqli_fetch_array($sqltblsub);
		$arrival_code=$rowtblsub['arrival_code'];
		$send_email=$arrival_id;
		$arrunldid=$rowtblsub['arrival_id'];
	}
	else
	{
		$arrival_code=$row_arr_home['arrival_code'];
		$send_email=$arrival_id;
		$arrunldid=$arrival_id;
	}
...
<tr class="Light">
		 ...
		 <td width="4%" align="center" valign="middle" class="smalltblheading"><?php if($send_email>0){?><a href='Javascript:void(0)' onclick='openpdfgen(<?php echo $arrunldid?>)'>Generate PDF</a><?php } else if($send_email2>0){?><a href='Javascript:void(0)' onclick='openpdfgen_new(<?php echo $arrunldid?>)'>Generate PDF</a><?php } ?><br/><br/><a href='Javascript:void(0)' onclick='openslocpopprint12(<?php echo $arrival_code?>)'>Download PDF</a><br/><br/><?php if($send_email>0){?><a href='Javascript:void(0)' <?php if($emailflg>0){echo "style='color:#FF0000'"; } ?> onclick='opensendemailpop(<?php echo $send_email?>,1)'>E-mail</a><?php } ?></td>
		</tr>
<?php
}
else
