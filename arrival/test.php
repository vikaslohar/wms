<?php
	...
	$sql_arr_home=mysqli_query($link,"select distinct(arrival_id) from tblarrival where arrival_type='Fresh Seed with PDN' order by arrival_id asc") or die(mysqli_error($link));
	...
	if($tot_arr_home >0) 
	{
		...
		{
			$t=0;
			$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row_arr_home['arrival_id']."' order by arrsub_id asc") or die(mysqli_error($link));
