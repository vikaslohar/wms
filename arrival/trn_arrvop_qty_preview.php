<?php
	...
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		...
		$state="P/M/G"."/".$got;	*/
				$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$subbinid."' and lotldg_binid='".$binid."' and lotldg_whid='".$whid."' and lotldg_variety='".$itemid."' and lotldg_crop='".$classid."'") or die(mysqli_error($link));
	...
	$state="";
		...
		if($row_arrsub['qc']=="UT")
		{
			$sql_sub_sub1244="insert into tbl_qctest(pp, moist, lotno, srdate, crop, variety, sampleno, trstage, qc, state, gs, oldlot, yearid,logid) values ('$vchk', '$moist', '$lotno', '$trdate', '$classid', '$itemid', '$ncode1', '$stage', '$qc', '$state',1 ,'$gln', '$yearid_id','$logid')";
			mysqli_query($link,$sql_sub_sub1244) or die(mysqli_error($link));
		}
		if($got1=="UT")
		{
			$sql_sub_sub1244="insert into tbl_gottest(gottest_got, gottest_lotno, gottest_srdate, gottest_crop, gottest_variety, gottest_sampleno, gottest_trstage, gottest_oldlot, yearid, logid) values ('$got1', '$lotno', '$trdate', '$classid', '$itemid', '$ncode1', '$stage', '$gln', '$yearid_id','$logid')";
			mysqli_query($link,$sql_sub_sub1244) or die(mysqli_error($link));
		}
	...
	$sql_code="SELECT MAX(arr_code) FROM tblarrival where yearcode='$yearid_id' and arrival_type='Opening Stock' ORDER BY arr_code DESC";
	$res_code=mysqli_query($link,$sql_code)or die(mysqli_error($link));
		...
		$sql_code1="SELECT MAX(ncode) FROM tblarrival_sub ORDER BY ncode DESC";
	$res_code1=mysqli_query($link,$sql_code1)or die(mysqli_error($link));
...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
 ...
 <?php 
$tid=$pid;
$sql_tbl=mysqli_query($link,"select * from tblarrival where arr_role='".$logid."' and arrival_type='Opening Stock' and arrival_id='".$tid."'") or die(mysqli_error($link));
...
<tr class="tblsubtitle" height="20">
 ...
 $quer_cn=mysqli_query($link,"SELECT * FROM tbl_parameters where plantcode='$plantcode' ");
	...
	$tp1="";
			if($row_cls['pcity'] =="Deorjhal") { $tp1="D";}
			else if($row_cls['pcity'] =="Gomchi") { $tp1="G";}
