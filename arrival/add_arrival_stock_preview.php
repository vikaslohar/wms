...
$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
	...
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		...
		if($tot_qc1=mysqli_num_rows($sql_qc1)>0)
		{
			...
			if($row_qc2['qcstatus']=="UT" || $row_qc2['qcstatus']=="RT") {$qflg=1;}
		}
		if($qflg==0)
		{
			...
			if($row_arrsub['got1']=="UT" || $row_arrsub['got1']=="RT")
			{
			...
			mysqli_query($link,$sql_sub_sub12) or die(mysqli_error($link));
			 ...
			 $sql_sub_upd="update tbl_stlotimp set stlotimp_trflg=1, stlotimp_trid='$pid' where stlotimp_id='".$lotnoid."' and plantcode='$plantcode'";
	...
	$sql_code="SELECT MAX(arr_code) FROM tblarrival where yearcode='$yearid_id' and arrival_type='StockTransfer Arrival' and plantcode='$plantcode' ORDER BY arr_code DESC";
...
<title>Arrival- Transaction - Arrival Stock Transfer Plant - Preview</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" style="border-bottom:solid; border-bottom-color:#F1B01E" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Arrival Stock Transfer - Plant - Preview</td>
	    </tr></table></td>
