...
$sql_tb="select distinct lotldg_subbinid, lotldg_binid, lotldg_whid from tbl_lot_ldg where orlot='".$lotn."' and lotldg_sstage='Raw' and plantcode='$plantcode' order by lotldg_subbinid"; 
$sql_qry=mysqli_query($link,$sql_tb) or die(mysqli_error($link)); 
  
while($row_tbl=mysqli_fetch_array($sql_qry))
{
$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_whid='".$row_tbl['lotldg_whid']."' and lotldg_binid='".$row_tbl['lotldg_binid']."' and lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and orlot='".$lotn."' and lotldg_sstage='Raw' and plantcode='$plantcode' order by lotldg_id desc") or die(mysqli_error($link));  
