...
$sql_issuesub=mysqli_query($link,"select * from tblissue_sub where issuesub_id='".$rid."'") or die(mysqli_error($link));
$row_issuesub=mysqli_fetch_array($sql_issuesub);


$class_sql=mysqli_query($link,"select classification_id, classification from tbl_classification where classification_id='".$row_issuesub['classification_id']."'") or die(mysqli_error($link));
$row_class = mysqli_fetch_array($class_sql);

$item_sql=mysqli_query($link,"select items_id, stores_item, uom from tbl_stores where items_id='".$row_issuesub['item_id']."'") or die(mysqli_error($link));
...
<table align="center" border="1" width="850" cellspacing="0" cellpadding="0" bordercolor="#4ea1e1" style="border-collapse:collapse" > 
<?php
/*$sql_issue=mysqli_query($link,"select distinct stlg_whid, stlg_subbinid, stlg_binid from tbl_stldg_good where stlg_trclassid='".$row_tbl_sub['classification_id']."' and stlg_tritemid='".$row_tbl_sub['items_id']."'") or die(mysqli_error($link));
*/
$sql_tbl_sub=mysqli_query($link,"select * from tblissue_sloc where issue_id='".$rid."'") or die(mysqli_error($link));
...
$sql_issuetbl=mysqli_query($link,"select * from tbl_stldg_good where stlg_id='".$row_tbl_sub['issue_rowid']."'") or die(mysqli_error($link)); 
$row_issuetbl=mysqli_fetch_array($sql_issuetbl);

$wareh=""; $binn=""; $subbinn=""; $sups="";$sqty=0; $slocs=""; $gd=""; $slups=0; $slqty=0;
  
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where whid='".$row_issuetbl['stlg_whid']."' order by perticulars") or die(mysqli_error($link));
...
$sql_issue=mysqli_query($link,"select distinct stlg_whid, stlg_subbinid, stlg_binid from tbl_stldg_good where stlg_trclassid='".$row_issuesub['classification_id']."' and stlg_tritemid='".$row_issuesub['item_id']."'") or die(mysqli_error($link));

while($row_issue=mysqli_fetch_array($sql_issue))
 { 
 
$sql_issue1=mysqli_query($link,"select max(stlg_id) from tbl_stldg_good where stlg_subbinid='".$row_issue['stlg_subbinid']."' and stlg_binid='".$row_issue['stlg_binid']."' and stlg_whid='".$row_issue['stlg_whid']."' and stlg_tritemid='".$row_issuesub['item_id']."'") or die(mysqli_error($link));
...
$sql_issuetbl=mysqli_query($link,"select * from tbl_stldg_good where stlg_id='".$row_issue1[0]."' and stlg_balqty > 0") or die(mysqli_error($link)); 
 ...
 { 
$sql_tbl_sub=mysqli_query($link,"select * from tblissue_sloc where issue_rowid='".$row_issuetbl['stlg_id']."' and issue_id='".$rid."'") or die(mysqli_error($link));
$tot_tblissue=mysqli_num_rows($sql_tbl_sub);
