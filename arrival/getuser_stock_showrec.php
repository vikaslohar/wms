...
$tot_row=0;
$lotqry=mysqli_query($link,"select * from tbl_stlotimp where stlotimp_id ='".$c."' and plantcode='$plantcode'")or die (mysqli_error($link));
...
<td width="147" align="right"  valign="middle" class="tblheading">Seed Status&nbsp;</td>
    <td width="272" align="left"  valign="middle" class="tbltext"   colspan="3">&nbsp;<input name="sstatus" type="text" size="6" class="tbltext" tabindex=""  maxlength="6"  value="" style="background-color:#CCCCCC" readonly="true"><a href="Javascript:void(0);" onclick="openslocpop1();">Select</a></td>
  </tr>
...
$s_good_new11=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where lotldg_lotno='".$row['stlotimp_lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));
//echo $r_good_new=mysqli_num_rows($s_good_new11);
while($row_issueg_new11=mysqli_fetch_array($s_good_new11))
{


	$sql_issueg1_new11=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issueg_new11['lotldg_subbinid']."' and lotldg_binid='".$row_issueg_new11['lotldg_binid']."' and lotldg_whid='".$row_issueg_new11['lotldg_whid']."' and lotldg_lotno='".$row['stlotimp_lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$subbinn=$row_subbinn['sname'];
	...
	$sql_issuetblg_new=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issueg1_new11[0]."' and lotldg_balqty > 0 and plantcode='$plantcode'") or die(mysqli_error($link)); 
