...
if($maintrid == 0)
{
$sql_main="insert into tblarrival (yearcode, arr_type, arrival_code, tr_date, grn_no, received_form, orgniser_id, farmerid, dc_date, dcno, mode_of_transit, tran_name, lr_no, vehicle_no, payment, courier_name, docket_no, name_of_person, no_of_pdn, remarks, arr_role, plantcode) values('$yearid_id', '$arrivaltype', '$txtid', '$dateofarrival', '$grnnumber', '$receivedfrom', '$txtorganizer', '$txtfarmer', '$dcdate', '$txtdcnumber', '$txt11', '$txttname', '$txtlrn', '$txtvn', '$txt13', '$txtcname', '$txtdc', '$txtpname', '$numberofpdn', '$txtremarks', '$logid', '$plantcode')";

if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=mysqli_insert_id($link);

$sql_sub="insert into tblarrival_sub (tr_id, pdn_no, pdn_date, spcode_m, spcode_f, crop_id, variety_id, plocation_id, ppersonal_id, farmer_id, geo_index, plot_no, harvestdate, got_status, qty_asper_pdn, no_bags_pdn, qty_actual, noof_bagsactual, qty_diff, noof_bagsdiff, moisture, visual_check, qc_status, sl_stage, seed_status, lot _no, nopdn, remarks, plantcode) values('$mainid','$txtpdno','$txtpdndate','$spcodem','$spcodef','$txtcrop','$txtvariety','$txtloc','$txtprod','$txtfar','$gcode','$txtplot','$sdate','$txtgstat','$txtdcqty','$txtdcnob','$txtactqty','$txtactnob','$txtdiffqty','$txtdiffnob','$txtmoist','$txtvisualck','$qc','$sstage','$sstatus','$txtlotno','$pdnum','$txtremarks', '$plantcode')";
if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
{
$subid=mysqli_insert_id($link);
if($god1==1)
{
$sql_sub_sub="insert into tblarr_sloc (arr_type, arr_tr_id, arr_id, crop_id, variety_id, whid, binid, subbin, qty, bags,  rowid, plantcode) values('$arrivaltype', '$mainid', '$subid', '$txtcrop', '$txtvariety', '$y', '$z', '$a1', '$b1', '$c1', '0', '0', '$rowid1', '$plantcode')";
mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
}
if($god2==1)
{
$sql_sub_sub="insert into tblarr_sloc (arr_type, arr_tr_id, arr_id, crop_id, variety_id, whid, binid, subbin, qty, bags,  rowid, plantcode) values('$arrivaltype','$mainid','$subid','$n','$o','$d1','$e1','$f1','$g1','$h1','0','0', '$rowid2', '$plantcode')";
mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
}
if($god3==1)
{
$sql_sub_sub="insert into tblarr_sloc (arr_type, arr_tr_id, arr_id, crop_id, variety_id, whid, binid, subbin, qty, bags,  rowid, plantcode) values('$arrivaltype','$mainid','$subid','$n','$o','$i1','$j1','$k1','$l1','$m1','0','0', '$rowid3', '$plantcode')";
mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
}
...
if($god1==1)
{
$sql_sub_sub="insert into tblarr_sloc (arr_type, arr_tr_id, arr_id, crop_id, variety_id, whid, binid, subbin, qty, bags,  rowid, plantcode) values('$arrivaltype', '$mainid', '$subid', '$n', '$o', '$y', '$z', '$a1', '$b1', '$c1', '0', '0', '$rowid1', '$plantcode')";
mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
}
if($god2==1)
{
