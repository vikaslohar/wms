...
$tot_row=0;
$lotqry=mysqli_query($link,"select * from tbllotimp where lotnumber='".$a."' and lotcrop='".$crop."' and lotvariety='".$variet."' and plantcode='$plantcode'")or die (mysqli_error($link));
...
//$sql_sub="insert into tblarrival_sub (arrival_id, qty, act, diff, qty1, act1, diff1, qc, lotno,moisture,vchk,sstatus,got,qcstatus,gemp,got1,orlot,lotcrop, lotvariety,sstage,remarks,testd,gotdate,dc_date,lotimpid) values('$mainid', '$txtdisp', '$recqtyp', '$txtdqtyp', '$txtrawp', '$txtrecbagp', '$txtdbagp', '$txtqtystat', '$txtlotp','$txtmoist','$txtvisualck','$status','$txtgstat','$txtqc1','$txtgermi','$gotstatus','$olot', '$txtcrop', '$txtvariety', '$sstage','$txtreason','$ddate','$ddate12','$tdate1','$txtlotnoid')";
...
{
$sql_main="update tblarrival set yearcode='$yearid_id', arrival_type='Maize Dry Arrival', arrival_code='$txtid', arrival_date='$tdate', party_id='$txtstfp', nolot='$txtlot', tmode='$txt1', trans_name='$txttname', trans_lorryrepno='$txtlrn', trans_vehno='$txtvn', trans_paymode='$txt13', courier_name='$txtcname', docket_no='$txtdc', pname_byhand='$txtpname', remarks='$txtremarks', dc_date='$tdate1',dcno='$txtdcno',arr_role='$logid'  where arrival_id = '$z1'";

if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=$z1;
 //$status;
 $sql_sub="update tblarrival_sub set arrival_id='$mainid', qty='$txtdisp', act='$recqtyp', diff='$txtdqtyp', qty1='$txtrawp', act1='$txtrecbagp', diff1='$txtdbagp', qc='$qc', moisture='$txtmoist', vchk='$txtvisualck', sstatus='$status', got='$txtgstat', sample='$sample', got1='$gotstatus', qcstatus='$txtqc1', gemp='$txtgermi', lotno='$txtlotp', sstage='$sstage', lotcrop='$txtcrop', lotvariety='$txtvariety', remarks='$txtreason', testd='$ddate', gotdate='$ddate12', dc_date='$tdate1', orlot='$olot' where arrsub_id='$subtrid'";
if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
{
$subid=$subtrid;
...
<input type="hidden" name="itmdchk" value="<?php echo $subtbltot;?>" /> 			  
		  ...
		  <br />
		  <div id="postingsubtable" style="display:block"><table align="center" border="1" width="950" cellspacing="0" cellpadding="0" bordercolor="#F1B01E" style="border-collapse:collapse"  > 
<tr class="Dark" height="30">
