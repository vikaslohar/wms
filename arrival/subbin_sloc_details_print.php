...
<title>Arrival - Transaction - Sloc details</title>
...
$sql_main123=mysqli_query($link,"select lotldg_crop, lotldg_variety from tbl_lot_ldg where lotldg_id='".$lid."'") or die(mysqli_error($link));  
...
{
$quer3=mysqli_query($link,"SELECT * FROM tblvariety where varietyid ='".$row_tbl123['lotldg_variety']."' and actstatus='Active' and vertype='PV'"); 
...
$sql_main=mysqli_query($link,"select lotldg_subbinid, lotldg_variety, lotldg_lotno from tbl_lot_ldg where lotldg_whid='".$wid."' and lotldg_binid='".$bid."' and lotldg_subbinid='".$slid."' group by lotldg_subbinid, lotldg_variety, lotldg_lotno order by lotldg_subbinid") or die(mysqli_error($link));  
...
<table width="800" height="282" border="0" align="center" cellpadding="0" cellspacing="0" >
  ...
  <form name="from" method="post" action="<?php $_SERVER['PHP_SELF']; ?>" onSubmit="post_value();">
	 ...
	 <input name="frm_action" value="submit" type="hidden"> 
      ...
      <?php 
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where whid='".$wid."' order by perticulars") or die(mysqli_error($link));
...
$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$slid."' and binid='".$bid."' and whid='".$wid."'") or die(mysqli_error($link));
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="800" bordercolor="#F1B01E" style="border-collapse:collapse">
...
$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_whid='".$wid."' and lotldg_binid='".$bid."' and lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_variety='".$vid."' and lotldg_crop='".$crop."'  and lotldg_lotno='".$row_tbl['lotldg_lotno']."'") or die(mysqli_error($link));  
...
$sql_item=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row_tbl_sub['lotldg_trid']."' and lotno='".$row_tbl_sub['lotldg_lotno']."'") or die(mysqli_error($link));
$row_item=mysqli_fetch_array($sql_item);


$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$row_tbl_sub['lotldg_subbinid']."' and binid='".$bid."' and whid='".$wid."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
...
?>			  
 <tr class="Light" height="20">
			 ...
			 <td width="107" align="center" valign="middle" class="tblheading"><?php echo $row_item['qc'];?></td>
			 <td align="center" valign="middle" class="tblheading"><?php echo $row_item['moisture'];?></td>
 		<!--/*   /*  <td width="7%" align="center" valign="middle" class="tblheading"><?php echo $row_item['gemp'];?></td>*/*/-->
			 <td align="center" valign="middle" class="tblheading"><?php echo $tdate;?></td>
             <td width="95" align="center" valign="middle" class="tblheading"><?php echo $row_item['got1'];?></td>
...
$sql_main=mysqli_query($link,"select subbinid, lotldg_variety, lotno, lotldg_crop from tbl_lot_ldg_pack where whid='".$wid."' and binid='".$bid."' and subbinid='".$slid."' group by subbinid, lotldg_variety, lotno order by subbinid") or die(mysqli_error($link));  
...
//echo $row_tbl['lotno'];
//echo $vid;
$sql_tbl1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where whid='".$wid."' and binid='".$bid."' and subbinid='".$row_tbl['subbinid']."' and lotldg_variety='".$row_tbl['lotldg_variety']."' and lotldg_crop='".$row_tbl['lotldg_crop']."'  and lotno='".$row_tbl['lotno']."'") or die(mysqli_error($link));  
$row_tbl1=mysqli_fetch_array($sql_tbl1);
$t1=mysqli_num_rows($sql_tbl1);
//echo $row_tbl1[0];
...
while($row_tbl_sub=mysqli_fetch_array($sql1))
{
$sql_class=mysqli_query($link,"select * from tblarrival where arrival_id='".$row_tbl_sub['lotldg_id']."'") or die(mysqli_error($link));
$row_class=mysqli_fetch_array($sql_class);

$sql_item=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row_tbl_sub['lotldg_id']."' and lotno='".$row_tbl_sub['lotno']."'") or die(mysqli_error($link));
$row_item=mysqli_fetch_array($sql_item);


$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$row_tbl_sub['subbinid']."' and binid='".$bid."' and whid='".$wid."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
