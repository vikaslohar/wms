...
if(isset($_REQUEST['a']))
{
 ...
 $subsubid=trim($_REQUEST['subsubtid']);
...
//exit;
	...
	$txtarrnop=trim($_REQUEST['txtarrnop']);
	$txtarrqty=trim($_REQUEST['txtarrqty']);	
	...
	$lossnop=trim($_REQUEST['txtlossnop']);
	$lossqty=trim($_REQUEST['txtlossqty']);
	
	//frm_action=submit&txt11=Transport&txt14=TBB&txtid=1&logid=AR2&gln1=&transactionid=TAP1%2FI%2FAR2&date=06-09-2017&txtstfp=1&adddchk=&txt1=Transport&txttname=dssdfsdf&txtlrn=werwer&txtvn=werwer&txt13=TBB&txtcname=&txtdc=&txtpname=&txtlot_1=DI90339%2F00000%2F01P&txtwhg_1=9&vbin_1=111&vsubbin_1=2201&itmdchk=0&arrbar=DI040005411%2CDI040005421%2CDI040005415%2CDI040005414%2CDI040005413&srno=2&arrtyp=unpackarr&arrsubtyp=NoP%20With%20Barcode&txtlot1=&txtlotnoid=&barcode=DI040005411&brflg=&brchflg=0&delbarcode=&upkbarcode=DI040005411&wtmp=7.000&txtlot1=DI90339%2F00000%2F01P&ups=50.000%20Gms&txtnop=140&txtqty=7.000&txtarrnop=130&txtarrqty=6.5&txtlossnop=10&txtlossqty=0.5&txtbalnop=0&txtbalqty=0&itmdchk=0&lossbook=Yes-Complete&txtlot11=&txtlotnoid=&maintrid=0&subtrid=0&ep_id=18&txtremarks=
...
{		
	...
	$balloosenop=$row_sub['stlotimpp_loosenop']-$row_sub['stlotimpp_arrloosenop'];
	$balnop=$nop-$arrnop-$lossnop;
	$balnomp=$nomp-1;
...
if($z1 == 0)
{
	...
	if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
	{
		...
		$sql_sub="insert into tbl_arrpack_sub (arrpack_id, arrpacks_crop, arrpacks_variety, arrpacks_lotno, arrpacks_pklable1, arrpacks_ups, arrpacks_eloosenop, arrpacks_enop, arrpacks_enomp, arrpacks_eqty, arrpacks_nop, arrpacks_qty, arrpacks_lossnop, arrpacks_lossqty, arrpacks_balloosenop, arrpacks_balnop, arrpacks_balnomp, arrpacks_balqty, arrpacks_packqc, arrpacks_packqcdate, arrpacks_qcstatus, arrpacks_qcdot, arrpacks_pp, arrpacks_moist, arrpacks_germ, arrpacks_gotstatus, arrpacks_gotdate, arrpacks_dop, arrpacks_dov, arrpacks_wtmp, arrpacks_srtype, arrpacks_srstatus, arrpacks_ssrtype, arrpacks_ssrstatus, arrpacks_ukpnomp, plantcode) values('$mainid', '$crop', '$variety', '$lotno', '$pklable1', '$ups', '$loosenop', '$nop', '$nomp', '$qty', '$arrnop', '$arrqty', '$lossnop', '$lossqty', '$balloosenop', '$balnop', '$balnomp', '$balqty', '$qcpacktyp', '$qcpackdate', '$qc', '$dot', '$pp', '$moist', '$germ', '$got', '$dogt', '$dop', '$dov', '$wtmp', '$srtype', '$srstatus', '$ssrtype', '$ssrstatus', '1', '$plantcode')";
		if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
		{
			$subid=mysqli_insert_id($link);
			
			$sql_sub_sub="insert into tbl_arrpack_subsub (arrpack_id, arrpacks_id, arrpackss_lotno, arrpackss_whid, arrpackss_binid, arrpackss_subbinid, arrpackss_nop, arrpackss_qty, arrpackss_status, plantcode) values('$mainid', '$subid', '$lotno', '$wh', '$bin', '$sbin', '$arrnop', '$arrqty', '$balqty1', '$plantcode')";
			mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
			$subsubid=mysqli_insert_id($link);
			
			$sql_sub_sub="insert into tbl_arrpack_barcode (arrpackss_id, arrpacks_id, arrpack_id, arrpackss2_lotno, arrpackss2_bartype, arrpackss2_barcode, arrpackss2_lotnop, arrpackss2_lotqty, arrpackss2_barqty, arrpackss2_grosswt, arrpackss2_arrflg, arrpackss2_upkflg, plantcode) values('$subsubid', '$subid', '$mainid', '$lotno', '$bartype', '$barcode',  '$lotnop', '$lotqty', '$arrqty', '$grswt', '1', '1', '$plantcode')";
			mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
		
		}
	}
	$sql_impsub="update tbl_stlotimp_pack_sub set stlotimpp_arrnop='$arrnop', stlotimpp_arrqty='$arrqty', stlotimpp_balloosenop='$balloosenop', stlotimpp_balnop='$balnop', stlotimpp_balnomp='$balnomp', stlotimpp_balqty='$balqty' where stlotimpp_id='$epid' and stlotimpp_lotno='".$lotno."' ";
...
{//echo "select * from tbl_arrpack_sub where arrpacks_lotno='".$lotno."' and arrpack_id='".$z1."' ";
	...
	$upknomp=$row_arrsub['arrpacks_ukpnomp']+1;
	...
	{
		$sql_sub="update tbl_arrpack_sub set arrpacks_eloosenop='$loosenop', arrpacks_enop='$nop', arrpacks_enomp='$nomp', arrpacks_eqty='$qty', arrpacks_nop='$arrnop', arrpacks_qty='$arrqty', arrpacks_balloosenop='$balloosenop', arrpacks_balnop='$balnop', arrpacks_balnomp='$balnomp', arrpacks_balqty='$balqty', arrpacks_ukpnomp='$upknomp' where arrpacks_id='$subtid' ";
		
		mysqli_query($link,$sql_sub) or die(mysqli_error($link));
		
		if($sbin==$sbin11)
		{
			$sql_sub_sub="update tbl_arrpack_subsub set arrpacks_nop='$arrnop', arrpackss_qty='$arrqty' where arrpacks_id='".$subtid."'";
			mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
		}
		else
		{
	...
	$sql_sub_sub="insert into tbl_arrpack_barcode (arrpackss_id, arrpacks_id, arrpack_id, arrpackss2_lotno, arrpackss2_bartype, arrpackss2_barcode, arrpackss2_lotnop, arrpackss2_lotqty, arrpackss2_barqty, arrpackss2_grosswt, arrpackss2_arrflg, arrpackss2_upkflg, plantcode) values('$subsubid', '$subtid', '$mainid', '$lotno', '$bartype', '$barcode',  '$lotnop', '$lotqty', '$arrqty', '$grswt', '1', '1', '$plantcode')";
	mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
...
<tr class="tblsubtitle">
	<td width="3%" align="center" valign="middle" class="tblheading">NoLP</td>
