<?php
	...
	if(isset($_REQUEST['exlotno']))
	{
	 $exlotno = $_REQUEST['exlotno'];
	}
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$stmt_lotimp=mysqli_query($link,"SELECT lotimpid, lotcrop, lotspcodef, lotspcodem, lotploc, lotstate, lotpper, lotorganiser, lotfarmer, lotplotno, pdnno, pdndate, prodtype  FROM tbllotimp WHERE trid=0 AND lotnumber='$loc' ") or die(mysqli_error($link));
		if(mysqli_num_rows($stmt_lotimp) > 0) 
		{
			$row_lotimp=mysqli_fetch_array($stmt_lotimp);
			$lotimpid=$row_lotimp['lotimpid'];
			$lotcrop=$row_lotimp['lotcrop']; 
			$lotspcodef=$row_lotimp['lotspcodef'];
			$lotspcodem=$row_lotimp['lotspcodem'];
			$lotploc=$row_lotimp['lotploc']; 
			$lotstate=$row_lotimp['lotstate']; 
			$lotpper=$row_lotimp['lotpper']; 
			$lotorganiser=$row_lotimp['lotorganiser']; 
			$lotfarmer=$row_lotimp['lotfarmer']; 
			$lotplotno=$row_lotimp['lotplotno']; 
			$pdnno=$row_lotimp['pdnno']; 
			$pdndate=$row_lotimp['pdndate']; 
			$prodtype=$row_lotimp['prodtype'];
			//looping through all the records 
			
		
			$popularname=$lotcrop."-Coded";
			$stmt_spc = mysqli_query($link,"SELECT variety, crop FROM tblspcodes WHERE spcodef = '$lotspcodef' AND spcodem = '$lotspcodem' ") or die(mysqli_error($link));
			if (mysqli_num_rows($stmt_spc) > 0) {
				$row_spc=mysqli_fetch_array($stmt_spc);
				$spcvariety=$row_spc['variety']; 
				$spccrop=$row_spc['crop'];
			
				$stmt_variety = mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety WHERE varietyid = '$spcvariety' ") or die(mysqli_error($link));
				if (mysqli_num_rows($stmt_variety) > 0) {
					$row_variety=mysqli_fetch_array($stmt_variety);
					$varietyid=$row_variety['varietyid'];
					$popularname=$row_variety['popularname'];
				}
			}
			$quer_cn=mysqli_query($link,"SELECT * FROM tbl_parameters where plantcode='$plantcode' ");
			$row_cls=mysqli_fetch_array($quer_cn);
			$tp1=$row_cls['code'];
			$lotno=$tp1.$loc."/00000/00R";
			$orlot=$tp1.$loc."/00000/00";
			
			$sql_sub="update tblarrival_sub_unld SET lotimpid='$lotimpid', lotcrop='$lotcrop', lotvariety='$popularname', lotno='$lotno', orlot='$orlot', old='$loc', pdndate='$pdndate', pdnno='$pdnno', spcodef='$lotspcodef', spcodem='$lotspcodem', organiser='$lotorganiser', farmer='$lotfarmer', ploc='$lotploc', lotstate='$lotstate', pper='$lotpper', plotno='$lotplotno', sstage='$stage', prodtype='$prodtype' where arrsub_id='$subid'";
			if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
			{
				$sql_sub_sub="update tblarrsub_sub_unld set lotno='$loc' where arrsub_id='$subid'";
...
function lotchk()
{
		if(document.from1.c1.value=="")
			...
			if(!isChar_W(document.from1.c1.value.charAt(0)))
				...
				var trid=document.from1.trid.value;
				var subid=document.from1.subid.value;
				var exlotno=document.from1.exlotno.value;
				var crop=document.from1.crop.value;
	window.location='getuser_fpdnunld_lotnoupd.php?tp='+tp+'&sqq='+txtlot1+'&trid='+trid+'&subid='+subid+'&exlotno='+exlotno+'&crop='+crop;
return true;
}
function mySubmit()
{
...
<td colspan="4" align="center" valign="middle"><table border="0" align="center" cellpadding="0" cellspacing="0" >
 ...
 <input type="hidden" name="exlotno" value="<?php echo $exlotno?>" />
...
<?php 
$arrlots="";
$sql_arrival=mysqli_query($link,"select arrival_id from tblarrival_unld where arrival_type='$tp' and arrtrflag=0 and plantcode='$plantcode'") or die(mysqli_error($link));
$tot_arrival=mysqli_num_rows($sql_arrival);
if($tot_arrival)
{
	while($row_arrival=mysqli_fetch_array($sql_arrival))
	{
		$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub_unld where arrival_id='".$row_arrival['arrival_id']."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql="select * from tbllotimp where lottrtype='$tp' and lotimpflg!=1 and trid=0 and lotcrop ='$crop' $sss order by lotnumber";
