...
<title>Arrival - Utility - Decode SLOC Look Up</title>
...
if($trvflag=="Yes" || $role=="admin")
	{	
	$sql_arr=mysqli_query($link,"select * from tblarrival where ardate>='$sdate' and ardate<='$edate' and cropid='".$crop."' and productionlocationid='".$loc. "'") or die(mysqli_error($link));
	}
	else
	{
	$sql_arr=mysqli_query($link,"select * from tblarrival where ardate>='$sdate' and ardate<='$edate' and cropid='".$crop."' and productionlocationid='".$loc. "' and logid='".$logid."'") or die(mysqli_error($link));
	}
	...
	while($row_arr=mysqli_fetch_array($sql_arr))
	{
		if($org!=0 && $farmer!=0)
		{
		$sql_arr_sub=mysqli_query($link,"select * from tblarrival_sub where arid='".$row_arr['arid']."' and orgid='".$org."' and farmerid='".$farmer."'") or die(mysqli_error($link));
		}
		else if($org!=0 && $farmer==0)
		{
		$sql_arr_sub=mysqli_query($link,"select * from tblarrival_sub where arid='".$row_arr['arid']."' and orgid='".$org."'") or die(mysqli_error($link));
		}
		else if($org==0 && $farmer!=0)
		{
		$sql_arr_sub=mysqli_query($link,"select * from tblarrival_sub where arid='".$row_arr['arid']."' and farmerid='".$farmer."'") or die(mysqli_error($link));
		}
...
else if($typ=="specifylot")
{
		...
		$sql_cls=mysqli_query($link,"SELECT * FROM tbllot where lotnumber='$lot' order by lotno Asc") or die(mysqli_error($link));
...
while($row_cls=mysqli_fetch_array($sql_cls))
{
	...
	{
	$sql_arr=mysqli_query($link,"select * from tblarrival where arid='".$row_sub['arid']."' and logid='".$logid."'") or die(mysqli_error($link));
	$tot_arr=mysqli_num_rows($sql_arr);
	}
	if($tot_arr > 0)
	{
