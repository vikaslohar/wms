...
//exit;
	...
	$txtnop=trim($_REQUEST['txtnop']);
	$txtarrnop=trim($_REQUEST['txtarrnop']);
	$txtarrnomp=trim($_REQUEST['txtarrnomp']);
	$txtarrqty=trim($_REQUEST['txtarrqty']);	
	$txtlossnop=trim($_REQUEST['txtlossnop']);
	$txtlossnomp=trim($_REQUEST['txtlossnomp']);
	$txtupknomp=trim($_REQUEST['txtupknomp']);
	$txtlossqty=trim($_REQUEST['txtlossqty']);
	$txtbalnop=trim($_REQUEST['txtbalnop']);
	$txtbalqty=trim($_REQUEST['txtbalqty']);
	
	$balnop=trim($_REQUEST['txtbalnop']);
	$balnomp=trim($_REQUEST['txtbalnomp']);
	$balqty=trim($_REQUEST['txtbalqty']);
	$srno=trim($_REQUEST['srno']);
		
	//frm_action=submit&txt11=By%20Hand&txt14=&txtid=58&logid=AR2&gln1=&transactionid=TAS58%2FI%2FAR2&date=22-08-2017&txtstfp=1&adddchk=&txt1=By%20Hand&txttname=&txtlrn=&txtvn=&txt13=Select&txtcname=&txtdc=&txtpname=sadasdasda&txtwhg1=WH&vbin_1=Bin&vsubbin_=SubBin&txtwhg1=WH&vbin_2=Bin&vsubbin_=SubBin&itmdchk=0&txtarrlot=DV91476%2F00000%2F00P%2CDV90518%2F00058%2F00P&arrtyp=unpackarr&arrsubtyp=NoP%20Without%20Barcode&txtlot1=&txtlotnoid=&barcode=&brflg=&brchflg=0&delbarcode=&upkbarcode=&txtlot11=DV91476%2F00000%2F00P&txtlotnoid=&wtmp=10.000&ups=100.000%20Gms&txtnop=300&txtnomp=3&txtqty=30.000&txtarrnop=130&txtarrqty=13&txtlossnop=70&txtlossnomp=0&txtlossqty=7&txtupknomp=2&txtbalnop=100&txtbalnomp=1&txtbalqty=10&itmdchk=0&lossbook=Yes-Partial&maintrid=0&subtrid=0&ep_id=5&txtremarks=
		
 $z1=$maintrid;		
//echo "select * from tbl_stlotimp_packsubsub where stlotimpps_barcode='".$barcode."' and stlotimpps_arrflg=0 ";
//exit;

	$sql_sub=mysqli_query($link,"select * from tbl_stlotimp_pack_sub where stlotimpp_id='".$epid."' and stlotimpp_lotno='".$txtlot11."' and plantcode='$plantcode' ") or die(mysqli_error($link));
	...
	$arrnomp=$arrnomp+$txtarrnomp;
	$arrqty=$arrqty+$txtarrqty;
	$balloosenop=$loosenop;
	
	/*$balnop=$nop-$arrnop;
	$balnomp=$nomp-txtarrnomp;
	$balqty=$qty-$arrqty;*/
...
if($z1 == 0)
{
	...
	if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
	{
		...
		$sql_sub="insert into tbl_arrpack_sub (arrpack_id, arrpacks_crop, arrpacks_variety, arrpacks_lotno, arrpacks_pklable1, arrpacks_ups, arrpacks_eloosenop, arrpacks_enop, arrpacks_enomp, arrpacks_eqty, arrpacks_loosenop, arrpacks_qty, arrpacks_balloosenop, arrpacks_balnop, arrpacks_balnomp, arrpacks_balqty, arrpacks_packqc, arrpacks_packqcdate, arrpacks_qcstatus, arrpacks_qcdot, arrpacks_pp, arrpacks_moist, arrpacks_germ, arrpacks_gotstatus, arrpacks_gotdate, arrpacks_dop, arrpacks_dov, arrpacks_wtmp, arrpacks_srtype, arrpacks_srstatus, arrpacks_ssrtype, arrpacks_ssrstatus, plantcode) values('$mainid', '$crop', '$variety', '$lotno', '$pklable1', '$ups', '$loosenop', '$nop', '$nomp', '$qty', '$txtarrnop', '$arrqty', '$balnop', '$nop', '$balnomp', '$balqty', '$qcpacktyp', '$qcpackdate', '$qc', '$dot', '$pp', '$moist', '$germ', '$got', '$dogt', '$dop', '$dov', '$wtmp', '$srtype', '$srstatus', '$ssrtype', '$ssrstatus', '$plantcode')";
	...
	$sql_impsub="update tbl_stlotimp_pack_sub set stlotimpp_arrnop='$arrnop', stlotimpp_arrnomp='$arrnomp', stlotimpp_arrqty='$arrqty', stlotimpp_balnop='$balloosenop', stlotimpp_balnomp='$balnomp', stlotimpp_balqty='$balqty' where stlotimpp_id='$epid' and stlotimpp_lotno='".$lotno."' ";
...
{//echo "select * from tbl_arrpack_sub where arrpacks_lotno='".$lotno."' and arrpack_id='".$z1."' ";
	...
	{
		$sql_sub="update tbl_arrpack_sub set arrpacks_loosenop='$txtarrnop', arrpacks_qty='$arrqty', arrpacks_balloosenop='$balnop', arrpacks_balqty='$balqty', arrpacks_lossnop='$txtlossnop', arrpacks_lossqty='$txtlossqty', arrpacks_ukpnomp='$txtupknomp' where arrpacks_id='$subtid' ";
		//exit;
		mysqli_query($link,$sql_sub) or die(mysqli_error($link));
		
		$sql_sub_sub="update tbl_arrpack_subsub set arrpackss_nop='$arrnop', arrpackss_qty='$arrqty' where arrpacks_id='".$subtid."'";
		mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
	}
