...
$sql_main="update tblarrival set tmode='$txt11', trans_name='$txttname', trans_lorryrepno='$txtlrn', trans_vehno='$txtvn', trans_paymode='$txt14', courier_name='$txtcname', docket_no='$txtdc', pname_byhand='$txtpname', remarks='$remarks' where arrival_id = '$pid'";
$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
	...
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		...
		$got1=$row_arrsub['got1'];
		$gotdate=$row_arrsub['gotdate'];
		...
		while($row_arrsub_sub=mysqli_fetch_array($sql_arrsub_sub))
		{
			...
			//$qty1=$row_arrsub_sub['qty_damage'];
				...
				$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$subbinid."' and lotldg_binid='".$binid."' and lotldg_whid='".$whid."' and lotldg_variety='".$itemid."' and lotldg_crop='".$classid."' and lotldg_lotno='".$lotno."' and plantcode='$plantcode'") or die(mysqli_error($link));
		...
		$state="P/M/G";
		/*if($row_arrsub['got1']=="UT")
		{
			$state="P/M/G/T";
		}*/
		if($qc=="UT" || $qc=="RT")
		{
			$sql_sub_sub1244="insert into tbl_qctest(pp, moist, got, lotno, srdate, crop, variety, gemp, sampleno, trstage, qc, testdate, state, gs, oldlot, bflg, yearid, logid, plantcode)values('$vchk', '$moist', '$got1', '$lotno', '$tdate11', '$classid', '$itemid', '$gemp', '$ncode1', '$stage', '$qc', '$tdate', '$state', '1', '$gln', '0', '$yearid_id', '$logid', '$plantcode')";
			mysqli_query($link,$sql_sub_sub1244) or die(mysqli_error($link));
		}
		if($gotrusl=="UT" || $gotrusl=="RT")
		{
			$sql_sub_sub1244="insert into tbl_gottest(gottest_got, gottest_lotno, gottest_srdate, gottest_crop, gottest_variety, gottest_sampleno, gottest_trstage, gottest_oldlot, yearid, logid, plantcode) values ('$got1', '$lotno', '$tdate11', '$classid', '$itemid', '$ncode1', '$stage', '$gln', '$yearid_id','$logid', '$plantcode')";
			mysqli_query($link,$sql_sub_sub1244) or die(mysqli_error($link));
		}	
		/*if(($row_arrsub['qc']=="UT" || $row_arrsub['qc']=="RT") && ($row_arrsub['got1']!="UT" && $row_arrsub['got1']!="RT"))
		{
		$sql_sub_sub12="insert into tbl_qctest(pp,moist, got,lotno,srdate, crop, variety,gemp,sampleno,trstage,qc,testdate,state,oldlot, bflg,yearid,logid)values('$vchk','$moist','$got1','$lotno','$tdate11','$classid','$itemid','$gemp','$ncode1','$stage','$qc','$tdate','$state','$gln','0','$yearid_id','$logid')";
		
		mysqli_query($link,$sql_sub_sub12) or die(mysqli_error($link));
		}
		if(($row_arrsub['qc']!="UT" && $row_arrsub['qc']!="RT") && ($row_arrsub['got1']=="UT" || $row_arrsub['got1']=="RT"))
		{
		$sql_sub_sub12="insert into tbl_qctest(pp,moist, got,lotno,srdate, crop, variety,gemp,sampleno,trstage,qc,testdate,state,oldlot,bflg,gotsmpdflg,yearid,logid)values('$vchk','$moist','$got1','$lotno','$tdate11','$classid','$itemid','$gemp','$ncode1','$stage','$qc','$tdate','$state','$gln','0','0','$yearid_id','$logid')";
		...
		mysqli_query($link,$sql_sub_sub12) or die(mysqli_error($link));
			 ...
			 $sql_sub_upd="update tbl_cobdrying set arrtflg=1 where trid='".$lotnoid."' and plantcode='$plantcode'";
	...
	$sql_code="SELECT MAX(arr_code) FROM tblarrival where yearcode='$yearid_id' and arrival_type='Maize Dry Arrival' and plantcode='$plantcode' ORDER BY arr_code DESC";
	$res_code=mysqli_query($link,$sql_code)or die(mysqli_error($link));
		...
		$sql_code1="SELECT MAX(ncode) FROM tblarrival where yearcode='$yearid_id' and arrival_type='Maize Dry Arrival' and plantcode='$plantcode' ORDER BY ncode DESC";
	$res_code1=mysqli_query($link,$sql_code1)or die(mysqli_error($link));
	...
	$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
		...
		echo "<script>window.location='select_arrival_maizedryop.php?p_id=$pid'</script>";	
	}
...
<title>Arrival- Transaction - Arrival Maize-Dried Seed - Preview</title>
...
winHandle=window.open('maizedry_view.php?itmid='+itm+'&remarks='+remarks,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" style="border-bottom:solid; border-bottom-color:#F1B01E" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Arrival Maize-Dried Seed - Preview</td>
	    </tr></table></td>
...
<tr class="tblsubtitle" height="20">
  <td colspan="6" align="center" class="tblheading"> Arrival Maize-Dried Seed - Preview </td>
</tr>
			 ...
			 <td width="75" align="center" rowspan="2" valign="middle" class="tblheading">Lot No.</td>
              <td colspan="2" align="center" valign="middle" class="tblheading">Dispatch</td>
			   ...
			   <td width="50" rowspan="2" align="center" valign="middle" class="tblheading">Germ. %</td>
			    ...
			    <td width="50" rowspan="2" align="center" valign="middle" class="tblheading">Status </td>
