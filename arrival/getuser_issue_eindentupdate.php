...
if($trid == 0)
{
$sql_main="insert into tblissue (issue_type, issue_code, issue_date, dcrefno, remarks, issue_role) values('eindent','$code','$tdate','$indentno','$remarks','$logid')";
...
for($num=0; $num<$numrec; $num++)
{
	$p1_array[$num];
	$p1_array1[$num];
$sql_itmldg=mysqli_query($link,"select stlg_balups, stlg_balqty, stlg_id, stlg_whid, stlg_binid, stlg_subbinid from tbl_stldg_good where stlg_id='".$p1_array[$num]."'") or die(mysqli_error($link));
...
<?php
$sr=1;
$sql_eindent_sub=mysqli_query($link,"select * from tbl_ieindent_sub where id_in=$tid") or die(mysqli_error($link));
while($row_eindent_sub=mysqli_fetch_array($sql_eindent_sub))
{
$classqry=mysqli_query($link,"select classification_id, classification from tbl_classification where classification_id='".$row_eindent_sub['classification_id']."'") or die(mysqli_error($link));
$noticia_class = mysqli_fetch_array($classqry);

$itemqry=mysqli_query($link,"select items_id, stores_item from tbl_stores where items_id='".$row_eindent_sub['items_id']."'") or die(mysqli_error($link));
$noticia_item = mysqli_fetch_array($itemqry);
$t_stldg1=0;
if($trid > 0)
{
$sql_tblissue=mysqli_query($link,"select * from tblissue_sloc where issue_tr_id='".$trid."' and classification_id='".$row_eindent_sub['classification_id']."' and item_id='".$row_eindent_sub['items_id']."'") or die(mysqli_error($link));
...
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where whid='".$row_tblissue['whid']."' order by perticulars") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);
$wareh=$row_whouse['perticulars']."/";

$sql_binn=mysqli_query($link,"select binname from tbl_bin where binid='".$row_tblissue['binid']."' and whid='".$row_tblissue['whid']."'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname']."/";

$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$row_tblissue['subbin']."' and binid='".$row_tblissue['binid']."' and whid='".$row_tblissue['whid']."'") or die(mysqli_error($link));
...
$sql_stldg1=mysqli_query($link,"select * from tbl_stldg_good where stlg_id='".$row_tblissue['issue_rowid']."'") or die(mysqli_error($link));
