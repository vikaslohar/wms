<?php
   ...
   // $yearid_id="09-10";	
	...
	if(isset($_REQUEST['txtor']))
	{
	$txtor=trim($_REQUEST['txtor']);
		}
	...
	if(isset($_REQUEST['txtvv']))
	{
	$txtvv=trim($_REQUEST['txtvv']);
		}
	if(isset($_REQUEST['txtgrn']))
	{
	$txtgrn=trim($_REQUEST['txtgrn']);
		}
	...
	{
		$txtparty=trim($_REQUEST['txtparty']);
		}
		/*$tdate11=$ee1;
		...
		$tyear1=substr($tdate11,6,4);
		$tdate1=$tyear1."-".$tmonth1."-".$tday1;*/
	 $sql_main="update tblarrival set grnno='$txtgrn',dcno='$txtdcno' ,orderno='$txtor',vvariety='$txtvv', nolot='$txtlot2', sstage='$txtstage',  tmode='$txt11', trans_name='$txttname', trans_lorryrepno='$txtlrn', trans_vehno='$txtvn', trans_paymode='$txt14', courier_name='$txtcname', docket_no='$txtdc', pname_byhand='$txtpname', remarks='$remarks', party_id='$txtparty' where arrival_id = '$pid'";
	...
	{
		$subtrid=$row_arrsub['arrsub_id'];
		if($row_arrsub['lotcrop']=="" || $row_arrsub['lotvariety']=="")
		{
			$sql_lotimp=mysqli_query($link,"select * from tbllotimp where lotnumber='".$row_arrsub['old']."' and plantcode='$plantcode'") or die(mysqli_error($link));																																																																														
			$row_lotimp=mysqli_fetch_array($sql_lotimp);																																																																														
			$crop=$row_lotimp['lotcrop'];
			$variety=$row_lotimp['lotvariety'];
			
			$sql_main="update tblarrival set lotcrop='$crop', lotvariety='$variety' where arrival_id='".$pid."'";
			mysqli_query($link,$sql_main) or die(mysqli_error($link));
			
			$sql_sub="update tblarrival_sub set lotcrop='$crop', lotvariety='$variety' where arrsub_id='$subtrid'";
			mysqli_query($link,$sql_sub) or die(mysqli_error($link));
			
			$sql_subsub="update tblarr_sloc set lotcrop='$crop', lotvariety='$variety' where arrsub_id='$subtrid'";
	...
	$trdate1=$row_arr['dc_date'];
	$stage=$row_arr['sstage'];	
	$crop1=$row_arr['lotcrop'];	
	$variety1=$row_arr['lotvariety'];	
	$sql_arrsub=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$pid."' and plantcode='$plantcode'") or die(mysqli_error($link));
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
	...
	if($crop1=="" && $crop!="")$crop1=$crop;
	if($crop1=="" && $crop=="")
	{
		$sql_lotimp=mysqli_query($link,"select * from tbllotimp where lotnumber='".$row_arrsub['old']."' and plantcode='$plantcode'") or die(mysqli_error($link));																																																																														
		$row_lotimp=mysqli_fetch_array($sql_lotimp);																																																																														
		$crop1=$row_lotimp['lotcrop'];
	}
	if($variety1=="" && $variety!="")$variety1=$variety;
	if($variety1=="" && $variety=="")
	{
		$sql_lotimp=mysqli_query($link,"select * from tbllotimp where lotnumber='".$row_arrsub['old']."' and plantcode='$plantcode'") or die(mysqli_error($link));																																																																														
		$row_lotimp=mysqli_fetch_array($sql_lotimp);																																																																														
		$variety1=$row_lotimp['lotvariety'];
	}
	
	$sql_crop=mysqli_query($link,"select * from tblcrop where cropname='".$crop1."'") or die(mysqli_error($link));
	$row_crop=mysqli_fetch_array($sql_crop);
	$classid=$row_crop['cropid'];
	$leduration=$row_arrsub['leduration'];
	$ledate=$row_arrsub['leupto'];
		
	$sql_veriety=mysqli_query($link,"select * from tblvariety where popularname='".$variety1."' and actstatus='Active' and vertype='PV'") or die(mysqli_error($link));
	$row_variety=mysqli_fetch_array($sql_veriety);
	$itemid=$row_variety['varietyid'];				
	
	//$classid=$row_arrsub['lotcrop'];
	...
	while($row_arrsub_sub=mysqli_fetch_array($sql_arrsub_sub))
	{
		...
		{
			$sqlsubsub1="insert into tbl_lemain (le_lotno, le_stage, le_duration, le_upto, plantcode) values( '$lotno' ,'$stage', '$leduration','$ledate', '$plantcode')";
			mysqli_query($link,$sqlsubsub1) or die(mysqli_error($link));
		}
		
		$sqlsubsub13="insert into tbl_learchive (lea_lotno, lea_stage, lea_duration, lea_upto, lea_date, lea_module, lea_logid, plantcode) values( '$lotno' ,'$stage', '$leduration','$ledate', '$trdate', 'Trading Arrival', '$logid', 'plantcode')";
		...
		{
			$sql_sub_sub12="insert into tbl_qctest(pp,moist, got,lotno,srdate, crop, variety,gemp,sampleno,trstage,qc,state,gs,oldlot, yearid,logid)values('$vchk','$moist','$got1','$lotno','$trdate','$classid','$itemid','$gemp','$ncode1','$stage','$qc','$state',1,'$gln','$yearid_id','$logid')";
		//exit;
		...
		$sql_code="SELECT MAX(arr_code) FROM tblarrival where yearcode='$yearid_id' and arrival_type='Trading' and plantcode='$plantcode' ORDER BY arr_code DESC";
		...
		$sql_code1="SELECT MAX(ncode) FROM tblarrival where yearcode='$yearid_id' and arrival_type='Trading' and plantcode='$plantcode' ORDER BY ncode DESC";
		$res_code1=mysqli_query($link,$sql_code1)or die(mysqli_error($link));
		...
		echo "<script>window.location='select_arrival_tradingop.php?p_id=$pid'</script>";	
...
<title>Arrival -Transaction -Trading Arrival - Preview</title>
...
winHandle=window.open('stockitem_sloc_details_print.php?itmid='+itm+'&remarks='+remarks,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" style="border-bottom:solid; border-bottom-color:#F1B01E" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Trading Arrival- Preview</td>
	    </tr></table></td>
...
$arrival_id=$row_tbl['arrival_id'];
	...
	$ddate1=explode("-",$row_tbl['dc_date']);
		$tdate1=$ddate1[2]."-".$ddate1[1]."-".$ddate1[0];
...
<tr class="tblsubtitle" height="20">
  <td colspan="4" align="center" class="tblheading">Trading Arrival Preview</td>
</tr>
...
<td align="left"  valign="middle" class="tbltext" colspan="3" id="vaddress">&nbsp;<?php echo $address;?>&nbsp;</td>
</tr>
