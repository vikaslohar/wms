...
$sql_sub="insert into tblarrival_sub (arrival_id, organiser, pdnno, pdndate, spcodef, spcodem, lotcrop, lotvariety, ploc, pper, farmer, plotno, gi, harvestdate, got, qty, act, diff, qty1, act1, diff1, moisture, vchk, qc, remarks, sstage, sstatus, lotno, old, got1, sample,qcsample,gssample,prodtype, lotstate, leduration, leupto, plantcode, prodgrade, tarewt) values('$mainid', '$organi', '$txtpdno', '$pdate', '$spcodef', '$spcodem', '$txtcrop', '$txtvariety', '$txtloc', '$txtprod', '$txtfar', '$txtplot', '$gcode', '$hdate1', '$txtgstat', '$txtdcqty', '$txtactqty', '$txtdiffqty', '$txtdcnob', '$txtactnob', '$txtdiffnob', '$txtmoist', '$txtvisualck', '$qc', '$txtremarks', '$sstage', '$sstatus', '$glotno', '$txtlot1', '$gotstatus', '$gscheckbox','$qc2','$gs1','$productiontype','$statename','$leduration','$ledate', '$plantcode', '$prodctiongrade', '$txttareqty')";
...
$sql_sub="update tblarrival_sub set arrival_id='$mainid', organiser='$organi', pdnno='$txtpdno', pdndate='$pdate', spcodef='$spcodef', spcodem='$spcodem', lotcrop='$txtcrop', lotvariety='$txtvariety', ploc='$txtloc', pper='$txtprod', farmer='$txtfar', plotno='$txtplot', gi='$gcode', harvestdate='$hdate1', got='$txtgstat', qty='$txtdcqty', act='$txtactqty', diff='$txtdiffqty', qty1='$txtdcnob', act1='$txtactnob', diff1='$txtdiffnob', moisture='$txtmoist', vchk='$txtvisualck', qc='$qc', remarks='$txtremarks', sstage='$sstage', sstatus='$sstatus',qcsample='$qc2' ,lotno='$glotno', got1='$gotstatus', sample='$gscheckbox', gssample='$gs1', leduration='$leduration', leupto='$ledate', prodgrade='$prodctiongrade', tarewt='$txttareqty' where arrsub_id='$subtrid'";
if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
{
$subid=$subtrid;
...
if($god1==1)
{
$sql_sub_sub="insert into tblarr_sloc (arr_type, arr_tr_id, arr_id, whid, binid, subbin, rowid, qty, bags, balqty, balbags, lotcrop, lotvariety ) values('Fresh Seed with PDN', '$mainid', '$subid', '$y', '$z', '$a1',  '$rowid1', '$b1', '$c1', '$b1', '$c1', '$txtcrop', '$txtvariety', '$plantcode')";
mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
}
if($god2==1)
{
...
$sqlarrsub=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$arrival_id."' and plantcode='$plantcode'") or die(mysqli_error($link));
			...
			if($variety11!="" && $variety11==$vrnew11)
			{
				...
				if($tot_spcdec > 0)
				{
					...
					if($row_spcdec['variety']!="")
					{
						...
						$sqltblarsub=mysqli_query($link,"update tblarrival_sub set lotvariety='$itemname' where arrival_id='".$arrival_id."' and arrsub_id='".$rowarrsub['arrsub_id']."' and plantcode='$plantcode'") or die(mysqli_error($link));	
					}
				}	
			}
		}
