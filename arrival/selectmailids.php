...
$arrlots="";
$qr="select * from tblarrival where arrival_id='".$pid."' ";
...
$sqlissue=mysqli_query($link,"select old from tblarrival_sub where arrival_id='".$pid."'") or die(mysqli_error($link));
$rowissue=mysqli_fetch_array($sqlissue); 

$sqlissue1=mysqli_query($link,"select arrival_id from tblarrival_sub_unld where old='".$rowissue['old']."'") or die(mysqli_error($link));
$rowissue1=mysqli_fetch_array($sqlissue1); 
$sqlissue2=mysqli_query($link,"select * from tblarrival_unld where arrival_id='".$rowissue1['arrival_id']."'") or die(mysqli_error($link));
$rowissue2=mysqli_fetch_array($sqlissue2); 
$lid=$rowissue2['arrival_code'];
$trid=$rowissue1['old'];
$nsflg=0; //echo $row['lotno']."<br />";
$sql_issueg2=mysqli_query($link,"select DISTINCT(lotstate) from tblarrival_sub where arrival_id='".$pid."'") or die(mysqli_error($link));
...
{
	if($val<>"")
	{//echo $row_issueg2['lotstate']."  =  ".$val."  =  ".$row_issueg21['productionpersonnel']."<br />";
		$fg=0;
