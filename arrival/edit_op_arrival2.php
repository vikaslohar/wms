...
if(isset($_REQUEST['pid']))
	...
	//exit;
		...
		echo "<script>window.location='trn_arrvop_qty_preview2.php?pid=$p_id'</script>";	
			
	}
			
			
$tid=$pid;
$sql_tbl=mysqli_query($link,"select * from tbl_opspa where arr_role='".$logid."' and opspa_id='".$tid."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_tbl=mysqli_fetch_array($sql_tbl);
 $tot=mysqli_num_rows($sql_tbl);		
$arrival_id=$row_tbl['opspa_id'];

$sql_tbl_sub=mysqli_query($link,"select * from tbl_opspa_sub where opspa_id='".$arrival_id."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$subtid=0;
	...
	$tdate=$tday."-".$tmonth."-".$tyear;
			...
			$code1="TOP".$row_tbl['opspa_tcode']."/".$yearid_id."/".$lgnid;
		//}
?>
...
<script src="farrival12.js"></script>
...
function openslocpop()
{
	...
	else
	{
		...
		winHandle=window.open('getuser_drying_lotno24.php?crop='+crop+'&variety='+variety+'&tid='+tid,'WelCome','top=170,left=180,width=420,height=350,scrollbars=yes');
		if(winHandle==null){
...
function chktyp()
{
	if(document.frmaddDepartment.txtlot1.value!="")
	{
		var lot=document.frmaddDepartment.txtlot1.value;
		showUser(lot,'lotdetails','showblock',lot,'','','');
	}
	else
	{
		setTimeout('chktyp()',200);
...
function chkdiffqty(conval)
{
	if(document.frmaddDepartment.txtdiffqty.value=="" || document.frmaddDepartment.txtdiffqty.value==0)
	{
		alert("You can not Post record.\nReason: The Difference Qty is ZERO or Blank");
...
function chkdiffqty1(conval1)
{
	if(document.frmaddDepartment.txtactnob.value=="" || document.frmaddDepartment.txtactnob.value==0)
	{
		alert("Please Enter Condition Seed NoB");
		document.frmaddDepartment.txtactqty.value="";
		document.frmaddDepartment.txtplqty.value="";
		document.frmaddDepartment.txtactnob.focus();
		return false;
	}
	else
	{
		if(parseFloat(conval1) > parseFloat(document.frmaddDepartment.txtdiffqty.value))
		{
			alert("Condition Seed Qty cannot be more than Difference Qty");
			document.frmaddDepartment.txtactqty.value="";
			document.frmaddDepartment.txtplqty.value="";
			document.frmaddDepartment.txtactqty.focus();
			return false;
		...
		{
			document.frmaddDepartment.txtplqty.value=parseFloat(document.frmaddDepartment.txtdiffqty.value)-parseFloat(conval1) 
		}
	}
}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" style="border-bottom:solid; border-bottom-color:#F1B01E" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Opening Stock - Condition Seed (Lots-listed in application) - Edit</td>
	    </tr></table></td>
...
<tr class="tblsubtitle" height="20">
  <td colspan="6" align="center" class="tblheading">Opening Stock - Condition Seed (Lots-listed in application) - Edit</td>
</tr>
...
$sql_sloc=mysqli_query($link,"select * from tbl_opspasub_sub where opspa_id='".$arrival_id."' and opspasub_id='".$row_tbl_sub['opspasub_id']."' and plantcode='$plantcode' order by opspasubsub_id") or die(mysqli_error($link));
...
<table align="center" width="950" cellpadding="5" cellspacing="5" border="0" >
<tr >
<td valign="top" align="right"><a href="home_optrn2.php" tabindex="20"><img src="../images/cancel.gif" border="0"style="display:inline;cursor:Pointer;" onclick="return confirm('Do You wish to Cancel this Transaction?');" /></a>&nbsp;&nbsp;<input name="Submit" type="image" src="../images/preview.gif" alt="Submit Value"  border="0" style="display:inline;cursor:Pointer;" tabindex="" onClick="return mySubmit();">&nbsp;&nbsp;</td>
</tr>
</table>
</td><td width="30"></td>
</tr>
