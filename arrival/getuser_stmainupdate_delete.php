...
//exit;
	...
	$barcode=trim($_REQUEST['delbarcode']);
...
$z1=$maintrid;		
//echo "select * from tbl_arrpack_barcode where arrpackss2_barcode='".$barcode."' and arrpack_id='".$z1."'";
$sql_barcode=mysqli_query($link,"select * from tbl_arrpack_barcode where arrpackss2_barcode='".$barcode."' and arrpack_id='".$z1."' and plantcode='$plantcode'") or die(mysqli_error($link));
$tot_barcode=mysqli_num_rows($sql_barcode);
while($row_barcode=mysqli_fetch_array($sql_barcode))
{		
	$bartype=$row_barcode['arrpackss2_bartype'];
	$lotnop=$row_barcode['arrpackss2_lotnop'];
	$lotqty=$row_barcode['arrpackss2_lotqty'];
	$barqty=$row_barcode['arrpackss2_barqty'];
	$grswt=$row_barcode['arrpackss2_grosswt'];
	//echo "select * from tbl_arrpack_sub where arrpacks_id='".$row_barcode['arrpacks_id']."' ";
	$sql_sub=mysqli_query($link,"select * from tbl_arrpack_sub where arrpacks_id='".$row_barcode['arrpacks_id']."' and plantcode='$plantcode' ") or die(mysqli_error($link));
	$tot_sub=mysqli_num_rows($sql_sub);
	while($row_sub=mysqli_fetch_array($sql_sub))
	{
		$nop=$row_sub['arrpacks_nop'];
		$loosenop=$row_sub['arrpacks_loosenop'];
		$nomp=$row_sub['arrpacks_enomp'];
		$qty=$row_sub['arrpacks_eqty'];
		
		$arrnop=$row_sub['arrpacks_nop'];
		$arrnomp=$row_sub['arrpacks_nomp'];
		$arrqty=$row_sub['arrpacks_qty'];
		
		$arrnop=$arrnop-$row_barcode['arrpackss2_lotnop'];
		$arrnomp=$arrnomp-1;
		$arrqty=$arrqty-$row_barcode['arrpackss2_lotqty'];
		
		$balnop=$nop+$arrnop;
		$balnomp=$nomp+1;
		$balqty=$qty+$arrqty;
	}
	//echo "select * from tbl_arrpack_subsub where arrpackss_id='".$row_barcode['arrpackss_id']."' ";
	$sql_subsub=mysqli_query($link,"select * from tbl_arrpack_subsub where arrpackss_id='".$row_barcode['arrpackss_id']."' and plantcode='$plantcode' ") or die(mysqli_error($link));
	$tot_subsub=mysqli_num_rows($sql_subsub);
	while($row_subsub=mysqli_fetch_array($sql_subsub))
	{
		$snop=$row_subsub['arrpackss_nop'];
		$snomp=$row_subsub['arrpackss_nomp'];
		$sqty=$row_subsub['arrpackss_qty'];
		
		$snop=$snop-$row_barcode['arrpackss2_lotnop'];
		$snomp=$snomp-1;
		$sqty=$sqty-$arrqty-$row_barcode['arrpackss2_lotqty'];
	}
	
	$sql_arrsub="update tbl_arrpack_sub set arrpacks_nop='$arrnop', arrpacks_nomp='$arrnomp', arrpacks_qty='$arrqty', arrpacks_balnop='$balnop', arrpacks_balnomp='$balnomp', arrpacks_balqty='$balqty' where arrpacks_id='".$row_barcode['arrpacks_id']."' ";
	mysqli_query($link,$sql_arrsub) or die(mysqli_error($link));
	
	$sql_sub_sub="update tbl_arrpack_subsub set arrpackss_nop='$snop', arrpackss_nomp='$snomp', arrpackss_qty='$sqty' where arrpacks_id='".$row_barcode['arrpackss_id']."'";
	mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
	
	$sql_impsub="update tbl_stlotimp_pack_sub set stlotimpp_arrnop='$arrnop', stlotimpp_arrnomp='$arrnomp', stlotimpp_arrqty='$arrqty', stlotimpp_balnop='$balloosenop', stlotimpp_balnomp='$balnomp', stlotimpp_balqty='$balqty' where stlotimpp_id='$epid' and stlotimpp_lotno='".$row_barcode['arrpackss2_lotno']."' ";
...
<tr class="Light" height="20">
	...
	<td width="10%" align="left" valign="middle" class="smalltbltext" id="wh<?php echo $srno;?>">&nbsp;<?php if($wh1!="") {echo $wh1; }else{?><select class="smalltbltext" id="txtwhg_<?php echo $srno?>" name="txtwhg_<?php echo $srno?>" style="width:70px;" onchange="wh(this.value,'<?php echo $srno?>');"  >
<option value="" selected>WH</option>
	...
	<td width="10%" align="left" valign="middle" class="smalltbltext" id="bin<?php echo $srno;?>">&nbsp;<?php if($bin!="") {echo $wh1; }else{?><select class="smalltbltext" id="vbin_<?php echo $srno;?>" name="vbin_<?php echo $srno;?>" style="width:60px;" onchange="bin2(this.value,'<?php echo $srno?>');" >
<option value="" selected>Bin</option>
	...
	<td width="10%" align="left" valign="middle" class="smalltbltext" id="subbin<?php echo $srno;?>">&nbsp;<?php if($subbin!="") {echo $subbin; }else{?><select class="smalltbltext" id="<?php echo $srno;?>" name="<?php echo $srno;?>" style="width:60px;" onchange="bin5(this.value,<?php echo $srno?>);"  >
