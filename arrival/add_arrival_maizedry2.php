<?php
	...
	$sql_tbl=mysqli_query($link,"select * from tblarrival where arr_role='".$logid."' and  arrival_type='Maize Dry Arrival' and plantcode='$plantcode' and arrtrflag=0") or die(mysqli_error($link));
	while($row_tbl=mysqli_fetch_array($sql_tbl))
	{
		$arrival_id=$row_tbl['arrival_id'];	
		
		$s_sub="delete from tblarrival_sub where arrival_id='".$arrival_id."' and plantcode='$plantcode'";
		mysqli_query($link,$s_sub) or die(mysqli_error($link));
		
		$s_sub_sub="delete from tblarr_sloc where arr_tr_id='".$arrival_id."' and plantcode='$plantcode'";
		mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
		
	}

	$s_sub="delete from tblarrival where arr_role='".$logid."' and arrival_type='Maize Dry Arrival' and arrtrflag=0 and plantcode='$plantcode'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='add_arrival_maizedry22.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='add_arrival_maizedry2.php'</script>";
		}
	}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#F1B01E" style="border-bottom:solid; border-bottom-color:#F1B01E" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Arrival Maize-Dried Seed</td>
	    </tr></table></td>
...
$sql_arr_home=mysqli_query($link,"select * from tblarrival where arrival_type='Stocktransfer' and arrtrflag=1 and plantcode='$plantcode'") or die(mysqli_error($link));
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tblarrival where arrival_type='Maize Dry Arrival' and arrtrflag=1 and plantcode='$plantcode' order by arrival_date desc, arr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tblarrival where arrival_type='Maize Dry Arrival' and arrtrflag=1 and plantcode='$plantcode' order by arrival_date desc, arr_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tblarrival where arrival_type='Maize Dry Arrival' and arrtrflag=1 order by arrival_date desc, arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tblarrival where arrival_type='Maize Dry Arrival' and arrtrflag=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
...
<tr class="Light">
         ...
         <td width="10%" align="center" valign="middle" class="smalltblheading"><a href="add_arrival_maizedry_viewpage.php?cropid=<?php echo $row_arr_home['arrival_id'];?>"><?php echo "AS".$row_arr_home['arr_code']."/".$yearid_id."/".$lrole;?></a></td>
