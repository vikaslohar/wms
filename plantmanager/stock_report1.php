<?php
	...
	/*$connnew = mysqli_connect("192.168.3.18","it","user@2024");
	if (!$connnew) {
	 echo "Not Connected";
	...
	echo "Connected";
       ...
       // $conn = mysqli_connect("192.168.3.18.3306","it","user@2024","seedtracwms_hyd");
        // $conn = mysqli_connect("192.168.3.18","root","Java#545271054","seedtracwms_hyd");

	$connnew = new mysqli("d192.168.3.18:3306","it","user@2024");
	if($connnew -> connect_errno)
	{
	      echo "Failed to establish the connection to the database.<BR>";
	      echo "Reason: ", $connnew -> connect_error;
	      exit();
	}
	else
	{
...
exit();
	...
	//$connnew = new mysqli("192.168.3.18","itadmin","user@2022","seedtracwms_hyd");
	//$connnew->set_charset("utf8mb4_unicode	");
	if($connnew -> connect_errno)
	{
	      echo "Failed to establish the connection to the database.<BR>";
	      echo "Reason: ", $connnew -> connect_error;
	...
	$connnew = mysqli_connect("dev.vnrseeds.in","root","Aax413W!.(6dX_YD");
	if (!$connnew) {
	 echo "Not Connected";
	}
	else {
	...
	*/
	...
	if($variety!="ALL")
	{	
		$sqlvar=mysqli_query($link,"select * from tblvariety where varietyid='".$variety."' ") or die(mysqli_error($link));
...
winHandle=window.open('stock_report2.php?slchk='+slchk+'&slchk2='+slchk2+'&txtcrop='+itemid+'&txtvariety='+vv+'&sdate='+sdate+'&txtplant='+txtplant,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <?php
	...
	if($txtplant!="Bandamailaram")
	{	
	$crp="ALL"; $ver="ALL";
	$qry="select Distinct lotldg_crop from tbl_lot_ldg where lotldg_id!=0 and lotldg_trdate<='$stdate' $platc ";
	$qry2="select Distinct lotldg_crop from tbl_lot_ldg_pack where lotdgp_id!=0 and lotldg_trdate<='$stdate' $platc ";
	$qry3="select Distinct salesrs_crop from tbl_salesrv_sub where salesrs_rettype='P2P' $platc ";
	if($crop!="ALL")
	{	
...
if($crval<>"")
{
	...
	$qry="select Distinct lotldg_variety from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_trdate<='$stdate' $platc ";
	$qry2="select Distinct lotldg_variety from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and lotldg_trdate<='$stdate' $platc ";
	$qry3="select Distinct salesrs_variety from tbl_salesrv_sub where salesrs_crop='".$crval."' $platc ";
	if($variety!="ALL")
	{	
		$qry.=" and lotldg_variety='$variety' ";
		$qry2.=" and lotldg_variety='$variety' ";
...
<tr height="25" >
	<td align="left" class="subheading" style="color:#303918;">&nbsp;&nbsp;Crop: <?php echo $crpn;?>&nbsp;&nbsp;|&nbsp;&nbsp;Variety: <?php echo $vern;?>|&nbsp;&nbsp;Plant: <?php if($txtplant=="ALL"){echo "Boriya &amp; Deorjhal";} else { if($txtplant!="Bandamailaram"){echo $txtplant;}}?></td>
...
<tr class="tblsubtitle" height="20">
  ...
  <td width="40"  align="center" valign="middle" class="tblheading">QC</td>
...
$srno=1; $totalrnob=0; $totalrqty=0; $totalcnob=0; $totalcqty=0; $totalpnob=0; $totalpnomp=0; $totalpqty=0; $totalnob=0; $totalqty=0; $totalsrnob=0; $totalsrqty=0; 
		...
		$qrylt="select Distinct orlot from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_trdate<='$stdate' $platc ";
		$qrylt2="select Distinct orlot from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and lotldg_trdate<='$stdate' $platc ";
		$qrylt3="select Distinct salesrs_orlot from tbl_salesrv_sub where salesrs_crop='".$crval."'  and salesrs_rettype='P2P' $platc ";
		if($variety!="ALL")
		{	
		...
		$sql_qct1=mysqli_query($link,"select MAX(tid) from tbl_qctest where oldlot='".$lt2."' $platc ") or die(mysqli_error($link));
		$row_qct1=mysqli_fetch_array($sql_qct1);
		
		$sql_qct=mysqli_query($link,"select * from tbl_qctest where tid='".$row_qct1[0]."' and oldlot='".$lt2."' $platc ") or die(mysqli_error($link));
		...
		{  
			$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crval."' and orlot='$lt2' and lotldg_variety='".$verval."' and lotldg_sstage='$stage' and lotldg_trdate<='$stdate' $platc group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
			while($row_is=mysqli_fetch_array($sql_is))
			{ 
				$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and orlot='$lt2' and lotldg_sstage='$stage' and lotldg_trdate<='$stdate' $platc order by lotldg_id desc ") or die(mysqli_error($link));
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select lotldg_balqty,lotldg_balbags,lotldg_got1,lotldg_got,lotldg_gemp,lotldg_gottestdate,lotldg_qc,lotldg_qctestdate from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' $platc order by lotldg_id asc") or die(mysqli_error($link)); 
		...
		{  
			...
			$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crval."' and orlot='$lt2' and lotldg_variety='".$verval."' and lotldg_sstage='$stage1' and lotldg_trdate<='$stdate' $platc group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
			while($row_is=mysqli_fetch_array($sql_is))
			{ 
				$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and orlot='$lt2' and lotldg_sstage='$stage1' and lotldg_trdate<='$stdate' $platc order by lotldg_id desc ") or die(mysqli_error($link));
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select lotldg_balqty,lotldg_balbags,lotldg_got1,lotldg_got,lotldg_gemp,lotldg_gottestdate,lotldg_qc,lotldg_qctestdate from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' $platc order by lotldg_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
		...
		//while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_is=mysqli_query($link,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and orlot='$lt2' and lotldg_variety='".$verval."' and lotldg_trdate<='$stdate' $platc group by subbinid order by lotdgp_id asc") or die(mysqli_error($link));
			while($row_is=mysqli_fetch_array($sql_is))
			{ 
				$sql_is1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_is['subbinid']."' and binid='".$row_is['binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and orlot='$lt2' and lotldg_trdate<='$stdate' $platc order by lotdgp_id desc ") or die(mysqli_error($link));
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select balqty,balnomp,packtype,wtinmp,lotldg_got1,lotldg_got,lotldg_gemp,lotldg_gottestdate,lotldg_qc,lotldg_qctestdate from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' $platc order by lotdgp_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_istbl))
		...
		$sql_arhome=mysqli_query($link,"select salesr_id,salesrs_id,salesrs_newlot from tbl_salesrv_sub where salesrs_crop='".$crval."' and salesrs_variety='".$verval."' and salesrs_rettype='P2P' and salesrs_dovfy<='$stdate' and salesrs_orlot='$lt2' $platc order by 'salesrs_id' asc") or die(mysqli_error($link));
		while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_istbl=mysqli_query($link,"select salesrss_qty, salesrss_nob from tbl_salesrvsub_sub where salesrs_id='".$row_arhome['salesrs_id']."' $platc ") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				while($row_issuetbl=mysqli_fetch_array($sql_istbl))
				{ 
					$s_srm=mysqli_query($link,"select * from tbl_salesrv where salesr_date<='$stdate' and salesr_id='".$row_arhome['salesr_id']."' $platc ") or die(mysqli_error($link));
					...
					if($t_srm > 0)
					{
						...
						if($qt>0)
						{
							...
							$sq_srrv=mysqli_query($link,"Select srrv_lotno from tbl_srrevalidate where srrv_lotno='".$row_arhome['salesrs_newlot']."' and srrv_date<='$stdate' $platc ") or die(mysqli_error($link));
							$tot_srrv=mysqli_num_rows($sq_srrv);
							
							$sq_p2c=mysqli_query($link,"Select unp_lotno from tbl_unpsp2c where unp_lotno='".$row_arhome['salesrs_newlot']."' and unp_date<='$stdate' $platc ") or die(mysqli_error($link));
...
$arryear='';		
$sqp2c=mysqli_query($link,"Select * from tbl_blends where blends_orlot='".$lt2."' $platc order by blendm_id desc, blends_id desc ") or die(mysqli_error($link));
...
if($txtplant=="ALL" || $txtplant=="Bandamailaram")
{
	...
	$connnew = mysqli_connect("172.16.16.52:3306","rootnew","New@201786");
	if ($connnew) {	
	...
	if($crop!="ALL")
	{
		$sqlcrop=mysqli_query($connnew,"select * from tblcrop where cropname='".$cropnam."'") or die(mysqli_error($connnew));
		$rowcrop=mysqli_fetch_array($sqlcrop);
		$crop=$rowcrop['cropid'];
	}	
	...
	if($variety!="ALL")
	{	
		$sqlvar=mysqli_query($connnew,"select * from tblvariety where popularname='".$verietynam."' ") or die(mysqli_error($connnew));
		$rowvar=mysqli_fetch_array($sqlvar);
		$variety=$rowvar['varietyid'];
	}	
	...
	$sql_arr_home1=mysqli_query($connnew,$qry) or die(mysqli_error($connnew));
	$sql_arr_home2=mysqli_query($connnew,$qry2) or die(mysqli_error($connnew));
	$sql_arr_home3=mysqli_query($connnew,$qry3) or die(mysqli_error($connnew));
 	$tot_arr_home=mysqli_num_rows($sql_arr_home1);

	$croparr="";
	while($row_arr_home12=mysqli_fetch_array($sql_arr_home1))
	{
		$sql_crop2=mysqli_query($connnew,"select * from tblcrop where cropid='".$row_arr_home12['lotldg_crop']."' order by cropname asc") or die(mysqli_error($connnew));
		$row312=mysqli_fetch_array($sql_crop2);
		if($croparr!="")
		$croparr=$croparr.",".$row312['cropname'];
		else
	...
	while($row_arr_home22=mysqli_fetch_array($sql_arr_home2))
	{
		$sql_crop2=mysqli_query($connnew,"select * from tblcrop where cropid='".$row_arr_home22['lotldg_crop']."' order by cropname asc") or die(mysqli_error($connnew));
		$row312=mysqli_fetch_array($sql_crop2);
		if($croparr!="")
		$croparr=$croparr.",".$row312['cropname'];
		else
	...
	while($row_arr_home23=mysqli_fetch_array($sql_arr_home3))
	{
		$sql_crop2=mysqli_query($connnew,"select * from tblcrop where cropid='".$row_arr_home23['salesrs_crop']."' order by cropname asc") or die(mysqli_error($connnew));
		$row312=mysqli_fetch_array($sql_crop2);
		if($croparr!="")
		$croparr=$croparr.",".$row312['cropname'];
		else
	...
	for($i=0; $i<count($cp); $i++)
	{
		if($cp[$i]!="")
		{
			$sql_crop2=mysqli_query($connnew,"select * from tblcrop where cropname='".$cp[$i]."' order by cropname asc") or die(mysqli_error($connnew));
			$row312=mysqli_fetch_array($sql_crop2);
			if($crop2!="")
			$crop2=$crop2.",".$row312['cropid'];
			else
...
if($crval<>"")
{
	...
	$sql_crop=mysqli_query($connnew,"select * from tblcrop where cropid='".$crval."'") or die(mysqli_error($connnew));
	$row31=mysqli_fetch_array($sql_crop);
	$crop1=$row31['cropname'];	
	...
	{	
		...
		$qry2.=" and lotldg_variety='$variety' ";
		$qry3.=" and salesrs_variety='$variety' ";
		$sql_var=mysqli_query($connnew,"select * from tblvariety where varietyid='".$variety."'") or die(mysqli_error($connnew));
		$row_var=mysqli_fetch_array($sql_var);
		$tot_var=mysqli_num_rows($sql_var);
		if($tot_var>0)
		$ver=$row_var['popularname'];
	...
	$sql_arr_home12=mysqli_query($connnew,$qry) or die(mysqli_error($connnew));
	$sql_arr_home22=mysqli_query($connnew,$qry2) or die(mysqli_error($connnew));
	$sql_arr_home23=mysqli_query($connnew,$qry3) or die(mysqli_error($connnew));

	$verarr="";
	while($row_arr_home12=mysqli_fetch_array($sql_arr_home12))
	{
		$sql_crop2=mysqli_query($connnew,"select * from tblvariety where varietyid='".$row_arr_home12['lotldg_variety']."' order by popularname asc") or die(mysqli_error($connnew));
		$row312=mysqli_fetch_array($sql_crop2);
		$tot312=mysqli_num_rows($sql_crop2);
		if($tot312>0)
		$vr=$row312['popularname'];
	...
	while($row_arr_home22=mysqli_fetch_array($sql_arr_home22))
	{
		$sql_crop2=mysqli_query($connnew,"select * from tblvariety where varietyid='".$row_arr_home22['lotldg_variety']."' order by popularname asc") or die(mysqli_error($connnew));
		$row312=mysqli_fetch_array($sql_crop2);
		$tot312=mysqli_num_rows($sql_crop2);
		if($tot312>0)
		$vr=$row312['popularname'];
	...
	while($row_arr_home23=mysqli_fetch_array($sql_arr_home23))
	{
		$sql_crop2=mysqli_query($connnew,"select * from tblvariety where varietyid='".$row_arr_home23['salesrs_variety']."' order by popularname asc") or die(mysqli_error($connnew));
		$row312=mysqli_fetch_array($sql_crop2);
		$tot312=mysqli_num_rows($sql_crop2);
		if($tot312>0)
		$vr=$row312['popularname'];
	...
	for($i=0; $i<count($cp2); $i++)
	{
		if($cp2[$i]!="")
		{
			$sql_crop2=mysqli_query($connnew,"select * from tblvariety where popularname='".$cp2[$i]."' order by popularname asc") or die(mysqli_error($connnew));
			$row312=mysqli_fetch_array($sql_crop2);
			$tot312=mysqli_num_rows($sql_crop2);
			if($tot312>0)
			$vr=$row312['varietyid'];
	...
	if($verval<>"")
	{
		...
		$sql_crp=mysqli_query($connnew,"select * from tblcrop where cropid='".$crval."'") or die(mysqli_error($connnew));
		$row_crp=mysqli_fetch_array($sql_crp);
		$crpn=$row_crp['cropname'];
		$sql_var=mysqli_query($connnew,"select * from tblvariety where varietyid='".$verval."' ") or die(mysqli_error($connnew));
		$vtot=mysqli_num_rows($sql_var);
		if($vtot>0)
		{
			$row_var=mysqli_fetch_array($sql_var);
...
?> 
  ...
  <table align="center" border="1" cellspacing="0" cellpadding="0" width="970" bordercolor="#2e81c1" style="border-collapse:collapse">
<tr height="25" >
	<td align="left" class="subheading" style="color:#303918;">&nbsp;&nbsp;Crop: <?php echo $crpn;?>&nbsp;&nbsp;|&nbsp;&nbsp;Veriety: <?php echo $vern;?>&nbsp;&nbsp;|&nbsp;&nbsp;Plant: <?php echo "Bandamailaram";?></td>
...
$srno=1; $totalrnob=0; $totalrqty=0; $totalcnob=0; $totalcqty=0; $totalpnob=0; $totalpnomp=0; $totalpqty=0; $totalnob=0; $totalqty=0; $totalsrnob=0; $totalsrqty=0; 
		...
		$sql_qct1=mysqli_query($connnew,"select MAX(tid) from tbl_qctest where oldlot='".$lt2."' $platc ") or die(mysqli_error($connnew));
		$row_qct1=mysqli_fetch_array($sql_qct1);
		
		$sql_qct=mysqli_query($connnew,"select * from tbl_qctest where tid='".$row_qct1[0]."' and oldlot='".$lt2."' $platc ") or die(mysqli_error($connnew));
		$row_qct=mysqli_fetch_array($sql_qct);
		...
		// Raw Seed Records
		//$sql_arhome=mysqli_query($connnew,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_sstage='$stage' and orlot='$lt2' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($connnew));
		//while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_is=mysqli_query($connnew,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crval."' and orlot='$lt2' and lotldg_variety='".$verval."' and lotldg_sstage='$stage' and lotldg_trdate<='$stdate' $platc group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($connnew));
			while($row_is=mysqli_fetch_array($sql_is))
			{ 
				$sql_is1=mysqli_query($connnew,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and orlot='$lt2' and lotldg_sstage='$stage' and lotldg_trdate<='$stdate' $platc order by lotldg_id desc ") or die(mysqli_error($connnew));
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($connnew,"select lotldg_balqty,lotldg_balbags,lotldg_got1,lotldg_got,lotldg_gemp,lotldg_gottestdate,lotldg_qc,lotldg_qctestdate from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' $platc order by lotldg_id asc") or die(mysqli_error($connnew)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_istbl))
		...
		// Condition Seed Records
		//$sql_arhome=mysqli_query($connnew,"select distinct lotldg_lotno from tbl_lot_ldg where  lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_sstage='$stage1' and orlot='$lt2' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($connnew));
		//while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			
			$sql_is=mysqli_query($connnew,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crval."' and orlot='$lt2' and lotldg_variety='".$verval."' and lotldg_sstage='$stage1' and lotldg_trdate<='$stdate' $platc group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($connnew));
			while($row_is=mysqli_fetch_array($sql_is))
			{ 
				$sql_is1=mysqli_query($connnew,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and orlot='$lt2' and lotldg_sstage='$stage1' and lotldg_trdate<='$stdate' $platc order by lotldg_id desc ") or die(mysqli_error($connnew));
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($connnew,"select lotldg_balqty,lotldg_balbags,lotldg_got1,lotldg_got,lotldg_gemp,lotldg_gottestdate,lotldg_qc,lotldg_qctestdate from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' $platc order by lotldg_id asc") or die(mysqli_error($connnew)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
		...
		// Pack Seed Records
		//$sql_arhome=mysqli_query($connnew,"select distinct lotno from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and orlot='$lt2' group by lotno order by lotdgp_id asc") or die(mysqli_error($connnew));
		//while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_is=mysqli_query($connnew,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and orlot='$lt2' and lotldg_variety='".$verval."' and lotldg_trdate<='$stdate' $platc group by subbinid order by lotdgp_id asc") or die(mysqli_error($connnew));
			while($row_is=mysqli_fetch_array($sql_is))
			{ 
				$sql_is1=mysqli_query($connnew,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_is['subbinid']."' and binid='".$row_is['binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and orlot='$lt2' and lotldg_trdate<='$stdate' $platc order by lotdgp_id desc ") or die(mysqli_error($connnew));
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($connnew,"select balqty,balnomp,packtype,wtinmp,lotldg_got1,lotldg_got,lotldg_gemp,lotldg_gottestdate,lotldg_qc,lotldg_qctestdate from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' $platc order by lotdgp_id asc") or die(mysqli_error($connnew)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_istbl))
		...
		// Sales Return Seed Records
		$sql_arhome=mysqli_query($connnew,"select salesr_id,salesrs_id,salesrs_newlot from tbl_salesrv_sub where salesrs_crop='".$crval."' and salesrs_variety='".$verval."' and salesrs_rettype='P2P' and salesrs_dovfy<='$stdate' and salesrs_orlot='$lt2' $platc order by 'salesrs_id' asc") or die(mysqli_error($connnew));
		while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_istbl=mysqli_query($connnew,"select salesrss_qty, salesrss_nob from tbl_salesrvsub_sub where salesrs_id='".$row_arhome['salesrs_id']."' $platc ") or die(mysqli_error($connnew)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				while($row_issuetbl=mysqli_fetch_array($sql_istbl))
				{ 
					$s_srm=mysqli_query($connnew,"select * from tbl_salesrv where salesr_date<='$stdate' and salesr_id='".$row_arhome['salesr_id']."' $platc ") or die(mysqli_error($connnew));
					$t_srm=mysqli_num_rows($s_srm);
					if($t_srm > 0)
					{
						$r_srm=mysqli_fetch_array($s_srm);
						...
						{
							$tot_p2c=0; $tot_srrv=0;
							$sq_srrv=mysqli_query($connnew,"Select srrv_lotno from tbl_srrevalidate where srrv_lotno='".$row_arhome['salesrs_newlot']."' and srrv_date<='$stdate' $platc ") or die(mysqli_error($connnew));
							$tot_srrv=mysqli_num_rows($sq_srrv);
							
							$sq_p2c=mysqli_query($connnew,"Select unp_lotno from tbl_unpsp2c where unp_lotno='".$row_arhome['salesrs_newlot']."' and unp_date<='$stdate' $platc ") or die(mysqli_error($connnew));
							$tot_p2c=mysqli_num_rows($sq_p2c);
...
$arryear='';		
$sqp2c=mysqli_query($connnew,"Select * from tbl_blends where blends_orlot='".$lt2."' $platc order by blendm_id desc, blends_id desc ") or die(mysqli_error($connnew));
if($totp2c=mysqli_num_rows($sqp2c)>0)
{
$rowsp2c=mysqli_fetch_array($sqp2c);
