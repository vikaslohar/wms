...
{ 
	...
	if($plantcode=='H')
	{
		...
		$ch = curl_init();
			$curlConfig = array(
				CURLOPT_URL            => "http://103.211.8.126:8080/wmsrepapi/reportlist.php",
				CURLOPT_POST           => true,
		...
		//$test=explode(",",$result);
		//print_r($phpArray['user']['orlotarray']);
		curl_close($ch);
		$lotdetar=($phpArray['user']['orlotarray']);
		//$lotdetar=explode(" - ",($phpArray['user']['orlotarray']));
	//	print_r($lotdetar);
		
		$htotrqty=$htotrqty+$lotdetar[0];
		$htotalrqty=$htotalrqty+$lotdetar[0];

		$htotcqty=$htotcqty+$lotdetar[1]; 
		$htotalcqty=$htotalcqty+$lotdetar[1];
		...
		$htotpqty=$htotpqty+$lotdetar[2]; 
		$htotalpqty=$htotalpqty+$lotdetar[2];
		...
		$htotsrqty=$htotsrqty+$lotdetar[3];
		$htotalsrqty=$htotalsrqty+$lotdetar[3];
