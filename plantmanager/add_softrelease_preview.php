<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		while($row_srsub=mysqli_fetch_array($sql_srsub))
		{
			...
			$slqty=0;
			$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where  orlot='".$lotno."'  and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT') AND plantcode='$plantcode'") or die(mysqli_error($link));
			$zz=mysqli_num_rows($sql_issue);
			while($row_issue=mysqli_fetch_array($sql_issue))
			{ 
				$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and orlot='".$lotno."'  and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT') AND plantcode='$plantcode'") or die(mysqli_error($link));
			...
			$slqty=0;
			$sql_issue=mysqli_query($link,"select distinct whid, subbinid, binid from tbl_lot_ldg_pack where orlot='".$lotno."' and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT') AND plantcode='$plantcode'") or die(mysqli_error($link));
			$zz=mysqli_num_rows($sql_issue);
			while($row_issue=mysqli_fetch_array($sql_issue))
			{ 
				$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and orlot='".$lotno."'  and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT') AND plantcode='$plantcode'") or die(mysqli_error($link));
				$row_issue1=mysqli_fetch_array($sql_issue1); 
		...
		$sql_code="SELECT MAX(softr_code) FROM tbl_softr  where yearcode='$yearid_id' AND plantcode='$plantcode' ORDER BY softr_code DESC";
