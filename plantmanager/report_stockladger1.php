...
///		echo "Outword stock Records <br/>";
		...
		while($row_is=mysqli_fetch_array($sql_is))
		{ //echo $row_is['dpss_id']."  =  ";
			$sql_istbl=mysqli_query($link,"select * from tbl_disp where disp_id='".$row_is['disp_id']."' and disp_dodc>='$stdate' and disp_dodc<='$etdate' and disp_partytype!='C&F' and disp_tflg=1 and plantcode='$plantcode' order by disp_id desc ") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
