...
$tot_row=0;
$lotqry=mysqli_query($link,"select * from tblarrival_sub where lotno='".$a."' and lotcrop='".$crop."' and lotvariety='".$variet."' and plantcode='$plantcode'")or die (mysqli_error($link));
...
$lot=$row['lotcrop'];	
 ...
 if($row['lotvariety']!="")
 {
 	...
 	$lotqry1=mysqli_query($link,"select * from tblvariety where popularname='".$variety."' ");
 ...
 else
 {
	...
	$x=$row_spc['variety'];
	$z=$row_spc['crop'];
	$lotqry1=mysqli_query($link,"select * from tblvariety where varietyid='".$x."' ");
	$t=mysqli_num_rows($lotqry1);
	$row11=mysqli_fetch_array($lotqry1)or die (mysqli_error($link));
 ...
 $sql_arr_home=mysqli_query($link,"select * from tblarrival where arrival_type='Fresh Seed with PDN' and  arrival_date <= '$edate' and arrival_date >= '$sdate' and lotcrop='".$itemid."' and lotvariety='".$vv."' and arrtrflag=1 and plantcode='$plantcode' order by arrival_date asc ") or die(mysqli_error($link));
