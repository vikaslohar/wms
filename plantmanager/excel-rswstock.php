...
session_start();
	...
	$dt=date("d-m-Y");	
	$crp="ALL"; $ver="ALL";
	$qry="select Distinct lotcrop from tblarrival_sub where prodtype='Potent' and plantcode='$plantcode'";
	$qry.=" group by lotcrop order by lotcrop asc";
	...
	{
		$sql_crop2=mysqli_query($link,"select * from tblcrop where cropname='".$row_arr_home12['lotcrop']."' order by cropname asc") or die(mysqli_error($link));
	...
	$dh="RSW_Crop_Variety_Lotno_wise_Stock_Report_As_on_Date_".$dt;
	$datahead = array("RSW Crop Variety Lotno wise Stock Report As on Date $dt");
	$filename=$dh.".xls";  
...
if($crval<>"")
{
	...
	{  
		...
		{  
			...
			$sqlarrsub=mysqli_query($link,"Select lotno from tblarrival_sub where lotno='$lotno' and prodtype='Potent' and plantcode='$plantcode'") or die (mysqli_error($link));
