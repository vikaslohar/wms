<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$sql_srmain="Insert into tbl_softr (softr_tcode, softr_date, softr_crop, softr_variety, softr_typ, softr_wh, softr_bin, softr_subbin, yearcode, plantcode) values('$txtid', '$tdate', '$crop', '$variety', '$typ', '$txtwh', '$txtbin', '$txtsubbin', '$yearid_id', '$plantcode')";
		if(mysqli_query($link,$sql_srmain) or die(mysqli_error($link)))
		{
			...
			for($i=0; $i<count($lot1); $i++)
			{
				if($lot1[$i]<>"")
				{
					$sql_srsub="Insert into tbl_softr_sub (softr_id, softrsub_lotno, softrsub_srtyp, softrsub_srflg, plantcode) values('$id', '$lot1[$i]', '$srt1[$i]', '1', '$plantcode')";
