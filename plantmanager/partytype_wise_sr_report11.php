...
winHandle=window.open('report_partytypewisesr22.php?&txtcrop='+itemid+'&txtvariety='+vv+'&sdate='+sdate+'&edate='+edate+'&txtptype='+txtptype+'&txtpp='+txtpp+'&txtstatesl='+txtstatesl,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
...
<tr class="light" height="20">
  <td align="center" class="tblheading">Periodical Party Type wise Sales Return Report</td>
</tr>
</table>
<table align="center" border="0" width="750" cellspacing="0" cellpadding="0" bordercolor="#2e81c1" style="border-collapse:collapse" > 
<tr class="light" height="20">
...
$sqlsrrets.="  and salesrs_vflg=1  order by salesrs_crop, salesrs_variety ";
$sql_srrets=mysqli_query($link,$sqlsrrets) or die(mysqli_error($link));
while($row_srrets=mysqli_fetch_array($sql_srrets))
{

$sql_srretsub24=mysqli_query($link,"select distinct salesrs_ups from tbl_salesrv_sub where salesrs_variety='".$row_srrets['salesrs_variety']."' and salesr_id IN ($mid) and salesrs_vflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_srretsub24=mysqli_fetch_array($sql_srretsub24))
{


$cropn=""; $varietyn=""; $totqty=0; $okqty=0; $failqty=0; $utqty=0; $lotnc=""; $lotnp="";

$sql_srretsub=mysqli_query($link,"select * from tbl_salesrv_sub where salesrs_variety='".$row_srrets['salesrs_variety']."' and salesrs_ups='".$row_srretsub24['salesrs_ups']."' and salesr_id IN ($mid) and salesrs_vflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_srretsub=mysqli_fetch_array($sql_srretsub))
{
	...
	$quer4=mysqli_query($link,"SELECT popularname FROM tblvariety  where varietyid='".$row_srretsub['salesrs_variety']."' and actstatus='Active'"); 
...
<tr height="25">
	...
	<td width="80" align="Center" class="tblheading"><?php echo $row_srretsub24['salesrs_ups'];?></td>
...
<tr >
<td valign="top" align="center"><a href="partytypewise_sr_report.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:pointer;"  /></a>&nbsp;&nbsp;<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" /></td>
</tr>
</table>
</td><td width="30"></td>
</tr>
