<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$sql_srmain="Update tbl_softr set softr_tcode='$txtid', softr_date='$tdate', softr_crop='$crop', softr_variety='$variety', softr_typ='$typ', softr_wh='$txtwh', softr_bin='$txtbin', softr_subbin='$txtsubbin', yearcode='$yearid_id' where softr_id='".$tid."'";
		if(mysqli_query($link,$sql_srmain) or die(mysqli_error($link)))
		{
			$sql_sr="delete from tbl_softr_sub where softr_id='".$tid."'";
			...
			for($i=0; $i<count($lot1); $i++)
			{
				if($lot1[$i]<>"")
				{
					$sql_srsub="Insert into tbl_softr_sub (softr_id, softrsub_lotno, softrsub_srtyp, softrsub_srflg, plantcode) values('$tid', '$lot1[$i]', '$srt1[$i]', '1', '$plantcode')";
					$ss=mysqli_query($link,$sql_srsub) or die(mysqli_error($link));
				}
			}
		}
		echo "<script>window.location='add_softrelease_preview.php?tid=$tid'</script>";	
	}
...
<title>Plant Manager - Transaction - Soft Release</title>
...
<tr class="tblsubtitle" height="20">
  <td colspan="6" align="center" class="tblheading">Soft Release - Edit</td>
</tr>
...
if($type=="sllot")	
{
$sql="SELECT distinct orlot, lotldg_got1 FROM tbl_lot_ldg WHERE lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_srflg='0' and lotldg_qc!='Fail' AND plantcode='$plantcode'";
}
else
{
$sql="SELECT distinct orlot, lotldg_got1 FROM tbl_lot_ldg WHERE lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_whid='".$wh."' and lotldg_binid='".$bin."' and lotldg_subbinid='".$subbin."' and lotldg_srflg='0' and lotldg_qc!='NUT' and lotldg_qc!='Fail' AND plantcode='$plantcode'";
}
...
<?php
$bind1_query=mysqli_query($link,"select binid, binname from tbl_bin where whid='".$whi."' AND plantcode='$plantcode'") or die(mysqli_error($link));
?>
...
$sql_qct=mysqli_query($link,"select * from tbl_qctest where oldlot='".$row_arr_home['orlot']."' and bflg=1 AND plantcode='$plantcode' order by tid desc") or die(mysqli_error($link));
...
//echo $flg;
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where  orlot='".$row_arr_home['orlot']."'  and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT') AND plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and orlot='".$row_arr_home['orlot']."'  and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT') AND plantcode='$plantcode'") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT') AND plantcode='$plantcode'") or die(mysqli_error($link)); 
...
$sql_issue=mysqli_query($link,"select distinct whid, subbinid, binid from tbl_lot_ldg_pack where orlot='".$row_arr_home['orlot']."' and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT') AND plantcode='$plantcode'") or die(mysqli_error($link));
$zz=mysqli_num_rows($sql_issue);
//if($zz==0)$flg=1;
 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
...
$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and orlot='".$row_arr_home['orlot']."'  and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT') AND plantcode='$plantcode'") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_issue1[0]."' and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT') AND plantcode='$plantcode'") or die(mysqli_error($link)); 
 $xxz=mysqli_num_rows($sql_issuetbl);
//if($xxz==0)$flg=1;
 while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
 { 
...
if($flg==0)
{
$sql_srsub=mysqli_query($link,"SELECT * FROM tbl_softr_sub WHERE softr_id='".$tid."' and softrsub_lotno='".$row_arr_home['orlot']."' AND plantcode='$plantcode'") or die(mysqli_error($link));
