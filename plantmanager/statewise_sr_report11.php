...
winHandle=window.open('report_statewisesr22.php?&txtcrop='+itemid+'&txtvariety='+vv+'&sdate='+sdate+'&edate='+edate+'&txtptype='+txtptype+'&txtpp='+txtpp+'&txtstatesl='+txtstatesl,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
	...
	while($rowsrretm=mysqli_fetch_array($sqlsrretm))
	{
		...
		$sqlsrrets2="select distinct salesrs_variety, salesrs_crop from tbl_salesrv_sub where salesr_id IN ($mid) and plantcode='$plantcode'";}
			...
			while($row_srrets2=mysqli_fetch_array($sql_srrets2))
			{
				...
				if($mid==""){$mid='NULL';
				$sqlsrrets="select distinct salesrs_ups from tbl_salesrv_sub where salesrs_variety='".$row_srrets2['salesrs_variety']."' and salesr_id=$mid and plantcode='$plantcode' order by salesrs_ups";}
				else{
				$sqlsrrets="select distinct salesrs_ups from tbl_salesrv_sub where salesrs_variety='".$row_srrets2['salesrs_variety']."' and salesr_id IN ($mid) and plantcode='$plantcode' order by salesrs_ups";}
				//$sqlsrrets="select distinct salesrs_ups from tbl_salesrv_sub where salesr_id='".$row_srretm['salesr_id']."' and salesrs_variety='".$row_srrets2['salesrs_variety']."' order by salesrs_ups";
				...
				while($row_srrets=mysqli_fetch_array($sql_srrets))
				{
					...
					if($mid==""){$mid='NULL';
					$sql_srretsub="select * from tbl_salesrv_sub where salesr_id=$mid and salesrs_variety='".$row_srrets2['salesrs_variety']."' and salesrs_ups='".$row_srrets['salesrs_ups']."' and plantcode='$plantcode'";}
					else{
					$sql_srretsub="select * from tbl_salesrv_sub where salesr_id IN ($mid) and salesrs_variety='".$row_srrets2['salesrs_variety']."' and salesrs_ups='".$row_srrets['salesrs_ups']."' and plantcode='$plantcode'";}
					...
					$diq5=explode(" ",$row_srrets['salesrs_ups']);
					$diq=explode(".",$diq5[0]);
					
					if($diq[1]==000){$ups2=$diq[0];}else{$ups2=$diq5[0];}
					$ups=$ups2." ".$diq5[1];
