<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$sql_srmain="Insert into tbl_softr2 (softr_tcode, softr_date, softr_crop, softr_variety, softr_typ, softr_wh, softr_bin, softr_subbin, yearcode, plantcode) values('$txtid', '$tdate', '$crop', '$variety', '$typ', '$txtwh', '$txtbin', '$txtsubbin', '$yearid_id', '$plantcode')";
		if(mysqli_query($link,$sql_srmain) or die(mysqli_error($link)))
		{
			...
			for($i=0; $i<count($lot1); $i++)
			{
				if($lot1[$i]<>"")
				{
					$sql_srsub="Insert into tbl_softr_sub2 (softr_id, softrsub_lotno, softrsub_srtyp, softrsub_srflg, plantcode) values('$id', '$lot1[$i]', '$srt1[$i]', '1', '$plantcode')";
					$ss=mysqli_query($link,$sql_srsub) or die(mysqli_error($link));
				}
			}
		}
		echo "<script>window.location='add_softrelease_preview2.php?tid=$id'</script>";	
	}
...
<tr class="tblsubtitle" height="20">
  <td colspan="6" align="center" class="tblheading">Super Soft Release - Add</td>
</tr>
...
<tr class="light" height="20">
  <td colspan="6" align="center" class="smalltblheading"><font color="#FF0000">Only Lots having status as:</font> <font color="#003366">QC UT / RT </font><font color="#FF0000">with </font> <font color="#003366">GOT-R UT / RT / OK / NUT</font><font color="#FF0000">&nbsp;&nbsp; will be displayed</font>&nbsp;&nbsp;</td>
</tr> 
<tr class="Light" height="30">
...
<table align="center" border="1" width="850" cellspacing="0" cellpadding="0" bordercolor="#2e81c1" style="border-collapse:collapse" > 
   ...
   <?php
$whd1_query=mysqli_query($link,"select whid, perticulars from tbl_warehouse AND plantcode='$plantcode' order by perticulars") or die(mysqli_error($link));
?>
<tr class="Light" height="30" >
