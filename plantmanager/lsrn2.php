...
<title>Plant-Transaction Super Soft Release - LSRN</title>
...
<table width="750" height="282" border="0" align="center" cellpadding="0" cellspacing="0" >
 ...
 <?php 
$tid=$pid;
$sql_tbl=mysqli_query($link,"select * from tbl_softr2 where softr_id='".$tid."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
<tr  height="20">
  <td colspan="6" align="center" class="Mainheading"><font color="#000000">Lot Super Soft Release Note (LSSRN)</font></td>
</tr>
</table><br />
...
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where  orlot='".$row_arr_home['softrsub_lotno']."'  and lotldg_balqty > 0 and plantcode='$plantcode' ") or die(mysqli_error($link));

 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
$txtdot=""; 

$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and orlot='".$row_arr_home['softrsub_lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and lotldg_balqty > 0  and (lotldg_got='UT' OR lotldg_got='RT' OR lotldg_qc='UT' OR lotldg_qc='RT') and plantcode='$plantcode'") or die(mysqli_error($link)); 
...
$sql_issue=mysqli_query($link,"select distinct whid, subbinid, binid from tbl_lot_ldg_pack where orlot='".$row_arr_home['softrsub_lotno']."' and balqty > 0 and plantcode='$plantcode' ") or die(mysqli_error($link));

 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
$txtdot=""; 

$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and orlot='".$row_arr_home['softrsub_lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_issue1[0]."' and balqty > 0  and (lotldg_got='UT' OR lotldg_got='RT' OR lotldg_qc='UT' OR lotldg_qc='RT') and plantcode='$plantcode'") or die(mysqli_error($link)); 
