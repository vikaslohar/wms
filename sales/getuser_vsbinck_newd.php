...
$sql_sbin=mysqli_query($link,"select * from tbldsubbin where plantcode='$plantcode' AND sid='".$a."'")or die("Error:".mysqli_error($link));
$row_sbn=mysqli_num_rows($sql_sbin); 
$row_sbin=mysqli_fetch_array($sql_sbin);
$tp=$row_sbin['status'];

/*$sqlveriety=mysqli_query($link,"select * from tblvariety where popularname='".$b."'") or die(mysqli_error($link));
...
{
$sqlveriety=mysqli_query($link,"select * from tblvariety where varietyid='".$b."'") or die(mysqli_error($link));
$rowvariety=mysqli_fetch_array($sqlveriety);
...
/*$sql_tr_222=mysqli_query($link,"select * from tbl_salesr_sub where salesrss_subbin='".$a."' order by arrsloc_id desc") or die(mysqli_error($link));
...
$sql_tr=mysqli_query($link,"select * from tbl_salesr_sub where salesr_id='".$trid."' and salesrs_variety!='".$varid11."' and salesrss_subbin='".$a."'") or die(mysqli_error($link));
...
//echo $row_issueg_new['lotldg_lotno'];
	...
	{	$xcflg++;
		...
		/*if($row_issuetblg_new['lotldg_trtype']=="Fresh Seed with PDN")
		{
			...
			{			
				$sql_veriety=mysqli_query($link,"select * from tblvariety where varietyid='".$row_issuetblg_new['lotldg_variety']."'") or die(mysqli_error($link));
...
if($tpmflg==0)
{
	...
	$sql_issuetblg=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='$plantcode' AND lotdgp_id='".$row_issueg1[0]."' and balqty > 0") or die(mysqli_error($link)); 
	 $totno=mysqli_num_rows($sql_issuetblg);
		...
		{		
			$sql_veriety2=mysqli_query($link,"select * from tblvariety where varietyid='".$row_issuetblg_new2['lotldg_variety']."'") or die(mysqli_error($link));
