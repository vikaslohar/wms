<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		while($row_arr=mysqli_fetch_array($sql_arr))
		{
			...
			$sql_arrsub=mysqli_query($link,"select * from tbl_salesrv_sub where plantcode='$plantcode' AND salesr_id='".$pid."'") or die(mysqli_error($link));
			while($row_arrsub=mysqli_fetch_array($sql_arrsub))
			{
				...
				while($row_arrsub_sub=mysqli_fetch_array($sql_arrsub_sub))
				{
					...
					$sql_sub_sub="insert into tbl_lot_ldg (yearcode, lotldg_lotno, lotldg_trtype, lotldg_trid, lotldg_trdate, lotldg_crop, lotldg_variety, lotldg_whid, lotldg_binid, lotldg_subbinid, lotldg_opbags, lotldg_opqty, lotldg_trbags, lotldg_trqty,  lotldg_balbags,  lotldg_balqty, lotldg_sstage, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_qc, orlot, lotldg_qctestdate) values('$yearid_id', '$lotno', 'Sales Recall', '$pid', '$trdate', '$crop', '$variety', '$whid', '$binid', '$subbinid', '$opups', '$opqty', '$ups', '$qty', '$balups', '$balqty' ,'$stage', '$moist', '$gemp', '$vchk', '$qc', '$gln', '$dot')";
					mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));*/
					}
				...
				{
					$sql_sub_sub12="insert into tbl_qctest(pp, moist, got, lotno, srdate, crop,  variety, sampleno, trstage, qc, state, gs, oldlot, bflg,gotsmpdflg,yearid,logid,plantcode)values('$vchk', '$moist', '$got1', '$lotno', '$trdate', '$classid', '$itemid', '$ncode1', '$stage', '$qc', '$state',1,'$gln', 0,0,'$yearid_id','$logid','$plantcode')";
				}	
				else
				{
...
<title>Sales Recall -Transaction - Verification - Preview</title>
...
{ 
	...
	{
		alert("Please Verify the Sales Recall Lots first");
		return false;
	}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" style="border-bottom:solid; border-bottom-color:#a8a09e" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Sales Recall - Verification - Preview</td>
	    </tr></table></td>
