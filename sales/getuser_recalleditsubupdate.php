...
//frm_action=submit&txt11=Transport&txt14=Paid&txtid=1&logid=SR1&gln1=&txtconchk=&txtptype=Dealer&txtcountrysl=&txtcountryl=&date=24-02-2014&dcdate=24-02-2014&txtdcno=test&txtpp=Dealer&rettype=Sales%20Return%20P%20to%20C&txtstatesl=Chhattisgarh&txtlocationsl=34&locationname=34&txtstfp=52&txtdcnopac=20&txtpackagetyp=Both&txt1=Transport&txttname=jhgf&txtlrn=hfg&txtvn=hgf1&txt13=Paid&txtcname=&txtdc=&txtpname=&txtcrop=28&txtvariety=8&itmdchk=&txtupsdc=10%20GMS&txtnopdc=20&txtqtydc=2.8&maintrid=0&subtrid=0
	...
	if(isset($_GET['txtrecallreason'])) { $rettype= $_GET['txtrecallreason']; }
	...
	if(isset($_GET['txtrecrefno'])) { $txtrecrefno = $_GET['txtrecrefno']; }
...
if($z1 == 0)
{
  $sql_main="insert into tbl_salesrv (salesr_yearcode, salesr_trtype, salesr_tcode, salesr_date, salesr_logid, salesr_dcno, salesr_dcdate, salesr_partytype, salesr_state, salesr_loc, salesr_party, salesr_tmode, salesr_tname, salesr_lrno, salesr_vehno, salesr_pmode, salesr_cname, salesr_docket, salesr_pname, salesr_retryp, salesr_dnop, salesr_dnop1, salesr_nob, salesr_nob1, salesr_tnop, salesr_tnop1, salesr_recallrefno, plantcode) values('$yearid_id', 'Sales Recall', '$txtid', '$date', '$logid', '$txtdcno', '$dot', '$txtpp', '$txtstatesl', '$locationname', '$txtstfp', '$txt1', '$txttname', '$txtlrn', '$txtvn', '$txt13', '$txtcname', '$txtdc', '$txtpname', '$rettype', '$txtdcnopbox', '$txtacnopbox', '$txtdcnobag', '$txtacnopbag', '$txtdcnopto', '$txtacnopto', '$txtrecrefno', '$plantcode')";

if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=mysqli_insert_id($link);

$sql_sub="insert into tbl_salesrv_sub (salesr_id, salesrs_crop, salesrs_variety, salesrs_ups, salesrs_nob, salesrs_qty, salesrs_yearcode, salesrs_wh, salesrs_bin, salesrs_upstype, plantcode) values('$mainid', '$txtcrop', '$txtvariety', '$txtupsdc', '$txtnopdc', '$txtqtydc', '$yearid_id', '$txtslwhd1', '$txtslbind1', '$txtupstyp', '$plantcode')";
...
{
$sql_main="update tbl_salesrv set salesr_yearcode='$yearid_id', salesr_trtype='Sales Recall', salesr_tcode='$txtid', salesr_date='$date', salesr_logid='$logid', salesr_dcno='$txtdcno', salesr_dcdate='$dot', salesr_partytype='$txtpp', salesr_state='$txtstatesl', salesr_loc='$locationname', salesr_party='$txtstfp', salesr_tmode='$txt1', salesr_tname='$txttname', salesr_lrno='$txtlrn', salesr_vehno='$txtvn', salesr_pmode='$txt13', salesr_cname='$txtcname', salesr_docket='$txtdc', salesr_pname='$txtpname', salesr_retryp='$rettype', salesr_dnop='$txtdcnopbox', salesr_dnop1='$txtacnopbox', salesr_nob='$txtdcnobag', salesr_nob1='$txtacnopbag', salesr_tnop='$txtdcnopto', salesr_tnop1='$txtacnopto' where salesr_id = '$z1'";
if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=$z1;

$sql_sub="update tbl_salesrv_sub set salesr_id='$mainid', salesrs_crop='$txtcrop', salesrs_variety='$txtvariety', salesrs_ups='$txtupsdc', salesrs_nob='$txtnopdc', salesrs_qty='$txtqtydc', salesrs_yearcode='$yearid_id', salesrs_wh='$txtslwhd1', salesrs_bin='$txtslbind1', salesrs_upstype='$txtupstyp' where salesrs_id='$subtrid'";
mysqli_query($link,$sql_sub) or die(mysqli_error($link));
}
}
?>
