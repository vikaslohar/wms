...
$sql_tbl=mysqli_query($link,"select * from tbl_salesrv where plantcode='$plantcode' AND salesr_logid='".$logid."' and  salesr_trtype='Sales Return' and salesr_flg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['salesr_id'];	
	
	$s_sub="delete from tbl_salesrv_sub where salesr_id='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_salesrvsub_sub where salesr_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
	$s_sub_sub2="delete from tbl_salesrvsub_sub2 where salesr_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub2) or die(mysqli_error($link));
	
}

	$s_sub="delete from tbl_salesrv where salesr_logid='".$logid."' and  salesr_trtype='Sales Return' and salesr_flg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	$sql_code="SELECT MAX(salesr_code) FROM tbl_salesrv where plantcode='$plantcode' AND salesr_trtype='Sales Return' and salesr_yearcode='$yearid_id'  ORDER BY salesr_code DESC";
...
<title>Sales - Transaction - Sales Return Pack to Condition</title>
...
function openslocpopprint2(itm)
{
winHandle=window.open('slrptc_verifysheet.php?itmid='+itm,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); }
}
function openslocpopprint3(itm)
{
winHandle=window.open('slrptc_verifysheet_annex.php?itmid='+itm,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); }
}
</script>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" style="border-bottom:solid; border-bottom-color:#a8a09e" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Sales Return</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><a href="add_sales_returnptc.php" style="text-decoration:none; color:#000000">Add </a></td>
</tr>
</table></td>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_salesrv where plantcode='$plantcode' AND salesr_trtype='Sales Return' and salesr_flg=1 order by salesr_date desc, salesr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_salesrv where plantcode='$plantcode' AND salesr_trtype='Sales Return' and salesr_flg=1 order by salesr_date desc, salesr_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_salesrv where salesr_trtype='Sales Return' and salesr_flg=1 order by salesr_date desc, salesr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_salesrv where salesr_trtype='Sales Return' and salesr_flg=1 ");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
...
<tr class="tblsubtitle" height="20">
	...
	<td width="18%" align="center" valign="middle" class="tblheading">Party Name</td>
	<td width="7%" align="center" valign="middle" class="tblheading">DC No.</td>
	...
	<td width="5%" align="center" valign="middle" class="tblheading">Status</td>
    <td width="6%" align="center" valign="middle" class="tblheading">Outputs</td>
</tr>
<?php
//$srno=1;
if($tot_arr_home > 0)
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_month=mysqli_query($link,"select * from tbl_partymaser where  p_id='".$row_arr_home['salesr_party']."' order by business_name")or die(mysqli_error($link));
	...
	while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
	{
		...
		$slups=$slups+$row_tbl_sub['salesrs_nob']; 
		$slqty=$slqty+$row_tbl_sub['salesrs_qty'];
		...
		if($ups!="")
		{
			$ups=$ups."<br>".$row_tbl_sub['salesrs_ups'];
		}
		else
		{
...
<tr class="Light">
	...
	<td width="9%" align="center" valign="middle" class="smalltblheading"><?php echo "SR".$row_arr_home['salesr_code']."/".$row_arr_home['salesr_yearcode']."/".$row_arr_home['salesr_logid'];?></td>
	<td width="7%" align="center" valign="middle" class="smalltblheading"><?php echo $trdate?></td>
	<td width="18%" align="center" valign="middle" class="smalltblheading"><?php echo $partyname?></td>
	<td width="7%" align="center" valign="middle" class="smalltblheading"><?php echo $got;?></td>
	<td width="10%" align="center" valign="middle" class="smalltblheading"><?php echo $crop?></td>
	<td width="14%" align="center" valign="middle" class="smalltblheading"><?php echo $variety?></td>
	<td width="9%" align="center" valign="middle" class="smalltblheading"><?php echo $ups?></td>
	<td width="6%" align="center" valign="middle" class="smalltblheading"><?php echo $bags?></td>
	<td width="7%" align="center" valign="middle" class="smalltblheading"><?php echo $qty?></td>
	<td width="5%" align="center" valign="middle" class="smalltblheading"><?php if($row_arr_home['salesr_vflg']!=1) { ?><a href="add_sales_returnptcverfy.php?pid=<?php echo $arrival_id?>">Verify</a><?php }else{ ?>Verified<?php } ?></td>
    <td width="6%" align="center" valign="middle" class="smalltblheading"><?php if($row_arr_home['salesr_vflg']!=1) { ?><a href="Javascript:void(0)" onclick="openslocpopprint2('<?php echo $arrival_id;?>');">SRV</a><br />
      <a href="Javascript:void(0)" onclick="openslocpopprint3('<?php echo $arrival_id;?>');">Annexure</a><?php } else{?><?php if($row_arr_home['salesr_partytype']=="Dealer" || $row_arr_home['salesr_partytype']=="Bulk" || $row_arr_home['salesr_partytype']=="Export Buyer") {?><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $arrival_id;?>');">SRN</a><?php } else { ?><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $arrival_id;?>');">STIN</a><?php } ?><br /><a href="Javascript:void(0)" onclick="openslocpopprint1('<?php echo $arrival_id;?>');">SRLCN</a><?php  }?></td>
</tr>
<?php
}
else
