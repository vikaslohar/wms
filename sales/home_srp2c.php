<?php
	...
	$s_sub="delete from tbl_unpsp2c where unp_logid='".$logid."' and unp_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	
	if(isset($_POST['frm_action'])=='submit')
	{
		echo "<script>window.location='home_srp2c.php'</script>";
	}
?>
...
<title>Sales Return - Transaction - Unpacking - P2C - Home</title>
...
{
winHandle=window.open('srp2c_note.php?itmid='+tid,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" style="border-bottom:solid; border-bottom-color:#a8a09e" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - SR Unpacking - Pack to Condition (P2C)</td>
	    </tr></table></td>
...
<!--<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#a8a09e" style="border-collapse:collapse">
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_unpsp2c where plantcode='$plantcode' AND unp_tflg=1 order by unp_date desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_unpsp2c where plantcode='$plantcode' AND unp_tflg=1 order by unp_date desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_unpsp2c where unp_tflg=1 and unp_logid='$logid' LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
//$tot_arr_home=0;

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_unpsp2c where unp_tflg=1 and unp_logid='$logid'");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="850" bordercolor="#a8a09e" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			  ...
			  <td width="95" align="center" valign="middle" class="smalltblheading">Transaction ID</td>
			  <td width="65" align="center" valign="middle" class="smalltblheading">Date</td>
			  ...
			  <td width="95" align="center" valign="middle" class="smalltblheading">Lot No.</td>
...
{
	$trdate=$row_arr_home['unp_date'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;
	
	$lrole=$row_arr_home['unp_logid'];
	$arrival_id=$row_arr_home['unp_id'];
	
	
	$wareh=""; $binn=""; $subbinn=""; $sloc=""; $nob=0; $qty=0; $tnob=0; $tqty=0; 
	
	$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='$plantcode' AND whid='".$row_arr_home['unp_cwh1']."' order by perticulars") or die(mysqli_error($link));
	$row_whouse=mysqli_fetch_array($sql_whouse);
	$wareh=$row_whouse['perticulars'];
	
	$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='$plantcode' AND binid='".$row_arr_home['unp_cbin1']."' and whid='".$row_arr_home['unp_cwh1']."'") or die(mysqli_error($link));
	$row_binn=mysqli_fetch_array($sql_binn);
	$binn=$row_binn['binname'];
	
	$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='$plantcode' AND sid='".$row_arr_home['unp_csbin1']."' and binid='".$row_arr_home['unp_cbin1']."' and whid='".$row_arr_home['unp_cwh1']."'") or die(mysqli_error($link));
	$row_subbinn=mysqli_fetch_array($sql_subbinn);
	$subbinn=$row_subbinn['sname'];
	$nob=$row_arr_home['unp_cnob1']; 
	$qty=$row_arr_home['unp_cqty1']; 
	$tnob=$tnob+$nob; 
	$tqty=$tqty+$qty; 
	if($sloc!="")
	...
	if($row_arr_home['unp_cqty2'] > 0)
	{
	$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='$plantcode' AND whid='".$row_arr_home['unp_cwh2']."' order by perticulars") or die(mysqli_error($link));
	$row_whouse=mysqli_fetch_array($sql_whouse);
	$wareh=$row_whouse['perticulars'];
	
	$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='$plantcode' AND binid='".$row_arr_home['unp_cbin2']."' and whid='".$row_arr_home['unp_cwh2']."'") or die(mysqli_error($link));
	$row_binn=mysqli_fetch_array($sql_binn);
	$binn=$row_binn['binname'];
	
	$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='$plantcode' AND sid='".$row_arr_home['unp_csbin2']."' and binid='".$row_arr_home['unp_cbin2']."' and whid='".$row_arr_home['unp_cwh2']."'") or die(mysqli_error($link));
	$row_subbinn=mysqli_fetch_array($sql_subbinn);
	$subbinn=$row_subbinn['sname'];
	$nob=$row_arr_home['unp_cnob2']; 
	$qty=$row_arr_home['unp_cqty2']; 
	$tnob=$tnob+$nob; 
	$tqty=$tqty+$qty; 
	if($sloc!="")
	$sloc=$sloc."<br />".$wareh."/".$binn."/".$subbinn." | ".$nob." | ".$qty;
	...
	$quer3=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropid='".$row_arr_home['unp_crop']."' order by cropname Asc");
	$noticia = mysqli_fetch_array($quer3);
	$crop=$noticia['cropname'];
	
	$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety  where varietyid='".$row_arr_home['unp_variety']."'  order by popularname Asc"); 
	$noticia_item = mysqli_fetch_array($quer4);
	$variety=$noticia_item['popularname'];
	
	$lotno=$row_arr_home['unp_orlot'];
...
<tr class="Light">
	...
	<td align="center" valign="middle" class="smalltbltext"><?php echo "PC".$row_arr_home['unp_code']."/".$row_arr_home['unp_yearcode']."/".$row_arr_home['unp_logid'];?></td>
    ...
    <td align="center" valign="middle" class="smalltbltext"><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $arrival_id;?>');">SR-P2C Note</a></a></td>
</tr>
<?php
}
else
