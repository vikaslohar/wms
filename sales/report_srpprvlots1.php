...
<title>Sales Return-Report-Pack Seed Pending Re-Validation Report</title>
...
winHandle=window.open('report_srpprvlots2.php?&txtcrop='+itemid+'&txtvariety='+vv,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
...
<?php
	...
	$crp="ALL"; $ver="ALL";
	$qry="select Distinct salesrs_crop from tbl_salesrv_sub where plantcode='$plantcode' AND salesrs_rettype='P2P' and salesrs_rvflg=0 ";
	if($crop!="ALL")
	{	
...
?>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" style="border-bottom:solid; border-bottom-color:#a8a09e" >
	    <tr >
	      <td width="813" height="25">&nbsp;Sales Return - Pack Seed Pending Re-Validation Report</td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
<tr class="tblsubtitle" height="20">
			...
			<td width="45"  align="center" valign="middle" class="tblheading">NoP</td>
...
while($row_arr_home1=mysqli_fetch_array($sql_arr_home1))
{
	...
	if($variety!="ALL")
	{
		$sql_arr_home=mysqli_query($link,"select distinct salesrs_variety from tbl_salesrv_sub where plantcode='$plantcode' AND salesrs_crop='".$row_arr_home1['salesrs_crop']."' and salesrs_variety='".$variety."'  and salesrs_rvflg=0 and salesrs_rettype='P2P' ") or die(mysqli_error($link));
	}
	else
	{
		$sql_arr_home=mysqli_query($link,"select distinct salesrs_variety from tbl_salesrv_sub where plantcode='$plantcode' AND salesrs_crop='".$row_arr_home1['salesrs_crop']."'  and salesrs_rvflg=0 and salesrs_rettype='P2P'") or die(mysqli_error($link));
	}	

while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{

$sql_arr_home2=mysqli_query($link,"select *  from tbl_salesrv_sub where plantcode='$plantcode' AND salesrs_crop='".$row_arr_home1['salesrs_crop']."' and salesrs_variety='".$row_arr_home['salesrs_variety']."' and salesrs_rvflg=0 and salesrs_rettype='P2P' ") or die(mysqli_error($link));
...
{	
	...
	$sql_osrmain=mysqli_query($link,"select * from tbl_salesrv where plantcode='$plantcode' AND salesr_id='".$row_arr_home2['salesr_id']."'") or die(mysqli_error($link));
	...
	$slocs="";
	$sql_salesvr_subsub=mysqli_query($link,"select * from tbl_salesrvsub_sub where plantcode='$plantcode' AND salesrs_id ='".$row_arr_home2['salesrs_id']."'") or die(mysqli_error($link));
...
<tr >
<td height="49" align="center" valign="top"><a href="report_srpprvlots.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" />
