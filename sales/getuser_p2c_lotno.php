...
$sss="";
$qr="select * from tbl_salesrv_sub where plantcode='$plantcode' AND salesrs_crop='".$crop."' and salesrs_variety='".$variety."' and salesrs_rettype='P2P' and salesrs_rvflg=0 and salesrs_ups='$ups'";
...
//echo $row['lotno']."<br/>";
$ccnt=0;
$sql_is=mysqli_query($link,"select distinct subbinid, binid, whid from tbl_lot_ldg_pack where  lotno='".$row['lotno']."'  and balqty > 0 group by subbinid, binid order by lotdgp_id asc") or die(mysqli_error($link));
...
<td align="right"  valign="middle" class="tbltext"><input type="radio" name="loc" value="<?php echo $row['salesrs_newlot'];?>" onclick="clk(this.value,'<?php echo $row['salesrs_newlot'];?>','<?php echo $flg1?>');" />&nbsp;</td>
<td align="left"  valign="middle" class="tbltext" >&nbsp;<?php echo $row['salesrs_newlot'];?></td>
</tr>
<?php
}
else
