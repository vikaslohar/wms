...
$arrlots="";
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_proslipsub where plantcode='$plantcode' AND proslipmain_id='".$tid."'") or die(mysqli_error($link));
...
$qr="select distinct lotldg_lotno from tbl_lot_ldg where plantcode='$plantcode' AND lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$stage."' $sss  order by lotldg_id desc";
$sql_tbl_sub=mysqli_query($link,$qr) or die(mysqli_error($link));

//$aaaass="select distinct(lotldg_lotno)  from tbl_lot_ldg where plantcode='$plantcode' AND lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$stage."' and lotldg_balqty > 0 $sss order by lotldg_id desc";			
$sql_tbl_sub=mysqli_query($link,$qr)or die(mysqli_error($link));
...
<table align="center" border="1" width="400" cellspacing="0" cellpadding="0" bordercolor="#a8a09e" style="border-collapse:collapse" > 
...
$sql_issueg1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' AND lotldg_lotno='".$row['lotldg_lotno']."'") or die(mysqli_error($link));
