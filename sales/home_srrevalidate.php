<?php
	...
	$sql_tbl=mysqli_query($link,"select * from tbl_srrevalidate where plantcode='$plantcode' AND srrv_logid='".$logid."' and srrv_tflg=0") or die(mysqli_error($link));
	while($row_tbl=mysqli_fetch_array($sql_tbl))
	{
		$arrival_id=$row_tbl['srrv_id'];	
		$s_sub="delete from tbl_srrevalidate_sub where srrv_id='".$arrival_id."'";
		mysqli_query($link,$s_sub) or die(mysqli_error($link));
	}
	$s_sub="delete from tbl_srrevalidate where srrv_logid='".$logid."' and srrv_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	
	if(isset($_POST['frm_action'])=='submit')
	{
		echo "<script>window.location='home_srrevalidate.php'</script>";
	}
?>
...
<title>Sales Return - Transaction - Re-Validate - Home</title>
...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" >
	  ...
	  <td width="970" class="Mainheading" height="25">
	  <table width="950" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" style="border-bottom:solid; border-bottom-color:#a8a09e" >
	    <tr >
	      <td width="970" height="25">&nbsp;Transaction - Sales Return - Pack Seed Re-Validate</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_p2c.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
</table></td>-->
...
<!--<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#a8a09e" style="border-collapse:collapse">
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_salesrv_sub where plantcode='$plantcode' AND salesrs_rettype='P2P' and salesrs_qc!='Fail' and salesrs_rvflg=0 order by salesrs_dovfy asc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_salesrv_sub where plantcode='$plantcode' AND salesrs_rettype='P2P' and salesrs_qc!='Fail' and salesrs_rvflg=0 order by salesrs_dovfy asc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_salesrv_sub where plantcode='$plantcode' AND salesrs_rettype='P2P' and salesrs_qc!='Fail' and salesrs_rvflg=0 order by salesrs_dovfy asc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_salesrv_sub where plantcode='$plantcode' AND salesrs_rettype='P2P' and salesrs_qc!='Fail' and salesrs_rvflg=0");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */

?>
<table align="center" border="0" width="950" cellspacing="0" cellpadding="0" bordercolor="#a8a09e" style="border-collapse:collapse" > 
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Sales Return - Pack Seed Re-Validate - Pending List</td>
...
<br />
  ...
  $sql_arr_home1=mysqli_query($link,"select * from tbl_srrevalidate where plantcode='$plantcode' AND srrv_tflg=1 order by srrv_dorvp desc LIMIT $start1, $limit1") or die(mysqli_error($link));
...
$query1 = "select * from tbl_srrevalidate where plantcode='$plantcode' AND srrv_tflg=1 order by srrv_dorvp desc";
...
$sql_arr_home2=mysqli_query($link,"select * from tbl_srrevalidate where srrv_tflg=1 order by srrv_dorvp desc LIMIT $from2, $max_results2") or die(mysqli_error($link));
$tot_arr_home2=mysqli_num_rows($sql_arr_home2);

$total_results2 = mysql_result(mysqli_query($link,"SELECT COUNT(*) as Num FROM tbl_srrevalidate where srrv_tflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
...
<table align="center" border="0" width="950" cellspacing="0" cellpadding="0" bordercolor="#a8a09e" style="border-collapse:collapse" > 
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Sales Return - Pack Seed Re-Validation - Completed List</td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="950" bordercolor="#a8a09e" style="border-collapse:collapse">
...
<td width="19" align="center" valign="middle" class="smalltblheading">#</td>
	...
	<td width="60" align="center" valign="middle" class="smalltblheading">NoP</td>
	<td width="65" align="center" valign="middle" class="smalltblheading">Qty</td>
	<td width="55" align="center" valign="middle" class="smalltblheading">QC Status</td>
	<td width="80" align="center" valign="middle" class="smalltblheading">DoT</td>
	<td width="180" align="center" valign="middle" class="smalltblheading">SLOC</td>
</tr>
<?php
...
{
	$trdate=$row_tbl_sub2['srrv_dorvp'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;
		
	$lrole=$row_tbl_sub2['srrv_logid'];
	$arrival_id=$row_tbl_sub2['srrv_id'];
	
	$crop=""; $variety=""; $lotno=""; $bags=""; $qty=""; $stage=0; $got=""; $qc=""; $phsrn=""; $nlotno=""; $ups=""; $slups=0; $slqty=0;
	/*$sql_tbl_sub=mysqli_query($link,"select * from tbl_salesrv_sub where salesr_id='".$arrival_id."' and salesrs_rettype='P2P'") or die(mysqli_error($link));
	$subtbltot=mysqli_num_rows($sql_tbl_sub);
	while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))*/
	{
		$slups=$row_tbl_sub2['srrv_bnop']; 
		$slqty=$row_tbl_sub2['srrv_bqty'];
		...
		$quer3=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropid='".$row_tbl_sub2['srrv_crop']."' order by cropname Asc");
		$noticia = mysqli_fetch_array($quer3);
		
		$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety  where varietyid='".$row_tbl_sub2['srrv_variety']."'  order by popularname Asc"); 
		$noticia_item = mysqli_fetch_array($quer4);
		
		$crop=$noticia['cropname'];
		$variety=$noticia_item['popularname'];	
		$ups=$row_tbl_sub2['srrv_ups'];	
		
		$lotno=$row_tbl_sub2['srrv_lotno'];	

$sql_arrival=mysqli_query($link,"select * from tbl_srrevalidate_sub where plantcode='$plantcode' AND srrv_id='".$arrival_id."'") or die(mysqli_error($link));
$row_arrival=mysqli_fetch_array($sql_arrival);

$dot="";
if($row_tbl_sub2['srrv_dot']!="")
{
	$dt=explode("-",$row_tbl_sub2['srrv_dot']);
	$dot=$dt[2]."-".$dt[1]."-".$dt[0];
}
$dgt=explode("-",$row_tbl_sub2['srrv_dogt']);
$dogt=$dgt[2]."-".$dgt[1]."-".$dgt[0];
$dgots=explode(" ",$row_tbl_sub2['srrv_got']);
$got=$dgots[0]." ".$row_tbl_sub2['srrv_got1'];
$qc=$row_tbl_sub2['srrv_qc'];
//echo $row_arrival['salesrs_typ'];

$nop="";$qty="";  $wareh=""; $binn=""; $subbinn=""; $slocs=""; 

$sq1=mysqli_query($link,"select * from tbl_srrevalidate_sub where plantcode='$plantcode' AND srrv_id='".$arrival_id."'") or die(mysqli_error($link));
$tot1=mysqli_num_rows($sq1);
if($tot1 > 0)
{
	while($row1=mysqli_fetch_array($sq1))
	{
		$nop=$nop+$row1['srrvs_nop'];
		$qty=$qty+$row1['srrvs_qty'];
		
		$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='$plantcode' AND whid='".$row1['srrvs_whid']."' order by perticulars") or die(mysqli_error($link));
		$row_whouse=mysqli_fetch_array($sql_whouse);
		$wareh=$row_whouse['perticulars'];
			
		$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='$plantcode' AND binid='".$row1['srrvs_binid']."' and whid='".$row1['srrvs_whid']."'") or die(mysqli_error($link));
		$row_binn=mysqli_fetch_array($sql_binn);
		$binn=$row_binn['binname'];
			
		$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='$plantcode' AND sid='".$row1['srrvs_sbinid']."' and binid='".$row1['srrvs_binid']."' and whid='".$row1['srrvs_whid']."'") or die(mysqli_error($link));
		$row_subbinn=mysqli_fetch_array($sql_subbinn);
		$subbinn=$row_subbinn['sname'];
		...
		if($slocs!="")
			$slocs=$slocs."<br />".$wareh."/".$binn."/".$subbinn." | ".$bags." | ".$qty;
		else
			$slocs=$wareh."/".$binn."/".$subbinn." | ".$bags." | ".$qty;
	}
}
else
{
	$nop=$row_tbl_sub2['srrv_bnop'];
	$qty=$row_tbl_sub2['srrv_bqty'];
}
...
<?php
	...
	$tono2=$srno2-1;
	echo "<table width='950' align='center' class='tbltext'><tr><td align='left' >$no2 - $tono2 of $total_results2 Records</td><td align='right'>Select a Page    "; 
