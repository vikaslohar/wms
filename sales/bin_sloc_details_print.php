...
<title>Sales Return-Bin card</title>
...
<?php
$srno=1;
$sql_tbl=mysqli_query($link,"select * from tbl_salesrvsub_sub where plantcode='$plantcode' AND salesrss_wh='".$wid."' and salesrss_bin='".$bid."'  order by salesrss_subbin") or die(mysqli_error($link));  
...
$sql_subbinn=mysqli_query($link,"select sname from tblsrsubbin where plantcode='$plantcode' AND sid='".$row_tbl['salesrss_subbin']."' and binid='".$bid."' and whid='".$wid."'") or die(mysqli_error($link));
