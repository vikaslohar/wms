...
//echo $a;
$sql_arrival=mysqli_query($link,"select * from tblarrival_sub where plantcode='$plantcode' AND orlot='$a'") or die(mysqli_error($link));
$row_arrival=mysqli_fetch_array($sql_arrival);

$rtotalups=0; $rtotalqty=0; $diff="";
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_binid, lotldg_subbinid  from tbl_lot_ldg where plantcode='$plantcode' AND lotldg_sstage='Raw' and orlot='$a'") or die(mysqli_error($link));
$tot_lot=mysqli_num_rows($sql_issue);
 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' AND lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."'  and lotldg_sstage='Raw' and orlot='$a'") or die(mysqli_error($link));
...
?><table align="center" border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#a8a09e" style="border-collapse:collapse" > 
