...
ob_start();session_start();
	...
	$dh="Periodical_Non-Verified_Sales_Return_Report";
	$datahead = array("Periodical Non-Verified Sales Return Report");
	$filename=$dh.".xls";  
	...
	$datahead5 = array("#","SR Transaction ID","Crop","Variety","UPS","Lot Number","NoP/NoB","Qty"); 
...
$sql_srretsub2=mysqli_query($link,"select salesr_tslrno, salesr_id, salesr_yearcode, salesr_logid, salesrs_newlot from tbl_salesrv where salesr_id IN ($mid) ") or die(mysqli_error($link));
...
while($row_srretsub=mysqli_fetch_array($sql_srretsub))
{
	...
	if($totqty>0)	
	{
			$data1[$d]=array($d,$srnno,$cropn,$varietyn,$ups,$lotno,$lotnp,$totqty);
			$d++;$cnt++;
	}
}
