<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$sql_code="SELECT MAX(salesr_code) FROM tbl_salesrv where plantcode='$plantcode' AND salesr_yearcode='$yearid_id' and salesr_trtype='Sales Recall' ORDER BY salesr_code DESC";
		...
		$sql_main="update tbl_salesrv set salesr_flg=1, salesr_code=$code, salesr_vflg=2 where salesr_id = '$pid'";
	
		$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
		echo "<script>window.location='home_recall.php'</script>";	
	}
...
<title>Sales Return -Transaction - Sales Recall - Preview</title>
...
winHandle=window.open('slrecall_details_print.php?itmid='+itm,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#a8a09e" style="border-bottom:solid; border-bottom-color:#a8a09e" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Sales Recall - Preview</td>
	    </tr></table></td>
