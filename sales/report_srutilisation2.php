...
$cont=0; $veriet="";
	...
	$qry2="select Distinct lotldg_crop, lotldg_variety from tbl_lot_ldg_pack where plantcode='$plantcode' AND lotldg_trdate<='".$edt."' and lotldg_trdate>='".$sdt."' and trtype='Sales Return' and balqty > 0";
...
/*if($variety!="ALL")
{
$sql_rr2=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where plantcode='$plantcode' AND lotldg_crop='".$verval."' and lotldg_variety='".$variety."' and balqty > 0 and trtype='Sales Return'") or die(mysqli_error($link));
}
else
{
$sql_rr2=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where plantcode='$plantcode' AND lotldg_crop='".$verval."' and balqty > 0 and trtype='Sales Return'") or die(mysqli_error($link));
}
...
while($row_rr2=mysqli_fetch_array($sql_rr2))
{
	...
	$sql_arr_home=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where plantcode='$plantcode' AND lotldg_crop='".$verval."' and lotldg_variety='".$row_rr2['lotldg_variety']."' and balqty > 0 and trtype='Sales Return' group by lotno order by lotldg_crop asc, lotldg_variety asc") or die(mysqli_error($link));
	while($row_arr_home=mysqli_fetch_array($sql_arr_home))
	{  
