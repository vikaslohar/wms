...
$sql_arr_home=mysqli_query($link,"select * from tbl_salesr where plantcode='$plantcode' AND salesr_date>='$tdate' and salesr_date<='$pdate' and salesr_trtype='Opening Stock Condition' and salesr_flg=1 and salesr_logid='$logid' order by salesr_date desc, salesr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_salesr where plantcode='$plantcode' AND salesr_date>='$tdate' and salesr_date<='$pdate' and salesr_trtype='Opening Stock Condition' and salesr_flg=1 and salesr_logid='$logid'");
