...
<title>Plant - Report - Financial Year wise Monthly Sales Return Report</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" style="border-bottom:solid; border-bottom-color:#2e81c1" >
	    <tr >
	      <td width="940" height="25">&nbsp; Financial Year wise Monthly Sales Return Report <?php if($plantcode=="D"){echo "Deorjhal Plant";} else if($plantcode=="B"){echo "Boriya Plant";} else {echo "";}?></td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
elseif ($selectedMonth=="ALL" && count($monthList)>0)
{
	...
	$sql1o="select distinct salesrs_crop from tbl_salesrv_sub where salesr_id IN($arrival_id) and salesrs_dovfy <= '$endDate' and salesrs_dovfy >= '$startDate' and plantcode='$plantcode'  ";
	if($txtcrop!="ALL")
	{
	...
	{
		$sql2o="select distinct salesrs_variety from tbl_salesrv_sub where salesr_id IN($arrival_id) and salesrs_dovfy <= '$endDate' and salesrs_dovfy >= '$startDate' and plantcode='$plantcode'  and salesrs_crop='".$row_tbl_sub1['salesrs_crop']."'";
		if($txtvariety!="ALL")
		{
	...
	foreach ($monthList as $item){
		...
		if($arrival_id!="")
		{
			...
			$sql1.=" and salesrs_variety='$vertyname' order by salesrs_crop asc";
			//echo $sql;
			$sql_tbl_sub1=mysqli_query($link,$sql1) or die(mysqli_error($link));
			$subtbltot1=mysqli_num_rows($sql_tbl_sub1);
			$row_tbl_sub1=mysqli_fetch_array($sql_tbl_sub1);
				
			$sq_var2=mysqli_query($link,"select vt, cropid from tblvariety where popularname='".$vertyname."' order by popularname Asc") or die(mysqli_error($link));
			$row_var2=mysqli_fetch_array($sq_var2);
			$crop=$row_var2['cropid'];
			$variety=$vertyname;	
			$vtype=$row_var2['vt'];	
				
			$sql="select SUM(salesrs_qtydc) from tbl_salesrv_sub where salesr_id IN($arrival_id) and salesrs_dovfy<='$endDate' and salesrs_dovfy>='$startDate' and plantcode='$plantcode' and salesrs_crop='".$row_tbl_sub1['salesrs_crop']."' and salesrs_variety='".$vertyname."' order by salesrs_crop, salesrs_variety asc";
			//echo $sql;
			$sql_tbl_sub=mysqli_query($link,$sql) or die(mysqli_error($link));
			$subtbltot=mysqli_num_rows($sql_tbl_sub);
			while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
