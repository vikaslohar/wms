<?php
	...
	if($itemid!="ALL")
	{
		$sqlcrop=mysqli_query($link,"select * from tblcrop where cropid='".$itemid."'") or die(mysqli_error($link));
		$rowcrop=mysqli_fetch_array($sqlcrop);
		$cropnam=$rowcrop['cropname'];
	}
	if($loc!="ALL")
	{	
		$sqlvar=mysqli_query($link,"select * from tblvariety where varietyid='".$loc."' ") or die(mysqli_error($link));
...
winHandle=window.open('preport_plant2.php?sdate='+sdate+'&txtcrop='+loc+'&txtvariety='+itemid+'&edate='+edate+'&txtvisualck='+typ+'&txtplant='+txtplant,'WelCome','top=20,left=80,width=1000,height=900,scrollbars=yes');
if(winHandle==null){
...
<?php	
	...
	$crop="ALL";	
	$vit="ALL";
	$sqltbls="select distinct lotcrop, sstage, lotvariety from tblarrival_sub where arrival_id!=0  $platc ";
...
<table align="center" border="0" cellspacing="0" cellpadding="0" width="650" style="border-collapse:collapse">
   <!--	<tr height="25" >
	...
	<td align="center" class="subheading" style="color:#303918; ">Variety: <?php echo $vit;?></td>
	<td align="right" class="subheading" style="color:#303918; ">Plant: <?php if($txtplant=="ALL"){echo "Boriya &amp; Deorjhal";} else { if($txtplant!="Bandamailaram"){echo $txtplant;}}?></td>
  	</tr>
	</table>
...
//$sqltblsub0="select * from tblarrival_sub where lotcrop='".$crop."' and lotvariety='".$row_tbl_s[1]."' and sstage='".$row_tbl_s[0]."'";
//if($itemid!="ALL")
//{
$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where lotcrop='".$row_tbl_s[0]."' and lotvariety='".$row_tbl_s[2]."' and sstage='".$row_tbl_s[1]."'  $platc  ") or die(mysqli_error($link));
/*}
if($loc=="ALL")
{
...
echo $row_tbl_sub['got']."  -  ".$row_tbl_sub['orlot']."  -  ".$row_tbl_sub['lotvariety']."<br/>";*/
$sql_armain=mysqli_query($link,"select * from tblarrival where arrival_date <= '$edate' and arrival_date >= '$sdate' and arrival_type='".$typ."' and arrtrflag=1 and arrival_id='".$row_tbl_sub['arrival_id']."' order by arrival_date desc") or die(mysqli_error($link));
...
$sql_armain12=mysqli_query($link,"select * from tblarrival where arrival_date <= '$edate' and arrival_date >= '$sdate' and arrival_type='".$typ."' and arrtrflag=1 $aaaa order by arrival_date desc") or die(mysqli_error($link));
...
if($txtplant=="ALL" || $txtplant=="Bandamailaram")
{
	...
	if (mysqli_connect_errno($connnew)) {
	  echo "Failed to connect to MySQL: " . mysqli_connect_error($connnew);
	  exit();
	}
	if ($connnew) {	
	...
	$crop="ALL";	
	$vit="ALL";
	$sqltbls="select distinct lotcrop, sstage, lotvariety from tblarrival_sub where arrival_id!=0   ";
	
	if($itemid!="ALL")
	{
	$sql_class=mysqli_query($connnew,"select * from tblcrop where cropid='".$itemid."'") or die(mysqli_error($connnew));
	$row_class=mysqli_fetch_array($sql_class);
	$crop=$row_class['cropname'];	 
	...
	if($loc!="ALL")
	{
	$sql_vit=mysqli_query($connnew,"select * from tblvariety where varietyid='".$loc."'  and vertype='PV'") or die(mysqli_error($connnew));
	$row_vit=mysqli_fetch_array($sql_vit);
	$vit=$row_vit['popularname'];
	$sqltbls.=" and lotvariety='".$vit."' ";
	...
	$sqltbls.=" group by lotcrop, lotvariety, sstage";
	//echo $sqltbls;
	$sql_tbl_s=mysqli_query($connnew,$sqltbls) or die(mysqli_error($connnew));
	$tot_arsub=mysqli_num_rows($sql_tbl_s);
	//echo $tot_arsub;
if($tot_arsub > 0)	
{
...
<table align="center" border="0" cellspacing="0" cellpadding="0" width="650" style="border-collapse:collapse">
   <!--	<tr height="25" >
    ...
    <td align="left" class="subheading" style="color:#303918; ">Crop: <?php echo $crop;?></td>
	<td align="center" class="subheading" style="color:#303918; ">Variety: <?php echo $vit;?></td>
	<td align="right" class="subheading" style="color:#303918; ">Plant: <?php echo "Bandamailaram";?></td>
  	</tr>
	</table>
...
//if($itemid!="ALL")
//{
$sql_tbl_sub=mysqli_query($connnew,"select * from tblarrival_sub where lotcrop='".$row_tbl_s[0]."' and lotvariety='".$row_tbl_s[2]."' and sstage='".$row_tbl_s[1]."'   ") or die(mysqli_error($connnew));
/*}
if($loc=="ALL")
{
$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where lotcrop='".$crop."' and lotvariety='".$row_tbl_s[1]."' and sstage='".$row_tbl_s[0]."'") or die(mysqli_error($link));
...
/*if($row_tbl_sub['got']=="GOT-R")
echo $row_tbl_sub['got']."  -  ".$row_tbl_sub['orlot']."  -  ".$row_tbl_sub['lotvariety']."<br/>";*/
$sql_armain=mysqli_query($connnew,"select * from tblarrival where arrival_date <= '$edate' and arrival_date >= '$sdate' and arrival_type='".$typ."' and arrtrflag=1 and arrival_id='".$row_tbl_sub['arrival_id']."' order by arrival_date desc") or die(mysqli_error($connnew));
$tot_armain=mysqli_num_rows($sql_armain);
if($tot_armain > 0)
{
	$arr_id=$row_tbl_sub['arrival_id'];
...
$aaaa="";
//echo $sql="select * from tblarrival where arrival_date <= '$edate' and arrival_date >= '$sdate' and arrival_type='".$typ."' and arrtrflag=1 $aaaa order by arrival_date desc";
$sql_armain12=mysqli_query($connnew,"select * from tblarrival where arrival_date <= '$edate' and arrival_date >= '$sdate' and arrival_type='".$typ."' and arrtrflag=1 $aaaa order by arrival_date desc") or die(mysqli_error($connnew));
$tot_armain12=mysqli_num_rows($sql_armain12);
if($tot_armain12 > 0 && $cnt > 0)
{
if($srno%2 != 0)
