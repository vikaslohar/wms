...
{ 
	...
	if($verval<>"")
	{
		...
		if($plantcode=='H')
		{
				...
				$sql_crph=mysqli_query($connnew,"select cropid, cropname from tblcrop where cropname='".$crop_name."'") or die(mysqli_error($connnew));
				$row_crph=mysqli_fetch_array($sql_crph);
				$crpid=$row_crph['cropid'];
				$sql_varh=mysqli_query($connnew,"select varietyid, popularname from tblvariety where popularname='".$verty."' ") or die(mysqli_error($connnew));
				if($vtoth=mysqli_num_rows($sql_varh)>0)
				{
					$row_varh=mysqli_fetch_array($sql_varh);
					$vertyid=$row_varh['varietyid'];
				}
				else
				{
					$vertyid=$verty;
				...
				$qrylt="select Distinct orlot from tbl_lot_ldg where lotldg_crop='".$crpid."' and lotldg_trdate<='$stdate'  and lotldg_qc!='Fail'  ";
				$qrylt2="select Distinct orlot from tbl_lot_ldg_pack where lotldg_crop='".$crpid."' and lotldg_trdate<='$stdate' and lotldg_qc!='Fail'   ";
				$qrylt3="select Distinct salesrs_orlot from tbl_salesrv_sub where salesrs_crop='".$crpid."'  and salesrs_rettype='P2P' and salesrs_qc!='Fail'  ";
				$qrylt.=" and lotldg_variety='$vertyid' ";
				$qrylt2.=" and lotldg_variety='$vertyid' ";
				$qrylt3.=" and salesrs_variety='$vertyid' ";
		...
		if($plantcode=='H')
		{
			...
			// Raw Seed Records
				...
				$sql_is=mysqli_query($connnew,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crpid."' and orlot='$lt2' and lotldg_variety='".$vertyid."'  and lotldg_trdate<='$stdate'  group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($connnew));
				while($row_is=mysqli_fetch_array($sql_is))
				{ 
					$sql_is1=mysqli_query($connnew,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crpid."' and lotldg_variety='".$vertyid."' and orlot='$lt2' and lotldg_trdate<='$stdate'   order by lotldg_id desc LIMIT 1 ") or die(mysqli_error($connnew));
					$row_is1=mysqli_fetch_array($sql_is1); 
			...
			// Pack Seed Records
				$sql_is=mysqli_query($connnew,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where lotldg_crop='".$crpid."' and orlot='$lt2' and lotldg_variety='".$vertyid."' and lotldg_trdate<='$stdate'  group by subbinid order by lotdgp_id asc") or die(mysqli_error($connnew));
				while($row_is=mysqli_fetch_array($sql_is))
				{ 
					$sql_is1=mysqli_query($connnew,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_is['subbinid']."' and binid='".$row_is['binid']."' and lotldg_crop='".$crpid."' and lotldg_variety='".$vertyid."' and orlot='$lt2' and lotldg_trdate<='$stdate'  order by lotdgp_id desc ") or die(mysqli_error($connnew));
					$row_is1=mysqli_fetch_array($sql_is1); 
			...
			// Sales Return Seed Records
				$sql_arhome=mysqli_query($connnew,"select salesr_id,salesrs_id,salesrs_newlot from tbl_salesrv_sub where salesrs_crop='".$crpid."' and salesrs_variety='".$vertyid."' and salesrs_rettype='P2P' and salesrs_dovfy<='$stdate' and salesrs_orlot='$lt2'  order by 'salesrs_id' asc") or die(mysqli_error($connnew));
				while($row_arhome=mysqli_fetch_array($sql_arhome))
				{  
