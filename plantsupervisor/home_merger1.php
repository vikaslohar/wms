...
$sql_tbl=mysqli_query($link,"select * from tbl_mergermain where mergerm_logid='".$logid."' and mergerm_tflg=0 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['mergerm_id'];	
	
	$s_sub="delete from tbl_mergersub where mergerm_id='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_mergersubsub where mergerm_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
}

	$s_sub="delete from tbl_mergermain where mergerm_logid='".$logid."' and mergerm_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
{
winHandle=window.open('merger_note1.php?itmid='+tid,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" >
	 ...
	 <td width="801" class="Mainheading" height="25"><table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" style="border-bottom:solid; border-bottom-color:#2e81c1" >
        <tr >
          <td width="810" height="25">&nbsp;Transaction - Lot Merger</td>
        </tr>
	    </table></td>
...
<tr height="15" class="tbltitle" >
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_lot_merger1.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onClick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_mergermain where mergerm_date<='$pdate' and mergerm_date>='$tdate' and mergerm_tflg=1 and plantcode='$plantcode' order by mergerm_date desc,mergerm_code desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_mergermain where mergerm_date<='$pdate' and mergerm_date>='$tdate' and mergerm_tflg=1 and plantcode='$plantcode' order by mergerm_date desc,mergerm_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_mergermain where mergerm_date<='$pdate' and mergerm_date>='$tdate' and mergerm_tflg=1 order by mergerm_date desc,mergerm_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_mergermain where mergerm_date<='$pdate' and mergerm_date>='$tdate' and mergerm_tflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
if($tot_arr_home > 0)
{
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$trdate=$row_arr_home['mergerm_date'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;	
	
	$trid=$row_arr_home['mergerm_id'];
	$drole=$row_arr_home['mergerm_logid'];
	$yrid=$row_arr_home['mergerm_yearid'];
	
	$dq=explode(".",$row_arr_home['mergerm_qty']);
	if($dq[1]==000){$mqty=$dq[0];}else{$mqty=$row_arr_home['mergerm_qty'];}

$classqry=mysqli_query($link,"select cropid, cropname from tblcrop where cropid='".$row_arr_home['mergerm_crop']."' order by cropname") or die(mysqli_error($link));
$noticia_class=mysqli_fetch_array($classqry);

$itemqry=mysqli_query($link,"select varietyid, popularname from tblvariety where varietyid='".$row_arr_home['mergerm_variety']."'") or die(mysqli_error($link));
...
$nnob=$row_eindent_sub2['mergerss_nob'];

$dq=explode(".",$row_eindent_sub2['mergerss_qty']);
if($dq[1]==000){$nqty=$dq[0];}else{$nqty=$row_eindent_sub2['mergerss_qty'];}
...
{
/*if($extlotno!="")
$extlotno=$extlotno."<br/>".$row_eindent_sub['mergers_lotno'];
else
$extlotno=$row_eindent_sub['mergers_lotno'];
*/
if($extnob!="")
$extnob=$extnob."<br/>".$row_eindent_sub['mergers_nob']; 
else
$extnob=$row_eindent_sub['mergers_nob']; 

$dq=explode(".",$row_eindent_sub['mergers_qty']);
if($dq[1]==000){$extqty1=$dq[0];}else{$extqty1=$row_eindent_sub['mergers_qty'];}

if($extqty!="")
$extqty=$extqty."<br/>".$extqty1;
else
$extqty=$extqty1;

}
$extlotno=$row_arr_home['mergerm_oldlot'];	
if($srno%2!=0)
{
?>			  
<tr class="Light">
         <td width="4%" align="center" valign="middle" class="smalltbltext"><?php echo $srno;?></td>
         <td width="9%" align="center" valign="middle" class="smalltbltext"><a href="Lot_merger_view.php?p_id=<?php echo $trid;?>"><?php echo "LM".$row_arr_home['mergerm_code']."/".$row_arr_home['mergerm_yearid']."/".$drole;?></a></td>
         ...
         <td width="11%" align="center" valign="middle" class="smalltbltext"><?php echo $row_arr_home['mergerm_newlot'];?></td>
         <td width="5%" align="center" valign="middle" class="smalltbltext"><?php echo $row_arr_home['mergerm_nob']?></td>
         ...
         <td width="5%" align="center" valign="middle" class="smalltbltext"><?php if($yrid=="P"){?><a href="Javascript:void(0)" onClick="openslocpopprint('<?php echo $trid;?>');">LMN</a><?php } else { ?><a href="Javascript:void(0)" onClick="openslocpopprint24('<?php echo $trid;?>');">LMN</a><?php } ?></td>
</tr>
<?php
}
else
