...
$arrlots="";
$sql_tbl_sub=mysqli_query($link,"select * from tbl_ivtsub where ivt_id='".$tid."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
//echo $arrlots;
$qr="select distinct(lotldg_lotno) from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='Condition' and lotldg_qc!='Fail' and lotldg_got!='Fail' and plantcode='$plantcode' order by lotldg_lotno asc ";
...
$sql_issue24=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='Condition' and lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_qc!='Fail' and lotldg_got!='Fail' and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_issue24=mysqli_fetch_array($sql_issue24))
{ 
	$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue24['lotldg_subbinid']."' and lotldg_binid='".$row_issue24['lotldg_binid']."' and lotldg_whid='".$row_issue24['lotldg_whid']."' and lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_sstage='Condition' and plantcode='$plantcode' order by lotldg_id desc ") or die(mysqli_error($link));
	$row_is1=mysqli_fetch_array($sql_is1);
	
	$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_sstage='Condition' and lotldg_balqty > 0 and lotldg_qc!='Fail' and lotldg_got!='Fail' and plantcode='$plantcode' order by lotldg_lotno asc") or die(mysqli_error($link)); 
