<?php
	...
	if($txtupsdc!="ALL")
	{
		$sqlrr=mysqli_query($link,"select distinct packtype from tbl_lot_ldg_pack where packtype='".$txtupsdc."' and lotldg_crop IN ($cp) AND plantcode='$plantcode' order by packtype asc") or die(mysqli_error($link));
	}
	else
	{
		$sqlrr=mysqli_query($link,"select distinct packtype from tbl_lot_ldg_pack where lotldg_crop IN ($cp) AND plantcode='$plantcode' order by packtype Asc") or die(mysqli_error($link));
	}
	...
	$qry="select Distinct lotldg_crop from tbl_lot_ldg_pack where lotldg_qc!='Fail' and lotldg_got!='Fail' AND plantcode='$plantcode' ";
...
if($txtupsdc!="ALL")
{
	$sql_rr=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where lotldg_crop='".$row_arr_home1['lotldg_crop']."' and packtype='".$txtupsdc."' and lotldg_qc!='Fail' and lotldg_got!='Fail' AND plantcode='$plantcode' order by lotdgp_id desc") or die(mysqli_error($link));
}
else
{
	$sql_rr=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_qc!='Fail' and lotldg_got!='Fail' AND plantcode='$plantcode' order by lotdgp_id desc") or die(mysqli_error($link));
}
...
while($row_rr=mysqli_fetch_array($sql_rr))
{
	...
	foreach($upp2 as $ups2)
	{
		...
		{	
			...
			//echo $ups2;
			$totqty=0; 
			$sql_rr2=mysqli_query($link,"select distinct packtype from tbl_lot_ldg_pack where lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and packtype='".$ups2."' and lotldg_qc!='Fail' and lotldg_got!='Fail'   AND plantcode='$plantcode' order by packtype asc") or die(mysqli_error($link));
			$tot_rr2=mysqli_num_rows($sql_rr2);
			//$row_rr2=mysqli_fetch_array($sql_rr2);
			while($row_rr2=mysqli_fetch_array($sql_rr2))
			{
				$totqty=0; $totnob=0; 
				$sql_arr_home=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and packtype='".$row_rr2['packtype']."' and lotldg_qc!='Fail' and lotldg_got!='Fail' AND plantcode='$plantcode' group by lotno order by lotdgp_id asc") or die(mysqli_error($link));
				while($row_arr_home=mysqli_fetch_array($sql_arr_home))
				{
					$sql_tbl_sub1=mysqli_query($link,"select distinct subbinid from tbl_lot_ldg_pack where lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and packtype='".$row_rr2['packtype']."' and lotno='".$row_arr_home['lotno']."' and lotldg_qc!='Fail' and lotldg_got!='Fail' AND plantcode='$plantcode' order by subbinid") or die(mysqli_error($link));
					$t2=mysqli_num_rows($sql_tbl_sub1);
					while($row_tbl=mysqli_fetch_array($sql_tbl_sub1))
					{
				
						$sql_tbl1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_tbl['subbinid']."' and lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and packtype='".$row_rr2['packtype']."' and lotno='".$row_arr_home['lotno']."' and lotldg_qc!='Fail' and lotldg_got!='Fail' AND plantcode='$plantcode' order by lotdgp_id desc") or die(mysqli_error($link));  
						$row_tbl1=mysqli_fetch_array($sql_tbl1);
	
					
						$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_tbl1[0]."' and balqty > 0 and lotldg_qc!='Fail' and lotldg_got!='Fail' AND plantcode='$plantcode'") or die(mysqli_error($link)); 
