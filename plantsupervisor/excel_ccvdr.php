...
if($txtupsdc=="ALL")
{
	...
	while($row_is11=mysqli_fetch_array($sql_is11))
	{ 
		$sqdm2="select * from tbl_dtdf where dtdf_id='".$row_is11['dtdf_id']."' and dtdf_date='$ndts' and dtdf_tflg=1 AND plantcode='$plantcode' AND plantcode='$plantcode' order by dtdf_date asc, dtdf_id asc";
		$sql_istbl2=mysqli_query($link,$sqdm2) or die(mysqli_error($link)); 
		$t2=mysqli_num_rows($sql_istbl2);
		if($t2 > 0)
		{
