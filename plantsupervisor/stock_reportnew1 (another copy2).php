<?php
	...
	//$connnew = mysqli_connect("localhost","root","ispl123") or die("Error:".mysqli_error($connnew));
	...
	//$connnew = new mysqli("localhost","root","ispl123","seedtracwms_hyd");
	
	/*$connnew = mysqli_connect("172.16.16.52","root","kvy782dragon201");
	if (!$connnew) {
	 	echo "Not Connected";
	}
	else {
		mysqli_select_db($connnew,"seedtracwms_hyd") or die("Error:".mysqli_error($connnew));
	}*/
...
{ 
	...
	if($verval<>"")
	{
		...
		{
			$qrylt="select Distinct orlot from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_trdate<='$stdate' and lotldg_qc!='Fail'  and lotldg_got!='Fail'  ";
			$qrylt2="select Distinct orlot from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and lotldg_trdate<='$stdate' and lotldg_qc!='Fail'  and lotldg_got!='Fail'  ";
			$qrylt3="select Distinct salesrs_orlot from tbl_salesrv_sub where salesrs_crop='".$crval."'  and salesrs_rettype='P2P' and salesrs_qc!='Fail'  and salesrs_got1!='Fail'   ";
		...
		// Raw Seed Records
			...
			{ 
				...
				$sql_istbl=mysqli_query($link,"select lotldg_balqty, lotldg_balbags, lotldg_sstage, plantcode from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' and lotldg_balqty>0  order by lotldg_id asc") or die(mysqli_error($link)); 
		...
		// Pack Seed Records
			...
			{ 
				...
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select balqty, balnomp, packtype, wtinmp, plantcode from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' and balqty>0  order by lotdgp_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_istbl))
		...
		// Sales Return Seed Records
			...
			{  
				$sql_istbl=mysqli_query($link,"select salesrss_qty, salesrss_nob, plantcode from tbl_salesrvsub_sub where salesrs_id='".$row_arhome['salesrs_id']."' and salesrss_qty>0   ") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_istbl))
	...
	$ltarr2="";
		...
		if($plantcode=='H')
		{
				...
				$sql_crp2=mysqli_query($connnew,"select * from tblcrop where cropname='".$crpn."'") or die(mysqli_error($connnew));
				$row_crp2=mysqli_fetch_array($sql_crp2);
				$crval2=$row_crp2['cropid'];
				
				$sql_varh=mysqli_query($connnew,"select * from tblvariety where popularname='".$verty."' ") or die(mysqli_error($connnew));
				$row_varh=mysqli_fetch_array($sql_varh);
				$verval2=$row_varh['varietyid'];
				$cvcod=$crop_name."-Coded";
				if($verty==$cvcod)
				{$verval2=$cvcod;}
				
				$qrylt="select Distinct orlot from tbl_lot_ldg where lotldg_crop='".$crval2."' and lotldg_trdate<='$stdate'  and lotldg_qc!='Fail'  and lotldg_got!='Fail' ";
				$qrylt2="select Distinct orlot from tbl_lot_ldg_pack where lotldg_crop='".$crval2."' and lotldg_trdate<='$stdate' and lotldg_qc!='Fail'  and lotldg_got!='Fail'  ";
				$qrylt3="select Distinct salesrs_orlot from tbl_salesrv_sub where salesrs_crop='".$crval2."'  and salesrs_rettype='P2P' and salesrs_qc!='Fail' and salesrs_got1!='Fail'   ";
				$qrylt.=" and lotldg_variety='$verval2' ";
				$qrylt2.=" and lotldg_variety='$verval2' ";
				$qrylt3.=" and salesrs_variety='$verval2' ";
				...
				{
					if($ltarr2!="")
					$ltarr2=$ltarr2.",".$row_arr_homelt12['orlot'];
					else
					$ltarr2=$row_arr_homelt12['orlot'];
				}
				
				while($row_arr_homelt22=mysqli_fetch_array($sql_arr_homelt22))
				{
					if($ltarr2!="")
					$ltarr2=$ltarr2.",".$row_arr_homelt22['orlot'];
					else
					$ltarr2=$row_arr_homelt22['orlot'];
				}
				
				while($row_arr_homelt23=mysqli_fetch_array($sql_arr_homelt23))
				{
					if($ltarr2!="")
					$ltarr2=$ltarr2.",".$row_arr_homelt23['salesrs_orlot'];
					else
					$ltarr2=$row_arr_homelt23['salesrs_orlot'];
					//echo $row_arr_homelt23['salesrs_orlot']."<br />";
				}
			}
		}
		
		//if($verty=="4004"){echo $ltarr2."<br />"; exit;}
		$ltn2=explode(",",$ltarr2);
		$ltn2=array_unique($ltn2);
		sort($ltn2);
		foreach($ltn2 as $lt2)
		...
		if($plantcode=='H')
		{
			...
			// Raw Seed Records
				...
				$sql_is=mysqli_query($connnew,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crval2."' and orlot='$lt2' and lotldg_variety='".$verval2."'  and lotldg_trdate<='$stdate'  group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($connnew));
				while($row_is=mysqli_fetch_array($sql_is))
				{ 
					$sql_is1=mysqli_query($connnew,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crval2."' and lotldg_variety='".$verval2."' and orlot='$lt2' and lotldg_trdate<='$stdate'   order by lotldg_id desc LIMIT 1 ") or die(mysqli_error($connnew));
					$row_is1=mysqli_fetch_array($sql_is1); 
					$sql_istbl=mysqli_query($connnew,"select lotldg_balqty, lotldg_balbags, lotldg_sstage, orlot from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' and lotldg_balqty>0  order by lotldg_id asc") or die(mysqli_error($connnew)); 
					$t=mysqli_num_rows($sql_istbl);
					if($t > 0)
					{
						while($row_issuetbl=mysqli_fetch_array($sql_istbl))
						{ 
							//echo $row_issuetbl['orlot']."  =  ".$row_issuetbl['lotldg_balqty']."<br />"; 
							$qt=$row_issuetbl['lotldg_balqty']; 
							if($qt<0)$qt=0;
							if($row_issuetbl['lotldg_sstage']=="Raw")
							{
			...
			// Pack Seed Records
				$sql_is=mysqli_query($connnew,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where lotldg_crop='".$crval2."' and orlot='$lt2' and lotldg_variety='".$verval2."' and lotldg_trdate<='$stdate'  group by subbinid order by lotdgp_id asc") or die(mysqli_error($connnew));
				while($row_is=mysqli_fetch_array($sql_is))
				{ 
					$sql_is1=mysqli_query($connnew,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_is['subbinid']."' and binid='".$row_is['binid']."' and lotldg_crop='".$crval2."' and lotldg_variety='".$verval2."' and orlot='$lt2' and lotldg_trdate<='$stdate'  order by lotdgp_id desc ") or die(mysqli_error($connnew));
					$row_is1=mysqli_fetch_array($sql_is1); 
					$sql_istbl=mysqli_query($connnew,"select balqty, balnomp, packtype, wtinmp from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' and balqty>0  order by lotdgp_id asc") or die(mysqli_error($connnew)); 
					$t=mysqli_num_rows($sql_istbl);
					if($t > 0)
					{
						while($row_issuetbl=mysqli_fetch_array($sql_istbl))
			...
			// Sales Return Seed Records
				$sql_arhome=mysqli_query($connnew,"select salesr_id,salesrs_id,salesrs_newlot from tbl_salesrv_sub where salesrs_crop='".$crval2."' and salesrs_variety='".$verval2."' and salesrs_rettype='P2P' and salesrs_dovfy<='$stdate' and salesrs_orlot='$lt2'  order by 'salesrs_id' asc") or die(mysqli_error($connnew));
				while($row_arhome=mysqli_fetch_array($sql_arhome))
				{  
					$sql_istbl=mysqli_query($connnew,"select salesrss_qty, salesrss_nob from tbl_salesrvsub_sub where salesrs_id='".$row_arhome['salesrs_id']."'  and salesrss_qty>0  ") or die(mysqli_error($connnew)); 
					$t=mysqli_num_rows($sql_istbl);
					if($t > 0)
					{
						while($row_issuetbl=mysqli_fetch_array($sql_istbl))
