...
<title>Plant- Transaction -Home_tagging</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" >
	  ...
	  <td width="820" class="Mainheading" height="25"><table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" style="border-bottom:solid; border-bottom-color:#2e81c1" >
        <tr >
          <td width="820" height="25">&nbsp;Transaction - Stock Transfer Lots Import</td>
        </tr>
	    </table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_stlotimp.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
	  </tr>
	  </table></td></tr>
...
<?php
	$sql_arr_home=mysqli_query($link,"select max(stlotimp_date),stlotimp_id from tbl_stlotimp where plantcode='$plantcode'") or die(mysqli_error($link));
	$tot_arr_home=mysqli_fetch_array($sql_arr_home);
	//echo $tot_arr_home[1];
	
	$sql_tbl=mysqli_query($link,"select * from tbl_stlotimp where stlotimp_date='".$tot_arr_home[0]."' and plantcode='$plantcode' order by stlotimp_id asc") or die(mysqli_error($link));
...
{
						if($lotno!="")
						$lotno=$lotno.", ".$row_arr_home['stlotimp_lotno'];
						else
						$lotno=$row_arr_home['stlotimp_lotno'];
}
?>			  
<tr class="Light">
         <td width="2%" align="center" valign="middle" class="tblheading"><?php echo $srno;?></td>
         <td width="9%" align="center" valign="middle" class="tblheading"><a href="add_stlotimp_list.php?sdate=<?php echo $trdate;?>&edate=<?php echo $trdate;?>"><?php echo $trdate;?></a></td>
