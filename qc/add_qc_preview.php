<?php
	...
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		...
		if($qc=="UT")
		{
			$sql_sub_sub="insert into tbl_qctest(lotno, oldlot, crop, variety, qc, trstage, srdate, sampleno, state, pp, moist, gemp, yearid,logid) values('$lotn', '$lotno', '$crop', '$variety', '$qc', '$stage','$tdate','$ncode1','$rsl','$p','$m','$g', '$yearid_id','$logid')";
			mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
		}
		if($got=="UT")
		{		
			$sql_sub_sub1245="insert into tbl_gottest(gottest_got, gottest_lotno, gottest_srdate, gottest_crop, gottest_variety, gottest_sampleno, gottest_trstage, gottest_oldlot, yearid, logid) values ('$got', '$lotn', '$tdate', '$crop', '$variety', '$ncode1', '$stage', '$lotno', '$yearid_id','$logid')";
