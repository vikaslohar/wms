	...
	$tdate=$e;
	 ...
	 $tdate=$tyear."-".$tmonth."-".$tday;
		...
		if(mysqli_query($link,$sql_sub_sub)or die(mysqli_error($link)))
		{
			...
			if($qq=mysqli_query($link,$sql_sub_sub12) or die(mysqli_error($link)))
			{
				if($got=="OK" || $got=="BL" || $got=="Fail")
				{
					...
					if(mysqli_num_rows($res_focusdbcode1) > 0)
					{
						...
						if($result=="Fail")
						{
							...
							$sql_crp=mysqli_query($link,"select * from tblcrop where cropname='".$crp."'") or die(mysqli_error($link));
							...
							$sql_var=mysqli_query($link,"select * from tblvariety where popularname='".$ver."'") or die(mysqli_error($link));
							...
							$sql_focusdb="insert into tbl_frn (wffrn_arrid, wffrn_docno, wffrn_date, wffrn_businessentity, wffrn_narration, wffrn_crop, wffrn_item, wffrn_batch, wffrn_code, wffrn_month, wffrn_yearcode, wffrn_trtype, wffrn_unit, wffrn_warehouse, wffrn_qty, wffrn_farmername, Doc_Type, wffrn_ploss, wffrn_pper, wffrn_vendorac, wffrn_department, wffrn_itemcode, account_code, account_name, wffrn_cropcode, constituent_lotnos) values('$tid', '".$doc_code."', '".$ogotdate."', 'HEAD OFFICE', '".$narration."', '".$rowfdb['wffrn_crop']."', '".$rowfdb['wffrn_item']."', '".$rowfdb['wffrn_batch']."', '".$doccode."', '".$monthName."', '".$fnyear."', 'GOT', 'KGS', 'Raw Seed', '".$rowfdb['wffrn_qty']."', '".$rowfdb['wffrn_farmername']."', 'Purchase Debit-CS-Seedtrac', '".$rowfdb['wffrn_ploss']."', '".$rowfdb['wffrn_pper']."', '".$farmercode."', '".$crptype."', '".$itemcode."', '".$accode."', '".$acname."', '".$cropcode."', '".$blendedcontlots."')";
