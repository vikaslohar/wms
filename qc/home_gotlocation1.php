...
$targetpage = $PHP_SELF; 
	...
	if($page) 
		...
		if($achar!="" && $char=="ALL")
	{
	$sql_arr_home = mysql_query("SELECT * FROM tbl_gotlocation where loc_name='".$achar."' order by loc_name LIMIT      $start, $limit"); 
	$query = "SELECT * FROM tbl_gotlocation where loc_name='".$achar."' order by loc_name";
	}
	else if( $char!="ALL"  && $achar=="")
	{ 
		$sql_arr_home = mysql_query("SELECT * FROM tbl_gotlocation where loc_name like '$char%' order by loc_name  LIMIT $start, $limit");
		$query = "SELECT * FROM tbl_gotlocation where loc_name like '$char%' order by loc_name";
	}
	else 
	{    
		$sql_arr_home=mysql_query("SELECT * FROM tbl_gotlocation order by loc_name LIMIT $start, $limit");
		$query = "SELECT * FROM tbl_gotlocation order by loc_name";
	}
//  $sql_arr_home=mysql_query("select * from tblvariety order by cropid, popularname desc LIMIT $start, $limit") or die(mysql_error());
 $tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_gotlocation order by loc_name"),0); 
...
$pagination = "";
	...
	if($achar!="")
	{
	$sql = mysql_query("SELECT * FROM tblproductionlocation where productionlocation like '".$achar."%' order by productionlocation  LIMIT $from, $max_results"); 
	$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblproductionlocation where productionlocation like '%".$achar."%'"),0); 
	}
	else if( 'ALL'!= $char)
	{
		$sql = mysql_query("SELECT * FROM tblproductionlocation where productionlocation like '".$char."%' order by state, productionlocation  LIMIT $from, $max_results");
		$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblproductionlocation where productionlocation like '".$char."%'"),0);  
	}
	else 
	{
		$sql = mysql_query("SELECT * FROM tblproductionlocation order by state, productionlocation LIMIT $from, $max_results"); 
		$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblproductionlocation"),0); 
	}
...
<?php
	...
	{
	$sql_f=mysql_query("select *  from tbl_gotlocation where loc_id='".$row['loc_id']."'");
  	$result_f=mysql_fetch_array($sql_f);
  	$num_of_farmers=mysql_num_rows($sql_f);
	
	$sql_f1=mysql_query("select * from tbl_state where state_id='".$row['state']."'");
  	$result_f1=mysql_fetch_array($sql_f1);
   $num_of_farmers1=mysql_num_rows($sql_f1);
	
	/*$sql_tra=mysql_query("select * from tblarrival where productionlocationid=".$row['productionlocationid'])or die(mysql_error());
  	$row_tra=mysql_fetch_array($sql_tra);
	$num_tra=mysql_num_rows($sql_tra);
	
	$sql_tra1=mysql_query("select * from tblissue where productionlocationid='".$row['productionlocationid']."'")or die(mysql_error());
