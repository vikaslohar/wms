...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from tbl_gotlocation order by loc_name LIMIT $start, $limit") or die(mysql_error());
 $tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_gotlocation"),0); 
...
<?php
	...
	$sql_f=mysql_query("select * from tbl_gotlocation where loc_name='".$row['loc_name']."'");
   ...
   $num_of_farmers1=mysql_num_rows($sql_f1);
	/*$sql_v=mysql_query("select * from tbl_partymaser where 	location_id =".$row['productionlocationid'])or die(mysql_error());
