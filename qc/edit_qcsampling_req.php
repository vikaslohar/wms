	...
	//exit;
		...
		if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
		{
			...
			foreach($pl as $val)
			{
				if($val<>"")
				{
					$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_lotno='".$val."' order by lotldg_id DESC") or die(mysqli_error($link)); 
		...
		echo "<script>window.location='add_qcsampling_preview.php?cropid=$p_id&txtvariety=$txtvariety&txtcrop=$txtcrop&date=$date'</script>";	
		...
		if(mysqli_num_rows($res_code) > 0)
			{
				...
				$t_code=$row_code['0'];
				$code=$t_code+1;
			$code1="TQS".$code."/".$yearid_id."/".$lgnid;
		}
		else
		{
			$code=1;
...
<title>Quality- Transaction -CSW  sampling</title>
...
var f=0;
	...
	if(document.frmaddDepartment.cntt.value > 0)
	{
...
$sql_tbl_sub2=mysqli_query($link,"select * from tbl_qcgen1 where arrival_id='".$arrival_id."'") or die(mysqli_error($link));
...
<form id="mainform" name="frmaddDepartment" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" onsubmit="return mySubmit();"   > 
	 ...
	 <input name="txt11" value="" type="hidden"> 
		...
		<input type="hidden" name="txtid" value="<?php echo $row_tbll['arr_code']?>" />
...
<td width="234"  align="left" valign="middle" class="tbltext">&nbsp;<?php echo "TQS".$row_tbll['arr_code']."/".$row_tbll['year_code']."/".$row_tbll['arr_role'];?></td>
...
<td width="176" align="right" valign="middle" class="tblheading">Seed Stage&nbsp;</td>
...
<option value="" selected>--Select Crop--</option>
	<?php while($noticia = mysqli_fetch_array($quer3)) { ?>
		<option <? if($noticia['cropid']==$row_tbll['crop']){ echo "Selected";} ?> value="<?php echo $noticia['cropid'];?>" />   
		<?php echo $noticia['cropname'];?>
		<?php } ?>
	</select>
              <font color="#FF0000">*</font>&nbsp;</td>
			   <?php
$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where cropname='".$row_tbll['crop']."' and actstatus='Active' order by popularname Asc"); 
?>
	<td width="137" align="right"  valign="middle" class="tblheading" >Variety&nbsp;</td>
...
<option value="" selected>--Select Variety--</option>
	<?php while($noticia_item = mysqli_fetch_array($quer4)) { ?>
		<option <? if($noticia_item['varietyid']==$row_tbll['variety']){ echo "Selected";} ?> value="<?php echo $noticia_item['varietyid'];?>" />   
		<?php echo $noticia_item['popularname'];?>
		<?php } ?></select>&nbsp;<font color="#FF0000">*</font>&nbsp;</td>
           </tr>
         <tr class="Light" height="30" id="vitem">
           <!--<td align="right"  valign="middle" class="tblheading">Lot Number &nbsp;</td>
           <td align="left" width="239" valign="middle" class="tbltext" style="border-color:#d21704" >&nbsp;<input name="txtlot1" type="text" size="20" class="tbltext" tabindex="" readonly="true" style="background-color:#ECECEC"  maxlength="20"  value=""  />&nbsp;<font color="#FF0000">*</font>&nbsp;<a href="Javascript:void(0);" onclick="openslocpop();">Select</a><input type="hidden" name="qcrsl" value="" /><input type="hidden" name="gotrsl" value="" /></td>-->
		  
		   <td align="right"  valign="middle" class="tblheading">&nbsp;Select QC Tests&nbsp;</td>
  <td  align="left"  valign="middle" class="tbltext" colspan="3" ><input name="txt1" <?php if($row_tbl_sub['pp']=="P"){ echo "checked";}  ?>  type="checkbox" class="tbltext" tabindex="0"  value="P" onclick="clk(this.value);"/>PP   
   &nbsp;
...
$sql_m=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='".$row_tbll['crop']."' and lotldg_variety='".$row_tbll['variety']."' ") or die(mysqli_error($link));
...
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$row_tbll['crop']."' and lotldg_variety='".$row_tbll['variety']."' and lotldg_lotno='".$val."'") or die(mysqli_error($link));
$trtr=mysqli_num_rows($sql_issue);
 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_variety='".$row_tbll['variety']."'  and lotldg_lotno='".$val."'") or die(mysqli_error($link));
...
$sstage=$row_issuetbl['lotldg_sstage']; 
$gotres=$row_issuetbl['lotldg_got']; 
...
if($qc!="OK" && $qc!="NUT" && $qc!="BL")
{
$vflg++;
}
else
{
	if($qc="Fail" && $gotres!="OK" && $gotres!="NUT" && $gotres!="BL")
	{
	$vflg++;
	}
}
...
{
$ltchk=0;
$sqltblsub=mysqli_query($link,"select * from tbl_qcgen1 where arrival_id='".$arrival_id."'") or die(mysqli_error($link));
...
<td align="center" valign="middle" class="tblheading"><input type="hidden" id="qcst_<?php echo $srn;?>" name="qcst_<?php echo $srn;?>" readonly="true" style="background-color:#CCCCCC" value="<?php echo $qc;?>" size="10" /><?php echo $sstage;?></td>
...
<?php if($cnt > 0){ ?>
<tr class="Light" height="30">
<td align="right" valign="middle" class="tblheading" colspan="6">Total&nbsp;&nbsp;</td>
...
<table align="center" width="850" cellpadding="5" cellspacing="5" border="0" >
<tr >
<td class="tblheading" valign="top" align="center">No Records found for selected Variety</td>
</tr>
</table>
<?php } ?>
</div>
