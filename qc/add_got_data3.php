...
if(isset($_POST['frm_action'])=='submit')
{
	...
	//exit;
	$sql_sub_sub2="insert into tbl_gottestsub_sub3(gottest_tid, gottests_id, gottestss_id, gottestss3_bedno, gottestss3_direction, gottestss3_state, gottestss3_gotlocation,  gottestss3_plantpopln, gottestss3_maleno, gottestss3_maleper, gottestss3_femaleno, gottestss3_femaleper, gottestss3_oofno, gottestss3_oofper, gottestss3_totno, gottestss3_totper, gottestss3_genpurity, gottestss3_doobrdate) values('$id', '$gots_id', '$gotss_id', '$bedno','$direction','$state', '$location','$plantpopn','$maleno','$maleper','$femaleno','$femaleper','$oofno','$oofper','$totno','$totper','$genpurity','$obdate')";
	...
	/*$sql_sub_sub1="update tbl_gottestsub_sub set geaflg=2 where gottestss_id=$gotss_id";
	$row_sub_sub1=mysqli_query($link,$sql_sub_sub1)or die(mysqli_error($link));*/
	
	echo "<script>window.location='add_got_data3.php?tid=$id&gotssid=$gotss_id&gotsid=$gots_id&interrarepl=$interrarepl'</script>";
}
...
function plantpop()
{
	...
	if(document.frmaddDept.txtnoofplants.value < document.frmaddDept.txtplantpopn.value)
 	{
		alert("Please enter valid plant population");
		document.frmaddDept.txtplantpopn.value="";
...
{
	//alert("In Function...");
	//alert(document.frmaddDept.txtplantpopn.value);
	var dt1=getDateObject(document.frmaddDept.obdate.value,"-");
	var dt2=getDateObject(document.frmaddDept.pdate.value,"-");
	var dt3=getDateObject(document.frmaddDept.trdate.value,"-");
	//alert(dt1);
	//alert(dt2);
	...
	if(dt1 < dt2)
	{
		alert("Date of Observation Should Be Greater Than Sowing Date");
		document.frmaddDept.obdate.value="";
		document.frmaddDept.obdate.focus();
		return false;
	}
	if(dt3!="" && (dt1 < dt3))
	{
		alert("Date of Observation Should Be Greater Than Transplanting Date");
		document.frmaddDept.obdate.value="";
		document.frmaddDept.obdate.focus();
		return false;
	}
...
<td width="282" align="center" valign="middle" class="tblheading">Date of Sowing</td>
	...
	<td width="321" align="center" valign="middle" class="tblheading">No. of Seeds</td>
</tr>
<?php
$rdate=""; $exdate="";
...
<tr class="Dark" height="25">
	...
	<td width="98" align="center" valign="middle" class="tblheading">No. of Plants</td>
</tr>
<?php
$pdate="";
...
<td width="112" align="center" valign="middle" class="smalltbltext"><?php echo $pdate1?><input name="trdate" type="hidden" value="<?php echo $pdate1?>"></td>
...
<td width="98" align="center" valign="middle" class="smalltbltext"><?php echo $rowss1['gottestss_plantpopln']?><input name="txtnoofplants" type="hidden" value="<?php echo $rowss1['gottestss_plantpopln']?>"></td>
</tr>
</table><br />
...
<?php
$sql_ss3=mysqli_query($link,"select * from tbl_gottestsub_sub3 where gottestss_id='".$gotssid."' order by gottestss3_id asc")or die(mysqli_error($link));
//$rowss= mysqli_fetch_array($sql_ss);
$srno1=1;
while($rowss3=mysqli_fetch_array($sql_ss3))
{ 
$sql_loc=mysqli_query($link,"select * from tbl_gotlocation where loc_id='".$rowss3['gottestss3_gotlocation']."'")or die(mysqli_error($link));
$rowloc=mysqli_fetch_array($sql_loc);

	$obrdate=$rowss3['gottestss3_doobrdate'];
	$tyear=substr($obrdate,0,4);
	$tmonth=substr($obrdate,5,2);
	$tday=substr($obrdate,8,2);
	$obrdate1=$tday."-".$tmonth."-".$tyear; 
