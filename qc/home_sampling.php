...
<?php
// $sql_arr_home=mysqli_query($link,"select * from tbl_qctest where aflg=0 and bflg=0 and cflg=0 group by sampleno, yearid order by srdate asc, crop asc, variety asc, sampleno DESC") or die(mysqli_error($link));
$sql_arr_home=mysqli_query($link,"select * from tbl_qctest where aflg=0 and bflg=0 and cflg=0 and qcflg=0 group by sampleno, yearid, trstage order by srdate, sampleno, yearid asc") or die(mysqli_error($link));
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="950" bordercolor="#d21704" style="border-collapse:collapse">
		...
		<td width="31" align="center" valign="middle" class="smalltblheading">SR/SSR Status</td>	
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	if($row_arr_home['trstage']!="Raw" && $row_arr_home['trstage']!="Condition")
	{
		$qry_reval=mysqli_query($link,"SELECT rv_rvflg FROM tbl_revalidate  where rv_lotno='".$row_arr_home['lotno']."'"); 
		while($row_reval=mysqli_fetch_array($qry_reval))
		{
			$pckflg=1;
		}
	}
	...
	$crop=""; $variety=""; $lotno=""; $stage=""; $got=""; $qc="";  $srstatus='';
...
//if($details=="" && $totqty==0)
{
	$sql_tblsub1=mysqli_query($link,"select distinct orlot from tbl_lot_ldg_pack where orlot='".$lotno."' order by orlot") or die(mysqli_error($link));
	$t_tblsub1=mysqli_num_rows($sql_tblsub1);
	while($rowtbl22=mysqli_fetch_array($sql_tblsub1))
	{
		$bags=0; $qty=0; $slocs="";
		$sql_tbl_sub1=mysqli_query($link,"select distinct subbinid, lotldg_variety, lotldg_crop, whid, binid from tbl_lot_ldg_pack where orlot='".$lotno."' and lotldg_sstage='Pack' group by subbinid, lotldg_variety, orlot order by subbinid") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_tbl_sub1);
		while($row_tbl22=mysqli_fetch_array($sql_tbl_sub1))
		{
			$ac=0; $acn=0; $gd=""; $slups=0;$slqty=0;
			$sql_tbl1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_tbl22['subbinid']."' and orlot='".$lotno."' and lotldg_sstage='Pack'") or die(mysqli_error($link));  
			$row_tbl1=mysqli_fetch_array($sql_tbl1);
...
if($t_tblsub1==0 && ($stage=="SR1" || $stage=="SR2" || $stage=="SR3" || $stage=="SR4" || $stage=="SR5") && $row_tbl_sub1['trstage']=="Pack")
{
	$bags=0; $qty=0; $slocs="";
	$ac=0; $acn=0; $gd=""; $slups=0;$slqty=0;

	$sql1=mysqli_query($link,"select * from tbl_salesrv_sub where salesrs_orlot='".$lotno."' and salesrs_stage='Pack'")or die(mysqli_error($link));
	$total_tbl=mysqli_num_rows($sql1);
			
	while($row_tbl=mysqli_fetch_array($sql1))
	{	
		//$lotldg_trid=$row_tbl['lotldg_trid'];
		$sql_tbl1=mysqli_query($link,"select * from tbl_salesrvsub_sub where salesrs_id='".$row_tbl['salesrs_id']."' ") or die(mysqli_error($link));  
		$row_tbl1=mysqli_fetch_array($sql_tbl1);
		
		$ac=0; $acn=0;
		$aq=explode(".",$row_tbl1['salesrss_nob']);
		if($aq[1]==000){$ac=$aq[0];}else{$ac=$row_tbl1['salesrss_nob'];}
		
		$an=explode(".",$row_tbl1['salesrss_qty']);
		if($an[1]==000){$acn=$an[0];}else{$acn=$row_tbl1['salesrss_qty'];}
		
		$wareh=""; $binn=""; $subbinn=""; $sups="";$sqty=0; 
			
		
		
		$sql_whouse=mysqli_query($link,"select perticulars from tblsrwarehouse where whid='".$row_tbl1['salesrss_wh']."'") or die(mysqli_error($link));
		$row_whouse=mysqli_fetch_array($sql_whouse);
		$wareh=$row_whouse['perticulars']."/";
			
		$sql_binn=mysqli_query($link,"select binname from tblsrbin where binid='".$row_tbl1['salesrss_bin']."' and whid='".$row_tbl1['salesrss_wh']."'") or die(mysqli_error($link));
		$row_binn=mysqli_fetch_array($sql_binn);
		$binn=$row_binn['binname']."/";
		
			
		$sql_subbinn=mysqli_query($link,"select sname from tblsrsubbin where sid='".$row_tbl1['salesrss_subbin']."' and binid='".$row_tbl1['salesrss_bin']."' and whid='".$row_tbl1['salesrss_wh']."'") or die(mysqli_error($link));
		$row_subbinn=mysqli_fetch_array($sql_subbinn);
		$subbinn=$row_subbinn['sname'];
		$slups=$slups+$row_tbl['salesrss_nob'];
			 
		$an24=explode(".",$row_tbl1['salesrss_qty']);
		if($an24[1]==000){$acn24=$an[0];}else{$acn24=$row_tbl1['salesrss_qty'];}
		$slqty=$slqty+$acn24;
...
$sql_srsts=mysqli_query($link,"select * from tbl_softr_sub where softrsub_lotno='".$lotno."' order by softrsub_id DESC ") or die(mysqli_error($link));  
$row_srsts=mysqli_fetch_array($sql_srsts);
$srstatus=$row_srsts['softrsub_srtyp'];

if($srstatus=="")
{
	$sql_srsts=mysqli_query($link,"select * from tbl_softr_sub2 where softrsub_lotno='".$lotno."' order by softrsub_id DESC ") or die(mysqli_error($link));  
	$row_srsts=mysqli_fetch_array($sql_srsts);
	$srstatus=$row_srsts['softrsub_srtyp'];
}
...
<tr class="Light">
	...
	<td width="31" align="center" valign="middle" class="smalltbltext"><?php echo $srstatus?></td>
