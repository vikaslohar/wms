...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_qcgotmain where gotm_resultflg=0 and gotm_finobrflg=1 order by gotm_tdate  ASC LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "SELECT COUNT(*) as num FROM tbl_qcgotmain where gotm_resultflg=0 and gotm_finobrflg=1";
...
<tr class="Light" height="20">
  <td align="center" class="tblheading">GOT Result Pending List</td>
</tr>
</table>
...
<tr class="tblsubtitle" height="20">
	...
	<td width="63" align="center" valign="middle" class="tblheading">GOT Data Review &amp; Update</td>
</tr>             
<?php
//$srno=1;

while($row_gotmain=mysqli_fetch_array($sql_arr_home))
{
	$smp=str_split($row_gotmain['gotm_sampleno']);
	$yrcode=$smp[1];
	$sampno=$smp[2].$smp[3].$smp[4].$smp[5].$smp[6].$smp[7];
	$sampno=intval($sampno);
		
	
	$sql_gotmain=mysqli_query($link,"select * from tbl_gottest where  gottest_sampleno='".$sampno."'  and yearid='".$yrcode."' and gottest_lotno='".$row_gotmain['gotm_lotno']."' and gottest_resultflg=0 order by gottest_tid DESC") or die(mysqli_error($link));
 	$tot_gotmain=mysqli_num_rows($sql_gotmain);
	
	while($row_arr_home=mysqli_fetch_array($sql_gotmain))
	{
	
	
		$lrole=$row_gotmain['gotm_acklogid'];
		$arrival_id=$row_gotmain['gotm_id'];
		$qc1=$sampno;
		$sampleno=$row_gotmain['gotm_sampleno'];
		$yriid=$yrcode;
...
?>			  
<tr class="Light">
	<td width="20" align="center" valign="middle" class="smallsmalltbltext"><?php echo $srno;?></td>
	<td width="70" align="center" valign="middle" class="smallsmalltbltext"><?php echo $row31['cropname'];?></td>
	<td align="center" valign="middle" class="smallsmalltbltext"><?php echo $vv;?></td>
	<td width="118" align="center" valign="middle" class="smallsmalltbltext"><?php echo $lotno?></td>
	<td width="71" align="center" valign="middle" class="smallsmalltbltext"><?php echo $trdate?></td>
	<td width="69" align="center" valign="middle" class="smallsmalltbltext"><?php echo $tdate1;?></td>
	<td align="center" valign="middle" class="smallsmalltbltext"><?php echo $tdatee;?></td>
	<td width="65" align="center" valign="middle" class="smallsmalltbltext"><?php echo $sampleno;?></a></td>
	<td align="center" valign="middle" class="smallsmalltbltext"><a href="add_gotdata_upd.php?tid=<?php echo $row_gotmain['gotm_id'];?>" >Review</a></td>
</tr>
<?php
}
else
