...
//ob_start(); 
	...
	$serveradr=explode($servername,$_SERVER['HTTP_REFERER']);
	$eurl=$scheme.'://'.$server_name.$serveradr[1];
	//$eurlnew=$scheme.'://'.$server_name.$serveradr[1];
	//echo $eurl=$scheme.'://'.$server_name.$port.$serveradr[1];
	/*echo "<br />";	
	...
	$nwlotn=str_split($lotn);
 	...
 	$nlotno=$nwlotn[0].$nwlotn[1].$nwlotn[2].$nwlotn[3].$nwlotn[4].$nwlotn[5].$nwlotn[6].$nwlotn[7].$nwlotn[8].$nwlotn[9].$nwlotn[10].$nwlotn[11].$nwlotn[12].$nwlotn[13].$nwlotn[14].$nwlotn[15];	
	...
	{
		$sqlck2=mysqli_query($link,"select * from tbl_qctest where sampleno='".$rowck['sampleno']."' and yearid='$yrcd' and  SUBSTRING(`oldlot`,1,7)='$nwlotno' order by tid asc") or die(mysqli_error($link));
		while($rowck2=mysqli_fetch_array($sqlck2))
		{
		echo $rowck2['tid']." ".$rowck['sampleno'];
		}
	}
	exit;*/
//echo "eurl ".$eurl; echo "<br />";		
//echo "eurlnew ".$eurlnew;
//echo $btnval;
//exit;
...
{
	$sql_chk=mysqli_query($link,"select * from tbl_qctest where oldlot='$nlotno'") or die(mysqli_error($link));
	$tot_chk=mysqli_num_rows($sql_chk);
	
	$sql_tbl=mysqli_query($link,"select * from tbl_qctest where oldlot='$nlotno' order by tid desc limit 1,1") or die(mysqli_error($link));
	...
	if($tot_tbl > 0 && $tot_chk > 1)
	{
		...
		$oldlotn=$row_tbl['oldlot'];
		$sql_sub_sub12="update tbl_qctest set qcstatus='Abort', testdate='$tdate',qcflg=1 where tid='$a'";
		$qq=mysqli_query($link,$sql_sub_sub12) or die(mysqli_error($link));
		
		$sql_sub="update tbl_lot_ldg set lotldg_qc='$qcresult', lotldg_gemp='$gempo', lotldg_moisture='$moisto', lotldg_qctestdate='$tdateo' where orlot='$nlotno'";
		$sql_sub2="update tbl_lot_ldg_pack set lotldg_qc='$qcresult', lotldg_gemp='$gempo', lotldg_moisture='$moisto', lotldg_qctestdate='$tdateo' where orlot='$nlotno'";
		$sql_sub3="update tbl_salesrv_sub set salesrs_qc='$qcresult', salesrs_dot='$tdateo' where salesrs_orlot='$nlotno' and (salesrs_qc='UT' OR salesrs_qc='RT')";	
	...
	else
	{
		...
		$sql_sub="update tbl_lot_ldg set lotldg_qc='NUT' where orlot='$nlotno'";
		$qq=mysqli_query($link,$sql_sub) or die(mysqli_error($link));
		$sql_sub2="update tbl_lot_ldg_pack set lotldg_qc='NUT' where orlot='$nlotno'";
		$qq2=mysqli_query($link,$sql_sub2) or die(mysqli_error($link));
	}
	
	$arrival_id21=0;
	$sql_tbl21=mysqli_query($link,"select * from tbl_qcgen1 where arr_role='".$logid."' and lotno='".$nlotno."'") or die(mysqli_error($link));
...
{ 
	if(document.frmaddDepartment.btnval.value==0)
	{
		...
		if(document.frmaddDepartment.result.value=="ON")
		{
			if(document.frmaddDepartment.leduration.value=="")
			{
				alert("Please Select Life Expentancy (LE) Duration.");
				document.frmaddDepartment.leduration.focus();
				return false;
			}
			if(document.frmaddDepartment.leupto.value=="")
			{
				alert("Life Expentancy (LE) cannot be blank.");
				document.frmaddDepartment.leduration.focus();
				return false;
			}
		}
...
function chkledur(qcval)
{
	if(qcval=="OK")
	{
	document.frmaddDepartment.leduration.disabled=false;
	}
	else
	{
	document.frmaddDepartment.leduration.disabled=true;
	}
}
...
<form id="mainform" name="frmaddDepartment" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>"  > 
	 ...
	 <input name="txt11" value="" type="hidden"> 
		...
		<input type="hidden" name="tid" value="<?php echo $a?>" />
...
$nob=0; $qty=0; $wareh=""; $binn=""; $subbinn=""; $sups="";$sqty=0; $slocs=""; $slocs2=""; $mflg=0; $harvestdt='';
if($stage!="Pack")
{
...
$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where orlot='".$lotnn."'") or die(mysqli_error($link));
...
<td align="right"  valign="middle" class="tblheading" >Result&nbsp;</td>
 <td align="left"  valign="middle" class="tbltext">&nbsp;<select class="tbltext" name="result" style="width:100px;" onchange="chkledur(this.value)"  >
    <option value="" selected>--Select--</option>
...
?>
 ...
 <td align="left"  valign="middle" class="tbltext"  >&nbsp;<select name="leduration" class="tbltext" tabindex="0" style="width:40px;" disabled="disabled" onchange="showledur(this.value)"  > 
...
<tr >
<td valign="top" align="Center"><a href="<?php echo $eurl;?>" tabindex="20"><img src="../images/back.gif" border="0"style="display:inline;cursor:hand;" onclick="return confirm('Do You wish to Cancel this Transaction?');" /></a>&nbsp;&nbsp;<img src="../images/abort.gif" border="0"style="display:inline;cursor:hand;" onclick="return smpabort('1');" />&nbsp;&nbsp;<!--<input name="image" type="image" style="display:inline;cursor:pointer;" onclick="smpabort('1');" src="../images/abort.gif" alt="Abort Value" border="0"/>&nbsp;&nbsp;--><input name="Submit" type="image" src="../images/submit.gif" alt="Submit Value"  border="0" style="display:inline;cursor:hand;" tabindex="" onClick="return mySubmit();">&nbsp;&nbsp;</td>
</tr>
</table>
</td><td width="30"></td>
</tr>
