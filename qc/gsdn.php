...
<?php
/*$zx=count(explode(",",$row['lotno']));
$qtid=$row['lotno'];
if($zx>1)
$sql_arr_home2=mysqli_query($link,"select distinct oldlot from tbl_qctest where aflg=0 and bflg=1 and cflg=0 and tid IN($qtid)  order by gottest_sampleno desc") or die(mysqli_error($link));
else
$sql_arr_home2=mysqli_query($link,"select distinct oldlot from tbl_qctest where aflg=0 and bflg=1 and cflg=0 and tid=$qtid  order by gottest_sampleno desc") or die(mysqli_error($link));
