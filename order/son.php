...
<table width="750" height="282" border="0" align="center" cellpadding="0" cellspacing="0" >
 ...
 <?php 
$tid=$itmid;
$sql_tbl=mysqli_query($link,"select * from tbl_orderm where plantcode='$plantcode' and   order_trtype='Order Sales' and orderm_id='".$tid."'") or die(mysqli_error($link));
...
<td align="left"  valign="middle" class="tbltext" colspan="6" id="vaddress"><div style="padding-left:4px"><?php echo $row3['address'];?><?php if($row3['city']!=""){ echo ", ".$row3['city'];}?><?php if($pin1!=""){ echo $pin1;}?>&nbsp;<?php echo $row3['state'];?><?php if($row_tbl['orderm_country']!=""){ echo ", ".$row_tbl['orderm_country'];}?><?php if($row3['mob']!="" && $row3['mob']!=0){echo ", Mob no.-".$row3['mob'];}?><?php if($row3['phone']!=""){echo ", Phone no. ".$row3['std']."-".$row3['phone'];}?><?php if($row3['tin']!=""){echo ", Tin no.-".$row3['tin'];}?><?php echo ".";?></div></td>
</tr>
<tr class="Light" height="30">
