...
$sqlpty1=mysqli_query($link,"select distinct orderm_partyname from tbl_orderm where plantcode='$plantcode' and orderm_date<='$edate' and orderm_date>='$sdate' and  orderm_party='0' order by orderm_date asc ") or die(mysqli_error($link));
	...
	$dh="Crop_Variety_wise_Compiled_Pending_Order_Report_Period_From_".$_REQUEST['sdate']."_To_".$_REQUEST['edate'];
	$filename = $dh.".xls";
	$data1 = array();
	$datahead = array("Crop Variety wise Compiled Pending Order Report");
...
if($cropval<>"")
{
	...
	$sql=mysqli_query($link,"Select distinct orderm_id from tbl_order_sub where order_sub_crop='".$cropval."' order by orderm_id") or die(mysqli_error($link));
	$tot_crpsub=mysqli_num_rows($sql);
	$cropname=$rowcrp2['cropname'];		
	
	$datahead4[$cnt]=array("Crop $cropname");
	$datahead5[$cnt]=array("#","Date","Order No","Party Name","Location","State","Variety","Size","Ordered Qty","Balance Qty"); 
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$ssub="select * from tbl_order_sub where plantcode='$plantcode' and orderm_id='$arrival_id' ";
...
$flg=0;
	...
	$ver1=$notici['popularname'];
		...
		{ 
			if($flg==0) 
			{
				$data1[$cnt][$d]=array($d,$trdate,$orno,$party,$loca,$statee,$ver1,$size,$oqty,$bqty); 
				$d++;
			}
		}
}
...
//}
if($d==1)
$data1[$cnt][$d]=array("","","","","No Pending Orders found","","","","",""); 
$cnt++;
//}
}
}
