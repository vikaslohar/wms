...
$sql_tbl=mysqli_query($link,"select * from tbl_orderm where plantcode='$plantcode' and order_trtype='Order TDF' and orderm_tflag=0 and logid='".$logid."' ") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tbl_orderm where  order_trtype='Order TDF' and orderm_tflag=0 and logid='".$logid."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='home_trial1.php?sdate=$sdate&edate=$edate'</script>";
		}
		else
		 {?>
		 <script>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#cc30cc" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#cc30cc" style="border-bottom:solid; border-bottom-color:#cc30cc" >
	    <tr >
	      <td width="820" height="25" class="Mainheading" > &nbsp; Transaction - Order-Trial/Demo/Free</td>
	    </tr></table></td>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_orderm where plantcode='$plantcode' and order_trtype='Order TDF' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 order by orderm_id desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_orderm where plantcode='$plantcode' and order_trtype='Order TDF' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 order by orderm_id desc";
...
/*if(!isset($_GET['page'])) { 
	...
	$sql_arr_home=mysqli_query($link,"select * from tbl_orderm where order_trtype='Order TDF' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 order by orderm_id desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);


$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_orderm where order_trtype='Order TDF' and orderm_tflag=1 and orderm_supflag=0 and	orderm_cancelflag=0");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
...
<tr >
<td valign="top" align="right"><a href="hoem_trial.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:Pointer;" /></a></td>
</tr>
</table>
