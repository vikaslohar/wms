<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_trial1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='home_trial.php'</script>";
		}
	}
...
<title>Order-Transaction - Order-Trial/Demo/Free</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#cc30cc" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#cc30cc" style="border-bottom:solid; border-bottom-color:#cc30cc" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Order-Trial/Demo/Free</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_trial.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
</table></td>
...
$sql_arr_home=mysqli_query($link,"select * from tblarrival where plantcode='$plantcode' and arrival_type='Stocktransfer' and arrtrflag=1") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
?>


<!--- Table Place Holder --->
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#cc30cc" style="border-collapse:collapse">
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_orderm where plantcode='$plantcode' and orderm_date >='$tdate' and orderm_date <='$pdate' and  order_trtype='Order TDF' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 order by orderm_id desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_orderm where plantcode='$plantcode' and orderm_date >='$tdate' and orderm_date <='$pdate' and  order_trtype='Order TDF' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 order by orderm_id desc";
...
/*if(!isset($_GET['page'])) { 
	...
	$sql_arr_home=mysqli_query($link,"select * from tbl_orderm where orderm_date >='$tdate' and orderm_date <='$pdate' and  order_trtype='Order TDF' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 order by orderm_id desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_orderm where orderm_date >='$tdate' and orderm_date <='$pdate' and  order_trtype='Order TDF' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
  if($tot_arr_home >0) { 
?>
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Order-Trial/Demo/Free</td>
</tr>
</table>
...
<!--    <td width="9%" align="center" valign="middle" class="tblheading">Transaction Id</td>-->
    <td width="10%" align="center" valign="middle" class="tblheading">Order Number</td>
    <td width="10%" align="center" valign="middle" class="tblheading"> Party Type</td>
    <td width="16%" align="center" valign="middle" class="tblheading">Party Name</td>
    <td width="18%" align="center" valign="middle" class="tblheading">Party Location </td>
    <td width="11%" align="center" valign="middle" class="tblheading">Consignee Location</td>
    <td width="8%" align="center" valign="middle" class="tblheading">Order By</td>
    <td width="4%" align="center" valign="middle" class="tblheading">Var. No.</td>
...
$subtbltot=mysqli_num_rows($sql_tbl_sub);
	...
	if($row_arr_home['orderm_partyselect']=="selectp") {	$partytype="Select"; } else { $partytype="Fill"; }
	
	if($partytype == "Select")
	{
	
	if($row_arr_home['orderm_party_type']=="Channel")
	{ $partytyp="Channel"; } 
	else if($row_arr_home['orderm_party_type']=="Stock Transfer") 
	{ $partytyp="Stock Transfer"; } 
	else if($row_arr_home['orderm_party_type']=="Individual") 
	{ $partytyp="Individual-TDF"; } 
	else
	{ $partytyp="Fill"; }
	
	$partyname=$row3['business_name']; 
	$partyadd=$row3['city'].", ".$row3['state'];
	}
	else
	{
	$partytyp="Fill";
	$partyname=$row_arr_home['orderm_partyname'];
	$partyadd=$row_arr_home['orderm_partycity'].", ".$row_arr_home['orderm_partystate'];
	}
...
?>
  <tr class="Light">
    ...
    <td width="7%" align="center" valign="middle" class="smalltbltext"><?php echo $trdate;?></td>
  <!--  <td width="9%" align="center" valign="middle" class="smalltbltext"><?php echo "OD".$row_arr_home['orderm_trcode']."/".$ycode."/".$lrole;?></td>-->
    <td width="10%" align="center" valign="middle" class="smalltbltext"><?php echo $row_arr_home['orderm_porderno'];?></td>
    <td width="10%" align="center" valign="middle" class="smalltbltext"><?php echo $partytyp;?></td>
    <td width="16%" align="center" valign="middle" class="smalltbltext"><?php echo $partyname;?></td>
    <td width="18%" align="center" valign="middle" class="smalltbltext"><?php echo $partyadd?></td>
    <td width="11%" align="center" valign="middle" class="smalltbltext"><?php if($row_arr_home['orderm_consigneeapp']=="Yes1"){?><?php echo $row_arr_home['orderm_concity'];?>, <?php echo $row_arr_home['orderm_constate'];}?></td>
    <td width="8%" align="center" valign="middle" class="smalltbltext"><?php echo $row_arr_home['orderm_placedby'];?></td>
    <td width="4%" align="center" valign="middle" class="smalltbltext"><?php echo $subtbltot;?></td>
    <td width="5%" align="center" valign="middle" class="smalltbltext"><a href="Javascript:void(0)" onClick="openslocpopprint('<?php echo $row_arr_home['orderm_id'];?>');">DON</a></td>
  </tr>
  <?php
}
else
...
<tr >
<td valign="top" align="right"><a href="home_trial.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:Pointer;" /></a></td>
</tr>
</table>
</form> 
