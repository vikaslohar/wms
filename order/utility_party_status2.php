...
<title>Order Booking -Utility-Party wise -Order Search		</title>
  ...
  <tr><td>
   ...
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#cc30cc" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#cc30cc" style="border-bottom:solid; border-bottom-color:#cc30cc" >
	    <tr >
	      <td width="813" height="25">&nbsp;Party wise -Order Search		
</td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
<td align="left"  valign="middle" class="smalltblheading"colspan="6">&nbsp;<?php echo $styp;?></td>
</tr>
<?php 
if($orsrval=="ordersearch")
{
...
<td align="left"  valign="middle" class="smalltblheading" colspan="6" id="vaddress">&nbsp;<?php echo $orderno;?></td>
</tr>
<?php
}
else if($orsrval=="partysearch")
...
if($b=="Commercial")
{ $ortyp="Order TDF";
$sql_main=mysqli_query($link,"select * from tbl_orderm where plantcode='$plantcode' and  orderm_party='".$partyname."' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 and orderm_dispatchflag!=1 and orderm_holdflag=0 order by orderm_date, orderm_porderno asc") or die(mysqli_error($link));
}
else if($b=="TDF")
{
$ortyp="Order TDF";
...
else
{
	if($fillpartyname1!="")
	{
	$sql_main=mysqli_query($link,"select * from tbl_orderm where plantcode='$plantcode' and  orderm_partyname='".$partyname."' and order_trtype='".$ortyp."' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 and orderm_dispatchflag!=1 and orderm_holdflag=0 order by orderm_date, orderm_porderno asc") or die(mysqli_error($link));
	}
	else
	{
	$sql_main=mysqli_query($link,"select * from tbl_orderm where plantcode='$plantcode' and  orderm_party='".$partyname."' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 and orderm_dispatchflag!=1 and orderm_holdflag=0 order by orderm_date, orderm_porderno asc") or die(mysqli_error($link));
...
if($orsrval=="ordersearch")
{
	...
	{
	$sql_main=mysqli_query($link,"select * from tbl_orderm where plantcode='$plantcode' and  orderm_porderno like '$orderno%' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 and orderm_dispatchflag!=1 and orderm_holdflag=0 order by orderm_date, orderm_porderno asc") or die(mysqli_error($link));
	}
}
else
{
	...
	{
	$sql_main=mysqli_query($link,"select * from tbl_orderm where plantcode='$plantcode' and  orderm_date <= '$edate1' and orderm_date >='$sdate1' and orderm_tflag=1 and orderm_supflag=0 and orderm_cancelflag=0 and orderm_dispatchflag!=1 and orderm_holdflag=0 order by orderm_date, orderm_porderno asc") or die(mysqli_error($link));
	}
}
$tot_main=mysqli_num_rows($sql_main);
while($row_main=mysqli_fetch_array($sql_main))
...
<tr >
<td height="49" align="center" valign="top"><a href="utility_party_status1.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;
