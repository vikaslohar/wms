...
$dh="All_Party_Consolidated_Pending_Order_Report_as_on_".$_REQUEST['sdate'];
$datahead = array("All Party Consolidated Pending Order Report as on ".$_REQUEST['sdate']);
$filename=$dh.".xls";  
		...
		while($row_11=mysqli_fetch_array($sql_arr_home11))
		{
			...
			if($tot_arr_home33 > 0)
			{
				while($row_33=mysqli_fetch_array($sql_arr_home33))
				{
					...
					if($parortot > 0)
					{
						$datahead1[$cnt] = array("Party Name:",$partyname);
						$datahead2[$cnt] = array("#","Order No","Order Date","Party Order Ref. No.","Crop","Variety","UPS","Qty","NoP","Total Qty","Pending Qty"); 
						...
						while($row_arr_home=mysqli_fetch_array($sql_arr_home))
						{
							...
							while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
							{
								...
								if($crop!="")
								{
									$crop=$crop.", ".$row_tbl_sub['order_sub_crop'];
									// $row_tbl_sub['lotcrop'];
								}
								else
								{
								...
								if($variety!="")
								{
									$variety=$variety.", ".$row_tbl_sub['order_sub_variety'];
								}
								else
								{
								...
								while($row_sloc=mysqli_fetch_array($sql_sloc))
								{
								 ...
								 $porefno=$row_arr_home['orderm_partyrefno'];	 
									...
									{	 
										...
										if($tot_arr_home > 0)
										{ 
											$data1[$cnt][$d]=array($d,$loc1,$trdate,$porefno,$cro,$variet,$up,$qt,$sstatus,$totqty,$penqty); 
											if($reptyp1=="hold")
											{	
						...
						$data2[$cnt]=array("","","","","","","","","Total",$totorqty,$totpenqty); 
						$cnt++;
					}
				}
			}
