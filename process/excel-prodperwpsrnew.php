...
$qry_m="select distinct pper from tblarrival_sub where sstage='Raw'  ";
if($txtprodper!="ALL")
{	
	$qry_m.=" and pper='".$txtprodper."' ";
}
	$qry_m.=" order by pper";
	
	$sql_arr_m=mysqli_query($link,$qry_m) or die(mysqli_error($link));
	$tot_arr_m=mysqli_num_rows($sql_arr_m);
	//exit;
while($row_arr_m=mysqli_fetch_array($sql_arr_m))
{

$txtprodper=$row_arr_m['pper'];

	$datahead2[$cnt] = array("Production Personnel",$txtprodper,"Crop",$crp,"Variety",$ver);
...
$datahead3[$cnt] = array("Date of Arrival","Days","FRN No.","Lot No.","SP Code Female","SP Code Male","Prod. Location","Organizer","Farmer","PDN Qty","Arrival Qty","Raw Seed Qty","Condition Seed Qty","Cond. Loss (RM + IM)","Date of Processing","GOT Result","DOGR","Physical Purity","Germination %","Genetic Purity %"); 
