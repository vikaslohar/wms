...
{	
	...
	window.location='getuser_pro_lotno_utility.php?crop='+val3+'&variety='+val4+'&sqq='+txtlot1;
	return true;	
	}
}	
...
$arrlots="";
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_proslipsub where plantcode='$plantcode'") or die(mysqli_error($link));
		...
		if($tot_arrsub > 0)
		{
			while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
			{
				if($arrlots!="")
				{
					$arrlots=$arrlots.",".$row_tbl_sub['proslipsub_orlot'];
				}
				else
				{
					  $arrlots=$row_tbl_sub['proslipsub_orlot'];
				}
			}
		}

$qr="select distinct orlot from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_balqty > 0 $sss  and plantcode='$plantcode' order by lotldg_id desc";
...
$sql_issueg1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where orlot='".$row['orlot']."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
if($totnog > 0)
{
$row_issueg22=mysqli_fetch_array($sql_issuetblg); 				
				
$flg=0; $flg1=0;
if($row_issueg22['lotldg_sstatus']!="")
{
	$ss=explode("/", $row_issueg22['lotldg_sstatus']);
	$z=count($ss);
	for($i=0; $i<$z; $i++)
	{
	if($ss[$i]=="R") $flg++;
...
$zz=explode(" ", $row_issueg22['lotldg_got1']);
...
if($zz[0]=="GOT-NR")
{
	...
	if(($row_issueg22['lotldg_qc']=="UT" || $row_issueg22['lotldg_qc']=="RT") && $row_issueg22['lotldg_srflg']==0)
	{
		$flg++; 
	}
	if($row_issueg22['lotldg_got']=="Fail" || $row_issueg22['lotldg_qc']=="Fail")
	{
		$flg1++; 
	}
}
...
{
	if(($row_issueg22['lotldg_got']=="UT" || $row_issueg22['lotldg_got']=="RT") && $row_issueg22['lotldg_srflg']==0)
	{
		$flg++; 
	}
	if($row_issueg22['lotldg_got']=="OK" && ($row_issueg22['lotldg_qc']=="UT" || $row_issueg22['lotldg_qc']=="RT") && $row_issueg22['lotldg_srflg']==0)
	{
		$flg++; 
	}
	if($row_issueg22['lotldg_got']=="Fail" || $row_issueg22['lotldg_qc']=="Fail")
...
<td align="right"  valign="middle" class="tbltext"><input type="radio" name="loc" value="<?php echo $row_issueg22['orlot'];?>" onclick="clk(this.value);" />&nbsp;</td>
<td align="left"  valign="middle" class="tbltext" >&nbsp;<?php echo $row_issueg22['orlot'];?></td>
</tr>
<?php
}
else
