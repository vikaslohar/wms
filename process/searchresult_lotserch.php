...
<?php
$mainids='';
 $sql_arr_home=mysqli_query($link,"select distinct blendm_id from tbl_blendss where blendss_newlot LIKE '$srval%'") or die(mysqli_error($link));
...
if($mainids!='')	
{
$sql_arr_home1=mysqli_query($link,"select * from tbl_blendm where blendm_bflg=1 and plantcode='$plantcode' and blendm_id IN ($mainids) order by blendm_date desc,blendm_code desc") or die(mysqli_error($link));
}
else
{
$sql_arr_home1=mysqli_query($link,"select * from tbl_blendm where blendm_bflg=1 and plantcode='$plantcode' order by blendm_date desc,blendm_code desc") or die(mysqli_error($link));
...
$sql_eindent_sub=mysqli_query($link,"select distinct blendss_newlot from tbl_blendss where blendm_id=$trid and plantcode='$plantcode' and blendss_newlot LIKE '$srval%'") or die(mysqli_error($link));
