...
<title>Processing - Transaction - Sloc details</title>
...
$sql_main=mysqli_query($link,"select lotldg_subbinid, lotldg_variety, lotldg_lotno from tbl_lot_ldg where lotldg_whid='".$wid."' and lotldg_binid='".$bid."' and lotldg_subbinid='".$slid."' and plantcode='$plantcode' group by lotldg_subbinid, lotldg_variety, lotldg_lotno order by lotldg_subbinid") or die(mysqli_error($link));  
...
$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$slid."' and binid='".$bid."' and whid='".$wid."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
?>
<table align="center" border="0" width="800" cellspacing="0" cellpadding="0" bordercolor="#adad11" style="border-collapse:collapse" >
...
$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_whid='".$wid."' and lotldg_binid='".$bid."' and lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_variety='".$vid."' and lotldg_crop='".$crop."'  and lotldg_lotno='".$row_tbl['lotldg_lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));  
...
$sql_item=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row_tbl_sub['lotldg_trid']."' and lotno='".$row_tbl_sub['lotldg_lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_main=mysqli_query($link,"select subbinid, lotldg_variety, lotno, lotldg_crop from tbl_lot_ldg_pack where whid='".$wid."' and binid='".$bid."' and subbinid='".$slid."' and plantcode='$plantcode' group by subbinid, lotldg_variety, lotno order by subbinid") or die(mysqli_error($link));  
...
$slups=0; $slqty=0; $nop1=0;
$wtinmp=$row_tbl_sub['wtinmp'];
$upspacktype=$row_tbl_sub['packtype'];
...
$penqty=(($row_tbl_sub['balqty'])-($wtinmp*$row_tbl_sub['balnomp']));
if($penqty > 0)
{
	$nop1=($ptp*$penqty);
}
