<?php
    ...
    $remarks = $_REQUEST['remarks'];
	 ...
	 $sql_main="update tbl_drying set remarks='$remarks' where trid='".$pid."'";
	...
	{
		$crop=$row_arr['crop'];
		$variety=$row_arr['variety'];
		$lotno=$row_arrsub['lotno'];
		$arrival_date=$row_arr['dryingdate'];
		$drefno=$row_arr['drefno'];
		
		$zzz=implode(",", str_split($row_arrsub['lotno']));
		//print_r($zzz);
		//$lot=$zzz[0].$zzz[2].$zzz[4].$zzz[6].$zzz[8].$zzz[10].$zzz[12].$zzz[14].$zzz[16].$zzz[18].$zzz[20].$zzz[22].$zzz[24]."C";
		$lot=$lotno;
		/*$onob=$row_arrsub['onob'];
		$oqty=$row_arrsub['oqty'];
		$nob1=$row_arrsub['nob1'];
		$qty1=$row_arrsub['qty1'];
		$adnob=$row_arrsub['adnob'];
		$nob=$onob-$nob1;*/
		$cnnt=0;
		
		$sql_arrsubsub=mysqli_query($link,"select * from tbl_dryingsubsub where trid='".$pid."' and subtrid='".$row_arrsub['subtrid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
		$a_sub=mysqli_num_rows($sql_arrsubsub);
		while($row_arrsubsub=mysqli_fetch_array($sql_arrsubsub))
		{
		
		$onob=$row_arrsubsub['onob'];
		$oqty=$row_arrsubsub['oqty'];
		$nob1=$row_arrsubsub['nnob'];
		$qty1=$row_arrsubsub['nqty'];
		$adnob=$row_arrsubsub['dryingloss'];
		$nob=$onob-$nob1;
		
		/*if($row_arrsubsub['samebin']=="Yes")
		{
		
		$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_arrsubsub['osubbin']."' and lotldg_binid='".$row_arrsubsub['obin']."' and lotldg_whid='".$row_arrsubsub['owh']."' and lotldg_variety='".$variety."' and lotldg_lotno='".$lotno."' order by lotldg_balqty desc") or die(mysqli_error($link));
		...
		{*/
		$otrid=0;
		$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_arrsubsub['osubbin']."' and lotldg_binid='".$row_arrsubsub['obin']."' and lotldg_whid='".$row_arrsubsub['owh']."' and lotldg_variety='".$variety."' and lotldg_lotno='".$lotno."' and plantcode='$plantcode' order by lotldg_balqty desc") or die(mysqli_error($link));
		$row_issue1=mysqli_fetch_array($sql_issue1); 
		...
		while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
		{
				...
				$genpurity=$row_issuetbl['lotldg_genpurity'];
				...
				$sstatus=$pl;
				 ...
				 $sql_ins_main="insert into tbl_lot_ldg (yearcode,lotldg_trtype, lotldg_trid, lotldg_trdate, lotldg_lotno, lotldg_crop, lotldg_variety, lotldg_whid, lotldg_binid, lotldg_subbinid, lotldg_opbags, lotldg_opqty, lotldg_trbags, lotldg_trqty, lotldg_balbags, lotldg_balqty, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_got, lotldg_qctestdate, lotldg_gottestdate, orlot, lotldg_gs, lotldg_resverstatus, lotldg_revcomment, lotldg_srtyp, lotldg_srflg, lotldg_genpurity, lotldg_mergerflg, lotldg_unlistflg, plantcode) values('$yearid_id','DRYSUO', '$pid', '$arrival_date', '$lotno', '$crop', '$variety', '$whid', '$binid', '$subbinid', '$opups', '$opqty', '$opups', '$opqty', '$balups', '$balqty', '$sstage', '$sstatus', '$moist', '$gemp', '$vchk', '$got1', '$qc', '$gotstatus', '$qctestdate', '$gottestdate', '$orlot', '$gs', '$resverstatus', '$revcomment', '$srtyp', '$srflg', '$genpurity', '$mergerflg', '$unlistflg', '$plantcode')";
				//exit;
				mysqli_query($link,$sql_ins_main) or die(mysqli_error($link));
		...
		$sql_issue122=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_arrsubsub['nsubbin']."' and lotldg_binid='".$row_arrsubsub['nbin']."' and lotldg_whid='".$row_arrsubsub['nwh']."' and lotldg_variety='".$variety."' and lotldg_lotno='".$lotno."' and plantcode='$plantcode' order by lotldg_id desc") or die(mysqli_error($link));
		$row_issue122=mysqli_fetch_array($sql_issue122); 
		$sql_issuetbl22=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue122[0]."'and lotldg_balqty>0 and plantcode='$plantcode'") or die(mysqli_error($link)); 
		...
		while($row_issuetbl22=mysqli_fetch_array($sql_issuetbl22))
		{
				if($tttttt22 == 0)
				{
					$whid=$row_arrsubsub['nwh'];
					$binid=$row_arrsubsub['nbin'];
					$subbinid=$row_arrsubsub['nsubbin'];
					$opups=$row_arrsubsub['onob'];
					$opqty=$row_arrsubsub['oqty'];
				}
				else
				{
					$whid=$row_issuetbl22['lotldg_whid'];
					$binid=$row_issuetbl22['lotldg_binid'];
					$subbinid=$row_issuetbl22['lotldg_subbinid'];
					$opups=$row_issuetbl22['lotldg_balbags'];
					$opqty=$row_issuetbl22['lotldg_balqty'];
				}
				...
				$sstage=$row_issuetbl22['lotldg_sstage'];
				$sstatus=$row_issuetbl22['lotldg_sstatus'];
				$moist=$row_issuetbl22['lotldg_moisture'];
				$gemp=$row_issuetbl22['lotldg_gemp'];
				$vchk=$row_issuetbl22['lotldg_vchk'];
				$got1=$row_issuetbl22['lotldg_got1'];
				$qc=$row_issuetbl22['lotldg_qc'];
				
				$gotstatus=$row_issuetbl22['lotldg_got'];
				$qctestdate=$row_issuetbl22['lotldg_qctestdate'];
				$gottestdate=$row_issuetbl22['lotldg_gottestdate'];
				$orlot=$row_issuetbl22['orlot'];
				$gs=$row_issuetbl22['lotldg_gs'];
				$resverstatus=$row_issuetbl22['lotldg_resverstatus'];
				$revcomment=$row_issuetbl22['lotldg_revcomment'];
				$srtyp=$row_issuetbl22['lotldg_srtyp'];
				$srflg=$row_issuetbl22['lotldg_srflg'];
				$genpurity=$row_issuetbl22['lotldg_genpurity'];
				$mergerflg=$row_issuetbl22['lotldg_mergerflg'];
				$unlistflg=$row_issuetbl22['lotldg_unlistflg'];
				...
				$sstatus=$pl;
				  ...
				  $sql_ins_main22="insert into tbl_lot_ldg (yearcode,lotldg_trtype, lotldg_trid, lotldg_trdate, lotldg_lotno, lotldg_crop, lotldg_variety, lotldg_whid, lotldg_binid, lotldg_subbinid, lotldg_opbags, lotldg_opqty, lotldg_trbags, lotldg_trqty, lotldg_balbags, lotldg_balqty, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_got, lotldg_qctestdate, lotldg_gottestdate, orlot, lotldg_gs, lotldg_resverstatus, lotldg_revcomment, lotldg_srtyp, lotldg_srflg, lotldg_genpurity, lotldg_mergerflg, lotldg_unlistflg, plantcode) values('$yearid_id','DRYSUC', '$pid', '$arrival_date', '$lotno', '$crop', '$variety', '$whid', '$binid', '$subbinid', '$opups', '$opqty', '$nob', '$adnob', '$balups', '$balqty', '$sstage', '$sstatus', '$moist', '$gemp', '$vchk', '$got1', '$qc', '$gotstatus', '$qctestdate', '$gottestdate', '$orlot', '$gs', '$resverstatus', '$revcomment', '$srtyp', '$srflg', '$genpurity', '$mergerflg', '$unlistflg', '$plantcode')";
				//exit;
				mysqli_query($link,$sql_ins_main22) or die(mysqli_error($link));
				
				$sql_itmg22="update tbl_subbin set status='$sstage' where sid='$subbinid'";
		...
		$sql_code1="SELECT MAX(ar_code) FROM tbl_drying where plantcode='$plantcode' ORDER BY ar_code DESC";
	$res_code1=mysqli_query($link,$sql_code1)or die(mysqli_error($link));
	 ...
	 $sql_main="update tbl_drying set dflg=1, arrflg=1, ar_code=$ncode1  where trid ='$pid'";
...
<title>Processing -Transaction - Drying Slip - Preview</title>
