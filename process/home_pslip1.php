...
$sql_tbl=mysqli_query($link,"select * from tbl_proslipmain where (logid='".$logid."' or logid='') and proslipmain_tflag=0 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['proslipmain_id'];	
	
	$s_sub="delete from tbl_proslipsub where proslipmain_id='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_proslipsubsub where proslipmain_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
	$s_sub_sub2="delete from tbl_proslipsubsub2 where proslipmain_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub2) or die(mysqli_error($link));
}

	$s_sub="delete from tbl_proslipmain where (logid='".$logid."' or logid='') and proslipmain_tflag=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_pslip1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		 {?>
		 <script>
...
<title>Processing - Transaction - Processing Slip - Home</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" style="border-bottom:solid; border-bottom-color:#adad11" >
	    <tr >
	      <td width="820" height="25" class="Mainheading">&nbsp;Transaction - Processing slip</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_process_ slip.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onClick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
</table></td>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_proslipmain where proslipmain_date>='$tdate' and proslipmain_date<='$pdate' and proslipmain_tflag=1 and plantcode='$plantcode' order by proslipmain_tcode desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_proslipmain where proslipmain_date>='$tdate' and proslipmain_date<='$pdate' and proslipmain_tflag=1 and plantcode='$plantcode' order by proslipmain_tcode desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_proslipmain where proslipmain_date>='$tdate' and proslipmain_date<='$pdate' and proslipmain_tflag=1 order by proslipmain_tcode desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
//$tot_arr_home=0;

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_proslipmain where proslipmain_date>='$tdate' and proslipmain_date<='$pdate' and proslipmain_tflag=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Processing slip</td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="970" bordercolor="#adad11" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="76" rowspan="2" align="center" valign="middle" class="smalltblheading">Trans Id</td>
...
{
	$trdate=$row_arr_home['proslipmain_date'];
	...
	$arrival_id=$row_arr_home['proslipmain_id'];
	
	$quer3=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropid='".$row_arr_home['proslipmain_crop']."' order by cropname Asc");
	$noticia = mysqli_fetch_array($quer3);
	$crop=$noticia['cropname'];
	
	$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$row_arr_home['proslipmain_variety']."'  order by popularname Asc"); 
...
$aq=explode(".",$row_tbl_sub['proslipsub_pnob']);
if($aq[1]==000){$onob=$aq[0];}else{$onob=$row_tbl_sub['proslipsub_pnob'];}

$an=explode(".",$row_tbl_sub['proslipsub_pqty']);
if($an[1]==000){$oqty=$an[0];}else{$oqty=$row_tbl_sub['proslipsub_pqty'];}

$aq2=explode(".",$row_tbl_sub['proslipsub_connob']);
if($aq2[1]==000){$cnob=$aq2[0];}else{$cnob=$row_tbl_sub['proslipsub_connob'];}

$an2=explode(".",$row_tbl_sub['proslipsub_conqty']);
if($an2[1]==000){$cqty=$an2[0];}else{$cqty=$row_tbl_sub['proslipsub_conqty'];}

$aq3=explode(".",$row_tbl_sub['proslipsub_rm']);
if($aq3[1]==000){$rmqty=$aq3[0];}else{$rmqty=$row_tbl_sub['proslipsub_rm'];}

$an3=explode(".",$row_tbl_sub['proslipsub_im']);
if($an3[1]==000){$imqty=$an3[0];}else{$imqty=$row_tbl_sub['proslipsub_im'];}

if($row_tbl_sub['proslipsub_pl']>0)
{
$an4=explode(".",$row_tbl_sub['proslipsub_pl']);
if($an4[1]==000){$plqty=$an4[0];}else{$plqty=$row_tbl_sub['proslipsub_pl'];}
}
else
{
$an4=explode(".",$row_tbl_sub['proslipsub_rpl']);
if($an4[1]==000){$plqty=$an4[0];}else{$plqty=$row_tbl_sub['proslipsub_rpl'];}
}

$an5=explode(".",$row_tbl_sub['proslipsub_tlqty']);
if($an5[1]==000){$tplqty=$an5[0];}else{$tplqty=$row_tbl_sub['proslipsub_tlqty'];}
		
		if($lotno!="")
		{
		$lotno=$lotno."<br>".$row_tbl_sub['proslipsub_lotno'];
		}
		else
		{
		$lotno=$row_tbl_sub['proslipsub_lotno'];
		}
		if($bags1!="")
		{
		$bags1=$bags1."<br>".$onob;
		...
		if($tplper!="")
		{
		$tplper=$tplper."<br>".$row_tbl_sub['proslipsub_tlper'];
		}
		else
		{
		$tplper=$row_tbl_sub['proslipsub_tlper'];
		}
...
<tr class="Light">
         ...
         <td align="center" valign="middle" class="smalltbltext"><a href="add_proslip_view.php?p_id=<?php echo $arrival_id;?>"><?php echo "PS".$row_arr_home['proslipmain_tcode']."/".$yearid_id."/".$lrole;?></a></td>
