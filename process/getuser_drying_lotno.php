...
<title>Processing - Transaction - Existing Lot selection</title>
...
$arrlots="";
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_dryingsub where trid='".$tid."' and sstage='".$stage."' and plantcode='$plantcode'") or die(mysqli_error($link));
		...
		$sql_tbl_main=mysqli_query($link,"select * from tbl_drying where dflg='0' and logid!='$logid' and plantcode='$plantcode'") or die(mysqli_error($link));
		$tot_arrmain=mysqli_num_rows($sql_tbl_main);
		if($tot_arrmain > 0)
		{
			while($row_tbl_main=mysqli_fetch_array($sql_tbl_main))
			{
				$sql_tbl_sub=mysqli_query($link,"select * from tbl_dryingsub where trid='".$row_tbl_main['trid']."' and sstage='".$stage."' and plantcode='$plantcode'") or die(mysqli_error($link));
				$tot_arrsub=mysqli_num_rows($sql_tbl_sub);
				if($tot_arrsub > 0)
				{
					while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
...
$aaaass="select distinct(lotldg_lotno)  from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$stage."' and lotldg_mergerflg=0 $sss and plantcode='$plantcode'";			
