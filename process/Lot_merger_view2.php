...
<title>Processing - Transction - Lot Blending - View</title>
...
$itemqry2=mysqli_query($link,"select varietyid, popularname from tblvariety where varietyid='".$row2['blendm_variety']."'") or die(mysqli_error($link));
...
$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$row2['blendm_crop']."' and lotldg_lotno='".$ltno."' and lotldg_variety='".$row2['blendm_variety']."' and lotldg_trtype='Blending' and plantcode='$plantcode' group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
$t=mysqli_num_rows($sql_is);		
if($t>1)
$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$row2['blendm_crop']."' and lotldg_lotno='".$ltno."' and lotldg_variety='".$row2['blendm_variety']."' and lotldg_trtype='Blending' and plantcode='$plantcode' group by lotldg_subbinid order by lotldg_id desc Limit 0,1") or die(mysqli_error($link));

while($row_is=mysqli_fetch_array($sql_is))
{ 
	$slups=0; $slqty=0; $wareh=""; $binn=""; $subbinn="";
	$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$row2['blendm_crop']."' and lotldg_lotno='".$ltno."' and lotldg_variety='".$row2['blendm_variety']."' and lotldg_trtype='Blending' and plantcode='$plantcode' order by lotldg_id desc ") or die(mysqli_error($link));
