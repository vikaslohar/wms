<?php
	...
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		...
		$lt=$zzz[28].$zzz[30];
		$lotcount=0;
		$sql_arrsub24=mysqli_query($link,"select pnpslipsub_orlot from tbl_pnpslipsub where pnpslipsub_orlot='".$lotchk."' and pnpslipsub_packtype='P' and plantcode='$plantcode' order by pnpslipsub_orlot") or die(mysqli_error($link));
		while($row_arrsub24=mysqli_fetch_array($sql_arrsub24))
		{
			$lotcount++;
		}
		$sql_arrsub24=mysqli_query($link,"select pnpslipsub_orlot from tbl_pnpslipsub where pnpslipsub_orlot='".$lotchk."' and pnpslipsub_packtype='E' and plantcode='$plantcode' order by pnpslipsub_orlot") or die(mysqli_error($link));
		while($row_arrsub24=mysqli_fetch_array($sql_arrsub24))
		{
			$lotcount++;
		}
		$lnt=sprintf("%02d",$lt+$lotcount);
		$lnt1=sprintf("%02d",$lnt+1);
		if($protyp="P")
		$lot=$zzz[0].$zzz[2].$zzz[4].$zzz[6].$zzz[8].$zzz[10].$zzz[12].$zzz[14].$zzz[16].$zzz[18].$zzz[20].$zzz[22].$zzz[24].$zzz[26].$lnt;
		else
		$lot=$zzz[0].$zzz[2].$zzz[4].$zzz[6].$zzz[8].$zzz[10].$zzz[12].$zzz[14].$zzz[16].$zzz[18].$zzz[20].$zzz[22].$zzz[24].$zzz[26].$lt;
		$oldlot=$lot;
		$orlot2=$lot;
		if($lotstage=="Raw")
		$lot=$lot."C";
		...
		$a="";
		$sql_barcode2=mysqli_query($link,"Select * from tbl_barcodestmp where bar_lotno='".$lotno."' and bar_psrn='".$drefno."' and plantcode='$plantcode'") or die(mysqli_error($link));
		...
		if($tot_barcode2 > 0)
		{
			while($row_barcode2=mysqli_fetch_array($sql_barcode2))
			{
				...
				$sql_barcode="Insert into tbl_barcodes (bar_trid, bar_trtype, bar_subtrid, bar_lotno, bar_barcode, bar_grosswt, bar_wtdate, bar_wttime, bar_poprid, logid, yearid, plantcode) values('$pid', 'PNPSLIP', '$subtranid', '$lotno', '$barcode', '$grosswt', '$dt', '$tim', '$poprid', '$logid', '$yearid_id', '$plantcode')";
				//echo "<br/>";
		...
		if($tot_isstblman==0)
		{
			$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_variety='".$variety."' and lotldg_lotno='".$lotno."' and plantcode='$plantcode' order by lotldg_id desc") or die(mysqli_error($link));
		...
		while($row_arrsubsub2=mysqli_fetch_array($sql_arrsubsub2))
		{
			...
			while($row_issuetbl22=mysqli_fetch_array($sql_issuetbl22))
			{
				...
				$srflg2=$row_issuetbl22['lotldg_srflg'];
				  ...
				  {$srtype=""; $srflg=0;}
				  $sql_ins_main22="insert into tbl_lot_ldg (yearcode,lotldg_trtype, lotldg_trid, lotldg_trdate, lotldg_lotno, lotldg_crop, lotldg_variety, lotldg_whid, lotldg_binid, lotldg_subbinid, lotldg_opbags, lotldg_opqty, lotldg_trbags, lotldg_trqty, lotldg_balbags, lotldg_balqty, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_got, lotldg_qctestdate, lotldg_gottestdate, orlot, lotldg_gs, lotldg_resverstatus, lotldg_revcomment, lotldg_genpurity, lotldg_srtyp, lotldg_srflg, plantcode) values('$yearcode2','PNPSLIPSUC', '$pid', '$arrival_date', '$lot', '$crop', '$variety', '$whid2', '$binid2', '$subbinid2', '$onob2', '$oqty2', '$nob12', '$qty12', '$balups2', '$balqty2', '$sstage2', '$sstatus2', '$moist2', '$gemp2', '$vchk2', '$got12', '$qc2', '$gotstatus2', '$qctestdate2', '$gottestdate2', '$orlot2', '$gs2', '$resverstatus2', '$revcomment2', '$geneticpurity2', '$srtype', '$srflg', '$plantcode')";
				//exit;
				...
				if($srflg>0)
				{
					if($protyp=="P")
					{
						...
						$sql_srmain="Insert into tbl_softr (softr_tcode, softr_code, softr_date, softr_crop, softr_variety, softr_typ, softr_wh, softr_bin, softr_subbin, yearcode, softr_tflg, plantcode) values('$code1', '$code', '$arrival_date', '$crop', '$variety', 'sllot', '$whid2', '$binid2', '$subbinid2', '$yearid_id', '1', '$plantcode')";
				...
				else*/
				{
					...
					{
						$sql_sub_sub13="insert into tbl_qctest(pp, moist, got, lotno, srdate, crop,  variety, sampleno, trstage, qc, state, gs, oldlot, spdate, dosdate, bflg, testdate, qcflg, qcstatus,gotsmpdflg,yearid,logid)values('$vchk2', '$moist2', '$got1', '$lot', '".$row_qc['srdate']."', '$crop', '$variety', '$ncode2', '$sstage2', '$qc2', '$state2',1,'$orlot2', '".$row_qc['spdate']."', '".$row_qc['dosdate']."',1,'".$row_qc['testdate']."',1, '$qc2',1,'$yrco','$logid')";
		...
		$lot2=$zzz[0].$zzz[2].$zzz[4].$zzz[6].$zzz[8].$zzz[10].$zzz[12].$zzz[14].$zzz[16].$zzz[18].$zzz[20].$zzz[22].$zzz[24].$zzz[26].$lnt;
		else
		$lot2=$zzz[0].$zzz[2].$zzz[4].$zzz[6].$zzz[8].$zzz[10].$zzz[12].$zzz[14].$zzz[16].$zzz[18].$zzz[20].$zzz[22].$zzz[24].$zzz[26].$lt;
		$orlot3=$lot2;
		$lot2=$lot2."P";
		$ycr=$zzz[2];
		...
		while($row_arrsubsub3=mysqli_fetch_array($sql_arrsubsub3))
		{
			...
			if($srflg>0)
			{
				//if($row_arrsub['pnpslipsub_packtype']=="P")
				{
					...
					$sql_srmain="Insert into tbl_softr (softr_tcode, softr_code, softr_date, softr_crop, softr_variety, softr_typ, softr_wh, softr_bin, softr_subbin, yearcode, softr_tflg, plantcode) values('$code1', '$code', '$arrival_date', '$crop', '$variety', 'sllot', '$whid3', '$binid3', '$subbinid3', '$yearid_id', '1', '$plantcode')";
					if(mysqli_query($link,$sql_srmain) or die(mysqli_error($link)))
					{
						$id=mysqli_insert_id($link);
				...
				else*/
				{
					...
					if($yrco=="")$yrco=$ycr;
						...
						if($row_arrsub['qc']=="UT")
						{
							...
							/*if($qc2=="UT" && $got1!="UT")
							{
							$sql_sub_sub12="insert into tbl_qctest(pp, moist, got, lotno, srdate, crop,  variety, sampleno, trstage, qc, state, gs, oldlot, spdate, dosdate, bflg, gotdate, gotflg, gotstatus,yearid,logid)values('$vchk2', '$moist2', '$got1', '$lot2', '".$row_qc['srdate']."', '$crop', '$variety', '$ncode1', '$sstage2', '$qc2', '$state',1,'$orlot3', '".$row_qc['spdate']."', '".$row_qc['dosdate']."',1,'".$row_qc['gotdate']."',1, '$got1','$yrco','$logid')";
							}
							else if($qc2!="UT" && $got1=="UT")
							{
							$sql_sub_sub12="insert into tbl_qctest(pp, moist, got, lotno, srdate, crop,  variety, sampleno, trstage, qc, state, gs, oldlot, spdate, dosdate, bflg, testdate, qcflg, qcstatus,gotsmpdflg,yearid,logid)values('$vchk2', '$moist2', '$got1', '$lot2', '".$row_qc['srdate']."', '$crop', '$variety', '$ncode1', '$sstage2', '$qc2', '$state',1,'$orlot3', '".$row_qc['spdate']."', '".$row_qc['dosdate']."',1,'".$row_qc['testdate']."',1, '$qc2',1,'$yrco','$logid')";
							}	
							else if($qc2=="UT" || $got1=="UT")
							{
							$sql_sub_sub12="insert into tbl_qctest(pp, moist, got, lotno, srdate, crop,  variety, sampleno, trstage, qc, state, gs, oldlot, spdate, dosdate, bflg,gotsmpdflg,yearid,logid)values('$vchk2', '$moist2', '$got1', '$lot2', '".$row_qc['srdate']."', '$crop', '$variety', '$ncode1', '$sstage2', '$qc2', '$state',1,'$orlot3', '".$row_qc['spdate']."', '".$row_qc['dosdate']."',1,1,'$yrco','$logid')";
							}	
							else
							{
							$sql_sub_sub12="insert into tbl_qctest(pp, moist, got, lotno, srdate, crop,  variety, sampleno, trstage, qc, state, gs, oldlot, spdate, dosdate, bflg, testdate, qcflg, gotdate, gotflg, qcstatus,gotsmpdflg, gotstatus,yearid,logid)values('$vchk2', '$moist2', '$got1', '$lot2', '".$row_qc['srdate']."', '$crop', '$variety', '$ncode1', '$sstage2', '$qc2', '$state',1,'$orlot3', '".$row_qc['spdate']."', '".$row_qc['dosdate']."',1,'".$row_qc['testdate']."',1,'".$row_qc['gotdate']."',1, '$qc2',1, '$got1','$yrco','$logid')";
							}	
							mysqli_query($link,$sql_sub_sub12) or die(mysqli_error($link));	*/
				//}
		...
		$sql_code1="SELECT MAX(pnpslipmain_tcode) FROM tbl_pnpslipmain where pnpslipmain_ttype='Packing Slip' and plantcode='$plantcode' ORDER BY pnpslipmain_tcode DESC";
	$res_code1=mysqli_query($link,$sql_code1)or die(mysqli_error($link));
...
<title>Processing -Transaction - Packing Slip- Preview</title>
