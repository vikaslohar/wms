...
function post_value()
{
	...
	if(f==0)
	{
		...
		opener.document.frmaddDept.txtnlottom.value=j;
		opener.getdetails();
		self.close();
	}
	else
...
if($stage=="Raw")
{ 
	$qr="select distinct(lotldg_lotno) from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$stage."' and lotldg_balqty > 0 and lotldg_mergerflg=0 $sss and plantcode='$plantcode'";
}
else if($stage=="Condition")
{ 
	$qr="select distinct(lotldg_lotno) from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$stage."' and lotldg_balqty > 0 and lotldg_mergerflg=0 $sss and plantcode='$plantcode'";
...
{ 
	$qr="select distinct(lotldg_lotno) from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_balqty > 0 and lotldg_mergerflg=0 $sss and plantcode='$plantcode'";
}
...
//print_r($zz);
/*$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_lotno='".$row['lotldg_lotno']."'  and lotldg_balqty > 0 group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
...
*/
//echo "select max(lotldg_id) from tbl_lot_ldg where lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_mergerflg=0 order by lotldg_id desc ";
$balaceqty=0;
//echo "select distinct lotldg_subbinid from tbl_lot_ldg where lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_balqty>0 and lotldg_mergerflg=0 order by lotldg_id desc ";
$sql_subbin=mysqli_query($link,"select distinct lotldg_subbinid from tbl_lot_ldg where lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_balqty>0 and lotldg_mergerflg=0 and plantcode='$plantcode' order by lotldg_id desc ") or die(mysqli_error($link));
while($row_subbin=mysqli_fetch_array($sql_subbin))
{
//echo "select max(lotldg_id) from tbl_lot_ldg where lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_subbinid='".$row_subbin['lotldg_subbinid']."' and lotldg_mergerflg=0 order by lotldg_id desc ";
 $sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_subbinid='".$row_subbin['lotldg_subbinid']."' and lotldg_mergerflg=0 and plantcode='$plantcode' order by lotldg_id desc ") or die(mysqli_error($link));
   $row_is1=mysqli_fetch_array($sql_is1);
   $sql_istb12=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_balqty >= 0 and lotldg_mergerflg=0 and plantcode='$plantcode' order by lotldg_id desc") or die(mysqli_error($link)); 	
   $row_istb12=mysqli_fetch_array($sql_istb12);
   $balaceqty=$balaceqty+$row_istb12['lotldg_balqty'];
}
if($balaceqty==0)
{
	$vflg++;
...
/*else
{
	 $sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_lotno='".$row['lotldg_lotno']."' and lotldg_subbinid='".$row_subbin['lotldg_subbinid']."' and lotldg_mergerflg=0 order by lotldg_id desc ") or die(mysqli_error($link));
   $row_is1=mysqli_fetch_array($sql_is1);
   $sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_balqty > 0 and lotldg_mergerflg=0 order by lotldg_id desc") or die(mysqli_error($link)); 
  $t=mysqli_num_rows($sql_istbl);
}*/
//echo $row['lotldg_lotno']." - ".$vflg." - ".$balaceqty."<br>";
//echo "select * from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_balqty > 0 and lotldg_mergerflg=0 order by lotldg_id desc";
  $sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_mergerflg=0 and plantcode='$plantcode' order by lotldg_id desc") or die(mysqli_error($link)); 
  $t=mysqli_num_rows($sql_istbl);
 // $balaceqty=0;
if($vflg==0 || $balaceqty > 0)
{
...
<tr class="light" height="20">
  <td colspan="4" align="left" class="tblheading">&nbsp;No Lots available to Blending. Reasons:</td>
</tr>
<tr class="light" height="20">
