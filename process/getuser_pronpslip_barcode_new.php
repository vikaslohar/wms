			...
			{ 
				...
				if($row>=2)
				{
					...
					{ 
						...
						$sql_barcode2=mysqli_query($link,"Select bar_barcodes from tbl_barcodestmp where bar_barcodes='".$data[1]."' and plantcode='$plantcode'") or die(mysqli_error($link));
						if($tot_barcode2=mysqli_num_rows($sql_barcode2) > 0)$flg++;
						$sql_barcode3=mysqli_query($link,"Select bar_barcode from tbl_barcodes where bar_barcode='".$data[1]."' and plantcode='$plantcode'") or die(mysqli_error($link));
						if($tot_barcode3=mysqli_num_rows($sql_barcode3) > 0)$flg1++;	
						$sql_barcode4=mysqli_query($link,"Select mpmain_barcode from tbl_mpmain where mpmain_barcode='".$data[1]."' and plantcode='$plantcode'") or die(mysqli_error($link));
