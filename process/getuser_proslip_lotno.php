...
?>
 <select class="tbltext" name="pcode" style="width:40px;">
  ...
  isNumberKey(event)"  onchange="lot2chk();"  /> <font size="+1"><b>/</b></font> <input name="stcode" type="text" size="4" class="tbltext" tabindex="0" maxlength="5" onkeypress="return isNumberKey(event)"  value="00000" onchange="slocshow();" /> <font size="+1"><b>/</b></font> <input name="stcode2" type="text" size="2" class="tbltext" tabindex="0" maxlength="2" onkeypress="return isNumberKey(event)"  value="00" />
	   <?php if($stage=="Raw"){?><b>R</b><?php } else if($stage=="Condition"){?><b>C</b><?php } else{?> <?php }?>&nbsp;<font color="#FF0000">*</font>&nbsp;&nbsp;<img src="../images/search.gif" border="0"  class="tbltext" name="sub" value="Search" style="display:inline;cursor:pointer; vertical-align:middle" onclick="lotchk();" /><!--<input type="image" src="../images/search.gif" border="0"  class="tbltext" name="sub" value="Search" style="display:inline;cursor:pointer; vertical-align:middle" onclick="lotchk();"> -->
    </form>
 </td></tr></table>
</td>
</tr>
...
$arrlots="";
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_proslipsub where proslipmain_id='".$tid."' and plantcode='$plantcode'") or die(mysqli_error($link));
		...
		$sql_tbl_main=mysqli_query($link,"select * from tbl_proslipmain where proslipmain_tflag='0' and logid!='$logid' and plantcode='$plantcode'") or die(mysqli_error($link));
		$tot_arrmain=mysqli_num_rows($sql_tbl_main);
		if($tot_arrmain > 0)
		{
			while($row_tbl_main=mysqli_fetch_array($sql_tbl_main))
			{
				$sql_tbl_sub=mysqli_query($link,"select * from tbl_proslipsub where proslipmain_id='".$row_tbl_main['proslipmain_id']."' and plantcode='$plantcode'") or die(mysqli_error($link));
				$tot_arrsub=mysqli_num_rows($sql_tbl_sub);
				if($tot_arrsub > 0)
				{
					while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
...
$qr="select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$stage."' $sss and plantcode='$plantcode' order by lotldg_id desc";
...
if($qty > 0)
{
$sql_issueg1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_lotno='".$row['lotldg_lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_issueg1=mysqli_fetch_array($sql_issueg1); 
				
$sql_issuetblg=mysqli_query($link,"select lotldg_lotno,lotldg_sstatus,lotldg_got1,lotldg_got,lotldg_qc,lotldg_srflg from tbl_lot_ldg where lotldg_id='".$row_issueg1[0]."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
$totnog=mysqli_num_rows($sql_issuetblg);
if($totnog > 0)
{
$row_24=mysqli_fetch_array($sql_issuetblg);
...
if($zz[0]=="GOT-NR")
{
	if(($row_24['lotldg_qc']=="UT" || $row_24['lotldg_qc']=="RT") && $row_24['lotldg_srflg']==0)
		$flg++; 
	if($row_24['lotldg_srflg']!=0 && $row_24['lotldg_srtyp']!="")
		$flg++; 
}
else
{
