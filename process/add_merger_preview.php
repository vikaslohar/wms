...
//	exit;
		...
		{
			$sql_arrsub=mysqli_query($link,"select * from tbl_blends where blendm_id='".$pid."' and plantcode='$plantcode'") or die(mysqli_error($link));
			while($row_arrsub=mysqli_fetch_array($sql_arrsub))
			{
				$lotno=$row_arrsub['blends_lotno'];
				
				$sqlm="update tbl_lot_ldg set lotldg_mergerflg=1 where lotldg_lotno='".$lotno."'";
				$xcs=mysqli_query($link,$sqlm) or die(mysqli_error($link));
			}
			
			$sql_code="SELECT MAX(blendm_code) FROM tbl_blendm  where blendm_yearid='$yearid_id' and plantcode='$plantcode' ORDER BY blendm_code DESC";
			...
			$sql_main="update tbl_blendm set blendm_tflg=1, blendm_code=$code where blendm_id='$pid'";
...
<title>Processing - Transction - Lot Blending - Preview</title>
