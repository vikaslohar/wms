<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		else if($beseltyp=="barbased")
		{
		
				echo "<script>window.location='getuser_pronpslip_barcode_cb.php?totnomp=$totnomp&tid=$tid&lotno=$lotno&txtpsrn=$txtpsrn&subtid=$subtid&dval=$dval&dobg=$dobg&operatorcode=$operatorcode&wtmaccode=$wtmaccode'</script>";
...
$oldlot2=$zzz[0].$zzz[2].$zzz[4].$zzz[6].$zzz[8].$zzz[10].$zzz[12].$zzz[14].$zzz[16].$zzz[18].$zzz[20].$zzz[22].$zzz[24].$zzz[26]."00";
$slflg=0;
$crpid="";
$sql_crop=mysqli_query($link,"Select cropid from tblcrop where cropname IN ('Bajra Seed','Bhindi','Maize Seed','Paddy Seed')") or die(mysqli_error($link));
while($row_crop=mysqli_fetch_array($sql_crop))
{
	if($crpid!="")
		$crpid=$crpid.",".$row_crop['cropid'];
	else
		$crpid=$row_crop['cropid'];
}
//echo $crpid;
//echo "Select * from tbl_lot_ldg where orlot='$oldlot'";
$crid=explode(",", $crpid);
$sql_lot=mysqli_query($link,"Select * from tbl_lot_ldg where orlot='$oldlot' and plantcode='$plantcode'") or die(mysqli_error($link));
if($tot_lot=mysqli_num_rows($sql_lot)==0)
$sql_lot=mysqli_query($link,"Select * from tbl_lot_ldg where orlot='$oldlot2' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_lot=mysqli_fetch_array($sql_lot);
$ltcrp=$row_lot['lotldg_crop'];
if(!in_array($ltcrp,$crid))$slflg++;
...
?>
<tr class="Light" height="25">
<td width="50%" align="right" height="30" valign="middle" class="tblheading">Barcode Range Wise&nbsp;</td>
<td width="50%" align="left"  valign="middle">&nbsp;<input type="radio" name="besel" value="barbased" onClick="seltp(this.value);" /></td>
</tr>
<?php } ?>
<tr class="light" height="25">
