...
$sql_tbl=mysqli_query($link,"select * from tbl_drying where logid='$logid' and   arrflg=0 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	 $arrival_id=$row_tbl['trid'];	
	
	$s_sub="delete from tbl_dryingsub where trid='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_dryingsubsub where trid='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
}

$s_sub="delete from tbl_drying where logid='$logid' and  arrflg=0";
mysqli_query($link,$s_sub) or die(mysqli_error($link));	
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" style="border-bottom:solid; border-bottom-color:#adad11" >
	    <tr >
	      <td width="820" height="25" class="Mainheading">&nbsp;Transaction - Drying Slip </td>
	    </tr></table></td>
...
$sql_arr_home=mysqli_query($link,"select * from tbl_drying  where arrflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_drying  where dryingdate between '$tdate' and '$pdate' and arrflg=1 and plantcode='$plantcode' order by arr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_drying  where dryingdate between '$tdate' and '$pdate' and arrflg=1 and plantcode='$plantcode' order by arr_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_drying  where dryingdate>='$tdate' and dryingdate<='$pdate' and arrflg=1 order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_drying where dryingdate>='$tdate' and dryingdate<='$pdate'  and arrflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Drying List </td>
</tr>
</table>
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_crop=mysqli_query($link,"select * from tblcrop where cropid='".$row_arr_home['crop']."'") or die(mysqli_error($link));
	$row31=mysqli_fetch_array($sql_crop);
		
	$sql_variety=mysqli_query($link,"select * from tblvariety where varietyid='".$row_arr_home['variety']."' ") or die(mysqli_error($link));
   ...
   $variety=$vername;
	...
	{
$sql_tbl_sub=mysqli_query($link,"select distinct(lotno) from tbl_dryingsub where trid='".$arrival_id."'") or die(mysqli_error($link));
$subtbltot=mysqli_num_rows($sql_tbl_sub);
