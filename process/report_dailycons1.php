...
<title>Processing - Report - Periodical Consolidated  Processing Report</title>
...
winHandle=window.open('report_dailycons2.php?sdate='+sdate+'&txtcrop='+itemid+'&txtvariety='+vv+'&edate='+edate+'&txtpmcode='+pmc,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
...
?>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" style="border-bottom:solid; border-bottom-color:#adad11" >
	    <tr >
	      <td width="813" height="25">&nbsp;Periodical Consolidated Processing Report</td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
<tr class="tblsubtitle" height="20">
			...
			<td width="115"  align="center" valign="middle" class="smalltblheading" rowspan="2">Crop</td>
...
$totalenob=0; $totaleqty=0; $totalpnob=0; $totalpqty=0; $totalrmqty=0; $totalimqty=0; $totalplqty=0; $totaltplqty=0; $totalrmper=0; $totalimper=0; $totalplper=0; $totaltplper=0;		
if($tot_arr_home > 0)
{
...
$totenob=0; $toteqty=0; $totpnob=0; $totpqty=0; $totrmqty=0; $totimqty=0; $totplqty=0; $tottplqty=0; $totrmper=0; $totimper=0; $totplper=0; $tottplper=0;	
...
if($an5[1]==000){$tplqty1=$an5[0];}else{$tplqty1=$row_issuetbl['proslipsub_tlqty'];}
	...
	$totrmper=$totrmper+$rmper;
	$totimper=$totimper+$imper;
	$totplper=$totplper+$plper;
	$tottplper=$tottplper+$totplper;
	
}
}
}

	$totalenob=$totalenob+$totenob; 
	$totaleqty=$totaleqty+$toteqty;
	$totalpnob=$totalpnob+$totpnob;
	$totalpqty=$totalpqty+$totpqty;
	
	$totalrmqty=$totalrmqty+$totrmqty;
	$totalimqty=$totalimqty+$totimqty;
	$totalplqty=$totalplqty+$totplqty;
	$totaltplqty=$totaltplqty+$tottplqty;
	
	$totalrmper=$totalrmper+$totrmper;
	$totalimper=$totalimper+$totimper;
	$totalplper=$totalplper+$totplper;
	$totaltplper=$totaltplper+$tottplper;
...
<tr class="Light">
			...
			<td align="center" valign="middle" class="smalltbltext"><?php echo $variety;?></td>
          	<td align="center" valign="middle" class="smalltbltext"><?php echo $totenob?></td>
         	<td align="center" valign="middle" class="smalltbltext"><?php echo $toteqty?></td>
         	<td align="center" valign="middle" class="smalltbltext"><?php echo $totpnob?></td>
           	...
           	<td align="center" valign="middle" class="smalltbltext"><?php echo $totrmqty;?></td>
           	<td align="center" valign="middle" class="smalltbltext"><?php echo $totrmper;?></td>
			<td align="center" valign="middle" class="smalltbltext"><?php echo $totimqty?></td>
		  	<td align="center" valign="middle" class="smalltbltext"><?php echo $totimper?></td>
         	<td align="center" valign="middle" class="smalltbltext"><?php echo $totplqty?></td>
		   	<td align="center" valign="middle" class="smalltbltext"><?php echo $totplper;?></td>
           	<td align="center" valign="middle" class="smalltbltext"><?php echo $tottplqty;?></td>
           	<td align="center" valign="middle" class="smalltbltext"><?php echo $tottplper;?></td>
...
{
 $lotno=""; $enob=""; $eqty=""; $pnob=""; $pqty=""; $rmqty1=""; $rmper1=""; $imqty1=""; $imper1=""; $plqty1=""; $plper1=""; $tplqty=""; $tplper=""; $pmc=""; $psn=""; $treats=""; $oprname="";
//$srno=1;
 $totenob=0; $toteqty=0; $totpnob=0; $totpqty=0; $totrmqty=0; $totimqty=0; $totplqty=0; $tottplqty=0; $totrmper=0; $totimper=0; $totplper=0; $tottplper=0;	
if($txtpmcode!="ALL")
{	
$sql_rr22=mysqli_query($link,"select * from tbl_pnpslipmain where pnpslipmain_variety='".$rowrr['pnpslipmain_variety']."' and pnpslipmain_date<='$edate' and pnpslipmain_date>='$sdate' and pnpslipmain_tflag=1 and pnpslipmain_promachcode='$txtpmcode' order by pnpslipmain_id asc") or die(mysqli_error($link));
}
else
{
$sql_rr22=mysqli_query($link,"select * from tbl_pnpslipmain where pnpslipmain_variety='".$rowrr['pnpslipmain_variety']."' and pnpslipmain_date<='$edate' and pnpslipmain_date>='$sdate' and pnpslipmain_tflag=1 order by pnpslipmain_id asc") or die(mysqli_error($link));
}
$tot_rr22=mysqli_num_rows($sql_rr22);
while($row_rr22=mysqli_fetch_array($sql_rr22))
{
	...
	$sql_sel1="select * from tbl_rm_promac where promac_id='".$row_rr22['pnpslipmain_promachcode']."' order by promac_type";
	$res1=mysqli_query($link,$sql_sel1) or die (mysqli_error($link));
	$noticia_item1 = mysqli_fetch_array($res1);  
	$pmc=$noticia_item1['promac_mac'].$noticia_item1['promac_macid'];
	...
	$query_popr=mysqli_query($link,"SELECT * FROM tbl_rm_proopr where proopr_id='".$row_rr22['pnpslipmain_proopr']."'") or die("Error: " . mysqli_error($link));
	$row_popr=mysqli_fetch_array($query_popr);
	$oprname=$row_popr['proopr_code'];
...
$sql_issuetbl=mysqli_query($link,"select * from tbl_pnpslipsub where pnpslipmain_id='".$row_rr22['pnpslipmain_id']."' and pnpslipsub_conqty>0 order by pnpslipsub_lotno asc") or die(mysqli_error($link)); 
$t=mysqli_num_rows($sql_issuetbl);
if($t > 0)
{
while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
...
if($an5[1]==000){$tplqty1=$an5[0];}else{$tplqty1=$row_issuetbl['pnpslipsub_tlqty'];}
	...
	$sql_arr_sub=mysqli_query($link,"select * from tblarrival_sub where SUBSTRING(orlot,1,7)='".$newlot1."'") or die(mysqli_error($link));
	$row_arr_sub=mysqli_fetch_array($sql_arr_sub);
	$prodgrade=$row_arr_sub['prodgrade'];
...
<tr class="Light">
			<td colspan="2" align="center" valign="middle" class="smalltblheading">Total</td>
		   	<td align="center" valign="middle" class="smalltblheading"><?php echo $totalenob?></td>
         	<td align="center" valign="middle" class="smalltblheading"><?php echo $totaleqty?></td>
         	<td align="center" valign="middle" class="smalltblheading"><?php echo $totalpnob?></td>
		   	<td align="center" valign="middle" class="smalltblheading"><?php echo $totalpqty;?></td>
           	<td align="center" valign="middle" class="smalltblheading"><?php echo $totalrmqty;?></td>
           	<td align="center" valign="middle" class="smalltblheading">&nbsp;</td>
			<td align="center" valign="middle" class="smalltblheading"><?php echo $totalimqty?></td>
		  	<td align="center" valign="middle" class="smalltblheading">&nbsp;</td>
         	<td align="center" valign="middle" class="smalltblheading"><?php echo $totalplqty?></td>
		   	<td align="center" valign="middle" class="smalltblheading">&nbsp;</td>
           	<td align="center" valign="middle" class="smalltblheading"><?php echo $totaltplqty;?></td>
           	<td align="center" valign="middle" class="smalltblheading">&nbsp;</td>
...
<tr >
<td  align="center" valign="middle"><a href="report_dailycons.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" /></td>
</tr>
</table>
</td><td ></td>
</tr>
