...
<title>Plant Manager-Report - Periodical Crop Variety wise Stock Ledger Report</title>
...
winHandle=window.open('report_stockladger2.php?slchk='+slchk+'&slchk2='+slchk2+'&txtcrop='+itemid+'&txtvariety='+vv+'&sdate='+sdate+'&edate='+edate,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
...
?>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" style="border-bottom:solid; border-bottom-color:#2e81c1" >
	    <tr >
	      <td width="813" height="25">Periodical Crop Variety wise Stock Ledger Report</td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
if($crval<>"")
{
	...
	if($verval<>"")
	{
		...
		$orc=0; $opk=0; $osr=0; $xc=0; $xv=0; $xst=0; $xd=0; $xz=0; $xp=0; $xf=0; $pr=0; $prl=0; $pkl=0; $ccl=0; $rvl=0; $drl=0; $cil=0; $crc=0; $cpk=0; $csr=0; $isr=0; $tdfl=0;
		// Opening stock Records  -----------------------------------------
//		echo "Opening stock Records <br/>";
		$sql_arhome=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trdate<='$dt2' and plantcode='$plantcode' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
		while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crval."' and lotldg_lotno='".$row_arhome['lotldg_lotno']."' and lotldg_variety='".$verval."' and lotldg_trdate<='$dt2' and plantcode='$plantcode' group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
			while($row_is=mysqli_fetch_array($sql_is))
			{ 
				$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_lotno='".$row_arhome['lotldg_lotno']."' and lotldg_trdate<='$dt2' and plantcode='$plantcode' order by lotldg_id desc ") or die(mysqli_error($link));
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_trdate<='$dt2' and lotldg_balqty>0 and plantcode='$plantcode' order by lotldg_id asc") or die(mysqli_error($link)); 
...
//						echo $row_issuetbl['lotldg_lotno']." - ".$qt."<br/>";
		...
		{  
			...
			{ 
				...
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and lotldg_trdate<='$dt2' and balqty>0 and plantcode='$plantcode' order by lotdgp_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_istbl))
...
//						echo $row_issuetbl['lotno']." - ".$qt."<br/>";
		...
		{  
			...
			if($t > 0)
			{
				while($row_issuetbl=mysqli_fetch_array($sql_istbl))
				{ 
					$s_srm=mysqli_query($link,"select * from tbl_salesrv where salesr_date<='$dt2' and salesr_id='".$row_arhome['salesr_id']."' and salesr_vflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
...
//				echo $row_arhome['lotldg_lotno']." - ".$qt."<br/>";
		...
		$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate' and lotldg_trtype='StockTransfer Arrival' and plantcode='$plantcode' order by lotldg_id asc") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_arhome);
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
...
//				echo $row_arhome['lotldg_lotno']." - ".$qt."<br/>";
		...
		// Inward - Sales Return ----------------
		$sql_arhome=mysqli_query($link,"Select * from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate'  and lotldg_trtype='Sales Return' and plantcode='$plantcode'") or die(mysqli_error($link));
		while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$s_srm=mysqli_query($link,"select * from tbl_salesrv where salesr_id='".$row_arhome['lotldg_trid']."' and salesr_partytype!='C&F' and salesr_vflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
			$t_srm=mysqli_num_rows($s_srm);
			if($t_srm > 0)
			{		
...
//						echo $row_arhome['lotldg_lotno']." - ".$qt."<br/>";
			...
			$s_srm2=mysqli_query($link,"select * from tbl_salesrv where salesr_id='".$row_arhome['lotldg_trid']."' and salesr_partytype='C&F' and salesr_vflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
...
//						echo $row_arhome['lotldg_lotno']." - ".$qt."<br/>";
		...
		$sql_arhome=mysqli_query($link,"Select * from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate'  and lotldg_trtype='SRP2C' and plantcode='$plantcode'") or die(mysqli_error($link));
		while($row_arhome=mysqli_fetch_array($sql_arhome))
...
//		echo "<br />";echo "<br />";
		...
		$sq_srrv=mysqli_query($link,"Select * from tbl_srrevalidate where srrv_date>='$stdate' and srrv_date<='$etdate' and srrv_variety='$verval' and plantcode='$plantcode'") or die(mysqli_error($link));
		$tot_srrv=mysqli_num_rows($sq_srrv);
		while($row_srrv=mysqli_fetch_array($sq_srrv))
		{ 
			$sql_arhome=mysqli_query($link,"select * from tbl_salesrv_sub where salesrs_newlot='".$row_srrv['srrv_lotno']."' and salesrs_crop='".$crval."' and salesrs_variety='".$verval."' and salesrs_rettype='P2P' and plantcode='$plantcode' order by salesrs_id asc") or die(mysqli_error($link));
			while($row_arhome=mysqli_fetch_array($sql_arhome))
			{  
				$sql_istbl=mysqli_query($link,"select * from tbl_salesrvsub_sub where salesrs_id='".$row_arhome['salesrs_id']."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_istbl))
					{ 
						$s_srm=mysqli_query($link,"select * from tbl_salesrv where salesr_id='".$row_arhome['salesr_id']."' and salesr_partytype!='C&F' and salesr_vflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
						$t_srm=mysqli_num_rows($s_srm);
						if($t_srm > 0)
						{
							while($r=mysqli_fetch_array($s_srm))
						...
						$s_srm2=mysqli_query($link,"select * from tbl_salesrv where salesr_id='".$row_arhome['salesr_id']."' and salesr_partytype='C&F' and salesr_vflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
						$t_srm2=mysqli_num_rows($s_srm2);
						if($t_srm2 > 0)
						{
...
//		echo "<br />";echo "<br />";
		$sql_arhome=mysqli_query($link,"select * from tbl_salesrv_sub where salesrs_crop='".$crval."' and salesrs_variety='".$verval."' and salesrs_rettype='P2P' and salesrs_dovfy>='$stdate'  and salesrs_dovfy<='$etdate' and plantcode='$plantcode' order by salesrs_id asc") or die(mysqli_error($link));
		while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_istbl=mysqli_query($link,"select * from tbl_salesrvsub_sub where salesrs_id='".$row_arhome['salesrs_id']."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
...
//									echo $row_arhome['salesrs_newlot']." - ".$qt."<br/>";
		...
		// Inward - IVT In ----------------
		$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate' and lotldg_trtype='IVTC' and plantcode='$plantcode' order by lotldg_id asc") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_arhome);
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
...
///		echo "Outword stock Records <br/>";
		...
		$sql_is=mysqli_query($link,"select * from tbl_dispsub_sub where dpss_crop='".$crval."' and dpss_variety='".$verval."' and plantcode='$plantcode' order by disp_id asc") or die(mysqli_error($link));
		$tdfgh=mysqli_num_rows($sql_is);
		while($row_is=mysqli_fetch_array($sql_is))
		{ //echo $row_is['dpss_id']."  =  ";
			$sql_istbl=mysqli_query($link,"select * from tbl_disp where disp_id='".$row_is['disp_id']."' and disp_dodc>='$stdate' and disp_dodc<='$etdate' and disp_partytype NOT IN ('C&F','Branch') and disp_tflg=1 and plantcode='$plantcode' order by disp_id desc ") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				while($r=mysqli_fetch_array($sql_istbl))
				{
					$qt=$row_is['dpss_qty']; 
					if($qt<0)$qt=0;
//					echo $row_is['dpss_lotno']." Disp not cnf br ".$qt."<br/>";
					$totdisp=$totdisp+$qt;
					$tototw=$tototw+$qt;
					$xc=$xc+$qt;
					$ccnt++;
			...
			$sql_istbl=mysqli_query($link,"select * from tbl_disp where disp_id='".$row_is['disp_id']."' and disp_dodc>='$stdate' and disp_dodc<='$etdate' and disp_partytype IN ('C&F','Branch') and disp_tflg=1 and plantcode='$plantcode' order by disp_id desc ") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				while($r=mysqli_fetch_array($sql_istbl))
				{
					...
					$qt=$row_is['dpss_qty']; 
					if($qt<0)$qt=0;
//					echo $row_is['dpss_lotno']." Disp  cnf br ".$qt."<br/>";
		...
		$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trtype='Dispatch Bulk' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate' and plantcode='$plantcode' order by lotldg_id asc") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_arhome);
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
...
//				echo $row_arhome['lotldg_lotno']." D-Bulk ".$qt."<br/>";
		...
		$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trtype='Qty-Rem' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate' and plantcode='$plantcode' order by lotldg_id asc") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_arhome);
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
...
//				echo $row_arhome['lotldg_lotno']." Rem ".$qt."<br/>";
		...
		$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and trtype='Qty-Rem' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate'  and plantcode='$plantcode' order by lotdgp_id asc") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_arhome);
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
			{  
				$sql_istbl2=mysqli_query($link,"select * from tbl_pswrem where pswrem_id='".$row_arhome['lotldg_id']."' and pswrem_ptype IS NULL and plantcode='$plantcode'") or die(mysqli_error($link)); 
...
//						echo $row_arhome['lotno']." PK-Rem ".$qt."<br/>";
				...
				$sql_istbl2=mysqli_query($link,"select * from tbl_pswrem where pswrem_id='".$row_arhome['lotldg_id']."' and pswrem_ptype!='C&F' and pswrem_ptype IS NOT NULL and plantcode='$plantcode'") or die(mysqli_error($link)); 
				$t2=mysqli_num_rows($sql_istbl2);
				if($t2 > 0)
				{
					while($r=mysqli_fetch_array($sql_istbl2))
...
//						echo $row_arhome['lotno']." PK-Rem ".$qt."<br/>";
		...
		$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trtype='Stock Transfer Out' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate' and plantcode='$plantcode' order by lotldg_id asc") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_arhome);
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
...
//				echo $row_arhome['lotldg_lotno']." ST-Out ".$qt."<br/>";
		...
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
			{   
				$sql_istbl2=mysqli_query($link,"select * from tbl_pswrem where pswrem_id='".$row_arhome['lotldg_id']."' and pswrem_ptype='C&F' and plantcode='$plantcode'") or die(mysqli_error($link)); 
				$t2=mysqli_num_rows($sql_istbl2);
				if($t2 > 0)
				{
					while($r=mysqli_fetch_array($sql_istbl2))
...
//						echo $row_arhome['lotno']." PK-Rem ".$qt."<br/>";
		...
		$sql_is=mysqli_query($link,"select * from tbl_discard_sub where crop='".$crval."' and variety='".$verval."' and plantcode='$plantcode' order by did_s asc") or die(mysqli_error($link));
		while($row_is=mysqli_fetch_array($sql_is))
		{ 
			$sql_istbl=mysqli_query($link,"select * from tbl_discard where tid='".$row_is['did_s']."' and tdate>='$stdate' and tdate<='$etdate' and plantcode='$plantcode'") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				while($r=mysqli_fetch_array($sql_istbl))
...
//					echo $row_is['lotnumber']." Discard ".$qt."<br/>";
		...
		$sql_is=mysqli_query($link,"select * from tbl_proslipmain where proslipmain_date>='$stdate' and proslipmain_date<='$etdate' and proslipmain_tflag=1 and proslipmain_crop='".$crval."' and proslipmain_variety='".$verval."' and plantcode='$plantcode' order by proslipmain_id asc") or die(mysqli_error($link));
		while($row_is=mysqli_fetch_array($sql_is))
		{ 
			$sql_istbl=mysqli_query($link,"select * from tbl_proslipsub where proslipmain_id='".$row_is['proslipmain_id']."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
...
//					echo $row_is2['proslipsub_lotno']." PR-Loss ".$qt."<br/>";
		...
		$sql_is=mysqli_query($link,"select * from tbl_pnpslipmain where pnpslipmain_date>='$stdate' and pnpslipmain_date<='$etdate' and pnpslipmain_tflag=1 and pnpslipmain_crop='".$crval."' and pnpslipmain_variety='".$verval."' and plantcode='$plantcode' order by pnpslipmain_id asc") or die(mysqli_error($link));
		while($row_is=mysqli_fetch_array($sql_is))
		{ 
			$sql_istbl=mysqli_query($link,"select * from tbl_pnpslipsub where pnpslipmain_id='".$row_is['pnpslipmain_id']."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				while($row_is2=mysqli_fetch_array($sql_istbl))
...
//					echo $row_is2['pnpslipsub_lotno']." PK-Loss ".($qt+$qt2+$qt3)."<br/>";
		...
		//echo "<br />";echo $rvl."  -  ";
		$sql_is=mysqli_query($link,"select * from tbl_revalidate where rv_date>='$stdate' and rv_date<='$etdate' and rv_rvflg=1 and rv_crop='".$crval."' and rv_variety='".$verval."' and plantcode='$plantcode' order by rv_id asc") or die(mysqli_error($link));
...
//				echo $row_is['rv_lotno']."  RV-Loss  ".($ltqt+$ltqt2)."<br/>";
		...
		$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trtype='DRYSUC' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate' and plantcode='$plantcode' order by lotldg_id asc") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_arhome);
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
...
//				echo $row_arhome['lotldg_lotno']." DRY-Loss ".$qt."<br/>";
		...
		$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trtype='Dispatch TDF' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate' and plantcode='$plantcode' order by lotldg_id asc") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_arhome);
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
...
//				echo $row_arhome['lotldg_lotno']." TDF ".$qt."<br/>";
		...
		$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and trtype='Dispatch TDF' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate' and plantcode='$plantcode' order by lotdgp_id asc") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_arhome);
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
...
//			echo $row_srrv['srrv_lotno']." RV-Loss ".$qt."<br/>";	
		...
		//echo $rvl."  -  ";
		$sql_is=mysqli_query($link,"select * from tbl_ci where ci_date>='$stdate' and ci_date<='$etdate' and ci_tflg=1 and ci_crop='".$crval."' and ci_variety='".$verval."' and plantcode='$plantcode' order by ci_id asc") or die(mysqli_error($link));
		$xt=mysqli_num_rows($sql_is);
		while($row_is=mysqli_fetch_array($sql_is))
		{ 
			$sql_istbl=mysqli_query($link,"select * from tbl_cisub where ci_id='".$row_is['ci_id']."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				while($row_is2=mysqli_fetch_array($sql_istbl))
		...
		// Inward - IVT Out ----------------
		$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trdate>='$stdate' and lotldg_trdate<='$etdate' and lotldg_trtype='IVTO' and plantcode='$plantcode' order by lotldg_id asc") or die(mysqli_error($link));
		$t=mysqli_num_rows($sql_arhome);
		if($t > 0)
		{
			while($row_arhome=mysqli_fetch_array($sql_arhome))
...
//		echo "Closing stock Records <br/>";
		...
		$sql_arhome=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_trdate<='$etdate' and plantcode='$plantcode' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
		while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_is=mysqli_query($link,"select distinct lotldg_subbinid from tbl_lot_ldg where  lotldg_crop='".$crval."' and lotldg_lotno='".$row_arhome['lotldg_lotno']."' and lotldg_variety='".$verval."' and lotldg_trdate<='$etdate' and plantcode='$plantcode' group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
			while($row_is=mysqli_fetch_array($sql_is))
			{ 
				$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_lotno='".$row_arhome['lotldg_lotno']."' and lotldg_trdate<='$etdate' and plantcode='$plantcode' order by lotldg_id desc ") or die(mysqli_error($link));
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_trdate<='$etdate' and lotldg_balqty>0 and plantcode='$plantcode' order by lotldg_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_istbl))
...
//						echo  $row_arhome['lotldg_lotno']."  CL-RC  ".$qt."<br/>";
		...
		{  
			...
			{ 
				...
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and lotldg_trdate<='$etdate' and balqty>0 and plantcode='$plantcode' order by lotdgp_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_istbl))
...
//						echo  $row_arhome['lotno']."  CL-PK  ".$qt."<br/>";
		...
		$sql_arhome=mysqli_query($link,"select * from tbl_salesrv_sub where salesrs_crop='".$crval."' and salesrs_variety='".$verval."' and salesrs_rettype='P2P' and salesrs_dovfy<='$etdate' and plantcode='$plantcode' order by 'salesrs_id' asc") or die(mysqli_error($link));
		while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_istbl=mysqli_query($link,"select * from tbl_salesrvsub_sub where salesrs_id='".$row_arhome['salesrs_id']."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				while($row_issuetbl=mysqli_fetch_array($sql_istbl))
				{ 
					$s_srm=mysqli_query($link,"select * from tbl_salesrv where salesr_date<='$etdate' and salesr_id='".$row_arhome['salesr_id']."' and salesr_vflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
					$t_srm=mysqli_num_rows($s_srm);
					if($t_srm > 0)
					{
						while($r=mysqli_fetch_array($s_srm))
...
//	echo $orc." + ".$opk." + ".$osr." + ".$isr." + ".$xc." + ".$xv." + ".$xd." + ".$xz." + ".$xp." + ".$xf." + ".$pr." + ".$prl." + ".$pkl." + ".$ccl." + ".$rvl." + ".$drl." + ".$cil." + ".$tdfl." + ".$totloss." = ".$tototw." => ".$crc." - ".$cpk." - ".$csr."<br>";	
...
$sql_rep="Insert into tmp_pmsldgrep (crop, variety, vertype, opstock, frnstock, srinstock, stinplant, stincnf, ivtin, cistock, totinstock, salesstock, purretstock, stoutplant, stoutcnf, tdfstock, ivtout, totloss, totoutstock, clstock, logid, repflg, plantcode) values ('$crop1', '$verty', '$vtyp', '$totopqty', '$totfrn', '$totsrn', '$totstp', '$totstcnf', '$totivtin', '$cirec', '$totinw', '$totdisp', '$totpret', '$totstpo', '$totstcnfo', '$tottdf', '$totivtout', '$totloss', '$tototw', '$totclqty', '$logid', '1', '$plantcode')";
...
<tr >
<td height="49" align="center" valign="top"><a href="report_stockladger.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" />
  <input type="hidden" name="txtinv" />&nbsp;<a href="excel-stockladger.php?txtcrop=<?php echo $_REQUEST['txtcrop']?>&txtvariety=<?php echo $_REQUEST['txtvariety']?>&slchk=<?php echo $slchk;?>&slchk2=<?php echo $slchk2;?>&sdate=<?php echo $sdate;?>&edate=<?php echo $edate;?>" target="_blank"><img src="../images/excelicon1.jpg" border="0" height="30" width="30" class="butn" alt="Export to Excel" style="display:inline;cursor:pointer;" /></a>&nbsp;</td>
</tr>
</table>
</td><td ></td>
</tr>
