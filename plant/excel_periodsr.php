<?php
	...
	$datahead6 = array("#","Party Type","Party Name","Location","SRN No.","State","Crop","Veriety"); 
	 if($txtups=="Yes")
	 {
	 array_push($datahead6,"UPS");
	 }
	array_push($datahead6,"Total Qty","Actual Received Qty","Damage Qty","Ex.(+)/ Sh.(-) Qty");
...
while($row_srretm=mysqli_fetch_array($sql_srretm))
{
	...
	if($totqty>0 || $okqty>0)						
	{
		$data1[$d]=array($d,$trdate,$ptype,$panme,$locn,$srn_no,$srstate,$cropn,$varietyn);
		if($txtups=="Yes")
		{
		array_push($data1[$d],$ups);
		}
		array_push($data1[$d],$totqty,$okqty,$failqty,$exshqty);
		$d++;$cnt++;
	}
	}
	}
