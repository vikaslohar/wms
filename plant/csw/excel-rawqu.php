...
session_start();
	...
	$dh="Quality_Based_Condition_Seed_Stock_Report_as_on_date_From_".$_REQUEST['sdate'];
	$datahead = array($dh);
	$datahead2 = array("Quality Based Condition Seed Stock Report as on date ",$_REQUEST['sdate']);
...
$totqty=0; $totqcq=0; $totuqq=0; $totufq=0; $totgcq=0; $totugq=0; $totgfq=0;
	$crop=""; $variety="";
	...
	$sql_arr_home=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where  lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and lotldg_sstage='Condition' and plantcode='$plantcode' ") or die(mysqli_error($link));
	while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	
	$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where  lotldg_lotno='".$row_arr_home['lotldg_lotno']."' and lotldg_sstage='Condition' and plantcode='$plantcode' ") or die(mysqli_error($link));


 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
 
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_trdate <= '$edate' and lotldg_lotno='".$row_arr_home['lotldg_lotno']."' and lotldg_sstage='Condition' and plantcode='$plantcode' ") or die(mysqli_error($link));
...
if($tot_arr_home > 0)			
{
$data1[$d]=array($d,$crop,$variety,$totqty,$totqcq,$totuqq,$totufq,$totgnutq,$totgcq,$totuqq,$totgfq); 
$d++;
}
}
//}
