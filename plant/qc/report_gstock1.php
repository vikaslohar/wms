...
if($rettyp=="periodical")
{
	if($variety!="ALL")
	{
		$qry="select * from tbl_gsample where gsdate >= '$sdate' and gsdate <= '$edate' and  gscrop='$crp' and gsvariety='$ver' and gsdisflg=0 and plantcode='$plantcode' order by gsdate asc ";
	}
	else 
	{
		$qry="select * from tbl_gsample where gsdate >= '$sdate' and gsdate <= '$edate' and  gscrop='$crp' and gsdisflg=0 and plantcode='$plantcode' order by gsdate asc ";
	}
}
else
{
	if($variety!="ALL")
	{
		$qry="select * from tbl_gsample where gsdate <= '$sdate' and  gscrop='$crp' and gsvariety='$ver' and gsdisflg=0 and plantcode='$plantcode' order by gsdate asc ";
	}
	else 
	{
		$qry="select * from tbl_gsample where gsdate <= '$sdate' and  gscrop='$crp' and gsdisflg=0 and plantcode='$plantcode' order by gsdate asc ";
	}
}
//echo $qry;
...
?>
    ...
    <?php
 //$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tblarrival where arrtrflag=1");
...
$quer3=mysqli_query($link,"SELECT * FROM tblbin  where binid='".$binn1."' and plantcode='$plantcode'"); 
	$row31=mysqli_fetch_array($quer3);
	  $binn=$row31['binname'];
	
	$quer4=mysqli_query($link,"SELECT * from tblwarehouse where whid ='".$wh1."' and plantcode='$plantcode'"); 
