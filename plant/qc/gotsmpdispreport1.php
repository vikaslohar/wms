...
$sql_class=mysqli_query($link,"select * from tblcrop where cropid='".$itemid."'") or die(mysqli_error($link));
	...
	{	
	$sql_arr_home=mysqli_query($link,"select * from tbl_qctest where dosdate <= '$edate' and dosdate >= '$sdate' and  gotsmpdflg=1 and plantcode='$plantcode' order by dosdate asc ") or die(mysqli_error($link));
	}
	else if($itemid!="ALL" && $vv=="ALL")
	{
	$sql_arr_home=mysqli_query($link,"select * from tbl_qctest where dosdate <= '$edate' and dosdate >= '$sdate' and crop='".$itemid."' and  gotsmpdflg=1 and plantcode='$plantcode' order by dosdate asc ") or die(mysqli_error($link));
	}
	else if($itemid!="ALL" && $vv!="ALL")
	{
	$sql_arr_home=mysqli_query($link,"select * from tbl_qctest where dosdate <= '$edate' and dosdate >= '$sdate' and crop='".$itemid."' and variety='$vv' and  gotsmpdflg=1 and plantcode='$plantcode' order by dosdate asc ") or die(mysqli_error($link));
	}
...
<tr class="Light" height="25">
         ...
         <td width="81" align="center" valign="middle" class="tblheading"><?php echo $tp1?><?php echo $yearid_id?><?php echo sprintf("%000006d",$qc1);?></td>
