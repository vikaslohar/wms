...
<?php	
	...
	if($trstage!="ALL")
	$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where gotdate='$sdate' and trstage='$txtstage' and plantcode='$plantcode' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
	else
...
if($trstage!="ALL")
	$sql_arr_home2=mysqli_query($link,"select MAX(tid) from tbl_qctest where sampleno='".$row_arr_home2['sampleno']."' and gotdate='$sdate' and trstage='$txtstage' and plantcode='$plantcode' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
else
...
$got=$row_arr_home['moist'];
$stage=$row_arr_home['gemp'];
	$sql_tbl_sub1=mysqli_query($link,"select distinct subbinid, lotldg_variety, lotldg_crop, whid, binid from tbl_lot_ldg_pack where lotno='".$row_arr_home['lotno']."' and plantcode='$plantcode' group by subbinid, lotldg_variety, lotno order by subbinid") or die(mysqli_error($link));
	$t=mysqli_num_rows($sql_tbl_sub1);
	 while($row_tbl=mysqli_fetch_array($sql_tbl_sub1))
	{
	
	$sql_tbl1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_tbl['subbinid']."' and lotno='".$row_arr_home['lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));  
$row_tbl1=mysqli_fetch_array($sql_tbl1);
