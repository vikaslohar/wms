...
else if($cid!="ALL" && $itemid!="ALL")	
{
	if($qcti=="rt")
	{
		$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where qcstatus='RT' and  srdate <= '$sdate' and crop='$cid' and variety='$vv_var' and qcflg=0 and state!='T' and plantcode='$plantcode' order by spdate asc ") or die(mysqli_error($link));
	}
	else  if($qcti=="ut")
	{
		$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where qcstatus='UT' and srdate <= '$sdate' and qcflg=0 and crop='$cid' and variety='$vv_var' and state!='T' and plantcode='$plantcode' order by spdate asc ") or die(mysqli_error($link));
	}
	else
	{
		$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where srdate <='$sdate' and crop='$cid' and variety='$vv_var' and qcflg=0 and state!='T' and plantcode='$plantcode' order by spdate asc ") or die(mysqli_error($link));
	}
}
