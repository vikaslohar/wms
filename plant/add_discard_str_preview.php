	...
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		...
		while($row_arrsub_sub=mysqli_fetch_array($sql_arrsub_sub))
		{
			...
			$stage=$row_arrsub_sub['sstage'];
				...
				$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$subbinid."' and lotldg_binid='".$binid."' and lotldg_whid='".$whid."' and orlot='".$lotno."' and lotldg_sstage='".$stage."' AND plantcode='".$plantcode."'") or die(mysqli_error($link));
 ...
 { 
	...
	if($totnog > 0)
	{
	  $cntd++;
		...
		$sql_code="SELECT MAX(dd_code) FROM tbl_discard where yearcode='$yearid_id' AND plantcode='".$plantcode."' ORDER BY dd_code DESC";
	$res_code=mysqli_query($link,$sql_code)or die(mysqli_error($link));
		...
		$sql_code1="SELECT MAX(ncode) FROM tbl_discard where yearcode='$yearid_id' AND plantcode='".$plantcode."' ORDER BY ncode DESC";
	...
	$s_chk=mysqli_query($link,"SELECT * FROM tbl_gate where yearcode='$yearid_id' AND plantcode='$plantcode'") or die (mysqli_error($link));
	$r_chk=mysqli_num_rows($s_chk);
	if($r_chk > 0)
	$sql_code2="SELECT MAX(gid) FROM tbl_gate where yearcode='$yearid_id' AND plantcode='$plantcode' ORDER BY gid DESC";
	else
	$sql_code2="SELECT MAX(gid) FROM tbl_gate WHERE plantcode='$plantcode' ORDER BY gid DESC";
