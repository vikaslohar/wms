...
$sql_tblsub=mysqli_query($link,"select * from tbl_discard_sub where did='".$rid."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
<div id="subdiv24">
<table align="center" border="1" width="750" cellspacing="0" cellpadding="0" bordercolor="#2e81c1" style="border-collapse:collapse" >  <tr class="tblsubtitle" height="20">
...
$sql_tbl_sub=mysqli_query($link,"select * from tbl_discard_sloc where discard_id=$rid and plantcode='$plantcode'") or die(mysqli_error($link));
$tot_tblissue=mysqli_num_rows($sql_tbl_sub);

while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
{

$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_tbl_sub['discard_rowid']."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
$row_issuetbl=mysqli_fetch_array($sql_issuetbl);

$wareh=""; $binn=""; $subbinn=""; $sups="";$sqty=0; $slocs=""; $gd=""; $slups=0; $slqty=0;
  
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where whid='".$row_issuetbl['lotldg_whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_mainissue=mysqli_query($link,"select distinct lotldg_sstage from tbl_lot_ldg where lotldg_crop='".$b."' and lotldg_variety='".$c."' and orlot='".$a."' and plantcode='$plantcode'") or die(mysqli_error($link));

while($row_mainissue=mysqli_fetch_array($sql_mainissue))
{ 

$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$b."' and lotldg_variety='".$c."' and orlot='".$a."' and lotldg_sstage='".$row_mainissue['lotldg_sstage']."' and plantcode='$plantcode'") or die(mysqli_error($link));

while($row_issue=mysqli_fetch_array($sql_issue))
 { 
 
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_variety='".$row_tblsub['variety']."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."'  and lotldg_balqty > 0 and plantcode='$plantcode'") or die(mysqli_error($link)); 
 ...
 { 
$sql_tbl_sub=mysqli_query($link,"select * from tbl_discard_sloc where discard_rowid='".$row_issuetbl['lotldg_id']."' and discard_id='".$rid."' and plantcode='$plantcode'") or die(mysqli_error($link));
$tot_tblissue=mysqli_num_rows($sql_tbl_sub);
