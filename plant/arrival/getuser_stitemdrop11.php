...
$sql_month=mysqli_query($link,"select * from tblspcodes where spcodef='".$a."' and spcodem='".$b."' and plantcode='$plantcode' order by variety")or die(mysqli_error($link));
$row= mysqli_fetch_array($sql_month);
 ...
 $quer3=mysqli_query($link,"SELECT varietyid,popularname FROM tblvariety where varietyid ='".$variety."' order by popularname"); 
 $rowvv=mysqli_fetch_array($quer3);
 $tt1=$rowvv['popularname'];
 $tot=mysqli_num_rows($quer3);	
	
	$sql_month1=mysqli_query($link,"select * from tblarrival_sub where spcodef='".$a."'and spcodem='".$b."' and plantcode='$plantcode'")or die(mysqli_error($link));
while($row_tbl_sub=mysqli_fetch_array($sql_month1))
{
   $lotno=$row_tbl_sub['lotno'];
//$row_month=mysqli_fetch_array($sql_month);
  $tt=mysqli_num_rows($sql_month1);
}
if($tt > 0)
{
?>
...
<?php
 $sql_tb="select * from tbl_lot_ldg where lotldg_lotno='".$lotno."' and plantcode='$plantcode' group by lotldg_subbinid, lotldg_lotno, lotldg_crop order by lotldg_subbinid"; 
 ?>
...
{
$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_whid='".$row_tbl['lotldg_whid']."' and lotldg_binid='".$row_tbl['lotldg_binid']."' and lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_variety='".$variety."' and lotldg_lotno='".$row_tbl['lotldg_lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));  
...
$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$row_tbl_sub['lotldg_subbinid']."' and binid='".$row_tbl_sub['lotldg_binid']."' and whid='".$row_tbl['lotldg_whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
