	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if(mysqli_query($link,$sql_srmain) or die(mysqli_error($link)))
		{
			...
			for($i=0; $i<count($lot1); $i++)
			{
				if($lot1[$i]<>"")
				{
					$sql_srsub="Insert into tbl_softr_sub2 (softr_id, softrsub_lotno, softrsub_srtyp, softrsub_srflg, plantcode) values('$tid', '$lot1[$i]', '$srt1[$i]', '1', '$plantcode')";
...
{
$sql="SELECT distinct orlot, lotldg_got1 FROM tbl_lot_ldg WHERE lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_whid='".$wh."' and lotldg_binid='".$bin."' and lotldg_subbinid='".$subbin."' and lotldg_srflg='0' and lotldg_qc!='NUT' and lotldg_qc!='Fail' AND plantcode='$plantcode' order by orlot";
}
...
$sql_qct=mysqli_query($link,"select * from tbl_qctest where oldlot='".$row_arr_home['orlot']."' AND plantcode='$plantcode' order by tid desc") or die(mysqli_error($link));
...
//echo $flg;
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where  orlot='".$row_arr_home['orlot']."'  and lotldg_balqty > 0  and lotldg_srflg='0' and lotldg_qc!='Fail' AND plantcode='$plantcode' order by orlot") or die(mysqli_error($link));
...
$sql_issue=mysqli_query($link,"select distinct whid, subbinid, binid from tbl_lot_ldg_pack where orlot='".$row_arr_home['orlot']."' and balqty > 0 and lotldg_srflg='0' and lotldg_qc!='Fail' AND plantcode='$plantcode' order by orlot") or die(mysqli_error($link));
$zz=mysqli_num_rows($sql_issue);
//if($zz==0)$flg=1;
 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
...
$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and orlot='".$row_arr_home['orlot']."' and lotldg_srflg='0' and lotldg_qc!='Fail' AND plantcode='$plantcode' order by orlot") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 
...
if($flg==0)
{
$sql_srsub=mysqli_query($link,"SELECT * FROM tbl_softr_sub2 WHERE softr_id='".$tid."' and softrsub_lotno='".$row_arr_home['orlot']."' AND plantcode='$plantcode'") or die(mysqli_error($link));
