...
{
	$sqlpnpm=mysqli_query($link,"select * from tbl_pnpslipmain where pnpslipmain_id='".$row_pakloss['pnpslipmain_id']."' and pnpslipmain_tflag=1 and plantcode='$plantcode'") or die(mysqli_error($link));
	if($totpnpm=mysqli_num_rows($sqlpnpm)>0)
	{
		$pakloss=$pakloss+$row_pakloss['pnpslipsub_packloss']+$row_pakloss['pnpslipsub_packcc'];
...
{	$prosn=""; $prodate=""; $promac=""; $proopr=""; $proin=""; $proout=""; $proloss=""; $ts="";
	$sql_pro_main=mysqli_query($link,"select * from tbl_proslipmain where proslipmain_id='".$row_repro['proslipmain_id']."' and plantcode='$plantcode'")or die(mysqli_error($link));
	$row_pro_main=mysqli_fetch_array($sql_pro_main);
	...
	$proin=$row_repro['proslipsub_pqty'];
	$proout=$row_repro['proslipsub_conqty'];
	$proloss=$row_repro
['proslipsub_tlqty'];
...
{
	$pakdate="";
	$sql_pkmain=mysqli_query($link,"select * from tbl_pnpslipmain where pnpslipmain_id='".$row_packing['pnpslipmain_id']."' and pnpslipmain_tflag=1 and plantcode='$plantcode'") or die(mysqli_error($link));
	$tot_pkmain=mysqli_num_rows($sql_pkmain);
	if($tot_pkmain>0)
	{
