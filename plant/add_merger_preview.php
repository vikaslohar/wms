...
//	exit;
	...
	while($row_arr=mysqli_fetch_array($sql_arr))
	{
		...
		$sql_arrsub=mysqli_query($link,"select * from tbl_mergersub where mergerm_id='".$pid."' and plantcode='$plantcode'") or die(mysqli_error($link));
		while($row_arrsub=mysqli_fetch_array($sql_arrsub))
		{
			$lotno=$row_arrsub['mergers_lotno'];
			
			$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$lotstage."' and lotldg_lotno='".$lotno."' and plantcode='$plantcode'") or die(mysqli_error($link));
			$trtr=mysqli_num_rows($sql_issue);
			while($row_issue=mysqli_fetch_array($sql_issue))
			{ 
				$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_variety='".$variety."'  and lotldg_lotno='".$lotno."' and plantcode='$plantcode'") or die(mysqli_error($link));
	...
	//exit;
			echo "<script>window.location='select_merger_op.php?p_id=$pid'</script>";	
}
?>
...
<td width="204"  align="left" valign="middle" class="tbltext">&nbsp;<?php echo "TLM".$row['mergerm_tcode']."/".$yearid_id."/".$lgnid;?></td>
