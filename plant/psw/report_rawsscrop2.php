<?php
	...
	$crp="ALL"; $ver="ALL";
	$qry="select Distinct lotldg_crop, lotldg_variety from tbl_lot_ldg_pack where lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode'";
...
<title>PSW - Report - Crop Variety wise Substandard Pack Seed Report</title>
...
$sql_rr=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode'") or die(mysqli_error($link));
...
while($row_rr=mysqli_fetch_array($sql_rr))
{
	...
	$sql_crop=mysqli_query($link,"select * from tblcrop where cropid='".$row_arr_home1['lotldg_crop']."'") or die(mysqli_error($link));
		...
		$variety=$row_rr['lotldg_variety'];
				...
				$ccnt=0;
$sql_arhome=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where  lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."'  and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and balqty > 0 and plantcode='$plantcode' group by lotno order by lotdgp_id asc") or die(mysqli_error($link));
	while($row_arhome=mysqli_fetch_array($sql_arhome))
{  
	$sql_is=mysqli_query($link,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where  lotno='".$row_arhome['lotno']."'  and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and balqty > 0 and plantcode='$plantcode' group by subbinid order by lotdgp_id asc") or die(mysqli_error($link));

 while($row_is=mysqli_fetch_array($sql_is))
 { 
$sql_is1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_is['subbinid']."' and binid='".$row_is['binid']."' and whid='".$row_is['whid']."' and lotno='".$row_arhome['lotno']."' and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode' order by lotdgp_id asc ") or die(mysqli_error($link));
$row_is1=mysqli_fetch_array($sql_is1); 

$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and balqty > 0 and plantcode='$plantcode' order by lotdgp_id asc") or die(mysqli_error($link)); 
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="750" bordercolor="#2e81c1" style="border-collapse:collapse">
  	<tr height="25">
    <td align="center" class="subheading" style="color:#303918; " colspan="6">Crop Variety wise Substandard Pack Seed Report</td>
  </tr>
<tr height="25" >
...
$totqty=0; $totnob=0; $totqc=""; $totdot=""; $totmost=""; $totgemp=""; $totgot=""; $reserve=""; $totsst="";	$sloc=""; $genpp=""; $dogr="";
	$sql_issue=mysqli_query($link,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where  lotno='".$row_arr_home['lotno']."'  and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and balqty > 0 and plantcode='$plantcode' group by subbinid order by lotdgp_id asc") or die(mysqli_error($link));

 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
$txtdot=""; 

$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and lotldg_sstage='Pack' and whid='".$row_issue['whid']."' and lotno='".$row_arr_home['lotno']."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode' order by lotdgp_id asc ") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_issue1[0]."' and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and balqty > 0 and plantcode='$plantcode' order by lotdgp_id asc") or die(mysqli_error($link)); 
