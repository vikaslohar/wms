<?php
	...
	require_once("../../include/connection.php");
		...
		$crp="ALL"; $ver="ALL";
	$qry="select Distinct lotldg_crop, lotldg_variety from tbl_lot_ldg_pack where lotldg_sstage='Pack' and plantcode='$plantcode'";
...
$totqty=0; $totqcq=0; $totuqq=0; $totufq=0; $totgcq=0; $totugq=0; $totgfq=0; $totgnutq=0;
	...
	$sql_arr_home=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where  lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and lotldg_sstage='Pack' and plantcode='$plantcode'") or die(mysqli_error($link));
	while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	
	$sql_issue=mysqli_query($link,"select distinct whid, subbinid, binid from tbl_lot_ldg_pack where lotno='".$row_arr_home['lotno']."' and lotldg_sstage='Pack' and plantcode='$plantcode'") or die(mysqli_error($link));


 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
 
$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and lotldg_trdate <= '$edate' and lotno='".$row_arr_home['lotno']."' and lotldg_sstage='Pack' and plantcode='$plantcode'") or die(mysqli_error($link));
