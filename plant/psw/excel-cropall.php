...
session_start();
	...
	$dh="Crop_Variety_wise_Pack_Seed_Report ".$dat;
	$datahead = array("Crop Variety wise Pack Seed Report  ".$dat);
	$filename=$dh.".xls";  
...
$sql_rr=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where  lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$totalbags=0; $totalqty=0;
	...
	$datahead2[$cnt] = array("#","Lot No.","Size","Qty"); 
	if($slchk=="yes") 
	{	
...
$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and lotno='".$row_arr_home['lotno']."' and packtype='".$row_rr2['packtype']."' and plantcode='$plantcode' order by lotdgp_id asc ") or die(mysqli_error($link));
...
$slqty=$acn1;
if($sloc!="")
$sloc=$sloc.$wareh.$binn.$subbinn." | ".$slqty.", ";
else
$sloc=$wareh.$binn.$subbinn." | ".$slqty.", ";
}
}
}
//echo $cnt;
...
$datahead2[$cnt] = array("#","Lot No.","NoB","Qty","SLOC"); 
$data1[$cnt][$d]=array($d,$lotn,$upssize,$totqty); 
if($slchk=="yes") 
{
