...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <?php
	...
	$crp="ALL"; $ver="ALL";
	$qry="select Distinct lotldg_crop, lotldg_variety from tbl_lot_ldg_pack where lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and balqty > 0 and plantcode='$plantcode'";
...
?>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" style="border-bottom:solid; border-bottom-color:#2e81c1" >
	    <tr >
	      <td width="813" height="25">Crop Variety wise Substandard Pack Seed Report</td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
$sql_rr=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and balqty > 0 and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode' order by lotdgp_id desc") or die(mysqli_error($link));
...
while($row_rr=mysqli_fetch_array($sql_rr))
{
	...
	$sql_crop=mysqli_query($link,"select * from tblcrop where cropid='".$row_arr_home1['lotldg_crop']."'") or die(mysqli_error($link));
		...
		$ccnt=0;
$sql_arhome=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where  lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."'  and balqty > 0 and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode' group by lotno order by lotdgp_id asc") or die(mysqli_error($link));
	while($row_arhome=mysqli_fetch_array($sql_arhome))
{  
	$sql_is=mysqli_query($link,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where  lotno='".$row_arhome['lotno']."'  and balqty > 0 and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode' group by subbinid order by lotdgp_id asc") or die(mysqli_error($link));
...
$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and balqty > 0 and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode' order by lotdgp_id asc") or die(mysqli_error($link)); 
...
$totqty=0; $totnob=0; $totqc=""; $totdot=""; $totmost=""; $totgemp=""; $totgot=""; $reserve=""; $totsst=""; $sloc=""; $genpp=""; $dogr="";
	$sql_issue=mysqli_query($link,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where  lotno='".$row_arr_home['lotno']."'  and balqty > 0 and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode' group by subbinid order by lotdgp_id asc") or die(mysqli_error($link));

 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
$txtdot=""; 

$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and lotno='".$row_arr_home['lotno']."' and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode' order by lotdgp_id asc ") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_issue1[0]."' and balqty > 0 and lotldg_sstage='Pack' and (lotldg_qc='Fail' OR lotldg_got='Fail') and plantcode='$plantcode' order by lotdgp_id asc") or die(mysqli_error($link)); 
