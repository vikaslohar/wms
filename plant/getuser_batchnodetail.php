...
?>
<table align="center" border="1" width="700" cellspacing="0" cellpadding="0" bordercolor="#2e81c1" style="border-collapse:collapse" > 
...
$sq2=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg WHERE orlot = '$baselot32' and plantcode='$plantcode' order by lotldg_id asc")or die("Error:".mysqli_error($link));
$totrow2=mysqli_num_rows($sq2);
if($totrow2 >= 1)
while($rowissue2=mysqli_fetch_array($sq2))
{ 
$lotqry=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg WHERE orlot = '$baselot32' and lotldg_lotno='".$rowissue2['lotldg_lotno']."' and plantcode='$plantcode'")or die("Error:".mysqli_error($link));
$tot_row=mysqli_num_rows($lotqry);
$nob=0; $qty=0; $sstage=""; $lotts=""; $upsz="";
while($row_issue=mysqli_fetch_array($lotqry))
{ 
	$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and orlot = '$baselot32'  and lotldg_lotno='".$rowissue2['lotldg_lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));
	$row_issue1=mysqli_fetch_array($sql_issue1); 
...
$abc27=sprintf("%02d",$i);
$baselot32=$baselot2.$abc27;
$sq2=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack WHERE orlot = '$baselot32' and plantcode='$plantcode' order by lotdgp_id asc")or die("Error:".mysqli_error($link));
$totrow2=mysqli_num_rows($sq2);
if($totrow2 >= 1)
while($rowissue2=mysqli_fetch_array($sq2))
{ 
$lotqry=mysqli_query($link,"select distinct whid, subbinid, binid from tbl_lot_ldg_pack WHERE orlot = '$baselot32' and lotno='".$rowissue2['lotno']."' and plantcode='$plantcode'")or die("Error:".mysqli_error($link));
$tot_row=mysqli_num_rows($lotqry);
$nob=0; $qty=0; $sstage=""; $lotts=""; $upsz="";
while($row_issue=mysqli_fetch_array($lotqry))
{ 
	$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and orlot = '$baselot32'  and lotno='".$rowissue2['lotno']."' and plantcode='$plantcode'") or die(mysqli_error($link));
	$row_issue1=mysqli_fetch_array($sql_issue1); 
