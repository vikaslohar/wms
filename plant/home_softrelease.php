...
$sql_tbl=mysqli_query($link,"select * from tbl_softr where softr_tflg=0 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['softr_id'];	
	
	$s_sub="delete from tbl_softr_sub where softr_id='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
}

	$s_sub="delete from tbl_softr where softr_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
<title>Plant- Transaction -Home - Soft Release</title>
...
{
winHandle=window.open('lsrn.php?itmid='+itm,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" >
	  ...
	  <td width="820" class="Mainheading" height="25"><table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" style="border-bottom:solid; border-bottom-color:#2e81c1" >
        <tr >
          <td width="820" height="25">&nbsp;Transaction - Soft Release (SR) </td>
        </tr>
	    </table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_softrelease.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
	  </tr>
	  </table></td></tr>
...
$sql_arr_home=mysqli_query($link,"select * from tbl_softr where softr_tflg=1 and plantcode='$plantcode' order by softr_id desc,softr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_softr where softr_tflg=1 and plantcode='$plantcode'");
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">SR in Progress List </td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="800" bordercolor="#2e81c1" style="border-collapse:collapse">
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_tbl_sub=mysqli_query($link,"select * from tbl_softr_sub where softr_id='".$arrival_id."' and softrsub_srflg='1' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and lotldg_balqty > 0  and (lotldg_qc='UT' OR lotldg_qc='RT') and plantcode='$plantcode'") or die(mysqli_error($link)); 
...
/*if(!isset($_GET['page'])) { 
	...
	$sql_dtchk=mysqli_query($link,"select * from tbl_softr where softr_tflg=1 order by softr_id desc LIMIT 0,1") or die(mysqli_error($link));
...
$sql_arr_home=mysqli_query($link,"select * from tbl_softr where softr_tflg=1 and softr_date>='$dt1' order by softr_id desc, softr_code desc") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_softr where softr_tflg=1 and softr_date>='$dt1'");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
...
<table align="center" border="0" width="800" cellspacing="0" cellpadding="0" bordercolor="#4ea1e1" style="border-collapse:collapse" > 
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">SR in Complete List </td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="800" bordercolor="#2e81c1" style="border-collapse:collapse">
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$lotno=""; $type=""; $qc="";$got="";
	$sql_tbl_sub=mysqli_query($link,"select * from tbl_softr_sub where softr_id='".$arrival_id."' and softrsub_srflg='0'") or die(mysqli_error($link));
	$subtbltot=mysqli_num_rows($sql_tbl_sub);
	while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
	{
