...
$sql_sub=mysqli_query($link,"select * from tbl_softr_sub2 where softrsub_id='".$pid."' and softr_id='".$trid."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_sub=mysqli_fetch_array($sql_sub);
$lotno=$row_sub['softrsub_lotno'];

$s_sub="delete from tbl_softr_sub2 where softrsub_id='".$pid."'";
mysqli_query($link,$s_sub) or die(mysqli_error($link));
$sql_t_sub=mysqli_query($link,"select * from tbl_softr_sub2 where softr_id='".$trid."' and plantcode='$plantcode'") or die(mysqli_error($link));
$tot_sub=mysqli_num_rows($sql_t_sub);
if($tot_sub == 0)
{
$s_sub_sub="delete from tbl_softr2 where softr_id='".$trid."'";
