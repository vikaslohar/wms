...
if($c!="ALL")
{
	...
	{
		if($b!='ALL')
			$sql_month=mysqli_query($link,"select * from tbl_partymaser where location_id='".$b."' and classification='".$c."' order by business_name")or die(mysqli_error($link));
		else
			$sql_month=mysqli_query($link,"select * from tbl_partymaser where classification='".$c."' order by business_name")or die(mysqli_error($link));
	}
	else
	{
		if($b!='ALL')
...
?>&nbsp;<select class="tbltext" name="txtpname" style="width:250px;">
