...
winHandle=window.open('report_arrrawcrop2.php?slchk='+slchk+'&slchk2='+slchk2+'&txtcrop='+itemid+'&txtvariety='+vv+'&txtqcstatus='+txtqcstatus+'&txtgotstatus='+txtgotstatus+'&txtprodgrade='+txtprodgrade+'&sdate='+sdate+'&edate='+edate,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <?php
	...
	$crp="ALL"; $ver="ALL";
	$qry="select Distinct lotldg_crop, lotldg_variety from tbl_lot_ldg where lotldg_sstage='Raw' and lotldg_balqty > 0 and plantcode='$plantcode'";
...
//$sql_arhome=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where  lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and lotldg_sstage='Raw'  and lotldg_balqty > 0 group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
...
<tr class="tblsubtitle" height="20">
		...
		<td width="120"  align="center" valign="middle" class="smalltblheading">Lot No.</td>
			...
			<?php if($slchk=="yes") { ?>
			<td width="136"  align="center" valign="middle" class="smalltblheading">SLOC</td>
			<?php } ?>
	        ...
	        <td width="76"  align="center" valign="middle" class="smalltblheading">DoT</td>
			<td width="72"  align="center" valign="middle" class="smalltblheading">GOT Status</td>
			<td width="89"  align="center" valign="middle" class="smalltblheading">Production Location</td>
			<td width="82"  align="center" valign="middle" class="smalltblheading">State</td>
			...
			<td width="72"  align="center" valign="middle" class="smalltblheading">Arrival ID</td>
			<?php if($slchk2=="yes") { ?>
...
$sql_arr_home=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where  lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and lotldg_sstage='Raw'  and lotldg_balqty > 0 and plantcode='$plantcode' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
	...
	//$sql_arr_home=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where  lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and lotldg_sstage='Raw'  and lotldg_balqty > 0 group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
...
$sloc=$wareh.$binn.$subbinn." | ".$slups." | ".$slqty."<br/>";
	...
	$sql_arrmain=mysqli_query($link,"Select * from tblarrival where arrival_id='".$row_rr['arrival_id']."' and arrival_date>='$sdate' and arrival_date<='$edate' and plantcode='$plantcode'") or die(mysqli_error($link));
	if($tot_arrmain=mysqli_num_rows($sql_arrmain)>0)
	{
		$row_arrmain=mysqli_fetch_array($sql_arrmain);
		
		$ardt=$row_arrmain['arrival_date'];
		$transactionid="AF".$row_arrmain['arr_code']."/".$row_arrmain['yearcode']."/".$row_arrmain['arr_role'];
		$lorrynumber=$row_arrmain['trans_lorryrepno'];
		$rdate=$harvesdt;
		$ryear=substr($rdate,0,4);
		$rmonth=substr($rdate,5,2);
		$rday=substr($rdate,8,2);
...
<tr class="Light">
			...
			<td align="center" valign="middle" class="smalltbltext"><?php echo $transactionid;?></td>
			<?php if($slchk2=="yes") { ?>
...
<tr class="Dark">
			...
			<td align="center" valign="middle" class="smalltblheading" colspan="14">&nbsp;</td>
</tr>
</table>			
<br />
<?php
...
<tr >
<td height="49" align="center" valign="top"><a href="report_arrrawcrop.php"><img src="../../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<img src="../../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" />
