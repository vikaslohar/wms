...
$sql_tbl=mysqli_query($link,"select * from tbl_ci where ci_logid='".$logid."' and ci_tflg=0 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['ci_id'];	
	
	$s_sub="delete from tbl_cisub where ci_id='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_cisub_sub where ci_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
}

	$s_sub="delete from tbl_ci where ci_logid='".$logid."' and ci_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
{
winHandle=window.open('spci_note.php?itmid='+tid,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" >
	  ...
	  <td width="801" class="Mainheading" height="25"><table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#2e81c1" style="border-bottom:solid; border-bottom-color:#2e81c1" >
        <tr >
          <td width="810" height="25">&nbsp;Transaction - SP Cycle Inventory</td>
        </tr>
	    </table></td>
...
<tr height="15" class="tbltitle" >
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_cycleinventory.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
	  </tr>
	  </table></td></tr>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_ci where ci_tflg=1 and plantcode='$plantcode' order by ci_date desc, ci_code desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_ci where ci_tflg=1 and plantcode='$plantcode' order by ci_date desc, ci_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_ci where ci_tflg=1 order by ci_date desc, ci_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);*/

//$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_ci where ci_tflg=1");
//$total_results2 = mysqli_fetch_array($total_results3);
//$total_results = $total_results2[0]; 
if($tot_arr_home > 0)
{
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="950" bordercolor="#2e81c1" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="8%" align="center" valign="middle" class="smalltblheading">Merger Date</td>
              ...
              <td width="11%" align="center" valign="middle" class="smalltblheading">Old Lot No.</td>
			  <td width="11%" align="center" valign="middle" class="smalltblheading">New Lot No.</td>
             ...
             <!-- <td width="8%" align="center" valign="middle" class="smalltblheading">No. of Merged Lots</td>
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$trdate=$row_arr_home['ci_date'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;	
	
	$trid=$row_arr_home['ci_id'];
	$drole=$row_arr_home['ci_logid'];
	$yrid=$row_arr_home['ci_yearcode'];
	
$classqry=mysqli_query($link,"select cropid, cropname from tblcrop where cropid='".$row_arr_home['ci_crop']."' order by cropname") or die(mysqli_error($link));
$noticia_class=mysqli_fetch_array($classqry);

$itemqry=mysqli_query($link,"select varietyid, popularname from tblvariety where varietyid='".$row_arr_home['ci_variety']."'") or die(mysqli_error($link));
$noticia_item=mysqli_fetch_array($itemqry);	

$extlotno=""; $tnob=""; $tqty=""; $tsloc=""; $newlotno="";
$sql_eindent_sub=mysqli_query($link,"select * from tbl_cisub where ci_id='$trid' and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_eindent_sub=mysqli_fetch_array($sql_eindent_sub))
{
if($extlotno!="")
$extlotno=$extlotno."<br/>".$row_eindent_sub['cisub_lotno'];
else
$extlotno=$row_eindent_sub['cisub_lotno'];

if($newlotno!="")
$newlotno=$newlotno."<br/>".$row_eindent_sub['cisub_newlotno'];
else
$newlotno=$row_eindent_sub['cisub_newlotno'];


$nsloc=""; $wareh=""; $binn=""; $subbinn=""; $nnob=""; $nqty=""; $nob=0; $qty=0;
$sql_eindent_sub2=mysqli_query($link,"select * from tbl_cisub_sub where ci_id='".$trid."' and cisub_id='".$row_eindent_sub['cisub_id']."' and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_eindent_sub2=mysqli_fetch_array($sql_eindent_sub2))
{

$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where whid='".$row_eindent_sub2['ciss_whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);
$wareh=$row_whouse['perticulars']."/";

$sql_binn=mysqli_query($link,"select binname from tbl_bin where binid='".$row_eindent_sub2['ciss_binid']."' and whid='".$row_eindent_sub2['ciss_whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname']."/";

$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$row_eindent_sub2['ciss_subbinid']."' and binid='".$row_eindent_sub2['ciss_binid']."' and whid='".$row_eindent_sub2['ciss_whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
$subbinn=$row_subbinn['sname'];

$nnob=$row_eindent_sub2['ciss_nob'];

$dq=explode(".",$row_eindent_sub2['ciss_qty']);
if($dq[1]==000){$nqty=$dq[0];}else{$nqty=$row_eindent_sub2['ciss_qty'];}
...
<tr class="Light">
         ...
         <td width="9%" align="center" valign="middle" class="smalltbltext"><?php echo "CI".$row_arr_home['ci_code']."/".$row_arr_home['ci_yearcode']."/".$drole;?></td>
         ...
         <td width="11%" align="center" valign="middle" class="smalltbltext"><?php echo $extlotno;?></td>
		 <td width="11%" align="center" valign="middle" class="smalltbltext"><?php echo $newlotno;?></td>
         <td width="5%" align="center" valign="middle" class="smalltbltext"><?php echo $tnob?></td>
         <td width="6%" align="center" valign="middle" class="smalltbltext"><?php echo $tqty?></td>
         <td width="18%" align="center" valign="middle" class="smalltbltext"><?php echo $tsloc;?></td>
        ...
        <td width="4%" align="center" valign="middle" class="smalltbltext"><?php echo $extnob;?></td>
         ...
         <td width="5%" align="center" valign="middle" class="smalltbltext"><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $trid;?>');">SPCIN</a></td>
</tr>
<?php
}
else
