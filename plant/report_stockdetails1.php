...
<div style="overflow:scroll; height:400px; width:974px;">
  <table align="center" border="1" cellspacing="0" cellpadding="0" width="2000" bordercolor="#2e81c1" style="border-collapse:collapse" id="example">
<thead>
<tr class="tblsubtitle" height="20">
	...
	<td width="55" colspan="6" align="center" valign="middle" class="tblheading">Opening Stock</td>
	<td colspan="7" align="center" valign="middle" class="tblheading">Inward Qty</td>
	<td width="55" colspan="7" align="center" valign="middle" class="tblheading">Processing &amp; Packing Details</td>
	<td colspan="8" align="center" valign="middle" class="tblheading">Outward Qty</td>
	<td width="55" colspan="6" align="center" valign="middle" class="tblheading">Closing Stock</td>
</tr>
<tr class="tblsubtitle" height="20">
  <td width="55" align="center" valign="middle" class="tblheading">Raw Seed</td>
  <td width="55" align="center" valign="middle" class="tblheading">Condition Seed</td>
  <td width="55" align="center" valign="middle" class="tblheading">Sales Return/Return from C&amp;F</td>
  <td width="55" align="center" valign="middle" class="tblheading">Packed Seed</td>
  <td width="55" align="center" valign="middle" class="tblheading">Unsold/Undestroyed stock of Discarded Seeds</td>
  <td width="55" align="center" valign="middle" class="tblheading">Total Opening</td>
  ...
  <td width="55" align="center" valign="middle" class="tblheading">Picked for Processing</td>
  <td width="55" align="center" valign="middle" class="tblheading">Processed Qty</td>
  <td width="55" align="center" valign="middle" class="tblheading">Processing Loss</td>
  <td width="55" align="center" valign="middle" class="tblheading">Picked for Packing</td>
  <td width="55" align="center" valign="middle" class="tblheading">Pack Qty</td>
  <td width="55" align="center" valign="middle" class="tblheading">Packing Loss</td>
  <td width="55" align="center" valign="middle" class="tblheading">Pack to Condition</td>		
  ...
  <td width="55" align="center" valign="middle" class="tblheading">Packed Seed</td>
  <td width="55" align="center" valign="middle" class="tblheading">Unsold/Undestroyed stock of Discarded Seeds</td>
  <td width="55" align="center" valign="middle" class="tblheading">Total Closing</td>
</tr>
</thead>
<tbody>
<?php
...
if($crval<>"")
{
	...
	if($verval<>"")
	{
		...
		$or=0; $oc=0; $opk=0; $osr=0; $ofs=0; $xc=0; $xv=0; $xst=0; $xd=0; $xz=0; $xp=0; $xf=0; $pr=0; $prl=0; $pkl=0; $ccl=0; $rvl=0; $drl=0; $cil=0; $cr=0; $cc=0; $cpk=0; $csr=0; $isr=0; $tdfl=0; $cfs=0; $totpickprocessqty=0; $totprocessqty=0; $totprocessloss=0; $totpickpackqty=0; $totpackqty=0; $totpackloss=0; $totptoc=0;
		...
		if($t > 0)
		{
		 ...
		 //    $totpickprocessqty=0; $totprocessqty=0; $totprocessloss=0; $totpickpackqty=0; $totpackqty=0; $totpackloss=0; $totptoc=0;
...
//		echo "<br />";echo "<br />";
	...
	//	echo "Outword stock Records <br/>";
		...
		while($row_is=mysqli_fetch_array($sql_is))
		{ 
			$sql_istbl=mysqli_query($link,"select proslipsub_pqty, proslipsub_conqty, proslipsub_tlqty from tbl_proslipsub where proslipmain_id='".$row_is['proslipmain_id']."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				...
				{ 
					...
					if($qt<0)$qt=0;
					$totpickprocessqty=$totpickprocessqty+$row_is2['proslipsub_pqty'];  
					$totprocessqty=$totprocessqty+$row_is2['proslipsub_conqty']; 
					$totprocessloss=$totprocessloss+$qt;  
		...
		while($row_is=mysqli_fetch_array($sql_is))
		{ 
			$sql_istbl=mysqli_query($link,"select pnpslipsub_tlqty, pnpslipsub_packloss, pnpslipsub_packcc, pnpslipsub_pickpqty, pnpslipsub_packqty from tbl_pnpslipsub where pnpslipmain_id='".$row_is['pnpslipmain_id']."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				while($row_is2=mysqli_fetch_array($sql_istbl))
				{ 
					...
					$totpickpackqty=$totpickpackqty+$row_is2['pnpslipsub_pickpqty']; 
					$totpackqty=$totpackqty+$row_is2['pnpslipsub_packqty']; 
...
//					echo $row_is2['pnpslipsub_lotno']." PK-Loss ".($qt+$qt2+$qt3)."<br/>";
		...
		$sql_p2cm=mysqli_query($link,"select unp_id from tbl_psunpp2c where unp_date>='$stdate' and unp_date<='$etdate' and unp_tflg=1 and unp_crop='".$crval."' and unp_variety='".$verval."' and plantcode='$plantcode' order by unp_id asc") or die(mysqli_error($link));
		while($row_p2cm=mysqli_fetch_array($sql_p2cm))
		{ 
			$sql_p2cs=mysqli_query($link,"select unp_qty from tbl_psunpp2c_sub where unp_id='".$row_p2cm['unp_id']."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
			$tp2cs=mysqli_num_rows($sql_p2cs);
			if($tp2cs > 0)
			{
				while($row_p2cs=mysqli_fetch_array($sql_p2cs))
				{ 
					$qt=$row_p2cs['unp_qty']; 
					if($qt<0)$qt=0;
					$totptoc=$totptoc+$qt;  
...
//				echo $row_is['rv_lotno']."  RV-Loss  ".($ltqt+$ltqt2)."<br/>";
		...
		while($row_is=mysqli_fetch_array($sql_is))
		{ //echo $row_is['dpss_id']."  =  ";
			$sql_istbl=mysqli_query($link,"select disp_id from tbl_disp where disp_id='".$row_is['disp_id']."' and disp_dodc>='$stdate' and disp_dodc<='$etdate' and disp_partytype!='C&F' and disp_tflg=1 and plantcode='$plantcode' order by disp_id desc ") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
...
$sql_rep="Insert into tmp_pmsldgrep3 (crop, variety, vertype, oprawseed, opconseed, oppackseed, opsrseed, opfailseed, opstock, frnstock, srinstock, stinplant, stincnf, ivtin, cistock, totinstock, pickforprocess, processqty, processloss, pickforpack, packqty, packloss, ptoc, salesstock, purretstock, stoutplant, stoutcnf, tdfstock, ivtout, totloss, totoutstock, clrawseed, clconseed, clpackseed, clsrseed, clfailseed, clstock, logid, repflg, plantcode) values ('$crop1', '$verty', '$vtyp', '$or', '$oc', '$opk', '$osr', '$ofs', '$totopqty', '$totfrn', '$totsrn', '$totstp', '$totstcnf', '$totivtin', '$cirec', '$totinw', '$totpickprocessqty', '$totprocessqty', '$totprocessloss', '$totpickpackqty', '$totpackqty', '$totpackloss', '$totptoc', '$totdisp', '$totpret', '$totstpo', '$totstcnfo', '$tottdf', '$totivtout', '$totloss', '$tototw', '$cr', '$cc', '$cpk', '$csr', '$cfs', '$totclqty', '$logid', '1', '$plantcode')";
...
<tr class="Light">
	...
	<td align="center" valign="middle" class="smalltbltext"><?php echo $or?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $oc?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $osr?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $opk?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $ofs?></td>
	...
	<td align="center" valign="middle" class="smalltbltext"><?php echo $totpickprocessqty;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $totprocessqty;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $totprocessloss;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $totpickpackqty;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $totpackqty;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $totpackloss;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $totptoc;?></td>
	...
	<td align="center" valign="middle" class="smalltbltext"><?php echo $cr?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $cc?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $csr?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $cpk?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $cfs?></td>
...
<tr >
<td height="49" align="center" valign="top"><a href="report_stockdetails.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<!--<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" />-->
  <input type="hidden" name="txtinv" />&nbsp;<a href="excel-stockdetails.php?txtcrop=<?php echo $_REQUEST['txtcrop']?>&txtvariety=<?php echo $_REQUEST['txtvariety']?>&slchk=<?php echo $slchk;?>&slchk2=<?php echo $slchk2;?>&sdate=<?php echo $sdate;?>&edate=<?php echo $edate;?>" target="_blank"><img src="../images/excelicon1.jpg" border="0" height="30" width="30" class="butn" alt="Export to Excel" style="display:inline;cursor:pointer;" /></a>&nbsp;</td>
</tr>
</table>
</td><td ></td>
</tr>
