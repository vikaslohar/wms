...
if (isset($_REQUEST['scode'])) {
	...
	if ($user_login != false) 
	{
		...
		if($action=="update")
		{
			$user_trlist = $db->GetFromEditDataUpdate($scode, $jdata);
		...
		{
			$user_trlist = $db->GetFromDeleteData($scode, $jdata);
			//print_r($user_trlist); exit;
			if ($user_trlist != false) 
			{
				$response["error"] = FALSE;
			...
			{
				$response["error"] = TRUE;
				$response["msg"] = "Transaction cannot be Deleted. Please Try Again.";
				echo json_encode($response);
			}
		}
	}
