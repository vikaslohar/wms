...
//	private $conn_vnr;
	...
	public function GetTrList($scode) {
		...
		$plantcode = $this->getPlantcode($scode);
        $stmt = $this->conn_ps->prepare("SELECT  slnew_id, slnew_date, slnew_logid, slnew_fromflg, slnew_toflg, slnew_tflg  FROM tbl_slocnew WHERE slnew_tflg!=1 and slnew_logid=? AND plantcode='$plantcode' ORDER BY slnew_id DESC");
       $stmt->bind_param("s", $scode);
        $stmt->execute();
        $stmt->store_result();
