...
/*					$userSR["crop"] = $slnf_crop;
					$userSR["variety"] = $slnf_variety;
        ...
        if ($stmt->num_rows > 0) {
			...
			while($stmt->fetch())
			{
				...
				if ($stmt_2->num_rows > 0) {
					...
					while($stmt_2->fetch())
					{
						$stmt_slocfrom = $this->conn_ps->prepare("SELECT   slnf_fdate, slnf_crop, slnf_variety, slnf_stage, slnf_lotno, slnf_ups  FROM tbl_slocnew_from WHERE slnew_id=? and slnf_id=? ");
						$stmt_slocfrom->bind_param("ii", $slnew_id, $slnf_id);
						$stmt_slocfrom->execute();
						$stmt_slocfrom->store_result();
						//if ($stmt_slocfrom->num_rows>0) {
						$stmt_slocfrom->bind_result($slnf_fdate, $slnf_crop, $slnf_variety, $slnf_stage, $slnf_lotno, $slnf_ups);
						$stmt_slocfrom->fetch();
						//}
						$stmt_slocfrom->close();
									...
									while($stmt_ldgraw2->fetch())
									{
										...
										if ($stmt_ldgraw3->num_rows > 0) {
											...
											{ 
												...
												$stmt_arrsub11->bind_param("ssisiiiisisisssssssssssssisssssisiisss", $lotno, $typ2, $slnew_id, $dt, $slnt_twh, $slnt_tbin, $slnt_tsbin, $lotldg_balbags, $lotldg_balqty, $slnt_tnob, $slnt_tqty, $slnt_tbalnob, $slnt_tbalqty, $yearcode, $lotldg_variety, $lotldg_crop, $lotldg_sstage, $lotldg_sstatus, $lotldg_moisture, $lotldg_gemp, $lotldg_vchk, $lotldg_got1, $lotldg_qc, $lotldg_qctestdate, $orlot, $lotldg_gs, $lotldg_resverstatus, $lotldg_revcomment, $lotldg_gottestdate, $lotldg_got, $lotldg_srtyp, $lotldg_srflg, $lotldg_genpurity, $lotldg_mergerflg, $lotldg_unlistflg, $leduration, $leupto, $plantcode);
