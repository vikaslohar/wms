			...
			while($stmt->fetch())
			{
				...
				if ($stmt_2->num_rows > 0) {
					...
					while($stmt_2->fetch())
					{
						...
						$slnttqty=0;
						$stmt_slocto = $this->conn_ps->prepare("SELECT  slnt_tqty  FROM tbl_slocnew_to WHERE slnew_id=? and slnf_id=? ");
						$stmt_slocto->bind_param("ii", $slnew_id, $slnf_id);
						$stmt_slocto->execute();
						$stmt_slocto->store_result();
						$stmt_slocto->bind_result($slnt_tqty);
...
// New Code ------------------------------------------------	
	...
	public function GetTranSLOCLotList($scode,$qrcode) {
...
//return "SELECT binname, binid  FROM tbl_bin WHERE whid = $whid and binname = $binnm ";
	...
	//return $jdata;
		...
		foreach($phpArray as $lotary)
		{
			...
			$stmt = $this->conn_ps->prepare("SELECT  slnew_id, slnew_date, slnew_logid, slnew_fromflg, slnew_toflg, slnew_tflg  FROM tbl_slocnew WHERE slnew_toflg=0 and slnew_tflg!=1 and slnew_logid=? AND plantcode='$plantcode' ORDER BY slnew_id DESC");
		   	$stmt->bind_param("s", $scode);
			$stmt->execute();
			$stmt->store_result();
			$userSR = array(); $user24=array();
