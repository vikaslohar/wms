						...
						if ($stmt_ldgraw3->num_rows > 0) {
							...
							{ 
								...
								if ($stmt_ldgraw4->num_rows > 0) {
									...
									$stmt_slnf = $this->conn_ps->prepare("SELECT slnf_id FROM tbl_slocnew_from WHERE slnf_crop=? and slnf_variety=? and slnf_lotno=?  and slnt_flg!=1 and slnf_fwh=? and slnf_fbin=? and slnf_fsbin=? order by slnf_id ASC");
		...
		//}	
			...
			//return $stmt_ldgraw2->num_rows;
											if ($stmt_ldgraw2->num_rows > 0) {
												...
												while($stmt_ldgraw2->fetch())
												{
													$stmt_ldgraw3 = $this->conn_ps->prepare("SELECT lotno, trtype, lotldg_id, lotldg_trdate, whid, binid, subbinid, opnop, optqty, nop, tqty, balnop, balqty, yearcode, lotldg_variety, lotldg_crop, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_qctestdate, orlot, lotldg_resverstatus, lotldg_revcomment, lotldg_gottestdate, lotldg_got, lotldg_srtyp, lotldg_srflg, plantcode, opnomp, nomp, balnomp FROM tbl_lot_ldg_pack WHERE lotdgp_id = ?  and plantcode=?");
													$stmt_ldgraw3->bind_param("is", $lotldgid, $plantcode);
													$result2=$stmt_ldgraw3->execute();
													$stmt_ldgraw3->store_result();
													if ($stmt_ldgraw3->num_rows > 0) {
														$stmt_ldgraw3->bind_result($lotldg_lotno, $lotldg_trtype, $lotldg_trid, $lotldg_trdate, $lotldg_whid, $lotldg_binid, $lotldg_subbinid, $lotldg_opbags, $lotldg_opqty, $lotldg_trbags, $lotldg_trqty, $lotldg_balbags, $lotldg_balqty, $yearcode, $lotldg_variety, $lotldg_crop, $lotldg_sstage, $lotldg_sstatus, $lotldg_moisture, $lotldg_gemp, $lotldg_vchk, $lotldg_got1, $lotldg_qc, $lotldg_qctestdate, $orlot, $lotldg_resverstatus, $lotldg_revcomment, $lotldg_gottestdate, $lotldg_got, $lotldg_srtyp, $lotldg_srflg, $plantcode, $opnomp, $nomp, $balnomp);
														//looping through all the records
														while($stmt_ldgraw3->fetch())
														{ 
															$typ='SLOCSUO'; $typ2='SLOCSUC';
															...
															$stmt_arrsub = $this->conn_ps->prepare("Insert into tbl_lot_ldg_pack (lotno, trtype, lotldg_id, lotldg_trdate, whid, binid, subbinid, opnop, optqty, nop, tqty, balnop, balnomp, yearcode, lotldg_variety, lotldg_crop, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_qctestdate, orlot, lotldg_resverstatus, lotldg_revcomment, lotldg_gottestdate, lotldg_got, lotldg_srtyp, lotldg_srflg, plantcode, opnomp, nomp, balqty) Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
															$stmt_arrsub->bind_param("ssisiiiisisissssssssssssisssssissss", $lotno, $typ, $slnew_id, $dt, $slnf_fwh, $slnf_fbin, $slnf_fsbin, $lotldg_balbags, $lotldg_balqty, $slnt_tnob, $slnt_tqty, $balb, $balmp, $yearcode, $lotldg_variety, $lotldg_crop, $lotldg_sstage, $lotldg_sstatus, $lotldg_moisture, $lotldg_gemp, $lotldg_vchk, $lotldg_got1, $lotldg_qc, $lotldg_qctestdate, $orlot, $lotldg_resverstatus, $lotldg_revcomment, $lotldg_gottestdate, $lotldg_got, $lotldg_srtyp, $lotldg_srflg, $plantcode, $balnomp, $slnt_tnomp, $balq);
															$result_arrsub = $stmt_arrsub->execute();
															if($result_arrsub){$flg=1;}
															//$arrsub_id=$stmt_arrsub->insert_id;
															$stmt_arrsub->close();
															...
															$stmt_arrsub11->bind_param("ssisiiiisisissssssssssssisssssissss", $lotno, $typ2, $slnew_id, $dt, $slnt_twh, $slnt_tbin, $slnt_tsbin, $zero, $zero, $slnt_tnob, $slnt_tqty, $slnt_tnob, $slnt_tnomp, $yearcode, $lotldg_variety, $lotldg_crop, $lotldg_sstage, $lotldg_sstatus, $lotldg_moisture, $lotldg_gemp, $lotldg_vchk, $lotldg_got1, $lotldg_qc, $lotldg_qctestdate, $orlot, $lotldg_resverstatus, $lotldg_revcomment, $lotldg_gottestdate, $lotldg_got, $lotldg_srtyp, $lotldg_srflg, $plantcode, $zero, $slnt_tnomp, $slnt_tqty);
															$result_arrsub11 = $stmt_arrsub11->execute();
															if($result_arrsub11){$flg=2;}
															//$arrsub_id=$stmt_arrsub11->insert_id;
															$stmt_arrsub11->close();
...
//return $slnt_id;
							if($whtype=="Regular")
							{
								...
								//if($slnt_variety!=$variety && $slnt_twh==$slocwh && $slnt_tbin=$slocbin && $slnt_tsbin==$slocsubbin ) {$slflg=4;}
