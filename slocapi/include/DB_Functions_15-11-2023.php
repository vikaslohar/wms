...
//	private $conn_vnr;
	...
	public function GetTrList($scode) {
		...
		$plantcode = $this->getPlantcode($scode);
        $stmt = $this->conn_ps->prepare("SELECT  slnew_id, slnew_date, slnew_logid, slnew_fromflg, slnew_toflg, slnew_tflg  FROM tbl_slocnew WHERE slnew_tflg!=1 AND  AND slnew_fromflg!=1 and slnew_logid=? AND plantcode='$plantcode' ORDER BY slnew_id DESC");
       $stmt->bind_param("s", $scode);
        $stmt->execute();
        $stmt->store_result();
...
/*					$userSR["crop"] = $slnf_crop;
	...
	public function GetTrFlagList($scode) {
		$plantcode = $this->getPlantcode($scode);
        $stmt = $this->conn_ps->prepare("SELECT  slnew_id, slnew_date, slnew_logid, slnew_fromflg, slnew_toflg, slnew_tflg  FROM tbl_slocnew WHERE slnew_logid=? AND plantcode='$plantcode' ORDER BY slnew_id DESC");
       $stmt->bind_param("s", $scode);
        $stmt->execute();
        $stmt->store_result();
		$userSR = array(); $user24=array();
	...
	//return "SELECT sname, sid  FROM tbl_subbin WHERE whid = $whid and binid = $binid  and plantcode='$plantcode' and sname='$slocsubbin' order by sname ASC";		
		...
		foreach($phpArray as $lotary)
		{
			...
			if ($stmt_slnmain->num_rows == 0) {
				...
				$stmt_slnmain2 = $this->conn_ps->prepare("SELECT MAX(slnew_tcode) FROM tbl_slocnew WHERE arrival_type = ? AND yearcode = ? AND plantcode=?");
				$stmt_slnmain2->bind_param("sss", $trtype, $ycode, $plantcode);
...
//return "Insert into tbl_slocnew_to ( slnew_id, slnf_id, slnt_tdate, slnt_twh, slnt_tbin, slnt_tsbin, slnt_textnob, slnt_textqty, slnt_tnob, slnt_tqty, slnt_tbalnob, slnt_tbalqty  ) Values('$slnew_id', '$slnf_id', '$dt', '$whid', '$binid', '$subbinid', '$extnob', '$extqty', '$nob', '$qty', '$balnob', '$balqty') ";					
	...
	//return $jdata;
		...
		$stmt = $this->conn_ps->prepare("SELECT  slnew_id, slnew_date, slnew_logid, slnew_fromflg, slnew_toflg, slnew_tflg  FROM tbl_slocnew WHERE slnew_fromflg=1 and slnew_toflg=2 and slnew_tflg!=1 and slnew_logid=? AND plantcode='$plantcode' ORDER BY slnew_id DESC");
		$stmt->bind_param("s", $scode);
		$stmt->execute();
		$stmt->store_result();
		$userSR = array(); $user24=array();
