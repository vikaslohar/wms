       ...
       $stmt->bind_param("s", $scode);
        ...
        if ($stmt->num_rows > 0) {
			...
			while($stmt->fetch())
			{
				...
				$slnf_id=0; $slnew_id=0; $slnf_fdate=''; $slnf_fwh=''; $slnf_fbin=''; $slnf_fsbin=''; $slnf_fextnob=''; $slnf_fextnomp=''; $slnf_fextwb=''; $slnf_fextqty=''; $slnf_fnob=''; $slnf_fnomp=''; $slnf_fwb=''; $slnf_fqty=''; $slnf_fbalnob=''; $slnf_fbalnomp=''; $slnf_fbalwb=''; $slnf_fbalqty ='';
...
//return "SELECT binname, binid  FROM tbl_bin WHERE whid = $whid and binname = $binnm ";
	...
	public function GetFromdataUpdate($scode, $slocwh, $slocbin, $jdata) {
	//return $jdata;
		$plantcode = $this->getPlantcode($scode);
		$flg=0; $lots=array();
		$dt=date("Y-m-d");
		...
		foreach($phpArray as $lotary)
		{
			...
			$stmtwh = $this->conn_ps->prepare("SELECT perticulars,whid FROM tbl_warehouse where perticulars = ?");
			$stmtwh->bind_param("s", $wh, $plantcode);
			$resultwh=$stmtwh->execute();
			$stmtwh->store_result();
			if ($stmtwh->num_rows > 0) {
				$stmtwh->bind_result($whperticulars,$whid);
	...
	//return "SELECT binname, binid  FROM tbl_bin WHERE whid = $whid and binname = $binnm ";
			$stmtbin = $this->conn_ps->prepare("SELECT binname, binid  FROM tbl_bin WHERE whid = ? and binname = ? ");
			$stmtbin->bind_param("is", $wh, $bin, $plantcode);
			$resultbin=$stmtbin->execute();
			$stmtbin->store_result();
			if ($stmtbin->num_rows > 0) {
				$stmtbin->bind_result($binname, $binid);
			...
			$stmt_sbin->bind_param("iiss", $wh, $bin, $subbin, $plantcode);
			$result2=$stmt_sbin->execute();
			$stmt_sbin->store_result();
			if ($stmt_sbin->num_rows > 0) {
				$stmt_sbin->bind_result($subbinname, $subbinid);
			...
			if ($stmt->num_rows > 0) {
				...
				while($stmt->fetch())
				{
					...
					$stmt_arrsub->bind_param("isssss1111s1s1s", $slnew_id, $dt, $cropid, $varietyid, $stage, $lotno, $whid, $binid, $subbinid, $extnob, $extqty, $nob, $qty, $balnob, $balqty);
					$result_arrsub = $stmt_arrsub->execute();
					$arrsub_id=$stmt_arrsub->insert_id;
					$stmt_arrsub->close();
				}
		...
		if($flg>0)
		{
			$stmt60 = $this->conn_ps->prepare("Update tbl_slocnew SET slnew_fromflg=2 where arrival_id = ? ");
