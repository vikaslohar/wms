			...
			while($stmt->fetch())
			{
				...
				if ($stmt_2->num_rows > 0) {
					...
					while($stmt_2->fetch())
					{
						...
						$slnttnob=0; $slnttqty=0;
						$stmt_slocto = $this->conn_ps->prepare("SELECT  slnt_tqty, slnt_tnob  FROM tbl_slocnew_to WHERE slnew_id=? and slnf_id=? ");
						$stmt_slocto->bind_param("ii", $slnew_id, $slnf_id);
						$stmt_slocto->execute();
						$stmt_slocto->store_result();
						$stmt_slocto->bind_result($slnt_tqty, $slnt_tnob);
						...
						if ($stmt_ldgraw3->num_rows > 0) {
							...
							{ 
								...
								if ($stmt_ldgraw4->num_rows > 0) {
									...
									$stmt_slnf = $this->conn_ps->prepare("SELECT slnf_id FROM tbl_slocnew_from WHERE slnf_crop=? and slnf_variety=? and slnf_lotno=?  and slnf_fbalqty=0 and slnf_fwh=? and slnf_fbin=? and slnf_fsbin=? order by slnf_id ASC");
...
//return "Insert into tbl_slocnew_from (slnew_id, slnf_fdate, slnf_crop, slnf_variety, slnf_stage, slnf_lotno, slnf_fwh, slnf_fbin, slnf_fsbin, slnf_fextnob, slnf_fextqty, slnf_fnob, slnf_fqty, slnf_fbalnob, slnf_fbalqty ) Values('$slnew_id', '$dt', '$cropid', '$varietyid', '$stage', '$lot_no', '$whid', '$binid', '$subbinid', '$extnob', '$extqty', '$nob', '$qty', '$balnob', '$balqty') ";
					...
					if ($stmt_slnf->num_rows == 0) {
						...
						$stmt_arrsub = $this->conn_ps->prepare("Insert into tbl_slocnew_from (slnew_id, slnf_fdate, slnf_crop, slnf_variety, slnf_stage, slnf_lotno, slnf_fwh, slnf_fbin, slnf_fsbin, slnf_fextnob, slnf_fextqty, slnf_fnob, slnf_fqty, slnf_fbalnob, slnf_fbalqty ) Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
						$stmt_arrsub->bind_param("isssssiiiisisis", $slnew_id, $dt, $cropid, $varietyid, $stage, $lot_no, $whid, $binid, $subbinid, $extnob, $extqty, $nob, $qty, $balnob, $balqty);
						$result_arrsub = $stmt_arrsub->execute();
	...
	public function GetTodataUpdate($scode, $slocwh, $slocbin, $slocsubbin, $jdata) {
	//return $jdata;
		$plantcode = $this->getPlantcode($scode);
		$flg=0; $lots=array();
		$dt=date("Y-m-d");
	...
	//return "SELECT sname, sid  FROM tbl_subbin WHERE whid = $whid and binid = $binid  and plantcode='$plantcode' and sname='$slocsubbin' order by sname ASC";		
		...
		foreach($phpArray as $lotary)
		{
			...
			/*
			$stmt_slnmain = $this->conn_ps->prepare("SELECT  slnew_id, slnew_date, slnew_logid, slnew_fromflg, slnew_toflg, slnew_tflg  FROM tbl_slocnew WHERE slnew_fromflg=1 and slnew_toflg!=1 and slnew_tflg!=1 and slnew_logid=? AND plantcode='$plantcode' ORDER BY slnew_id DESC");
		   	$stmt_slnmain->bind_param("s", $scode);
			$stmt_slnmain->execute();
			$stmt_slnmain->store_result();
			if ($stmt_slnmain->num_rows == 0) {
			
				$stmt_slnmain2 = $this->conn_ps->prepare("SELECT MAX(slnew_tcode) FROM tbl_slocnew WHERE plantcode=?");
				$stmt_slnmain2->bind_param("s", $plantcode);
				$stmt_slnmain2->execute();
				$stmt_slnmain2->store_result();
				$arrivalcode=0;
				if ($stmt_slnmain2->num_rows > 0) {
					// user existed 
					$stmt_slnmain2->bind_result($arrival_code);
					$stmt_slnmain2->fetch();
					$arrivalcode=$arrival_code+1;
					$stmt_slnmain2->close();
				} else {
				...
				$stmt_arrm = $this->conn_ps->prepare("Insert into tbl_slocnew ( slnew_date, slnew_tcode, slnew_logid, plantcode ) Values(?,?,?,?) ");
				$stmt_arrm->bind_param("ssss", $dt, $arrivalcode, $scode, $plantcode);
				$result_arrm = $stmt_arrm->execute();
				$arrmain_id=$stmt_arrm->insert_id;
				$stmt_arrm->close();
			}*/
...
//return "select slnf_id from tbl_slocnew_from where slnew_id=$slnew_id and slnf_crop=$cropid and slnf_variety=$varietyid and slnf_lotno='$lot_no' ";
					...
					if ($stmt_slocto->num_rows==0) {
//return "Insert into tbl_slocnew_to ( slnew_id, slnf_id, slnt_tdate, slnt_twh, slnt_tbin, slnt_tsbin, slnt_textnob, slnt_textqty, slnt_tnob, slnt_tqty, slnt_tbalnob, slnt_tbalqty  ) Values('$slnew_id', '$slnf_id', '$dt', '$whid', '$binid', '$subbinid', '$extnob', '$extqty', '$nob', '$qty', '$balnob', '$balqty') ";					
						$stmt_arrsub = $this->conn_ps->prepare("Insert into tbl_slocnew_to ( slnew_id, slnf_id, slnt_tdate, slnt_twh, slnt_tbin, slnt_tsbin, slnt_textnob, slnt_textqty, slnt_tnob, slnt_tqty, slnt_tbalnob, slnt_tbalqty  ) Values(?,?,?,?,?,?,?,?,?,?,?,?) ");
						$stmt_arrsub->bind_param("iisiiiisisis", $slnew_id, $slnf_id, $dt, $whid, $binid, $subbinid, $extnob, $extqty, $nob, $qty, $balnob, $balqty);
						$result_arrsub = $stmt_arrsub->execute();
						if($result_arrsub){$flg=1;}
						$arrsub_id=$stmt_arrsub->insert_id;
						$stmt_arrsub->close();
	...
	public function GetToSLOCchk($scode, $slocwh, $slocbin, $slocsubbin, $crop, $variety, $lotno, $stage) {
	
		$plantcode = $this->getPlantcode($scode); $slflg=0;
        $stmt = $this->conn_ps->prepare("SELECT  slnew_id FROM tbl_slocnew WHERE slnew_tflg!=1 and slnew_logid=? AND plantcode='$plantcode' ORDER BY slnew_id DESC");
       $stmt->bind_param("s", $scode);
        $stmt->execute();
        $stmt->store_result();
		$userSR = array(); $user24=array();
        ...
        if ($stmt->num_rows > 0) {
			...
			$stmt->fetch();
			{
				...
				$stmt_2->bind_param("issss", $slnew_id, $crop, $variety, $stage, $lotno);
				$result2=$stmt_2->execute();
				$stmt_2->store_result();
				if ($stmt_2->num_rows > 0) {
					$stmt_2->bind_result($slnf_id, $whid, $binid, $subbinid);
					//looping through all the records
					while($stmt_2->fetch())
					{
						...
						$stmt_slocto = $this->conn_ps->prepare("SELECT  slnt_id  FROM tbl_slocnew_to WHERE slnew_id=? and slnf_id=? ");
						$stmt_slocto->bind_param("ii", $slnew_id, $slnf_id);
						$stmt_slocto->execute();
						$stmt_slocto->store_result();
						...
						if ($stmt_slocto->num_rows==0) {
							...
							if($stage=="Raw")
							{
								$stmt_ldgraw = $this->conn_ps->prepare("SELECT distinct(lotldg_variety) FROM tbl_lot_ldg WHERE lotldg_whid=? and lotldg_binid=? and plantcode=? and lotldg_subbinid=?  and lotldg_variety!=? and lotldg_sstage='Raw'");
								$stmt_ldgraw->bind_param("iisss", $whid, $binid, $plantcode, $subbinid, $varietyid);
								$result2=$stmt_ldgraw->execute();
								$stmt_ldgraw->store_result(); 
								if ($stmt_ldgraw->num_rows > 0) {
									$stmt_ldgraw->bind_result($subbinid);
							...
							{
								$stmt_ldgraw = $this->conn_ps->prepare("SELECT distinct(lotldg_variety) FROM tbl_lot_ldg WHERE lotldg_whid=? and lotldg_binid=? and plantcode=? and lotldg_subbinid=?  and lotldg_variety!=? and lotldg_sstage='Condition'");
								$stmt_ldgraw->bind_param("iisss", $whid, $binid, $plantcode, $subbinid, $varietyid);
								$result2=$stmt_ldgraw->execute();
								$stmt_ldgraw->store_result(); 
								if ($stmt_ldgraw->num_rows > 0) {
							...
							{
								$stmt_ldgraw = $this->conn_ps->prepare("SELECT distinct(lotldg_variety) FROM tbl_lot_ldg_pack WHERE whid=? and binid=? and plantcode=? and subbinid=?  and lotldg_variety!=?");
								$stmt_ldgraw->bind_param("iisss", $whid, $binid, $plantcode, $subbinid, $varietyid);
								$result2=$stmt_ldgraw->execute();
								$stmt_ldgraw->store_result(); 
								if ($stmt_ldgraw->num_rows > 0) {
										...
										if ($stmt_ldgraw2->num_rows > 0) {
											...
											while($stmt_ldgraw2->fetch())
											{
												...
												if ($stmt_ldgraw3->num_rows > 0) {
													...
													while($stmt_ldgraw3->fetch())
													{ 
														$stmt_ldgraw4 = $this->conn_ps->prepare("SELECT lotldg_crop, lotldg_variety, lotno, balbags, balqty FROM tbl_lot_ldg_pack WHERE lotdgp_id = ? and balqty > 0 and plantcode=? ");
														$stmt_ldgraw4->bind_param("is", $lotldgid, $plantcode);
														$result2=$stmt_ldgraw4->execute();
														$stmt_ldgraw4->store_result();
														if ($stmt_ldgraw4->num_rows > 0) {
	...
	public function GetTodataFinalSubmit($scode) {
	//return $jdata;
       ...
       $stmt->bind_param("s", $scode);
        ...
        if ($stmt->num_rows > 0) {
			...
			while($stmt->fetch())
			{
				...
				$slflag=0;
				$stmt_slnf = $this->conn_ps->prepare("SELECT  slnf_id, slnf_fqty  FROM tbl_slocnew_from WHERE slnew_id = ? ");
				$stmt_slnf->bind_param("i", $slnew_id);
				$result2=$stmt_slnf->execute();
				$stmt_slnf->store_result();
				if ($stmt_slnf->num_rows > 0) {
					$stmt_slnf->bind_result($slnf_id, $slnf_fqty);
					//looping through all the records
				...
				if($slflag==0)
				{
					...
					$stmt_2 = $this->conn_ps->prepare("SELECT  slnew_id, slnf_id, slnt_tdate, slnt_twh, slnt_tbin, slnt_tsbin, slnt_textnob, slnt_textqty, slnt_tnob, slnt_tqty, slnt_tbalnob, slnt_tbalqty FROM tbl_slocnew_to WHERE slnew_id = ? order by slnf_id ASC");
					$stmt_2->bind_param("i", $slnew_id);
					$result2=$stmt_2->execute();
					$stmt_2->store_result();
					if ($stmt_2->num_rows > 0) {
						...
						while($stmt_2->fetch())
						{
							$stmt_slocfrom = $this->conn_ps->prepare("SELECT   slnf_fdate, slnf_crop, slnf_variety, slnf_stage, slnf_lotno, slnf_ups,  slnf_fwh, slnf_fbin, slnf_fsbin, slnf_fnob, slnf_fqty, slnf_fbalnob, slnf_fbalqty  FROM tbl_slocnew_from WHERE slnew_id=? and slnf_id=?  order by slnf_id ASC");
							$stmt_slocfrom->bind_param("ii", $slnew_id, $slnf_id);
							$stmt_slocfrom->execute();
							$stmt_slocfrom->store_result();
							//if ($stmt_slocfrom->num_rows>0) {
							$stmt_slocfrom->bind_result($slnf_fdate, $slnf_crop, $slnf_variety, $slnf_stage, $slnf_lotno, $slnf_ups,  $slnf_fwh, $slnf_fbin, $slnf_fsbin, $slnf_fnob, $slnf_fqty, $slnf_fbalnob, $slnf_fbalqty );
							$stmt_slocfrom->fetch();
							//}
							$stmt_slocfrom->close();
										...
										while($stmt_ldgraw2->fetch())
										{
											...
											if ($stmt_ldgraw3->num_rows > 0) {
												...
												{ 
													...
													$stmt_arrsub->bind_param("ssisiiiisisisssssssssssssisssssisiisss", $lotno, $typ, $slnew_id, $dt, $slnf_fwh, $slnf_fbin, $slnf_fsbin, $lotldg_balbags, $lotldg_balqty, $slnf_fnob, $slnf_fqty, $slnf_fbalnob, $slnf_fbalqty, $yearcode, $lotldg_variety, $lotldg_crop, $lotldg_sstage, $lotldg_sstatus, $lotldg_moisture, $lotldg_gemp, $lotldg_vchk, $lotldg_got1, $lotldg_qc, $lotldg_qctestdate, $orlot, $lotldg_gs, $lotldg_resverstatus, $lotldg_revcomment, $lotldg_gottestdate, $lotldg_got, $lotldg_srtyp, $lotldg_srflg, $lotldg_genpurity, $lotldg_mergerflg, $lotldg_unlistflg, $leduration, $leupto, $plantcode);
													$result_arrsub = $stmt_arrsub->execute();
													if($result_arrsub){$flg=1;}
													//$arrsub_id=$stmt_arrsub->insert_id;
													$stmt_arrsub->close();
													...
													$stmt_arrsub11->bind_param("ssisiiiisisisssssssssssssisssssisiisss", $lotno, $typ2, $slnew_id, $dt, $slnt_twh, $slnt_tbin, $slnt_tsbin, $lotldg_balbags, $lotldg_balqty, $slnt_tnob, $slnt_tqty, $slnt_tnob, $slnt_tqty, $yearcode, $lotldg_variety, $lotldg_crop, $lotldg_sstage, $lotldg_sstatus, $lotldg_moisture, $lotldg_gemp, $lotldg_vchk, $lotldg_got1, $lotldg_qc, $lotldg_qctestdate, $orlot, $lotldg_gs, $lotldg_resverstatus, $lotldg_revcomment, $lotldg_gottestdate, $lotldg_got, $lotldg_srtyp, $lotldg_srflg, $lotldg_genpurity, $lotldg_mergerflg, $lotldg_unlistflg, $leduration, $leupto, $plantcode);
													$result_arrsub11 = $stmt_arrsub11->execute();
													if($result_arrsub11){$flg=2;}
...
//return "Insert into tbl_slocnew_from (slnew_id, slnf_fdate, slnf_crop, slnf_variety, slnf_stage, slnf_lotno, slnf_fwh, slnf_fbin, slnf_fsbin, slnf_fextnob, slnf_fextqty, slnf_fnob, slnf_fqty, slnf_fbalnob, slnf_fbalqty ) Values('$slnew_id', '$dt', '$cropid', '$varietyid', '$stage', '$lot_no', '$whid', '$binid', '$subbinid', '$extnob', '$extqty', '$nob', '$qty', '$balnob', '$balqty') ";
					...
					$result_slnf=$stmt_slnf->execute();
					$stmt_slnf->store_result();
//return " No. of Rows ".$stmt_slnf->num_rows;
					if ($stmt_slnf->num_rows > 0) {
						$stmt_slnf->bind_result($slnf_id, $slnewid);
						$stmt_slnf->fetch();
						
						$stmt_arrsub = $this->conn_ps->prepare("UPDATE tbl_slocnew_from  SET  slnf_fextnob=?, slnf_fextqty=?, slnf_fnob=?, slnf_fqty=?, slnf_fbalnob=?, slnf_fbalqty=? where slnf_id=?  ");
						$stmt_arrsub->bind_param("isisisi", $extnob, $extqty, $nob, $qty, $balnob, $balqty, $slnf_id);
						$result_arrsub = $stmt_arrsub->execute();
						if($result_arrsub){$flg=1;}
						$arrsub_id=$stmt_arrsub->insert_id;
						$stmt_arrsub->close();
...
//return "select slnf_id from tbl_slocnew_from where slnew_id=$slnew_id and slnf_crop=$cropid and slnf_variety=$varietyid and slnf_lotno='$lot_no' ";
					...
					$stmt_slocfrom->fetch();
					$stmt_slocfrom->close();
//return "SELECT  slnt_id  FROM tbl_slocnew_to WHERE slnew_id=$slnew_id and slnf_id=$slnf_id, and slnt_twh=$whid and slnt_tbin=$binid and slnt_tsbin=$subbinid";
					$stmt_slocto = $this->conn_ps->prepare("SELECT  slnt_id  FROM tbl_slocnew_to WHERE slnew_id=? and slnf_id=? and slnt_twh=? and slnt_tbin=? and slnt_tsbin=?");
					$stmt_slocto->bind_param("iiiii", $slnew_id, $slnf_id, $whid, $binid, $subbinid);
					$stmt_slocto->execute();
					$stmt_slocto->store_result();
					//return "No. of Rows ".$stmt_slocto->num_rows;
					if ($stmt_slocto->num_rows>0) {
						$stmt_slocto->bind_result($slnt_id);
						$stmt_slocto->fetch();
//return "UPDATE tbl_slocnew_to SET slnt_textnob=$extnob, slnt_textqty=$extqty, slnt_tnob=$nob, slnt_tqty=$qty, slnt_tbalnob=$balnob, slnt_tbalqty=$balqty where slnt_id=$slnt_id) ";
						$stmt_arrsub = $this->conn_ps->prepare("UPDATE tbl_slocnew_to SET slnt_textnob=?, slnt_textqty=?, slnt_tnob=?, slnt_tqty=?, slnt_tbalnob=?, slnt_tbalqty=? where slnt_id=? ");
						$stmt_arrsub->bind_param("isisisi", $extnob, $extqty, $nob, $qty, $balnob, $balqty, $slnt_id);
						$result_arrsub = $stmt_arrsub->execute();
						if($result_arrsub){$flg=1;}
						$arrsub_id=$stmt_arrsub->insert_id;
						$stmt_arrsub->close();
					...
					while($stmt_ldgraw2->fetch())
					{
						...
						if ($stmt_ldgraw3->num_rows > 0) {
							...
							{ 
								...
								$stmt_ldgraw4 = $this->conn_ps->prepare("SELECT lotldg_crop, lotldg_variety, lotldg_sstage, lotno, balnop, balnomp, balqty, packtype, whid, binid, subbinid FROM tbl_lot_ldg_pack WHERE lotdgp_id = ? and balqty>0 and plantcode=? ");
								$stmt_ldgraw4->bind_param("is", $lotldgid, $plantcode);
								$result2=$stmt_ldgraw4->execute();
								$stmt_ldgraw4->store_result();
								if ($stmt_ldgraw4->num_rows > 0) {
									$stmt_ldgraw4->bind_result($lotldg_crop, $lotldg_variety, $lotldg_sstage, $lotldg_lotno, $lotldg_balbags, $balnomp, $balqty, $packtype, $whid, $binid, $subbinid);
									$stmt_ldgraw4->fetch();
									if($balqty>0)
									{
										$stmt_crop = $this->conn_ps->prepare("select cropid, cropname from tblcrop where cropid=?");
