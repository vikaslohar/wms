...
$mid=""; $cnt=0; $d=1;
$sql_srretm=mysqli_query($link,"select * from tbl_salesrv where salesr_state='".$txtstatesl."' and salesr_date<='".$edt."' and salesr_date>='".$sdt."' and salesr_trtype='Sales Return' and plantcode='$plantcode' ") or die(mysqli_error($link));
...
$sql_srretsub25=mysqli_query($link,"select distinct salesrs_rettype from tbl_salesrv_sub where salesrs_variety='".$row_srrets['salesrs_variety']."' and salesr_id IN ($mid) and salesrs_vflg=1 and plantcode='$plantcode' ") or die(mysqli_error($link));

while($row_srretsub25=mysqli_fetch_array($sql_srretsub25))
{
$sql_srretsub2=mysqli_query($link,"select distinct salesrs_newlot from tbl_salesrv_sub where salesrs_rettype='".$row_srretsub25['salesrs_rettype']."' and salesrs_variety='".$row_srrets['salesrs_variety']."' and salesr_id IN ($mid) and salesrs_vflg=1 and plantcode='$plantcode' ") or die(mysqli_error($link));
...
//echo $lotnc;
$sql_srretsub=mysqli_query($link,"select * from tbl_salesrv_sub where salesrs_variety='".$row_srrets['salesrs_variety']."' and salesr_id IN ($mid) and plantcode='$plantcode' and salesrs_vflg=1") or die(mysqli_error($link));
