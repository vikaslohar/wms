...
<title>Viewer - Periodical Party wise Sales Return Report</title>
...
winHandle=window.open('report_partywisesr2.php?&txtcrop='+itemid+'&txtvariety='+vv+'&sdate='+sdate+'&edate='+edate+'&txtptype='+txtptype+'&txtcountryl='+txtcountryl+'&txtpp='+txtpp+'&txtstatesl='+txtstatesl+'&locationname='+locationname+'&txtstfp='+txtstfp,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#ef0388" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#ef0388" style="border-bottom:solid; border-bottom-color:#ef0388" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Report - Periodical Party wise Sales Return Report</td>
	    </tr></table></td>
...
<tr class="light" height="20">
  <td align="center" class="tblheading">Periodical Party wise Sales Return Report</td>
</tr>
</table>
<table align="center" border="0" width="750" cellspacing="0" cellpadding="0" bordercolor="#ef0388" style="border-collapse:collapse" > 
<tr class="light" height="20">
...
<tr class="light" height="20">
  ...
  <td width="26%" align="right" class="tblheading">Location:&nbsp;<?php echo $location;?></td>
...
<?php
$srno=1; $mid=""; $cnt=0;
$sql_srretm=mysqli_query($link,"select * from tbl_salesrv where salesr_party='".$txtstfp."' and salesr_date<='".$edt."' and salesr_date>='".$sdt."' and salesr_trtype='Sales Return' ") or die(mysqli_error($link));
...
if($mid==""){$mid='NULL';
$sqlsrrets="select distinct salesrs_variety, salesrs_crop from tbl_salesrv_sub where salesr_id=$mid ";}
else{
$sqlsrrets="select distinct salesrs_variety, salesrs_crop from tbl_salesrv_sub where salesr_id IN ($mid) ";}
if($crop!="ALL")
{
...
$sql_srretsub2=mysqli_query($link,"select distinct salesrs_newlot, salesrs_rettype from tbl_salesrv_sub where salesrs_variety='".$row_srrets['salesrs_variety']."' and salesrs_vflg=1 and salesr_id IN ($mid)") or die(mysqli_error($link));
...
$sql_srretsub=mysqli_query($link,"select * from tbl_salesrv_sub where salesrs_variety='".$row_srrets['salesrs_variety']."' and salesrs_vflg=1") or die(mysqli_error($link));
while($row_srretsub=mysqli_fetch_array($sql_srretsub))
{
	/*$sql_srretss=mysqli_query($link,"Select * from tbl_salesrvsub_sub where salesrs_id='".$row_srretsub['salesrs_id']."'") or die(mysqli_error($link));
	...
	$sql_srretss2=mysqli_query($link,"Select * from tbl_salesrvsub_sub2 where salesrs_id='".$row_srretsub['salesrs_id']."'") or die(mysqli_error($link));
	...
	if($lotnc!="")
	{
		$sql_arr_home=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where  lotldg_crop='".$row_srrets['salesrs_crop']."' and lotldg_variety='".$row_srrets['salesrs_variety']."' and lotldg_lotno IN($lotnc) and lotldg_balqty > 0 and lotldg_trtype='Sales Return' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
	
		while($row_arr_home=mysqli_fetch_array($sql_arr_home))
		{  
			$sql_issue=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where lotldg_lotno='".$row_arr_home['lotldg_lotno']."' and lotldg_balqty > 0 and lotldg_trtype='Sales Return' group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
			...
			{ 
				...
				$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and lotldg_balqty > 0 and lotldg_trtype='Sales Return' order by lotldg_id asc") or die(mysqli_error($link)); 
	...
	if($lotnp!="")
	{
		$sql_arr_home=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where  lotldg_crop='".$row_srrets['salesrs_crop']."' and lotldg_variety='".$row_srrets['salesrs_variety']."' and lotno IN($lotnp) and balqty > 0 and trtype='Sales Return' group by lotno order by lotdgp_id asc") or die(mysqli_error($link));
	
		while($row_arr_home=mysqli_fetch_array($sql_arr_home))
		{  
			$sql_issue=mysqli_query($link,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where lotno='".$row_arr_home['lotno']."' and balqty > 0 and trtype='Sales Return' group by subbinid order by lotdgp_id asc") or die(mysqli_error($link));
			while($row_issue=mysqli_fetch_array($sql_issue))
			{ 
				$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and lotno='".$row_arr_home['lotno']."' and trtype='Sales Return' order by lotdgp_id asc ") or die(mysqli_error($link));
				$row_issue1=mysqli_fetch_array($sql_issue1); 
				
				$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_issue1[0]."' and balqty > 0 and trtype='Sales Return' order by lotdgp_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_issuetbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
...
<tr >
<td valign="top" align="center"><a href="partywise_sr_report.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:pointer;"  /></a>&nbsp;&nbsp;<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" /></td>
</tr>
</table>
</td><td width="30"></td>
</tr>
