<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$txtpmcode=trim($_POST['txtpmcode']);
		$sid=trim($_POST['sid']);
		$s=0;
		$sid3=explode(",",$sid);
		foreach($sid3 as $sd)
		{
			if($sd<>"")
			{
				$sqlmain="update tblarrival_sub set psflg=1 where lotno='$sd'  ";
				if($a123=mysqli_query($link,$sqlmain) or die(mysqli_error($link)))
				{
					$s++;
				}
			}
		...
		if($s>0)
			echo "<script>window.location='add_lotps.php'</script>";
		else
			echo "<script>window.location='add_lotps_preview.php?sdate=$sdate&edate=$edate&txtcrop=$cid&txtvariety=$itemid&txtpmcode=$txtpmcode&sid=$sid'</script>";
	}
...
<title>Viewer - Transaction - Settlement</title>
...
winHandle=window.open('addlotps_print.php?sdate='+sdate+'&txtcrop='+itemid+'&txtvariety='+vv+'&edate='+edate+'&txtpmcode='+pmc+'&sid='+sid,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); }
}
function selall()
{
	var sr=document.frmaddDepartment.cnt.value;
	//alert(sr);
	for (var i=1; i<=sr; i++)
	{
		var gprs="pschk_"+i;
...
{ 
	...
	{	
		document.frmaddDepartment.sid.value=sid;
		return true;	 
	}
}
...
?>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#ef0388" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#ef0388" style="border-bottom:solid; border-bottom-color:#ef0388" >
	    <tr >
	      <td width="813" height="25">Transaction - Settlement</td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
	  ...
	  <td align="center" colspan="4" >
		...
		<input name="txtpmcode" value="<?php echo $_REQUEST['txtpmcode'];?>" type="hidden">  
...
{
if($ltcnts>1)
	$qry24="select distinct lotcrop from tblarrival_sub where organiser='".$txtpmcode."' and plantcode='$plantcode'  and lotno IN ($sid2) ";
else	
	$qry24="select distinct lotcrop from tblarrival_sub where organiser='".$txtpmcode."' and plantcode='$plantcode'  and lotno='$sid' ";	
...
{
if($ltcnts>1)
$qry23="select distinct lotvariety from tblarrival_sub where organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and plantcode='$plantcode' and lotno IN ($sid2) ";
else
$qry23="select distinct lotvariety from tblarrival_sub where organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and plantcode='$plantcode' and lotno='$sid' ";
...
$arid='';
if($ltcnts>1)
	$qry2="select arrival_id from tblarrival_sub where organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and plantcode='$plantcode' and lotvariety='".$row_arr_home23['lotvariety']."' and sstage='Raw' and lotno IN ($sid2)";
else
	$qry2="select arrival_id from tblarrival_sub where organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and plantcode='$plantcode' and lotvariety='".$row_arr_home23['lotvariety']."' and sstage='Raw' and lotno='$sid'";
...
<?php
	...
	$qry="select arrival_id, arrival_date, yearcode from tblarrival where  arrival_type='Fresh Seed with PDN' and arrtrflag=1 and plantcode='$plantcode' and arrival_id IN($arid)";
...
<tr class="tblsubtitle" height="20">
	...
	<!--<td width="40"  align="center" valign="middle" class="smalltblheading" >PS<br /><a href="Javascript:void(0);" onclick="selall()">CA</a>/<a href="Javascript:void(0);" onclick="clrall()">CL</a></td>-->
</tr>
...
{
if($ltcnts>1)
	$sql_rr=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row_arr_home1['arrival_id']."' and organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and lotvariety='".$row_arr_home23['lotvariety']."' and sstage='Raw' and psflg=0 and plantcode='$plantcode' and lotno IN ($sid2) order by orlot asc") or die(mysqli_error($link));
else
	$sql_rr=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row_arr_home1['arrival_id']."' and organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and lotvariety='".$row_arr_home23['lotvariety']."' and sstage='Raw' and psflg=0 and plantcode='$plantcode' and lotno='$sid' order by orlot asc") or die(mysqli_error($link));
...
<tr class="Light">
	...
	<td width="50" align="center" valign="middle" class="smalltbltext"><b><?php echo $lotno1;?></b></td>
	...
	<!--<td align="center" valign="middle" class="smallesttbltext"><input type="checkbox" name="pschk<?php echo $cnt?>" id="pschk_<?php echo $cnt?>" value="<?php echo $lotno1;?>" /></td>-->
</tr>
<?php
}
else
...
<tr >
<td valign="top" align="right"><a href="edit_lotps.php?sdate=<?php echo $_REQUEST['sdate'];?>&edate=<?php echo $_REQUEST['edate'];?>&txtcrop=<?php echo $_REQUEST['txtcrop'];?>&txtvariety=<?php echo $_REQUEST['txtvariety'];?>&txtpmcode=<?php echo $_REQUEST['txtpmcode'];?>&sid=<?php echo $_REQUEST['sid'];?>"><img src="../images/edit.gif" border="0"style="display:inline;cursor:pointer;" /></a>&nbsp;&nbsp;<a href="Javascript:void(0)" onClick="openslocpopprint();"><img src="../images/printpreview.gif" border="0"style="display:inline;cursor:pointer;" /></a>&nbsp;&nbsp;<input name="Submit" type="image" src="../images/finalsubmit.gif" alt="Submit Value"  border="0" style="display:inline;cursor:pointer;" tabindex="" onClick="return mySubmit();">&nbsp;&nbsp;</td>
<!--<td  align="center" valign="middle"><a href="add_lotps.php"><img src="../images/cancel.gif" border="0" onclick="return confirm('Do You wish to Cancel this Transaction?');"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<input name="Submit" type="image" src="../images/preview.gif" alt="Submit Value" onclick="return mySubmit();"   border="0" style="display:inline;cursor:pointer;"  />&nbsp;&nbsp; <input type="hidden" name="sid" value="<?php echo $_REQUEST['sid'];?>" /></td>-->
</tr>
</table>
</td><td ></td>
</tr>
