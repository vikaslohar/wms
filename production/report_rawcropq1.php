...
if($crval<>"")
{
	...
	if($verval<>"")
	{
		...
		// Pack Seed Records
		/*$sql_arhome=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' group by lotno order by lotdgp_id asc") or die(mysqli_error($link));
		while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
...
if($ins=mysqli_query($link,$sql_rep) or die(mysqli_error($link)))
{
	...
	for($i=0; $i<$ct; $i++)
	{
		$uidd=$ui[$i];
		$upsize=$up[$i];
		$sql_repsub="Insert into tmp_vwasrrep2 (tid, upssize, upsval) values ('$tiiid', '$upsize', '$uidd')";
