...
var vv=document.frmaddDepartment.txtloc.value;
...
?>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#ef0388" >
	 ...
	 <input name="frm_action" value="submit" type="hidden"> 
	   ...
	   <input name="txtloc" value="<?php echo $vv;?>" type="hidden"> 
            ...
            <tr class="tblsubtitle" height="20">
              <td width="4%"align="center" valign="middle" class="tblheading">#</td> 
			   <td width="9%" align="center" valign="middle" class="tblheading">Date</td>
			    <td width="15%" align="center" valign="middle" class="tblheading">Arrival  Type</td>
...
<?php
$srno=1;
$sql_armain=mysqli_query($link,"select * from tblarrival where arrival_date <= '$edate' and arrival_date >= '$sdate' and arrtrflag=1 group by arrival_date order by arrival_date desc ") or die(mysqli_error($link));
$tot_armain+mysqli_num_rows($sql_armain);

while($row_armain=mysqli_fetch_array($sql_armain))
{
$sql_arr_home=mysqli_query($link,"select * from tblarrival where arrival_date='".$row_armain['arrival_date']."' and arrtrflag=1  order by arrival_date desc ") or die(mysqli_error($link));
...
while($row=mysqli_fetch_array($sql_arr_home))
	...
	if($vv=="ALL" )
	{
		...
		{
			$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row['arrival_id']."' and lotcrop='".$cit."'") or die(mysqli_error($link));
		}
		else
		{
			$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row['arrival_id']."' and lotcrop='".$cit."'") or die(mysqli_error($link));
	...
	else
	{
		...
		/*if($row['arrival_type']=="Trading" || $row['arrival_type']=="StockTransfer Arrival")
		{
			$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row['arrival_id']."' and lotvariety='".$vit."' and lotcrop='".$itemid."'") or die(mysqli_error($link));
		}
		else
		{*/
			$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row['arrival_id']."' and lotvariety='".$vit."' and lotcrop='".$cit."'") or die(mysqli_error($link));
		//}
		$tot_arsub=mysqli_num_rows($sql_tbl_sub);
	}
...
<tr class="Light">
         <td width="4%" align="center" valign="middle" class="tblheading"><?php echo $srno;?></td>
		   <td width="9%" align="center" valign="middle" class="tblheading"><?php echo $stlg_trdate;?></td>
		    <td width="15%" align="center" valign="middle" class="tblheading"><?php echo $row['arrival_type']?></td>
         ...
         <td width="8%" align="center" valign="middle" class="tblheading"><?php echo $stage?></td>
              <td width="9%" align="center" valign="middle" class="tblheading"><?php echo $row_tbl_sub['got1'];?></td>
         </tr>
<?php
}
else
