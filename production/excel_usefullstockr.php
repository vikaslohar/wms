<?php
	...
	$cp="";
	$sql_crp=mysqli_query($link,"select * from tblcrop where cropname IN('Paddy','Maize','Bajra') order by cropname ASC") or die(mysqli_error($link));
...
while($row_rr=mysqli_fetch_array($sql_rr))
{
	...
	foreach($upp2 as $ups2)
	{
		...
		{	
			...
			{
				$totqty=0; $totnob=0; 
				$sql_arr_home=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and packtype='".$row_rr2['packtype']."' and lotldg_qc!='Fail' and plantcode='$plantcode' and lotldg_got!='Fail'  group by lotno order by lotdgp_id asc") or die(mysqli_error($link));
				while($row_arr_home=mysqli_fetch_array($sql_arr_home))
				{
					$sql_tbl_sub1=mysqli_query($link,"select distinct subbinid from tbl_lot_ldg_pack where lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_rr['lotldg_variety']."' and packtype='".$row_rr2['packtype']."' and lotno='".$row_arr_home['lotno']."' and lotldg_qc!='Fail' and plantcode='$plantcode' and lotldg_got!='Fail' order by subbinid") or die(mysqli_error($link));
