...
<title>Viewer - Party wise Damage Material Return Report</title>
...
$md=explode(",",$mid);
if(count($md) >1)
$sqlsrrets2="select distinct salesrs_crop from tbl_salesrv_sub where salesr_id IN ($mid) ";
else
$sqlsrrets2="select distinct salesrs_crop from tbl_salesrv_sub where salesr_id=$mid ";
if($crop!="ALL")
{
...
if(count($md) >1)
$sqlsrrets1="select distinct salesrs_variety from tbl_salesrv_sub where salesr_id IN ($mid) ";
else
$sqlsrrets1="select distinct salesrs_variety from tbl_salesrv_sub where salesr_id=$mid ";
...
if(count($md) >1)
$sqlsrrets="select * from tbl_salesrv_sub where salesr_id IN ($mid) ";
else
$sqlsrrets="select * from tbl_salesrv_sub where salesr_id=$mid ";
...
$sqlsrretm=mysqli_query($link,"select * from tbl_salesrv where salesr_id='".$row_tbl_sub['salesr_id']."'") or die(mysqli_error($link));
