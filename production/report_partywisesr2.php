...
<tr class="light" height="20">
  <td colspan="6" align="center" class="tblheading">Periodical Party wise Sales Return Report</td>
</tr>
</table>
<table align="center" border="0" width="750" cellspacing="0" cellpadding="0" bordercolor="#ef0388" style="border-collapse:collapse" > 
<tr class="light" height="20">
...
if($mid==""){$mid='NULL';
$sqlsrrets="select distinct salesrs_variety, salesrs_crop from tbl_salesrv_sub where salesr_id=$mid and plantcode='$plantcode' ";}
else{
$sqlsrrets="select distinct salesrs_variety, salesrs_crop from tbl_salesrv_sub where salesr_id IN ($mid) and plantcode='$plantcode' ";}
if($crop!="ALL")
{
...
$sql_srretsub2=mysqli_query($link,"select distinct salesrs_newlot, salesrs_rettype from tbl_salesrv_sub where salesrs_variety='".$row_srrets['salesrs_variety']."' and salesrs_vflg=1 and salesr_id IN ($mid) and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_srretsub=mysqli_query($link,"select * from tbl_salesrv_sub where salesrs_variety='".$row_srrets['salesrs_variety']."' and salesrs_vflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_srretsub=mysqli_fetch_array($sql_srretsub))
{
	...
	$sql_srretss2=mysqli_query($link,"Select * from tbl_salesrvsub_sub2 where salesrs_id='".$row_srretsub['salesrs_id']."' and plantcode='$plantcode'") or die(mysqli_error($link));
	...
	if($lotnp!="")
	{
		$sql_arr_home=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where  lotldg_crop='".$row_srrets['salesrs_crop']."' and lotldg_variety='".$row_srrets['salesrs_variety']."' and lotno IN($lotnp) and balqty > 0 and trtype='Sales Return' and plantcode='$plantcode' group by lotno order by lotdgp_id asc") or die(mysqli_error($link));
	
		while($row_arr_home=mysqli_fetch_array($sql_arr_home))
		{  
			$sql_issue=mysqli_query($link,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where lotno='".$row_arr_home['lotno']."' and balqty > 0 and trtype='Sales Return' and plantcode='$plantcode' group by subbinid order by lotdgp_id asc") or die(mysqli_error($link));
			while($row_issue=mysqli_fetch_array($sql_issue))
			{ 
				$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and lotno='".$row_arr_home['lotno']."' and trtype='Sales Return' and plantcode='$plantcode' order by lotdgp_id asc ") or die(mysqli_error($link));
				$row_issue1=mysqli_fetch_array($sql_issue1); 
...
<tr >
<td align="right" colspan="3">&nbsp;&nbsp;<img src="../images/Vista-printer.png" height="29" width="35" border="0" onClick="javascript:window.print();" style="cursor:pointer" />&nbsp;<a href="excel_partywisesr.php?txtcrop=<?php echo $_REQUEST['txtcrop']?>&txtvariety=<?php echo $_REQUEST['txtvariety']?>&sdate=<?php echo $_REQUEST['sdate']?>&edate=<?php echo $_REQUEST['edate']?>&txtptype=<?php echo $_REQUEST['txtptype']?>&txtcountryl=<?php echo $_REQUEST['txtcountryl']?>&txtpp=<?php echo $_REQUEST['txtpp']?>&txtstatesl=<?php echo $_REQUEST['txtstatesl']?>&locationname=<?php echo $_REQUEST['locationname']?>&txtstfp=<?php echo $_REQUEST['txtstfp']?>" target="_blank"><img src="../images/excelicon1.jpg" border="0" height="30" width="30" class="butn" alt="Export to Excel" style="display:inline;cursor:pointer;" /></a>&nbsp;<img src="../images/close_icon2.jpg" height="30"  border="0" onClick="window.close()" style="cursor:pointer" /></td>
</tr>
</table>
</body>
</html>
