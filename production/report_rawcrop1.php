...
<?php
$srno=1; $dttime=date("d-m-Y H:i:s");
if($tot_arr_home > 0)
{
$crps=explode(",",$crop2);
//print_r($crps);
...
if($crval<>"")
{
	...
	if($verval<>"")
	{
		...
		{  
			...
			$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crval."' and lotldg_lotno='".$row_arhome['lotldg_lotno']."' and lotldg_variety='".$verval."' and plantcode='$plantcode' and lotldg_sstage='$stage1' group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
			while($row_is=mysqli_fetch_array($sql_is))
			{ 
				$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and plantcode='$plantcode' and lotldg_lotno='".$row_arhome['lotldg_lotno']."' and lotldg_sstage='$stage1' order by lotldg_id desc ") or die(mysqli_error($link));
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' order by lotldg_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
...
$sql_rep="Insert into tmp_vwcrsrep (crop, variety, rsqty, csqty, psqty, srqty, totqty, logid, dttime, repflg, plantcode) values ('$crop1', '$verty', '$totrqty', '$totcqty', '$totpqty', '$totsrqty', '$tqty', '$logid', '$dttime', '1',  '$plantcode')";
