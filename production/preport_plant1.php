...
<?php	
	...
	$sql_tbl_s=mysqli_query($link,"select distinct sstage from tblarrival_sub where plantcode='$plantcode' group by sstage") or die(mysqli_error($link));
	$tot_arsub=mysqli_num_rows($sql_tbl_s);
	}
	else
	{
...
if($loc=="ALL" && $itemid!="ALL")
{
$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where lotcrop='".$crop."' and sstage='".$row_tbl_s[0]."' and plantcode='$plantcode' order by lotvariety asc") or die(mysqli_error($link));
}
else if($loc=="ALL" && $itemid=="ALL")
{
$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where sstage='".$row_tbl_s[0]."' and plantcode='$plantcode' order by lotvariety asc") or die(mysqli_error($link));
}
else
{
$sql_tbl_sub=mysqli_query($link,"select * from tblarrival_sub where lotcrop='".$crop."' and lotvariety='".$row_tbl_s[1]."' and sstage='".$row_tbl_s[0]."' and plantcode='$plantcode' order by lotvariety asc") or die(mysqli_error($link));
}
