	...
	//exit;
		if($btnval==0)
		{
			...
			foreach($p as $val)
			{
				if($val <> "")
				{
					$sql_chk=mysqli_query($link,"select * from tbl_qctest where tid='$val'") or die(mysqli_error($link));
					...
					$sql_chk2=mysqli_query($link,"select tid from tbl_qctest where sampleno='$smpn' and yearid='$yrd' and oldlot='$oltn'") or die(mysqli_error($link));
					$tot_chk2=mysqli_num_rows($sql_chk2);
					while($row_chk2=mysqli_fetch_array($sql_chk2))
					{
						if($nid!="")
						$nid=$nid.",".$row_chk2['tid'];
						else
						$nid=$row_chk2['tid'];
					}
				}
		...
		{
			$tid=$_POST['dtid'];
			$sql_in1="update tbl_qctest set gotsmpdflg=2 where tid='$tid'";	
...
<?php
 $sql_arr_home2=mysqli_query($link,"select distinct sampleno from tbl_qctest where aflg=0 and bflg=1 and cflg=0 and gotflg=0 and gotsmpdflg=0 and got='UT' order by sampleno desc") or die(mysqli_error($link));
...
{
$sql_arr_home3=mysqli_query($link,"select distinct lotno from tbl_qctest where sampleno='".$row_arr_home2['sampleno']."' and aflg=0 and bflg=1 and cflg=0 and gotflg=0 and gotsmpdflg=0 and got='UT'  order by sampleno desc") or die(mysqli_error($link));
$tot_arr_home3=mysqli_num_rows($sql_arr_home3);
while($row_arr_home3=mysqli_fetch_array($sql_arr_home3))
{
$sql_arr_home4=mysqli_query($link,"select max(tid) from tbl_qctest where lotno='".$row_arr_home3['lotno']."' and sampleno='".$row_arr_home2['sampleno']."' and aflg=0 and bflg=1 and cflg=0 and gotflg=0 and gotsmpdflg=0 and got='UT'  order by sampleno desc") or die(mysqli_error($link));
$row_arr_home4=mysqli_fetch_array($sql_arr_home4);

$sql_arr_home=mysqli_query($link,"select * from tbl_qctest where lotno='".$row_arr_home3['lotno']."' and sampleno='".$row_arr_home2['sampleno']."' and tid='".$row_arr_home4[0]."' and aflg=0 and bflg=1 and cflg=0 and gotflg=0 and gotsmpdflg=0 and got='UT'  order by sampleno desc") or die(mysqli_error($link));
...
if($bags!="")
	...
	$sq=mysqli_query($link,"select sampleno,trstage from tbl_qctest where sampleno='$qc1' and yearid='$yriid'") or die(mysqli_error($link));
	$toto=mysqli_num_rows($sq);
	if($toto>1)
	{
		if($row_arr_home['trstage']=="Condition" || $row_arr_home['trstage']=="Pack" || $row_arr_home['trstage']=="")$fg=1;
	}

 $row_arr_home['variety'];
...
<tr class="Light">
         ...
         <!--<td width="49" align="center" valign="middle" class="tblheading"><input name="image" type="image" style="display:inline;cursor:pointer;" onclick="deleterec('1','<?php echo $row_arr_home['tid'];?>');" src="../images/delete.png" alt="Abort Value" border="0"/></td>-->
</tr>
<?php
}
else
...
<tr >
<td valign="top" align="right"><a href="select.php" tabindex="20"><img src="../images/back.gif" border="0"style="display:inline;cursor:Pointer;" /></a>&nbsp;<input name="Submit" type="image" src="../images/post.gif" alt="Submit Value"  border="0" style="display:inline;cursor:hand;" tabindex="" onClick="return mySubmit();"><input type="hidden" name="chk" value="" /><input type="hidden" name="foccode1" value="" /><input type="hidden" name="foccode" value="" /> <input type="hidden" name="srno" value="<?php echo $srno;?>" /><input type="hidden" name="tt" value="" />&nbsp;&nbsp;&nbsp;</td>
</tr>
</table>
<?php
}
