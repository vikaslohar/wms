<?php
	...
	$quer3=mysqli_query($link,"SELECT * FROM tblvariety where varietyid ='".$variety."' and actstatus='Active'"); 
...
function post_value()
{
//alert(opener.document.frmaddDepartment.txtlot2.value=document.from.foccode2.value);
opener.document.frmaddDepartment.txtlot1.value=document.from.foccode.value;
opener.document.frmaddDepartment.txtlot2.value=document.from.foccode2.value;
opener.document.frmaddDepartment.gsid.value=document.from.foccode3.value;
self.close();
}

function clk(val,val1,val2)
{
//alert(document.from.val1.value);
document.from.foccode.value=val;
document.from.foccode2.value=val1;
document.from.foccode3.value=val2;
}
...
$qr="select distinct(lotno) from tbl_gsample where gscrop='".$aa."' and gsvariety='".$tt."' $sss";
...
if($tot_row > 0)
{
$srno=1; $wh=""; $binn=""; $slocs="";
while ($row=mysqli_fetch_array($sql_tbl_sub))
{
$lot=$row['lotno'];

$sql=mysqli_query($link,"select * from tbl_gsample where gscrop='".$aa."' and gsvariety='".$tt." ' and lotno='".$lot."'");
...
<td align="right"  valign="middle" class="tbltext"><input type="radio" name="loc" value="<?php echo $row['lotno'];?>" onclick="clk(this.value,'<?php echo $slocs?>','<?php echo $gsid?>');" />&nbsp;</td>
