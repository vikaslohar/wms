...
if(isset($_POST['frm_action'])=='submit')
{
	...
	while($row_ck=mysqli_fetch_array($sql_ck))
	{
		...
		if($ores=="RT")
		{
			$sql_sub_sub="insert into tbl_qctest(lotno,  oldlot, crop, variety, srdate, spdate, sampleno, state, qc, trstage, pp, moist, gemp, qcstatus, testdate, gotdate, dosdate, got, gotstatus, aflg, bflg, cflg, qcflg, gotflg, gsflg, gs, gotrefno, gotauth, doswdate, gotsmpdflg, stsno, qcrefno, yearid, genpurity, dotransplant, bedno, plantpopln, direction, gotlocation, maleno, maleper, femalepno, femaleper, oofno, oofper, totalno, totalper, doobr, remarks) values('$olot', '$oldlot', '$crp', '$ver', '$srdt', '$spdt', '$smpno', '$stats', '$oqc', '$stge', '$opp', '$omt', '$ogmp', '$oqcst', '$oqtdt', '$ogotdate', '$odosdate', '$ogot', '$ogotstatus', '$oaflg', '$obflg', '$ocflg', '$oqcflg', '$ogotflg', '$ogsflg', '$ogs', '$ogotrefno', '$ogotauth', '$odoswdate', '$ogotsmpdflg', '$ostsno', '$oqcrefno', '$yearid', '$opurity', '$dotr1', '$bedno1', '$plantpop1', '$direction1', '$gotloc1', '$maleno1', '$maleper1', '$femaleno1', '$femaleper1', '$oofno', '$oofper1', '$totalno1', '$totalper1', '$doobr1', '$remarks1')";
			if(mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link)))
			{
				...
				if($result!="RT")
				{
					...
					$sql_sub_sub12="update tbl_qctest set doswdate='$doswdate', dotransplant='$dotr', bedno='$bedno', direction='$direction',  gotlocation='$gotloc', plantpopln='$plantpop' where tid='$id'";
		...
		else
		{
			if($result!="RT")
			{
				...
				$sql_sub_sub12="update tbl_qctest set doswdate='$doswdate', dotransplant='$dotr', bedno='$bedno', direction='$direction',  gotlocation='$gotloc', plantpopln='$plantpop' where sampleno='$osamp22' and yearid='$yearid22'";
				mysqli_query($link,$sql_sub_sub12) or die(mysqli_error($link));//exit;
			}
			else
			{
				$sql_sub_sub12="update tbl_qctest set doswdate='$doswdate', dotransplant='$dotr', bedno='$bedno', direction='$direction',  gotlocation='$gotloc', plantpopln='$plantpop' where sampleno='$osamp22' and yearid='$yearid22'";
...
<td valign="top" align="right"><!--<a href="home_gotdata.php"><img src="../images/cancel.gif" border="0"style="display:inline;cursor:pointer;" onclick="return confirm('Do You wish to Cancel this Transaction?');" /></a>--><img src="../images/back.gif" border="0" onclick="window.close()" />&nbsp;
  <input name="Submit" type="image" src="../images/submit.gif" alt="Submit Value" onclick="return mySubmit();"   border="0" style="display:inline;cursor:pointer;"  />  <input type="hidden" name="txtsamp" value="<?php echo $sampl?>" />&nbsp;&nbsp;</td>
</tr>
</table>		
</form>
</td></tr>
