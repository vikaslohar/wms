...
<?php
$quer=mysqli_query($link,"select * from tbl_gottest where gottest_tid='".$rowss1['gottest_tid']."'")or die(mysqli_error($link));
$rowmar= mysqli_fetch_array($quer);

$quer1=mysqli_query($link,"select * from tblvariety where varietyid='".$rowmar['gottest_variety']."'")or die(mysqli_error($link));
$rowmar1=mysqli_fetch_array($quer1);
//echo "select * from tbl_basepair where varietyid='".$rowmar1['popularname']."'";
$quer12=mysqli_query($link,"select * from tbl_basepair where varietyid='".$rowmar1['popularname']."'")or die(mysqli_error($link));
$totmar12=mysqli_num_rows($quer12);
$rowmar12=mysqli_fetch_array($quer12);
$markername="";
if($rowmar12['markername1']!="")
	$markername=$rowmar12['markername1'];
if($rowmar12['markername2']!="")
	$markername=$markername.",".$rowmar12['markername2'];
if($rowmar12['markername3']!="")
	$markername=$markername.",".$rowmar12['markername3'];	
if($rowmar12['markername4']!="")
	$markername=$markername.",".$rowmar12['markername4'];
$markername1=explode(",",$markername);
$marcnt=count($markername1);
//echo $marcnt;
?>

<td width="235" align="center" valign="middle" class="smalltbltext"><?php if($totmar12>0 && $marcnt>0){?><select class="tbltext" name="txtmarkername" style="width:150px;" onchange="markername()">
<option value="" selected>--Select--</option>
<?php foreach($markername1 as $name){?>
<option value="<?php echo $name?>"><?php echo $name?></option>
<?php }?>
</select><?php }else{?><input name="txtmarkername" type="text" size="20" class="tbltext" tabindex=""  maxlength="20"  value="" onchange="markername()"><?php }?>&nbsp;<font color="#FF0000">*</font>&nbsp;&nbsp;</td>
</tr>
</table>
