...
else if($cid!="ALL" && $itemid!="ALL")	
{
	if($qcti=="rt")
	{
		$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where qcstatus='RT' and  srdate <= '$sdate' and crop='$cid' and variety='$itemid' and qcflg=0 and state!='T' order by spdate asc ") or die(mysqli_error($link));
	}
	else  if($qcti=="ut")
	{
