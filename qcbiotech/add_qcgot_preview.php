<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		{		
			$sql_arr_home3=mysqli_query($link,"select distinct sampleno, yearid from tbl_qctest where tid='".$fid."' and aflg=0 and bflg=1 and cflg=0 and gotflg=0 and gotsmpdflg=0 and got='UT' order by sampleno desc") or die(mysqli_error($link));
			$tot_arr_home3=mysqli_num_rows($sql_arr_home3);
			while($row_arr_home3=mysqli_fetch_array($sql_arr_home3))
			{
				$sql_sub_sub12="update tbl_qctest set dosdate='$date', gotsmpdflg=1 where sampleno='".$row_arr_home3['sampleno']."' and yearid='".$row_arr_home3['yearid']."'";
