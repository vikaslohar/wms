...
<?php	
	...
	if($trstage!="ALL")
	$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where gotdate='$sdate' and trstage='$txtstage' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
	else
...
if($trstage!="ALL")
	$sql_arr_home2=mysqli_query($link,"select MAX(tid) from tbl_qctest where sampleno='".$row_arr_home2['sampleno']."' and gotdate='$sdate' and trstage='$txtstage' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
else
	$sql_arr_home2=mysqli_query($link,"select MAX(tid) from tbl_qctest where sampleno='".$row_arr_home2['sampleno']."' and gotdate='$sdate' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
