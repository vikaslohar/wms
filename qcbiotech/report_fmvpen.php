...
<title>Quality-Report - GOT Transplanting Pending Summery Report</title>
  ...
  <tr><td>
   ...
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" style="border-bottom:solid; border-bottom-color:#d21704" >
	    <tr >
	      <td width="813" height="25">&nbsp;Report - GOT Transplanting Pending Summery Report</td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
<table align="center" border="0" cellspacing="0" cellpadding="0" width="500" style="border-collapse:collapse">
   <!--	<tr height="25" >
    <td align="center" class="subheading" style="color:#303918; ">Lot Destination Report:</td>
  	...
  	<tr height="25">
    <td align="center" class="subheading" style="color:#303918;" colspan="2">GOT Transplanting Pending Summery Report As on Date: <?php echo date("d-m-Y");?> </td>
  	</tr>
</table>
...
<?php
$srno=1;
$sql="SELECT distinct(gotm_crop) FROM tbl_qcgotmain WHERE gotm_ackflg=1 and gotm_retestflg=0 and gotm_resamplingflg=0 and gotm_sowingflg=1 and gotm_transplantflg>0 AND gotm_fieldmflg=0  ";
//echo $sql;
...
$sqlmax2="SELECT distinct(gotm_variety) FROM tbl_qcgotmain WHERE gotm_ackflg=1 and gotm_retestflg=0 and gotm_resamplingflg=0 and gotm_sowingflg=1 and gotm_transplantflg>0 AND gotm_fieldmflg=0 and gotm_crop='".$row_arr_home['gotm_crop']."'";
...
while($row_arr_home2=mysqli_fetch_array($sql_max))
{
	...
	while($row_arr_home4=mysqli_fetch_array($sql_max4))
	{
		...
		$tdt=date("Y-m-d"); $gsdd="";
		$sql_arrmain=mysqli_query($link,"SELECT gotsow_dosow, gotsow_state, gotsow_loc, gotsow_nooftraylot, gotsow_bedno, gotsow_treynumber FROM tbl_qcgotsowing WHERE gotm_id='".$gotm_id."' ") or die(mysqli_error($link));
		$row_arrmain=mysqli_fetch_array($sql_arrmain); 
		
		if($row_arrmain['gotsow_dosow']!='' && $row_arrmain['gotsow_dosow']!='0000-00-00' && $row_arrmain['gotsow_dosow']!=NULL)
		{
			$arrival_date1=explode("-",$row_arrmain['gotsow_dosow']);
			$tpsowingdate=$arrival_date1[2]."-".$arrival_date1[1]."-".$arrival_date1[0];
			
			if($row_arrmain['gotsow_dosow']!='')
			{
				$m=$arrival_date1[1];
				$de=$arrival_date1[2];
				$y=$arrival_date1[0];
...
<tr class="Light" height="25">
	...
	<td width="123" align="center" valign="middle" class="tblheading"><a href="report_fmvpendet.php?txtcrop=<?php echo $row_arr_home['gotm_crop'];?>&txtvariety=<?php echo $row_arr_home3['gotm_variety']; ?>"><?php echo $lotno?></a></td>
</tr>
<?php
}
else
