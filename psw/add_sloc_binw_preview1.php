...
{	
	$sql_arr=mysqli_query($link,"select * from tbl_sloc_binw2 where plantcode='$plantcode' and slid='".$pid."'") or die(mysqli_error($link));
	...
	$sql_sloc_sub22=mysqli_query($link,"select * from tbl_sloc_binw_sub2 where plantcode='$plantcode' and slocid='".$pid."'") or die(mysqli_error($link));
	$row_sloc_sub22=mysqli_fetch_array($sql_sloc_sub22);
	$whid1=$row_sloc_sub22['whid'];
	$binid1=$row_sloc_sub22['binid'];
	//$subbinid1=$row_sloc_sub22['subbinid'];
	/*$classid=$row_sloc_sub22['crop'];
	$itemid=$row_sloc_sub22['variety'];*/
	
	$sql_issue23=mysqli_query($link,"select distinct (subbinid)  from tbl_lot_ldg_pack where plantcode='$plantcode' and whid='".$who."' and binid='".$bino."'") or die(mysqli_error($link));
	while ($row_issue23=mysqli_fetch_array($sql_issue23))
	{
		$subbino=$row_issue23['subbinid'];
		
		$sql_sbn=mysqli_query($link,"select sid, sname from tbl_subbin where plantcode='$plantcode' and sid='".$subbino."' order by sname")or die("Error:".mysqli_error($link));
		$row_sbn=mysqli_fetch_array($sql_sbn);
		$sbn=$row_sbn['sname'];
		
		$sql_sbn2=mysqli_query($link,"select sid, sname from tbl_subbin where plantcode='$plantcode' and sname='".$sbn."' and binid='".$binid1."' and whid='".$whid1."' order by sname")or die("Error:".mysqli_error($link));
		$row_sbn2=mysqli_fetch_array($sql_sbn2);
		$subbinid1=$row_sbn2['sid'];
		
		
		
		
		$sql_issue=mysqli_query($link,"select distinct (lotno)  from tbl_lot_ldg_pack where plantcode='$plantcode' and whid='".$who."' and binid='".$bino."' and subbinid='".$subbino."'") or die(mysqli_error($link));
		while ($row_issue=mysqli_fetch_array($sql_issue))
		{
			...
			while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
			{
				...
				$sql_mps=mysqli_query($link,"Select * from tbl_mpmain where plantcode='$plantcode' and plantcode='$plantcode' and mpmain_lotno='$lotno'") or die(mysqli_error($link));
	...
	$sql_main="update tbl_sloc_binw2 set supflg=1 where slid='".$pid."'";
	$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
	//exit;
	echo "<script>window.location='select_sloc_binw_op1.php?p_id=$pid'</script>";	
}
?>
...
<title>PSW -Transaction -SLOC Updation Bin wise</title>
...
winHandle=window.open('binw_slupdation_print1.php?&pid='+pid,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" style="border-bottom:solid; border-bottom-color:#0BC5F4" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - SLOC Updation Bin wise - Preview </td>
	    </tr></table></td>
	    
	  </tr>
	  </table></td></tr>
<?php
	$sql1=mysqli_query($link,"select * from tbl_sloc_binw2 where plantcode='$plantcode' and slid='".$pid."'")or die(mysqli_error($link));
	...
	$sql_sub=mysqli_query($link,"select * from tbl_sloc_binw_sub2 where plantcode='$plantcode' and slocid='".$pid."'")or die(mysqli_error($link));
	...
	//$sql_sbn=mysqli_query($link,"select sid, sname from tbl_subbin where sid='".$row['subbin']."' order by sname")or die("Error:".mysqli_error($link));
	//$row_sbn=mysqli_fetch_array($sql_sbn);
?> 
...
<table align="center" border="1" width="750" cellspacing="0" cellpadding="0" bordercolor="#0BC5F4" style="border-collapse:collapse"  cols="2">
  <tr class="tblsubtitle">
	...
	<td width="70" align="center" valign="middle" class="tblheading" >Sub-Bin</td>
...
$sql_iss23=mysqli_query($link,"select distinct (subbinid)  from tbl_lot_ldg_pack where plantcode='$plantcode' and whid='".$row['wh']."' and binid='".$row['bin']."' order by subbinid") or die(mysqli_error($link));
$tot23=mysqli_num_rows($sql_iss23);
while($row_iss23=mysqli_fetch_array($sql_iss23))
{ 
$a=$row_iss23['subbinid'];
$sql_iss=mysqli_query($link,"select distinct (lotno)  from tbl_lot_ldg_pack where plantcode='$plantcode' and whid='".$row['wh']."' and binid='".$row['bin']."' and subbinid='".$a."'") or die(mysqli_error($link));
$tot=mysqli_num_rows($sql_iss);
while($row_iss=mysqli_fetch_array($sql_iss))
{ 

$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' and subbinid='".$a."' and binid='".$row['bin']."' and whid='".$row['wh']."' and lotno='".$row_iss['lotno']."' ") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 
...
<tr class="Light" height="30">
	...
	<td width="70" align="center" valign="middle" class="tblheading"><?php echo $row_sbn2['sname'];?></td>
	...
	<td width="256"  align="center"  valign="middle">&nbsp;<?php echo $lot;?>&nbsp;<input type="hidden" name="lotsn" id="lotsn<?php echo $srno;?>" value="<?php echo $lot;?>" /></td>
</tr>
 <?php
}
else
...
<?php
$cntflg=0;
$sql_iss23=mysqli_query($link,"select distinct (lotldg_subbinid)  from tbl_lot_ldg where plantcode='$plantcode' and lotldg_whid='".$row['wh']."' and lotldg_binid='".$row['bin']."' order by lotldg_subbinid") or die(mysqli_error($link));
$tot23=mysqli_num_rows($sql_iss23);
while($row_iss23=mysqli_fetch_array($sql_iss23))
{ 
$a=$row_iss23['lotldg_subbinid'];
$sql_iss=mysqli_query($link,"select distinct (lotldg_lotno)  from tbl_lot_ldg where plantcode='$plantcode' and lotldg_whid='".$row['wh']."' and lotldg_binid='".$row['bin']."' and lotldg_subbinid='".$a."'") or die(mysqli_error($link));
$tot=mysqli_num_rows($sql_iss);
while($row_iss=mysqli_fetch_array($sql_iss))
{ 

$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_subbinid='".$a."' and lotldg_binid='".$row['bin']."' and lotldg_whid='".$b."' and lotldg_lotno='".$row_iss['lotldg_lotno']."' ") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 
...
<?php
	...
	//$sql_sbn2=mysqli_query($link,"select sid, sname from tbl_subbin where sid='".$row_sub['subbinid']."' order by sname")or die("Error:".mysqli_error($link));
	//$row_sbn2=mysqli_fetch_array($sql_sbn2);
?>  
<tr class="Light" height="25">
	<td width="110" align="right"  valign="middle" class="tblheading">&nbsp;WH&nbsp; </td>                                   
