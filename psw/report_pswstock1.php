...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <?php
	...
	$qry="select Distinct lotldg_crop, lotldg_variety from tbl_lot_ldg_pack where plantcode='$plantcode' and balqty > 0 ";
...
$totqty=0; $totnob=0; $totqc=""; $totdot=""; $totmost=""; $totgemp=""; $totgot=""; $reserve=""; $totsst=""; $txtdot="";	$sloc="";
	$sql_issue=mysqli_query($link,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where plantcode='$plantcode' and lotno='".$row_arr_home['lotno']."' and packtype='".$row_rr2['packtype']."' group by subbinid order by lotdgp_id desc") or die(mysqli_error($link));

 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
 

$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' and subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and packtype='".$row_rr2['packtype']."'and lotno='".$row_arr_home['lotno']."' order by lotdgp_id desc ") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='$plantcode' and lotdgp_id='".$row_issue1[0]."' and balqty > 0 order by lotdgp_id desc") or die(mysqli_error($link)); 
...
{	
	...
	if($tot_rps=mysqli_num_rows($sql_rps)>0) {$cnt=1; } else {$cnt=0; }
}
if($slchk=="yes")
{	
...
?>
<table align="center" border="0" cellspacing="0" cellpadding="0" width="950" bordercolor="#0BC5F4" style="border-collapse:collapse">
<tr height="25" >
