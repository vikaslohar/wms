...
$arrlots="";
/**/$sql_arrival=mysqli_query($link,"select trid from tbl_drying where plantcode='$plantcode' and crop='".$crop."' and variety='".$variety."'") or die(mysqli_error($link));
$tot_arrival=mysqli_num_rows($sql_arrival);
if($tot_arrival)
{
	while($row_arrival=mysqli_fetch_array($sql_arrival))
	{$row_arrival['trid'];
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_dryingsub where plantcode='$plantcode' and   trid='".$row_arrival['trid']."'") or die(mysqli_error($link));
...
$sql_tbl_sub=mysqli_query($link,"select distinct lotldg_lotno, lotldg_alflg, lotldg_dispflg, lotldg_rvflg  from tbl_lot_ldg where plantcode='$plantcode' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$tp."' $sss")or die(mysqli_error($link));
