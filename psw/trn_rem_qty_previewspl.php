<?php
		...
		while($row_arrsub_sub=mysqli_fetch_array($sql_arrsub_sub))
		{
			...
			while($rowarrsubsub=mysqli_fetch_array($sqlarrsubsub))
			{
				...
				if($pty=="PACKSMC" || $pty=="PACKNMC")
				{
					...
					$mplotno=$rowarrsubsub['mpmain_lotno'];
					...
					if($lotno==$mplotno)
					{
						if($bcflg<=$nomp)
						{
							$sqlsubchk="update tbl_mpmain set mpmain_dflg='1' where mpmain_id ='".$rowarrsubsub['mpmain_id']."'";
	...
	echo "<script>window.location='select_remspl_op.php?p_id=$pid'</script>";	
}
?>
...
<title>PSW -Transaction - Pack Seed SP Release - Preview</title>
...
winHandle=window.open('qtyremspl_print.php?itmid='+itm,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" style="border-bottom:solid; border-bottom-color:#0BC5F4" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Pack Seed SP Release - Preview </td>
	    </tr></table></td>
...
<td width="229" colspan="3" align="left" valign="middle" class="tbltext">&nbsp;<?php echo $tdate1;?>&nbsp;</td>
</tr>
