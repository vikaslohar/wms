...
{	
	$sql_arr=mysqli_query($link,"select * from tbl_sloc_upsw where plantcode='$plantcode' and slid='".$pid."'") or die(mysqli_error($link));
	$row_arr=mysqli_fetch_array($sql_arr);
	/*$classid=$row_arr['crop'];
	$itemid=$row_arr['variety'];*/
	$upssize=$row_arr['upssize'];
	...
	$bino="";
	$subbino="";
	$sql_sloc_sub22=mysqli_query($link,"select * from tbl_sloc_upsw_sub where plantcode='$plantcode' and slocid='".$pid."'") or die(mysqli_error($link));
	while($row_sloc_sub22=mysqli_fetch_array($sql_sloc_sub22))
	{
		$who=$row_sloc_sub22['whid'];
		$bino=$row_sloc_sub22['binid'];
		$subbino=$row_sloc_sub22['subbinid'];
		$ltnn=$row_sloc_sub22['lotno'];
		...
		while ($row_issue=mysqli_fetch_array($sql_issue))
		{
			...
			while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
			{
				...
				/*$balnop1=$balnop1+$opnop;
				$balups1=$balups1+$opups;
				$balqty1=$balqty1+$opqty;
				$opnop1=$opnop1+$opnop;
				$opups1=$opups1+$opups;
				$opqty1=$opqty1+$opqty;
	...
	//}	
		...
		$sql_sloc_sub2=mysqli_query($link,"select * from tbl_sloc_upsw_sub2 where plantcode='$plantcode' and slocid='".$pid."'") or die(mysqli_error($link));
		while($row_sloc_sub2=mysqli_fetch_array($sql_sloc_sub2))
		{
			$whid1=$row_sloc_sub2['whid'];
			$binid1=$row_sloc_sub2['binid'];
			$subbinid1=$row_sloc_sub2['subbinid'];
			...
			while ($row_issue=mysqli_fetch_array($sql_issue))
			{
				...
				while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
				{
					...
					$binid=$row_issuetbl['binid'];
					$subbinid=$row_issuetbl['subbinid'];
					$opnop=$row_issuetbl['opnop'];
					$opups=$row_issuetbl['opnomp'];
					...
					$sql_ins_sub11="insert into tbl_lot_ldg_pack (yearcode, trtype, trstage, lotldg_id, lotldg_trdate, lotno, packtype, packlabels, barcodes, wtinmp, lotldg_crop, lotldg_variety, whid, binid, subbinid, opnop, opnomp, optqty, nop, nomp, tqty, balnop, balnomp, balqty, lotldg_sstage, lotldg_sstatus, lotldg_moisture, lotldg_gemp, lotldg_vchk, lotldg_got1, lotldg_qc, lotldg_got, lotldg_qctestdate, lotldg_gottestdate, orlot, lotldg_resverstatus, lotldg_revcomment, lotldg_genpurity, lotldg_dop, lotldg_valperiod, lotldg_valupto, lotldg_srtyp, lotldg_srflg, lotldg_rvflg, lotldg_alflg, lotldg_dispflg, lotldg_altrids, lotldg_alqtys, lotldg_alnomps, lotldg_spremflg, plantcode) values('$ycode','UPWSUC', '$sstage', '$pid', '$trdate', '$lotno', '$pcktyp', '$packlabels', '$barcodes', '$wtinmp', '$classid', '$itemid', '$whid1', '$binid1', '$subbinid1', '$opnop1', '$opups1', '$opqty1', '$nop1', '$ups1', '$qty1', '$balnop', '$balups', '$balqty', '$sstage', '$sstatus', '$moist', '$gemp', '$vchk', '$got1', '$qc', '$gotstatus', '$qctestdate', '$gottestdate', '$orlot', '$resverstatus', '$revcomment', '$geneticpurity', '$lotldg_dop', '$lotldg_valperiod', '$lotldg_valupto', '$lotldg_srtyp', '$lotldg_srflg', '$rvflg', '$alflg', '$dispflg', '$altrids', '$alqtys', '$alnomps', '$spremflg', '$plantcode')";
	...
	$sql_main="update tbl_sloc_upsw set supflg=1 where slid='".$pid."'";
	$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
	//exit;
	echo "<script>window.location='select_sloc_upsw_op.php?p_id=$pid'</script>";	
}
?>
...
winHandle=window.open('upsw_slupdation_print.php?&pid='+pid,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" style="border-bottom:solid; border-bottom-color:#0BC5F4" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - SLOC Consolidation - UPS wise - Preview </td>
	    </tr></table></td>
