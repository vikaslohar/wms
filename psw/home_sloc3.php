...
$sql_tbl=mysqli_query($link,"select * from tbl_sloc_binw where plantcode='$plantcode' and surole='".$logid."' and supflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['slid'];	
	
	$s_sub="delete from tbl_sloc_binw_sub where slocid='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
}

	$s_sub="delete from tbl_sloc_binw where surole='".$logid."' and supflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
<title>psw- Transaction - Sloc Update Sub-Bin wise -  Home</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" >
	  ...
	  <table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" style="border-bottom:solid; border-bottom-color:#0BC5F4" >
	    <tr >
	      <td width="810" height="25">&nbsp;Transaction - SLOC Updation - Sub-Bin wise</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_sloc_binw.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
...
<?php
 ...
 $targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_sloc_binw where plantcode='$plantcode' and supflg=1 and stage='Pack' order by sldate desc, code desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_sloc_binw where plantcode='$plantcode' and supflg=1 and stage='Pack' order by sldate desc, code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_sloc_binw where supflg=1 and stage='Pack' order by code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_sloc_binw where supflg=1 and stage='Pack'");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
?>
