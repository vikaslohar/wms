...
$sql_tbl=mysqli_query($link,"select * from tbl_drying where plantcode='$plantcode' and stage='Pack' and   arrflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	*/
	...
	$s_sub="delete from tbl_drying where stage='Pack' and  arrflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	/**/
...
<title>psw - Transaction - QC Sampling Request</title>
...
$sql_arr_home=mysqli_query($link,"select * from tbl_drying where plantcode='$plantcode' and stage='Pack' and arrflg=1") or die(mysqli_error($link));
...
$sql_arr_home=mysqli_query($link,"select * from tbl_drying where plantcode='$plantcode' and stage='Pack'and arrflg=1  order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

 $total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_drying where plantcode='$plantcode' and stage='Pack' and arrflg=1");
...
<table align="center" border="0" width="600" cellspacing="0" cellpadding="0" bordercolor="#0BC5F4" style="border-collapse:collapse" > 
