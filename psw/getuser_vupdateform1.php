...
if($z1 == 0)
{
   $sql_main="insert into tbl_drying(arr_code, dryingdate,crop, variety,stage,drefno, plantcode)values('$c','$tdate1','$o','$p','Pack','$txtdrefno', '$plantcode')";
...
{
$sql_main="update tbl_drying set  dryingdate='$tdate1',crop='$o',variety='$p',stage='Pack'  where trid='$z1'";
if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=$z1;
...
$sql_qc=mysqli_query($link,"SELECT distinct(variety) FROM tbl_psw_main WHERE crop='".$row_tbl['crop']."' and variety NOT RLIKE '^[-+0-9.E]+$'") or die(mysqli_error($link)); 
$tt1=mysqli_num_rows($sql_qc);
$vv1 = mysqli_fetch_array($sql_qc);
echo $pp=$vv1['variety'];
$sql_month=mysqli_query($link,"select varietyid, popularname from tblvariety where cropname='".$row_tbl['crop']."' order by popularname Asc")or die(mysqli_error($link));
...
?>
	 ...
	 <?php
$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$row_tbl['variety']."' order by popularname Asc"); 
