<?php
	...
	{ 
		...
		$code=trim($_POST['code']);
	 	...
	 	$sql_in1="update tbl_sloc_binw2 set sldate='$tdate', wh='$who', bin='$bino', yearcode='$yearid_id', surole='$lgnid',stage='$stageo' where slid ='$pid'";
		
		if(mysqli_query($link,$sql_in1) or die(mysqli_error($link)))
		{
			$mainid=$pid;
			$sql_del="delete from tbl_sloc_binw_sub2 where slocid='$pid'";
			...
			foreach($p_array as $val)
			{
				if($val <> "")
				{
		 		$sql_in="insert into tbl_sloc_binw_sub2(slocid, crop, variety, lotno, whid, binid, plantcode) values('$mainid', '$crop', '$variety', '$val', '$whn', '$binn', '$plantcode')";
				mysqli_query($link,$sql_in) or die(mysqli_error($link));
				}
			}
		}
			echo "<script>window.location='add_sloc_binw_preview1.php?pid=$mainid'</script>";	
	}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" style="border-bottom:solid; border-bottom-color:#0BC5F4" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - SLOC Updation Bin wise</td>
	    </tr></table></td>
...
<?php
$sql_month=mysqli_query($link,"select binid, binname from tbl_bin where plantcode='$plantcode' and whid='".$row['wh']."' order by binname")or die("Error:".mysqli_error($link));
?>                  
...
<?php
$sql_month2=mysqli_query($link,"select sid, sname from tbl_subbin where plantcode='$plantcode' and binid='".$row['bin']."' order by sname")or die("Error:".mysqli_error($link));
?>                  
...
<tr >
<td valign="top" align="right"><a href="home_sloc2.php"><img src="../images/cancel.gif" border="0"  style="display:inline;cursor:pointer;" onclick="return confirm('Do You wish to Cancel this Transaction?');"/></a>&nbsp;<input name="Submit" type="image" src="../images/preview.gif" alt="Submit Value" onClick="return mySubmit();"  border="0" style="display:inline;cursor:pointer;" />&nbsp;&nbsp;</td>
</tr>
</table>
</div>
<?php
...
<table align="center" width="750" cellpadding="5" cellspacing="5" border="0" >
<tr >
<td valign="top" align="right"><a href="home_sloc2.php"><img src="../images/cancel.gif" border="0"  style="display:inline;cursor:pointer;" onclick="return confirm('Do You wish to Cancel this Transaction?');"/></a>&nbsp;</td>
</tr>
</table> 
</div>
<?php
