...
$sss="";
$qr="select distinct lotno, lotldg_alflg, lotldg_dispflg, lotldg_rvflg from tbl_lot_ldg_pack where plantcode='$plantcode' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and packtype='".$ups."' and lotldg_spremflg=1 $sss  order by lotdgp_id desc";
...
//echo $row['lotno']."<br/>";
$ccnt=0;
$sql_is=mysqli_query($link,"select distinct subbinid, binid, whid from tbl_lot_ldg_pack where plantcode='$plantcode' and  lotno='".$row['lotno']."'  and balqty > 0 group by subbinid, binid order by lotdgp_id asc") or die(mysqli_error($link));
$xt=mysqli_num_rows($sql_is);
 while($row_is=mysqli_fetch_array($sql_is))
 { 
$sql_is1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' andsubbinid='".$row_is['subbinid']."' and binid='".$row_is['binid']."' and whid='".$row_is['whid']."' and lotno='".$row['lotno']."' order by lotdgp_id asc ") or die(mysqli_error($link));
$row_is1=mysqli_fetch_array($sql_is1); 
//echo $row_is1[0]."  ";
$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='$plantcode' and lotdgp_id='".$row_is1[0]."' and balqty > 0 and lotldg_spremflg=1 order by lotdgp_id asc") or die(mysqli_error($link)); 
...
/*if($trid!="" && $trid > 0)
{
	$sqltblsub=mysqli_query($link,"select * from tbl_pspunpp2c where unp_id='".$trid."'") or die(mysqli_error($link));
	$subtbltot=mysqli_num_rows($sqltblsub);
	while($rowtblsub=mysqli_fetch_array($sqltblsub))
	{
		if($rowtblsub['unp_lotno']==$row['lotno'])
		{
			$flg++;
		}
	}
