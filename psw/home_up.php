...
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	/*$s_sub="delete from tbl_psw where arr_role='".$logid."' arrtrflag=1";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	*/
...
$sql_arr_home=mysqli_query($link,"select * from tbl_psw_main where plantcode='$plantcode' and arr_role='$logid' and arrtrflag=1 order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
