...
$sql_tbl=mysqli_query($link,"select * from tbl_psw_main where plantcode='$plantcode' and arr_role='".$logid."' and   arrtrflag=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	 $arrival_id=$row_tbl['arrival_id'];	
	
	$s_sub="delete from tbl_psw where arrival_id='".$arrival_id."'";
	...
	*/
	...
	$s_sub="delete from tbl_psw_main where arr_role='".$logid."'  and arrtrflag=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
$sql_arr_home=mysqli_query($link,"select * from tbl_psw_main where plantcode='$plantcode' and arr_role='$logid' and arrtrflag=1") or die(mysqli_error($link));
...
$sql_arr_home=mysqli_query($link,"select * from tbl_psw_main where plantcode='$plantcode' and arr_role='$logid' order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_psw_main where plantcode='$plantcode' and arr_role='$logid' and arrtrflag=1");
...
$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='$plantcode' and lotno='".$row_tbl_sub['lotno']."' and balqty > 0 order by lotdgp_id desc") or die(mysqli_error($link));
