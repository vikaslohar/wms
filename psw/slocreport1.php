...
<title>psw - Utility - SLOC Search</title>
...
if($sltyp=="whwise")
{
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='$plantcode' and whid='".$whid."' order by perticulars") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);

$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='$plantcode' and binid='".$bid."' and whid='".$whid."'") or die(mysqli_error($link));
...
{
$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='$plantcode' and sid='".$sid."' and binid='".$bid."' and whid='".$whid."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
$subbinn=$row_subbinn['sname'];
}		
?>	 
   <table align="center" border="1" bordercolor="#0BC5F4" cellspacing="0" cellpadding="0" width="750" style="border-collapse:collapse">
...
<td width="50%" align="right"  valign="middle" class="tblheading">Stage: Pack&nbsp;&nbsp;</td>
</tr>
...
if($sid=='ALL')
{ 
  $sql_tb="select distinct(lotldg_subbinid) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_whid='".$whid."' and lotldg_binid='".$bid."' and lotldg_sstage='Pack' order by lotldg_subbinid";  
}
 else
{
$sql_tb="select distinct(lotldg_subbinid) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_whid='".$whid."' and lotldg_binid='".$bid."' and lotldg_subbinid='".$sid."'  and lotldg_sstage='Pack' order by lotldg_subbinid";  
 }
...
{ 
$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_whid='".$whid."' and lotldg_binid='".$bid."' and lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_sstage='Pack' group by lotldg_lotno order by lotldg_id desc") or die(mysqli_error($link));  
$t1=mysqli_num_rows($sql_tbl1);
while($row_tbl1=mysqli_fetch_array($sql_tbl1))
{

$sql1=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='$plantcode' and lotldg_id='".$row_tbl1[0]."' and lotldg_sstage='Pack' and lotldg_balqty > 0")or die(mysqli_error($link));
...
$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='$plantcode' and sid='".$row_tbl['lotldg_subbinid']."' and binid='".$bid."' and whid='".$whid."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
...
else if($sltyp=="cvwise")
{
		...
		$sql_variety=mysqli_query($link,"select * from tblvariety where varietyid='".$txtvariety."' and actstatus='Active'") or die(mysqli_error($link));
		$row_variety=mysqli_fetch_array($sql_variety);
		$tot_variety=mysqli_num_rows($sql_variety);
		if($tot_variety>0)
		{
...
if($lotsltyp=='all')
{ 
  $sql_tb="select distinct(lotldg_lotno) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_crop='".$txtcrop."' and lotldg_variety='".$txtvariety."' and lotldg_sstage='Pack' order by lotldg_id desc";  
}
else
{
	$sql_tb="select distinct(lotldg_lotno) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_crop='".$txtcrop."' and lotldg_variety='".$txtvariety."' and lotldg_sstage='Pack' and lotldg_lotno='".$txtlot1."' order by lotldg_id desc";  
}
...
while($row_tbl=mysqli_fetch_array($sql_qry))
{
$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_crop='".$txtcrop."' and lotldg_variety='".$txtvariety."' and lotldg_lotno='".$row_tbl['lotldg_lotno']."' and lotldg_sstage='Pack' group by lotldg_lotno order by lotldg_id desc") or die(mysqli_error($link));  
$t1=mysqli_num_rows($sql_tbl1);
while($row_tbl1=mysqli_fetch_array($sql_tbl1))
{
...
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where plantcode='$plantcode' and lotldg_lotno='".$lotno."' and lotldg_sstage='Pack' ") or die(mysqli_error($link));
 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
 
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_sstage='Pack' and lotldg_lotno='".$lotno."'") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_arr_home=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='$plantcode' and lotldg_id='".$row_issue1[0]."' and lotldg_sstage='Pack' and lotldg_balqty > 0") or die(mysqli_error($link)); 
...
{
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='$plantcode' and whid='".$row_tbl_sub1['lotldg_whid']."' order by perticulars") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);
$wareh=$row_whouse['perticulars']."/";

$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='$plantcode' and binid='".$row_tbl_sub1['lotldg_binid']."' and whid='".$row_tbl_sub1['lotldg_whid']."'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname']."/";

$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='$plantcode' and sid='".$row_tbl_sub1['lotldg_subbinid']."' and binid='".$row_tbl_sub1['lotldg_binid']."' and whid='".$row_tbl_sub1['lotldg_whid']."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
$subbinn=$row_subbinn['sname'];
...
$whid1=$txtwh;
$bid1=$txtbin;
$sql_whouse1=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='$plantcode' and whid='".$whid1."' order by perticulars") or die(mysqli_error($link));
$row_whouse1=mysqli_fetch_array($sql_whouse1);
$wh=$row_whouse1['perticulars'];

if($bid1!="ALL")
{
$sql_binn1=mysqli_query($link,"select binname from tbl_bin where plantcode='$plantcode' and binid='".$bid1."' and whid='".$whid1."'") or die(mysqli_error($link));
...
?>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="400" bordercolor="#0BC5F4" style="border-collapse:collapse">
...
<?php
$sql_subbinn2=mysqli_query($link,"select * from tbl_subbin where plantcode='$plantcode' and whid='".$whid1."' and binid='".$bid1."' and status='".$cc."' order by sname") or die(mysqli_error($link));
...
<?php
$sql_subbinn111=mysqli_query($link,"select distinct(binid) from tbl_subbin where plantcode='$plantcode' and whid='".$whid1."' group by binid order by binid ") or die(mysqli_error($link));
$zz=mysqli_num_rows($sql_subbinn111);

while($rrr=mysqli_fetch_array($sql_subbinn111))
{
$sql_subbinn2=mysqli_query($link,"select * from tbl_subbin where plantcode='$plantcode' and whid='".$whid1."' and binid='".$rrr['binid']."' and status='".$cc."' order by binid") or die(mysqli_error($link));
$T=mysqli_num_rows($sql_subbinn2);
if($T==20)
{
$sql_binn11=mysqli_query($link,"select binname from tbl_bin where plantcode='$plantcode' and binid='".$rrr['binid']."'order by binname") or die(mysqli_error($link));
