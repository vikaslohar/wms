<?php
	...
	$y2=$row_yr['year2'];
		...
		$sql_good=mysqli_query($link,"select max(stlg_id) from tbl_lot_ldg") or die(mysqli_error($link));
		$row_good=mysqli_fetch_array($sql_good);
				
		$sql_good1=mysqli_query($link,"select stlg_trdate from tbl_lot_ldg where stlg_id='".$row_good[0]."'") or die(mysqli_error($link));
...
$sql_tbl=mysqli_query($link,"select * from tbl_sloc_binw2 where plantcode='$plantcode' and surole='".$logid."' and supflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['slid'];	
	
	$s_sub="delete from tbl_sloc_binw_sub2 where slocid='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
}

	$s_sub="delete from tbl_sloc_binw2 where surole='".$logid."' and supflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='add_arrival1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
...
<title>psw- Transaction - Sloc Update Bin wise -  Home</title>
...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" >
	  ...
	  <table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" style="border-bottom:solid; border-bottom-color:#0BC5F4" >
	    <tr >
	      <td width="810" height="25">&nbsp;Transaction - SLOC Updation - Bin wise</td>
	    </tr></table></td>
	  <td width="139" height="25" align="right" class="submenufont" >
	  <table border="3" align="right" bordercolor="#0BC5F4" cellspacing="0" cellpadding="0" width="130" style="border-collapse:collapse;">

<tr height="15" class="tbltitle" >
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_sloc_binw1.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
...
<?php
 ...
 $targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_sloc_binw2 where plantcode='$plantcode' and supflg=1 and stage='Pack' order by code desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_sloc_binw2 where plantcode='$plantcode' and supflg=1 and stage='Pack' order by code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_sloc_binw2 where supflg=1 and stage='Pack' order by code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_sloc_binw2 where supflg=1 and stage='Pack'");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
?>

<table align="center" border="0" width="700" cellspacing="0" cellpadding="0" bordercolor="#0BC5F4" style="border-collapse:collapse" > 
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">SLOC Updation - Bin wise</td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#0BC5F4" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="9%" align="center" valign="middle" class="tblheading">Date</td>
			  ...
			  <td width="34%" align="center" valign="middle" class="tblheading">Variety</td>
              <td width="21%" align="center" valign="middle" class="tblheading">Lot No.</td>
            </tr>
<?php
//$srno=1;
if($tot_arr_home > 0)
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$trdate=$row_arr_home['sldate'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;	
	$srole=$row_arr_home['surole'];
	$pid=$row_arr_home['slid'];
...
{
$quer3=mysqli_query($link,"select * from tblcrop where cropid='".$row_sub['crop']."'") or die(mysqli_error($link));
$noticia_class=mysqli_fetch_array($quer3);

$itemqry1=mysqli_query($link,"select * from tblvariety where varietyid='".$row_sub['variety']."' ") or die(mysqli_error($link));
...
<tr class="Light">
         ...
         <td width="10%" align="center" valign="middle" class="tblheading"><?php echo "SU".$row_arr_home['scode']."/".$yearid_id."/".$srole;?></td>
		 <td width="9%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
         <td width="23%" align="center" valign="middle" class="tblheading"><?php echo $noticia_class['cropname'];?></td>
         <td width="34%" align="center" valign="middle" class="tblheading"><?php echo $row_itm['popularname']?></td>
         <td width="21%" align="center" valign="middle" class="tblheading"><?php echo $row_sub['lotno']?></td>
</tr>
<?php
}
else
...
<tr >
<td valign="top" align="center"><a href="home_sloc_main.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;</td>
</tr>
</table>
