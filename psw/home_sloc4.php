...
$sql_tbl=mysqli_query($link,"select * from tbl_sloc_psw where plantcode='$plantcode' and surole='".$logid."' and supflg=0 and sluptype='barcodewise'") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	$s_sub2="delete from tbl_sloc_psw_sub2 where slocid='".$arrival_id."'";
	mysqli_query($link,$s_sub2) or die(mysqli_error($link));
}

	$s_sub="delete from tbl_sloc_psw where surole='".$logid."' and supflg=0 and sluptype='barcodewise'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
<title>PSW- Transaction - Sloc Update WH-RO -  Home</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" >
	  ...
	  <table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#0BC5F4" style="border-bottom:solid; border-bottom-color:#0BC5F4" >
	    <tr >
	      <td width="810" height="25">&nbsp;Transaction - SLOC Updation - WH-RO</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_sloc4.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
...
<?php
 ...
 $targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_sloc_psw where plantcode='$plantcode' and supflg=1 and sluptype='barcodewise' order by slid desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_sloc_psw where plantcode='$plantcode' and supflg=1 and sluptype='barcodewise' order by slid desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_sloc_psw where supflg=1 order by slid desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_sloc_psw where supflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="850" bordercolor="#0BC5F4" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="8%" align="center" valign="middle" class="tblheading">Date</td>
			  ...
			  <td width="12%" align="center" valign="middle" class="tblheading">E/P</td>
			  <td width="12%" align="center" valign="middle" class="tblheading">NoMP</td>
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$quer3=mysqli_query($link,"select * from tblcrop where cropid='".$row_arr_home['crop']."'") or die(mysqli_error($link));
	$noticia_class=mysqli_fetch_array($quer3);
	
	$itemqry1=mysqli_query($link,"select * from tblvariety where varietyid='".$row_arr_home['variety']."' ") or die(mysqli_error($link));
	$row_itm=mysqli_fetch_array($itemqry1);
	
	$nomp=0;
	$qty=0;
	$sltype='';
	$itemqry=mysqli_query($link,"select * from tbl_sloc_psw_sub where plantcode='$plantcode' and slocid='".$row_arr_home['slocid']."' ") or die(mysqli_error($link));
	while($rowitm=mysqli_fetch_array($itemqry))
	{
		$nomp=$nomp+$rowitm['nomp'];
		$qty=$qty+$rowitm['qty'];
		$sltype=$rowitm['sltype'];
	}
...
<tr class="Light">
         ...
         <td width="9%" align="center" valign="middle" class="tblheading"><?php echo "SU".$row_arr_home['scode']."/".$row_arr_home['yearcode']."/".$srole;?></td>
		 <td width="8%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
         <td width="12%" align="center" valign="middle" class="tblheading"><?php echo $noticia_class['cropname'];?></td>
         <td width="19%" align="center" valign="middle" class="tblheading"><?php echo $row_itm['popularname']?></td>
         <td width="14%" align="center" valign="middle" class="tblheading"><?php echo $row_arr_home['lotno']?></td>
		 <td width="12%" align="center" valign="middle" class="tblheading"><?php echo ucwords($sltype);?></td>
		 <td width="12%" align="center" valign="middle" class="tblheading"><?php echo $nomp?></td>
