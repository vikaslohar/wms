...
$sql_main=mysqli_query($link,"select lotldg_subbinid, lotldg_variety, lotldg_lotno, lotldg_crop from tbl_lot_ldg where plantcode='$plantcode' and lotldg_whid='".$wid."' and lotldg_binid='".$bid."' and lotldg_subbinid='".$slid."' group by lotldg_subbinid, lotldg_variety, lotldg_lotno order by lotldg_subbinid") or die(mysqli_error($link));  
...
if($dq[1]==000){$dcq=$dq[0];}else{$dcq=$row_tbl_sub['balqty'];}
	...
	$tmonth=substr($tdate,5,2);
	$tday=substr($tdate,8,2);
	echo $tdate=$tday."-".$tmonth."-".$tyear;
	if($tdate=="00-00-0000"){$tdate="";}
