<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$sql_arr=mysqli_query($link,"select * from tbl_pspunpp2c where plantcode='$plantcode' and unp_id='".$pid."'") or die(mysqli_error($link));
		while($row_arr=mysqli_fetch_array($sql_arr))
		{
			...
			$sql_sloc_sub22=mysqli_query($link,"select * from tbl_pspunpp2c_sub plantcode='$plantcode' and where unp_id='".$pid."'") or die(mysqli_error($link));
			while($row_sloc_sub22=mysqli_fetch_array($sql_sloc_sub22))
			{
				...
				$sql_issueg=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where plantcode='$plantcode' and lotldg_subbinid='".$subbinid1."'") or die(mysqli_error($link));
	
				while($row_issueg=mysqli_fetch_array($sql_issueg))
				{ 
					$sql_issueg1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='$plantcode' and lotldg_subbinid='".$subbinid1."' and lotldg_lotno='".$row_issueg['lotldg_lotno']."'") or die(mysqli_error($link));
				...
				$sql_issueg=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where plantcode='$plantcode' and subbinid='".$subbinid1."'") or die(mysqli_error($link));
				
				while($row_issueg=mysqli_fetch_array($sql_issueg))
				{ 
					$sql_issueg1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' and subbinid='".$subbinid1."' and lotno='".$row_issueg['lotno']."'") or die(mysqli_error($link));
					$row_issueg1=mysqli_fetch_array($sql_issueg1); 
				...
				if($cntg==0)
				{
					$sql_itmg="update tbl_subbin set status='Empty' where sid='$subbinid1'";
					mysqli_query($link,$sql_itmg) or die(mysqli_error($link));
				}
					
				$sql_mps1="update tbl_mpmain set mpmain_upflg='1' where mpmain_lotno='$lotno' and mpmain_dflg!=1";
				mysqli_query($link,$sql_mps1) or die(mysqli_error($link));
			}
			
			
			
			$sql_arrsub=mysqli_query($link,"select * from tbl_pspunpp2c_sub2 where plantcode='$plantcode' and unp_id='".$pid."'") or die(mysqli_error($link));
		...
		$sql_code="SELECT MAX(unp_code) FROM tbl_pspunpp2c where plantcode='$plantcode' and unp_yearcode='$yearid_id'  ORDER BY unp_code DESC";
		...
		$sql_btslm2="update tbl_pspunpp2c set unp_tflg=1, unp_code='$code' where unp_id='$pid'";
...
<title>PSW -Transaction - Pack Seed Unpacking - Pack to Condition (P2C) - Preview</title>
...
//return false;
	...
	{
		alert("Please Verify the PSW Lots first");
		return false;
	}*/
...
$sql_sub=mysqli_query($link,"Select * from tbl_pspunpp2c_sub where plantcode='$plantcode' and unp_id='$tid'") or die(mysqli_error($link));
...
<tr class="Light" height="20">
	...
	<td width="107" align="center" valign="middle" class="tblheading"><?php echo $enomp;?></td>
	...
	<td width="107" align="center" valign="middle" class="tblheading"><?php echo $nomp;?></td>
	...
	<td width="107" align="center" valign="middle" class="tblheading"><?php echo $bnomp;?></td>
