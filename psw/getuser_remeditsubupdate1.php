...
if(isset($_GET['srno1'])) { $srno1 = $_GET['srno1']; }
if(isset($_GET['subtrid'])) { $subtrid = $_GET['subtrid']; }
if(isset($_GET['txtlotno_1'])) { $txtlotno_1 = $_GET['txtlotno_1']; }	

if(isset($_GET['upspacktype_1'])) { $upspacktype_1 = $_GET['upspacktype_1']; }	
...
if($z1 == 0)
{
   $sql_main="insert into tbl_pswrem(pswrem_tcode, pswrem_date, logid, yearcode, plantcode, plantcode)values('$txtid','$tdate1','$logid', '$yearid_id', '$plantcode', '$plantcode')";
	if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
	{
		$mainid=mysqli_insert_id($link);
		$sql_sub="insert into tbl_pswrem_sub (pswrem_id, crop, variety, lotnumber, upssize, plantcode, plantcode)values('$mainid', '$txtcrop', '$txtvariety', '$txtlotno_1', '$upspacktype_1', '$plantcode', '$plantcode')";
...
{
	$sql_main="update tbl_pswrem set  pswrem_date='$tdate1', logid='$logid', yearcode='$yearid_id'  where pswrem_id='".$z1."'";
	if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
	{
		$mainid=$z1;
		$sql_sub="update tbl_pswrem_sub set pswrem_id='$mainid', crop='$txtcrop', variety='$txtvariety', lotnumber='$txtlotno_1', upssize='$upspacktype_1' where pswremsub_id='".$subtrid."'";
		if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
		{	
			$subid=$subtrid;
			$sql_del=mysqli_query($link,"delete from tbl_pswremsub_sub where pswremsub_id='".$subtrid."'") or die(mysqli_error($link));
			for($i=1; $i<=$srno1;$i++)
			{
				$wh="wh_".$i;
				$bin="bin_".$i;
...
<tr class="tblsubtitle" height="20">
  <td colspan="6" align="center" class="tblheading">Add Pack Seed Release</td>
</tr>
