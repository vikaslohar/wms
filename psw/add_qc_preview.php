<?php
	...
	//exit;
		...
		$sql_arr=mysqli_query($link,"select * from tbl_psw_main where plantcode='$plantcode' and arrival_id='".$pid."'") or die(mysqli_error($link));
	while($row_arr=mysqli_fetch_array($sql_arr))
	{
	//$partyid=$row_arr['party_id'];
	$tdate11=$row_arr['arrival_date'];
	$ststus=$row_arr['status'];	
	 $sql_arrsub=mysqli_query($link,"select * from tbl_psw where plantcode='$plantcode' and arrival_id='".$pid."'") or die(mysqli_error($link));
	...
	$res_code1=mysqli_query($link,$sql_code1)or die(mysqli_error($link));
		...
		$sql_sub_sub="insert into tbl_qctest(lotno, oldlot, crop, variety, srdate, sampleno, state, qc, trstage, yearid,logid, plantcode) values('$lotno','$zx[0]','$crop', '$variety', '$tdate1','$ncode1', '$state', 'UT', 'Pack', '$yearid_id','$logid', '$plantcode')";
		mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
/*$sql_itm="update tbl_lot_ldg_pack set Undertest='$ststus' arrival_id = '$pid'";
				mysqli_query($link,$sql_itm) or die(mysqli_error($link));
*/			
//exit;
	  $sql="update tbl_lot_ldg_pack set lotldg_qc='UT' where lotno='".$lotno."'";

	$a12=mysqli_query($link,$sql) or die(mysqli_error($link));
	
	 $sql5="update tbl_lot_ldg set lotldg_qc='UT' where orlot='".$zx[0]."'";

	$a125=mysqli_query($link,$sql5) or die(mysqli_error($link));
	//exit;
	}
		$sql_code="SELECT MAX(arr_code) FROM tbl_psw_main  where plantcode='$plantcode' ORDER BY arr_code DESC";
	...
	$res_code=mysqli_query($link,$sql_code)or die(mysqli_error($link));
		...
		else
		{
		 ...
		 $sql_main="update tbl_psw_main set arrtrflag=1 where arrival_id ='$pid'";

	$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
	/*echo $pid;	
		echo $sql_main="update tbl_psw set arrtrflag=1 where arrival_id ='$pid'";
...
<title>psw - Transaction - QC Sampling Request- Preview</title>
...
<td width="234"  align="left" valign="middle" class="tbltext" colspan="3">&nbsp;Pack Seed</td>
</tr>
...
<tr class="tblsubtitle" height="20">
			 ...
			 <!-- <td width="73" align="center" valign="middle" class="tblheading">UPS</td>-->
			  <td width="73" align="center" valign="middle" class="tblheading">NoP</td>
			  <td width="73" align="center" valign="middle" class="tblheading">NoMP</td>
...
$row31=mysqli_fetch_array($quer3);
 ...
 $row_tbl_sub['lotno'];
 $totqty=0; $totnob=0; $totnomp=0; $upssize=""; $totqc=""; $totdot=""; $totmost=""; $totgemp=""; $totgot=""; $reserve=""; $totsst=""; 	$sloc=""; 
...
if($penqty > 0)
{
	...
	if($upssize!="")
	$upssize=$upssize."<br/>".$row_issuetbl['packtype'];
	else
