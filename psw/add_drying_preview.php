<?php
	...
	//exit;
		...
		$sql_arr=mysqli_query($link,"select * from tbl_drying where where plantcode='$plantcode' trid='".$pid."'") or die(mysqli_error($link));
	while($row_arr=mysqli_fetch_array($sql_arr))
	{
	//$partyid=$row_arr['party_id'];
	$tdate11=$row_arr['arrival_date'];
	$ststus=$row_arr['status'];	
	 $sql_arrsub=mysqli_query($link,"select * from tbl_dryingsub where plantcode='$plantcode' and trid='".$pid."'") or die(mysqli_error($link));
...
<title>psw -Transaction - Drying Slip- Preview</title>
