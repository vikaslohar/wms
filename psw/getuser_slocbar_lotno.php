<?php
	...
	if(isset($_REQUEST['txtwhfrom']))
	{
		$txtwhto = $_REQUEST['txtwhfrom'];
	}
...
{	
	...
	window.location='getuser_sloc_lotno.php?crop='+val3+'&variety='+val4+'&sqq='+txtlot1+'&txtwhto='+txtwhto;
	return true;	
	}
}	
...
<td colspan="4" align="center" valign="middle"><table border="0" align="center" cellpadding="0" cellspacing="0" >
 ...
 <input type="hidden" name="txtwhto" value="<?php echo $txtwhto?>" /> 
   <?php
...
$wh=12;
if($txtwhto=="WH-RO")
{$qr="select distinct(lotno) from tbl_lot_ldg_pack where plantcode='$plantcode' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and whid='".$wh."' and balqty > 0  $sss";}
else
{$qr="select distinct(lotno) from tbl_lot_ldg_pack where plantcode='$plantcode' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and whid!='".$wh."' and balqty > 0  $sss";}
//echo $qr;
$sql_tbl_sub2=mysqli_query($link,$qr) or die(mysqli_error($link));
$tot_row=mysqli_num_rows($sql_tbl_sub2);
?>
...
{
if($txtwhto=="WH-RO")
$qr1="select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotno='".$row2['lotno']."' and whid='".$wh."'  order by lotdgp_id desc";
else
$qr1="select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotno='".$row2['lotno']."' and whid!='".$wh."'  order by lotdgp_id desc";
$sql_tbl_sub1=mysqli_query($link,$qr1) or die(mysqli_error($link));
while ($row1=mysqli_fetch_array($sql_tbl_sub1))
{
$qr1="select * from tbl_lot_ldg_pack where plantcode='$plantcode' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotdgp_id='".$row1[0]."'";
...
$ccnt=0;
if($txtwhto=="WH-RO")
$sql_is=mysqli_query($link,"select distinct subbinid, binid, whid from tbl_lot_ldg_pack where plantcode='$plantcode' and  lotno='".$row['lotno']."' and whid='".$wh."' group by subbinid, binid order by lotdgp_id asc") or die(mysqli_error($link));
else
$sql_is=mysqli_query($link,"select distinct subbinid, binid, whid from tbl_lot_ldg_pack where plantcode='$plantcode' and  lotno='".$row['lotno']."' and whid!='".$wh."' group by subbinid, binid order by lotdgp_id asc") or die(mysqli_error($link));
...
{ 
if($txtwhto=="WH-RO")
$sql_is1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' and subbinid='".$row_is['subbinid']."' and binid='".$row_is['binid']."' and whid='".$row_is['whid']."' and lotno='".$row['lotno']."' and whid='".$wh."' order by lotdgp_id asc ") or die(mysqli_error($link));
else
$sql_is1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='$plantcode' and subbinid='".$row_is['subbinid']."' and binid='".$row_is['binid']."' and whid='".$row_is['whid']."' and lotno='".$row['lotno']."' and whid!='".$wh."' order by lotdgp_id asc ") or die(mysqli_error($link));
$row_is1=mysqli_fetch_array($sql_is1); 
//echo $row_is1[0]."  ";
$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='$plantcode' and lotdgp_id='".$row_is1[0]."' and balqty > 0 and lotldg_spremflg=0 order by lotdgp_id asc") or die(mysqli_error($link)); 
$t=mysqli_num_rows($sql_istbl);
if($t > 0)
{
$r=mysqli_fetch_array($sql_istbl);
$ccnt++;
}
}
...
$ct=0;
$sql_mps=mysqli_query($link,"Select mpmain_lotno from tbl_mpmain where plantcode='$plantcode' and ( mpmain_trtype='PACKSMC' or mpmain_trtype='PACKNMC') and mpmain_lotno='".$row['lotno']."' and mpmain_dflg=0 and mpmain_upflg=0") or die(mysqli_error($link));
...
if($tot_mps > 0)
{
	while($row_mps=mysqli_fetch_array($sql_mps))
	{
		...
		{
			if($row['lotno']==$lotarr[$i])
			{
				$ct++;
			}
		}
	}
	//echo $row['lotno']." = ".$ct."<br />";
}
	//echo $ct."</br>"; 					
$sql_mpl=mysqli_query($link,"Select mpmain_lotno from tbl_mpmain where plantcode='$plantcode' and mpmain_trtype='PACKLMC' and mpmain_variety='".$row['lotldg_variety']."' and mpmain_upssize='".$ups."' and mpmain_dflg=0 and mpmain_upflg=0 and mpmain_alflg=0") or die(mysqli_error($link));
...
<tr class="light" height="20">
  <td colspan="4" align="left" class="tblheading">&nbsp;4. Lots are not Packed.</td>
</tr>
<?php
}
?>
