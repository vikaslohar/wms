	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if(mysqli_query($link,$sql_srmain) or die(mysqli_error($link)))
		{
			...
			for($i=0; $i<count($lot1); $i++)
			{
				if($lot1[$i]<>"")
				{
					$sql_srsub="Insert into tbl_softr_sub (softr_id, softrsub_lotno, softrsub_srtyp, softrsub_srflg) values('$tid', '$lot1[$i]', '$srt1[$i]', '1')";
...
if($type=="sllot")	
{
$sql="SELECT distinct orlot, lotldg_got1 FROM tbl_lot_ldg WHERE lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_srflg='0' and lotldg_qc!='Fail'";
}
else
{
$sql="SELECT distinct orlot, lotldg_got1 FROM tbl_lot_ldg WHERE lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_whid='".$wh."' and lotldg_binid='".$bin."' and lotldg_subbinid='".$subbin."' and lotldg_srflg='0' and lotldg_qc!='NUT' and lotldg_qc!='Fail'";
}
...
//echo $flg;
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where  orlot='".$row_arr_home['orlot']."'  and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT')") or die(mysqli_error($link));
...
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and orlot='".$row_arr_home['orlot']."'  and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT')") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT')") or die(mysqli_error($link)); 
...
$sql_issue=mysqli_query($link,"select distinct whid, subbinid, binid from tbl_lot_ldg_pack where orlot='".$row_arr_home['orlot']."' and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT')") or die(mysqli_error($link));
$zz=mysqli_num_rows($sql_issue);
//if($zz==0)$flg=1;
 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
...
$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and orlot='".$row_arr_home['orlot']."'  and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT')") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_issue1[0]."' and lotldg_srflg='0' and lotldg_qc!='Fail' and (lotldg_qc='UT' OR lotldg_qc='RT')") or die(mysqli_error($link)); 
 $xxz=mysqli_num_rows($sql_issuetbl);
//if($xxz==0)$flg=1;
 while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
 { 
...
if($flg==0)
{
$sql_srsub=mysqli_query($link,"SELECT * FROM tbl_softr_sub WHERE softr_id='".$tid."' and softrsub_lotno='".$row_arr_home['orlot']."'") or die(mysqli_error($link));
