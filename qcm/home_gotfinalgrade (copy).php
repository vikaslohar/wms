...
/*$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_btsplatingsub_sub2 where btsplatingss2_resultverifyflg=1 and btsplatingss2_abortflg=0 order by btsplatingss2_resultdate asc,btsplatingss2_lorryno asc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);

//$total_results = mysql_result(mysqli_query($link,"SELECT COUNT(*) as Num FROM tbl_qctest where bflg=1   and qcflg=0"),0); 

$query = "select * from tbl_btsplatingsub_sub2 where btsplatingss2_resultverifyflg=1 and btsplatingss2_abortflg=0 order by btsplatingss2_resultdate asc,btsplatingss2_lorryno asc";
...
<tr class="Light">
	...
	<td width="12%" align="center" valign="middle" class="smalltbltext"><?php echo $crop;?></td>
	<td width="13%" align="center" valign="middle" class="smalltbltext"><?php echo $variety;?></td>
	...
	<td width="5%" align="center" valign="middle" class="smalltbltext"><?php echo $btsgrade;?></td>
...
$targetpage1 = $PHP_SELF; 
  ...
  $sql_arr_home1=mysqli_query($link,"select * from tbl_gotgrade order by gotgrade_date desc,gotgrade_id desc LIMIT $start1, $limit1") or die(mysqli_error($link));
 $tot_arr_home1=mysqli_num_rows($sql_arr_home1);

//$total_results = mysql_result(mysqli_query($link,"SELECT COUNT(*) as Num FROM tbl_qctest where bflg=1   and qcflg=0"),0); 

$query1 = "select * from tbl_gotgrade order by gotgrade_date desc,gotgrade_id desc";
