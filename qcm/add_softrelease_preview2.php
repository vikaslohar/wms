<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$sql_srsub=mysqli_query($link,"SELECT * FROM tbl_softr_sub2 WHERE softr_id='".$tid."'") or die(mysqli_error($link));
		...
		while($row_srsub=mysqli_fetch_array($sql_srsub))
		{
			...
			$slqty=0;
			$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where  orlot='".$lotno."'  and lotldg_srflg='0' and lotldg_got!='Fail' and lotldg_got!='OK'") or die(mysqli_error($link));
			$zz=mysqli_num_rows($sql_issue);
			while($row_issue=mysqli_fetch_array($sql_issue))
			{ 
				$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and orlot='".$lotno."'  and lotldg_srflg='0' and lotldg_got!='Fail' and lotldg_got!='OK'") or die(mysqli_error($link));
				$row_issue1=mysqli_fetch_array($sql_issue1); 
				
				$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and lotldg_srflg='0' and lotldg_got!='Fail' and lotldg_got!='OK'") or die(mysqli_error($link)); 
			...
			$slqty=0;
			$sql_issue=mysqli_query($link,"select distinct whid, subbinid, binid from tbl_lot_ldg_pack where orlot='".$lotno."' and lotldg_srflg='0' and lotldg_got!='Fail' and lotldg_got!='OK'") or die(mysqli_error($link));
			$zz=mysqli_num_rows($sql_issue);
			while($row_issue=mysqli_fetch_array($sql_issue))
			{ 
				$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and orlot='".$lotno."'  and lotldg_srflg='0' and lotldg_got!='Fail' and lotldg_got!='OK'") or die(mysqli_error($link));
				$row_issue1=mysqli_fetch_array($sql_issue1); 
				
				$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_issue1[0]."' and lotldg_srflg='0' and lotldg_got!='Fail' and lotldg_got!='OK'") or die(mysqli_error($link)); 
				$xxz=mysqli_num_rows($sql_issuetbl);
				while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
				{ 
		...
		$sql_code="SELECT MAX(softr_code) FROM tbl_softr2  where yearcode='$yearid_id'  ORDER BY softr_code DESC";
...
$sql2="SELECT distinct orlot, lotldg_got1 FROM tbl_lot_ldg_pack WHERE lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_srflg='0' and lotldg_qc!='Fail' order by orlot";
}
else
{
...
$sql2="SELECT distinct orlot, lotldg_got1 FROM tbl_lot_ldg_pack WHERE lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and whid='".$wh."' and binid='".$bin."' and subbinid='".$subbin."' and lotldg_srflg='0' and lotldg_qc!='Fail' order by orlot";
}
...
$sql_srsub24=mysqli_query($link,"SELECT * FROM tbl_softr_sub2 WHERE softr_id='".$tid."' and softrsub_lotno='".$orlt."'") or die(mysqli_error($link));
