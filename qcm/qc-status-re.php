<?php
	...
	$itemid = $_REQUEST['txtitem'];
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='cropwise-variety_report1.php?sdate=$sdate&edate=$edate&txtclass=$cid&txtitem=$itemid'</script>";
		}
		else
		 {?>
		 <script>
		...
		if($flg!=0)
		{*/
		/*echo "<script>window.location='discardreport1.php'</script>";	*/
		/*}
		else
		{
		?>
...
function modetchk(class)
{
showUser(class,'item','indents','','','','','');
}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <td width="940" class="Mainheading" height="25">
	    ...
	    <tr >
	      <td width="813" height="25"class="Mainheading"  >Quality Staus Report</td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
<tr class="tblsubtitle" height="25">
  <td colspan="4" align="center" class="tblheading">Quality Staus   Report</td>
</tr>
...
?>
		 <?php 
$qry=mysqli_query($link,"select classification_id, classification from tbl_classification order by classification") or die(mysqli_error($link));
?>
		 <?php 
$qry=mysqli_query($link,"select classification_id, classification from tbl_classification order by classification") or die(mysqli_error($link));
?>
...
<td width="318"  align="right"  valign="middle" class="tblheading">Crop</td>                                   
           <td width="454" colspan="3" align="left"  valign="middle">&nbsp;<select class="tbltext" name="txtclass" style="width:230px;"   onchange="modetchk(this.value)">
...
<option value="ALL" >--------------------------ALL---------------------</option>
	<?php while($noticia_class = mysqli_fetch_array($qry)) { ?>
		...
		<?php } ?>
		 ...
		 <tr class="Light" height="25">
           <td width="318"  align="right"  valign="middle" class="tblheading"> Display Lot after Date Of Test</td>                                   
           <td align="left"  valign="middle" colspan="3">&nbsp;<select class="tbltext" name="txtclass" style="width:230px;"   onchange="modetchk(this.value)">
<option value="ALL" >--------------------------ALL---------------------</option>
	<?php while($noticia_class = mysqli_fetch_array($qry)) { ?>
		<option  value="<?php echo $noticia_class['classification_id'];?>" />   
