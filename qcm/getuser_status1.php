	...
	$lotn=$_POST['lotnochk'];
		...
		if($btnval!=0 && $btnval!=2)
		{
			...
			if($tot_tbl > 0 && $tot_chk > 1)
			{
				...
				$sql_sub="update tbl_lot_ldg set lotldg_qc='$qcresult', lotldg_got='$gotresult' where orlot='$lotn'";
			...
			/*$sql_chk=mysqli_query($link,"select * from tbl_qctest where oldlot='$lotnochk'") or die(mysqli_error($link));
			$tot_chk=mysqli_num_rows($sql_chk);
			if($tot_chk == 1)
			{
				$sql_chk22=mysqli_query($link,"select * from tbl_lot_ldg where orlot='$lotn' order by lotldg_id desc") or die(mysqli_error($link));
				$tot_chk=mysqli_num_rows($sql_chk22);
				if($tot_chk22 > 0)
				{
				$row_chk22=mysqli_fetch_array($sql_chk22);
				$qcer=$row_chk22['lotldg_qc'];
				$sql_tbl="update tbl_lot_ldg set lotldg_qc='$qcer' where orlot='".$lotnochk."'";
				$a=mysqli_query($link,$sql_tbl)or die(mysqli_error($link));
				}
			}*/
		...
		else if($btnval==0)
		{
			...
			$sql_in1="update tbl_qctest set spdate='$tdate', bflg=1, stsno='$code' where tid='$tid'";	
			$aa=mysqli_query($link,$sql_in1)or die(mysqli_error($link));
		}
		else
		{
