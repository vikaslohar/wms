 ...
 $t=0;
	while($row_arr_home25=mysqli_fetch_array($sql_arr_home))
	{
		...
		{ 
			...
			$sql_arr_home2=mysqli_query($link,"select max(tid) from tbl_qctest where oldlot='".$row_arr_home25['orlot']."' and state='P/M/G/T' order by tid desc") or die(mysqli_error($link));
