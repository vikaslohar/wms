...
if (isset($_REQUEST['mobile1'])) {
	...
	else
	{
		...
		$sow_nurrangeno = $_REQUEST['sow_nurrangeno'];
		if(isset($_REQUEST['treynumber'])){$sow_treynumber = $_REQUEST['treynumber'];}else{$sow_treynumber='';}
		if(isset($_REQUEST['nurserymedium'])){$sow_nurserymedium = $_REQUEST['nurserymedium'];}else {$sow_nurserymedium = '';}
		
		$user_login = $db->GetGoTSowingUpdate($sampleno, $dosow, $sow_nurplotno, $sow_noofseeds, $sow_sptype, $scode, $sow_state, $sow_loc, $sow_noofcellstray, $sow_nooftraylot, $sow_bedno, $sow_direction, $sow_noofrows, $sow_noofplants, $retest, $retesttype, $retest_reason, $sow_nurrangeno, $sow_nurserymedium,$sow_treynumber);
