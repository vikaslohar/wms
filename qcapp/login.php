...
//	print_r($user_login);
	if ($user_login != false) 
	{
		...
		$response["user"]["oprtype"] = $user_login["userlevel"];
		
		//$user_plantcode = $db->getPlantdetails();
		$response["user"]["pcode"] = $user_login["plantcode"];
		...
		//echo json_encode($response);
		$user_compflg6 = $db->getQcOprDetails();
		//print_r($user_compflg);
		//exit;
		if ($user_compflg6 == false) 
		{
			// user is not found with the credentials
			$response["error"] = TRUE;
			$response["error_msg"] = "Operator details not found. Please try again or contact Administrator";
			//echo json_encode($response);
		}
		else
		{
			//print_r($user_compflg);
			//$response["user"]["comparray"] =$user_compflg;
			$response["user"]["qcoprarray"] = $user_compflg6;
			//echo json_encode($response);
		}
