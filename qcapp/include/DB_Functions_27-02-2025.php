		...
		if ($stmt_sub->num_rows==0) 
		{
			...
			if ($stmt_main->num_rows==0) 
			{
				...
				if ($stmt->num_rows > 0) 
				{
					...
					$stmt_got3 = $this->conn_ps->prepare("Insert into tbl_btssdldispm (btssdldispm_code, btssdldispm_tflg, oprid) values(?,2, ?) ");
					$stmt_got3->bind_param("ii", $tcode, $oprid);
					if($result_got3=$stmt_got3->execute())
					{	
