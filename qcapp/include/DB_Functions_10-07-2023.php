...
//return "srf no".$srf_tcode;
				...
				if($totcnt>20 || $totcnt==0)
				{
					...
					if ($stmt_srfm2->num_rows > 0) {
						...
						$srfno=$plantcode.$ycode.$baryrcode."SRF".sprintf("%00004d",$srf_tcode);
						$srfseries='new';
						//$stmt_srfm2->fetch();
						$stmt_srfm2->close();
					}
...
//return "srf no2 ".$srf_tcode;
	...
	public function GetSampleCollect($scode, $sampleno, $smpdate, $srfseries, $srfno, $email) {
		$samno=str_split($sampleno);
		$sampno=$samno[2].$samno[3].$samno[4].$samno[5].$samno[6].$samno[7];
		$plantid=$samno[0];
		$yearid=$samno[1];
		$sampflg=0;
		$flg=0;
        $stmt = $this->conn_ps->prepare("SELECT tid, lotno, oldlot  FROM tbl_qctest WHERE qcflg=0 and sampleno=? and yearid=? and plantcode=? ORDER BY tid DESC");
        $stmt->bind_param("sss", $sampno, $yearid, $plantid);
        $stmt->execute();
        $stmt->store_result();
		$userSR = array(); $user24=array();
        ...
        if ($stmt->num_rows > 0) {
			...
			while($stmt->fetch())
			{
				...
				//else {$smpdate='';}
				$dt=date("d-m-Y h:i:sa"); $one=1; $dt2=date("Y-m-d");
				if($smpdate!=''){
...
//return "INSERT INTO tbl_qcsrf (srf_date, srf_tcode, srf_code, srf_srfno, srf_tid, srf_lotno, srf_orlot, srf_sampleno, srf_logid, srf_yrcode, srf_year) VALUES ('$dt2','$srf_tcode','$srf_code','$srfno','$tid','$lotno','$oldlot','$sampleno','$scode','$ycode','$baryrcode') ";
							...
							$result_srfm6=$stmt_srfm6->execute();
							$stmt_srfm6->close();
					...
					{
						$flg=1;
						$stmt_got = $this->conn_ps->prepare("UPDATE tbl_gottest SET gottest_spdate=?, gottest_bflg=? WHERE gottest_sampleno=? and yearid=? and plantcode=? ");
						$stmt_got->bind_param("sisss", $smpdate, $one, $sampno, $yearid, $plantid);
	...
	public function GetSamplePPUpdate($sampleno, $samplewt, $pureseed, $pureseedper, $imseed, $imseedper, $lightseed, $lightseedper, $ocseedno, $ocseedinkg, $odvseedno, $odvseedinkg, $dcseed, $dcseedper, $phseedno, $phseedinkg, $ppphoto, $scode, $new_retImageString_ret) {
		$samno=str_split($sampleno);
		$sampno=$samno[2].$samno[3].$samno[4].$samno[5].$samno[6].$samno[7];
		$plantid=$samno[0];
		$yearid=$samno[1];
        ...
        if ($stmt->num_rows > 0) {
			...
			if ($stmtsamp->num_rows == 0) 
			{
				...
				$stmt_samp->bind_param("sssssssssssssssssssi", $sampleno, $samplewt, $pureseed, $pureseedper, $imseed, $imseedper, $lightseed, $lightseedper, $ocseedno, $ocseedinkg, $odvseedno, $odvseedinkg, $dcseed, $dcseedper, $phseedno, $phseedinkg, $scode, $dt, $new_retImageString_ret, $two);
				$result_samp=$stmt_samp->execute();
				if($result_samp){$flg++;}
				$stmt_samp->close();
			}
...
public function GetSampleGempInfo($sampleno) {
	...
	public function GetGempSetupUpdate($sampleno, $testtype, $sgtnorep, $fgtnorep, $fgtmtype, $docsrefno) {
		$samno=str_split($sampleno);
		$sampno=$samno[2].$samno[3].$samno[4].$samno[5].$samno[6].$samno[7];
		$plantid=$samno[0];
		$yearid=$samno[1];
        ...
        if ($stmt->num_rows > 0) {
			...
			if ($stmtsamp->num_rows == 0) 
			{
				...
				$stmt_samp = $this->conn_ps->prepare("Insert into tbl_qcgdata (qcg_sampleno, qcg_testtype, qcg_seedsize, qcg_noofseedinonerep, qcg_sgtnoofrep, qcg_vigtesttype, qcg_vignoofrep, qcg_setupflg, qcg_setupdt, qcg_noofseedinonerepfgt, qcg_docsrefno) values(?,?,?,?,?,?,?,?,?,?,?) ");
				$stmt_samp->bind_param("sssssssisis", $sampleno, $testtype, $seedsize, $nosior, $sgtnorep, $fgtmtype, $fgtnorep, $one, $sdt, $nosiorfgt, $docsrefno);
				$result_samp=$stmt_samp->execute();
				if($result_samp){$flg++;}
				$stmt_samp->close();
			}
        ...
        if ($stmt->num_rows > 0) {
			...
			if ($stmtsamp->num_rows > 0) 
			{
				...
				if($testtype=="Standard Germination Test")
				{
					...
					{
						$stmt_samp = $this->conn_ps->prepare("UPDATE tbl_qcgdata SET qcg_sgtoobnormal1=?, qcg_sgtoobnormal2=?, qcg_sgtoobnormal3=?, qcg_sgtoobnormal4=?, qcg_sgtoobnormal5=?, qcg_sgtoobnormal6=?, qcg_sgtoobnormal7=?, qcg_sgtoobnormal8=?, qcg_sgtoobnormalavg=?, qcg_sgtoobnormaldt=?, qcg_sgtoobnormallogid=?, qcg_sgtoneobflg=?, qcg_sgtremark=? where qcg_sampleno=?  and qcg_germpflg!=1 and qcg_retestflg=0");
						$stmt_samp->bind_param("iiiiiiiiississ", $sgtoobnormal1, $sgtoobnormal2, $sgtoobnormal3, $sgtoobnormal4, $sgtoobnormal5, $sgtoobnormal6, $sgtoobnormal7, $sgtoobnormal8, $sgtoobnormalavg, $sgtoobnormaldt, $scode, $one, $remarks, $sampleno);
						$result_samp=$stmt_samp->execute();
						if($result_samp){$flg++;}
						$stmt_samp->close();
					}
					...
					if($sgtfcfltype=="Final Count")
					{
						...
						$stmt_samp->bind_param("iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiissiississs", $sgtnormal1, $sgtnormal2, $sgtnormal3, $sgtnormal4, $sgtnormal5, $sgtnormal6, $sgtnormal7, $sgtnormal8, $sgtnormalavg, $sgtabnormal1, $sgtabnormal2, $sgtabnormal3, $sgtabnormal4, $sgtabnormal5, $sgtabnormal6, $sgtabnormal7, $sgtabnormal8, $sgtabnormalavg, $sgthardfug1, $sgthardfug2, $sgthardfug3, $sgthardfug4, $sgthardfug5, $sgthardfug6, $sgthardfug7, $sgthardfug8, $sgthardfugavg, $sgtdead1, $sgtdead2, $sgtdead3, $sgtdead4, $sgtdead5, $sgtdead6, $sgtdead7, $sgtdead8, $sgtdeadavg, $scode, $sgtdt, $sgtgermp, $one, $sgtvremark, $oprremark, $two, $fdt, $remarks, $sampleno);
						$result_samp=$stmt_samp->execute();
						if($result_samp){$flg++;}
						$stmt_samp->close();
					}
				...
				if($testtype=="Field Germination Test")
				{
					...
					{
						$stmt_samp = $this->conn_ps->prepare("UPDATE tbl_qcgdata SET qcg_vigtesttype=?, qcg_vigoobnormal1=?, qcg_vigoobnormal2=?, qcg_vigoobnormal3=?, qcg_vigoobnormal4=?, qcg_vigoobnormal5=?, qcg_vigoobnormal6=?, qcg_vigoobnormal7=?, qcg_vigoobnormal8=?, qcg_vigoobnormalavg=?, qcg_vigoobnormaldt=?, qcg_vigoobnormallogid=?, qcg_vigoneobflg=?, qcg_fgtremark=? where qcg_sampleno=?  and qcg_germpflg!=1 and qcg_retestflg=0");
						$stmt_samp->bind_param("siiiiiiiiississ", $fgtmtype, $fgtoobnormal1, $fgtoobnormal2, $fgtoobnormal3, $fgtoobnormal4, $fgtoobnormal5, $fgtoobnormal6, $fgtoobnormal7, $fgtoobnormal8, $fgtoobnormalavg, $fgtoobnormaldt, $scode, $one, $remarks, $sampleno);
						$result_samp=$stmt_samp->execute();
						if($result_samp){$flg++;}
						$stmt_samp->close();
					}
					...
					{
						$stmt_samp = $this->conn_ps->prepare("UPDATE tbl_qcgdata SET qcg_vigtesttype=?, qcg_vignormal1=?, qcg_vignormal2=?, qcg_vignormal3=?, qcg_vignormal4=?, qcg_vignormal5=?, qcg_vignormal6=?, qcg_vignormal7=?, qcg_vignormal8=?, qcg_vignormalavg=?, qcg_vigabnormal1=?, qcg_vigabnormal2=?, qcg_vigabnormal3=?, qcg_vigabnormal4=?, qcg_vigabnormal5=?, qcg_vigabnormal6=?, qcg_vigabnormal7=?, qcg_vigabnormal8=?, qcg_vigabnormalavg=?, qcg_vigdead1=?, qcg_vigdead2=?, qcg_vigdead3=?, qcg_vigdead4=?, qcg_vigdead5=?, qcg_vigdead6=?, qcg_vigdead7=?, qcg_vigdead8=?, qcg_vigdeadavg=?, qcg_viglogid=?, qcg_vigdt=?, qcg_viggermp=?, qcg_vigflg=?, qcg_vigvremark=?, qcg_oprremark=?, qcg_germpflg=?, qcg_germpdatadt=?, qcg_fgtremark=?  where qcg_sampleno=?  and qcg_germpflg!=1 and qcg_retestflg=0");
						$stmt_samp->bind_param("siiiiiiiiiiiiiiiiiiiiiiiiiiissiississs", $fgtmtype, $fgtnormal1, $fgtnormal2, $fgtnormal3, $fgtnormal4, $fgtnormal5, $fgtnormal6, $fgtnormal7, $fgtnormal8, $fgtnormalavg, $fgtabnormal1, $fgtabnormal2, $fgtabnormal3, $fgtabnormal4, $fgtabnormal5, $fgtabnormal6, $fgtabnormal7, $fgtabnormal8, $fgtabnormalavg, $fgtdead1, $fgtdead2, $fgtdead3, $fgtdead4, $fgtdead5, $fgtdead6, $fgtdead7, $fgtdead8, $fgtdeadavg, $scode, $fgtdt, $fgtgermp, $one, $fgtvremark, $oprremark, $two, $fdt, $remarks, $sampleno);
						$result_samp=$stmt_samp->execute();
						if($result_samp){$flg++;}
						$stmt_samp->close();
					}
	...
	public function GetSampleDensityUpdate($sampleno, $samplewt, $scode) {
		$samno=str_split($sampleno);
		$sampno=$samno[2].$samno[3].$samno[4].$samno[5].$samno[6].$samno[7];
		$plantid=$samno[0];
		$yearid=$samno[1];
        ...
        if ($stmt->num_rows > 0) {
			...
			if ($stmtsamp->num_rows == 0) 
			{
				$stmt_samp = $this->conn_ps->prepare("Insert into tbl_densitydata (density_date, density_crop, density_variety, density_lotno, density_sampleno, density_orlot, density_rawwtdate, density_rawsampwt, density_rawwtflg) values(?,?,?,?,?,?,?,?,?) ");
				$stmt_samp->bind_param("ssssssssi", $dt, $crop, $variety, $lotno, $sampleno, $oldlot, $dt, $samplewt, $one);
				$result_samp=$stmt_samp->execute();
				if($result_samp){$flg++;}
				$stmt_samp->close();
			}
