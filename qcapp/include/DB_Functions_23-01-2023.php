...
public function GetSampleGempInfo($sampleno) {
	...
	public function GetSampleInfobtsdisp($sampleno) {
		...
		//return "SELECT gottest_tid, gottest_dosdate, gottest_crop, gottest_variety, gottest_lotno, gottest_totnosamp, gottest_smpdispstate, gottest_smpdisploc  FROM tbl_gottest WHERE gottest_gotflg=0 and gottest_gotsampdflg=1 and gottest_sampleno=$sampno and yearid='$yearid' ORDER BY gottest_tid DESC";
		 $stmt = $this->conn_ps->prepare("SELECT tid, srdate, crop, variety, lotno, trstage, state, spdate, sampcldate, sampclrole  FROM tbl_qctest WHERE sampleno=? and yearid=? ORDER BY tid DESC");
	...
	public function GetSample_Germination_Update($sampleno) {
		...
		//return "SELECT gottest_tid, gottest_dosdate, gottest_crop, gottest_variety, gottest_lotno, gottest_totnosamp, gottest_smpdispstate, gottest_smpdisploc  FROM tbl_gottest WHERE gottest_gotflg=0 and gottest_gotsampdflg=1 and gottest_sampleno=$sampno and yearid='$yearid' ORDER BY gottest_tid DESC";
		$stmt = $this->conn_ps->prepare("SELECT tid, srdate, crop, variety, lotno, trstage, state, spdate, sampcldate, sampclrole, oldlot  FROM tbl_qctest WHERE sampleno=? and yearid=? ORDER BY tid DESC");
	...
	public function GetSampleDensityUpdate($sampleno, $samplewt, $scode) {
        ...
        $stmt = $this->conn_ps->prepare("SELECT tid, srdate, crop, variety, lotno, trstage, state, spdate, sampcldate, sampclrole, oldlot  FROM tbl_qctest WHERE qcflg=0 and sampleno=? and yearid=? ORDER BY tid DESC");
        $stmt->bind_param("ss", $sampno, $yearid);
        $stmt->execute();
        $stmt->store_result();
		$userSR = array(); $user24=array();
