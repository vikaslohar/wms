...
winHandle=window.open('preport_plant22.php?sdate='+sdate+'&txtcrop='+loc+'&txtvariety='+itemid+'&edate='+edate+'&txtvisualck='+typ,'WelCome','top=20,left=80,width=1000,height=900,scrollbars=yes');
if(winHandle==null){
...
<?php	
	...
	if($itemid=="ALL")
	{
	$sql_arrsubcrop=mysqli_query($link,"select distinct lotcrop from tblarrival_sub order by lotcrop asc") or die(mysqli_error($link));
	}
	else
	{
	...
	$sql_arrsubcrop=mysqli_query($link,"select distinct lotcrop from tblarrival_sub where plantcode='$plantcode' AND lotcrop='".$crop."' order by lotcrop asc") or die(mysqli_error($link));
	}
...
<table align="center" border="0" cellspacing="0" cellpadding="0" width="650" style="border-collapse:collapse">
   <!--	<tr height="25" >
	...
	{
		$cnt=0;
		$sql_subvar=mysqli_query($link,"select * from tblarrival_sub where plantcode='$plantcode' AND lotcrop='".$row_arrsubcrop[0]."' order by lotvariety") or die(mysqli_error($link));
		while($row_subvar=mysqli_fetch_array($sql_subvar))
		{
			$sql_arr=mysqli_query($link,"select * from tblarrival where plantcode='$plantcode' AND arrival_id='".$row_subvar['arrival_id']."' and arrival_date <= '$edate' and arrival_date >= '$sdate' and arrival_type='".$typ."' and arrtrflag=1 order by arrival_id") or die(mysqli_error($link));
...
<?php 	
	if($loc=="ALL" && $itemid!="ALL")
		$sql_arrsubvar=mysqli_query($link,"select distinct sstage, lotvariety from tblarrival_sub where plantcode='$plantcode' AND lotcrop='".$row_arrsubcrop[0]."' group by lotvariety, sstage") or die(mysqli_error($link));
	else if($loc!="ALL" && $itemid!="ALL")
		$sql_arrsubvar=mysqli_query($link,"select distinct sstage, lotvariety from tblarrival_sub where plantcode='$plantcode' AND lotcrop='".$row_arrsubcrop[0]."' and lotvariety='".$vit."' group by lotvariety, sstage") or die(mysqli_error($link));
	else
	{
		$sql_arrsubvar=mysqli_query($link,"select distinct sstage, lotvariety from tblarrival_sub where plantcode='$plantcode' AND lotcrop='".$row_arrsubcrop[0]."' group by lotvariety, sstage") or die(mysqli_error($link));
	}
	...
	while($row_arrsubvar=mysqli_fetch_array($sql_arrsubvar))
	{
		...
		$sql_var=mysqli_query($link,"select * from tblarrival_sub where lotvariety='".$row_arrsubvar[1]."' and sstage='".$row_arrsubvar[0]."' order by lotvariety asc") or die(mysqli_error($link));
		while($row_var=mysqli_fetch_array($sql_var))
		{
			$sql_arrmain=mysqli_query($link,"select * from tblarrival where plantcode='$plantcode' AND arrival_id='".$row_var['arrival_id']."' and arrival_date <= '$edate' and arrival_date >= '$sdate' and arrival_type='".$typ."' and arrtrflag=1 order by lotvariety") or die(mysqli_error($link));
