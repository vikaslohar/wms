...
<?php 
$arrlots="";
$sql_arrival=mysqli_query($link,"select arrival_id from tbl_dryarrival where plantcode='".$plantcode."' and   arrival_type='$tp' and arrtrflag=0") or die(mysqli_error($link));
$tot_arrival=mysqli_num_rows($sql_arrival);
if($tot_arrival)
{
	while($row_arrival=mysqli_fetch_array($sql_arrival))
	{
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_dryarrival_sub where plantcode='".$plantcode."' and   arrival_id='".$row_arrival['arrival_id']."'") or die(mysqli_error($link));
...
$sql="select * from tbllotimp where lottrtype='$tp' and lotimpflg=0 and lotcrop='Maize Seed' $sss order by lotnumber";
