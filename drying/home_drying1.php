...
mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='home_drying1.php?sdate=$sdate&edate=$edate'</script>";
		}
		else
		 {?>
		 <script>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" style="border-bottom:solid; border-bottom-color:#adad11" >
	    <tr >
	      <td width="820" height="25" class="Mainheading">&nbsp;Transaction - Cob Drying Slip </td>
	    </tr></table></td>
...
$sql_arr_home=mysqli_query($link,"select * from tbl_cobdrying  where  plantcode='".$plantcode."' and  arrflg=1") or die(mysqli_error($link));
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_cobdrying  where  plantcode='".$plantcode."' and  dryingdate between '$tdate' and '$pdate' and arrflg=1 order by arr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_cobdrying  where  plantcode='".$plantcode."' and  dryingdate between '$tdate' and '$pdate' and arrflg=1 order by arr_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_cobdrying  where  plantcode='".$plantcode."' and  dryingdate>='$tdate' and dryingdate<='$pdate' and arrflg=1 order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_cobdrying where dryingdate>='$tdate' and dryingdate<='$pdate'  and arrflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
   ...
   $variety=$vername;
	...
	{
$sql_tbl_sub=mysqli_query($link,"select distinct(lotno) from tbl_cobdryingsub where trid='".$arrival_id."'") or die(mysqli_error($link));
$subtbltot=mysqli_num_rows($sql_tbl_sub);
...
<tr >
<td valign="top" align="right"><a href="home_drying.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:Pointer;" /></a></td>
</tr>
</table>
