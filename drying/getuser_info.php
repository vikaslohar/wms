...
<title>Drying- Transaction-Existing Lot Conversion</title>
...
{
var cnt=0;
for (var i = 0; i < document.from.lotnumber.length; i++) {          
		 
		  if(document.from.dest[i].checked == true)
			{
				if(document.from.foccode.value =="")
				{
				document.from.foccode.value=document.from.lotnumber[i].value;
				//document.from.foccode1.value=document.from.dest[i].value;
				}
				else
				{
				document.from.foccode.value = document.from.foccode.value +','+document.from.lotnumber[i].value;
				//document.from.foccode1.value = document.from.foccode1.value +'\n'+document.from.dest[i].value;
				}
				cnt++;
			}
			
		}
document.from.cnt.value=cnt;
}
...
$sql_tbl_sub=mysqli_query($link,"select * from tbllotimp") or die(mysqli_error($link));
...
<td align="right"  valign="middle" class="tbltext"><input type="radio" name="loc" value="<?php echo $row['did'];?>" />&nbsp;</td>
