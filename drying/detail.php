...
<body topmargin="0" >
<?php  
$sql_item=mysqli_query($link,"select * from tblstock where plantcode='".$plantcode."' and   cropid='".$itmid."'") or die(mysqli_error($link));
$row_item=mysqli_fetch_array($sql_item);
$classid=$row_item['classification_id'];

$classqry=mysqli_query($link,"select classification_id, classification from tbl_classification where plantcode='".$plantcode."' and   classification_id='".$classid."'") or die(mysqli_error($link));
...
<table width="400" height="282" border="0" align="center" cellpadding="0" cellspacing="0" >
  ...
  <form name="from" method="post" action="<?php $_SERVER['PHP_SELF']; ?>" onSubmit="post_value();">
	 ...
	 <input name="frm_action" value="submit" type="hidden"> 
      ...
      <table  border="1" cellspacing="0" cellpadding="0" width="400" align="center" bordercolor="#adad11" style="border-collapse:collapse">
<tr class="tblsubtitle" height="25">
  <td colspan="5" align="center" class="subheading" valign="middle" style="border:thin; border-color:#adad11">&nbsp;SLOC</td>
</tr>
<tr class="tblsubtitle" height="25">
  <td colspan="5" align="center" class="smalltblheading" valign="middle" style="border:thin; border-color:#adad11">&nbsp;<?php echo $noticia_class['classification'];?>&nbsp;&nbsp;|&nbsp;&nbsp;<?php echo $row_item['stores_item'];?></td>
</tr>
...
$sql_issue=mysqli_query($link,"select distinct stlg_whid, stlg_subbinid, stlg_binid from tbl_stldg_good where plantcode='".$plantcode."' and   stlg_trclassid='".$classid."' and stlg_tritemid='".$itmid."'") or die(mysqli_error($link));
...
$sql_issue1=mysqli_query($link,"select max(stlg_id) from tbl_stldg_good where plantcode='".$plantcode."' and   stlg_subbinid='".$row_issue['stlg_subbinid']."' and stlg_binid='".$row_issue['stlg_binid']."' and stlg_whid='".$row_issue['stlg_whid']."' and stlg_tritemid='".$itmid."'") or die(mysqli_error($link));
...
<?php	 $tot1=0; $tot2=0; 
	$sql_issue=mysqli_query($link,"select distinct stld_whid, stld_subbinid, stld_binid from tbl_stldg_damage where plantcode='".$plantcode."' and   stld_trclassid='".$classid."' and stld_tritemid='".$itmid."'") or die(mysqli_error($link));
$t=mysqli_num_rows($sql_issue);
$srno=1;
$totups=0; $totqty=0; $cnt=0;
 while($row_issue=mysqli_fetch_array($sql_issue))
 ...
 $sql_issue1=mysqli_query($link,"select max(stld_id) from tbl_stldg_damage where plantcode='".$plantcode."' and   stld_subbinid='".$row_issue['stld_subbinid']."' and stld_binid='".$row_issue['stld_binid']."' and stld_whid='".$row_issue['stld_whid']."' and stld_tritemid='".$itmid."'") or die(mysqli_error($link));

$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_issuetbl=mysqli_query($link,"select * from tbl_stldg_damage where plantcode='".$plantcode."' and   stld_id='".$row_issue1[0]."' and stld_balqty > 0") or die(mysqli_error($link)); 
//echo $t=mysqli_num_rows($sql_issuetbl);
 while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
 {
	$wareh=""; $binn=""; $subbinn=""; $sups="";$sqty=0; $slocs="";
$slups=0; $slqty=0;$gd="";
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='".$plantcode."' and   whid='".$row_issuetbl['stld_whid']."' order by perticulars") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);
$wareh=$row_whouse['perticulars']."/";

$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='".$plantcode."' and   binid='".$row_issuetbl['stld_binid']."' and whid='".$row_issuetbl['stld_whid']."'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname']."/";

$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='".$plantcode."' and   sid='".$row_issuetbl['stld_subbinid']."' and binid='".$row_issuetbl['stld_binid']."' and whid='".$row_issuetbl['stld_whid']."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
//$r=mysqli_num_rows($sql_subbinn);
$subbinn=$row_subbinn['sname'];
