<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		while($row_arr=mysqli_fetch_array($sql_arr))
		{
			...
			{
				$suid=$row_arrsub['subtrid'];
				$lotno=$row_arrsub['lotno'];
				$sqlsub="update tbl_dryarrival_sub set dryflg='1' where lotno='$lotno' and batchflg=1 and dryflg=0";
...
winHandle=window.open('drying_print2.php?itmid='+itm,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
if(winHandle==null){
...
<td width="234"  align="left" valign="middle" class="tbltext" >&nbsp;<?php echo $row_tbl['crop'];?></td>
...
$sql_whouse1=mysqli_query($link,"select perticulars from tbldrywarehouse where plantcode='".$plantcode."' and   whid='".$row_tbl_subsub['nwh']."' order by perticulars") or die(mysqli_error($link));
$row_whouse1=mysqli_fetch_array($sql_whouse1);
$wareh1=$row_whouse1['perticulars']."/";

$sql_binn1=mysqli_query($link,"select binname from tbldrybin where plantcode='".$plantcode."' and   binid='".$row_tbl_subsub['nbin']."' and whid='".$row_tbl_subsub['nwh']."'") or die(mysqli_error($link));
$row_binn1=mysqli_fetch_array($sql_binn1);
$binn1=$row_binn1['binname']."/";

$sql_subbinn1=mysqli_query($link,"select sname from tbldrysubbin where plantcode='".$plantcode."' and   sid='".$row_tbl_subsub['nsubbin']."' and binid='".$row_tbl_subsub['nbin']."' and whid='".$row_tbl_subsub['nwh']."'") or die(mysqli_error($link));
