...
if($z1 == 0)
{
   $sql_main="insert into tbl_dryarrival (yearcode,arrival_type, arrival_code, arrival_date, grnno, dc_date,disp_date,  nolot, tmode, trans_name, trans_lorryrepno, trans_vehno, trans_paymode, courier_name, docket_no, pname_byhand, remarks, arr_role,plantcode) values('$yearid_id', 'Fresh Seed with PDN', '$txtid', '$tdate', '$grn', '$ddate','$hdate12', '$noofpdn', '$txt1', '$txttname', '$txtlrn', '$txtvn', '$txt13', '$txtcname', '$txtdc', '$txtpname', '$txtremarks1', '$logid','$plantcode')";

if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=mysqli_insert_id($link);

$sql_sub="insert into tbl_dryarrival_sub (arrival_id, organiser, pdnno, pdndate, spcodef, spcodem, lotcrop, lotvariety, ploc, pper, farmer, plotno, gi, harvestdate, got, qty, act, diff, qty1, act1, diff1, moisture, vchk, qc, remarks, sstage, sstatus, lotno, old, got1, sample,qcsample,gssample,prodtype, lotstate,plantcode) values('$mainid', '$organi', '$txtpdno', '$pdate', '$spcodef', '$spcodem', '$txtcrop', '$txtvariety', '$txtloc', '$txtprod', '$txtfar', '$txtplot', '$gcode', '$hdate1', '$txtgstat', '$txtdcqty', '$txtactqty', '$txtdiffqty', '$txtdcnob', '$txtactnob', '$txtdiffnob', '$txtmoist', '$txtvisualck', '$qc', '$txtremarks', '$sstage', '$sstatus', '$glotno', '$txtlot1', '$gotstatus', '$gscheckbox','$qc2','$gs1','$productiontype','$statename','$plantcode')";
...
if($god1==1)
{
	$sql_sub_sub="insert into tbl_dryarr_sloc (arr_type, arr_tr_id, arr_id, whid, binid, subbin, rowid, qty, bags, balqty, balbags, lotcrop, lotvariety,plantcode) values('Fresh Seed with PDN', '$mainid', '$subid', '$y', '$z', '$a1',  '$rowid1', '$b1', '$c1', '$b1', '$c1', '$txtcrop', '$txtvariety','$plantcode')";
	mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
}
if($god2==1)
{
	$sql_sub_sub="insert into tbl_dryarr_sloc (arr_type, arr_tr_id, arr_id, whid, binid, subbin, rowid, qty, bags, balqty, balbags, lotcrop, lotvariety,plantcode) values('Fresh Seed with PDN', '$mainid', '$subid', '$d1', '$e1', '$f1',  '$rowid1', '$g1', '$h1', '$g1', '$h1', '$txtcrop', '$txtvariety','$plantcode')";
	mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
}
...
{
$sql_main="update tbl_dryarrival set arrival_date='$tdate', grnno='$grn', dc_date='$ddate',disp_date='$hdate12',  nolot='$noofpdn', tmode='$txt1', trans_name='$txttname', trans_lorryrepno='$txtlrn', trans_vehno='$txtvn', trans_paymode='$txt13', courier_name='$txtcname', docket_no='$txtdc', pname_byhand='$txtpname', remarks='$txtremarks1', arr_role='$logid'  where arrival_id = '$z1'";
if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=$z1;

$sql_sub="update tbl_dryarrival_sub set arrival_id='$mainid', organiser='$organi', pdnno='$txtpdno', pdndate='$pdate', spcodef='$spcodef', spcodem='$spcodem', lotcrop='$txtcrop', lotvariety='$txtvariety', ploc='$txtloc', pper='$txtprod', farmer='$txtfar', plotno='$txtplot', gi='$gcode', harvestdate='$hdate1', got='$txtgstat', qty='$txtdcqty', act='$txtactqty', diff='$txtdiffqty', qty1='$txtdcnob', act1='$txtactnob', diff1='$txtdiffnob', moisture='$txtmoist', vchk='$txtvisualck', qc='$qc', remarks='$txtremarks', sstage='$sstage', sstatus='$sstatus',qcsample='$qc2' ,lotno='$glotno', got1='$gotstatus', sample='$gscheckbox',gssample='$gs1' where arrsub_id='$subtrid'";
if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
{
$subid=$subtrid;
$sql_del=mysqli_query($link,"delete from tbl_dryarr_sloc where arr_id='$subtrid'") or die(mysqli_error($link));
