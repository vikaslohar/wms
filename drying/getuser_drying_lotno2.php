...
{	
	...
	window.location='getuser_drying_lotno.php?crop='+val3+'&variety='+val4+'&sqq='+txtlot1+'&stage='+stage;
	return true;	
	}
}	
...
$arrlots="";
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_cobdryingsub where plantcode='".$plantcode."' and   trid='".$tid."' and sstage='".$stage."'") or die(mysqli_error($link));
		...
		$sql_tbl_main=mysqli_query($link,"select * from tbl_cobdrying where plantcode='".$plantcode."' and   dflg='0' and logid!='$logid'") or die(mysqli_error($link));
		$tot_arrmain=mysqli_num_rows($sql_tbl_main);
		if($tot_arrmain > 0)
		{
			while($row_tbl_main=mysqli_fetch_array($sql_tbl_main))
			{
				$sql_tbl_sub=mysqli_query($link,"select * from tbl_cobdryingsub where plantcode='".$plantcode."' and   trid='".$row_tbl_main['trid']."' and sstage='".$stage."'") or die(mysqli_error($link));
				$tot_arrsub=mysqli_num_rows($sql_tbl_sub);
				if($tot_arrsub > 0)
				{
					while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
...
$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$variety."' and actstatus='Active' order by popularname Asc"); 
$noticia_item = mysqli_fetch_array($quer4);

$aaaass="select distinct(lotno)  from tbl_dryarrival_sub where plantcode='".$plantcode."' and lotcrop='".$noticia['cropname']."' and lotvariety='".$noticia_item['popularname']."' and batchflg=0 and dryflg=0  $sss";			
