...
<title>Drying - Utility - SLOC Search</title>
...
if($sid=='ALL')
{ 
 	$sql_tb="select lotldg_subbinid, lotldg_variety, lotldg_crop,lotldg_lotno from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_whid='".$whid."' and lotldg_binid='".$bid."' group by lotldg_subbinid, lotldg_variety, lotldg_lotno order by lotldg_subbinid";  
}
 else
{
	$sql_tb="select lotldg_subbinid, lotldg_variety, lotldg_crop,lotldg_lotno from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_whid='".$whid."' and lotldg_binid='".$bid."' and lotldg_subbinid='".$sid."' group by lotldg_subbinid, lotldg_variety, lotldg_lotno order by lotldg_subbinid";  
 }
...
{
$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_whid='".$whid."' and lotldg_binid='".$bid."' and lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_variety='".$row_tbl['lotldg_variety']."' and lotldg_crop='".$row_tbl['lotldg_crop']."'  and lotldg_lotno='".$row_tbl['lotldg_lotno']."'") or die(mysqli_error($link));  
...
$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='".$plantcode."' and   sid='".$row_tbl_sub['lotldg_subbinid']."' and binid='".$bid."' and whid='".$whid."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
...
{	
$sql_arr_home=mysqli_query($link,"select distinct orlot from tbl_lot_ldg where plantcode='".$plantcode."'  and lotldg_crop='$txtcrop' and lotldg_variety='$txtvariety'and  lotldg_sstage='$txtstage' order by lotldg_lotno desc") or die(mysqli_error($link));
...
$sql_qc=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and   orlot='".$lotno."' and lotldg_sstage='".$txtstage."' and lotldg_subbinid='".$row_qc_sub['lotldg_subbinid']."' and lotldg_balqty > 0");
$tt=mysqli_num_rows($sql_qc);
while($row_qc=mysqli_fetch_array($sql_qc))
{

$sql_month=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_id='".$row_qc[0]."'")or die(mysqli_error($link));
...
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='".$plantcode."' and   whid='".$row_month['lotldg_whid']."'") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);
$wareh=$row_whouse['perticulars']."/";

$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='".$plantcode."' and   binid='".$row_month['lotldg_binid']."' and whid='".$row_month['lotldg_whid']."'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname']."/";


$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='".$plantcode."' and   sid='".$row_month['lotldg_subbinid']."' and binid='".$row_month['lotldg_binid']."' and whid='".$row_month['lotldg_whid']."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
$subbinn=$row_subbinn['sname'];
...
?>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="400" bordercolor="#adad11" style="border-collapse:collapse">
  <tr class="Dark" height="30">
...
/*while($row_tbl=mysqli_fetch_array($sql_qry))
{
$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_whid='".$whid1."' and lotldg_binid='".$bid1."'") or die(mysqli_error($link));  
$row_tbl1=mysqli_fetch_array($sql_tbl1);
