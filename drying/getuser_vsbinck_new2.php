...
$sql_tr_222=mysqli_query($link,"select * from tblarr_sloc where plantcode='".$plantcode."' and   subbin='".$a."' order by arrsloc_id desc") or die(mysqli_error($link));
...
$sql_tr=mysqli_query($link,"select * from tblarr_sloc where plantcode='".$plantcode."' and   arr_tr_id='".$trid."' and lotvariety!='".$varid11."' and subbin='".$a."'") or die(mysqli_error($link));
...
{	
	...
	$sql_t=mysqli_query($link,"select * from tblarr_sloc where plantcode='".$plantcode."' and   arr_tr_id='".$trid."' and lotvariety='".$varid11."' and subbin='".$a."'") or die(mysqli_error($link));
...
if($tpmflg==0)
{
	if(($tp==$typ || $tp=="Empty"))
	{	$cnt=0;
		$sst_ldg=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_subbinid='".$a."' and lotldg_variety!='".$b."'") or die(mysqli_error($link));
		 $ss_ldg=mysqli_num_rows($sst_ldg);
		while($row_ssldg=mysqli_fetch_array($sst_ldg))
		{
		$sql_lot=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_subbinid='".$a."' and lotldg_variety='".$row_ssldg['lotldg_variety']."'") or die(mysqli_error($link));
	while($row_lot=mysqli_fetch_array($sql_lot))
 	{ 
		$s_good=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_subbinid='".$a."' and lotldg_variety='".$row_ssldg['lotldg_variety']."'") or die(mysqli_error($link));
		 $r_good=mysqli_num_rows($s_good);
		
		while($row_issueg=mysqli_fetch_array($s_good))
 	{ //echo $b;
	$sql_issueg1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_subbinid='".$row_issueg['lotldg_subbinid']."' and lotldg_binid='".$row_issueg['lotldg_binid']."' and lotldg_whid='".$row_issueg['lotldg_whid']."' and lotldg_variety='".$row_ssldg['lotldg_variety']."' and lotldg_lotno='".$row_lot['lotldg_lotno']."'") or die(mysqli_error($link));
