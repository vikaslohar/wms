...
$sql_tbl=mysqli_query($link,"select * from tblarrival where plantcode='".$plantcode."' and   arr_role='".$logid."' and  arrival_type='Opening Stock' and arrtrflag=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tblarrival where arr_role='".$logid."' and arrival_type='Opening Stock' and arrtrflag=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" style="border-bottom:solid; border-bottom-color:#adad11" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Opening Stock</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="trn_op_add.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tblarrival where plantcode='".$plantcode."' and   arrival_date>='$tdate' and arrival_date<='$pdate' and arrival_type='Opening Stock' and arrtrflag=1 order by arrival_date desc, arr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tblarrival where plantcode='".$plantcode."' and   arrival_date>='$tdate' and arrival_date<='$pdate' and arrival_type='Opening Stock' and arrtrflag=1 order by arrival_date desc, arr_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tblarrival where plantcode='".$plantcode."' and   arrival_date>='$tdate' and arrival_date<='$pdate' and arrival_type='Opening Stock' and arrtrflag=1 order by arrival_date desc, arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tblarrival where  plantcode='".$plantcode."' and   arrival_date>='$tdate' and arrival_date<='$pdate' and arrival_type='Opening Stock' and arrtrflag=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
...
<tr class="Light">
         ...
         <td width="9%" align="center" valign="middle" class="smalltblheading"><a href="trn_arrvop_qty_viewpage.php?pid=<?php echo $row_arr_home['arrival_id'];?>"><?php echo "OP".$row_arr_home['arr_code']."/".$yearid_id."/".$lrole;?></a></td>
...
<tr >
<td valign="top" align="right"><a href="home_optrn.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:Pointer;" /></a></td>
</tr>
</table>
</td>
<td width="30"></td>
