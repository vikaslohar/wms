...
$sql_tbl_sub=mysqli_query($link,"select * from tbl_ieindent_sub where plantcode='".$plantcode."' and   eid=$rid") or die(mysqli_error($link));
$row_tbl_sub=mysqli_fetch_array($sql_tbl_sub);

$class_sql=mysqli_query($link,"select classification_id, classification from tbl_classification where plantcode='".$plantcode."' and   classification_id='".$row_tbl_sub['classification_id']."'") or die(mysqli_error($link));
$row_class = mysqli_fetch_array($class_sql);

$item_sql=mysqli_query($link,"select items_id, stores_item from tbl_stores where plantcode='".$plantcode."' and   items_id='".$row_tbl_sub['items_id']."'") or die(mysqli_error($link));
...
$sql_issue1=mysqli_query($link,"select max(stlg_id) from tbl_stldg_good where plantcode='".$plantcode."' and   stlg_subbinid='".$row_issue['stlg_subbinid']."' and stlg_binid='".$row_issue['stlg_binid']."' and stlg_whid='".$row_issue['stlg_whid']."' and stlg_tritemid='".$row_tbl_sub['items_id']."'") or die(mysqli_error($link));
...
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='".$plantcode."' and   whid='".$row_issue['stlg_whid']."' order by perticulars") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);
$wareh=$row_whouse['perticulars']."/";

$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='".$plantcode."' and   binid='".$row_issue['stlg_binid']."' and whid='".$row_issue['stlg_whid']."'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname']."/";

$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='".$plantcode."' and   sid='".$row_issue['stlg_subbinid']."' and binid='".$row_issue['stlg_binid']."' and whid='".$row_issue['stlg_whid']."'") or die(mysqli_error($link));
