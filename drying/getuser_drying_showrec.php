...
//$lotqry=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_crop ='".$b."' and lotldg_variety='".$c."' and lotldg_sstage='$stge' and lotldg_lotno='$a'")or die (mysqli_error($link));
//echo "select * from tbl_dryarrival_sub where plantcode='".$plantcode."' and   lotcrop='".$noticia['cropname']."' and lotvariety='".$noticia_item['popularname']."' and batchflg=0 and dryflg=0";
$lotqry=mysqli_query($link,"select * from tbl_dryarrival_sub where plantcode='".$plantcode."' and   lotcrop='".$noticia['cropname']."' and lotvariety='".$noticia_item['popularname']."' and batchflg=0 and dryflg=0")or die (mysqli_error($link));
...
$sql_issue=mysqli_query($link,"select * from tbl_dryarrival_sub where plantcode='".$plantcode."' and   lotcrop='".$noticia['cropname']."' and lotvariety='".$noticia_item['popularname']."' and lotno='$a' and batchflg=0 and dryflg=0") or die(mysqli_error($link));
...
?>
  <tr class="Light" height="30">
    ...
    <input name="txtdisp<?php echo $srno2?>" id="txtdisp<?php echo $srno2?>" type="hidden" size="4" class="smalltbltext" tabindex="" maxlength="5"  onkeypress="return isNumberKey(event)" value="<?php echo $slups;?>" style="background-color:#CCCCCC"  readonly="true" /></td>
    <td width="81" align="center"  valign="middle" class="smalltbltext"><?php echo $slqty;?>
    <input name="txtqty<?php echo $srno2?>" id="txtqty<?php echo $srno2?>" type="hidden" size="8" class="smalltbltext"  maxlength="9" style="background-color:#CCCCCC" value="<?php echo $slqty;?>"/></td>
	<td width="62" align="center"  valign="middle" class="smalltbltext"><input type="checkbox" name="chkbox<?php echo $srno2;?>" id="chkbox<?php echo $srno2;?>" onclick="chkboxchk(<?php echo $srno2?>,<?php echo $row_sloc['whid'];?>,<?php echo $row_sloc['binid'];?>,<?php echo $row_sloc['subbin'];?>)" value="samebin" /> <input type="hidden" name="samebin<?php echo $srno2;?>" id="samebin<?php echo $srno2;?>" value="No" /></td>
...
<?php
$bind1_query=mysqli_query($link,"select binid, binname from tbldrybin  order by binname") or die(mysqli_error($link));
?>
