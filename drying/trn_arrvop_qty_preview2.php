<?php
	...
	$sql_arr=mysqli_query($link,"select * from tbl_opspa where plantcode='".$plantcode."' and opspa_id='".$pid."'") or die(mysqli_error($link));
	...
	$sql_arrsub=mysqli_query($link,"select * from tbl_opspa_sub where plantcode='".$plantcode."' and opspa_id='".$pid."'") or die(mysqli_error($link));
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		...
		$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_variety='".$itemid."' and lotldg_crop='".$classid."' and orlot='".$orlot."'") or die(mysqli_error($link));
		...
		$sql_arrsub_sub=mysqli_query($link,"select * from tbl_opspasub_sub where plantcode='".$plantcode."' and opspa_id='".$pid."' and opspasub_id='".$row_arrsub['opspasub_id']."'") or die(mysqli_error($link));
	...
	$sql_code="SELECT MAX(opspa_code) FROM tbl_opspa where plantcode='".$plantcode."' and yearcode='$yearid_id' ORDER BY opspa_code DESC";
	...
	$sql_code1="SELECT MAX(ncode) FROM tbl_opspa where plantcode='".$plantcode."' ORDER BY ncode DESC";
