...
<body topmargin="0" >
<?php  
/*$sql1=mysqli_query($link,"select * from tblarr_sloc where plantcode='".$plantcode."' and   whid='".$wid."' and binid='".$bid."'  and arr_tr_id='".$pid."' order by subbin")or die(mysqli_error($link));
...
<td width="400" align="left"  valign="middle" class="tblheading">&nbsp;Bin Card</td>
<td width="400" align="right"  valign="middle" class="tblheading">SLOC Details:&nbsp;&nbsp;<?php echo $row_whouse['perticulars'];?>/<?php echo $row_binn['binname'];?>&nbsp;</td>
</tr>
...
<?php
$srno=1;
$sql_tbl=mysqli_query($link,"select lotldg_subbinid, lotldg_variety, lotldg_crop,lotldg_lotno from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_whid='".$wid."' and lotldg_binid='".$bid."' group by lotldg_subbinid, lotldg_variety, lotldg_lotno order by lotldg_subbinid") or die(mysqli_error($link));  
//echo $t=mysqli_num_rows($sql_tbl);
while($row_tbl=mysqli_fetch_array($sql_tbl))
{

$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_whid='".$wid."' and lotldg_binid='".$bid."' and lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_variety='".$row_tbl['lotldg_variety']."' and lotldg_crop='".$row_tbl['lotldg_crop']."'  and lotldg_lotno='".$row_tbl['lotldg_lotno']."'") or die(mysqli_error($link));  
...
<?php
$srno=1;
$sql_tbl=mysqli_query($link," select subbinid, lotldg_variety, lotldg_crop, lotno from tbl_lot_ldg_pack where plantcode='".$plantcode."' and   whid='".$wid."' and binid='".$bid."' group by subbinid, lotldg_variety, lotno order by subbinid") or die(mysqli_error($link));  
//echo $t=mysqli_num_rows($sql_tbl);
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
...
$sql_item=mysqli_query($link,"select * from tblarrival_sub where plantcode='".$plantcode."' and   arrival_id='".$row_tbl_sub['lotldg_id']."'") or die(mysqli_error($link));
$row_item=mysqli_fetch_array($sql_item);
