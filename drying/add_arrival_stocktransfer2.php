...
$sql_tbl=mysqli_query($link,"select * from tblarrival where plantcode='".$plantcode."' and   arr_role='".$logid."' and  arrival_type='StockTransfer Arrival' and arrtrflag=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='add_arrival_stocktransfer22.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='add_arrival_stocktransfer2.php'</script>";
		}
	}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" style="border-bottom:solid; border-bottom-color:#adad11" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Arrival Stock Transfer - Plant</td>
	    </tr></table></td>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tblarrival where plantcode='".$plantcode."' and   arrival_type='StockTransfer Arrival' and arrtrflag=1 order by arrival_date desc, arr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tblarrival where plantcode='".$plantcode."' and   arrival_type='StockTransfer Arrival' and arrtrflag=1 order by arrival_date desc, arr_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tblarrival where plantcode='".$plantcode."' and   arrival_type='StockTransfer Arrival' and arrtrflag=1 order by arrival_date desc, arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tblarrival where  plantcode='".$plantcode."' and   arrival_type='StockTransfer Arrival' and arrtrflag=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; 
*/
    if($tot_arr_home >0) { 
...
<tr class="Light">
         ...
         <td width="10%" align="center" valign="middle" class="smalltblheading"><a href="add_arrival_stock_viewpage.php?cropid=<?php echo $row_arr_home['arrival_id'];?>"><?php echo "AS".$row_arr_home['arr_code']."/".$yearid_id."/".$lrole;?></a></td>
