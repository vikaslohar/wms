...
$sql_tbl_sub=mysqli_query($link,"select * from tbl_stlotimp where plantcode='".$plantcode."' and stlotimp_impflg=0 and stlotimp_trflg=0 $sss order by stlotimp_lotno") or die(mysqli_error($link));
...
{
$ct=0;
$sql_arrival=mysqli_query($link,"select arrival_id from tblarrival where plantcode='".$plantcode."' and   plantcode='".$plantcode."' and arrival_type='$tp' and arrtrflag=0") or die(mysqli_error($link));
$tot_arrival=mysqli_num_rows($sql_arrival);
if($tot_arrival)
{
	while($row_arrival=mysqli_fetch_array($sql_arrival))
	{
		$sql_tbl_sub1=mysqli_query($link,"select * from tblarrival_sub where plantcode='".$plantcode."' and   arrival_id='".$row_arrival['arrival_id']."'") or die(mysqli_error($link));
...
if($ct==0)
{
	...
	$dq=explode(".",$row['stlotimp_qty']);
if($dq[1]==000){$dcq=$dq[0];}else{$dcq=$row['stlotimp_qty'];}
//echo $row['stlotimp_id'];
if($srno%2!=0)
{
?>
<tr class="Dark" height="25">
<td align="right"  valign="middle" class="tbltext"><input type="radio" name="loc" value="<?php echo $row['stlotimp_lotno'];?>" onclick="clk(this.value,'<?php echo $row['stlotimp_id'];?>');" />&nbsp;</td>
<td align="left"  valign="middle" class="tbltext" >&nbsp;<?php echo $row['stlotimp_lotno'];?></td>
