...
$sql_tbl_sub=mysqli_query($link,"select * from tbl_cobdryingsub where plantcode='".$plantcode."' and   subtrid='".$a."'") or die(mysqli_error($link));
...
$quer3=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropname='".$row['lotcrop']."' order by cropname Asc");
$noticia = mysqli_fetch_array($quer3);

$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where popularname='".$row['lotvariety']."' and actstatus='Active' order by popularname Asc"); 
...
$sql_issue=mysqli_query($link,"select * from tbl_dryarrival_sub where plantcode='".$plantcode."' and   lotno='".$row['lotno']."' and batchflg=0 and dryflg=0") or die(mysqli_error($link));

while($row_issue=mysqli_fetch_array($sql_issue))
{ 
$dq=explode(".",$row_issue['qty']);
if($dq[1]==000){$dcq=$dq[0];}else{$dcq=$row_issue['qty'];}

$dn=explode(".",$row_issue['qty1']);
if($dn[1]==000){$dcn=$dn[0];}else{$dcn=$row_issue['qty1'];}

$aq=explode(".",$row_issue['act']);
if($aq[1]==000){$ac=$aq[0];}else{$ac=$row_issue['act'];}

$an=explode(".",$row_issue['act1']);
if($an[1]==000){$acn=$an[0];}else{$acn=$row_issue['act1'];}

$diq=explode(".",$row_issue['diff']);
if($diq[1]==000){$difq=$diq[0];}else{$difq=$row_issue['diff'];}

$din=explode(".",$row_issue['diff1']);
if($din[1]==000){$difn=$din[0];}else{$difn=$row_issue['diff1'];}



$sql_sloc=mysqli_query($link,"select * from tbl_dryarr_sloc where plantcode='".$plantcode."' and   arr_tr_id='".$row_issue['arrival_id']."' and arr_id='".$row_issue['arrsub_id']."' order by arrsloc_id") or die(mysqli_error($link));
while($row_sloc=mysqli_fetch_array($sql_sloc))
{$slups=0; $slqty=0;$wareh=""; $binn=""; $subbinn=""; $sups="";$sqty=0; $slocs=""; $gd="";
...
$slups=$slups+$row_sloc['bags']+$row_sloc['ups_damage'];
if($sups!="")
$sups=$sups.$slups."<br/>";
else
$sups=$slups."<br/>";
...
$sql_tbl_subsub=mysqli_query($link,"select * from tbl_cobdryingsubsub where plantcode='".$plantcode."' and   subtrid='".$a."' and owh='".$row_sloc['whid']."' and obin='".$row_sloc['binid']."' and osubbin='".$row_sloc['subbin']."'") or die(mysqli_error($link));
...
?>
  <tr class="Light" height="30">
    ...
    <td align="center"  valign="middle" class="smalltbltext" ><?php echo $slocs;?><!--<input name="sloc<?php echo $srno2?>" type="text" size="10" class="smalltbltext"  maxlength="12" style="background-color:#CCCCCC" value=""/>--><input type="hidden" name="extslwhg<?php echo $srno2?>" value="<?php echo $row_sloc['whid'];?>" /><input type="hidden" name="extslbing<?php echo $srno2?>" value="<?php echo $row_sloc['binid'];?>" /><input type="hidden" name="extslsubbg<?php echo $srno2?>" value="<?php echo $row_sloc['subbin'];?>" /></td>

    <td width="69"  align="center" valign="middle" class="smallsmalltbltext"><?php echo $slups;?>
    ...
    <td width="81" align="center"  valign="middle" class="smalltbltext"><?php echo $slqty;?>
	...
	<td width="62" align="center"  valign="middle" class="smalltbltext"><input type="checkbox" name="chkbox<?php echo $srno2;?>" id="chkbox<?php echo $srno2;?>" <?php if($rowsubsub['samebin']=="Yes") echo "checked"; ?> onclick="chkboxchk(<?php echo $srno2?>,<?php echo $row_sloc['whid'];?>,<?php echo $row_sloc['binid'];?>,<?php echo $row_sloc['subbin'];?>)" value="samebin" /> <input type="hidden" name="samebin<?php echo $srno2;?>" id="samebin<?php echo $srno2;?>" value="<?php echo $rowsubsub['samebin'];?>" /></td>
...
<?php
$bind1_query=mysqli_query($link,"select binid, binname from tbldrybin where plantcode='".$plantcode."' and   whid='".$rowsubsub['nwh']."' order by binname") or die(mysqli_error($link));
?>
...
<tr>	
<?php
$sql_whouse1=mysqli_query($link,"select perticulars from tbldrywarehouse where plantcode='".$plantcode."' and   whid='".$rowsubsub['nwh']."' order by perticulars") or die(mysqli_error($link));
$row_whouse1=mysqli_fetch_array($sql_whouse1);
$wareh1=$row_whouse1['perticulars'];

$sql_binn1=mysqli_query($link,"select binname from tbldrybin where plantcode='".$plantcode."' and   binid='".$rowsubsub['nbin']."' and whid='".$rowsubsub['nwh']."'") or die(mysqli_error($link));
$row_binn1=mysqli_fetch_array($sql_binn1);
$binn1=$row_binn1['binname'];

$sql_subbinn1=mysqli_query($link,"select sname from tbldrysubbin where plantcode='".$plantcode."' and   sid='".$rowsubsub['nsubbin']."' and binid='".$rowsubsub['nbin']."' and whid='".$rowsubsub['nwh']."'") or die(mysqli_error($link));
