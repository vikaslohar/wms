...
if($z1 == 0)
{
 $sql_main="insert into tblarrival (nolot,vvariety,sstage, lotcrop, lotvariety,yearcode,arrival_type, arrival_code, arrival_date,grnno,dc_date,dcno,orderno,party_id, tmode, trans_name, trans_lorryrepno, trans_vehno, trans_paymode, courier_name, docket_no, pname_byhand, remarks, arr_role,plantcode) values('$lot2','$vv','$st','$crop','$variety','$yearid_id','Trading','$c','$tdate','$g','$tdate1','$d','$or','$party','$a','$i','$j','$k','$b','$l','$m','$pname','$y1','$logid','$plantcode')";

if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
{
$mainid=mysqli_insert_id($link);
 $sql_sub="insert into tblarrival_sub (lotcrop, lotvariety,old,sample,sstatus,arrival_id,lotno, qty, act, diff, qty1, act1, diff1,moisture ,vchk,qc,glotno,lotoldlot,got,got1,orlot,remarks,sstage,gssample,plantcode) values('$crop','$variety','$f','$sample','$status','$mainid','$n','$txtdcqty', '$txtactqty', '$txtdiffqty', '$txtdcnob', '$txtactnob', '$txtdiffnob','$w','$x','$qc','$lotno','$oldlot','$got','$gotstatus','$orlot','$remarks','$st','$gs1','$plantcode')";
...
if($god1==1)
{
$sql_sub_sub="insert into tblarr_sloc (arr_type, arr_tr_id, arr_id, whid, binid, subbin, rowid, qty, bags, balqty, balbags, lotcrop, lotvariety,plantcode) values('Trading', '$mainid', '$subid', '$y', '$z', '$a1',  '$rowid1', '$b1', '$c1', '$balqty1', '$balbags1', '$crop', '$variety','$plantcode')";
mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
}
if($god2==1)
{
$sql_sub_sub="insert into tblarr_sloc (arr_type, arr_tr_id, arr_id, whid, binid, subbin, rowid, qty, bags, balqty, balbags, lotcrop, lotvariety,plantcode) values('Trading', '$mainid', '$subid', '$d1', '$e1', '$f1','$rowid1', '$g1', '$h1', '$balqty2', '$balbags2', '$crop', '$variety','$plantcode')";
mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link));
}
...
{
$mainid=$z1;
  $sql_sub="update tblarrival_sub set sample='$sample',sstatus='$status',arrival_id='$mainid',lotno='$n',qty='$txtdcqty', act='$txtactqty', diff='$txtdiffqty', qty1='$txtdcnob', act1='$txtactnob', diff1='$txtdiffnob', moisture='$w',vchk='$x',qc='$qc',glotno='$lotno',lotoldlot='$oldlot',got='$got',got1='$gotstatus',lotcrop='$crop',lotvariety='$variety',remarks='$remarks1',sstage='$st' ,gssample='$gs1' where arrsub_id='$subtrid'";
if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
{
 $subid=$subtrid;
