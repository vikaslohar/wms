...
$sql_month=mysqli_query($link,"select * from tblarrival_sub where plantcode='".$plantcode."' and   lotcrop='".$crop."' and spcodef='".$b."' and spcodem='".$c."' order by lotvariety")or die(mysqli_error($link));
...
$quer22=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where popularname='".$variety."' and actstatus='Active' and vertype='PV' order by cropname Asc"); 
$noticia22 = mysqli_fetch_array($quer22);
$variet=$noticia22['varietyid'];

if($tt > 0)
{
/*$sql_tbll=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_trid='".$arid."'") or die(mysqli_error($link));
$row_tbll=mysqli_fetch_array($sql_tbll);
$whid=$row_tbll['lotldg_whid'];
$bid=$row_tbll['lotldg_binid'];
$sid=$row_tbll['lotldg_subbinid'];*/

 $sql_tb="select * from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_variety='".$variet."' group by lotldg_subbinid, lotldg_lotno, lotldg_crop order by lotldg_subbinid";  
?>
...
{
$sql_tbl1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_whid='".$row_tbl['lotldg_whid']."' and lotldg_binid='".$row_tbl['lotldg_binid']."' and lotldg_subbinid='".$row_tbl['lotldg_subbinid']."' and lotldg_variety='".$variet."' and lotldg_lotno='".$row_tbl['lotldg_lotno']."'") or die(mysqli_error($link));  
...
$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='".$plantcode."' and   sid='".$row_tbl_sub['stlg_subbinid']."' and binid='".$bid."' and whid='".$whid."'") or die(mysqli_error($link));
