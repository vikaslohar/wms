...
$sql_tbl_sub=mysqli_query($link,"select * from tbl_opspa_sub where plantcode='".$plantcode."' and opspasub_id='".$a."'") or die(mysqli_error($link));
$row_tbl_sub=mysqli_fetch_array($sql_tbl_sub);
$tot_tbl_sub=mysqli_num_rows($sql_tbl_sub);
$tid=$row_tbl_sub['opspa_id'];
$subtid=$a;

$sql_tbl=mysqli_query($link,"select * from tbl_opspa where plantcode='".$plantcode."' and opspa_id='".$tid."'") or die(mysqli_error($link));
...
<table align="center" border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#adad11" style="border-collapse:collapse" > 
<tr class="Light" height="30" >
...
<table align="center" border="1" width="950" cellspacing="0" cellpadding="0" bordercolor="#adad11" style="border-collapse:collapse" >
  ...
  <?php
$sql_sub_sloc=mysqli_query($link,"select * from tbl_opspasub_sub where plantcode='".$plantcode."' and opspa_id='".$arrival_id."' and opspasub_id='".$a."' order by opspasubsub_id") or die(mysqli_error($link));
...
$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='".$plantcode."' and   sid='".$row_sub_sloc['subbinid']."' and binid='".$row_sub_sloc['binid']."' and whid='".$row_sub_sloc['whid']."'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
$subbinn=$row_subbinn['sname'];

$s_good_new=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_subbinid='".$row_sub_sloc['subbinid']."'") or die(mysqli_error($link));
...
$row_issueg_new=mysqli_fetch_array($s_good_new);
	...
	$sql_issuetblg_new=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and   lotldg_id='".$row_issueg1_new[0]."'") or die(mysqli_error($link)); 
...
?>
  <tr class="Light" height="30" >
    ...
    <?php
$bing1_query=mysqli_query($link,"select binid, binname from tbl_bin where plantcode='".$plantcode."' and   whid='".$row_sub_sloc['whid']."' order by binname") or die(mysqli_error($link));
?>
    ...
    <?php
$subbing1_query=mysqli_query($link,"select sid, sname from tbl_subbin where plantcode='".$plantcode."' and   whid='".$row_sub_sloc['whid']."' and binid='".$row_sub_sloc['binid']."' order by sname") or die(mysqli_error($link));
?>
