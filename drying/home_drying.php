...
$sql_tbl=mysqli_query($link,"select * from tbl_cobdrying where plantcode='".$plantcode."' and   logid='$logid' and   arrflg=0") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	 $arrival_id=$row_tbl['trid'];	
	
	$s_sub="delete from tbl_cobdryingsub where trid='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_cobdryingsubsub where trid='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
}

	$s_sub="delete from tbl_cobdrying where logid='$logid' and  arrflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
{
winHandle=window.open('drying_export.php?itmid='+tid,'WelCome','top=170,left=180,width=200,height=200,scrollbars=yes');
if(winHandle==null){
alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); }
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#adad11" style="border-bottom:solid; border-bottom-color:#adad11" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Cob Drying Slip </td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_drying.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
...
$sql_arr_home=mysqli_query($link,"select * from tbl_cobdrying where plantcode='".$plantcode."' and   stage='RSW' and arrflg=1") or die(mysqli_error($link));
...
<?php
	$sql_arr_home=mysqli_query($link,"select * from tbl_cobdrying where plantcode='".$plantcode."' and   arrflg=2  order by ar_code desc") or die(mysqli_error($link));
	if($tot_arr_home=mysqli_num_rows($sql_arr_home)>0)
?>
<table align="center" border="0" width="600" cellspacing="0" cellpadding="0" bordercolor="#adad11" style="border-collapse:collapse" > 
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Batch Cob Drying Pending List </td>
</tr>
</table>
...
<tr class="tblsubtitle" height="20">
	...
	<td width="9%" align="center" valign="middle" class="tblheading">TRID</td>
	...
	<td width="18%" align="center" valign="middle" class="tblheading"> No. of Lots</td>
	<td width="16%" align="center" valign="middle" class="tblheading">Update</td>
</tr>
<?php
$srno=1;
$crop=""; $variety=""; 
...
{
	$trdate=$row_arr_home['dryingdate'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;	
	if($tryear=="")$tryear=$year2;
	$sql_year=mysqli_query($link,"select * from tblyears where years='".$tryear."'") or die(mysqli_error($link));
	$row_year=mysqli_fetch_array($sql_year);
	
	$lot=$row_arr_home['lotno'];	
	$lrole=$row_arr_home['arr_role'];
	$arrival_id=$row_arr_home['trid'];
	
	$sql_crop=mysqli_query($link,"select * from tblcrop where cropname='".$row_arr_home['crop']."'") or die(mysqli_error($link));
	$row31=mysqli_fetch_array($sql_crop);
		
	$sql_variety=mysqli_query($link,"select * from tblvariety where popularname='".$row_arr_home['variety']."' ") or die(mysqli_error($link));
	$rowvv=mysqli_fetch_array($sql_variety);
	if($totver=mysqli_num_rows($sql_variety) > 0)
	{
		$vername=$rowvv['popularname'];
	}
	else
	{
		$vername=$row_arr_home['variety'];
	}	
...
<tr class="Light">
	...
	<td width="9%" align="center" valign="middle" class="tblheading"><?php echo "PD".$row_arr_home['arr_code']."/".$row_year['ycode']."/".$row_arr_home['logid'];?></td>
	<td width="15%" align="center" valign="middle" class="tblheading"><?php echo $crop?></td>
	<td width="16%" align="center" valign="middle" class="tblheading"><?php echo $variety?></td>
	<td width="18%" align="center" valign="middle" class="tblheading"><?php echo $subtbltot?></td>
	<td width="16%" align="center" valign="middle" class="tblheading"><a href="edit_batchdrying.php?p_id=<?php echo $row_arr_home['trid']?>" >Update</a></td>
</tr>
<?php
}
else
...
{
?>
<tr  height="25"><td colspan="7" align="center" class="subheading">No Batch found for Cob Drying Updation</td></tr>
<?php
}
?>
</table><br />
...
<?php
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_cobdrying where plantcode='".$plantcode."' and   arrflg=1  order by ar_code desc LIMIT $start, $limit") or die(mysqli_error($link));
  $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_cobdrying where plantcode='".$plantcode."' and   arrflg=1  order by ar_code desc";
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Cob Drying List </td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="850" bordercolor="#adad11" style="border-collapse:collapse">
...
<tr class="tblsubtitle" height="20">
	...
	<td width="16%" align="center" valign="middle" class="tblheading">Variety</td>
	<td width="18%" align="center" valign="middle" class="tblheading">No. of Lots</td>
	<td width="18%" align="center" valign="middle" class="tblheading">Export Lot(s) for Arrival</td>
</tr>
<?php
//$srno=1;
$crop=""; $variety=""; 
...
<tr class="Light">
	<td width="6%" align="center" valign="middle" class="tblheading"><?php echo $srno;?></td>
	<td width="8%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
	<td width="9%" align="center" valign="middle" class="tblheading"><a href="add_drying_view.php?p_id=<?php echo $row_arr_home['trid']?>"><?php echo "PD".$row_arr_home['ar_code']."/".$row_year['ycode']."/".$row_arr_home['logid'];?></a></td>
	<td width="15%" align="center" valign="middle" class="tblheading"><?php echo $crop?></td>
	<td width="16%" align="center" valign="middle" class="tblheading"><?php echo $variety?></td>
	<td width="18%" align="center" valign="middle" class="tblheading"><?php echo $subtbltot?></td>
	<td width="18%" align="center" valign="middle" class="tblheading"><?php if($row_arr_home['exportflg']==0){?><a href="Javascript:void(0);" onclick="openexport(<?php echo $row_arr_home['trid']?>)">Export</a><?php } else{?>Exported<?php }?></td>
</tr>
<?php
}
else
...
{
?>
  <tr  height="25"><td colspan="7" align="center" class="subheading">No Records found for selected Period</td></tr>
