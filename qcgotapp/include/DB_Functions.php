...
//	private $conn_vnr;
	...
	public function GetSampleInfoack($sampleno) {
        ...
        if ($stmt->num_rows > 0) {
			...
			$flg=0;
			$stmtgsampack = $this->conn_ps->prepare("SELECT gotm_sampleno FROM tbl_qcgotmain WHERE gotm_ackflg=1 and gotm_sampleno = ? ");
				...
				while($stmt->fetch())
				{
					...
					if($smpdate!='')
					{
						...
						if ($stmtgsampack->num_rows == 0) 
						{
							...
							$stmt_gotsampack = $this->conn_ps->prepare("Insert into tbl_qcgotmain (gotm_tdate, gotm_dosd, gotm_crop, gotm_variety, gotm_lotno, gotm_sampleno, gotm_noofsamples, gotm_dispstate, gotm_disploc, gotm_ackremark, gotm_ackdate, gotm_ackflg, gotm_gottid, gotm_acklogid) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
							$stmt_gotsampack->bind_param("sssssssssssiis", $dt, $gottest_dosdate, $gottest_crop, $gottest_variety, $gottest_lotno, $sampleno, $gottest_totnosamp, $gottest_smpdispstate, $gottest_smpdisploc, $ackremark, $dt, $one, $gottest_tid, $scode);
