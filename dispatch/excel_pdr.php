...
$croparr="";
	...
	$dh="Periodical_Dispatch_Report_Period_From_".$sdate."_To_".$edate;
	$datahead = array("Periodical Dispatch Report");
	$filename=$dh.".xls";  
				...
				while($rowdispm2=mysqli_fetch_array($sql_istbl2))
				{
					...
					while($row_dsub = mysqli_fetch_array($quer_dsub))
					{
						...
						{ 
							...
							$dcno=$row_param['code']."/"."TD"."/".$row_code1['dnote_yearcode']."/".$row_code1['dnote_code'];
							...
							if($tt==0)
							{
								$sql_month=mysqli_query($link,"select * from tbl_orderm where order_trtype='Order TDF' and orderm_partyselect!='selectp' and orderm_partyname='".$rowdispm2['dtdf_party']."' and orderm_holdflag=0 and orderm_dispatchflag!=1 and orderm_cancelflag!=1")or die("Error:".mysqli_error($link));
								$row_month=mysqli_fetch_array($sql_month);
								$bname=$row_month['orderm_partyname'];
								$location=$row_month['orderm_partycity'];
