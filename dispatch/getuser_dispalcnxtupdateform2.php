...
for($j=1; $j<$sn; $j++)
{
	...
	$sq=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$tid' and disps_variety='$evariety' and disps_ups='$eups' and disps_upstype='$eupstyp'") or die(mysqli_error($link));
	if($to=mysqli_num_rows($sq) > 0)
	{
		$ro=mysqli_fetch_array($sq);
		$sbid=$ro['disps_id'];	
		$sqlsubsub="update tbl_disp_sub set disps_nvariety='$etxtpvariety' where plantcode='".$plantcode."' and  disps_id='$sbid'";
...
$sql_arrsub=mysqli_query($link,"select * from tbl_dispsub_sub where plantcode='".$plantcode."' and  disp_id='".$tid."' and disps_id='".$subtrid."'") or die(mysqli_error($link));
$a_arrsub=mysqli_num_rows($sql_arrsub);
if($a_arrsub==0)
{
	$s_sub="delete from tbl_disp_sub where plantcode='".$plantcode."' and  disps_id='".$subtrid."'";
