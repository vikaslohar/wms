...
$sql_tbl=mysqli_query($link,"select * from tbl_discard where plantcode='".$plantcode."' and ddrole='".$logid."' and ddflg!=1") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['tid'];	
	
	$s_sub="delete from tbl_discard_sub where did_s='".$arrival_id."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_discard_sloc where discard_trid='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
}

	$s_sub="delete from tbl_discard where ddrole='".$logid."' and ddflg!=1";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
<title>Dispatch -  Transaction - Discard Home</title>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_discard where plantcode='".$plantcode."' and  ddflg=1 order by tid desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_discard where  plantcode='".$plantcode."' and ddflg=1 order by tid desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_discard where  ddflg=1 order by tid desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_discard where  ddflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
