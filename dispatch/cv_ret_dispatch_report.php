...
<?php
$crpname='';$crpid='';
$quer3=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropname='Paddy Seed' OR cropname='Maize Seed' order by cropname Asc"); 
while($noticia = mysqli_fetch_array($quer3))
{
if($crpname!="")$crpname=$crpname.",".$noticia['cropname']; else $crpname=$noticia['cropname'];
if($crpid!="")$crpid=$crpid.",".$noticia['cropid']; else $crpid=$noticia['cropid'];
}
?>            
<td align="right"  valign="middle" class="smalltblheading">Crop&nbsp;</td>
<td align="left"  valign="middle" class="smalltbltext">&nbsp;<input type="text" class="smalltbltext" name="txtcropname" style="width:150px; background-color:#CCCCCC" readonly="true" value="<?php echo $crpname;?>"  /><input type="hidden" name="txtcrop" value="<?php echo $crpid;?>" /></td>
<?php
