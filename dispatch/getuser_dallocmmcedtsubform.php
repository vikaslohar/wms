...
<tr class="Dark" height="30">
	...
	<td width="275" align="center"  valign="middle" class="tbltext"><?php if($tor>0 ){ ?><img border="0" src="../images/edit.png"  style="display:inline;cursor:pointer;" onclick="editrecmmc('<?php echo $sid;?>','<?php echo $tid?>','<?php echo $sn;?>')" /><input type="hidden" name="selsh<?php echo $sn;?>" id="selsh_<?php echo $sn;?>" onclick="selitm('<?php echo $sn;?>','<?php echo $vt?>','<?php echo $up1?>','<?php echo $mmcqty?>','<?php echo $ordno ?>','<?php echo $upstyp?>','<?php echo $sid?>','<?php echo $tid?>')" value="<?php echo $sn;?>"  /><?php } else { ?><input type="hidden" name="selsh<?php echo $sn;?>" id="selsh_<?php echo $sn;?>" onclick="selitm('<?php echo $sn;?>','<?php echo $vt?>','<?php echo $up1?>','<?php echo $mmcqty?>','<?php echo $ordno ?>','<?php echo $upstyp?>','<?php echo $sid?>','<?php echo $tid?>')" value="<?php echo $sn;?>"  /><?php } ?></td>
</tr>
<?php
$sn++;
//}
...
$y=mysqli_num_rows($sqq_sub);
while($roo_sub=mysqli_fetch_array($sqq_sub))
{
$subtrid=$roo_sub['dallocs_id'];

$sqq=mysqli_query($link,"Select * from tbl_dallocsub_sub where plantcode='".$plantcode."' and  dalloc_id='$tid' and dallocs_id='$subtrid' and dallocss_altype='lotwise'") or die(mysqli_error($link));
...
$flg=0; $lotno=""; $nob=0; $qty=0; $totqty=0; $totnob=0; $crop=""; $variety=""; $qc=""; $dot=""; $sloc=""; $qcstsap='';
	...
	while($rowmonth=mysqli_fetch_array($sqlmonth))
	{
		...
		while($rowmonth3=mysqli_fetch_array($sqlmonth3))
		{
			...
			if($days<30)$flg++;
			//echo $lotn."  =  ".$roo['dallocss_lotno']."  -  ".$dot."  -  ".$dov."  -  ".$flg."<br>";
			$vflg=0;
...
if($totmmc1=mysqli_num_rows($sqmmc1)>0)
{$mq=0;	 $bnlp=0;
while($rowmmc1=mysqli_fetch_array($sqmmc1))
{
	$mq=$mq+$rowmmc1['dmmc_eqty'];
	$bnlp=$bnlp+$rowmmc1['dmmc_enolp'];
}
$mmqty=$mq;
}
//echo $mq."  -  ".$mmqty;
...
<tr class="Dark" height="30">
	...
	<td align="center"  valign="middle" class="smalltbltext"><?php if($mq>0) { ?><img border="0" src="../images/edit.png"  style="display:inline;cursor:pointer;" onclick="editrecsubmmc('<?php echo $sno1;?>','<?php echo $lotno;?>','<?php echo $subtid;?>','<?php echo $tid?>','<?php echo $lotups;?>','<?php echo $variety?>','<?php echo $bnlp?>','<?php echo $mmqty?>','<?php echo $roo['dallocss_id']?>')" /><input type="hidden" name="selmmc" id="selmmc_<?php echo $sno1;?>" value="<?php echo $sno1;?>"  onclick="selctmmc(this.value,'<?php echo $lotno;?>','<?php echo $subtid;?>','<?php echo $tid?>','<?php echo $lotups;?>','<?php echo $variety?>','<?php echo $roo['dallocss_nolp']?>','<?php echo $mmqty?>','<?php echo $roo['dallocss_id']?>')" /><?php } else {?><input type="checkbox" name="selmmc" id="selmmc_<?php echo $sno1;?>" value="<?php echo $sno1;?>"  onclick="selctmmc(this.value,'<?php echo $lotno;?>','<?php echo $subtid;?>','<?php echo $tid?>','<?php echo $lotups;?>','<?php echo $variety?>','<?php echo $roo['dallocss_nolp']?>','<?php echo $mmqty?>','<?php echo $roo['dallocss_id']?>')" /><?php } ?><input type="hidden" name="ssids" id="ssids_<?php echo $sno1;?>" value="<?php echo $roo['dallocss_id']?>" /></td>
