...
if($logid=="" && $lgnid!="") $logid=$lgnid;
if($logid=="" && $lgnid=="") $logid='DP1';
if($yearid_id=="")
{
	$sq_yr=mysqli_query($link,"Select * from tbl_lgenyear plantcode='".$plantcode."' and  order by lgenyearid desc") or Die(mysqli_error($link));
...
//if($brflg==0)		
{
	...
	if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
	{
		...
		if($mchksel!="")
		{
			...
			if($subtrid==0)
			{
			 	$sql_subsub="insert into tbl_disp_sub (disp_id, disps_crop, disps_variety, disps_noorders, disps_ordno, disps_upstype, disps_ups, disps_oqty, disps_onop, disps_qty, disps_bqty, disps_onomp, disps_tnomp, disps_nomp, disps_bnomp, disps_nvariety,plantcode) values ('$mainid', '$ecrop', '$evariety', '$enoordno', '$eordno', '$eupstyp', '$eups', '$eqty', '$enop', '$rqty', '$bnop', '$txtornomp', '$txtnomp', '$txtlonomp', '$txtorblnomp','$txtpvariety','$plantcode')";
	...
	else
	{
		...
		if($mchksel!="")
		{
			...
			if(mysqli_query($link,$sql_subsub) or die(mysqli_error($link)))
			{
				...
				if($barcode!="" && $brflg==0)
				{
					$sqlbar1=mysqli_query($link,"Select * from tbl_mpmain where mpmain_barcode='".$barcode."' and mpmain_dflg=0 and mpmain_upflg=0 and mpmain_rvflg=0") or die(mysqli_error($link));
					if($totbar1=mysqli_num_rows($sqlbar1) > 0)
					{
						$rowbar1=mysqli_fetch_array($sqlbar1);
