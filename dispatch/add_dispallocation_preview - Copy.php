<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		$sql_arr=mysqli_query($link,"select * from tbl_dalloc where plantcode='".$plantcode."' and  dalloc_id='".$pid."'") or die(mysqli_error($link));
		while($row_arr=mysqli_fetch_array($sql_arr))
		{
			$ptype=$row_arr['dalloc_partytype'];
			$sql_arrsub=mysqli_query($link,"select * from tbl_dalloc_sub where plantcode='".$plantcode."' and  dalloc_id='".$pid."'") or die(mysqli_error($link));
			$a_arrsub=mysqli_num_rows($sql_arrsub);
			while($row_arrsub=mysqli_fetch_array($sql_arrsub))
			{
				$arrival_date=$row_arr['dalloc_date'];
				$cro=$row_arrsub['dallocs_crop'];
				$variet=$row_arrsub['dallocs_variety'];
				$ordernos=$row_arrsub['dallocs_ordno'];
				$subid=$row_arrsub['dallocs_id'];
				$tqt=$row_arrsub['dallocs_qty'];
				$tupsstype=$row_arrsub['dallocs_upstype'];
				if($tupsstype=="NST")
				{
					$zz=explode(" ",$row_arrsub['dallocs_ups']);
					$dq=explode(".",$zz[0]);
					$xfd=count($dq);
					if($upstyp=="NST")
					{
				...
				{
					$tupss=$row_arrsub['dallocs_ups'];
				}
				if($row_arrsub['dallocs_qty'] > 0)
				{
					$xc2=explode(" ",$row_arrsub['dallocs_ups']);
					if($xc2[1]=="Gms")
					{
						$ptp2=1000/$xc2[0];
					}
					...
					$sql_arrsub2=mysqli_query($link,"select * from tbl_dallocsub_sub where plantcode='".$plantcode."' and  dalloc_id='".$pid."' and dallocs_id='".$subid."'") or die(mysqli_error($link));
					$a_arrsub2=mysqli_num_rows($sql_arrsub2);
					while($row_arrsub2=mysqli_fetch_array($sql_arrsub2))
					{
						$lotno=$row_arrsub2['dallocss_lotno'];
						$ssid=$row_arrsub2['dallocss_id'];
						...
						$sql_main23="update tbl_barcodes set bar_dallocflg=1 where bar_barcode='".$row_arrsub2['dpss_barcode']."'";
						...
						{ 
							...
							{		
								if($tqt>0)
								{
									...
									$sql_in1="update tbl_orderm set orderm_dallocatchflag=2 where orderm_id='$orid'";	
					...
					foreach($tid2400 as $tid)
					{
						if($tid25<>"")
						{
							...
							if($t==$totordersub)
							{
								$sql_in1234="update tbl_orderm set orderm_dallocatchflag=1 where orderm_id='$tid25'";	
								$aaaaaaa=mysqli_query($link,$sql_in1234)or die(mysqli_error($link));	
							}
						}
					}
					
					$sql_main2="update tbl_dalloc_sub set dallocs_flg=1 where dallocs_id='$subid' and dalloc_id='$pid'";
					$a1234562=mysqli_query($link,$sql_main2) or die(mysqli_error($link));	
				}
				else
				{
					$sql_main2="delete from tbl_dalloc_sub where dallocs_id='".$row_arrsub['dallocs_id']."' and dalloc_id='$pid'";
					$a1234562=mysqli_query($link,$sql_main2) or die(mysqli_error($link));
				}	
			}
		}
		
		$sql_code1="SELECT MAX(dalloc_code) FROM tbl_dalloc where plantcode='".$plantcode."' ORDER BY dalloc_code DESC";
		...
		$sql_main="update tbl_dalloc set dalloc_tflg=1, dalloc_code=$ncode1 where dalloc_id='$pid'";
		$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));	
		
		//exit; 		
		echo "<script>window.location='select_dallocpack_op.php?p_id=$pid'</script>";	
	}
...
<title>Dispatch - Transaction - Dispatch Allocation - Preview</title>
...
winHandle=window.open('dallocpack_print.php?&pid='+pid,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
if(winHandle==null){
...
{ 	
	...
	if(sbmval==0)
	{
		...
		{
			document.frmaddDepartment.sbmval.value=sbmval;
			return true;	 
		}
		else
		{
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" style="border-bottom:solid; border-bottom-color:#378b8b" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Dispatch Allocation - Preview</td>
	    </tr></table></td>
...
<tr class="Dark" height="30">
	<td width="27"  align="center"  valign="middle" class="smalltblheading">#</td>
	<td width="104" align="center"  valign="middle" class="smalltblheading">Crop</td>
	<td width="143" align="center"  valign="middle" class="smalltblheading">Variety</td>
	<td width="45" align="center"  valign="middle" class="smalltblheading">UPS Type</td>
	<td width="85" align="center"  valign="middle" class="smalltblheading">UPS</td>
	<td width="50" align="center"  valign="middle" class="smalltblheading">NoP</td>
	<td width="60" align="center"  valign="middle" class="smalltblheading">Qty</td>
	<td width="135" align="center"  valign="middle" class="smalltblheading">Order No</td>
	<td width="50" align="center"  valign="middle" class="smalltblheading">NoMP</td>
	<td width="53" align="center"  valign="middle" class="smalltblheading">Barcodes</td>
	<td width="60" align="center"  valign="middle" class="smalltblheading">Qty Allocated</td>
	<td width="60" align="center"  valign="middle" class="smalltblheading">Total Gross Weight</td>
	<!--<td width="275" align="center"  valign="middle" class="smalltblheading">Qty Balance</td>-->
	<td width="50" align="center"  valign="middle" class="smalltblheading">Qty MMC</td>
</tr>
<?php 
...
//$row_dalcsubsub2=mysqli_fetch_array($sql_dalcsubsub2);
{
	...
	$sq23=mysqli_query($link,"Select distinct dallocss3_barcode from tbl_dallocsub_sub3 where plantcode='".$plantcode."' and  dallocs_id='$sid' and dalloc_id='$tid'") or die(mysqli_error($link));
...
?>
<tr class="Dark" height="30">
	<td width="27"  align="center"  valign="middle" class="tblheading"><?php echo $sn;?></td>
	<td width="104" align="center"  valign="middle" class="tbltext"><?php echo $cro?>
	  <input type="hidden" name="ecrop<?php echo $sn;?>" id="ecrop_<?php echo $sn;?>" value="<?php echo $cro;?>" /></td>
	<td width="143" align="center"  valign="middle" class="tbltext"><?php echo $variet?>
	  <input type="hidden" name="evariety<?php echo $sn;?>" id="evariety_<?php echo $sn;?>" value="<?php echo $variet;?>" /></td>
	<td width="45" align="center"  valign="middle" class="tbltext"><?php echo $upstyp?>
	  <input type="hidden" name="eupstyp<?php echo $sn;?>" id="eupstyp_<?php echo $sn;?>" value="<?php echo $upstyp;?>" /></td>
	<td width="85" align="center"  valign="middle" class="tbltext"><?php echo $up1?>
	  <input type="hidden" name="eups<?php echo $sn;?>" id="eups_<?php echo $sn;?>" value="<?php echo $up1;?>" /></td>
	<td width="50" align="center"  valign="middle" class="tbltext"><?php echo $sstatus;?>
	  <input type="hidden" name="enop<?php echo $sn;?>" id="enop_<?php echo $sn;?>" value="<?php echo $sstatus;?>" /></td>

	<td width="60" align="center"  valign="middle" class="tbltext"><?php echo $qt;?>
	  <input type="hidden" name="eqty<?php echo $sn;?>" id="eqty_<?php echo $sn;?>" value="<?php echo $qt;?>" /></td>
	<td width="135" align="center"  valign="middle" class="tbltext" title="<?php echo $norno ?>"><?php echo $norno;?>
	  <input type="hidden" name="eordno<?php echo $sn;?>" id="eordno_<?php echo $sn;?>" value="<?php echo $norno;?>" /><input type="hidden" name="enoordno<?php echo $sn;?>" id="enoordno_<?php echo $sn;?>" value="<?php echo $nord;?>" /></td>
	<td width="50" align="center"  valign="middle" class="tbltext">&nbsp;<?php echo $nnomp;?>
	  <input type="hidden" name="rnob<?php echo $sn;?>" id="rnob_<?php echo $sn;?>" value="<?php echo $nnomp;?>" /></td>
	...
	<td width="60" align="center"  valign="middle" class="tbltext">&nbsp;<?php echo $nqty;?>
	  <input type="hidden" name="rqty<?php echo $sn;?>" id="rqty_<?php echo $sn;?>" value="<?php echo $nqty;?>" /></td>
	<td width="60" align="center"  valign="middle" class="smalltbltext"><?php echo $grswt;?>
	  <input type="hidden" name="grswt<?php echo $sn;?>" id="grswt_<?php echo $sn;?>" value="<?php echo $grswt;?>" /></td>
	...
	<td width="50" align="center"  valign="middle" class="tbltext"><?php if($mmcqty>0){ ?><?php echo $mmcqty;?> <?php } else { ?><?php } ?></td>
...
<tr >
<td valign="top" align="right"><!--<input name="Submit" id="subbutn" type="image" src="../images/mmc.gif" alt="Submit Value"  border="0" style="display:inline;cursor:Pointer;" tabindex="" onClick="return mySubmit(1);">-->&nbsp;&nbsp;<a href="add_dispmmc.php?pid=<?php echo $pid;?>"><img src="../images/mmc.gif" border="0"style="display:inline;cursor:Pointer;" /></a>&nbsp;&nbsp;</td>
