...
//echo $tid." ".$subtid."  ".$row_dept4['popularname'];
if($tid>0 && ($subtid==0 || $subtid==""))
{
	$sql2tid=mysqli_query($link,"Select dalloc_id, dallocs_id from tbl_dalloc_sub where plantcode='".$plantcode."' and  dalloc_id='$tid' and dallocs_variety='".$row_dept4['popularname']."' ") or die(mysqli_error($link));
...
if($lotn<>"")
{
	...
	$flg=0; $lotno=""; $nob=0; $qty=0; $totqty=0; $totnob=0; $crop=""; $variety=""; $qc=""; $dot=""; $sloc=""; $qcstsap=''; $bnmp=0; $bqty1=0;
	...
	//echo $lotno."  =  ".$bnmp."  -  ".$totnob." -> ".$totextqtys."  -  ".$bqty1."  =  ".$avlqty."<br/>";
	...
	$sql_var=mysqli_query($link,"SELECT varietyid, popularname, cropname FROM tblvariety where varietyid='".$verids."' and actstatus='Active' order by popularname Asc") or die(mysqli_error($link)); 
	$row_var = mysqli_fetch_array($sql_var);
	$variety=$row_var['popularname'];
	
	$sql_crp=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropid='".$row_var['cropname']."' order by cropname Asc") or die(mysqli_error($link));
	...
	{//echo "Select * from tbl_dallocsub_sub where plantcode='".$plantcode."' and  dalloc_id='$tid' and dallocss_lotno='$lotno'";
...
<?php
$bind1_query=mysqli_query($link,"select binid, binname from tbldbbin order by binname") or die(mysqli_error($link));
?>
...
else
{
	...
	for($i=0; $i<count($lotn); $i++)
	{
		...
		if($ltn<>"")
		{
			...
			$sql_crp=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where plantcode='".$plantcode."' and  cropid='".$row_barcode1['mpmain_crop']."' order by cropname Asc"); 
			$row_crp = mysqli_fetch_array($sql_crp);
			$crop=$row_crp['cropname'];
			
			$sql_var=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where plantcode='".$plantcode."' and  varietyid='".$row_barcode1['mpmain_variety']."' and actstatus='Active' order by popularname Asc"); 
			$row_var = mysqli_fetch_array($sql_var);
			$variety=$row_var['popularname'];
