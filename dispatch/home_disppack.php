<?php
	...
	$sq=mysqli_query($link,"Select * from tbl_disp where plantcode='".$plantcode."' and  disp_tflg=0 and disp_logid='$logid'") or die(mysqli_error($link));
...
<title>Dispatch - Transaction - Dispatch - Direct Loading / Non-Allocation Type - Home</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" style="border-bottom:solid; border-bottom-color:#378b8b" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Dispatch - Direct Loading / Non-Allocation Type</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_disppack.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
</table></td>
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_arr_home2=mysqli_query($link,"select distinct disps_variety from tbl_disp_sub where disp_id='$tid' order by disps_id asc") or die(mysqli_error($link));
	$tot_arr_home2=mysqli_num_rows($sql_arr_home2);
	while($row_arr_home2=mysqli_fetch_array($sql_arr_home2))
	{
	 	
		$sql_sub=mysqli_query($link,"Select * from tbl_disp_sub where disps_variety='".$row_arr_home2['disps_variety']."' and disp_id='$tid'") or die(mysqli_error($link));
		...
		{	
			...
			$sq2=mysqli_query($link,"Select distinct dpss_lotno from tbl_dispsub_sub where disps_id='$sid' and disp_id='$tid'") or die(mysqli_error($link));
			...
			{	
				...
				{	
					$tnp=0;	$tqty=0;
					$lot2=$ro2['dpss_lotno']; 
					$sq3=mysqli_query($link,"Select * from tbl_dispsub_sub where dpss_lotno='$lot2' and disps_id='$sid' and disp_id='$tid'") or die(mysqli_error($link));
...
<tr class="Light">
    ...
    <td align="center" valign="middle" class="smalltbltext"><?php if($row_arr_home['disp_tflg']==1) {?><a href="select_disppack_op.php?p_id=<?php echo $arrival_id;?>">Outputs</a><?php } else if($row_arr_home['disp_logid']==$logid){ ?><a href="edit_disppack.php?pid=<?php echo $arrival_id;?>"><img border="0" src="../images/edit.png"  style="display:inline;cursor:pointer;"  /></a><?php } else { echo $row_arr_home['disp_logid']; } ?></td>
</tr>
<?php
}
else
...
$query = "select * from tbl_disp where disp_tflg=1 and plantcode='".$plantcode."' and disp_type='direct' order by disp_dodc DESC, disp_code DESC";
...
<tr class="Light">
	...
	<td align="center" valign="middle" class="smalltbltext"><?php echo $nqty;?></td>
	<!--<td align="center" valign="middle" class="smalltbltext"><?php echo $sloc;?></td>-->
    <td align="center" valign="middle" class="smalltbltext"><?php if($row_arr_home['disp_tflg']==1) {?><a href="select_disppack_op.php?p_id=<?php echo $arrival_id;?>">Outputs</a><?php } else { ?><?php } ?></td>
</tr>
<?php
}
else
