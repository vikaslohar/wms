...
<title>Dispatch - Transaction - Lot Selection</title>
...
{	
	...
	window.location='getuser_rem_lotno.php?crop='+val3+'&variety='+val4+'&sqq='+txtlot1;
	return true;	
	}
}	
...
?>
 <select class="smalltbltext" name="pcode" style="width:40px;">
    <option value="" >--Select--</option>
	<option value="<?php echo $a;?>" ><?php echo $a;?></option>
    <?php while($noticia = mysqli_fetch_array($quer5)) { ?>
    <option value="<?php echo $noticia['stcode'];?>" />  
    <?php echo $noticia['stcode'];?>
    <?php } ?> </select>&nbsp;&nbsp;<select class="smalltbltext" name="ycodee" id="ycodee" style="width:40px;" onchange="ycodchk();">
    <option value="" selected="selected">--Select--</option>
    <?php while($noticia = mysqli_fetch_array($quer4)) { ?>
    <option value="<?php echo $noticia['ycode'];?>" />  
    <?php echo $noticia['ycode'];?>
    <?php } ?></select><input name="txtlot2" type="text" size="4" class="smalltbltext"  maxlength="5" onkeypress="return 
  isNumberKey(event)"  onchange="lot2chk();"  /> <font size="+1"><b>/</b></font> <input name="stcode" type="text" size="4" class="smalltbltext" tabindex="0" maxlength="5" onkeypress="return isNumberKey(event)"  value="00000" onchange="slocshow();" /> <font size="+1"><b>/</b></font> <input name="stcode2" type="text" size="2" class="smalltbltext" tabindex="0" maxlength="2" onkeypress="return isNumberKey(event)"  value="00"  />
	   <b>P</b>&nbsp;<font color="#FF0000">*</font>&nbsp;&nbsp;<img src="../images/search.gif" border="0"  class="smalltbltext" name="sub" value="Search" style="display:inline;cursor:pointer; vertical-align:middle" onclick="lotchk();" /><!--<input type="image" src="../images/search.gif" border="0"  class="smalltbltext" name="sub" value="Search" style="display:inline;cursor:pointer; vertical-align:middle" onclick="lotchk();"> -->
    </form>
 </td></tr></table>
   <tr>
  <td valign="top">
...
$sss="";
$qr2="select distinct lotno, lotldg_alflg, lotldg_dispflg, lotldg_rvflg from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and packtype='".$txtups."' and lotldg_alflg=0 and lotldg_dispflg!=1 and lotldg_rvflg=0 $sss ";
$sql_tbl_sub2=mysqli_query($link,$qr2) or die(mysqli_error($link));
$tot_row2=mysqli_num_rows($sql_tbl_sub2);
?>
...
{
$srno=1;
while ($row2=mysqli_fetch_array($sql_tbl_sub2))
{
//$qr="select lotno, lotldg_alflg, lotldg_dispflg, lotldg_rvflg from tbl_lot_ldg_pack where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and packtype='".$txtups."' and lotno='".$row2['lotno']."' and lotldg_alflg=0 and lotldg_dispflg!=1 and lotldg_rvflg=0 $sss ";
...
//echo $row['lotno']."<br/>";
$ccnt=0;
$sql_is=mysqli_query($link,"select distinct subbinid, binid, whid from tbl_lot_ldg_pack where  plantcode='".$plantcode."' and  lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and packtype='".$txtups."' and lotno='".$row2['lotno']."' and lotldg_alflg=0 and lotldg_dispflg!=1 and lotldg_rvflg=0 and whid=12 group by subbinid, binid order by lotdgp_id asc") or die(mysqli_error($link));
$xt=mysqli_num_rows($sql_is);
 while($row_is=mysqli_fetch_array($sql_is))
 { 
$sql_is1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  subbinid='".$row_is['subbinid']."' and binid='".$row_is['binid']."' and whid='".$row_is['whid']."' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and packtype='".$txtups."' and lotno='".$row2['lotno']."' and lotldg_alflg=0 and lotldg_dispflg!=1 and lotldg_rvflg=0 order by lotdgp_id asc ") or die(mysqli_error($link));
$row_is1=mysqli_fetch_array($sql_is1); 
//echo $row_is1[0]."  ";
$sql_istbl=mysqli_query($link,"select balqty from tbl_lot_ldg_pack where plantcode='".$plantcode."' and  lotdgp_id='".$row_is1[0]."' and balqty > 0 order by lotdgp_id asc") or die(mysqli_error($link)); 
...
if($trid!="" && $trid > 0)
{
$sqltbl=mysqli_query($link,"select * from tbl_pswrem where plantcode='".$plantcode."' and  pswrem_id='".$trid."'") or die(mysqli_error($link));
$rowtbl=mysqli_fetch_array($sqltbl);
$tot=mysqli_num_rows($sqltbl);		
$arrival_id=$rowtbl['pswrem_id'];

$sqltblsub=mysqli_query($link,"select * from tbl_pswrem_sub where plantcode='".$plantcode."' and  pswrem_id='".$arrival_id."'") or die(mysqli_error($link));
...
/*$lotnum=$row['lotldg_lotno'];
if (!in_array($lotnum, $lotchk)) 
{
$flg++;
}*/
...
<td align="right"  valign="middle" class="smalltbltext"><input type="radio" name="loc" value="<?php echo $row2['lotno'];?>" onclick="clk(this.value,'<?php echo $row2['lotno'];?>','<?php echo $flg1?>');" />&nbsp;</td>
<td align="left"  valign="middle" class="smalltbltext" >&nbsp;<?php echo $row2['lotno'];?></td>
</tr>
<?php
}
else
