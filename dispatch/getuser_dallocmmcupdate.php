<?php
	...
	$sq2=mysqli_query($link,"Select distinct dmmc_lotno from tbl_dallocmmc where dalloc_id='$mainid' and dmmc_flg=2") or die(mysqli_error($link));
	while($ro2=mysqli_fetch_array($sq2))
	{
		$txtolotno=$ro2['dmmc_lotno'];
		$sq=mysqli_query($link,"Select * from tbl_dallocmmc where  dalloc_id='$mainid' and dmmc_flg=2 and dmmc_lotno='".$txtolotno."'") or die(mysqli_error($link));
		while($ro=mysqli_fetch_array($sq))
		{
			...
			$sql_subsub="update tbl_dallocmmc set dmmc_barcode='$barcode', dmmc_wtmp='$mmcnetwt', dmmc_grosswt='$mmcgrwt', dmmc_wh='$txtwhg1', dmmc_bin='$txtbing1', dmmc_bccode=1, dmmc_flg=1, dmmc_mptype='$mptyp' where dmmc_id='$dmmcid'";
			if(mysqli_query($link,$sql_subsub) or die(mysqli_error($link)))
			{
				//$mmcsid=mysqli_insert_id($link);
				$scnt=0;
				/*$sql_arrsubsub=mysqli_query($link,"select distinct subbinid, binid, whid from tbl_lot_ldg_pack where lotno='".$txtolotno."' and packtype='".$packtype."' and subbinid='$extslsubbg' order by lotdgp_id desc") or die(mysqli_error($link));
				$a_sub=mysqli_num_rows($sql_arrsubsub);
				if($a_sub==0)
				{*/
					$sql_arrsubsub=mysqli_query($link,"select distinct subbinid, binid, whid from tbl_lot_ldg_pack where  lotno='".$txtolotno."' and packtype='".$packtype."' order by lotdgp_id desc") or die(mysqli_error($link));
				//}
				while($row_arrsubsub=mysqli_fetch_array($sql_arrsubsub))
				{
					if($scnt==0)
					{
						$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where  subbinid='".$row_arrsubsub['subbinid']."' and binid='".$row_arrsubsub['binid']."' and whid='".$row_arrsubsub['whid']."' and packtype='".$packtype."' and lotno='".$txtolotno."' order by lotdgp_id desc") or die(mysqli_error($link));
				...
				$t++;
			 	$sql_subsub1="update tbl_dallocsub_sub2 set dallocss2_nolp='$txtbalnobp' where dalloc_id='$mainid' and dallocs_id='$sid' and dallocss2_lotno='$txtolotno' and dallocss2_subbin='$extslsubbg'";
				$asdf1=mysqli_query($link,$sql_subsub1) or die(mysqli_error($link));	
				
				
				$sql_subsub4="insert into tbl_dallocsub_sub3 (dalloc_id, dallocs_id, dallocss_id, dallocss3_barcode, dallocss3_weight, dallocss3_grossweight, dallocss3_crop, dallocss3_variety, dallocss3_lotno, dallocss3_ups, dallocss3_dov, dallocss3_dot, dallocss3_qc,plantcode) values ('$mainid', '$sid', '$ssid', '$barcode', '$mmcnetwt', '$mmcgrwt', '$crop', '$variety', '$txtolotno', '$packtype', '$valupto', '$qctestdate', '$qc','$plantcode')";
		...
		if($t>0)
		{
		 	$sql_subsub2="update tbl_dallocsub_sub set dallocss_nolp='$blnop' where dalloc_id='$mainid' and dallocs_id='$sid' and dallocss_lotno='$txtolotno'";
			$asdf2=mysqli_query($link,$sql_subsub2) or die(mysqli_error($link));	
		}
	}
	/*if($t>0)
	{
	 	$sql_subsub4="insert into tbl_dallocsub_sub3 (dalloc_id, dallocs_id, dallocss_id, dallocss3_barcode, dallocss3_weight, dallocss3_grossweight, dallocss3_crop, dallocss3_variety, dallocss3_lotno, dallocss3_ups, dallocss3_dov, dallocss3_dot, dallocss3_qc) values ('$mainid', '$sid', '$ssid', '$barcode', '$mmcnetwt', '$mmcgrwt')";
		mysqli_query($link,$sql_subsub4) or die(mysqli_error($link));
	}*/
...
$sqlsloc=mysqli_query($link,"select distinct order_sub_sub_ups from tbl_order_sub_sub where  orderm_id in($arrivalid) and order_sub_id='$orsid' and order_sub_subbal_qty>0 order by order_sub_sub_ups") or die(mysqli_error($link));
...
if($to=mysqli_num_rows($sq) > 0)
{
	...
	$sq23=mysqli_query($link,"Select distinct dallocss3_barcode, dallocss3_grossweight from tbl_dallocsub_sub3 where  dallocs_id='$sid' and dalloc_id='$tid'") or die(mysqli_error($link));
	...
	$sq3=mysqli_query($link,"Select * from tbl_dallocsub_sub where  dallocs_id='$sid' and dalloc_id='$tid'") or die(mysqli_error($link));
