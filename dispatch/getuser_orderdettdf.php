...
{
	$sqlmonth=mysqli_query($link,"select distinct(orderm_id) from tbl_orderm where plantcode='".$plantcode."' and  orderm_partyname='$a' and orderm_dispatchflag!='1' and orderm_supflag!='1' and orderm_cancelflag!='1' and orderm_tflag=1 and order_trtype='Order TDF' order by orderm_id")or die("Error:".mysqli_error($link));
...
//$sqlmon=mysqli_query($link,"select * from tbl_order_sub where plantcode='".$plantcode."' and  order_sub_variety='".$verrid."' and orderm_id in($arrivalid) and order_sub_totbal_qty>0 order by order_sub_id")or die("Error:".mysqli_error($link));
...
while($row_sloc=mysqli_fetch_array($sql_sloc))
{
	...
	$up1=$qt1." ".$zz[1];
	$up222=$row_sloc['order_sub_sub_ups'];
	if($up!="")
	$up=$up.$up1."<br/>";
	else
	$up=$up1."<br/>";
...
<tr class="Dark" height="30">
	...
	<td width="275" align="center"  valign="middle" class="tbltext"><?php echo $up222?><input type="hidden" name="upstp<?php echo $sn;?>" id="upstp_<?php echo $sn;?>" value="<?php echo $up222?>" /></td>
	...
	<td width="275" align="center"  valign="middle" class="tbltext"><input type="radio" name="selsh<?php echo $sn;?>" id="selsh_<?php echo $sn;?>" onclick="selshow('<?php echo $sn;?>','<?php echo $cro?>','<?php echo $variet?>','<?php echo $ordno ?>','<?php echo $up222?>')" value="<?php echo $sn;?>" /></td>
</tr>
<?php
$sn++;
}
...
?>
<input type="hidden" name="sn" value="<?php echo $sn;?>" /><input type="hidden" name="mchksel" value="" /><input type="hidden" name="selcrp" value="" /><input type="hidden" name="selver" value="" /><input type="hidden" name="selord" value="" /><input type="hidden" name="selups" value="" />
</table>
<table align="center" border="1" width="950" cellspacing="0" cellpadding="0" bordercolor="#378b8b" style="border-collapse:collapse" > 
<tr class="Dark" height="30">
