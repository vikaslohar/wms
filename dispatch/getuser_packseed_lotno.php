...
$arrlots="";
$sql_tbl_sub=mysqli_query($link,"select stouts_lotno from tbl_stouts where stoutm_id='".$trid."'") or die(mysqli_error($link));
...
if($tot_arrsub > 0)
{
	...
	//echo $arrlots;	
	//echo "select distinct(lotno) from tbl_lot_ldg_pack where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and packtype='".$stage."' and lotldg_rvflg=0 $sss ";
 $qr="select distinct(lotno) from tbl_lot_ldg_pack where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and packtype='".$stage."' and lotldg_rvflg=0 $sss ";
...
$vflg=0; $ccnt=0;
//echo "select max(lotdgp_id) from tbl_lot_ldg_pack where lotno='".$row['lotno']."' order by lotdgp_id desc ";
$sql_is1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where lotno='".$row['lotno']."' order by lotdgp_id desc ") or die(mysqli_error($link));
$row_is1=mysqli_fetch_array($sql_is1);
//echo "select * from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and balqty > 0 order by lotdgp_id desc";
$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and balqty > 0 order by lotdgp_id desc") or die(mysqli_error($link)); 
...
<td align="right"  valign="middle" class="tbltext"><input type="radio" name="loc" value="<?php echo $row['lotno'];?>" onclick="clk(this.value,'<?php echo $row['lotno'];?>');" />&nbsp;</td>
