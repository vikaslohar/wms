	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		else
		{
			...
			else
			{
				...
				{
					$sqlb1="update tbl_mpmain set mpmain_alflg=0, mpmain_altrid='0' where plantcode='".$plantcode."' and mpmain_barcode='".$ro['dallocss3_barcode']."'";
					$adcs=mysqli_query($link,$sqlb1) or die(mysqli_error($link));
				}
				$sqlb1="update tbl_mpmain set mpmain_alflg=0, mpmain_altrid='0' where plantcode='".$plantcode."' and mpmain_altrid='".$mainid."'";
				$adcs=mysqli_query($link,$sqlb1) or die(mysqli_error($link));
				...
				if($totre=mysqli_num_rows($sq23)>0)
					$sql_main="update tbl_dalloc set dalloc_dflg='1' where plantcode='".$plantcode."' and dalloc_id='$mainid'";
				else
					$sql_main="update tbl_dalloc set dalloc_tflg='0' where plantcode='".$plantcode."' and dalloc_id='$mainid'";
...
{	
	...
	for (var i=1; i<sn; i++)
	{
		...
		//alert(document.getElementById(sls).checked);
...
function Bagschk1(Bagsval1, val, typ)
{
	...
	if(Bagsval1!="" && Bagsval1 > 0)
	{
		...
		else
		{
			...
			//alert(Bagsval1);
			//alert(document.frmaddDepartment.ewtmp.value);
...
function bin(bin1val, binno)
{
	...
	{	//alert(document.frmaddDepartment.eseltyp.value);
...
if($to=mysqli_num_rows($sq) > 0)
{
	...
	$tor=1;
	//echo "Select * from tbl_dallocsub_sub where dallocs_id='$sid' and dalloc_id='$tid'";
...
<input type="hidden" name="totnewsloc" value="0" />
</div>
<br />
