...
<title>Dispatch - Transaction - Dispatch Bulk - Lot Details</title>
...
{
$sno=1;
$sqq=mysqli_query($link,"Select * from tbl_dbulk_sub where plantcode='".$plantcode."' and dbulks_id='$sid' and dbulk_id='$pid'") or die(mysqli_error($link));
while($roo=mysqli_fetch_array($sqq))
{
	$sq23=mysqli_query($link,"Select * from tbl_dbulksub_sub where plantcode='".$plantcode."' and dbulks_id='$sid' and dbulk_id='$pid'") or die(mysqli_error($link));
	$totre=mysqli_num_rows($sq23);
	while($row23=mysqli_fetch_array($sq23))
	{
		$lotno=$row23['dbss_lotno'];
		$vers=$roo['dbulks_variety'];
		$crps=$roo['dbulks_crop'];
		$nob=$row23['dbss_nob'];
		$qqt=$row23['dbss_qty'];
