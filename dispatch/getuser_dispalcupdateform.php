...
if(isset($_POST['subsubtrid'])) { $subsubtrid=$_POST['subsubtrid']; }
	...
	$sqlcode="SELECT dalloc_id FROM tbl_dalloc where plantcode='".$plantcode."' and  dalloc_dflg!=1 and dalloc_party='$txtstfp' ORDER BY dalloc_id DESC";
	...
	if($z1 == 0)
	{
		...
		if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
		{
			...
			/*for($j=1; $j<$sn; $j++)
			{
				...
				if(isset($_POST[$etxtpvarietyx])) { $etxtpvariety= $_POST[$etxtpvarietyx]; }
				
				$sq=mysqli_query($link,"Select * from tbl_disp_sub where disp_id='$mainid' and disps_variety='$evariety' and disps_ups='$eups' and disps_upstype='$eupstyp'") or die(mysqli_error($link));
				if($to=mysqli_num_rows($sq) > 0)
				{
					$ro=mysqli_fetch_array($sq);
					$sbid=$ro['disps_id'];	
echo					$sqlsubsub="update tbl_disp_sub set disps_nvariety='$etxtpvariety' where disps_id='$sbid' and disps_variety='$evariety' and disps_ups='$eups' and disps_upstype='$eupstyp'";
					$qpod=mysqli_query($link,$sqlsubsub) or die(mysqli_error($link));
				}
			}*/
			...
			$sqllot2=mysqli_query($link,"Select * from tbl_dalloc_sub where plantcode='".$plantcode."' and  dalloc_id='$dalcid' order by dallocs_variety ASC, dallocs_ups ASC, dallocs_upstype ASC") or die(mysqli_error($link));
			$totlot2=mysqli_num_rows($sqllot2);
			while($rowlot2=mysqli_fetch_array($sqllot2))
			{
				$ecrop=$rowlot2['dallocs_crop'];
				$evariety=$rowlot2['dallocs_variety'];
				$eupstyp=$rowlot2['dallocs_upstype'];
				$enop=$rowlot2['dallocs_allocnop'];
				$eqty=$rowlot2['dallocs_allocqty'];
				$eordno=$rowlot2['dallocs_ordno'];
				$enoordno=$rowlot2['dallocs_noorders'];
				$eups=$rowlot2['dallocs_ups'];
				$txtornomp=$rowlot2['dallocs_allocnomp'];
						
				$sqlsubsub="insert into tbl_disp_sub (disp_id, disps_crop, disps_variety, disps_noorders, disps_ordno, disps_upstype, disps_ups, disps_oqty, disps_onop, disps_onomp, disps_nvariety,disps_bqty,plantcode) values ('$mainid', '$ecrop', '$evariety', '$enoordno', '$eordno', '$eupstyp', '$eups', '$eqty', '$enop', '$txtornomp', '$txtpvariety','$eqty','$plantcode')";
				if($qpod=mysqli_query($link,$sqlsubsub) or die(mysqli_error($link)))
				{
					$disps_id=mysqli_insert_id($link);
				}
				for($j=1; $j<$sn; $j++)
				{
					$etxtpvarietyx="txtpvariety".$j;
					$nevarietyx="evariety".$j;
					$neupstypx="eupstyp".$j;
					$neupsx="eups".$j;
					
					if(isset($_POST[$nevarietyx])) { $nevariety= $_POST[$nevarietyx]; }
					if(isset($_POST[$neupstypx])) { $neupstyp= $_POST[$neupstypx]; }
					if(isset($_POST[$neupsx])) { $neups= $_POST[$neupsx]; }
					if(isset($_POST[$etxtpvarietyx])) { $etxtpvariety= $_POST[$etxtpvarietyx]; }
					
					if($nevariety==$evariety && $neupstyp==$eupstyp && $neups==$eups)
					{
						$sq=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$evariety' and disps_ups='$eups' and disps_upstype='$eupstyp' and disps_id='$disps_id'") or die(mysqli_error($link));
						if($to=mysqli_num_rows($sq) > 0)
						{
							$ro=mysqli_fetch_array($sq);
							$sbid=$ro['disps_id'];	
							$sqlsubsub="update tbl_disp_sub set disps_nvariety='$etxtpvariety' where disps_id='$sbid' and disps_variety='$evariety' and disps_ups='$eups' and disps_upstype='$eupstyp'";
							$qpod=mysqli_query($link,$sqlsubsub) or die(mysqli_error($link));
						}
					}
				}
			...
			if($barcode!="" && $brflg==0)
			{
				...
				if($totbar1=mysqli_num_rows($sqlbar1) > 0)
				{
					...
					if($pty=="PACKSMC" || $pty=="PACKNMC")
					{
						...
						foreach($ltno as $lotno)
						{
							if($lotno<>"")	
							{
								...
								/*$sqs=mysqli_query($link,"Select * from tbl_disp_sub where disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_qty>='$qty'") or die(mysqli_error($link));
								$tos=mysqli_num_rows($sqs);
								$ros=mysqli_fetch_array($sqs);
							echo	$sid=$ros['disps_id'];*/
								$totalcsub=0; $dalcuptyp="";
								$sqalcsub3=mysqli_query($link,"Select * from tbl_dallocsub_sub3 where plantcode='".$plantcode."' and  dalloc_id='$dalcid' and dallocss3_lotno='$lotno' and dallocss3_barcode='$barcode'")or die(mysqli_error($link));
								if($totalcsub3=mysqli_num_rows($sqalcsub3)>0)
								{
									$rowalcsub3=mysqli_fetch_array($sqalcsub3);
									
									$sqalcsub=mysqli_query($link,"Select * from tbl_dalloc_sub where plantcode='".$plantcode."' and  dalloc_id='$dalcid' and dallocs_id='".$rowalcsub3['dallocs_id']."'")or die(mysqli_error($link));
									$totalcsub=mysqli_num_rows($sqalcsub);
									$rowalcsub=mysqli_fetch_array($sqalcsub);
									$dalcuptyp=$rowalcsub['dallocs_upstype'];
								}
								if($dalcuptyp!="")
									$sqs=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_oqty>='$qty' and disps_upstype='$dalcuptyp'") or die(mysqli_error($link));
								else
									$sqs=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_oqty>='$qty'") or die(mysqli_error($link));
								$tos=mysqli_num_rows($sqs);
								$ros=mysqli_fetch_array($sqs);
								$sid2=$ros['disps_id'];
								$sid=$sid2;
								if($ros['disps_bqty']!="" && $ros['disps_bqty']!=NULL && (round($ros['disps_bqty'],3)<round($qty,3)))
								{ 
									if($dalcuptyp!="")
									$sqs2=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_oqty>='$qty' and disps_id!='$sid2' and disps_upstype='$dalcuptyp'") or die(mysqli_error($link));
									else
									$sqs2=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_oqty>='$qty' and disps_id!='$sid2'") or die(mysqli_error($link));
									$tos2=mysqli_num_rows($sqs2);
									$ros2=mysqli_fetch_array($sqs2);
									$sid3=$ros2['disps_id'];
									$sid=$sid3;
								}
								...
								$sql_pnp2=mysqli_query($link,"Select max(pnpslipsub_id) from tbl_pnpslipsub where plantcode='".$plantcode."' and  pnpslipsub_plotno='".$lotno."' order by pnpslipsub_id desc") or die(mysqli_error($link));
								$row_pnp2=mysqli_fetch_array($sql_pnp2);
								
								$sql_pnp=mysqli_query($link,"Select pnpslipsub_qcdot from tbl_pnpslipsub where plantcode='".$plantcode."' and  pnpslipsub_plotno='".$lotno."' and pnpslipsub_id='".$row_pnp2[0]."' order by pnpslipsub_qcdot desc") or die(mysqli_error($link));
								...
								$sqlpnp2=mysqli_query($link,"Select max(rv_id) from tbl_revalidate where plantcode='".$plantcode."' and  rv_newlot='".$lotno."' order by rv_id desc ") or die(mysqli_error($link));
								$rowpnp2=mysqli_fetch_array($sqlpnp2);
								
								$sqlpnp=mysqli_query($link,"Select rv_dot from tbl_revalidate where plantcode='".$plantcode."' and  rv_newlot='".$lotno."' and rv_id='".$rowpnp2[0]."' order by rv_dot desc ") or die(mysqli_error($link));
						...
						//$qcdot2=$trdate[2]."-".$trdate[1]."-".$trdate[0];
								...
								if($sid>0)
								{
							 		$sql_subsub2="insert into tbl_dispsub_sub (disp_id, disps_id, dpss_barcode, dpss_crop, dpss_variety, dpss_ups, dpss_lotno, dpss_qty, dpss_grosswt, dpss_dov, dpss_qc, dpss_dot, dpss_barcodetype,plantcode) values ('$mainid', '$sid', '$barcode', '$crop2', '$variety2', '$ups', '$lotno', '$qty', '$grweight', '$dov', '$qc', '$dot', '$packtyp','$plantcode')";
								...
								if($brflg==0)
								{
									$sq=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_id='$sid'") or die(mysqli_error($link));
									if($to=mysqli_num_rows($sq) > 0)
									{
										$ro=mysqli_fetch_array($sq);
										$rq=$rq+$ro['disps_qty'];
										$tore=$tore+$ro['disps_nomp'];
										$txtornomp=$ro['disps_onomp'];
										$eqty=$ro['disps_oqty'];
									}
									$rbq=$eqty-$rq;
									$rn=$txtornomp-$tore;
									$sql_subsub5="update tbl_disp_sub set disps_qty='$rq', disps_bqty='$rbq', disps_onomp='$txtornomp', disps_tnomp='$txtornomp', disps_nomp='$tore', disps_bnomp='$rn' where disps_id='$sid'";
					...
					else
					{
						$qty=$rowbar1['mpmain_wtmp'];			
						$lotn=$rowbar1['mpmain_lotno'].",";
						$ltnop1=$rowbar1['mpmain_lotnop'].",";
						$ups1=$rowbar1['mpmain_upssize'].",";
						$crop1=$rowbar1['mpmain_crop'].",";
						$variety1=$rowbar1['mpmain_variety'].",";
						...
						for($i=0; $i<$ltcount; $i++)
						{
							...
							if($lotno<>"")	
							{
								...
								if($dalcuptyp!="")
								$sqs=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_oqty>='$ltqt' and disps_upstype='$dalcuptyp'") or die(mysqli_error($link));
								else
								$sqs=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_oqty>='$ltqt'") or die(mysqli_error($link));			
								$tos=mysqli_num_rows($sqs);
								$ros=mysqli_fetch_array($sqs);
								$sid2=$ros['disps_id'];
								$sid=$sid2;
								if($ros['disps_bqty']!="" && $ros['disps_bqty']!=NULL && (round($ros['disps_bqty'],3)<round($ltqt,3)))
								{ 
									if($dalcuptyp!="")
									$sqs2=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_oqty>='$ltqt' and disps_id!='$sid2' and disps_upstype='$dalcuptyp'") or die(mysqli_error($link));
									else
									$sqs2=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_oqty>='$ltqt' and disps_id!='$sid2'") or die(mysqli_error($link));
									$tos2=mysqli_num_rows($sqs2);
									$ros2=mysqli_fetch_array($sqs2);
									$sid3=$ros2['disps_id'];
									$sid=$sid3;
						...
						//$qcdot2=$trdate[2]."-".$trdate[1]."-".$trdate[0];
								...
								if($sid>0)
								{
							 		$sql_subsub2="insert into tbl_dispsub_sub (disp_id, disps_id, dpss_barcode, dpss_crop, dpss_variety, dpss_ups, dpss_lotno, dpss_qty, dpss_grosswt, dpss_dov, dpss_qc, dpss_dot, dpss_barcodetype,plantcode) values ('$mainid', '$sid', '$barcode', '$crop2', '$variety2', '$ups', '$lotno', '$ltqt', '$grweight', '$dov', '$qc', '$dot', '$packtyp','$plantcode')";
									$sdfg=mysqli_query($link,$sql_subsub2) or die(mysqli_error($link));
									$rq=$ltqt;			
								}
								else
				...
				if($brflg==0)
				{
					$sqlb1="update tbl_mpmain set mpmain_dflg=2 where mpmain_barcode='".$barcode."'";
					$adcs=mysqli_query($link,$sqlb1) or die(mysqli_error($link));	
								
					$sql_subsub5="update tbl_disp_sub set disps_flg=2 where disps_id='$sid'";
					$asdf5=mysqli_query($link,$sql_subsub5) or die(mysqli_error($link));	
				}	
			}	
	...
	$mainid=$z1;	
		...
		$sql_main="update tbl_disp set tmode='$txt11', trans_name='$txttname', trans_lorryrepno='$txtlrn', trans_vehno='$txtvn', trans_paymode='$txt13', courier_name='$txtcname', docket_no='$txtdc', pname_byhand='$txtpname', disp_remarks='$remarks', disp_tflg='2', disp_type='allocation' where disp_id='$mainid'";
		if(mysqli_query($link,$sql_main) or die(mysqli_error($link)))
		{
			//$mainid=mysqli_insert_id($link);
...
echo					$sqlsubsub="update tbl_disp_sub set disps_nvariety='$etxtpvariety' where disps_id='$sbid' and disps_variety='$evariety' and disps_ups='$eups' and disps_upstype='$eupstyp'";
			...
			while($rowlot2=mysqli_fetch_array($sqllot2))
			{
				...
				$sq=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$mainid' and disps_variety='$evariety' and disps_ups='$eups' and disps_upstype='$eupstyp'") or die(mysqli_error($link));
				if($to=mysqli_num_rows($sq) > 0)
				{
					$ro=mysqli_fetch_array($sq);
					$sbid=$ro['disps_id'];	
					$dspnqt=0;
					if($ro['disps_qty']>0)$dspnqt=$ro['disps_qty'];
					$baqty=$eqty-$dspnqt;	
					$sqlsubsub="update tbl_disp_sub set disps_noorders='$enoordno', disps_ordno='$eordno', disps_oqty='$eqty', disps_onop='$enop', disps_onomp='$txtornomp', disps_bqty='$baqty', disps_nvariety='$newvariet' where disps_id='$sbid'";
					$qpod=mysqli_query($link,$sqlsubsub) or die(mysqli_error($link));
				}
				else
				{
					$sqlsubsub="insert into tbl_disp_sub (disp_id, disps_crop, disps_variety, disps_noorders, disps_ordno, disps_upstype, disps_ups, disps_oqty, disps_onop, disps_onomp, disps_bqty,plantcode) values ('$mainid', '$ecrop', '$evariety', '$enoordno', '$eordno', '$eupstyp', '$eups', '$eqty', '$enop', '$txtornomp', '$eqty','$plantcode')";
					if($qpod=mysqli_query($link,$sqlsubsub) or die(mysqli_error($link)))
					{
						$disps_id=mysqli_insert_id($link);
					}
				...
				if($totbar1=mysqli_num_rows($sqlbar1) > 0)
				{
					...
					if($pty=="PACKSMC" || $pty=="PACKNMC")
					{
						...
						//$qcdot2=$trdate[2]."-".$trdate[1]."-".$trdate[0];
								...
								if($srfl==1 && $qcdot2!="" && $qcdot2!="0000-00-00")$dot=$qcdot2;*/
					...
					else
					{
						...
						for($i=0; $i<$ltcount; $i++)
						{
							...
							if($lotno<>"")	
							{
								...
								/*$sqs=mysqli_query($link,"Select * from tbl_disp_sub where disp_id='$mainid' and disps_variety='$variet' and disps_ups='$ups' and disps_qty>='$ltqt'") or die(mysqli_error($link));
								$tos=mysqli_num_rows($sqs);
								$ros=mysqli_fetch_array($sqs);
								$sid=$ros['disps_id'];*/
								$totalcsub=0; $dalcuptyp="";
...
$sq3=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disps_crop='$cro' and disps_variety='$variet' and disps_ups='$up1' and disps_flg!=1 and disps_upstype='$upstyp' and disp_id='$tid'") or die(mysqli_error($link));
...
//echo $subtid;
/*if($subtid!=0)
$sq24="Select * from tbl_disp_sub where disps_crop='$cro' and disps_variety='$variet' and disps_ups='$up1' and disps_id='$subtid' and disps_flg!=1 and disp_id='$tid'";
else*/
$sq24="Select * from tbl_disp_sub where disps_crop='$cro' and disps_variety='$variet' and disps_ups='$up1' and disps_upstype='$upstyp' and disps_flg!=1 and disp_id='$tid'";

$sq=mysqli_query($link,$sq24) or die(mysqli_error($link));
$nups=""; $nnob=0; $nqty=0; $nbqty=$qt;  $dbsflg=0; $barcds=""; $newvariet="";
...
<?php
/*$totre=0; $nolots=0;
$sq=mysqli_query($link,"Select * from tbl_disp_sub where disps_id='$sid' and disps_flg!=1 and disp_id='$tid'") or die(mysqli_error($link));
if($to=mysqli_num_rows($sq) > 0)
{
...
$tlnomp=$tnomp-$nomp;
	...
	$sq23=mysqli_query($link,"Select distinct dpss_lotno from tbl_dispsub_sub where disps_id='$sid' and disp_id='$tid'") or die(mysqli_error($link));
...
$sq3=mysqli_query($link,"Select distinct dpss_barcode from tbl_dispsub_sub where plantcode='".$plantcode."' and  disp_id='$tid'") or die(mysqli_error($link));
while($ro3=mysqli_fetch_array($sq3))
{
	$bchnflg++;
}
$sq2=mysqli_query($link,"Select distinct dpss_lotno from tbl_dispsub_sub where plantcode='".$plantcode."' and  disp_id='$tid'") or die(mysqli_error($link));
$totrec=mysqli_num_rows($sq2);
?>
...
if($totrec=mysqli_num_rows($sq2) > 0)
{
	while($ro2=mysqli_fetch_array($sq2))
	{
		...
		$sq3=mysqli_query($link,"Select * from tbl_dispsub_sub where plantcode='".$plantcode."' and  dpss_lotno='$lot2' and disp_id='$tid'") or die(mysqli_error($link));
		while($ro3=mysqli_fetch_array($sq3))
		{
			$crps=$ro3['dpss_crop']; 
...
<?php 
$sno=1;
$sq6=mysqli_query($link,"Select * from tbl_dispsub_sub where plantcode='".$plantcode."' and  disp_id='$tid' and dpss_barcode='$barcode'") or die(mysqli_error($link));
...
{
	$lotnst=$rowbarc1['mpmain_lotno'].",";
	$ltnopds1=$rowbarc1['mpmain_lotnop'].",";
	$upsds1=$rowbarc1['mpmain_upssize'].",";
	$verds1=$rowbarc1['mpmain_variety'].",";
	
	$lotn=explode(",",$lotnst);
	$uups=explode(",",$upsds1);
	$ltnps=explode(",",$ltnopds1);
	$verdss=explode(",",$verds1);
	$ltcount=count($lotn);
	for($q=0; $q<$ltcount; $q++)
	{
		$lotno=$lotn[$q];
...
if($brflg!=0)
{
	...
	if($brflg==11)
	$msgs="Barcode $barcode cannot be Dispatched. Reason: Barcode not stored properly while Loading. Please Load again.";
?>
<tr class="Dark" height="30">
...
<table align="center" border="0" width="950" cellspacing="0" cellpadding="0" bordercolor="#378b8b" style="border-collapse:collapse" > 
<tr class="light">
  <td align="center" class="tblheading"><font size="+4" color="<?php if($to6==0){ echo '#FF0000'; } else { if($bchnflg%2==0) echo '#0000FF'; else echo '#009900';}?>"><?php echo $barcode;?></font></td>
</tr>
</table>
