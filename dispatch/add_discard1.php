<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='add_discard1.php?sdate=$sdate&edate=$edate'</script>";
		}
		else
		 {?>
		 <script>
...
<title>Dispatch -  Transazction - Discard Home</title>
...
cellpadding="0">
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" >
	  ...
	  <table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" style="border-bottom:solid; border-bottom-color:#378b8b" >
	    <tr >
	      <td width="810" height="25" class="Mainheading">&nbsp;Transaction - Material Discard </td>
	    </tr></table></td>
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#378b8b" style="border-collapse:collapse">
  ...
  <td width="135" class="tblheading" align="left"><input type="image" src="../images/search.gif" border="0" onClick="return mySubmit();"  /></td>
  </tr>
  </table><br />
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_discard where plantcode='".$plantcode."' and  tdate between '$tdate' and '$pdate' and ddflg=1 order by yearcode desc, dd_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);

//$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_qctest where plantcode='".$plantcode."' and  bflg=1   and qcflg=0"),0);

$query = "select * from tbl_discard where plantcode='".$plantcode."' and  tdate between '$tdate' and '$pdate' and ddflg=1 order by yearcode desc, dd_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_discard where plantcode='".$plantcode."' and  tdate between '$tdate' and '$pdate' and ddflg=1 order by yearcode desc, dd_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_discard where plantcode='".$plantcode."' and  tdate between '$tdate' and '$pdate' and ddflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
?>

<table align="center" border="0" width="700" cellspacing="0" cellpadding="0" bordercolor="#378b8b" style="border-collapse:collapse" > 
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	//$quer3=mysqli_query($link,"SELECT party_name FROM tbl_partymaser  where plantcode='".$plantcode."' and  p_id='".$row_arr_home['party_id']."'");
