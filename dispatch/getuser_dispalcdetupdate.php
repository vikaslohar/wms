...
if(isset($_REQUEST['h'])) { $subsubtrid=$_REQUEST['h']; }
if(isset($_REQUEST['g'])) { $rwsel=$_REQUEST['g']; }
...
if($barcode!="")
{
	$sq33=mysqli_query($link,"Select * from tbl_dispsub_sub where plantcode='".$plantcode."' and  disp_id='$tid' and dpss_barcode='$barcode'") or die(mysqli_error($link));
	while($ro33=mysqli_fetch_array($sq33))
	{
		$sid=$ro33['disps_id'];
		
		$sq333=mysqli_query($link,"Select * from tbl_disp_sub where plantcode='".$plantcode."' and  disp_id='$tid' and disps_id='$sid'") or die(mysqli_error($link));
		$ro333=mysqli_fetch_array($sq333);
		$oqqt=$oqqt+$ro333['disps_oqty'];
	
		$ssq="Delete from tbl_dispsub_sub where disp_id='$tid' and dpss_barcode='$barcode'";
		$sdq=mysqli_query($link,$ssq) or die(mysqli_error($link));
		//$barcode=""; 
		$sq=mysqli_query($link,"Select * from tbl_dispsub_sub where plantcode='".$plantcode."' and  disp_id='$tid' and disps_id='$sid' and dpss_barcode!='$barcode'") or die(mysqli_error($link));
		...
		$bqqt=$oqqt-$qqt;
		$sql_subsub5="update tbl_disp_sub set disps_tnomp='$l', disps_nomp='$tore', disps_qty='$qqt', disps_bqty='$bqqt' where disps_id='$sid'";
...
/*if($subtid!=0)
$sq=mysqli_query($link,"Select * from tbl_disp_sub where disps_crop='$cro' and disps_variety='$variet' and disps_ups='$up1' and disps_id='$subtid' and disps_flg!=1 and disp_id='$tid'") or die(mysqli_error($link));
else*/
...
$sq23=mysqli_query($link,"Select distinct dpss_lotno from tbl_dispsub_sub where disps_id='$sid2' and disp_id='$tid'") or die(mysqli_error($link));
...
<table align="center" border="0" width="950" cellspacing="0" cellpadding="0" bordercolor="#378b8b" style="border-collapse:collapse" > 
<tr class="light">
  <td align="center" class="tblheading"><font size="+4" color="#FF0000"><?php echo $barcode;?></font></td>
</tr>
</table>
