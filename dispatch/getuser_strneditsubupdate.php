...
if(isset($_REQUEST['txtextnob2'])) { $txtextnob2 = $_REQUEST['txtextnob2']; }
if(isset($_REQUEST['txtextqty2'])) { $txtextqty2 = $_REQUEST['txtextqty2']; }
if(isset($_REQUEST['recnobp2'])) { $recnobp2 = $_REQUEST['recnobp2']; }
if(isset($_REQUEST['recqtyp2'])) { $recqtyp2 = $_REQUEST['recqtyp2']; }
if(isset($_REQUEST['txtbalnobp2'])) { $txtbalnobp2 = $_REQUEST['txtbalnobp2']; }
if(isset($_REQUEST['txtbalqtyp2'])) { $txtbalqtyp2 = $_REQUEST['txtbalqtyp2']; }
}
...
{*/
$mainid=$z1;
   $sql_sub="update tbl_proslipsub set proslipsub_lotno='$txtlot1', proslipmain_id='$mainid', proslipsub_onob='$txtonob', proslipsub_oqty='$txtoqty', proslipsub_processtype='$protyp', proslipsub_pnob='$pnob', proslipsub_pqty='$pqty' , proslipsub_bnob='$txtbalnobp1', proslipsub_bqty='$txtbalqtyp1', proslipsub_connob='$txtconnob', proslipsub_conqty='$txtconqty', proslipsub_rm='$txtconrem', proslipsub_im='$txtconim', proslipsub_pl='$pl', proslipsub_rpl='$rpl', proslipsub_tlqty='$txtconloss', proslipsub_tlper='$txtconper', proslipsub_remarks='$txtremarks', proslipsub_sstatus='$softstatus', proslipsub_orlot='$orlot' where proslipsub_id='".$subtrid."'";
if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
{
	$suid=$subtrid;
	$s_sub="delete from tbl_proslipsubsub where proslipsub_id='".$subtrid."' and proslipmain_id='".$mainid."'";
	...
	$s_sub="delete from tbl_proslipsubsub2 where proslipsub_id='".$subtrid."' and proslipmain_id='".$mainid."'";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	if($txtconslqty1!="")
	{
	$sql_sub2="insert into tbl_proslipsubsub2 (proslipsub_id, proslipmain_id, proslipsubsub_wh, proslipsubsub_bin, proslipsubsub_subbin, proslipsubsub_onob, proslipsubsub_oqty, proslipsubsub_pnob, proslipsubsub_pqty, proslipsubsub_bnob, proslipsubsub_bqty,plantcode) values ('$suid', '$mainid', '$txtslwhg1', '$txtslbing1', '$txtslsubbg1', '0', '0', '$txtconslnob1', '$txtconslqty1', '$txtconslnob1', '$txtconslqty1','$plantcode')";
...
$sql_tbl=mysqli_query($link,"select * from tbl_proslipmain where plantcode='".$plantcode."' and  proslipmain_id='".$tid."'") or die(mysqli_error($link));
...
<td width="145" align="left"  valign="middle" class="smalltbltext" >&nbsp;<input type="text" class="smalltbltext" name="txtcrop1" style="background-color:#CCCCCC" readonly="true" value="<?php echo $noticia['cropname'];?>" size="20" />   <input type="hidden" class="smalltbltext" name="txtcrop" style="background-color:#CCCCCC" readonly="true" value="<?php echo $noticia['cropid'];?>" />&nbsp;</td>
			   <?php
$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$row_tbl['proslipmain_variety']."' order by popularname Asc"); 
...
?>
    ...
    <?php
$sql_sel1="select * from tbl_rm_promac where plantcode='".$plantcode."' and  promac_id='".$row_tbl['proslipmain_promachcode']."' order by promac_type";
...
$query_popr=mysqli_query($link,"SELECT * FROM tbl_rm_proopr where plantcode='".$plantcode."' and  proopr_id='".$row_tbl['proslipmain_proopr']."'") or die("Error: " . mysqli_error($link));
...
<?php
$arrival_id;
$sql_tbl_sub=mysqli_query($link,"select * from tbl_proslipsub where plantcode='".$plantcode."' and  proslipmain_id='".$arrival_id."' order by proslipsub_id asc") or die(mysqli_error($link));
...
$difq="";$difq1="";
$sloc=""; $sloc1=""; $cnt++; 
$sql_tbl_subsub=mysqli_query($link,"select * from tbl_proslipsubsub2 where plantcode='".$plantcode."' and  proslipsub_id='".$row_tbl_sub['proslipsub_id']."' and proslipmain_id='".$arrival_id."' order by proslipsubsub_id asc") or die(mysqli_error($link));
...
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where plantcode='".$plantcode."' and  whid='".$row_tbl_subsub['proslipsubsub_wh']."' order by perticulars") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);
$wareh=$row_whouse['perticulars']."/";

$sql_binn=mysqli_query($link,"select binname from tbl_bin where plantcode='".$plantcode."' and  binid='".$row_tbl_subsub['proslipsubsub_bin']."' and whid='".$row_tbl_subsub['proslipsubsub_wh']."'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname']."/";

$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where plantcode='".$plantcode."' and  sid='".$row_tbl_subsub['proslipsubsub_subbin']."' and binid='".$row_tbl_subsub['proslipsubsub_bin']."' and whid='".$row_tbl_subsub['proslipsubsub_wh']."'") or die(mysqli_error($link));
...
<table align="center" border="1" width="850" cellspacing="0" cellpadding="0" bordercolor="#378b8b"  > 
<tr class="Light" height="30">
<td align="right" width="204" valign="middle" class="smalltblheading" style="border-color:#378b8b">&nbsp;Lot No.&nbsp;</td>
<td align="left" width="272" valign="middle" class="smalltbltext" style="border-color:#378b8b">&nbsp;<input name="txtlot1" type="text" size="20" class="smalltbltext" tabindex=""  maxlength="20"  value="" readonly="true" style="background-color:#CCCCCC" >&nbsp;<font color="#FF0000">*</font>&nbsp;<a href="Javascript:void(0);" onclick="openslocpop();">Select</a></td>
<td align="left" width="366" valign="middle" class="smalltblheading" style="border-color:#378b8b">&nbsp;<a href="javascript:void(0);" onclick="getdetails();" >Get details</a></td>
</tr>
</table>
