...
$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));
	...
	*/
	...
	$sql_arr=mysqli_query($link,"select * from tbl_discard where tid='".$pid."'") or die(mysqli_error($link));
	while($row_arr=mysqli_fetch_array($sql_arr))
	{
	$partyid=$row_arr['party_name'];
	$trdate=$row_arr['tdate'];
	
	$sql_arrsub=mysqli_query($link,"select * from tbl_discard_sub where did_s='".$pid."'") or die(mysqli_error($link));
	while($row_arrsub=mysqli_fetch_array($sql_arrsub))
	{
		$classid=$row_arrsub['calssification_id'];
		$itemid=$row_arrsub['items_id'];
		
		$sql_arrsub_sub=mysqli_query($link,"select * from tbl_discard_sloc where discard_trid='".$pid."' and discard_id='".$row_arrsub['did']."'") or die(mysqli_error($link));
		while($row_arrsub_sub=mysqli_fetch_array($sql_arrsub_sub))
		{
			...
			$qty=$row_arrsub_sub['qty_discard'];
				...
				$sql_issue1=mysqli_query($link,"select max(stld_id) from tbl_stldg_damage where stld_subbinid='".$subbinid."' and stld_binid='".$binid."' and stld_whid='".$whid."'") or die(mysqli_error($link));
				$row_issue1=mysqli_fetch_array($sql_issue1); 
					
				$sql_issuetbl=mysqli_query($link,"select * from tbl_stldg_damage where stld_id='".$row_issue1[0]."'") or die(mysqli_error($link)); 
...
/*$a="IS";
	$sql_code="SELECT MAX(issue_code) FROM tblissue where issue_type='stocktr' ORDER BY issue_code DESC";
...
<td width="659" align="left"  valign="middle" class="tbltext"><div style="padding-left:3px"><?php echo $row['remarks'];?></div></td>
</tr>
</table>
<br />
