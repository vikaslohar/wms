...
$sql_month=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='$b' and lotldg_variety='$c' and lotldg_sstage='Condition'")or die("Error:".mysqli_error($link));
while($row_month=mysqli_fetch_array($sql_month))
{
$flg=0;
$lotno=""; $nob=0; $qty=0; $totqty=0; $totnob=0; $crop=""; $variety=""; $qc=""; $dot=""; $sloc=""; 

	$sqlmonth=mysqli_query($link,"select distinct lotldg_subbinid from tbl_lot_ldg where lotldg_crop='$b' and lotldg_variety='$c' and lotldg_sstage='Condition' and lotldg_lotno='".$row_month['lotldg_lotno']."'")or die("Error:".mysqli_error($link));
	while($rowmonth=mysqli_fetch_array($sqlmonth))
	{
		$sqlmonth2=mysqli_query($link,"select MAX(lotldg_id) from tbl_lot_ldg where lotldg_crop='$b' and lotldg_variety='$c' and lotldg_sstage='Condition' and lotldg_lotno='".$row_month['lotldg_lotno']."' and lotldg_subbinid='".$rowmonth['lotldg_subbinid']."'")or die("Error:".mysqli_error($link));
...
//echo $flg;
$llttn=""; $xcltn=array();
$sqq=mysqli_query($link,"Select * from tbl_dbulksub_sub where dbulks_id='$subtid'") or die(mysqli_error($link));
