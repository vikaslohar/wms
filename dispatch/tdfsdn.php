...
<title>Dispatch - Transaction - Dispatch TDF - TDFSDN</title>
...
<tr  height="20">
  <td colspan="6" align="center" class="Mainheading"><font color="#000000">TDF Seed Dispatch Note (TDFSDN)</font></td>
</tr>
</table><br />	  
...
$sql23="select * from tbluser where scode='".$row_tbl['dtdf_logid']."'";
...
$sql_code1="SELECT * FROM tbl_dispnote where dnote_trid='$tid' and dnote_trtype='Dispatch TDF Seed'";
...
$code1=$row_param['code']."/".$row_code1['dnote_yearcode']."/"."TD"."/".sprintf("%004d",$row_code1['dnote_code']);
...
{
	if($ordernos!="")
	$ordernos=$ordernos.",".$row_arrsub['dtdfs_ordno'];
	else
...
<td width="168" align="right" valign="middle" class="tblheading">&nbsp;TDFSDN No.&nbsp;</td>
...
<td width="194" align="left" valign="middle" class="tbltext">&nbsp;<?php echo $row_tbl['dtdf_dcno'];?></td>
</tr>-->
<?php
$phone=''; $gstin='';
...
$business_name=$noticia['business_name'];
$address=$noticia['address'].", ".$noticia['city'].", ".$noticia['state'];
if($noticia['pin']!="" && $noticia['pin']>0) { $address=$address." - ".$noticia['pin']."."; }
if($noticia['phone']!="")
$phone=$noticia['std'].", ".$noticia['phone'];
if($noticia['mob']!="" && $noticia['mob']>0)
{
if($phone!="")
$phone=$phone.", ".$noticia['mob'];
else
$phone=$noticia['mob'];
}
$gstin=$noticia['gstin'];
	...
	//else
	//echo $row_ord['orderm_partyselect'];
	if($row_ord['orderm_partyselect']=="fillp")
	{
		...
		if($row_ord['orderm_partypin']!="" && $row_ord['orderm_partypin']>0) { $address=$address." - ".$row_ord['orderm_partypin']."."; }
		if($row_ord['orderm_partyphno']!="" && $row_ord['orderm_partyphno']!=0)
		{
			$phone=$row_ord['orderm_partyphstd']." - ".$row_ord['orderm_partyphno'];
		}
		if($row_ord['orderm_partymobile']!="" && $row_ord['orderm_partymobile']!=0)
		{
			if($phone!="")
			$phone=$phone.", ".$row_ord['orderm_partymobile'];
			else
			$phone=$row_ord['orderm_partymobile'];
		}
	}
if($business_name==""){$business_name=$row_tbl['dtdf_party'];}
?> 
...
<td align="left"  valign="middle" class="tbltext" colspan="3" ><div align="justify" class="tbltext" style="padding:2px 5px 5px 5px"><?php echo $address;echo "   Contact-".$phone;?></div></td>
</tr>
<tr class="Light" height="25">
<td align="right"  valign="middle" class="tblheading">GSTIN&nbsp;</td>
<td align="left"  valign="middle" class="tbltext" colspan="3" ><?php if($gstin!="") { echo $gstin; }?></td>
</tr>
<tr class="Light" height="25">
...
<?php 
$sn=1;  $totqty;
//echo "select distinct dtdfs_variety from tbl_dtdf_sub where dtdf_id='$tid' and dtdfs_flg=1 order by dtdfs_id asc";
$sql_arr_home=mysqli_query($link,"select distinct dtdfs_variety from tbl_dtdf_sub where dtdf_id='$tid' and dtdfs_flg=1 order by dtdfs_id asc") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
if($tot_arr_home >0) 
{ 
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
//echo "Select DISTINCT dtdfs_ups from tbl_dtdf_sub where dtdfs_variety='".$row_arr_home['dtdfs_variety']."' and dtdf_id='$tid'";
$sql_sub2=mysqli_query($link,"Select DISTINCT dtdfs_ups from tbl_dtdf_sub where dtdfs_variety='".$row_arr_home['dtdfs_variety']."' and dtdfs_ups!='' and dtdf_id='$tid'") or die(mysqli_error($link));
while($row_sub2=mysqli_fetch_array($sql_sub2))
{
$nups=""; $nups2=""; $nnob=""; $nqty=""; $lotno=""; $dot=""; $tnob=0; $tqty=0; $dov='';
//echo "Select * from tbl_dtdf_sub where dtdfs_variety='".$row_arr_home['dtdfs_variety']."' AND dtdfs_ups='".$row_sub2['dtdfs_ups']."' and dtdf_id='$tid'";
$sql_sub=mysqli_query($link,"Select * from tbl_dtdf_sub where dtdfs_variety='".$row_arr_home['dtdfs_variety']."' AND dtdfs_ups='".$row_sub2['dtdfs_ups']."' and dtdf_id='$tid'") or die(mysqli_error($link));
while($row_sub=mysqli_fetch_array($sql_sub))
{
$crpnm=$row_sub['dtdfs_crop'];  
$nvariety=$row_sub['dtdfs_nvariety']; 
$nups2=$row_sub2['dtdfs_ups']; 

if($nvariety=="" || $nvariety=="undefined")$nvariety=$row_arr_home['dtdfs_variety'];

$sqq=mysqli_query($link,"Select * from tbl_dtdfsub_sub where dtdfs_id='".$row_sub['dtdfs_id']."'") or die(mysqli_error($link));
while($roo=mysqli_fetch_array($sqq))
{
if($roo['dbss_qty']>0)
{

$zz24=str_split($roo['dbss_lotno']);
$ltno24=$zz24[0].$zz24[1].$zz24[2].$zz24[3].$zz24[4].$zz24[5].$zz24[6].$zz24[7].$zz24[8].$zz24[9].$zz24[10].$zz24[11].$zz24[12].$zz24[13].$zz24[14].$zz24[15];
	
if($lotno!="")
$lotno=$lotno."<br />".$ltno24;
else
$lotno=$ltno24;
if($nnob!="")
...
$sql_var=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where popularname='".$row_arr_home['dtdfs_variety']."' and actstatus='Active' order by popularname Asc"); 
$row_var = mysqli_fetch_array($sql_var);
$variety=$row_var['varietyid'];

$vflg=0;
if($row_sub['dtdfs_stage']!="Pack")	
{
$sqlmonth2=mysqli_query($link,"select MAX(lotldg_id) from tbl_lot_ldg where lotldg_crop='$crop' and lotldg_variety='$variety' and lotldg_lotno='".$roo['dbss_lotno']."'")or die("Error:".mysqli_error($link));
...
//echo "select MAX(lotdgp_id) from tbl_lot_ldg_pack where lotldg_crop='$crop' and lotldg_variety='$variety' and packtype='$nups' and lotno='".$roo['dbss_lotno']."'";

$sqlmonth2=mysqli_query($link,"select MAX(lotdgp_id) from tbl_lot_ldg_pack where lotldg_crop='$crop' and lotldg_variety='$variety' and packtype='$nups2' and lotno='".$roo['dbss_lotno']."'")or die("Error:".mysqli_error($link));
...
//echo "  =  ";
$trdate=$rowmonth3['lotldg_valupto'];//echo "<br/>";
$trdate=explode("-",$trdate);
$dov2=$trdate[2]."-".$trdate[1]."-".$trdate[0];
...
<tr class="Dark" height="30">
	<td width="275" align="right"  valign="middle" class="tbltext" colspan="9">Grand Total&nbsp;</td>
