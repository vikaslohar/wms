...
$srno=1;
//echo $ttype;
$sql_disp=mysqli_query($link,"select * from tbl_disp where plantcode='".$plantcode."' and  disp_id='".$ttype."' order by disp_id asc") or die(mysqli_error($link));
while($row_disp=mysqli_fetch_array($sql_disp))
{
	$sql_dispss=mysqli_query($link,"select * from tbl_dispsub_sub where plantcode='".$plantcode."' and  disp_id='".$row_disp['disp_id']."' and dpss_lotno='".$lotno12."' order by dpss_id asc") or die(mysqli_error($link));
	while($row_dispss=mysqli_fetch_array($sql_dispss))
	{
		$lotnetwt=0; $barcode=""; $bartype=""; $grwt=""; $netwt=""; $barups="";
		if($row_dispss['dpss_barcodetype']=="MMC")
		{
			$sql_dismmc=mysqli_query($link,"select * from tbl_dallocmmc where plantcode='".$plantcode."' and  dmmc_barcode='".$row_dispss['dpss_barcode']."' and dmmc_lotno='".$lotno12."' order by dmmc_id asc") or die(mysqli_error($link));
...
<tr class="Light" height="20">
	...
	<td width="99" align="center" valign="middle" class="tbltext"><?php echo $barups;?></td>
</tr>
<?php
$srno++;
}}?>
