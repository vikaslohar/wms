...
<?php
/*	$tot4=0;  $tsln=0;
	$sq42=mysqli_query($link,"Select distinct dmmc_bin, dmmc_wh from tbl_dtdfmmc where dtdf_id='$tid' and dmmc_flg=1") or die(mysqli_error($link));
...
$nb=0;
$mbqt=0;
$sq4=mysqli_query($link,"Select * from tbl_dtdfmmc where dtdf_id='$tid' and dmmc_flg=1 and dmmc_bin='".$ro4['dmmc_bin']."'") or die(mysqli_error($link));
while($ro42=mysqli_fetch_array($sq4))
{
	$mbqt=$mbqt+$ro42['dmmc_qty'];
}
$sq43=mysqli_query($link,"Select distinct dmmc_barcode from tbl_dtdfmmc where dtdf_id='$tid' and dmmc_flg=1 and dmmc_bin='".$ro4['dmmc_bin']."'") or die(mysqli_error($link));
...
<?php
$whd1_query=mysqli_query($link,"select whid, perticulars from tbldbwarehouse where whid='".$ro4['dmmc_wh']."' order by perticulars") or die(mysqli_error($link));
$noticia_whd1 = mysqli_fetch_array($whd1_query);
?>
...
<?php
$bind1_query=mysqli_query($link,"select binid, binname from tbldbbin where whid='".$ro4['dmmc_wh']."' and binid='".$ro4['dmmc_bin']."' order by binname") or die(mysqli_error($link));
$noticia_bing1 = mysqli_fetch_array($bind1_query);
?>
...
<tr >
<td valign="top" align="right"><img src="../images/update.gif" border="0"style="display:inline;cursor:Pointer;" id="updatebutn" onClick="pmmcup();" />&nbsp;&nbsp;</td>
</tr>
</table>