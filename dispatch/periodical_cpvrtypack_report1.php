<?php
	...
	require_once("../include/connection.php");
		...
		$vcreqsel=trim($_REQUEST['vcreqsel']);	
...
var vcreqsel=document.frmaddDepartment.vcreqsel.value;
...
winHandle=window.open('periodical_cpvrtypack_report2.php?txtcrop='+itemid+'&txtvariety='+vv+'&txtupsdc='+txtupsdc+'&vcreqsel='+vcreqsel+'&dotage='+dotage+'&durtyp='+durtyp+'&fillagetyp='+fillagetyp+'&totdays='+totdays,'WelCome','top=20,left=80,width=950,height=600,scrollbars=yes');
if(winHandle==null){
...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <?php
	...
	$cp=""; $curdate=date("Y-m-d");
	$sql_crp=mysqli_query($link,"select * from tblcrop where croptype='Field Crop' order by cropname ASC") or die(mysqli_error($link));
	...
	if($txtupsdc!="ALL")
	{
		$sqlrr=mysqli_query($link,"select distinct packtype from tbl_lot_ldg_pack where plantcode='".$plantcode."' and packtype='".$txtupsdc."' and lotldg_crop IN ($cp) order by packtype asc") or die(mysqli_error($link));
	}
	else
	{
		$sqlrr=mysqli_query($link,"select distinct packtype from tbl_lot_ldg_pack where plantcode='".$plantcode."' and lotldg_crop IN ($cp) order by packtype Asc") or die(mysqli_error($link));
	}
...
?>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" style="border-bottom:solid; border-bottom-color:#378b8b" >
	    <tr >
	      <td width="813" height="25">Crop Variety Wise Packed Seed Report As on <?php echo date("d-m-Y");?></td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
	  ...
	  <td align="center" colspan="4" >
		...
		<input name="vcreqsel" value="<?php echo $vcreqsel;?>" type="hidden">
		<input name="durtyp" value="<?php echo $durtyp;?>" type="hidden">
		<input name="dotage" value="<?php echo $dotage;?>" type="hidden">
		<input name="fillagetyp" value="<?php echo $fillagetyp;?>" type="hidden">
		<input name="totdays" value="<?php echo $totdays;?>" type="hidden">
...
if($vcreqsel=="yes")
{
	if($durtyp=="dsel")
	{
		...
		if($dotage=="1")
		$durations="1 Month and More";
		if($dotage=="2")
		$durations="2 Month and More";
		if($dotage=="3")
		$durations="3 Month and More";
		if($dotage=="4")
		$durations="4 Month and More";
		if($dotage=="5")
		$durations="5 Month and More";
		if($dotage=="6")
		$durations="6 Month and More";
		if($dotage=="7")
		$durations="7 Month and More";
		if($dotage=="more8")
		$durations="8 Months and More";
	}
...
<tr height="25" >
	<td align="left" class="subheading" style="color:#303918;">&nbsp;&nbsp;Crop: <?php echo $crp;?>&nbsp;&nbsp;|&nbsp;&nbsp;Variety: <?php echo $variety;?>&nbsp;&nbsp;|&nbsp;&nbsp;Size: <?php echo $txtupsdc;?></td>
...
?>
<tr height="25" >
	<td align="left" class="subheading" style="color:#303918;">&nbsp;&nbsp;Validity Check: <?php echo $durations;?></td>
...
if($txtupsdc!="ALL")
{
	$sql_rr=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg_pack where plantcode='".$plantcode."' and lotldg_crop='".$row_arr_home1['lotldg_crop']."' and packtype='".$txtupsdc."' and lotldg_qc!='Fail' and lotldg_got!='Fail'  order by lotdgp_id desc") or die(mysqli_error($link));
}
else
{
			...
			while($row_rr2=mysqli_fetch_array($sql_rr2))
			{
				...
				while($row_arr_home=mysqli_fetch_array($sql_arr_home))
				{
					...
					while($row_tbl=mysqli_fetch_array($sql_tbl_sub1))
					{
						...
						$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where plantcode='".$plantcode."' and lotdgp_id='".$row_tbl1[0]."' and lotldg_qc!='Fail' and lotldg_got!='Fail' ") or die(mysqli_error($link)); 
									...
									if($vcreqsel=="yes")
									{
										...
										if($durtyp=="dfill")
										{
											if($fillagetyp=="less")
											{
												...
												if($dt2!="")
												{
													if($row_issuetbl['lotldg_valupto']>$dt2)$flg++;
												}
											}
										...
										else
										{
											if($dotage=="less1")
											{
												...
												//echo $row_issuetbl['lotldg_valupto']." = ".$dt2."  -  ".$flg."<br />";
											}
											else if($dotage=="1")
											{
												$dt=1;
...
<tr >
<td height="30" align="center" valign="top"><a href="periodical_cpvrtypack_report.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" /><input type="hidden" name="txtinv" /></td>
</tr>
</table>
