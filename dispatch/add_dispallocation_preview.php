<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		while($row_arr=mysqli_fetch_array($sql_arr))
		{
			...
			$sql_arrsub2=mysqli_query($link,"select * from tbl_dallocsub_sub3 where plantcode='".$plantcode."' and  dalloc_id='".$pid."'") or die(mysqli_error($link));
			$a_arrsub2=mysqli_num_rows($sql_arrsub2);
			while($row_arrsub2=mysqli_fetch_array($sql_arrsub2))
			{
				$sql_main2="update tbl_mpmain set mpmain_alflg=1, mpmain_altrid='$pid' where mpmain_barcode='".$row_arrsub2['dallocss3_barcode']."'";
		...
		$sql_main="update tbl_dalloc set dalloc_tflg=1, dalloc_alflg=2, dalloc_code=$ncode1 where dalloc_id='$pid'";
		$a123456=mysqli_query($link,$sql_main) or die(mysqli_error($link));	
		//exit; 		
		if($sbmval==1)
			echo "<script>window.location='add_dispmmc.php?pid=$pid'</script>";	
		else
			echo "<script>window.location='home_dispallocation.php?pid=$pid'</script>";	
	}
?>
...
<tr class="Dark" height="30">
	...
	<td width="60" align="center"  valign="middle" class="tbltext">&nbsp;<a href="Javascript:void(0);" onClick="showmlots1('<?php echo $sid;?>')"><?php echo $nqty;?></a>
