<?php
	...
	$sq=mysqli_query($link,"Select * from tbl_disp where plantcode='".$plantcode."' and  plantcode='".$plantcode."' and  disp_tflg=0 and disp_logid='$logid'") or die(mysqli_error($link));
	while($ro=mysqli_fetch_array($sq))
	{
		$arid=$ro['disp_id'];
		$sql_arrsub=mysqli_query($link,"select * from tbl_disp_sub where plantcode='".$plantcode."' and  plantcode='".$plantcode."' and  disp_id='".$arid."'") or die(mysqli_error($link));
		$a_arrsub=mysqli_num_rows($sql_arrsub);
		while($row_arrsub=mysqli_fetch_array($sql_arrsub))
		{
			$subid=$row_arrsub['disps_id'];
			$sql_arrsub2=mysqli_query($link,"select * from tbl_dispsub_sub where plantcode='".$plantcode."' and  plantcode='".$plantcode."' and  disp_id='".$arid."' and disps_id='".$subid."'") or die(mysqli_error($link));
			...
			if($a_arrsub2==0)
			{
				echo $s_sub="delete from tbl_disp_sub where disps_id='".$subid."'";
	...
	$ssub="delete from tbl_disp where disp_logid='".$logid."' and disp_tflg=0";
...
<title>Dispatch - Transaction - Dispatch - Direct Loading / Non-Allocation Type with Barcode Range - Home</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" style="border-bottom:solid; border-bottom-color:#378b8b" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Dispatch - Direct Loading / Non-Allocation Type with Barcode Range</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_disppack_rng.php" style="text-decoration:none; color:#000000">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#000000">Add </a><?php } ?></td>
</tr>
</table></td>
...
<!--<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#378b8b" style="border-collapse:collapse">
  ...
  <?php
$sql_arr_home=mysqli_query($link,"select * from tbl_disp where plantcode='".$plantcode."' and  plantcode='".$plantcode."' and  disp_tflg=2 and disp_type='range' order by disp_id DESC") or die(mysqli_error($link));
...
<tr class="Light">
    ...
    <td align="center" valign="middle" class="smalltbltext"><?php if($row_arr_home['disp_tflg']==1) {?><a href="select_disppackrng_op.php?p_id=<?php echo $arrival_id;?>">Outputs</a><?php } else if($row_arr_home['disp_logid']==$logid){ ?><a href="edit_disppack_rng.php?pid=<?php echo $arrival_id;?>"><img border="0" src="../images/edit.png"  style="display:inline;cursor:pointer;"  /></a><?php } else { echo $row_arr_home['disp_logid']; } ?></td>
</tr>
<?php
}
else
...
<br />
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_disp where plantcode='".$plantcode."' and  disp_tflg=1 and disp_type='range' order by disp_dodc DESC, disp_code DESC LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_disp where plantcode='".$plantcode."' and  disp_tflg=1 and disp_type='range' order by disp_dodc DESC, disp_code DESC";
...
<tr class="Light">
	...
	<td align="center" valign="middle" class="smalltbltext"><?php echo $nqty;?></td>
	<!--<td align="center" valign="middle" class="smalltbltext"><?php echo $sloc;?></td>-->
    <td align="center" valign="middle" class="smalltbltext"><?php if($row_arr_home['disp_tflg']==1) {?><a href="select_disppackrng_op.php?p_id=<?php echo $arrival_id;?>">Outputs</a><?php } else { ?><?php } ?></td>
</tr>
<?php
}
else
