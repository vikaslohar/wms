<?php
	...
	$sql_code="SELECT MAX(stoutm_tcode) FROM tbl_stoutm where plantcode='".$plantcode."' ORDER BY stoutm_tcode DESC";
...
/*function editrec(v1,v2)
{
	if(confirm('Do u wish to delete this item?')==true)
	{
		showUser(v1,'edittable','edit',v2,'','','');
	}
	else
	{
		return false;
...
function chkbarcode1(mltval)
{
	...
	if(document.frmaddDepartment.txttobenomp.value==document.frmaddDepartment.loadnomp.value)
	{
...
function stocknomp(nomp)
{
	...
	if(nomp>document.frmaddDepartment.txtavlnomp.value)
	{
		alert("Invalid NoMP");
	...
	//alert(document.frmaddDepartment.wtmp.value);
...
<form id="mainform" name="frmaddDepartment" method="post" action="<?php $_SERVER['PHP_SELF']; ?>" onSubmit="return mySubmit();"   > 
	 <input name="frm_action" value="submit" type="hidden"> 
		...
		<input type="hidden" name="yearcodes" value="I" /> 
...
<?php 
//echo "SELECT * FROM tbl_stoutmpack where stoutmp_id='".$maintrid."'";
...
$quer=mysqli_query($link,"SELECT * FROM tbl_partymaser  where classification='Stock Transfer-Plant' and p_id='".$row_transitmode['stoutmp_plantid']."' order by classification");
...
<td  colspan="3" align="left"  valign="middle" class="tbltext" id="vitem1">&nbsp;<?php echo $row_quer['business_name'];?><input type="hidden" name="txttoplant" value="<?php echo $row_transitmode['stoutmp_plantid'];?>"/></td>
</tr>
...
<?php 
$srno=1;
//echo "Select * from tbl_stoutspack where stoutmp_id=$maintrid and stoutsp_subflg=1 order by stoutsp_id asc";
$sql_sub=mysqli_query($link,"Select * from tbl_stoutspack where plantcode='".$plantcode."' and stoutmp_id=$maintrid order by stoutsp_id asc") or die(mysqli_error($link));
...
<input type="hidden" name="maintrid" value="<?php echo $maintrid;?>" /><input type="hidden" name="subtrid" value="<?php echo $subtid;?>" /><!--<input type="hidden" name="totbarcs" value="<?php echo $barc;?>" />-->
