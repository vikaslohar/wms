...
//exit;
	...
	//echo $barcode;
	$sql_batype=mysqli_query($link,"SELECT * FROM tbl_mpmain where plantcode='".$plantcode."' and  mpmain_barcode='".$delbarcode."' ");
...
if($brflg==0 || $brflg=="")	
{
	//echo "SELECT * FROM tbl_stoutsspack where stoutssp_barcode='".$delbarcode."' and stoutmp_id='".$maintrid."' order by stoutssp_id asc";
	$sql_main=mysqli_query($link,"SELECT * FROM tbl_stoutsspack where plantcode='".$plantcode."' and  stoutssp_barcode='".$delbarcode."' and stoutmp_id='".$maintrid."' order by stoutssp_id asc") or die(mysqli_error($link));
	while($row_main=mysqli_fetch_array($sql_main))
	{
		$subid=$row_main['stoutsp_id'];
		
		$sql_sub=mysqli_query($link,"SELECT * FROM tbl_stoutspack where plantcode='".$plantcode."' and  stoutsp_id='".$subid."' and stoutmp_id='".$maintrid."' order by stoutsp_id asc") or die(mysqli_error($link));
		$row_sub=mysqli_fetch_array($sql_sub);
		$loadnomp=$row_sub['stoutsp_loadnomp']-1;
		$loadqty=$row_sub['stoutsp_loadqty']-$row_main['stoutssp_lotqty'];
		$balnomp=$row_sub['stoutsp_balnomp']+1;
		$balqty=$row_sub['stoutsp_balqty']+$row_main['stoutssp_lotqty'];
		$loadgrswt=$row_sub['stoutsp_loadgrswt']+$row_main['stoutssp_grosswt'];
		
		$sql_sub1="update tbl_stoutspack set stoutsp_loadgrswt='$loadgrswt', stoutsp_loadnomp='$loadnomp', stoutsp_loadqty='$loadqty', stoutsp_balnomp='$balnomp', stoutsp_balqty='$balqty' where stoutsp_id='".$subid."' and stoutmp_id='".$maintrid."'";
	//exit;
		mysqli_query($link,$sql_sub1) or die(mysqli_error($link));
		
		$sql_sub2=mysqli_query($link,"SELECT * FROM tbl_stoutspack where plantcode='".$plantcode."' and  stoutsp_id='".$subid."' and stoutmp_id='".$maintrid."' order by stoutsp_id asc") or die(mysqli_error($link));
		$row_sub2=mysqli_fetch_array($sql_sub2);
		
		if($row_sub2['stoutsp_loadqty']==0)
		{
			$sql_sub4="delete from tbl_stoutspack where stoutsp_id='".$subid."' and stoutmp_id='".$maintrid."'";
			mysqli_query($link,$sql_sub4) or die(mysqli_error($link));
		}
	}
	$sql_subsub="delete from tbl_stoutsspack where stoutssp_barcode='".$delbarcode."' and stoutmp_id='".$maintrid."'";
	mysqli_query($link,$sql_subsub) or die(mysqli_error($link));
	
	$sql_mpmain="update tbl_mpmain set mpmain_dflg=0 where mpmain_barcode='".$delbarcode."' ";
	mysqli_query($link,$sql_mpmain) or die(mysqli_error($link));
	
	$sql_barcode="update tbl_barcodes set bar_dispflg=0 where bar_barcode='".$delbarcode."' ";
...
<tr class="Dark" height="30">
	...
	<td align="center"  valign="middle" class="smalltbltext"><input type="text" size="3" name="txttobenomp" id="txttobenomp" value="<?php echo $tobenomp?>" onchange="stocknomp()" /></td>
...
<div id="barchk"><input type="hidden" name="brflg" value="" /><input type="hidden" name="brchflg" value="0" /></div><br />
