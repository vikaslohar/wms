<?php
	...
	//frm_action=submit&txt14=&txtid=1&logid=DP1&getdetflg=0&txtconchk=&txtptype=Branch&txtcountrysl=&txtcountryl=&rettype=&extdcno=&plantcodes=D%2CG%2CH&yearcodes=A%2CD%2CF%2CK%2CN%2CP%2CS&trsbmval=0&date=29-12-2015&txtdcno=test&dcdate=29-12-2015&txtpp=Branch&txtstatesl=Chhattisgarh&txtlocationsl=56&locationname=56&txtstfp=907&ecrop1=Bitter%20Gourd&evariety1=Improved%20Katahi&eupstyp1=ST&eups1=100.000%20Gms&enop1=4000&eqty1=10&eordno1=OT563%2F15-16%2FOB2&rnob1=&rnob1=&rqty1=&grswt1=&bnop1=10&selsh=1&mchksel=1&txtornos=OT563%2F15-16%2FOB2&txtveridno=15&txtupsnos=100.000%20Gms&txteqty=10&sn=2&totbarcs=&txtecrop=Bitter%20Gourd&txtevariety=Improved%20Katahi&txteupstyp=ST&txteups=100.000%20Gms&txtenop=4000&txtornomp=1&txteordno=OT563%2F15-16%2FOB2&txteoqty=10&txtallonolp=0&txtallnomp=1&txttobealqty=10&txttlonomp=0&txttlonolp=0&txtloqty=0&txtlonolots=0&txtorblnomp=0&txtorblnolp=0&txtorblqty=0&ewtmp=10&emptnop=100&allctyp=barcodewise&allocationtype=barcodewise&brflg=0&brchflg=1&barcode=DP010060405&totnewsloc=0&maintrid=0&subtrid=0&subsubtrid=0&newsloc=
				...
				if(mysqli_query($link,$sql_subsub) or die(mysqli_error($link)))
				{
					...
					if($allctyp=="barcodewise")
					{
						...
						if($barcode!="" && $brflg==0)
						{
							...
							if($totbar1=mysqli_num_rows($sqlbar1) > 0)
							{
								...
								for($i=0; $i<$x-1; $i++)
								{
									...
									if($lotno<>"")	
									{
										...
										$sql_subsub2="insert into tbl_dallocsub_sub (dalloc_id, dallocs_id, dallocss_altype, dallocss_lotno, dallocss_ups, dallocss_wtmp, dallocss_mptnop, dallocss_seltyp, dallocss_qty, dallocss_nomp ,plantcode) values ('$mainid', '$sid', '$allctyp', '$lotno', '$ups', '$qty', '$ltnps', '$eseltyp', '$ltqt', '1','$plantcode')";
							...
							$sqlb1="update tbl_mpmain set mpmain_alflg=2, mpmain_altrid='$mainid' where plantcode='".$plantcode."' and mpmain_barcode='".$barcode."'";
							$adcs=mysqli_query($link,$sqlb1) or die(mysqli_error($link));	
							
							if($binshifting=="yes")
							{
								$rn2=0; $rq2=0; $rnbc2=0; $rqbc2=0;
								/*$sq2=mysqli_query($link,"Select * from tbl_dallocsub_sub where dallocss_id='$ssid'") or die(mysqli_error($link));
								...
								$sqbc23=mysqli_query($link,"Select distinct dallocss3_barcode from tbl_dallocsub_sub3 where plantcode='".$plantcode."' and dallocs_id='$sid'") or die(mysqli_error($link));
								if($tobc23=mysqli_num_rows($sqbc23) > 0)
								{
									while($robc23=mysqli_fetch_array($sqbc23))
									{
										$sqbc2=mysqli_query($link,"Select * from tbl_dallocsub_sub3 where plantcode='".$plantcode."' and dallocs_id='$sid' and dallocss3_barcode='".$robc23['dallocss3_barcode']."' LIMIT 0,1") or die(mysqli_error($link));
								...
								else
								{
									...
									$nslnp=$rn2; $nslnq=$rq2;
									/*$sq41=mysqli_query($link,"Select * from tbl_dallocsub_sub4 where dalloc_id='$mainid' and dallocss4_wh='$txtwhg' and dallocss4_bin='$txtbing'") or die(mysqli_error($link));
...
else
{
	...
	if($mchksel!="")
	{
		...
		{
			$sql_subsub="update tbl_dalloc_sub set dallocs_noorders='$enoordno', dallocs_ordno='$eordno', dallocs_ordno='$eordno', dallocs_allocnop='$txtallonolp', dallocs_allocnomp='$txtallnomp', dallocs_allocqty='$txttobealqty' where dalloc_id='$mainid' and dallocs_id='$subtrid'";
		}	
		if(mysqli_query($link,$sql_subsub) or die(mysqli_error($link)))
		{
			if($subtrid==0)
			...
			if($allctyp=="barcodewise")
			{
				if($barcode!="" && $brflg==0)
				{
					...
					if($totbar1=mysqli_num_rows($sqlbar1) > 0)
					{
						...
						for($i=0; $i<$x-1; $i++)
						{
							...
							if($lotno<>"")	
							{
								...
								{
									$ssid=$subsubtrid;
									$rn29=0; $rq29=0; $rbn29=0; $rbq29=0;
									$sq29=mysqli_query($link,"Select * from tbl_dallocsub_sub where plantcode='".$plantcode."' and dallocss_id='$ssid'") or die(mysqli_error($link));
									if($to29=mysqli_num_rows($sq29) > 0)
									{
										while($ro29=mysqli_fetch_array($sq29))
										{
											$rn29=$rn29+$ro29['dallocss_nomp']; 
											$rq29=$rq29+$ro29['dallocss_qty']; 
										}
									}
									
									$rbn29=$rn29; $rbq29=$rq29;
									$sql_subsub2="update tbl_dallocsub_sub  set dallocss_altype='$allocationtype', dallocss_lotno='$txtolotno', dallocss_ups='$ups', dallocss_wtmp='$qty', dallocss_mptnop='$ltnps', dallocss_seltyp='$eseltyp', dallocss_qty='$rbq29', dallocss_nomp='$rbn29' where dallocss_id='$subsubtrid'";
									if(mysqli_query($link,$sql_subsub2) or die(mysqli_error($link)))
									{
										$ssid=$subsubtrid;
									}
								...
								//$sql_subsub2="insert into tbl_dallocsub_sub (dalloc_id, dallocs_id, dallocss_altype, dallocss_lotno, dallocss_ups, dallocss_wtmp, dallocss_mptnop, dallocss_seltyp, dallocss_qty) values ('$mainid', '$sid', '$allctyp', '$lotno', '$ups', '$qty', '$ltnps', '$eseltyp', '$qty')";
								//if(mysqli_query($link,$sql_subsub2) or die(mysqli_error($link)))
								//{
								//	$ssid=mysqli_insert_id($link);
								//}
					...
					$sqlb1="update tbl_mpmain set mpmain_alflg=2, mpmain_altrid='$mainid' where mpmain_barcode='".$barcode."'";
					$adcs=mysqli_query($link,$sqlb1) or die(mysqli_error($link));	
					...
					if($binshifting=="yes")
					{
						...
						$sqbc23=mysqli_query($link,"Select distinct dallocss3_barcode from tbl_dallocsub_sub3 where dallocs_id='$sid'") or die(mysqli_error($link));
						if($tobc23=mysqli_num_rows($sqbc23) > 0)
						{
							while($robc23=mysqli_fetch_array($sqbc23))
							{
								...
								if($tobc23=mysqli_num_rows($sqbc23) > 0)
								{
									...
									{
										$rn2=$rn2+1; 
										//$rqbc2=$rqbc2+$robc2['dallocss3_weight']; 
									}
								}
							}
						}
							
						$sq411=mysqli_query($link,"Select * from tbl_dallocsub_sub4 where dalloc_id='$mainid'") or die(mysqli_error($link));
						if(mysqli_num_rows($sq411)==0){$newsloc=1; $k=$tsln;} else {$k=$tsln+1;}
						if($newsloc!="")
						{
							//$k=$tsln+1;
							...
							if($txtbing>0)
							{
								$sql_sub4="Insert into tbl_dallocsub_sub4 (dalloc_id, dallocs_id, dallocss4_wh, dallocss4_bin, dallocss4_nomp, dallocss4_qty) values('$mainid', '$sid', '$txtwhg', '$txtbing', '$rn2', '$rq2')";
								$aweds=mysqli_query($link,$sql_sub4) or die(mysqli_error($link));
							}
						}
						else
...
<td width="714" colspan="3" align="left"  valign="middle" class="tbltext"><input type="radio" name="allctyp" class="tbltext" value="lotwise" onClick="alloctype(this.value);" />&nbsp;Pick to Allocate&nbsp;&nbsp;<input type="radio" name="allctyp" checked="checked" class="tbltext" value="barcodewise" onClick="alloctype(this.value);" />&nbsp;Scan to Allocate&nbsp;<font color="#FF0000" >*</font>&nbsp;<input type="hidden" name="allocationtype" value="barcodewise" /><input type="hidden" name="eseltyp" value="barsel" /></td></tr>
</table>
<br />
...
<!--<input type="text" name="maintrid" value="<?php echo $tid;?>" />
<input type="text" name="subtrid" value="<?php echo $subtid;?>" />
<input type="text" name="subsubtrid" value="<?php echo $subsubtid;?>" />-->
</div>
<table align="center" width="950" cellpadding="5" cellspacing="5" border="0" >
<tr >
