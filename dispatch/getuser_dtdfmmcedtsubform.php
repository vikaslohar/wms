...
<?php 
$sno1=1;
$sqq_sub=mysqli_query($link,"Select * from tbl_dtdf_sub where plantcode='".$plantcode."' and dtdf_id='$tid' and dtdfs_id='$subtid' and dtdfs_stage='Pack'") or die(mysqli_error($link));
...
$flg=0; $lotno=""; $nob=0; $qty=0; $totqty=0; $totnob=0; $crop=""; $variety=""; $qc=""; $dot=""; $sloc=""; $qcstsap='';
	...
	while($rowmonth=mysqli_fetch_array($sqlmonth))
	{
		...
		while($rowmonth3=mysqli_fetch_array($sqlmonth3))
		{
			...
			if($days<30)$flg++;
			//echo $lotn."  =  ".$roo['dbss_lotno']."  -  ".$dot."  -  ".$dov."  -  ".$flg."<br>";
			$vflg=0;
...
<tr class="Dark" height="30">
	...
	<td align="center"  valign="middle" class="smalltbltext"><?php if($mq>0) { ?><img border="0" src="../images/edit.png"  style="display:inline;cursor:pointer;" onclick="editrecsubmmc('<?php echo $sno1;?>','<?php echo $lotno;?>','<?php echo $subtid;?>','<?php echo $tid?>','<?php echo $lotups;?>','<?php echo $variety?>','<?php echo $bnlp?>','<?php echo $mmqty?>','<?php echo $roo['dbss_id']?>')" /><input type="hidden" name="selmmc" id="selmmc_<?php echo $sno1;?>" value="<?php echo $sno1;?>"  onclick="selctmmc(this.value,'<?php echo $lotno;?>','<?php echo $subtid;?>','<?php echo $tid?>','<?php echo $lotups;?>','<?php echo $variety?>','<?php echo $roo['dbss_nob']?>','<?php echo $mmqty?>','<?php echo $roo['dbss_id']?>')" /><?php } else {?><input type="checkbox" name="selmmc" id="selmmc_<?php echo $sno1;?>" value="<?php echo $sno1;?>"  onclick="selctmmc(this.value,'<?php echo $lotno;?>','<?php echo $subtid;?>','<?php echo $tid?>','<?php echo $lotups;?>','<?php echo $variety?>','<?php echo $roo['dbss_nob']?>','<?php echo $mmqty?>','<?php echo $roo['dbss_id']?>')" /><?php } ?></td>
