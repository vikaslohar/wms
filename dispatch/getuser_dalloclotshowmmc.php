<?php
	...
	if($ltnno!="")
	{
		...
		if($ltn<>"")
		{
			...
			$sq3=mysqli_query($link,"Select * from tbl_dallocsub_sub where plantcode='".$plantcode."' and dallocs_id='$subtid' and dalloc_id='$tid' and dallocss_lotno='$ltn' and dallocss_nolp>0 and dallocss_id='".$ssids[$i]."'") or die(mysqli_error($link));
			else
			$sq3=mysqli_query($link,"Select * from tbl_dallocsub_sub where plantcode='".$plantcode."' and dallocs_id='$subtid' and dalloc_id='$tid' and dallocss_lotno='$ltn' and dallocss_nolp>0") or die(mysqli_error($link));
			...
			while($row3=mysqli_fetch_array($sq3))
			{
				...
				$qt=$ptp*$row3['dallocss_nolp'];
				$mmqty=$mmqty+$qt;
				$tnb=$tnb+$row3['dallocss_nolp'];
				$ssid=$row3['dallocss_id'];
			}
	
	$mq=0;	 $bnlp=0; $tq=0;
	$sqmmc1=mysqli_query($link,"Select * from tbl_dallocmmc where plantcode='".$plantcode."' and dalloc_id='$tid' and dallocs_id='$subtid' and dmmc_lotno='$ltn' and dmmc_flg=2") or die(mysqli_error($link));
...
//echo "select * from tbl_dallocsub_sub2 where dallocss2_lotno='".$ltn."' and dalloc_id='$tid' and dallocs_id='$subtid' and dallocss_id='$ssid'";	
$sql_dlsub_sub=mysqli_query($link,"select * from tbl_dallocsub_sub2 where plantcode='".$plantcode."' and dallocss2_lotno='".$ltn."' and dalloc_id='$tid' and dallocs_id='$subtid' and dallocss_id='".$ssid."'") or die(mysqli_error($link));
...
$qt=$ptp*$row_dllsub_sub['dallocss2_nolp'];
$tnob=$row_dllsub_sub['dallocss2_nolp']; 
$tqty=$qt; 
