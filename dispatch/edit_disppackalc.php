<?php
	...
	$sql_subsub5="update tbl_disp_sub set disps_flg=2 where disps_flg=0 and disps_id='$pid'";
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($trsbmval==0)
		{
			...
			for($j=1; $j<$sn; $j++)
			{
				...
				if($to=mysqli_num_rows($sq) > 0)
				{
					...
					$sbid=$ro['disps_id'];	
					$sqlsubsub="update tbl_disp_sub set disps_nvariety='$etxtpvariety' where disps_id='$sbid'";
		...
		{
			$sql_main="update tbl_disp set disp_tflg='0' where disp_id='$p_id'";
			$as=mysqli_query($link,$sql_main) or die(mysqli_error($link));
			echo "<script>window.location='home_disppackalc.php'</script>";	
		}
	}
...
<title>Dispatch - Transaction - Dispatch - Allocation Type</title>
...
{ 
	...
	{
		window.location='home_disppackalc.php';
		return true;
	} 	 
}
...
function chkbarcode1(mltval)
{
	...
	else
	{
		...
		if(flg==0)
		{
			...
			var mchksel=document.frmaddDepartment.txtstfp.value;
			var upstyp='ST,NST';
...
function showpmode(mltval)
{
	...
	else
	{
		...
		else
		{
			...
			{
				alert("Barcode cannot be Dispatched.\n\nReason: Barcode not Allocated to Selected Party");
			}
...
function selitm(sno,ver,ups,qt,ordn)
{
	//alert(sno);
	if(document.frmaddDepartment.txt11.value=="")
	{
	...
	//else
	{
		...
		var trid=document.frmaddDepartment.maintrid.value;
		showUser(party,'orderdetails','showordsel',sno,ordn,ver,ups,qt,trid)
		//document.frmaddDepartment.barcode.focus();
	}
}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" style="border-bottom:solid; border-bottom-color:#378b8b" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Dispatch - Allocation Type</td>
	    </tr></table></td>
...
<form id="mainform" name="frmaddDepartment" method="post" action="<?php $_SERVER['PHP_SELF']; ?>" onSubmit="return mySubmit();" enctype="multipart/form-data"   > 
	 <input name="frm_action" value="submit" type="hidden"> 
		...
		<input type="hidden" name="txtid" value="<?php echo $row_tbl['disp_tcode']?>" />
		<input type="hidden" name="logid" value="<?php echo $row_tbl['disp_logid']?>" />
		<input type="hidden" name="getdetflg" value="0" />
		<input type="hidden" name="txtconchk" value="" />
		<input type="hidden" name="txtptype" value="<?php echo $row_tbl['disp_partytype']; ?>" />
		<input type="hidden" name="txtcountrysl" value="<?php echo $row_tbl['disp_location']; ?>" />
		<input type="hidden" name="txtcountryl" value="<?php echo $row_tbl['disp_location']; ?>" />
...
$nups=""; $nnob=0; $nqty=0; $nbqty=$qt;  $dbsflg=0; $totrec=0; $barcds="";  $newvariet="";
...
//echo "Select * from tbl_dalloc_sub where plantcode='".$plantcode."' and dalloc_id='$dalcid' and dallocs_ups='$up1' and dallocs_variety='$variet' and dallocs_upstype='$upstyp' and dallocs_allocqty>0 order by dallocs_variety ASC"."<br />";
...
//}
?>
<input type="hidden" name="mchksel" value="<?php echo $sn24?>" /><input type="hidden" name="txtornos" value="<?php echo $ordnos;?>" /><input type="hidden" name="txtveridno" value="<?php echo $veridno;?>" /><input type="hidden" name="txtupsnos" value="<?php echo $upsnos;?>" /><input type="hidden" name="txteqty" value="<?php echo $qt;?>" />
<input type="hidden" name="sn" value="<?php echo $sn;?>" /><input type="hidden" name="totbarcs" value="<?php echo $totbarcs;?>" />
<!--<input type="hidden" name="mchksel" value="" /><input type="hidden" name="txtornos" value="" /><input type="hidden" name="txtveridno" value="" /><input type="hidden" name="txtupsnos" value="" /><input type="hidden" name="txteqty" value="" />-->
</table>
</div>	
<br />

<div id="postingsubtable" style="display:block">
<div id="barupdetails" ></div><br />
...
<td width="230"  align="right"  valign="middle" class="smalltblheading">Scan Barcode&nbsp;</td>
...
<td width="656" align="left"  valign="middle" class="smalltbltext">&nbsp;<input type="text" name="txtremarks1" class="smalltbltext" size="100" maxlength="90" value="<?php echo $row_tbl['disp_remarks'];?>" ></td>
</tr>
</table>
