...
<td colspan="3" align="left" valign="middle" class="smalltbltext" >&nbsp;<?php echo $row['pname'];?></td>
</tr>
<?php
}
}
...
<br />
<?php
$sql_arr_home=mysqli_query($link,"select * from tbl_disp_sub where disp_id='$arrival_id' and disps_flg=1 order by disps_id asc") or die(mysqli_error($link));
...
{ 
	...
	{
		$tnob=$tnob+$row_arr_home['disps_nomp'];
		$tqty=$tqty+$row_arr_home['disps_qty'];
	}
}
$sql_arr_home2=mysqli_query($link,"select * from tbl_dispsub_sub where disp_id='$arrival_id' order by dpss_id asc") or die(mysqli_error($link));
