...
{	
	...
	val7=document.from1.pcode.value;
	val8=document.from1.stcode2.value;
	var tid=document.from1.tid.value;
	var txtlot1=val7+val2+val5+"/"+val6+"/"+val8;
	window.location='getuser_proslip_lotno.php?crop='+val3+'&variety='+val4+'&sqq='+txtlot1+'&tid='+tid;
	return true;	
	}
}	
...
?>
  ...
  isNumberKey(event)"  onchange="lot2chk();"  /> <font size="+1"><b>/</b></font> <input name="stcode" type="text" size="4" class="tbltext" tabindex="0" maxlength="5" onkeypress="return isNumberKey(event)"  value="00000" onchange="slocshow();" /> <font size="+1"><b>/</b></font> <input name="stcode2" type="text" size="2" class="tbltext" tabindex="0" maxlength="2" onkeypress="return isNumberKey(event)"  value="00"  />
	  &nbsp;<font color="#FF0000">*</font>&nbsp;&nbsp;<img src="../images/search.gif" border="0"  class="tbltext" name="sub" value="Search" style="display:inline;cursor:pointer; vertical-align:middle" onclick="lotchk();" /><!--<input type="image" src="../images/search.gif" border="0"  class="tbltext" name="sub" value="Search" style="display:inline;cursor:pointer; vertical-align:middle" onclick="lotchk();"> -->
    </form>
 </td></tr></table>
   <tr>
  <td valign="top">
...
$arrlots="";
$sql_tbl_sub=mysqli_query($link,"select * from tbl_discard_sub where plantcode='".$plantcode."' and  crop='".$crop."' and variety='".$variety."' and did_s='$tid'") or die(mysqli_error($link));
...
if($tot_arrsub > 0)
{
	while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
	{
		if($arrlots!="")
		{
			$arrlots=$arrlots.",".$row_tbl_sub['lotnumber'];
		}
		else
		{
			  $arrlots=$row_tbl_sub['lotnumber'];
		}
	}
}	


$qr="select distinct(orlot) from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and (lotldg_qc='Fail' or lotldg_got='Fail') $sss ";
...
{
$ccnt=0; $flg=0;
$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_binid, lotldg_whid from tbl_lot_ldg where  plantcode='".$plantcode."' and  orlot='".$row['orlot']."' and (lotldg_qc='Fail' or lotldg_got='Fail') group by lotldg_subbinid, lotldg_binid order by lotldg_id asc") or die(mysqli_error($link));
$xt=mysqli_num_rows($sql_is);
while($row_is=mysqli_fetch_array($sql_is))
{ 
$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_whid='".$row_is['lotldg_whid']."' and orlot='".$row['orlot']."' and (lotldg_qc='Fail' or lotldg_got='Fail') order by lotldg_id asc ") or die(mysqli_error($link));
$row_is1=mysqli_fetch_array($sql_is1); 
//echo $row_is1[0]."  ";
$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where plantcode='".$plantcode."' and  lotldg_id='".$row_is1[0]."' and lotldg_balqty > 0 and (lotldg_qc='Fail' or lotldg_got='Fail') order by lotldg_id asc") or die(mysqli_error($link)); 
...
//echo $row['orlot']." - ".$ccnt."<br>";
$parray=explode(",", $arrlots);
if(in_array($row['orlot'],$parray))
{
	//if($ccnt==0)
	$flg++;
}
...
<td align="right"  valign="middle" class="tbltext"><input type="radio" name="loc" value="<?php echo $row['orlot'];?>" onclick="clk(this.value,'<?php echo $row['orlot'];?>');" />&nbsp;</td>
