...
//exit;
	...
	$bartyp="NLC";
	if($row_batype['mpmain_trtype']=="PACKLMC" || $row_batype['mpmain_trtype']=="PACKNLC")
		$lotsnop=$row_batype['mpmain_lotnop'];
	if($row_batype['mpmain_trtype']=="PACKSMC" || $row_batype['mpmain_trtype']=="PACKNMC")
				...
				//exit;
					if(mysqli_query($link,$sql_sub) or die(mysqli_error($link)))
					{
						...
						if($barcode!="")
						{
							...
							$sql_lotqty=mysqli_query($link,"SELECT * FROM tbl_barcodes where plantcode='".$plantcode."' and  bar_barcode='".$barcode."'") or die(mysqli_error($link));
							...
							$sql_barcode="update tbl_barcodes set bar_dispflg=2 where bar_barcode='".$barcode."' ";
		...
		$row_lotchk=mysqli_fetch_array($sql_lotchk);
			if($num_lotchk>0)
			{
				...
				if($barcode!="")
				{
					...
					$sql_lotqty=mysqli_query($link,"SELECT * FROM tbl_barcodes where plantcode='".$plantcode."' and  bar_barcode='".$barcode."'");
					$row_lotqty=mysqli_fetch_array($sql_lotqty);
					$grswt1=$row_lotqty['bar_grosswt'];
