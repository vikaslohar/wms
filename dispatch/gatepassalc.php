...
$arrival_id=$row['disp_id'];
$ptype=$row['disp_partytype'];

 
$sql23="select * from tbluser where scode='".$row['disp_logid']."'";
...
$sql_code1="SELECT * FROM tbl_gatepass where trid='$tid' and trtype='Dispatch Pack Seed'";
$res_code1=mysqli_query($link,$sql_code1)or die(mysqli_error($link));
$row_code1=mysqli_fetch_array($res_code1);

$perticulars="Dispatch Pack Seed";
...
$sql_code12="SELECT * FROM tbl_dispnote where dnote_trid='$tid' and dnote_trtype='Dispatch Pack Seed' and dnote_ptype='$ptype'";
...
if($ptype=="C&F" || $ptype=="Branch")
{
	...
	$ntyps="STDN";
	$code=$row_param['code']."/"."ST"."/".$row_code12['dnote_yearcode']."/".$row_code12['dnote_code'];
}

//$code=$row_param['code'].$ycode."/"."SD"."/".$row['disp_ncode'];
...
//if($row['pid']=="Yes")
//{
	$quer3=mysqli_query($link,"SELECT * FROM tbl_partymaser where p_id='".$row['disp_party']."'"); 
...
<td align="left"  valign="middle" class="smalltbltext" colspan="3" id="vaddress"><div align="justify" class="smalltbltext" style="padding:2px 5px 5px 5px"><?php echo $address;?></div></td>
</tr>
<?php
$txt11=$row['tmode'];
if($txt11!= "") 
...
<br />
<?php
$sql_arr_home=mysqli_query($link,"select distinct dpss_barcode from tbl_dispsub_sub where disp_id='$arrival_id' order by dpss_barcode asc") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
$cnt=0; $tqty=0; $tnob=0;$tgrqty=0;
if($tot_arr_home >0) 
{ 
	while($row_arr_home=mysqli_fetch_array($sql_arr_home))
	{	$b=0;
		$sql_arr_home2=mysqli_query($link,"select * from tbl_dispsub_sub where disp_id='$arrival_id' and dpss_barcode='".$row_arr_home['dpss_barcode']."'") or die(mysqli_error($link));
		...
		{ 
			while($row_arr_home2=mysqli_fetch_array($sql_arr_home2))
			{
				...
				$tqty=$tqty+$row_arr_home2['dpss_qty'];
				$b++;
			}
		}
	}
...
<tr class="Light">
		 ...
		 <td align="center" valign="middle" class="smalltblheading"><?php echo $tgrqty;?>&nbsp;Kgs.</td>
   </tr>
</table>
<br />
