<?php
	...
	$dh="Periodical_Dispatch_Report_DC_wise_Period_From_".$sdate."_To_".$edate;
	$datahead = array("Periodical Dispatch Report - DC wise");
	$filename=$dh.".xls";  
	...
	$cnt=0;
	$datahead3= array("#","Dispatch Date","DC No.","Party Name","Location","State","Dispatch Type","Transport Name","Vehicle No.","LR No.","Qty","NoMP","Gross Weight");
...
$d=1; 
	...
	if($t > 0)
	{
		while($rowdispm=mysqli_fetch_array($sql_istbl))
		{
			...
			$quer3=mysqli_query($link,"select * from tblfnyears where plantcode='".$plantcode."' and years_flg != 0 and years_status='a'"); 
...
if($nqty>0)						
{	
$totnqty=$totnqty+$nqty; $totnomp=$totnomp+$nomp; $totgrtotal=$totgrtotal+$grwt;		
$data1[$d]=array($d,$trdate,$dcno,$bname,$location,$state,$disptype,$trasname,$vehno,$lrno,$nqty,$nomp,$grwt);
$d++;$cnt++;
}
}
}
...
if($nqty>0)						
{							
$totnqty=$totnqty+$nqty; $totnomp=$totnomp+$nomp; $totgrtotal=$totgrtotal+0;
$data1[$d]=array($d,$trdate,$dcno,$bname,$location,$state,$disptype,$trasname,$vehno,$lrno,$nqty,$nomp,$grwt);	
//$data1[$d]=array($d,$trdate,$dcno,$bname,$location,$state,$disptype,$nqty,$nomp);
$d++;$cnt++;
}
}
}
...
$totnqty=$totnqty+$nqty; $totnomp=$totnomp+$nomp; $totgrtotal=$totgrtotal+$grwt;
$data1[$d]=array($d,$trdate,$dcno,$bname,$location,$state,$disptype,$trasname,$vehno,$lrno,$nqty,$nomp,$grwt);				
//$data1[$d]=array($d,$trdate,$dcno,$bname,$location,$state,$disptype,$nqt,$nomp);
$d++;$cnt++;
}
}
}
...
if($totnqty>0)
{
$data1[$d]=array('','','','','','','Grand Total',$totnqty,$totnomp);
}
if($cnt==0)
{
