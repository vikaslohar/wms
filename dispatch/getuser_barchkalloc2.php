<?php
	...
	$sqlbarcode5=mysqli_query($link,"Select mpmain_barcode from tbl_mpmain where plantcode='".$plantcode."' and  mpmain_barcode='".$b."' and mpmain_alflg!=0") or die(mysqli_error($link));
	...
	if($eupstpy!="")
	{
		...
		//if($eupstpy!=$utpy){if($flg==0)$flg=5;  echo " UPS type not matching";}
	}
	...
	if($tot_barcode1>0)
	{
		...
		/*if($ltnno!="")
		{
			if($ltnno!=$row_barcode1['mpmain_lotno'])
			$flg=12;
		}*/
		/*$brltn=$row_barcode1['mpmain_lotno'].",";
		$barltno=explode(",",$brltn);
		if(!in_array($ltnno,$barltno))
		$flg=12;*/
	}
	...
	if($eloqty>$etbloqtyy){if($flg==0)$flg=12;}
