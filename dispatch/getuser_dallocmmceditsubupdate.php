	...
	if($mchksel!="")
	{
		for($i=1; $i<=$snlo; $i++)
		{
			...
			for($k=1; $k<=$srno2; $k++)
			{
				...
				$sq=mysqli_query($link,"Select * from tbl_dallocsub_sub2 where plantcode='".$plantcode."' and dalloc_id='$mainid' and dallocs_id='$subtrid' and dallocss2_lotno='$txtolotno' and dallocss2_subbin='$extslsubbg'") or die(mysqli_error($link));
				$ro=mysqli_fetch_array($sq);
				$ssid=$ro['dallocss_id'];
				
				$s_sub="delete from tbl_dallocmmc where plantcode='".$plantcode."' and dalloc_id='".$mainid."' and dallocss_id='".$ssid."' and dallocs_id='".$subtrid."' and dmmc_lotno='".$txtolotno."' and dmmc_flg=2";
