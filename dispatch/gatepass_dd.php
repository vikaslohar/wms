...
<title>Dispatch - Transction-Gate Pass</title>
...
<body>
	  ...
	  <?php 
	$sql1=mysqli_query($link,"SELECT * FROM tbl_discard  where tid=$pid")or die(mysqli_error($link));
...
$sql_gate=mysqli_query($link,"select * from tbl_gate where trid='".$pid."' and trtype='Discard'") or die (mysqli_error($link));
...
<td width="178"  align="left" valign="middle" class="smalltbltext">&nbsp;<?php echo "MD".$row['dd_code']."/".$row['yearcode']."/".$gpcode;?></td>
</tr>	
		  <tr class="Dark" >
...
<table width="750" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#378b8b" style="border-collapse:collapse"> 
...
<?php
$sr=1; $totnob=0; $totqty=0;
$sql_eindent_sub=mysqli_query($link,"select * from tbl_discard_sub where did_s=$pid") or die(mysqli_error($link));
...
$tto=0;
$sql_veriety=mysqli_query($link,"select varietyid, popularname from tblvariety where varietyid='".$row_eindent_sub['variety']."'") or die(mysqli_error($link));
...
if($trid > 0)
{
$sql_tblissue=mysqli_query($link,"select * from tbl_discard_sloc where discard_trid='".$trid."' and discard_id='".$row_eindent_sub['did']."' and crop='".$row_eindent_sub['crop']."' and variety='".$row_eindent_sub['variety']."'") or die(mysqli_error($link));
...
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where whid='".$row_tblissue['whid']."'") or die(mysqli_error($link));
...
$sql_stldg1=mysqli_query($link,"select * from tbl_lot_ldg where  lotldg_id='".$row_tblissue['discard_rowid']."'") or die(mysqli_error($link));
