...
<?php if($eseltyp=="barsel"){?>
	<td width="277" align="center" class="smalltblheading">Barcode</td>
<?php } ?>	
...
<?php if($eseltyp=="barsel"){?>
	<td align="center" valign="middle" class="tbltext"><?php echo $evert;?><input type="hidden" name="txtbarcodee" id="txtbarcodee" value="<?php echo $evert;?>" /></td>
<?php } ?>
...
<input type="hidden" name="eseltyp" value="<?php echo $eseltyp;?>" />
</table>
<?php 
$srno2=0;
if($eseltyp=="lotsel")
...
{ 
 ...
 { 
	...
	//echo $lotno."  =  ".$bqty1."  =  ".$nmp." => ".$totextpouches."  =  ".$tnob."  =  ".$tqty."<br/>";
	//$tnob=$bnob;
	//$avlqty=$nop1*$ptp1;
	
	if($tnob>$tnb)$tnob=$tnb;
	if($nmp==0)$tnob=0;
	//$totnob=$totnomp;
	//$totqty=$avlqty;
	...
	if($row_issuetbl['lotldg_got']=="BL")$fgl=1;
	if($row_issuetbl['lotldg_qc']=="BL")$fgl=1;
	if($zz[0]=="GOT-R")
	{
		...
		if(($row_issuetbl['lotldg_got']=="UT" || $row_issuetbl['lotldg_got']=="RT") && $row_issuetbl['lotldg_srflg']==0)
		{
			$fgl=1;
		}
	}
	if(($row_issuetbl['lotldg_qc']=="UT" || $row_issuetbl['lotldg_qc']=="RT") && $row_issuetbl['lotldg_srflg']==0)
	{
		$fgl=1;
	}
	if($row_issuetbl['lotldg_srflg']!=0 && $row_issuetbl['lotldg_srtyp']!='dispatch')
	{
		$fgl=1;
	}
...
?>
 <tr class="Light" height="30">
    <td align="center"  valign="middle" class="smalltbltext" colspan="8" style="color:#FF0000" >Lot cannot Allocated: 1. Check QC/GOT.  2. Check SR/SSR.<input name="recnobp<?php echo $srno2?>" id="recnobp<?php echo $srno2?>" type="hidden" size="7" class="smalltbltext" tabindex=""   maxlength="7" onkeypress="return isNumberKey1(event)" onchange="qtychk1(this.value,<?php echo $srno2?>,'add');" value=""  /><input name="recqtyp<?php echo $srno2?>" id="recqtyp<?php echo $srno2?>" type="hidden" size="8" class="smalltbltext" tabindex="" maxlength="10" onchange="Bagschk1(this.value,<?php echo $srno2?>,'add');"  onkeypress="return isNumberKey(event)"  value="" /><input name="txtbalnobp<?php echo $srno2?>" id="txtbalnobp<?php echo $srno2?>" type="hidden" size="7" class="smalltbltext" tabindex="" maxlength="7" onkeypress="return isNumberKey(event)"  value="" style="background-color:#CCCCCC" readonly="true" /><input name="txtbalqtyp<?php echo $srno2?>" id="txtbalqtyp<?php echo $srno2?>" type="hidden" size="8" class="smalltbltext" tabindex=""   maxlength="10" onkeypress="return isNumberKey1(event)" style="background-color:#CCCCCC" readonly="true" value="" /></td>
</tr>
<?php
}
}
...
<?php
	$tot4=0; 
	//$sq4=mysqli_query($link,"Select * from tbl_dallocsub_sub4 where dalloc_id='$tid'") or die(mysqli_error($link));
	//$tot4=mysqli_num_rows($sq4);
?>
<table align="center" border="1" width="950" cellspacing="0" cellpadding="0" bordercolor="#378b8b" style="border-collapse:collapse" > 
<tr class="Light" height="20">
  <td width="50%" align="right" class="tblheading">Bin Shifting&nbsp;</td>
  <td align="left" valign="middle" class="tblheading"><input type="radio" name="binshift" value="yes" onclick="selnslsts(this.value);"<?php if($eseltyp=="barsel")echo "checked";?> />&nbsp;Yes&nbsp;&nbsp;<input type="radio" name="binshift" value="no" onclick="selnslsts(this.value);" <?php if($eseltyp=="barsel")echo "disabled";?> />&nbsp;No&nbsp;<font color="#FF0000">*</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span id="barcupload" ></span><input type="hidden" name="bcvalues" value="<?php if($eseltyp=="barsel")echo "yes";?>" /></td>
</tr>
...
<div id="shownsloc" style="display:<?php if($eseltyp=="barsel")echo "block"; else if($tot4>0) echo "block"; else echo "none";?>">
<table align="center" border="1" width="950" cellspacing="0" cellpadding="0" bordercolor="#378b8b" style="border-collapse:collapse" > 
<tr class="tblsubtitle" height="20">
...
<tr >
<td valign="top" align="right" id="frmbutn"><img src="../images/post.gif" border="0"style="display:inline;cursor:Pointer;" onClick="pform();" />&nbsp;&nbsp;</td>
</tr>
</table>