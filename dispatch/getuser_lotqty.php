...
<tr class="tblsubtitle" height="20">
  <td colspan="14" align="center" class="tblheading">Pick To Allocate</td>
</tr>
...
<?php
$sno=1; $crop=""; $variety=""; $lotno=""; $ui1="";
$sql_allocs=mysqli_query($link,"Select * from tbl_dalloc_sub where plantcode='".$plantcode."' and   dallocs_id='".$sid."'") or die(mysqli_error($link));
$tot_allocs=mysqli_num_rows($sql_allocs);
$row_allocs=mysqli_fetch_array($sql_allocs);

$crop=$row_allocs['dallocs_crop'];
$variety=$row_allocs['dallocs_variety'];
$ui1=$row_allocs['dallocs_ups'];

$sql_allocss=mysqli_query($link,"Select distinct dallocss_lotno from tbl_dallocsub_sub where plantcode='".$plantcode."' and   dallocs_id='".$sid."' and dallocss_altype='lotwise'") or die(mysqli_error($link));
$tot_allocss=mysqli_num_rows($sql_allocss);
while($row_allocss=mysqli_fetch_array($sql_allocss))
{
	$qty=0; $nomp=0;
	$sql_allocss1=mysqli_query($link,"Select * from tbl_dallocsub_sub where plantcode='".$plantcode."' and   dallocss_lotno='".$row_allocss['dallocss_lotno']."' and dallocs_id='".$sid."' and dallocss_altype='lotwise'") or die(mysqli_error($link));
	$tot_allocss1=mysqli_num_rows($sql_allocss1);
	while($row_allocss1=mysqli_fetch_array($sql_allocss1))
	{
		$qty=$qty+$row_allocss1['dallocss_qty'];
		$nomp=$nomp+$row_allocss1['dallocss_nomp'];
	}
	$lotno=$row_allocss['dallocss_lotno'];
	
	$sql_lot=mysqli_query($link,"Select * from tbl_lot_ldg_pack where plantcode='".$plantcode."' and   lotno='".$lotno."' order by lotdgp_id desc") or die(mysqli_error($link));
...
<table align="center" border="1" width="750" cellspacing="0" cellpadding="0" bordercolor="#378b8b" style="border-collapse:collapse" > 
<tr class="tblsubtitle" height="20">
  <td colspan="14" align="center" class="tblheading">Scan To Allocate</td>
</tr>
<tr class="Light" height="25">
	<td width="19" align="center" class="smalltblheading">#</td>
	<td width="73" align="center" class="smalltblheading">Barcode</td>
...
<?php
$sno=1; $ssid=""; $barcodes="";
$sql_allocss=mysqli_query($link,"Select * from tbl_dallocsub_sub where plantcode='".$plantcode."' and   dallocs_id='".$sid."' and dallocss_altype='barcodewise'") or die(mysqli_error($link));
$tot_allocss=mysqli_num_rows($sql_allocss);
while($row_allocss=mysqli_fetch_array($sql_allocss))
{
	if($ssid!="")
		$ssid=$ssid.",".$row_allocss['dallocss_id'];
	else
		$ssid=$row_allocss['dallocss_id'];
}
//echo $ssid;
if($ssid!="")
{//echo count($ssid);
	if(count(explode(",",$ssid))>1)
		$sql_allocss3=mysqli_query($link,"Select distinct dallocss3_barcode from tbl_dallocsub_sub3 where plantcode='".$plantcode."' and   dallocss_id in ($ssid)") or die(mysqli_error($link));
	else
		$sql_allocss3=mysqli_query($link,"Select distinct dallocss3_barcode from tbl_dallocsub_sub3 where plantcode='".$plantcode."' and   dallocss_id='$ssid'") or die(mysqli_error($link));

	$tot_allocss3=mysqli_num_rows($sql_allocss3);
	while($row_allocss3=mysqli_fetch_array($sql_allocss3))
	{
		if($barcodes!="")
			$barcodes=$barcodes.",".$row_allocss3['dallocss3_barcode'];
		else
			$barcodes=$row_allocss3['dallocss3_barcode'];
	}
}
if($barcodes!="")
{
...
if($totbarc1>0)
{
	...
	if($pty=="PACKSMC" || $pty=="PACKNMC")
	{
		...
		$sql_crp=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropid='".$rowbarc1['mpmain_crop']."' order by cropname Asc"); 
