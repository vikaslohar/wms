...
<title>Dispatch - Transaction - Pack Seed SP Release-RO</title>
...
function getdetails()
{
	...
	else
	{
		...
		showUser(get,'postingsubtable','get',crop,variety,tid,lotid,ups,'');
...
function qtychk1(qtyval1,val)
{
	...
	{	
		...
		if(parseFloat(qtb)>0)
		{
			alert("NoP can only be equal to 0");
...
function barcodupd(ltn,dval2)
{
	var totnomp=document.frmaddDepartment.rtotalups.value;
	var tid=document.frmaddDepartment.maintrid.value;
	var subtid=document.frmaddDepartment.subtrid.value;
	var lotno=document.frmaddDepartment.txtlot1.value;
	var txtpsrn=document.frmaddDepartment.txtdcno.value;
	var dval=1;
	...
	{
		var nmp="txtrecnomp_"+i;
		var blnmp="txtdnomp_"+i;
		
		tnmp=parseInt(tnmp)+parseInt(document.getElementById(nmp).value);
		bnmp=parseInt(bnmp)+parseInt(document.getElementById(blnmp).value);
	}
	if(parseInt(tnmp)!="" && parseInt(tnmp) > 0 && parseInt(bnmp)!="" && parseInt(bnmp)>0)
	{
		if(dval2=="edit")
		{ 
			winHandle=window.open('getuser_pronpslip_barcode3_new.php?totnomp='+bnmp+'&tid='+tid+'&lotno='+lotno+'&txtpsrn='+txtpsrn+'&subtid='+subtid+'&dval='+dval,'WelCome','top=170,left=180,width=450,height=450,scrollbars=yes');
			if(winHandle==null){
			alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); }
		}
		else
		{
			winHandle=window.open('getuser_pronpslip_barcode_new.php?totnomp='+bnmp+'&tid='+tid+'&lotno='+lotno+'&txtpsrn='+txtpsrn+'&subtid='+subtid+'&dval='+dval,'WelCome','top=170,left=180,width=450,height=450,scrollbars=yes');
			if(winHandle==null){
			alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); }
		}
	}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" style="border-bottom:solid; border-bottom-color:#378b8b" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Pack Seed SP Release-RO</td>
	    </tr></table></td>
...
<td width="229" align="left" valign="middle" class="smalltbltext">&nbsp;<input name="txtdcno" type="text" size="20" class="smalltbltext" maxlength="20" onChange="dcdchk();" tabindex="0"/>&nbsp;<font color="#FF0000">*</font>&nbsp;</td>
</tr>
<tr class="Light" height="30">
