<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($trsbmval==0)
		{
			...
			echo "<script>window.location='add_disppack_preview.php?pid=$p_id'</script>";	
			//exit;
		}
		else
		{
			...
			$as=mysqli_query($link,$sql_main) or die(mysqli_error($link));
			echo "<script>window.location='home_disppack.php'</script>";	
		}
	}
...
<title>Dispatch - Transaction - Dispatch - Direct Loading / Non-Allocation Type</title>
...
{ 
	...
	{
		window.location='home_disppack.php';
		return true;
	} 	 
}
...
function showpmode(mltval)
{
	...
	else
	{
		...
		else
		{
			...
			{
				alert("Barcode cannot be Dispatched.\n\nReason: Barcode not present within current Warehouse.");
			}*/
			pform();
			//showUser(bardet,'bardetails','ordrbar',mltval,trid,ver,ups,mchksel,brflg)
			//setTimeout(function(){document.frmaddDepartment.barcode.value=""; document.frmaddDepartment.barcode.focus();},400);
...
function nompchk()
{
	...
	{	
		...
		document.frmaddDepartment.txttlonomp.value=parseInt(document.frmaddDepartment.txttlonomp.value);
		document.frmaddDepartment.txtorblnomp.value=parseInt(document.frmaddDepartment.txtorblnomp.value);
	}
}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#378b8b" style="border-bottom:solid; border-bottom-color:#378b8b" >
	    <tr >
	      <td width="813" height="25" class="Mainheading">&nbsp;Transaction - Dispatch - Direct Loading / Non-Allocation Type</td>
	    </tr></table></td>
...
if($arrivalid!="")
{
//echo "select distinct order_sub_crop from tbl_order_sub where orderm_id in($arrivalid) and order_sub_totbal_qty>0 and order_sub_hold_flag=0 order by order_sub_crop";
...
//echo $arrivalid;
$cp="";
//echo "SELECT cropid, cropname FROM tblcrop where cropid IN ($cpr) order by cropname Asc";
if($cpr!="")
{
