...
while($rowsloc=mysqli_fetch_array($sqlsloc))
{
	...
	while($rowsloc2=mysqli_fetch_array($sqlsloc2))
	{
		...
		while($rowtblsub=mysqli_fetch_array($sqlmon))
		{
			...
			$sql_m=mysqli_query($link,"select * from tbl_orderm where plantcode='".$plantcode."' and  orderm_id='".$rowtblsub['orderm_id']."' and (order_trtype='Order Sales' OR order_trtype='Order Stock') and order_trtype!='Order TDF' and orderm_tflag=1")or die("Error:".mysqli_error($link));
...
?>
  <tr class="Dark" height="30">
    ...
    <td width="275" align="center"  valign="middle" class="smalltbltext" title="<?php echo $barcds ?>"><?php if($barcds!=""){?>
        <a href="Javascript:void(0);" onclick="showmbarcodes('<?php echo $barcds;?>')">Details</a>
      <?php } ?>
      <input type="hidden" name="rbarcodes<?php echo $sn;?>" id="rbarcodes_<?php echo $sn;?>" value="<?php echo $ordno;?>" /></td>
    ...
    <!--<td width="275" align="center"  valign="middle" class="smalltbltext"><input type="radio" name="selsh" id="selsh_<?php echo $sn;?>" value="<?php echo $sn;?>" onclick="selitm('<?php echo $sn;?>','<?php echo $variety?>','<?php echo $up1 ?>','<?php echo $qt?>','<?php echo $ordno?>');" /></td>-->
  </tr>
  <?php
$sn++;
}
...
?>
  ...
  <input type="hidden" name="txtornos" value="<?php echo $ordnos;?>" />
  <input type="hidden" name="txtveridno" value="<?php echo $veridno;?>" />
  <input type="hidden" name="txtupsnos" value="<?php echo $upsnos;?>" />
  <input type="hidden" name="txteqty" value="<?php echo $qt;?>" />
  <input type="hidden" name="mchksel" value="<?php echo $sn?>" />
