...
<tr class="tblsubtitle" height="20">
	...
	<td align="center" valign="middle" class="smalltblheading" colspan="6">OK</td>
...
// Raw Seed Records
$sql_arhome=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."'  group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
while($row_arhome=mysqli_fetch_array($sql_arhome))
{  
	$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crop."' and lotldg_lotno='".$row_arhome['lotldg_lotno']."' and lotldg_variety='".$variety."'  group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
	while($row_is=mysqli_fetch_array($sql_is))
	{ 
		$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_lotno='".$row_arhome['lotldg_lotno']."'  order by lotldg_id desc ") or die(mysqli_error($link));
