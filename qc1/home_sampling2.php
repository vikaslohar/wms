...
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	//exit;
		if($btnval==0)
		{
			echo "<script>window.location='add_new_request _tr.php?foccode=$qcs'</script>";
		}
		else
		{
			$tid=$_POST['dtid'];
...
<?php
 $sql_arr_home2=mysqli_query($link,"select distinct lotno from tbl_qctest where aflg=0 and bflg=1 and cflg=0 and gotflg=0 and gotsmpdflg=0 and got='UT' and qcstatus!='RT' order by sampleno desc") or die(mysqli_error($link));
...
{
$sql_arr_home3=mysqli_query($link,"select distinct sampleno from tbl_qctest where lotno='".$row_arr_home2['lotno']."' and aflg=0 and bflg=1 and cflg=0 and gotflg=0 and gotsmpdflg=0 and got='UT' and qcstatus!='RT' order by sampleno desc") or die(mysqli_error($link));
$tot_arr_home3=mysqli_num_rows($sql_arr_home3);
while($row_arr_home3=mysqli_fetch_array($sql_arr_home3))
{
$sql_arr_home4=mysqli_query($link,"select max(tid) from tbl_qctest where lotno='".$row_arr_home2['lotno']."' and sampleno='".$row_arr_home3['sampleno']."' and aflg=0 and bflg=1 and cflg=0 and gotflg=0 and gotsmpdflg=0 and got='UT' and qcstatus!='RT' order by sampleno desc") or die(mysqli_error($link));
$row_arr_home4=mysqli_fetch_array($sql_arr_home4);

$sql_arr_home=mysqli_query($link,"select * from tbl_qctest where lotno='".$row_arr_home2['lotno']."' and sampleno='".$row_arr_home3['sampleno']."' and tid='".$row_arr_home4[0]."' and aflg=0 and bflg=1 and cflg=0 and gotflg=0 and gotsmpdflg=0 and got='UT' and qcstatus!='RT' order by sampleno desc") or die(mysqli_error($link));
...
<tr class="Light">
         ...
         <td width="119" align="center" valign="middle" class="tblheading"><?php echo $lotno?></td>
