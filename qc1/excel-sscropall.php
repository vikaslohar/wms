<?php
	...
	$crp="ALL"; $ver="ALL"; $stage="ALL"; 	$cnt1=1;
	if($txtstage!="ALL")
	{
		if($txtstage=="Raw")
		{
			$qry="select Distinct lotldg_crop from tbl_lot_ldg where lotldg_id!=0 and lotldg_sstage='$txtstage' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and (lotldg_qc='Fail' OR lotldg_got='Fail')";
		}
		...
		{
			$qry2="select Distinct lotldg_crop from tbl_lot_ldg_pack where lotdgp_id!=0 and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate'";
		}
		else
		{
			$qry="select Distinct lotldg_crop from tbl_lot_ldg where lotldg_id!=0 and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate'";
			$qry2="select Distinct lotldg_crop from tbl_lot_ldg_pack where lotdgp_id!=0 and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate'";
		}
	}
	else
...
if($crval<>"")
{
	...
	if($txtstage!="ALL")
	{
		if($txtstage=="Raw")
		{
			$qry="select Distinct lotldg_variety from tbl_lot_ldg where lotldg_id!=0 and lotldg_sstage='$txtstage' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_crop='".$crval."'";
		}
		else if($txtstage=="Condition")
		{
			$qry="select Distinct lotldg_variety from tbl_lot_ldg where lotldg_id!=0 and lotldg_sstage='$txtstage' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_crop='".$crval."'";
		...
		{
			$qry2="select Distinct lotldg_variety from tbl_lot_ldg_pack where lotdgp_id!=0 and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and lotldg_crop='".$crval."'";
		}
		else
		{
	...
	{
		$qry="select Distinct lotldg_variety from tbl_lot_ldg where lotldg_id!=0 and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and lotldg_crop='".$crval."'";
		$qry2="select Distinct lotldg_variety from tbl_lot_ldg_pack where lotdgp_id!=0 and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and lotldg_crop='".$crval."'";
	}	
	...
	if($txtstage!="ALL")
	{
		if($txtstage!="Pack")
		{
			$sql_arhome=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_sstage='$txtstage' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate'  group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
			while($row_arhome=mysqli_fetch_array($sql_arhome))
			{  
				$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crval."' and lotldg_lotno='".$row_arhome['lotldg_lotno']."' and lotldg_variety='".$verval."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate'  group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
				while($row_is=mysqli_fetch_array($sql_is))
				{ 
					$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotldg_lotno='".$row_arhome['lotldg_lotno']."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate'  order by lotldg_id desc ") or die(mysqli_error($link));
					$row_is1=mysqli_fetch_array($sql_is1); 
					$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_balqty > 0 and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' order by lotldg_id asc") or die(mysqli_error($link)); 
		...
		{
			$sql_arhome=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and (lotldg_qc='Fail' OR lotldg_got='Fail')  and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' group by lotno order by lotdgp_id asc") or die(mysqli_error($link));
			
			while($row_arhome=mysqli_fetch_array($sql_arhome))
			{  
				$sql_is=mysqli_query($link,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and lotno='".$row_arhome['lotno']."' and lotldg_variety='".$verval."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' group by subbinid order by lotdgp_id asc") or die(mysqli_error($link));
				while($row_is=mysqli_fetch_array($sql_is))
				{ 
					$sql_is1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_is['subbinid']."' and binid='".$row_is['binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and lotno='".$row_arhome['lotno']."' and (lotldg_qc='Fail' OR lotldg_got='Fail')  and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' order by lotdgp_id desc ") or die(mysqli_error($link));
					$row_is1=mysqli_fetch_array($sql_is1); 
					$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and balqty>0 and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' order by lotdgp_id asc") or die(mysqli_error($link)); 
					$t=mysqli_num_rows($sql_istbl);
					if($t > 0)
					{
							$ccnt++;
	...
	{
		$sql_arhome=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
		while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_crop='".$crval."' and lotldg_lotno='".$row_arhome['lotldg_lotno']."' and lotldg_variety='".$verval."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate'  group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
			while($row_is=mysqli_fetch_array($sql_is))
			{ 
				$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_lotno='".$row_arhome['lotldg_lotno']."'  and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' order by lotldg_id desc ") or die(mysqli_error($link));
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_balqty > 0 and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' order by lotldg_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
...
if($ccnt>0)	
{
	...
	$srno=0; $totalbags=0; $totalqty=0; $d=1;
	...
	if($txtstage!="ALL")
	{
		if($txtstage!="Pack")
		{
			$sql_arr_home=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where  lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_sstage='$txtstage' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
			...
			{  
				...
				$sql_issue=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_lotno='".$row_arr_home['lotldg_lotno']."'  and lotldg_balqty > 0 and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and lotldg_sstage='$txtstage' group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
			
				while($row_issue=mysqli_fetch_array($sql_issue))
				{ 
					$txtdot=""; 
				
					$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_lotno='".$row_arr_home['lotldg_lotno']."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and lotldg_sstage='$txtstage' order by lotldg_id asc ") or die(mysqli_error($link));
					$row_issue1=mysqli_fetch_array($sql_issue1); 
				
					$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and lotldg_balqty > 0 and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and lotldg_sstage='$txtstage' order by lotldg_id asc") or die(mysqli_error($link)); 
		...
		else
		{
			...
			{  
				...
				{ 
					...
					$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and lotno='".$row_arr_home['lotno']."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' order by lotdgp_id asc ") or die(mysqli_error($link));
					$row_issue1=mysqli_fetch_array($sql_issue1); 
					
					$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg_pack where lotdgp_id='".$row_issue1[0]."' and balqty > 0 and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and (lotldg_qc='Fail' OR lotldg_got='Fail') order by lotdgp_id asc") or die(mysqli_error($link)); 
					$t=mysqli_num_rows($sql_issuetbl);
					if($t > 0)
					{
						while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
	...
	{
		$sql_arr_home=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where  lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' group by lotldg_lotno order by lotldg_id asc") or die(mysqli_error($link));
		while($row_arr_home=mysqli_fetch_array($sql_arr_home))
		{  
			$lotn=$row_arr_home['lotldg_lotno'];
			$wareh=""; $binn=""; $subbinn=""; $slups=0; $slqty=0;	 $cnt=0;
			$totqty=0; $totnob=0; $totqc=""; $totdot=""; $totmost=""; $totgemp=""; $totgot=""; $reserve=""; $totsst=""; $sloc=""; $genpp=""; $dogr=""; $stage="";
			$sql_issue=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_lotno='".$row_arr_home['lotldg_lotno']."'  and lotldg_balqty > 0 and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));
				
			while($row_issue=mysqli_fetch_array($sql_issue))
			{ 
				$txtdot=""; 
					
				$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_lotno='".$row_arr_home['lotldg_lotno']."' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and (lotldg_qc='Fail' OR lotldg_got='Fail') order by lotldg_id asc ") or die(mysqli_error($link));
				$row_issue1=mysqli_fetch_array($sql_issue1); 
					
				$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and lotldg_balqty > 0 and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and (lotldg_qc='Fail' OR lotldg_got='Fail') order by lotldg_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_issuetbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_issuetbl))
		...
		$sql_arr_home=mysqli_query($link,"select distinct lotno from tbl_lot_ldg_pack where lotldg_crop='".$crval."' and lotldg_variety='".$verval."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' group by lotno order by lotdgp_id asc") or die(mysqli_error($link));
		while($row_arr_home=mysqli_fetch_array($sql_arr_home))
		{  
			$lotn=$row_arr_home['lotno'];
			$wareh=""; $binn=""; $subbinn=""; $slups=0; $slqty=0;	 $cnt=0;
			$totqty=0; $totnob=0; $totqc=""; $totdot=""; $totmost=""; $totgemp=""; $totgot=""; $reserve=""; $totsst=""; $sloc=""; $genpp=""; $dogr=""; $stage="";
			$sql_issue=mysqli_query($link,"select distinct subbinid, whid, binid from tbl_lot_ldg_pack where lotno='".$row_arr_home['lotno']."' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and (lotldg_qc='Fail' OR lotldg_got='Fail') group by subbinid order by lotdgp_id asc") or die(mysqli_error($link));
				
			while($row_issue=mysqli_fetch_array($sql_issue))
			{ 
				$txtdot=""; 
					
				$sql_issue1=mysqli_query($link,"select max(lotdgp_id) from tbl_lot_ldg_pack where subbinid='".$row_issue['subbinid']."' and binid='".$row_issue['binid']."' and whid='".$row_issue['whid']."' and lotno='".$row_arr_home['lotno']."' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' and (lotldg_qc='Fail' OR lotldg_got='Fail') order by lotdgp_id asc ") or die(mysqli_error($link));
				$row_issue1=mysqli_fetch_array($sql_issue1); 
