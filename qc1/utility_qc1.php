...
$t=mysqli_num_rows($sql_ltn);
if($t==0)
{
$sql_ltn=mysqli_query($link,"select * from tbl_lot_ldg_pack where orlot='".$lotno."'") or die(mysqli_error($link));
$xx=mysqli_num_rows($sql_ltn);
}
...
if($xx==0 && $t==0)
{
$sql_ltn=mysqli_query($link,"select * from tbl_salesrv_sub where salesrs_orlot='".$lotno."'") or die(mysqli_error($link));
$xx=mysqli_num_rows($sql_ltn);
$row_lnt=mysqli_fetch_array($sql_ltn);

$quer2=mysqli_query($link,"SELECT  cropname,cropid FROM tblcrop where cropid='".$row_lnt['salesrs_crop']."'"); 
$row_dept=mysqli_fetch_array($quer2);
$crpname=$row_dept['cropname'];

$quer3=mysqli_query($link,"SELECT * FROM tblvariety where varietyid ='".$row_lnt['salesrs_variety']."'"); 
$rowvv=mysqli_fetch_array($quer3);
$vv=$rowvv['popularname'];

$crop=$row_lnt['salesrs_crop'];
$variety=$row_lnt['salesrs_variety'];
$xflg=1;
}
}
...
<tr class="light" height="25">   
	...
	<td align="right" width="50%" class="subheading" style="color:#303918;">GOT status on Arrival: <?php if($xflg==0)echo $gotatarr; else echo "GOT-NR OK";?>&nbsp;</td>
</tr>
</table><br />
...
rsort($dt1);
//print_r($dt1);
$sql_tbl2=mysqli_query($link,"select * from tbl_qctest where crop='".$crop."' and variety='$variety' and oldlot='$lot' group by sampleno,qcrefno order by tid desc") or die(mysqli_error($link));
...
<tr class="tblsubtitle" height="20">
  ...
  if($dval<>"")
  { 
  $sql_tbl=mysqli_query($link,"select *  from tbl_qctest where crop='".$crop."' and variety='".$variety."' and oldlot='".$lot."' and srdate='".$dval."' group by sampleno, qcrefno order by testdate desc, tid desc") or die(mysqli_error($link));
