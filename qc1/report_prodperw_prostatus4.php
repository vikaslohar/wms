...
<title>QC Supervisor - Report - Production Personnel wise Processing Status Report</title>
...
var pmc=document.frmaddDepartment.txtprodper.value;
winHandle=window.open('report_prodperw_prostatus5.php?sdate='+sdate+'&txtcrop='+itemid+'&txtvariety='+vv+'&edate='+edate+'&txtpmcode='+pmc+'&txtprodper='+txtprodper,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
...
?>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" style="border-bottom:solid; border-bottom-color:#d21704" >
	    <tr >
	      <td width="813" height="25">&nbsp;Production Personnel wise Processing Status Report - As on Date: <?php echo date("d-m-Y");?></td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
	  ...
	  <td align="center" colspan="4" >
		...
		<input name="txtpmcode" value="<?php echo $txtpmcode;?>" type="hidden">  
		 ...
		 <input name="txtprodper" value="<?php echo $_REQUEST['txtprodper'];?>" type="hidden">
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="970" style="border-collapse:collapse">
  ...
  <tr class="Dark" height="25" >
	<!-- <td align="left" width="40%" class="subheading" style="color:#303918; ">&nbsp;Production Personnel: <?php echo $txtprodper;?></td>-->
...
<?php
	$sql2="select * from tbl_proslipmain where proslipmain_date <='".$edate."' and proslipmain_date >='".$sdate."' ";
	if($crop!="ALL")
	{	
...
if($tot_rr222>0)
{
$qry24="select distinct lotcrop from tblarrival_sub where sstage='Raw'  ";
if($crop!="ALL")
{	
...
if($txtprodper!="ALL")
{
	$qry24.=" and pper='".$txtprodper."' ";
}
	$qry24.=" order by lotcrop";
/*if($crop!="ALL" && $variety!="ALL")
{	
	$qry24="select distinct lotcrop from tblarrival_sub where organiser='".$txtpmcode."' and lotcrop='".$crp."' and lotvariety='".$ver."'  and sstage='Raw' order by lotcrop";
}
else if($crop!="ALL" && $variety=="ALL")
{
	$qry24="select distinct lotcrop from tblarrival_sub where organiser='".$txtpmcode."' and lotcrop='".$crp."' and sstage='Raw' order by lotcrop";
}
else
{
	$qry24="select distinct lotcrop from tblarrival_sub where organiser='".$txtpmcode."' and sstage='Raw' order by lotcrop";
}	*/
...
$qry23="select distinct lotvariety from tblarrival_sub where lotcrop='".$row_arr_home24['lotcrop']."' and sstage='Raw' ";
	if($variety!="ALL")
	{
	...
	if($txtprodper!="ALL")
	{
		$qry23.=" and pper='".$txtprodper."' ";
	}	
	$qry23.=" order by lotvariety";
/*if($variety!="ALL")
{
	$qry23="select distinct lotvariety from tblarrival_sub where organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and lotvariety='".$ver."' and sstage='Raw' order by lotvariety";
}
else
{
	$qry23="select distinct lotvariety from tblarrival_sub where organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and sstage='Raw' order by lotvariety";
}	*/
...
$arid='';
	if($txtpmcode!="ALL")
	{
		...
		{
			$qry2="select arrival_id from tblarrival_sub where organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and lotvariety='".$row_arr_home23['lotvariety']."' and sstage='Raw'";
		}
	}
	else
	{
		...
		{
			$qry2="select arrival_id from tblarrival_sub where lotcrop='".$row_arr_home24['lotcrop']."' and lotvariety='".$row_arr_home23['lotvariety']."' and sstage='Raw'";
		}
	}
/*$arid='';
	$qry2="select arrival_id from tblarrival_sub where organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and lotvariety='".$row_arr_home23['lotvariety']."' and sstage='Raw'";*/
...
<?php
	...
	$qry="select arrival_id, arrival_date, yearcode from tblarrival where  arrival_type='Fresh Seed with PDN' and arrtrflag=1 and arrival_id IN($arid)";
...
<tr class="tblsubtitle" height="20">
  ...
  <td width="35" align="center" valign="middle" class="smalltblheading" >Days</td>
			...
			<td width="98"  align="center" valign="middle" class="smalltblheading" >Prod. Per.</td>
			<td width="98" align="center" valign="middle" class="smalltblheading">Organizer</td>
...
while($row_arr_home1=mysqli_fetch_array($sql_arr_home1))
{
	...
	{	
		...
		{
			$sql_rr=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row_arr_home1['arrival_id']."' and organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and lotvariety='".$row_arr_home23['lotvariety']."' and sstage='Raw' order by orlot asc") or die(mysqli_error($link));
		}
	}
	else
	{
		...
		{
			$sql_rr=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row_arr_home1['arrival_id']."' and lotcrop='".$row_arr_home24['lotcrop']."' and lotvariety='".$row_arr_home23['lotvariety']."' and sstage='Raw' order by orlot asc") or die(mysqli_error($link));
		}
	}
	//$sql_rr=mysqli_query($link,"select * from tblarrival_sub where arrival_id='".$row_arr_home1['arrival_id']."' and organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and lotvariety='".$row_arr_home23['lotvariety']."' and sstage='Raw' order by orlot asc") or die(mysqli_error($link));
...
$cswqty=""; $tplqty="";  $flg=0;
$sql="select * from tbl_proslipmain where proslipmain_date <='".$edate."' and proslipmain_date >='".$sdate."' ";
	if($crop!="ALL")
	{	
	...
	//echo $tot_rr22=mysqli_num_rows($sql_rr22);echo " - ";
...
<tr class="Light">
    ...
    <td align="center" valign="middle" class="smallesttbltext"><?php echo $prodper?></td>
...
<tr >
<td  align="center" valign="middle"><a href="report_prodperw_prostatus3.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" />&nbsp;&nbsp;<a href="excel-prodperwpsr6.php?txtcrop=<?php echo $_REQUEST['txtcrop'];?>&sdate=<?php echo $_REQUEST['sdate'];?>&edate=<?php echo $_REQUEST['edate'];?>&txtvariety=<?php echo $_REQUEST['txtvariety'];?>&txtpmcode=<?php echo $_REQUEST['txtpmcode'];?>&txtprodper=<?php echo $_REQUEST['txtprodper'];?>" target="_blank"><img src="../images/excelicon1.jpg" border="0" height="30" width="30" class="butn" alt="Export to Excel" style="display:inline;cursor:pointer;" /></a></td>
</tr>
</table>
</td><td ></td>
</tr>
