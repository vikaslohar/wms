<?php
	...
	date_default_timezone_set('Asia/Calcutta');
		...
		$sdate = $_REQUEST['sdate'];//date("d-m-Y");
...
winHandle=window.open('stock_report2.php?slchk='+slchk+'&slchk2='+slchk2+'&txtcrop='+itemid+'&txtvariety='+vv+'&sdate='+sdate,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
...
<tr class="tblsubtitle" height="20">
	...
	<td width="40" align="center" valign="middle" class="tblheading" rowspan="2">Arrival Year &and; Plant</td>
</tr>
<tr class="tblsubtitle" height="20">
  ...
  <td width="40"  align="center" valign="middle" class="tblheading">SLOC</td>
...
$srno=1; $totalrnob=0; $totalrqty=0; $totalcnob=0; $totalcqty=0; $totalpnob=0; $totalpnomp=0; $totalpqty=0; $totalnob=0; $totalqty=0; $totalsrnob=0; $totalsrqty=0; 
		...
		$totrnob=0; $totrqty=0; $totcnob=0; $totcqty=0; $totpnob=0; $totpnomp=0; $totpqty=0; $ccnt=0; $tnob=0; $tqty=0; $totsrnob=0; $totsrqty=0; $ups=""; $qcresult=""; $gotresult=""; $slocs=''; $plant='';
		$sql_var=mysqli_query($link,"select * from tblvariety where varietyid='".$verval."' ") or die(mysqli_error($link));
		$row_var=mysqli_fetch_array($sql_var);
		...
		{  
			...
			{ 
				...
				$sql_istbl=mysqli_query($link,"select lotldg_balqty, lotldg_balbags, lotldg_got1, lotldg_got, lotldg_gemp, lotldg_gottestdate, lotldg_qc, lotldg_qctestdate, lotldg_whid, lotldg_binid, lotldg_subbinid, plantcode  from tbl_lot_ldg where lotldg_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' order by lotldg_id asc") or die(mysqli_error($link)); 
		...
		{  
			...
			{ 
				...
				$row_is1=mysqli_fetch_array($sql_is1); 
				$sql_istbl=mysqli_query($link,"select balqty, balnomp, packtype, wtinmp, lotldg_got1, lotldg_got, lotldg_gemp, lotldg_gottestdate, lotldg_qc, lotldg_qctestdate, whid, binid, subbinid, plantcode from tbl_lot_ldg_pack where lotdgp_id='".$row_is1[0]."' and lotldg_trdate<='$stdate' order by lotdgp_id asc") or die(mysqli_error($link)); 
				$t=mysqli_num_rows($sql_istbl);
				if($t > 0)
				{
					while($row_issuetbl=mysqli_fetch_array($sql_istbl))
		...
		while($row_arhome=mysqli_fetch_array($sql_arhome))
		{  
			$sql_istbl=mysqli_query($link,"select salesrss_qty, salesrss_nob from tbl_salesrvsub_sub where salesrs_id='".$row_arhome['salesrs_id']."'") or die(mysqli_error($link)); 
			$t=mysqli_num_rows($sql_istbl);
			if($t > 0)
			{
				while($row_issuetbl=mysqli_fetch_array($sql_istbl))
...
$arryear='';		
$sqp2c=mysqli_query($link,"Select * from tbl_blends where blends_orlot='".$lt2."' order by blendm_id desc, blends_id desc ") or die(mysqli_error($link));
...
if($plant=="D")$plantname='Deorjhal';
if($plant=="B")$plantname='Boriya';
if($plant=="H")$plantname='Hyderabad';
...
<tr class="Light">
	...
	<td align="center" valign="middle" class="smalltbltext"><?php echo $arryear."-".$plantname?></td>
</tr>
<?php
}
else
...
<tr class="Light">
	...
	<td align="center" valign="middle" class="smalltbltext" colspan="8">&nbsp;</td>
</tr>
</table>			
<br />
<?php
...
<tr >
<td height="49" align="center" valign="top"><a href="stock_report.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" />&nbsp;<a href="excel_stockreport.php?txtcrop=<?php echo $_REQUEST['txtcrop']?>&txtvariety=<?php echo $_REQUEST['txtvariety']?>&slchk=<?php echo $_REQUEST['slchk'];?>&slchk2=<?php echo $_REQUEST['slchk2'];?>&sdate=<?php echo $sdate;?>" target="_blank"><img src="../images/excelicon1.jpg" border="0" height="30" width="30" class="butn" alt="Export to Excel" style="display:inline;cursor:pointer;" /></a>
