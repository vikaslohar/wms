<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		for($k=0; $k<count($fctrid); $k++)
		{
			...
			if($samplenumber!='' && $germptestresult!='')
			{
				$dt=date("d-m-Y h:i:sa"); $one=1; $two=2; $moist=0; $vchk='Acceptable'; $zero=0;
				$tdate=date("Y-m-d");
				if($germptestresult=="RT")
				{$zero=1;}
				$sql_in1="update tbl_qcgdata set qcg_germp='$germinationper', qcg_le='$leduration', qcg_supremark='$txtremark', qcg_suplogid='$logid', qcg_germpflg='$one', qcg_germpdt='$tdate', qcg_docsrefno='$txtrefno', qcg_retult='$germptestresult', qcg_retestflg='$zero' where qcg_sampleno='$samplenumber' and qcg_retestflg=0";	
				if($aa=mysqli_query($link,$sql_in1)or die(mysqli_error($link)))
				{
					/*if($germptestresult=="RT")
					{
						$sql_germpdata=mysqli_query($link,"select * from tbl_qcgdata where qcm_sampno='".$samplenumber."' order by qcg_id DESC LIMIT 0,1 ") or die(mysqli_error($link));
						if(mysqli_num_rows($sql_germpdata)>0)
						{
							$row_germpdata=mysqli_fetch_array($sql_germpdata);
							$sql_germpdata_sub="insert into tbl_qcgdata(lotno,  oldlot, crop, variety, srdate, spdate, sampleno, state, qc, trstage, pp, moist, gemp, qcstatus, testdate, gotdate, dosdate, got, gotstatus, aflg, bflg, cflg, qcflg, gotflg, gsflg, gs, gotrefno, gotauth, doswdate, gotsmpdflg, stsno, qcrefno, yearid, logid) values('$olotno22', '$oldlotn', '$crp', '$ver', '$srdt', '$spdt', '$smpno', '$stats', '$oqc', '$stge', '$opp', '$omt', '$ogmp', '$oqcst', '$oqtdt', '$ogotdate', '$odosdate', '$ogot', '$ogotstatus', '$oaflg', '$obflg', '$ocflg', '$oqcflg', '$ogotflg', '$ogsflg', '$ogs', '$ogotrefno', '$ogotauth', '$odoswdate', '$ogotsmpdflg', '$ostsno', '$oqcrefno', '$yearid', '$logid')";
							if(mysqli_query($link,$sql_germpdata_sub) or die(mysqli_error($link)))
							{
							}
						}	
					}*/
							...
							while($row_ck=mysqli_fetch_array($sql_ck))
							{
								...
								if($ores=="RT")
								{
									...
									if(mysqli_query($link,$sql_sub_sub) or die(mysqli_error($link)))
									{
										...
										if($result=="OK" || $result=="Fail" || $result=="BL")
										{
											$sql_sub_sub12="update tbl_qctest set  gemp='$gemp', qcstatus='$result', testdate='$tdate', qcflg=1, qcrefno='$txtrefno' where tid='$id'";
								...
								{ 
									if($result=="OK" || $result=="Fail" || $result=="BL")
									{
										$sql_sub_sub12="update tbl_qctest set gemp='$gemp', qcstatus='$result', testdate='$tdate', qcflg=1, qcrefno='$txtrefno' where tid='".$rowck2['tid']."' and sampleno='$osamp22' and yearid='$yearid22'";
									...
									{
										$sql_sub_sub12="update tbl_qctest set gemp='$gemp', qcstatus='$result', qcflg=0, testdate='$tdate', qcrefno='$txtrefno' where tid='".$rowck2['tid']."' and sampleno='$osamp22' and yearid='$yearid22'";
									}
								...
								{	
									...
									if($tot_chk > 0)
									{
										...
										/*if($zz[0]=="GOT-NR")
										{*/
											if(($row_chk['lotldg_got']=="OK" || $row_chk['lotldg_got']=="Fail" || $row_chk['lotldg_got']=="BL") && $row_chk['lotldg_srflg']==1)
											{
												$x="";
...
{
var flagcode=document.frmaddDept.flagcode.value;
var foccode=document.frmaddDept.foccode.value;
var foccode1=document.frmaddDept.foccode1.value;
var foccode2=document.frmaddDept.foccode2.value;
var foccode3=document.frmaddDept.foccode3.value;
var foccode4=document.frmaddDept.foccode4.value;

winHandle=window.open('qcdatamgp_print.php?oflagcode='+flagcode+'&ofoccode='+foccode+'&ofoccode1='+foccode1+'&ofoccode2='+foccode2+'&ofoccode3='+foccode3+'&ofoccode4='+foccode4,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
...
<tr class="Light">
	...
	<td align="center" valign="middle" class="smalltbltext"><input type="hidden" name="xfoccode3<?php echo $i?>" id="xfoccode3_<?php echo $i?>" value="<?php echo $xfoccode3[$i]?>" /><select name="germptestresult" class="smalltbltext" style="size:70px;">
	<option value="OK" <?php if($xfoccode3[$i]=="OK") { echo "selected"; }?>>OK</option>
	<option value="BL" <?php if($xfoccode3[$i]=="BL") { echo "selected"; }?>>BL</option>
	<option value="RT" <?php if($xfoccode3[$i]=="RT") { echo "selected"; }?>>RT</option>
	<option value="Fail" <?php if($xfoccode3[$i]=="Fail") { echo "selected"; }?>>Fail</option>
	</select></td>
...
<table cellpadding="5" cellspacing="5" border="0" width="950">
    <tr >
      <td align="right" colspan="3"><a href="home_qcdatamgp.php" tabindex="20"><img src="../images/back.gif" border="0"style="display:inline;cursor:hand;" /></a>&nbsp;&nbsp;<a href="Javascript:void(0)" onClick="openslocpopprint1();"><img src="../images/printpreview.gif" border="0"style="display:inline;cursor:pointer;" /></a>&nbsp;&nbsp;<input name="image" type="image" style="display:inline;cursor:pointer;" onclick="return mySubmit();" src="../images/submit.gif" alt="Submit Value" border="0"/>
	  </td>
    </tr>
  </table>
</div>
