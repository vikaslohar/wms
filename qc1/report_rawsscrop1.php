...
var txtsdate=document.frmaddDepartment.txtsdate.value;
var txtedate=document.frmaddDepartment.txtedate.value;
winHandle=window.open('report_rawsscrop2.php?txtcrop='+itemid+'&txtvariety='+vv+'&txtstage='+txtstage+'&sdate='+txtsdate+'&edate='+txtedate,'WelCome','top=20,left=80,width=850,height=600,scrollbars=yes');
if(winHandle==null){
...
?>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" >
	  ...
	  <td align="center" colspan="4" >
		...
		<input name="txtsdate" value="<?php echo $_REQUEST['sdate'];?>" type="hidden">  
		<input name="txtedate" value="<?php echo $_REQUEST['edate'];?>" type="hidden">  
...
if($crval<>"")
{
	...
	if($txtstage!="ALL")
	{
		if($txtstage=="Raw")
		{
			$qry="select Distinct lotldg_variety from tbl_lot_ldg where lotldg_id!=0 and lotldg_sstage='$txtstage' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate'v and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_crop='".$crval."'";
		}
		else if($txtstage=="Condition")
		{
		...
		if($txtstage!="Pack")
		{
			...
			{  
				...
				{ 
					...
					$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_lotno='".$row_arr_home['lotldg_lotno']."' and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_sstage='$txtstage' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' order by lotldg_id asc ") or die(mysqli_error($link));
					$row_issue1=mysqli_fetch_array($sql_issue1); 
				
					$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and lotldg_balqty > 0 and (lotldg_qc='Fail' OR lotldg_got='Fail') and lotldg_sstage='$txtstage' and lotldg_trdate>='$sdate' and lotldg_trdate<='$edate' order by lotldg_id asc") or die(mysqli_error($link)); 
