...
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		//exit;	
		echo "<script>window.location='home_qcdatammp_preview.php?oflagcode=$flagcode&ofoccode=$foccode&ofoccode1=$foccode1'</script>";
...
function openfilter()
{
	...
	window.open('home_qcdatammp.php?srsno='+srsno);
}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" style="border-bottom:solid; border-bottom-color:#d21704" >
	    <tr >
	      <td width="940" height="25">&nbsp;Transaction - QC Moisture Data Verification and Moisture % Update</td>
	    </tr></table></td>
...
<tr class="Light" height="20">
  <td align="center" colspan="12" class="tblheading">QC Moisture Data Verification Pending List</td>
 <td width="225" align="left" class="tblheading">SRF No.&nbsp;
   <input type="text" class="smalltbltext" size="16"  name="srsno" id="srsno" value="<?php echo $srsno;?>" style="background-color:#FFFFFF; border-color:#378b8b"  />&nbsp;&nbsp;<a href="Javascrip:void(0);" onclick="openfilter();">Search</a>&nbsp;&nbsp;&nbsp;</td>
...
<tr class="tblsubtitle" height="20">
	<td width="25" height="22"align="center" valign="middle" class="smalltblheading" rowspan="2">#</td>
	<td width="88" align="center" valign="middle" class="smalltblheading" rowspan="2">SRF No.</td>
	<td width="88" align="center" valign="middle" class="smalltblheading" rowspan="2">Crop</td>
	<td width="125" align="center" valign="middle" class="smalltblheading" rowspan="2">Variety</td>
	<td width="98" align="center" valign="middle" class="smalltblheading" rowspan="2">Lot No.</td>
	<td width="54" align="center" valign="middle" class="smalltblheading" rowspan="2">Qty</td>
	<td width="51" align="center" valign="middle" class="smalltblheading" rowspan="2">Stage</td>
	<td width="69" align="center" valign="middle" class="smalltblheading" rowspan="2">DOSC</td>
	<td width="69" align="center" valign="middle" class="smalltblheading" rowspan="2">Date of Test</td>
	<td width="72" rowspan="2" align="center" valign="middle" class="smalltblheading">Moisture % by Moisture Meter</td>
	<td width="85" rowspan="2" align="center" valign="middle" class="smalltblheading">Moisture % by Oven Method</td>
	<td width="94" align="center" valign="middle" class="smalltblheading" rowspan="2">Select</td>
	<td align="center" valign="middle" class="smalltblheading" colspan="2">Moisture %</td>
</tr>
<tr class="tblsubtitle" height="20">
...
$sqlmgdata=mysqli_query($link,"select * from tbl_qcmdata where qcm_moistflg!=1 order by qcm_mmrepdt, qcm_m3repdt, qcm_sampno ASC ") or die(mysqli_error($link));
while($rowmgdata=mysqli_fetch_array($sqlmgdata))
{	

$smpnumber2=str_split($rowmgdata['qcm_sampno']);
...
while($row_arr_home24=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_arr_home2=mysqli_query($link,"select max(tid) from tbl_qctest where bflg=1 and qcflg=0 and state!='T' and sampleno='".$row_arr_home24['sampleno']."' order by qcrefno ASC, tid desc") or die(mysqli_error($link));
	...
	{
	$moistflg=0; $ppflg=0; $germflg=0; $resultflg=0; $moistpercentages=''; $ppresult='';  $srfno="";
	...
	$qcm_haommoistper=0; $qcm_mmrmoistper=0; $qcm_m3repdt=''; $qcm_mmrepdt='';
	if($row_arr_home['state']!="G")
	{
		...
		while($row_mdata=mysqli_fetch_array($sql_mdata))
		{
			...
			$qcm_m3repdt=$row_mdata['qcm_m3repdt']; 
			$qcm_mmrepdt=$row_mdata['qcm_mmrepdt'];	
		}
	}
	else
	{
	...
	$setupdt='';
	if($qcm_m3repdt!='' && $qcm_m3repdt!='0000-00-00' && $qcm_m3repdt!='--' && $qcm_m3repdt!='- -')
	{
		$qcm_m3repdt1=explode(" ",$qcm_m3repdt);
		//$qcm_m3repdt2=explode("-",$qcm_m3repdt1[0]);
		$setupdt=$qcm_m3repdt1[0];//$qcm_m3repdt2[2]."-".$qcm_m3repdt2[1]."-".$qcm_m3repdt2[0];
	}
	
	if($qcm_mmrepdt!='' && $qcm_mmrepdt!='0000-00-00' && $qcm_mmrepdt!='--' && $qcm_mmrepdt!='- -')
	{
		$qcm_mmrepdt1=explode(" ",$qcm_mmrepdt);
		//$qcm_mmrepdt2=explode("-",$qcm_mmrepdt1[0]);
		$setupdt=$qcm_mmrepdt1[0];//$qcm_mmrepdt2[2]."-".$qcm_mmrepdt2[1]."-".$qcm_mmrepdt2[0];
	}
	if($moistflg>0) 
	{
...
?>			  
<tr class="Light">
	<td width="25" align="center" valign="middle" class="smalltbltext"><?php echo $srno;?></td>
	<td width="25" align="center" valign="middle" class="smalltbltext"><?php echo $srfno;?></td>
	<td width="88" align="center" valign="middle" class="smalltbltext"><?php echo $row31['cropname'];?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $vv;?></td>
	<td width="98" align="center" valign="middle" class="smalltbltext"><?php echo $lotno?></td>
	...
	<td align="center" valign="middle" class="smalltbltext"><?php echo $qcm_mmrmoistper;?></td>
	<td align="center" valign="middle" class="smalltbltext"><?php echo $qcm_haommoistper;?></td> 
	
	<td align="center" valign="middle" class="smalltbltext">&nbsp;<?php if($moistflg==1){echo "Verified";}else if($moistflg==2){?><a href="Javascript:void(0);" onclick="openmoistdata('<?php echo $arrival_id?>')">Review</a><?php } else if($moistflg==3){echo "In-Progress";} else {echo "YTS";}?><?php if($moistflg==2){ ?><input type="checkbox" name="germpsel" id="germpsel_<?php echo $sel;?>" value="<?php echo $arrival_id?>" /><?php }else {echo "";}?></td>
	
	<td width="56" align="center" valign="middle" class="smalltbltext"><?php if($qcm_mmrmoistper>0 && $qcm_haommoistper>0){?>
	  <input type="radio" name="testtype<?php echo $srno;?>" id="testtype_<?php echo $srno;?>" value="MM" onclick="setgermpval(this.value,'<?php echo $srno;?>')" />MM&nbsp;<input type="radio" name="testtype<?php echo $srno;?>" id="testtype_<?php echo $srno;?>" checked="checked" value="Oven" onclick="setgermpval(this.value,'<?php echo $srno;?>')" />Oven<input type="hidden" name="germptesttype" id="germptesttype_<?php echo $srno;?>" value="Oven" /><?php } else if($qcm_mmrmoistper>0 && $qcm_haommoistper==0){?><input type="radio" name="testtype<?php echo $srno;?>" id="testtype_<?php echo $srno;?>" value="MM" checked="checked" onclick="setgermpval(this.value,'<?php echo $srno;?>')" />MM&nbsp;<input type="radio" name="testtype<?php echo $srno;?>" id="testtype_<?php echo $srno;?>" value="Oven" disabled="disabled" onclick="setgermpval(this.value,'<?php echo $srno;?>')" />Oven<input type="hidden" name="germptesttype" id="germptesttype_<?php echo $srno;?>" value="MM" /><?php } else {?><input type="radio" name="testtype<?php echo $srno;?>" id="testtype_<?php echo $srno;?>" value="MM" onclick="setgermpval(this.value,'<?php echo $srno;?>')" disabled="disabled" />MM&nbsp;<input type="radio" name="testtype<?php echo $srno;?>" id="testtype_<?php echo $srno;?>" value="Oven" checked="checked" onclick="setgermpval(this.value,'<?php echo $srno;?>')" />Oven<input type="hidden" name="germptesttype" id="germptesttype_<?php echo $srno;?>" value="Oven" /><?php } ?></td>
	  
	<td width="56" align="center" valign="middle" class="smalltbltext"><input type="text" name="germpercentage" id="germpercentage_<?php echo $srno;?>" value="<?php if($qcm_mmrmoistper>0 && $qcm_haommoistper==0){ echo $qcm_mmrmoistper;} else if($qcm_mmrmoistper==0 && $qcm_haommoistper>0){ echo $qcm_haommoistper;} else if($qcm_mmrmoistper>0 && $qcm_haommoistper>0){ echo $qcm_haommoistper;} else { echo "";}?>" readonly="true" style="background-color:#CCCCCC" size="2" /> <input type="hidden" name="sgtgermp" id="sgtgermp_<?php echo $srno;?>" value="<?php echo $qcm_mmrmoistper;?>" /> <input type="hidden" name="fgtgermp" id="fgtgermp_<?php echo $srno;?>" value="<?php echo $qcm_haommoistper;?>" /> </td>
