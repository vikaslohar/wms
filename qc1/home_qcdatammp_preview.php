	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		for($k=0; $k<count($fctrid); $k++)
		{
			...
			if($samplenumber!='')
			{
				...
				$tdate=date("Y-m-d");
				$sql_in1="update tbl_qcmdata set qcm_moistper='$germinationper', qcm_mmsrepdt='$dt', qcm_mmsreplogid='$logid', qcm_moistflg ='$one', qcm_moistdt='$dt', qcm_mmrsflg='$two' where qcm_sampno='$samplenumber'";	
					...
					while($rowck=mysqli_fetch_array($sqlck))
					{
						...
						while($rowck2=mysqli_fetch_array($sqlck2))
						{
							...
							while($row_ck=mysqli_fetch_array($sql_ck))
							{
								...
								$sql_sub_sub12="update tbl_qctest set moist='$moist' where tid='".$rowck2['tid']."' and sampleno='$osamp22' and yearid='$yearid22'";
								$qq=mysqli_query($link,$sql_sub_sub12) or die(mysqli_error($link));

								$sql_sub="update tbl_lot_ldg set lotldg_moisture='$moist' where orlot='$oldlotn'";
								$sql_sub2="update tbl_lot_ldg_pack set lotldg_moisture='$moist' where orlot='$oldlotn'";
		...
		{
			echo "<script>window.location='home_qcdatammp.php'</script>";
		}
	}
...
<table cellpadding="5" cellspacing="5" border="0" width="950">
    <tr >
      <td align="right" colspan="3"><a href="home_qcdatammp.php" tabindex="20"><img src="../images/back.gif" border="0"style="display:inline;cursor:hand;" /></a>&nbsp;<input name="image" type="image" style="display:inline;cursor:pointer;" onclick="return mySubmit();" src="../images/submit.gif" alt="Submit Value" border="0"/>
	  </td>
    </tr>
  </table>
</div>
