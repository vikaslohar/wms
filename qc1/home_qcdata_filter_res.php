...
if($reptyp=="lotno")
{ 
 $sql_arr_home=mysqli_query($link,"select distinct sampleno from tbl_qctest where oldlot='$lotno' and bflg=1 and spdate IS NOT NULL and spdate!='0000-00-00' and qcflg=0 and state!='T' order by  spdate ASC, tid desc ") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
}
 else if($reptyp=="sno")
{ 
$sql_arr_home=mysqli_query($link,"select distinct sampleno from tbl_qctest where sampleno='".$sampl."' and bflg=1 and spdate IS NOT NULL and spdate!='0000-00-00' and qcflg=0 and state!='T' order by  spdate ASC, tid desc ") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
}
else 
{
$sql_arr_home=mysqli_query($link,"select distinct sampleno from tbl_qctest where crop='".$txtcrop."' and variety='".$txtvariety."' and bflg=1 and spdate IS NOT NULL and spdate!='0000-00-00' and qcflg=0 and state!='T' order by  spdate ASC, tid desc ") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
}
...
while($row_arr_home24=mysqli_fetch_array($sql_arr_home))
{
	...
	if($reptyp=="select")
	{ 
	$sql_arr_home2=mysqli_query($link,"select max(tid) from tbl_qctest where bflg=1 and qcflg=0 and state!='T' and sampleno='".$row_arr_home24['sampleno']."' and  crop='".$txtcrop."' and variety='".$txtvariety."' order by spdate ASC, tid desc") or die(mysqli_error($link));
	}
	else if($reptyp=="lotno")
	{ 
	$sql_arr_home2=mysqli_query($link,"select max(tid) from tbl_qctest where oldlot='$lotno' and bflg=1 and qcflg=0 and state!='T' and sampleno='".$row_arr_home24['sampleno']."' order by spdate ASC, tid desc") or die(mysqli_error($link));
	}
	else
	{
