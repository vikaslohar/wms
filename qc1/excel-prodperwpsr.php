...
function dateDiff($start, $end) 
{
  ...
  return round($diff / 86400);
	...
	$datahead = array("Production Personnel wise Processing Status Report for ".$ttpp." - As on Date ".$dat);
	$filename=$dh.".xls";  
	$datahead2 = array("Crop",$crp,"Variety",$ver);
...
$datahead3[$cnt] = array("Date of Arrival","Days","FRN No.","Lot No.","Prod. Location","Prod. Personnel","Organizer","Farmer","PDN Qty","Arrival Qty","Raw Seed Qty","Condition Seed Qty","Cond. Loss (RM + IM)","Date of Processing","GOT Result","DOGR","Physical Purity","Germination %","Genetic Purity %"); 
...
$seedsstatus="";	
$sql_qc=mysqli_query($link,"select max(tid) from tbl_qctest where lotno='$lotno'") or die(mysqli_error($link));
$row_qc=mysqli_fetch_array($sql_qc);

$sqlqc=mysqli_query($link,"select * from tbl_qctest where lotno='$lotno' and tid='".$row_qc[0]."'") or die(mysqli_error($link));
...
if($tot_rr > 0)
{
$data1[$cnt][$d]=array($trdate1,$diff,$frn,$lotno1,$ploc,$prodper,$pper,$farmer,$pqty,$arrqty,$rswqty,$cswqty,$tplqty,$prsldate,$gotr,$dogr,$seedsstatus,$gemp,$genpurity); 
$d++;$cntt++;
}
