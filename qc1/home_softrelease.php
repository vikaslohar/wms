...
$sql_tbl=mysqli_query($link,"select * from tbl_softr where softr_tflg=0") or die(mysqli_error($link));
...
<title>QC Manager - Transaction -Home - Soft Release</title>
...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_softr where softr_tflg=1 order by softr_date desc, softr_id desc,softr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);

//$total_results = mysql_result(mysqli_query($link,"SELECT COUNT(*) as Num FROM tbl_qctest where bflg=1   and qcflg=0"),0); 

$query = "select * from tbl_softr where softr_tflg=1";
...
$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and lotldg_balqty > 0  and (lotldg_qc='UT' OR lotldg_qc='RT')") or die(mysqli_error($link)); 
...
$total_results = mysql_result(mysqli_query($link,"SELECT COUNT(*) as Num FROM tbl_softr where softr_tflg=1 and softr_date>='$dt1'"),0); 
