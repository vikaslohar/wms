<?php
	...
	//if($logid!="QM1" && $logid!="QS1" && $logid!="QS2" && $logid!="QS3" && $logid!="QS4")
	if($role!="QCS")
	{
		echo '<script language="JavaScript" type="text/JavaScript">';
	...
	$sql=mysqli_query($link,"Select * from tbl_blendm where blendm_id='$pid'") or die(mysqli_error($link));
	$tot=mysqli_num_rows($sql);
	$row=mysqli_fetch_array($sql);
	if($row['blendm_vlogid']!="" && $row['blendm_vlogid']!="$logid")
	{
		echo "<script>alert('Cannot Authorise the Transaction. Reason: Another Operator Authorizing it.');window.location='home_merger.php'</script>";
	}
	else
	{
		$sq="update tbl_blendm set blendm_vlogid='$logid' where blendm_id='$pid'";
	...
	{	
		...
		if($gflg > 0)
		{
			echo "<script>alert('Cannot Authorise the transaction. Reason: Cannot create new Group/Re-group with single lot. Select 2 or more lots to create new Group/Re-group.');window.location='add_lotmerger.php?p_id=$trid;'</script>";
		}
		else
		{
...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" >
	  ...
	  <td width="32" height="20"><img src="../images/rupee1.jpg" align="right" width="30" height="20" />&nbsp;</td>
	  <td width="940" class="Mainheading" height="20">
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" style="border-bottom:solid; border-bottom-color:#d21704" >
	    <tr >
	      <td width="813" height="20" class="Mainheading">&nbsp;Transaction - Lot Blending</td>
	    </tr></table></td>
...
$itemqry=mysqli_query($link,"select * from tblvariety where varietyid='".$row['blendm_variety']."' and actstatus='Active'") or die(mysqli_error($link));
...
<td width="825" align="left"  valign="middle" class="smalltbltext">&nbsp;<input type="text" name="txtremarks" class="tbltext" size="132" maxlength="150" value="" ></td>
</tr></table>
