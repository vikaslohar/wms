  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" style="border-bottom:solid; border-bottom-color:#d21704" >
        <tr >
          <td width="940" height="25">&nbsp;Transaction - Density Data Update</td>
        </tr></table></td>
...
$targetpage = $PHP_SELF; 
	...
	$sql_arr_home=mysqli_query($link,"select * from tbl_blendm where blendm_stage!='Condition' and blendm_date>='2023-03-01' and blendm_tflg=1 and blendm_crop IN ($cropids) order by blendm_date DESC, blendm_crop, blendm_variety ASC ") or die(mysqli_error($link));
	$tot_arr_home=mysqli_num_rows($sql_arr_home);
	
	//$total_results = mysql_result(mysqli_query($link,"SELECT COUNT(*) as Num FROM tbl_qctest where bflg=1   and qcflg=0"),0); 
	
	$query = "select * from tbl_blendm where blendm_stage!='Condition' and blendm_tflg=1 and blendm_date>='2023-03-01' and blendm_crop IN ($cropids)";
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Density Data Update Pending List </td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="800" bordercolor="#d21704" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
              ...
              <td width="9%" align="center" valign="middle" class="tblheading">Blending Date</td>
			  <td width="9%" align="center" valign="middle" class="tblheading">Processing Date</td>
              ...
              <td width="8%" align="center" valign="middle" class="tblheading">Density Data</td>
            </tr>
<?php
//$srno=1;
if($tot_arr_home > 0)
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$arrival_id=$row_arr_home['blendm_id'];
	
	$quer3=mysqli_query($link,"SELECT cropid, cropname FROM tblcrop where cropid='".$row_arr_home['blendm_crop']."' order by cropname Asc")or die(mysqli_error($link));
	$noticia = mysqli_fetch_array($quer3);
	
	$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$row_arr_home['blendm_variety']."' order by popularname Asc")or die(mysqli_error($link)); 
	$noticia4 = mysqli_fetch_array($quer4);

	$crop=$noticia['cropname'];
	$variety=$noticia4['popularname'];	
	
	$density_data=0; $tot_blends=0; $xnt=0;
	$quer_sub=mysqli_query($link,"SELECT blends_newlot, blends_orlot, blends_lotno FROM tbl_blends where blendm_id='".$row_arr_home['blendm_id']."' and blends_sstatus=1 order by blends_orlot Desc")or die(mysqli_error($link)); 
	$tot_blends=mysqli_num_rows($quer_sub);
	while($row_bsub = mysqli_fetch_array($quer_sub))
	{
		$lotno=$row_bsub['blends_newlot'];
		$orlot=$row_bsub['blends_orlot'];
	
	
		$sql_density=mysqli_query($link,"SELECT density_lotno, density_clotno FROM tbl_densitydata where density_clotno='".$lotno."' order by density_id Asc")or die(mysqli_error($link)); 
		while($row_density = mysqli_fetch_array($sql_density))
		{
			$xnt++;
		}
	}
	if($tot_blends==$xnt) { echo $density_data=1;}

	$pro_id=0; $prdate='';
	$sql_prosub=mysqli_query($link,"SELECT * FROM tbl_proslipsub where proslipsub_lotno='".$lotno."' order by proslipsub_id Asc")or die(mysqli_error($link)); 
	...
	$density_id=0;
	$sql_density=mysqli_query($link,"SELECT density_id FROM tbl_density where density_blendedorlot='".$orlot."' order by density_id Asc")or die(mysqli_error($link)); 
	while($row_density = mysqli_fetch_array($sql_density))
	{
		$density_id=$row_density['density_id'];
	}

//echo $lotno." - ".$density_id." - ".$pro_id."<br/>";

if($density_id==0 && $pro_id>0)
{	
if($srno%2!=0)
{
?>			  
<tr class="Light">
	...
	<td width="9%" align="center" valign="middle" class="smalltblheading"><?php echo $prodate;?></td>
	...
	<td width="9%" align="center" valign="middle" class="smalltblheading"><a href="add_density.php?olotno=<?php echo $orlot;?>">Update</a></td>
</tr>
<?php
}
else
