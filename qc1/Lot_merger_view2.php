...
<?php
//echo "select * from tbl_blendm where blendm_id=$pid";
$trid=$pid;
$sql12=mysqli_query($link,"select * from tbl_blendm where blendm_id=$pid")or die(mysqli_error($link));
...
$sql_sub=mysqli_query($link,"select distinct blends_group from tbl_blends where blendm_id='$pid' and blends_group>0 and blends_delflg=0 group by blends_group order by blends_group asc") or die(mysqli_error($link));
...
$sq_sub=mysqli_query($link,"Select * from tbl_blendss where blendm_id='$pid' and blendss_newlot='$nlotno'") or die(mysqli_error($link));
