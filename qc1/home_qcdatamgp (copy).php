...
<tr class="Light" height="20">
 ...
 <td width="185" align="left" class="tblheading">SRS No.&nbsp;
...
if($filterdate!="" && $srsno!="")
{
	...
	$sqlmgdata=mysqli_query($link,"select * from tbl_qcgdata where qcg_germpflg!=1 and qcg_germpdatadt='$extdos' and  qcg_docsrefno ='$srsno' and qcg_retestflg=0 order by qcg_docsrefno, qcg_germpdatadt, qcg_sampleno ASC ") or die(mysqli_error($link));
}
else if($filterdate!="" && $srsno=="")
{ 
	$filterdate2=explode("-",$filterdate);
	$extdos=$filterdate2[2]."-".$filterdate2[1]."-".$filterdate2[0];
	$sqlmgdata=mysqli_query($link,"select * from tbl_qcgdata where qcg_germpflg!=1 and qcg_germpdatadt='$extdos' and qcg_retestflg=0 order by qcg_docsrefno, qcg_germpdatadt, qcg_sampleno ASC ") or die(mysqli_error($link));
}
else if($filterdate=="" && $srsno!="")
{
	$sqlmgdata=mysqli_query($link,"select * from tbl_qcgdata where qcg_germpflg!=1 and  qcg_docsrefno ='$srsno' and qcg_retestflg=0 order by qcg_docsrefno, qcg_germpdatadt, qcg_sampleno ASC ") or die(mysqli_error($link));
}
else
{
	$sqlmgdata=mysqli_query($link,"select * from tbl_qcgdata where qcg_germpflg!=1 and qcg_retestflg=0 order by qcg_docsrefno, qcg_germpdatadt, qcg_sampleno ASC ") or die(mysqli_error($link));
}
...
<tr class="Light">
	...
	<td align="center" valign="middle" class="smalltbltext">&nbsp;<?php if($germflg==1){echo "Verified";}else if($germflg==2){?><a href="Javascript:void(0);" onclick="opengermpdata('<?php echo $arrival_id?>')">Review</a><?php } else if($germflg==3){echo "In-Progress";} else {echo "YTS";}?><?php if($germflg==2){if(($pp=="P/M/G" || $pp=="P/M/G/T") ){ ?><input type="checkbox" name="germpsel" id="germpsel_<?php echo $sel;?>" value="<?php echo $arrival_id?>" /><?php } else if($pp=="G"){?><input type="checkbox" name="germpsel" id="germpsel_<?php echo $sel;?>" value="<?php echo $arrival_id?>" /><?php }else {?><input type="checkbox" name="germpsel" id="germpsel_<?php echo $sel;?>" value="<?php echo $arrival_id?>" disabled="disabled" /><?php }} else {echo "";}?></td>
 ...
 if($crop_name=="Maize Seed"){ if($qcg_vignormalavg>=90 && $qcg_vignormalavg<=93){echo $leslab1;} else if($qcg_vignormalavg>=94 && $qcg_vignormalavg<=96){echo $leslab2;} else if($qcg_vignormalavg>=97){echo $leslab3;} else {echo "0";} }
	...
	<td width="60" align="center" valign="middle" class="smalltbltext"><input type="text" class="smalltbltext" name="qcdocrefno" id="qcdocrefno_<?php echo $srno;?>" value="<?php echo $qcg_docsrefno;?>" size="5" /></td>
