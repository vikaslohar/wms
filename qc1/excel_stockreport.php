...
if($crval<>"")
{
	...
	if($verval<>"")
	{
		...
		$datahead2[$cnt]= array("#","Lot No.","Raw Seed","","Condition Seed","","Pack Seed","","","","Sales Return","","Total","","Quality Status","","","","","GP%","SLOC","Production Personnel","Production Location","Harvest Date","Arrival Year","Blended Year","Treatment Schema",'Plant'); 
		...
		$gemp=''; $qcresult=''; $dot=''; $gotresult=''; $dogr=''; $gpurity=''; $harvestdt=''; $arryear=''; $ploc=""; $pper=""; $blendyear="";  $plant='';
		$sql_var=mysqli_query($link,"select * from tblvariety where varietyid='".$verval."'") or die(mysqli_error($link));
		$row_var=mysqli_fetch_array($sql_var);
		...
		if($harvestdt=="0000-00-00" || $harvestdt=="--")$harvestdt="";
		...
		$trday=substr($trdate,8,2);
		$arryear=$tryear;
		if($arryear=="0000-00-00" || $arryear=="--")$arryear="";
		...
		$treatschema='';
		$sql_issuetbl=mysqli_query($link,"select * from tbl_proslipsub where proslipsub_lotno LIKE '$lt2%' ") or die(mysqli_error($link)); 
		$t=mysqli_num_rows($sql_issuetbl);
		$row_issuetbl=mysqli_fetch_array($sql_issuetbl);
		$sql_rr22=mysqli_query($link,"select * from tbl_proslipmain where proslipmain_id='".$row_issuetbl['proslipmain_id']."' ") or die(mysqli_error($link));
		$tot_rr22=mysqli_num_rows($sql_rr22);
		$row_rr22=mysqli_fetch_array($sql_rr22);
		$treatschema=$row_rr22['proslipmain_treattype'];
...
if($tqty > 0)
{
	$data1[$cnt][$d]=array($d,$lt2,$totrnob,$totrqty,$totcnob,$totcqty,$ups,$totpnob,$totpnomp,$totpqty,$totsrnob,$totsrqty,$tnob,$tqty,$gemp,$qcresult,$dot,$gotresult,$dogr,$gpurity,$slocs,$pper,$ploc,$harvestdt,$arryear,$blendyear,$treatschema, $plantname); 
	$d++;
}
}
}
