...
<title>QC Manager-Report -Period Wise Report</title>
...
winHandle=window.open('report_sfr2.php?sdate='+sdate+'&edate='+edate+'&ddate='+ddate+'&txtcrop='+itemid+'&txtvariety='+vv+'&sftyp='+sftyp,'WelCome','top=20,left=80,width=670,height=600,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   ...
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" >
	  ...
	  <table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#d21704" style="border-bottom:solid; border-bottom-color:#d21704" >
	    <tr >
	      <td width="813" height="25">&nbsp;Soft Release Report</td>
	    </tr></table></td>
	  </tr>
	  </table></td></tr>
...
?>
    ...
    <td align="right" class="subheading" style="color:#303918; ">Variety: <?php echo $vart;?></td>
  	</tr>
</table>
<?php
$sql2="select distinct softr_crop from tbl_softr where softr_tflg=1 ";	
if($sftyp=="periodical"){ $sql2.=" and softr_date <= '$edate' and softr_date >= '$sdate' "; }
else { $sql2.=" and softr_date <= '$ddate' "; }
if($itemid!="ALL") { $sql2.=" and softr_crop='".$itemid."' "; }
if($vv!="ALL") { $sql2.=" and softr_variety='".$vv."' "; }
$sql2.=" order by softr_crop asc ";

$sql_arrhome=mysqli_query($link,$sql2) or die(mysqli_error($link));
$tot_arrhome=mysqli_num_rows($sql_arrhome);
while($row_arrhome=mysqli_fetch_array($sql_arrhome))
{
	$sql3="select distinct softr_variety from tbl_softr where softr_tflg=1 and softr_crop='".$row_arrhome['softr_crop']."' ";	
	if($sftyp=="periodical"){ $sql3.=" and softr_date <= '$edate' and softr_date >= '$sdate' "; }
	else { $sql3.=" and softr_date <= '$ddate' "; }
	if($vv!="ALL") { $sql3.=" and softr_variety='".$vv."' "; }
	$sql3.=" order by softr_variety asc ";
	
	$sqlarr_home=mysqli_query($link,$sql3) or die(mysqli_error($link));
	$totarr_home=mysqli_num_rows($sqlarr_home);
	while($rowarr_home=mysqli_fetch_array($sqlarr_home))
	{


	

		$quer22=mysqli_query($link,"SELECT  cropname,cropid FROM tblcrop where cropid='".$row_arrhome['softr_crop']."'"); 
		$row_dept2=mysqli_fetch_array($quer22);
		$crpname=$row_dept2['cropname'];
		
		$quer42=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$rowarr_home['softr_variety']."' "); 
		$row_dept42=mysqli_fetch_array($quer42);
		$varty=$row_dept42['popularname'];

	
?>  
<table align="center" border="0" cellspacing="0" cellpadding="0" width="650" style="border-collapse:collapse">
	<tr height="25" >
	 <td align="left" class="subheading" style="color:#303918; ">Crop: <?php echo $crpname; ?></td>
    <td align="right" class="subheading" style="color:#303918; ">Variety: <?php echo $varty;?></td>
  	</tr>
</table>
...
$sql="select * from tbl_softr where softr_tflg=1 and softr_crop='".$row_arrhome['softr_crop']."' and softr_variety='".$rowarr_home['softr_variety']."' ";	
if($sftyp=="periodical"){ $sql.=" and softr_date <= '$edate' and softr_date >= '$sdate' "; }
else { $sql.=" and softr_date <= '$ddate' "; }
$sql.=" order by softr_date asc ";
...
<tr >
<td height="49" align="center" valign="top"><a href="report_sfr.php"><img src="../images/back.gif" border="0"  style="display:inline;cursor:pointer;"/></a>&nbsp;&nbsp;<img src="../images/printpreview.gif" onclick="openprint()" style="cursor:pointer" border="0" /><input type="hidden" name="txtinv" /></td>
</tr>
</table>
</td><td ></td>
</tr>
