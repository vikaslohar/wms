		...
		if ($user_login != false) 
		{
			if($scanAction=="Existing")
			{
				...
				//	print_r($user_trlist); exit;
					...
					{
						$user_trlist = $db->checkBarcodeInLot($lotno, $qrcode);
						if($user_trlist==4)
						{
							$response["status"] = FALSE;
							$response["msg"] = "Barcode not packed in the selected lot. Please check and try again";
							echo json_encode($response);
						}
						else
						{
					...
					{
						$response["status"] = FALSE;
						$response["msg"] = "Barcode already dispatched";
						echo json_encode($response);
					}
					else if($user_trlist==3)
					{
						$response["status"] = FALSE;
						$response["msg"] = "Barcode already unpackaged";
						echo json_encode($response);
					}
					else if($user_trlist==4)
					{
						$response["status"] = FALSE;
						$response["msg"] = "Barcode already allocated for dispatch";
						echo json_encode($response);
					}
