...
//	print_r($user_login);
	if ($user_login != false) 
	{
		...
		$response["user"]["mobile1"] = $user_login['loginid'];
		...
		$user_plantcode = $db->getPlantdetails($oprid);
		$response["user"]["pcode"] = $user_plantcode;
		
		$user_plcode = $db->getPLCode($oprid);
		$response["user"]["plcode"] = $user_plcode;
		
		$user_yearcode = $db->getYearCode();
		$response["user"]["yrcode"] = $user_yearcode;
		...
		if ($user_compflg == false) 
		{
			...
			$response["user"]["mobile1"] = '';
			$response["user"]["role"] = '';
			$response["user"]["deviceid"] = '';
			$response["user"]["sessionid"] = '';
			$response["user"]["rolesh"] = '';
			$response["user"]["scode"] = '';
			$response["user"]["pcode"] = '';
			$response["msg"] = "Invalid Login. Please try again or contact Administrator";
