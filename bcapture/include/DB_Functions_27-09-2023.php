...
public function GetTranSetupLotyrcodelist() {
	...
	public function GetLMCQRcodeUpdate($scode, $trid, $scantype, $qrcode, $cropname, $varietyname, $ups, $grosswt, $mptype, $mpwt) {
		...
		if($scantype=="wbqrcode")
		{
			$stmt_2 = $this->conn_ps->prepare("SELECT wb_extqrcode FROM tbl_wbqrcode WHERE wb_extqrcode = ? and wb_crop=? and wb_variety=? and wb_ups=? and wb_mpqlinkflg=0  and wb_actflg=1 ");
...
// New Code ------------------------------------------------	
	...
	function GetLMCQRScanningFinalize($scode, $trtype, $qrcode, $grosswt, $mptype, $wh_id, $bin_id, $subbin_id)
	{
		...
		$stmt_24 = $this->conn_ps->prepare("SELECT lgenyearcode, lgenyear FROM tbl_lgenyear order by lgenyearid DESC ");
		//$stmt_2->bind_param("s", $pdate);
		...
		if ($stmt_wbs->num_rows > 0) {
			...
			$stmt_packsstbl->bind_param("iisssssssssss", $packm_id, $packsub_id, $wb_lotno, $wb_ups, $wb_mpwt, $wh_id, $bin_id, $subbin_id, $balnop, $balqty, $nop, $zero, $plantcode);
			$result_packsstbl = $stmt_packsstbl->execute();
			if($result_packsstbl){$flg=$flg+1;}  
			$stmt_packsstbl->close();
				
			$stmt60 = $this->conn_ps->prepare("UPDATE tbl_wbqrcode SET wb_mpblinkflg=?, wb_mpqlinkflg=?, wb_mpgrosswt=? where wb_mpbarcode=? and wb_mpblinkflg=0 and wb_mpqlinkflg=2 and wb_linklogid=? ");
	...
	public function GetBinList($scode, $mobile1, $whid) {
		$plantcode = $this->getPlantcode($scode);
		$flg=0; $userSR=array();
		//return "SELECT binid, binname FROM tbl_bin WHERE whid = $whid";
