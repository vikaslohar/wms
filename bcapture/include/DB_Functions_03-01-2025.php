...
//	private $conn_vnr;
	...
	public function updateReason($barcode, $trid, $reason) {
		...
		// Bind the parameters
		$stmt1->bind_param("sss", $reason, $barcode, $trid);
...
// return "Update tbl_pnpslipsub SET pnpslipsub_slabelno='$slblno', pnpslipsub_mingrwt='$mingrwt', pnpslipsub_maxgrwt='$maxgrwt' pnpslipsub_lblflg=1 where pnpslipmain_id = $trid ";
			$stmt6 = $this->conn_ps->prepare("Update tbl_pnpslipsub SET pnpslipsub_slabelno=?, pnpslipsub_slabelno_temp1=?, pnpslipsub_slabelno_temp2=?, pnpslipsub_mingrwt=?, pnpslipsub_maxgrwt=?, pnpslipsub_lblflg=1 where pnpslipmain_id = ? ");
	...
	public function GetMinMaxGrwtUpdate($scode, $trid, $mingrwt, $maxgrwt) {
		$plantcode = $this->getPlantcode($scode);
		$flg=0;
...
// return "Update tbl_pnpslipsub SET pnpslipsub_slabelno='$slblno', pnpslipsub_mingrwt='$mingrwt', pnpslipsub_maxgrwt='$maxgrwt' pnpslipsub_lblflg=1 where pnpslipmain_id = $trid ";
	...
	/*public function GetQRcodeUpdate($scode, $trid, $scantype, $qrcode, $grossWeight) {
		...
		$stmtm = $this->conn_ps->prepare("SELECT pnpslipmain_id, pnpslipmain_date, pnpslipmain_crop, pnpslipmain_variety, pnpslipmain_promachcode, pnpslipmain_proopr, pnpslipmain_treattype, pnpslipmain_trtype, pnpslipmain_dop, yearid, pnpslipbar_logid FROM tbl_pnpslipmain WHERE pnpslipmain_id = ?");
		$stmtm->bind_param("i", $trid);
		$stmtm->execute();
		$stmtm->store_result();
		$arrivalcode=0;
		$pnpslipmain_id=''; $pnpslipmain_date=''; $pnpslipmain_crop=''; $pnpslipmain_variety=''; $pnpslipmain_promachcode=''; $pnpslipmain_proopr=''; $pnpslipmain_treattype=''; $pnpslipmain_ttype=''; $pnpslipmain_dop=''; $yearid=''; $pnpslipbar_logid='';
		if ($stmtm->num_rows > 0) {
			// user existed 
			$stmtm->bind_result($pnpslipmain_id, $pnpslipmain_date, $pnpslipmain_crop, $pnpslipmain_variety, $pnpslipmain_promachcode, $pnpslipmain_proopr, $pnpslipmain_treattype, $pnpslipmain_ttype, $pnpslipmain_dop, $yearid, $pnpslipbar_logid);
			$stmtm->fetch();
		}
		$stmtm->close();
		
		$stmt = $this->conn_ps->prepare("SELECT pnpslipsub_lotno, pnpslipsub_pickpqty, pnpslipsub_ups, pnpslipsub_wtmp, pnpslipsub_nop, pnpslipsub_wbnop, pnpslipsub_wbwt, pnpslipsub_wbinmp, pnpslipsub_remarks, pnpslipsub_qcdot, pnpslipsub_valupto, pnpslipsub_pouchccqty, pnpslipsub_pouchmrp, pnpslipsub_gmmrp, pnpslipsub_lblflg, pnpslipsub_slabelno, pnpslipsub_elabelno, pnpslipsub_plotno, pnpslipsub_id  FROM tbl_pnpslipsub WHERE pnpslipmain_id = ? ");
		$stmt->bind_param("i", $pnpslipmain_id);
		$result=$stmt->execute();
		$stmt->store_result();
		if ($stmt->num_rows > 0) {
			$stmt->bind_result($pnpslipsub_lotno, $pnpslipsub_pickpqty, $pnpslipsub_ups, $pnpslipsub_wtmp, $pnpslipsub_nop, $pnpslipsub_wbnop, $pnpslipsub_wbwt, $pnpslipsub_wbinmp, $pnpslipsub_remarks, $pnpslipsub_qcdot, $pnpslipsub_valupto, $pnpslipsub_pouchccqty, $pnpslipsub_pouchmrp, $pnpslipsub_gmmrp, $pnpslipsub_lblflg,  $pnpslipsub_slabelno, $pnpslipsub_elabelno, $pnpslipsub_plotno, $pnpslipsub_id);
			//looping through all the records
			$stmt->fetch();
		}	
		$stmt->close();
			
		$stmt600 = $this->conn_ps->prepare("INSERT INTO tbl_pnpslipbarcode (pnpslipsub_id, pnpslipmain_id, pnpslipbar_lotno, pnpslipbar_ups, pnpslipbar_barcode, pnpslipbar_wtmp, pnpslipbar_grosswt, pnpslipbar_wtdate, pnpslipbar_wtmcode, pnpslipbar_wtmopr, pnpslipbar_wtgrfr, pnpslipbar_wtgrto, pnpslipbar_bctype, pnpslipbar_logid, pnpslipbar_yearid) VALUES (?,?,?,?)  ");
		$stmt600->bind_param("iisi", $pnpslipsub_id, $pnpslipmain_id, $pnpslipsub_lotno, $pnpslipsub_ups, $qrcode, $pnpslipsub_wtmp, $grossWeight, $pnpslipsub_lotno, $pnpslipsub_lotno, $pnpslipsub_lotno, $pnpslipsub_lotno, $pnpslipsub_lotno, $pnpslipsub_lotno, $pnpslipsub_lotno, $yearid);
...
public function GetQRcodeUpdate($scode, $trid, $scantype, $qrcode, $grossWeight) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        $stmt_insert->bind_param(
            ...
            $scantype, $pnpslipbar_logid, $yearid, $plantcode
        );
        $result_insert = $stmt_insert->execute();
        if ($result_insert) {
