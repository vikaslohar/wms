...
public function GetTranSetupLotyrcodelist() {
	...
	public function GetUPSList($scode, $mobile1, $cropname, $varietyname) {
		...
		$stmt_ver = $this->conn_ps->prepare("SELECT varietyid, popularname, gm, wtmp FROM tblvariety WHERE popularname = ? ");
		$stmt_ver->bind_param("s", $varietyname);
		$result_ver=$stmt_ver->execute();
		$stmt_ver->store_result();
		if ($stmt_ver->num_rows > 0) {
			$stmt_ver->bind_result($varietyid, $popularname, $gm, $wtmp);
			//looping through all the records 
			$stmt_ver->fetch();
			$stmt_ver->close();
		}
...
//return $m;
	...
	public function GetTranDetails($scode,$trid, $mobile1) {
        ...
        if ($stmt->num_rows > 0) {
			...
			$wbcnt=0; $mpcnt=0; $wbwt=0; $wtmp=0; $totwbcnt=0;  $mpqrcflg=0; $mpbarcflg=0; $wb_mpqlinkflg=0; $wb_mpblinkflg=0;
	...
	public function GetLMCQRcodeUpdate($scode, $trid, $scantype, $qrcode, $cropname, $varietyname, $ups, $grosswt) {
		$plantcode = $this->getPlantcode($scode);
		...
		else if($scantype=="mpqrcode")
		{
			...
			if ($stmt_2->num_rows == 0) {
				...
				$stmt60->bind_param("ssssss", $dt, $trtyp, $qrcode, $one, $wtmp, $scode);
				$result60 = $stmt60->execute();
				if($result60){$flag=1;} 
				$stmt60->close();
			}
	...
	public function GetLMCWBDetails($scode, $cropname, $varietyname, $ups) {
		...
		$stmt_2->bind_param("siis", $scode, $cropid, $varietyid, $ups);
		$result2=$stmt_2->execute();
		$stmt_2->store_result();
		if ($stmt_2->num_rows == 0) {
			$userSR["wbcnt"] = $wbcnt;
