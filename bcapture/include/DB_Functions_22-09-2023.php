...
// New Code ------------------------------------------------	
	...
	function GetLMCQRScanningFinalize($scode, $trtype, $qrcode, $grosswt, $mptype, $wh_id, $bin_id, $subbin_id)
	{
		...
		$flg=0; $dt=date("Y-m-d"); $one=1; $two=2; $zero=0;
		$wh=''; $bin=''; $subbin='';
		if($trtype=="lmc") { $trtype='LMC'; $trtype2="PACKLMC";} if($trtype=="nlc") { $trtype='NLC'; $trtype2="PACKNLC";}
		...
		$stmt_24 = $this->conn_ps->prepare("SELECT lgenyearcode, lgenyear FROM tbl_lgenyearorder order by lgenyearid DESC ");
		//$stmt_2->bind_param("s", $pdate);
		$result24=$stmt_24->execute();
		$stmt_24->store_result();
		if ($stmt_24->num_rows > 0) {
			$stmt_24->bind_result($lgenyearcode, $lgenyear);
			//looping through all the records
			$stmt_24->fetch();
			$stmt_24->close();
		}
		...
		if ($stmt_wbs->num_rows > 0) {
			...
			while($stmt_wbs->fetch())
			{
				...
				$stmt_packsubsub->bind_param("iisssssssssss", $packm_id, $packsub_id, $wb_lotno, $wb_ups, $wb_mpwt, $whid, $binid, $subbinid, $balnop, $balnomp, $balqty, $nop, $qty, $zero, $plantcode);
		...
		$stmt_mpmain->bind_param("sissssssssssssssisss", $dt, $packm_id, $packagingtyp, $trtype2, $wb_crop, $wb_variety, $wb_lotno, $wb_ups, $wb_mpbarcode, $trtype, $wb_mpwt, $tnop, $lotnop, $wh_id, $bin_id, $subbin_id, $one, $lgenyearcode, $scode, $plantcode);
		...
		$stmt_packsstbl = $this->conn_ps->prepare("insert into tbl_packagingsub_sub (packaging_id, packagingsub_id, packagingsubsub_lotno, packagingsubsub_upssize, packagingsubsub_wtmp, extwh, extbin, extsubbin, packagingsubsub_extnop, packagingsubsub_extqty, packagingsubsub_nop, packagingsubsub_balpch, plantcode)  Values(?,?,?,?,?,?,?,?,?,?,?,?,?) ");
		$stmt_packsstbl->bind_param("iisssssssssss", $packm_id, $packsub_id, $wb_lotno, $wb_ups, $wb_mpwt, $wh_id, $bin_id, $subbin_id, $balnop, $balnomp, $balqty, $nop, $qty, $zero, $plantcode);
	...
	public function GetBinList($scode, $mobile1, $whid) {
		...
		$stmt_pnps = $this->conn_ps->prepare("SELECT binid, binname FROM tblbin WHERE whid = ? and plantcode=? ");
		$stmt_pnps->bind_param("is", $whid, $plantcode);
		$result_pnps=$stmt_pnps->execute();
		$stmt_pnps->store_result();
		if ($stmt_pnps->num_rows > 0) {
	...
	public function GetSubBinList($scode, $mobile1, $whid, $binid) {
		...
		$stmt_pnps = $this->conn_ps->prepare("SELECT sid, sname FROM tbl_subbin WHERE whid=? and binid=? and plantcode=? ");
		$stmt_pnps->bind_param("iis", $whid, $binid, $plantcode);
		$result_pnps=$stmt_pnps->execute();
		$stmt_pnps->store_result();
		if ($stmt_pnps->num_rows > 0) {
