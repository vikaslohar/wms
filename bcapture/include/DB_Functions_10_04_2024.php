...
//return "SELECT wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_totwt, wb_type FROM tbl_wbqrcode WHERE wb_linklogid='$scode' and wb_mpblinkflg!=1 and wb_mptype='$mptype' and wb_lotno='$wblotno' ";
	...
	public function GetRPSTranDetails($scode,$trid, $mobile1) {
		...
		$stmt = $this->conn_ps->prepare("SELECT  packaging_id, packaging_type, packaging_tdate, packaging_tcode, packaging_slipno,  packaging_crop, packaging_variety, packaging_lotno, packaging_upssize, packaging_remarks, packaging_mptype  FROM tbl_rpspackaging WHERE packaging_tflg=2 and packaging_id=? ORDER BY packaging_id ASC");
        $stmt->bind_param("i", $trid);
        $stmt->execute();
        $stmt->store_result();
		$userSR = array(); $user24=array();
        ...
        if ($stmt->num_rows > 0) {
			...
			while($stmt->fetch())
			{
				...
				if ($stmt_2->num_rows == 0) {
					...
					$userSR["remarks"] = $packagingsub_remarks;
	...
	public function GetRPSLblNoUpdate($scode, $trid, $lbltype, $lblno) {
		$plantcode = $this->getPlantcode($scode);
		$flg=0;
		...
		if($lbltype=="start")
		{
			...
			{$slblno=$lblno;}
			$stmt6 = $this->conn_ps->prepare("Update tbl_rpspackaging_sub SET packagingsub_slabelno=? where packaging_id = ? ");
			$stmt6->bind_param("si", $slblno, $trid);
		}
		else
		{
			...
			{$elblno=$lblno;}
			$stmt6 = $this->conn_ps->prepare("Update tbl_rpspackaging_sub SET packagingsub_elabelno=? where packaging_id = ? ");
			$stmt6->bind_param("si", $elblno, $trid);
		}
			...
			else if($scantype=="mpqrcode")
			{
				...
				if ($stmt_2->num_rows > 0) {
					...
					if($result60)
					{
						...
						$stmt600 = $this->conn_ps->prepare("DELETE FROM tbl_wbmpqrcode WHERE wb_mpqrcode=?  ");
						$stmt600->bind_param("s", $qrcode);
						$result600 = $stmt600->execute();
						$stmt600->close();
					} 
