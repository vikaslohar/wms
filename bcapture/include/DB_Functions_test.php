...
//return "SELECT wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_totwt, wb_type FROM tbl_wbqrcode WHERE wb_linklogid='$scode' and wb_mpblinkflg!=1 and wb_mptype='$mptype' and wb_lotno='$wblotno' ";
			...
			$stmt_2 = $this->conn_ps->prepare("SELECT wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_totwt, wb_type FROM tbl_wbqrcode WHERE wb_linklogid=? and wb_mpblinkflg!=1 and wb_mptype=? and wb_lotno LIKE '%$wblotno%' ");
			$stmt_2->bind_param("ss", $scode, $mptype);
			$result2=$stmt_2->execute();
			$stmt_2->store_result();
			if ($stmt_2->num_rows > 0) {
	...
	function GetMMCQRScanningFinalize($scode, $trtype, $qrcode, $grosswt, $mptype, $wh_id, $bin_id, $subbin_id)
	{
		...
		if ($stmt_wbs->num_rows > 0) {
			...
			$packagingtyp="Packaging";
			$stmt_mpmain = $this->conn_ps->prepare("insert into tbl_mpmain ( mpmain_date, mpmain_trid, mpmain_trtyp, mpmain_trtype, mpmain_crop, mpmain_variety, mpmain_lotno, mpmain_upssize, mpmain_barcode, mpmain_mptype, mpmain_wtmp, mpmain_mptnop, mpmain_lotnop, mpmain_wh, mpmain_bin, mpmain_subbin, mpmain_tflg, mpmain_yearcode, mpmain_logid, plantcode )  Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
			$stmt_mpmain->bind_param("sissssssssssssssisss", $dt, $packm_id, $packagingtyp, $trtype2, $crops, $varieties, $lotnos, $upsar, $wb_mpbarcode, $mptype, $wb_mpwt, $tnop, $lotnop, $wh_id, $bin_id, $subbin_id, $one, $lgenyearcode, $scode, $plantcode);
			$result_mpmain = $stmt_mpmain->execute();
			if($result_mpmain){$flg=$flg+1;}  
			$stmt_mpmain->close();	
