...
public function GetTranSetupLotyrcodelist() {
	...
	public function GetUPSList($scode, $mobile1, $cropname, $varietyname) {
		$plantcode = $this->getPlantcode($scode);
		$flg=0; $userSR=array();
		$stmt_crop = $this->conn_ps->prepare("SELECT cropid, croptype, cropname FROM tblcrop WHERE cropname = ? ");
		$stmt_crop->bind_param("s", $cropname);
		...
		/*
		$stmt_2 = $this->conn_ps->prepare("SELECT distinct pnpslipmain_id FROM tbl_pnpslipmain WHERE pnpslipmain_trtype = 'wb' and pnpslipmain_crop=? and pnpslipmain_variety=?");
		...
		if ($stmt_2->num_rows > 0) {
			...
			while($stmt_2->fetch())
			{
				$stmt_pnps = $this->conn_ps->prepare("SELECT pnpslipsub_ups FROM tbl_pnpslipsub WHERE pnpslipmain_id = ? ");
				$stmt_pnps->bind_param("i", $pnpslipmain_id);
				$result_pnps=$stmt_pnps->execute();
				$stmt_pnps->store_result();
				if ($stmt_pnps->num_rows > 0) {
					$stmt_pnps->bind_result($pnpslipsub_ups);
					//looping through all the records 
					$stmt_pnps->fetch();
					array_push($userSR, $pnpslipsub_ups);
					$stmt_pnps->close();
				}
			}
			$stmt_2->close();
	...
	public function GetLotList($scode, $mobile1, $cropname, $varietyname, $ups) {
		$plantcode = $this->getPlantcode($scode);
		$flg=0; $userSR=array(); $user10=array();
		$stmt_crop = $this->conn_ps->prepare("SELECT cropid, croptype, cropname FROM tblcrop WHERE cropname = ? ");
		$stmt_crop->bind_param("s", $cropname);
	...
	public function GetTranDetails($scode,$trid, $mobile1) {
        ...
        if ($stmt->num_rows > 0) {
			...
			if ($stmt_2->num_rows == 0) {
				...
				$stmt_3 = $this->conn_ps->prepare("SELECT DISTINCT wb_mpqrcode FROM tbl_wbqrcode WHERE wb_pnptrid = ? ");
				...
				while($stmt_3->fetch())
				{
					if($wb_mpqrcode!="")
					{
						$stmt_4 = $this->conn_ps->prepare("SELECT wb_mpwt FROM tbl_wbqrcode WHERE wb_pnptrid = ? and wb_mpqrcode=?");
	...
	public function GetQRcodeUpdate($scode, $trid, $scantype, $qrcode) {
		...
		{
			$stmt_2 = $this->conn_ps->prepare("SELECT wb_mpqrcode FROM tbl_wbqrcode WHERE wb_mpqrcode = ? ");
			$stmt_2->bind_param("s", $qrcode);
			$result2=$stmt_2->execute();
			$stmt_2->store_result();
			if ($stmt_2->num_rows == 0) {
	...
	public function GetLMCQRcodeUpdate($scode, $trid, $scantype, $qrcode, $cropname, $varietyname, $ups, $grosswt, $mptype, $mpwt) {
		...
		if($scantype=="wbqrcode")
		{
			$stmt_2 = $this->conn_ps->prepare("SELECT wb_extqrcode FROM tbl_wbqrcode WHERE wb_extqrcode = ? and wb_crop=? and wb_variety=? and wb_ups=? ");
		...
		else if($scantype=="mpqrcode")
		{
			...
			if ($stmt_2->num_rows == 0) {
				...
				$stmt60->bind_param("ssssss", $dt, $trtyp, $qrcode, $one, $mpwt, $scode);
				$result60 = $stmt60->execute();
				if($result60){$flag=1;} 
				$stmt60->close();
			}
		...
		else if($scantype=="mpbarcode")
		{
			...
			if ($stmt_2->num_rows == 0) {
				...
				$stmt60->bind_param("siss", $qrcode, $one, $grosswt, $scode);
				$result60 = $stmt60->execute();
				if($result60){$flag=1;} 
				$stmt60->close();
			}
	...
	public function GetLWBFinalize($scode, $qrcode, $lotarray) {
		...
		if ($stmt_2->num_rows > 0) {
			...
			if ($stmtqrchk->num_rows > 0) {
				...
				foreach($lotar as $lotary)
				{
					if($lotary<>"")
					{
						$lotary2=explode("-", $lotary);
						...
						$stmt_ldgraw0->bind_param("s", $lotary2[0]);
						$result2=$stmt_ldgraw0->execute();
						$stmt_ldgraw0->store_result();
						if ($stmt_ldgraw0->num_rows > 0) {
						...
						$pchqty=$ptp1*$lotary2[1];
						if($lots!="") { $lots=$lots.",".$lotary2[0]; } else { $lots=$lotary2[0]; }
						if($pch!="") { $pch=$pch.",".$lotary2[1]; } else { $pch=$lotary2[1]; }
						if($qty!="") { $qty=$qty.",".$pchqty; } else { $qty=$pchqty; }
						
						$trtyp2='LMC';
				
						$stmt_wbqrsub = $this->conn_ps->prepare("insert into tbl_wbqrcode_sub (wb_id, wb_extqrcode, wb_type, wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_mptype)  Values(?,?,?,?,?,?,?,?,?,?) ");
						$stmt_wbqrsub->bind_param("issssssiss", $wb_id, $qrcode, $trtyp, $lotldg_crop, $lotldg_variety, $packtype, $lotary2[0], $lotary2[1], $pchqty, $trtyp2);
						$result_wbqrsub = $stmt_wbqrsub->execute();
						$stmt_wbqrsub->close();
...
// New Code ------------------------------------------------	
	...
	function GetLMCQRScanningFinalize($scode, $trtype, $qrcode, $grosswt)
	{
		$plantcode = $this->getPlantcode($scode);
		$flg=0; $dt=date("Y-m-d"); $one=1; $two=2; $zero=0;
		
		$stmtm = $this->conn_ps->prepare("SELECT  wb_extqrcode, wb_type, wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_mptype, wb_mpqrcode, wb_mpbarcode, wb_mpwt, wb_mpgrosswt  FROM tbl_wbqrcode WHERE wb_mpqrcode = ?");
		$stmtm->bind_param("s", $qrcode);
		$stmtm->execute();
		$stmtm->store_result();
		$arrivalcode=0;
		$wb_extqrcode=""; $wb_type=""; $wb_crop=""; $wb_variety=""; $wb_ups=""; $wb_lotno=""; $wb_nop=""; $wb_qty=""; $wb_mptype=""; $wb_mpqrcode=""; $wb_mpbarcode=""; $wb_mpwt=""; $wb_mpgrosswt="";
		
		if ($stmtm->num_rows > 0) {
			// user existed 
			$stmtm->bind_result($wb_extqrcode, $wb_type, $wb_crop, $wb_variety, $wb_ups, $wb_lotno, $wb_nop, $wb_qty, $wb_mptype, $wb_mpqrcode, $wb_mpbarcode, $wb_mpwt, $wb_mpgrosswt);
			$stmtm->fetch();
			/*while($stmtm->fetch())
			{
		...
		$packtype='LMC';
		$qtydc=0; $nobdc=0; $tarewt=0;
		$stmt_packm = $this->conn_ps->prepare("Insert into tbl_packaging (packaging_type, packaging_tdate, packaging_tcode, packaging_date, packaging_yearid, packaging_logid, packaging_barcode, packaging_mptype, packaging_trtype, plantcode, packaging_tflg) Values(?,?,?,?,?,?,?,?,?,?,?) ");
		$stmt_packm->bind_param("ssisssssssi", $arrival_id, $lotimpid, $lotcrop, $popularname, $qtydc, $nobdc, $tarewt, $lotno, $orlot, $exoldlot, $pdndate, $pdnno, $lotspcodef, $lotspcodem, $lotorganiser, $lotfarmer, $lotploc, $lotstate, $lotpper, $lotplotno, $stage, $prodtype, $plantcode);
		...
		$stmt_packsub->bind_param("isss", $arrival_id, $lotimpid, $lotcrop, $popularname, $qtydc, $nobdc, $tarewt, $lotno, $orlot, $exoldlot, $pdndate, $pdnno, $lotspcodef, $lotspcodem, $lotorganiser, $lotfarmer, $lotploc, $lotstate, $lotpper, $lotplotno, $stage, $prodtype, $plantcode);
	
		$result_packsub = $stmt_packsub->execute();
		$packsub_id=$stmt_packsub->insert_id;
		$stmt_packsub->close();
		
		$stmt_packsubsub = $this->conn_ps->prepare("insert into tbl_packagingsub_sub (packaging_id, packagingsub_id, packagingsubsub_lotno, packagingsubsub_upssize, packagingsubsub_wtmp, packagingsubsub_wtnop, extwh, extbin, extsubbin, packagingsubsub_extnop, packagingsubsub_extqty, packagingsubsub_nop, packagingsubsub_nomp, packagingsubsub_balpch, packagingsubsub_barcodes, packagingsubsub_sltyp, plantcode)  Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
		$stmt_packsubsub->bind_param("iisssssssssssssss", $trid, $arrsub_id, $exoldlot, $grosswt, $netqty, $trwt, $plantcode);
		$result_packsubsub = $stmt_packsubsub->execute();
		$stmt_packsubsub->close();
		
		$stmt_packsubsub2 = $this->conn_ps->prepare("insert into tbl_packagingsub_sub2 (packaging_id, packagingsub_id, packagingsubsub_lotno, packagingsubsub_upssize, packagingsubsub_wh, packagingsubsub_bin, packagingsubsub_subbin, packagingsubsub_nomp, packagingsubsub_nopch, packagingsubsub_totpch, packagingsubsub_totqty, plantcode)  Values(?,?,?,?,?,?,?,?,?,?,?,?) ");
		$stmt_packsubsub2->bind_param("iisssssiiiss", $trid, $arrsub_id, $exoldlot, $grosswt, $netqty, $trwt, $plantcode);
		$result_packsubsub2 = $stmt_packsubsub2->execute();
		$stmt_packsubsub2->close();
		*/
	...
	public function GetQRcodeDelete($scode, $trid, $scantype, $qrcode, $trtype) {
		...
		if($trtype=="smc")
		{
			if($scantype=="wbqrcode")
			{
				...
				if ($stmt_2->num_rows > 0) {
					...
					$stmtqrchk = $this->conn_ps->prepare("SELECT wb_crop, wb_variety, wb_ups, wb_id FROM tbl_wbqrcode WHERE wb_extqrcode = ? and wb_actflg>0");
					$stmtqrchk->bind_param("s", $qrcode);
					$result_qrchk=$stmtqrchk->execute();
					$stmtqrchk->store_result();
					if ($stmtqrchk->num_rows > 0) {
			...
			{
				$stmt_2 = $this->conn_ps->prepare("SELECT wb_crop, wb_variety, wb_ups, wb_id FROM tbl_wbqrcode WHERE wb_mpqrcode = ? ");
				$stmt_2->bind_param("s", $qrcode);
				$result2=$stmt_2->execute();
				$stmt_2->store_result();
				if ($stmt_2->num_rows > 0) {
		...
		else
		{
			...
			{
				$stmt_2 = $this->conn_ps->prepare("SELECT wb_crop, wb_variety, wb_ups FROM tbl_wbqrcode WHERE wb_mpqrcode = ? ");
				$stmt_2->bind_param("s", $qrcode);
				$result2=$stmt_2->execute();
				$stmt_2->store_result();
				if ($stmt_2->num_rows > 0) {
