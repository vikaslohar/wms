...
if(isset($_REQUEST['scode'])) {
	...
	{
		$chk_barcode = $db->isBarCodeExistedOFL($qrcode, $trid, 0);
		//	print_r($user_login);
	...
	if($flg==0)
	{
		...
		{
			if($reason==""){
				$user_trlist = $db->GetQRcodeUpdateOFL($scode, $trid, $scantype, $qrcode, $grossWeight);
				...
				$chk_barcode = $db->updateReasonOFL($qrcode, $trid, $reason);
				//print_r($user_trlist); exit;
