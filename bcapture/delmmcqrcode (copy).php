...
if(isset($_REQUEST['scode'])) {
	...
	if($scantype!="mpbarcode")
	{
		...
		if(count($qrcode2)!=13)
		{
			...
			$user_mmcdetails=$db->GetMMCqtDetails($scode,$trtype);
			$response["user"] = $user_mmcdetails;
			$user_wbdetails=$db->GetMMCWBDetails($scode,$trtype);
	...
	if($flg==0)
	{
		...
		{
			// user is found
			$user_trlist = $db->GetMMCQRcodeDelete($scode, $trid, $scantype, $qrcode, $trtype);
			//	print_r($user_login);
