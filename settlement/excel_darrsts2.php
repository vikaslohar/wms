...
function dateDiff($start, $end) 
{
  ...
  return round($diff / 86400);
	...
	$dh="Periodical_Arrival_Trading_Report_As_on_Date_".$dat;
	$datahead = array("Periodical Trading Arrival Report - As on Date ".$dat);
	$filename=$dh.".xls";  
...
$cnt=1;
$qry="select arrival_id, arrival_date, yearcode, tmode, trans_name, trans_lorryrepno, trans_vehno, trans_paymode, courier_name, docket_no, pname_byhand from tblarrival where plantcode='$plantcode' AND arrival_date <='".$edate."' and arrival_date >='".$sdate."' and arrival_type='Trading' and arrtrflag=1";
$sql_arr_home1=mysqli_query($link,$qry) or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home1);
	
$datahead3= array("Date of Arrival","FRN No.","Lot No.","Crop","Variety","Type","PDN Qty","Arrival Qty","Raw Seed Qty","Condition Seed Qty","Cond. Loss (RM + IM)","Date of Processing","GOT Result","DOGR","QC Result","DoT","Germination %","Genetic Purity %","ForTool","ForYear","Agreement ID","lotno","Mode of Transport","Transport Name","Lorry Receipt No.","Vehicle No.","Payment Mode","Courier Name","Docket No.","By Hand Person Name"); 
...
$sql_rr=mysqli_query($link,"select * from tblarrival_sub where plantcode='$plantcode' AND arrival_id='".$row_arr_home1['arrival_id']."' order by orlot asc") or die(mysqli_error($link));
...
$farmerid=''; $qcstatus=''; $dot='';  $fortool=''; $foryear=''; $agreementid=''; $lotimp='';
...
$sql_qctbl=mysqli_query($link,"select MIN(tid) from tbl_qctest where plantcode='$plantcode' AND oldlot='".$row_rr['orlot']."'") or die(mysqli_error($link)); 
$row_qctbl=mysqli_fetch_array($sql_qctbl);

$sql_qctbl2=mysqli_query($link,"select * from tbl_qctest where plantcode='$plantcode' AND oldlot='".$row_rr['orlot']."' and tid='".$row_qctbl[0]."'") or die(mysqli_error($link)); 
$row_qctbl2=mysqli_fetch_array($sql_qctbl2);

$sql_qctbl3=mysqli_query($link,"select MAX(tid) from tbl_qctest where plantcode='$plantcode' AND oldlot='".$row_rr['orlot']."' and sampleno='".$row_qctbl2['sampleno']."' and yearid='".$row_qctbl2['yearid']."'") or die(mysqli_error($link)); 
$row_qctbl3=mysqli_fetch_array($sql_qctbl3);

$sql_qctbl4=mysqli_query($link,"select * from tbl_qctest where plantcode='$plantcode' AND tid='".$row_qctbl3[0]."'") or die(mysqli_error($link)); 
...
if($tot_rr > 0)
{
$data1[$d]=array($trdate1,$frn,$lotno1,$crpnm,$vername,$vernm,$pqty,$arrqty,$rswqty,$cswqty,$tplqty,$prsldate,$gotr,$dogr,$qcstatus,$dot,$gemp,$genpurity,$fortool,$foryear,$agreementid,$lotimp, $tmode, $trans_name, $trans_lorryrepno, $trans_vehno, $trans_paymode, $courier_name, $docket_no, $pname_byhand); 
$d++;$cntt++;
}
}
}
