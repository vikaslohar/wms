...
function dateDiff($start, $end) 
{
  ...
  return round($diff / 86400);
	...
	$dh="Organiser_wise_Processing_Status_Report_for_".$typ11."_As_on_Date_".$dat;
	$datahead = array("Organiser wise Processing Status Report - As on Date ".$dat);
	$filename=$dh.".xls";  
...
$cnt=1;
$d=1;
$datahead3= array("Date of Arrival","Days","FRN No.","Organiser","Crop","Variety","Variety Type","Lot No.","Prod. Location","Prod. Personel","Farmer","PDN Qty","Arrival Qty","Raw Seed Qty","Condition Seed Qty","Cond. Loss (RM + IM)","Date of Processing","GOT Result","DOGR","QC Result","DOT","Physical Purity","Germination %","First Germination %","Genetic Purity %","Farmer ID","ForTool","ForYear","Agreement ID","lotno"); 


if($txtpmcode2!="ALL")
{	
	$qryorg="select distinct organiser from tblarrival_sub where plantcode='$plantcode' AND organiser='".$txtpmcode2."' and sstage='Raw' order by organiser ";
}
else
{
	$qryorg="select distinct organiser from tblarrival_sub where plantcode='$plantcode' and sstage='Raw' order by organiser ";
}	

$sql_arr_org=mysqli_query($link,$qryorg) or die(mysqli_error($link));
$tot_arr_org=mysqli_num_rows($sql_arr_org);
	
while($row_arr_org=mysqli_fetch_array($sql_arr_org))
{
$txtpmcode=$row_arr_org['organiser'];

if($crop!="ALL" && $variety!="ALL")
{	
	$qry24="select distinct lotcrop from tblarrival_sub where plantcode='$plantcode' AND organiser='".$txtpmcode."' and lotcrop='".$crp."' and lotvariety='".$ver."'  and sstage='Raw' order by lotcrop";
}
else if($crop!="ALL" && $variety=="ALL")
{
	$qry24="select distinct lotcrop from tblarrival_sub where plantcode='$plantcode' AND organiser='".$txtpmcode."' and lotcrop='".$crp."' and sstage='Raw' order by lotcrop";
}
else
{
	$qry24="select distinct lotcrop from tblarrival_sub where plantcode='$plantcode' AND organiser='".$txtpmcode."' and sstage='Raw' order by lotcrop";
}	
...
if($variety!="ALL")
{
	$qry23="select distinct lotvariety from tblarrival_sub where plantcode='$plantcode' AND organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and lotvariety='".$ver."' and sstage='Raw' order by lotvariety";
}
else
{
	$qry23="select distinct lotvariety from tblarrival_sub where plantcode='$plantcode' AND organiser='".$txtpmcode."' and lotcrop='".$row_arr_home24['lotcrop']."' and sstage='Raw' order by lotvariety";
}	
...
$qry="select arrival_id, arrival_date, yearcode from tblarrival where plantcode='$plantcode' AND arrival_date <='".$edate."' and arrival_date >='".$sdate."' and arrival_type='Fresh Seed with PDN' and arrtrflag=1 and arrival_id IN($arid) order by arrival_date ASC";
...
$farmerid=''; $fortool=''; $foryear=''; $agreementid=''; $lotimp='';
...
$fortool=$row_lotimp['fortool']; 
$foryear=$row_lotimp['foryear'];
$agreementid=$row_lotimp['agreement_id'];
$lotimp=$row_lotimp['lotnumber'];
}
...
$varietytype='';
	...
	$vername=$row_var23['popularname'];
	$varietytype=$row_var23['vt'];
...
{
	$tstdate1=$rowqc['testdate'];
	$tsyear1=substr($tstdate1,0,4);
	$tsmonth1=substr($tstdate1,5,2);
	$tsday1=substr($tstdate1,8,2);
	$qcdot=$tsday1."/".$tsmonth1."/".$tsyear1;
}
$gemp2='';
$sqlqc2=mysqli_query($link,"select * from tbl_qctest where plantcode='$plantcode' AND lotno='$lotno' and qcstatus IN ('OK','Fail','BL') order by tid ASC LIMIT 0,1") or die(mysqli_error($link));
$rowqc2=mysqli_fetch_array($sqlqc2);
$gemp2=$rowqc2['gemp'];
...
if($tot_rr > 0)
{
$data1[$d]=array($trdate1,$diff,$frn,$txtpmcode,$crpnmae,$vername,$varietytype,$lotno1,$ploc,$pper,$farmer,$pqty,$arrqty,$rswqty,$cswqty,$tplqty,$prsldate,$gotr,$dogr,$qcstatuss,$qcdot,$seedsstatus,$gemp,$gemp2,$genpurity,$farmerid,$fortool,$foryear,$agreementid,$lotimp); 
$d++;
}
