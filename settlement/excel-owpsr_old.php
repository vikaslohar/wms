...
$qry="select arrival_id, arrival_date, yearcode from tblarrival where plantcode='$plantcode' AND arrival_date <='".$edate."' and arrival_date >='".$sdate."' and arrival_type='Fresh Seed with PDN' and arrtrflag=1 and arrival_id IN($arid)";
	$sql_arr_home1=mysqli_query($link,$qry) or die(mysqli_error($link));
	$tot_arr_home=mysqli_num_rows($sql_arr_home1);
	
$datahead3[$cnt] = array("Date of Arrival","Days","FRN No.","Lot No.","Prod. Location","Prod. Personel","Farmer","PDN Qty","Arrival Qty","Raw Seed Qty","Condition Seed Qty","Cond. Loss (RM + IM)","Date of Processing","GOT Result","DOGR","QC Result","DOT","Physical Purity","Germination %","First Germination %","Genetic Purity %","Farmer ID","ForTool","ForYear","Agreement ID","lotno"); 
...
/*if($crop!="ALL" && $variety!="ALL")
{	
	$sql_rr=mysqli_query($link,"select * from tblarrival_sub where plantcode='$plantcode' AND arrival_id='".$row_arr_home1['arrival_id']."' and lotcrop='".$crp."' and lotvariety='".$ver."' and organiser='".$txtpmcode."' and sstage='Raw' order by orlot asc") or die(mysqli_error($link));
}
if($crop!="ALL" && $variety=="ALL")
{	
	$sql_rr=mysqli_query($link,"select * from tblarrival_sub where plantcode='$plantcode' AND arrival_id='".$row_arr_home1['arrival_id']."' and lotcrop='".$crp."' and organiser='".$txtpmcode."' and sstage='Raw' order by orlot asc") or die(mysqli_error($link));
}

if($crop=="ALL" && $variety=="ALL")
{	
	$sql_rr=mysqli_query($link,"select * from tblarrival_sub where plantcode='$plantcode' AND arrival_id='".$row_arr_home1['arrival_id']."' and organiser='".$txtpmcode."' and sstage='Raw' order by orlot asc") or die(mysqli_error($link));
}
else
{
	$sql_rr=mysqli_query($link,"select * from tblarrival_sub where plantcode='$plantcode' AND arrival_id='".$row_arr_home1['arrival_id']."' and organiser='".$txtpmcode."' and sstage='Raw' order by orlot asc") or die(mysqli_error($link));
...
$gemp2='';
$sqlqc2=mysqli_query($link,"select * from tbl_qctest where plantcode='$plantcode' AND lotno='$lotno' and qcstatus IN ('OK','Fail','BL') order by tid ASC LIMT 0,1") or die(mysqli_error($link));
...
if($tot_rr > 0)
{
$data1[$cnt][$d]=array($trdate1,$diff,$frn,$lotno1,$ploc,$pper,$farmer,$pqty,$arrqty,$rswqty,$cswqty,$tplqty,$prsldate,$gotr,$dogr,$qcstatuss,$qcdot,$seedsstatus,$gemp,$gemp2,$genpurity,$farmerid,$fortool,$foryear,$agreementid,$lotimp); 
$d++;$cntt++;
}
