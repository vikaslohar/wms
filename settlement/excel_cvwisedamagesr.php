...
session_start();
	...
	$dh="Crop_Variety_wise_Damage_Sales_Return_Report";
	$datahead = array("Crop Variety wise Damage Sales Return Report");
	$filename=$dh.".xls";  
...
$mid=""; $cnt=0; $d=1;
$sqlsrrtm="select * from tbl_salesrv where plantcode='$plantcode' AND salesr_date<='".$edt."' and salesr_date>='".$sdt."' and salesr_trtype='Sales Return' ";
if($txtstatesl!="ALL")
{
		...
		while($row_tbl_sub1=mysqli_fetch_array($sql_srrets1))
		{
			...
			while($row_tbl_sub=mysqli_fetch_array($sql_srrets))
			{
				...
				*/
				...
				//if($tot_arr_home > 0)
				{	
					$data1[$d]=array($d,$state,$ptype,$cropn,$varietyn,$slups,$difq3);
					$d++;$cnt++;
				}
			}
			$data1[$d]=array("","","","","","Total",$tndqt);
			$d++;	
		}
	}	
	if($cnt==0)
