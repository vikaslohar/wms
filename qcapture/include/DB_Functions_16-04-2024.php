...
public function GetTranSetupLotyrcodelist() {
	...
	public function GetLblNoUpdate($scode, $trid, $lbltype, $lblno, $macoprname) {
		$plantcode = $this->getPlantcode($scode);
		$flg=0;
		if($macoprname!=''){
		...
		if ($stmt_2->num_rows > 0) {
			...
			$stmt60->bind_param("sii", $dt, $proopr_id, $trid);
			$result60 = $stmt60->execute();
			//if($result60){$flg=1;}
			$stmt60->close();
	...
	public function GetTranDetails($scode,$trid, $mobile1) {
        ...
        if ($stmt->num_rows > 0) {
			...
			if ($stmt_2->num_rows == 0) {
				...
				if($pnpslipsub_qcdttype!='DoT' && $pnpslipsub_qcdot=="0000-00-00"){
