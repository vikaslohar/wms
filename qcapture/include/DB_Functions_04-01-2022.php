...
//	private $conn_vnr;
	...
	public function GetTrList($scode) {
        $stmt = $this->conn_ps->prepare("SELECT arrival_id, arrival_code, disp_date, arrsetupflag, setuplogid, arrunldflag, unldlogid, arrtrflag, logid, arrtrnunldtype, dc_date, dcno FROM tblarrival_unld WHERE arrtrflag!=1 and arrtrnunldtype='online'  ORDER BY arrival_id DESC");
        //$stmt->bind_param("ss", $email, $password);
        $stmt->execute();
        $stmt->store_result();
				...
				if($jdata['unloadingData'][$i]<>"")
				{
					...
					if ($stmt_arsub->num_rows == 0) 
					{
						...
						if ($stmt_lotimp->num_rows > 0) {
							...
							$stmt_arrsub = $this->conn_ps->prepare("Insert into tblarrival_sub_unld (arrival_id, lotimpid, lotcrop, lotvariety, qty, act1, tarewt, lotno, orlot, old, pdndate, pdnno, spcodef, spcodem, organiser, farmer, ploc, lotstate, pper, plotno, sstage, prodtype) Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
							$stmt_arrsub->bind_param("iissssssssssssssssssss", $arrival_id, $lotimpid, $lotcrop, $popularname, $qtydc, $nobdc, $tarewt, $lotno, $orlot, $exoldlot, $pdndate, $pdnno, $lotspcodef, $lotspcodem, $lotorganiser, $lotfarmer, $lotploc, $lotstate, $lotpper, $lotplotno, $stage, $prodtype);
						...
						{
							$stmt_arrsub = $this->conn_ps->prepare("insert into tblarrival_sub_unld (arrival_id, old, lotno, orlot)  Values(?,?,?,?) ");
							$stmt_arrsub->bind_param("isss", $trid, $exoldlot, $lotno, $orlot);
							$result_arrsub = $stmt_arrsub->execute();
							$arrsub_id=$stmt_arrsub->insert_id;
							$stmt_arrsub->close();
