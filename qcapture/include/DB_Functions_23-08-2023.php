...
//	private $conn_vnr;
	...
	public function GetTrList($scode, $mobile1) {
		$plantcode = $this->getPlantcode($scode);
         $stmt = $this->conn_ps->prepare("SELECT pnpslipmain_id, pnpslipmain_date, pnpslipmain_crop, pnpslipmain_variety, pnpslipmain_promachcode, pnpslipmain_proopr, pnpslipmain_treattype, pnpslipmain_ttype FROM tbl_pnpslipmain WHERE pnpslipmain_tflag=2 and pnpslipmain_trtype='wb' AND plantcode=? ORDER BY pnpslipmain_id ASC");
	...
	public function GetQRcodeUpdate($scode, $trid, $scantype, $qrcode) {
		...
		if ($stmt_2->num_rows > 0) {
			...
			else if($scantype=="mpqrcode")
			{
				...
				$stmt60 = $this->conn_ps->prepare("UPDATE tbl_wbqrcode SET wb_linkdate=?, wb_mptype=?, wb_mpqrcode=?, wb_mpqlinkflg=?, wb_mpwt=? where wb_pnptrid=? and wb_lotno=? and wb_mpqrcode='' and wb_mpqlinkflg=0 ");
				$stmt60->bind_param("ssssiis", $dt, $trtyp, $qrcode, $one, $pnpslipsub_wtmp, $trid, $pnpslipsub_lotno);
				$result60 = $stmt60->execute();
				if($result60){$flg=1;} 
				$stmt60->close();
			}
			else if($scantype=="mpbarcode")
			{
				...
				$stmt60 = $this->conn_ps->prepare("UPDATE tbl_wbqrcode SET wb_mpbarcode=?, wb_mpblinkflg=? where wb_pnptrid=? and wb_lotno=? and wb_mpbarcode='' and wb_mpblinkflg=0 and wb_mpqlinkflg=1 ");
				$stmt60->bind_param("siis", $qrcode, $one, $trid, $pnpslipsub_lotno);
				$result60 = $stmt60->execute();
				if($result60){$flg=1;} 
				$stmt60->close();
			}
