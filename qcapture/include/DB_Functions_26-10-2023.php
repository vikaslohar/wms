...
// New Code ------------------------------------------------	
	...
	public function GetQRcodeDelete($scode, $trid, $scantype, $qrcode, $trtype) {
		...
		else if($trtype=="lmc" || $trtype=="Lmc" || $trtype=="LMC")
		{
			if($scantype=="wbqrcode")
			{
				$stmt_2 = $this->conn_ps->prepare("SELECT wb_extqrcode FROM tbl_wbqrcode WHERE wb_extqrcode = ? and wb_mpqlinkflg!=1 ");
	...
	public function GetMMCQRcodeUpdate($scode, $trid, $scantype, $qrcode, $grosswt, $mptype) {
		...
		$wbcnt=0; $mpcnt=0; $wbwt=0; $wtmp=0; $totwbcnt=0; 
		$stmt_2 = $this->conn_ps->prepare("SELECT wb_qty, wb_type FROM tbl_wbqrcode WHERE wb_linklogid=? and wb_mpqlinkflg=2 and wb_mptype=? ");
	...
	public function GetMMCWBDetails($scode, $mptype) {
		...
		$stmt_1 = $this->conn_ps->prepare("SELECT distinct wb_lotno FROM tbl_wbqrcode WHERE wb_linklogid=? and wb_mpqlinkflg=2 and wb_mptype=?  ");
		...
		{
			$userSR=array(); 
			$wb_crop='';  $wb_variety=''; $wb_ups=''; $wb_lotno=''; $wb_nop=0; $wb_qty=0; $wb_totwt=0;
		
			$stmt_2 = $this->conn_ps->prepare("SELECT wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_totwt, wb_type FROM tbl_wbqrcode WHERE wb_linklogid=? and wb_mpqlinkflg=2 and wb_mptype=? and wb_lotno=? ");
			$stmt_2->bind_param("sss", $scode, $mptype, $wblotno);
			$result2=$stmt_2->execute();
			$stmt_2->store_result();
			if ($stmt_2->num_rows > 0) {
				...
				$stmt_ldgraw0->bind_param("s", $wblotno);
				$result2=$stmt_ldgraw0->execute();
				$stmt_ldgraw0->store_result();
				if ($stmt_ldgraw0->num_rows > 0) {
					...
					$userSR["ups"] = $wb_ups;
					$userSR["qty"] = $wtmp;
					$userSR["dop"] = $lotldg_dop;
					$userSR["dov"] = $lotldg_valupto;
					array_push($user24, $userSR);
				}
			}
		}
...
//return "SELECT  wb_extqrcode, wb_type, wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_mptype, wb_mpqrcode, wb_mpbarcode, wb_mpwt, wb_mpgrosswt, wb_id  FROM tbl_wbqrcode WHERE wb_mpbarcode = '$qrcode'";		
