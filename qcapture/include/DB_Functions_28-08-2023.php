...
public function GetTranSetupLotyrcodelist() {
	...
	public function GetUPSList($scode, $mobile1, $cropname, $varietyname) {
		...
		$stmt_ver = $this->conn_ps->prepare("SELECT varietyid, popularname, gm FROM tblvariety WHERE popularname = ? ");
		$stmt_ver->bind_param("s", $varietyname);
		$result_ver=$stmt_ver->execute();
		$stmt_ver->store_result();
		if ($stmt_ver->num_rows > 0) {
			$stmt_ver->bind_result($varietyid, $popularname, $gm);
			//looping through all the records 
			$stmt_ver->fetch();
			$stmt_ver->close();
		}
		...
		$stmt_ups = $this->conn_ps->prepare("SELECT wt, ups, uom FROM tblups WHERE uid IN($gm) ");
		//$stmt_ups->bind_param("s", $gm);
		$result_ups=$stmt_ups->execute();
		$stmt_ups->store_result();
		if ($stmt_ups->num_rows > 0) {
			$stmt_ups->bind_result($wt, $ups, $uom);
			//looping through all the records 
