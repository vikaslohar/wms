...
//	private $conn_vnr;
	...
	public function GetTrList($scode, $mobile1) {
		$plantcode = $this->getPlantcode($scode);
         $stmt = $this->conn_ps->prepare("SELECT pnpslipmain_id, pnpslipmain_date, pnpslipmain_crop, pnpslipmain_variety, pnpslipmain_promachcode, pnpslipmain_proopr, pnpslipmain_treattype, pnpslipmain_ttype, pnpslipmain_packlinetype, pnpslipmain_manualtype, pnpslipmain_printflg FROM tbl_pnpslipmain WHERE pnpslipmain_tflag=2 and pnpslipmain_trtype='wb' AND pnpslipmain_wbactflag=0 AND plantcode=? AND pnpslipmain_packlinetype!='Manual' ORDER BY pnpslipmain_id ASC");
        $stmt->bind_param("s", $plantcode);
        $stmt->execute();
        $stmt->store_result();
		$userSR = array(); $user24=array();
		$pnpslipmain_id=''; $pnpslipmain_date=''; $pnpslipmain_crop=''; $pnpslipmain_variety=''; $pnpslipmain_promachcode=''; $pnpslipmain_proopr=''; $pnpslipmain_treattype=''; $pnpslipmain_ttype=''; $pnpslipmain_packlinetype=''; $pnpslipmain_printflg=0; $pnpslipmain_packlinecode=''; $pnpslipmain_manualtype='';
        if ($stmt->num_rows > 0) {
            // user existed 
			$stmt->bind_result($pnpslipmain_id, $pnpslipmain_date, $pnpslipmain_crop, $pnpslipmain_variety, $pnpslipmain_promachcode, $pnpslipmain_proopr, $pnpslipmain_treattype, $pnpslipmain_ttype, $pnpslipmain_packlinetype, $pnpslipmain_manualtype, $pnpslipmain_printflg);
			while($stmt->fetch())
			{
			if($pnpslipmain_date==NULL){$pnpslipmain_date='';} if($pnpslipmain_crop==NULL){$pnpslipmain_crop='';} if($pnpslipmain_variety==NULL){$pnpslipmain_variety='';} if($pnpslipmain_promachcode==NULL){$pnpslipmain_promachcode='';} if($pnpslipmain_proopr==NULL){$pnpslipmain_proopr='';} if($pnpslipmain_treattype==NULL){$pnpslipmain_treattype='';} if($pnpslipmain_ttype==NULL){$pnpslipmain_ttype='';}  if($pnpslipmain_packlinetype==NULL){$pnpslipmain_packlinetype='';}  if($pnpslipmain_packlinecode==NULL){$pnpslipmain_packlinecode='';} if($pnpslipmain_manualtype==NULL){$pnpslipmain_manualtype='';} 
			...
			$userSR["trtype"] = $trtyp;
			$userSR["pnpslipmain_packlinetype"] = $pnpslipmain_packlinetype;
			$userSR["pnpslipmain_manualtype"] = $pnpslipmain_manualtype;
			
			
			$pnpslipsub_lotno=''; $pnpslipsub_pickpqty='0.00'; $pnpslipsub_ups=''; $pnpslipsub_wtmp='0'; $pnpslipsub_nop='0'; $pnpslipsub_wbnop='0'; $pnpslipsub_wbwt='0';  $pnpslipsub_wbinmp='0'; $pnpslipsub_plcode1=''; $pnpslipsub_slabelno='';
			$stmt_2 = $this->conn_ps->prepare("SELECT pnpslipsub_lotno, pnpslipsub_pickpqty, pnpslipsub_ups, pnpslipsub_wtmp, pnpslipsub_nop, pnpslipsub_wbnop, pnpslipsub_wbwt, pnpslipsub_wbinmp, pnpslipsub_plcode1, pnpslipsub_slabelno  FROM tbl_pnpslipsub WHERE pnpslipmain_id = ? ");
			...
			if ($stmt_2->num_rows == 0) {
				...
				$userSR["pnpslipmain_printflg"] = $pnpslipmain_printflg;
				$userSR["pnpslipsub_plcode1"] = $pnpslipsub_plcode1;
				$userSR["pnpslipsub_slabelno"] = $pnpslipsub_plcode1;
				$stmt_2->close();

			} else {
				$stmt_2->bind_result($pnpslipsub_lotno, $pnpslipsub_pickpqty, $pnpslipsub_ups, $pnpslipsub_wtmp, $pnpslipsub_nop, $pnpslipsub_wbnop, $pnpslipsub_wbwt, $pnpslipsub_wbinmp, $pnpslipsub_plcode1, $pnpslipsub_slabelno);
				//looping through all the records
				$stmt_2->fetch();
				if($pnpslipsub_lotno==NULL){$pnpslipsub_lotno='';} if($pnpslipsub_pickpqty==NULL){$pnpslipsub_pickpqty='0.00';} if($pnpslipsub_ups==NULL){$pnpslipsub_ups='';} if($pnpslipsub_wtmp==NULL){$pnpslipsub_wtmp='0';} if($pnpslipsub_nop==NULL){$pnpslipsub_nop='0';} if($pnpslipsub_wbnop==NULL){$pnpslipsub_wbnop='0';} if($pnpslipsub_wbwt==NULL){$pnpslipsub_wbwt='0';}  if($pnpslipsub_wbinmp==NULL){$pnpslipsub_wbinmp='0';} if($pnpslipsub_plcode1==NULL){$pnpslipsub_plcode1='';}if($pnpslipsub_slabelno==NULL){$pnpslipsub_ups='';}
...
public function GetTranSetupLotyrcodelist() {
	...
	public function GetCropList($scode, $mobile1, $packtype) {
		...
		$stmt_2 = $this->conn_ps->prepare("SELECT distinct pnpslipmain_crop FROM tbl_pnpslipmain  ");
		//$stmt_2->bind_param("i", $trid);
		$result2=$stmt_2->execute();
		$stmt_2->store_result();
		if ($stmt_2->num_rows > 0) {
			...
			while($stmt_2->fetch())
			{
				//if($packtype!="SWB" && $packtype!="LWB" && $packtype!="NWB" && $packtype!="NLW")
				if($packtype=="LMC" || $packtype=="NLC" || $packtype=="SMC" || $packtype=="NMC")
				{
					$trtyp=NULL;
						
					$stmtqrchk = $this->conn_ps->prepare("SELECT wb_crop, wb_variety, wb_ups, wb_id, wb_extqrcode FROM tbl_wbqrcode WHERE wb_mpqlinkflg=2  and wb_linklogid=? and wb_mptype!='MMC'");
					...
					if ($stmtqrchk->num_rows > 0) {
						...
						{
							$stmt60 = $this->conn_ps->prepare("UPDATE tbl_wbqrcode SET wb_mptype=?, wb_mpqrcode=?, wb_mpbarcode=?, wb_mpwt=?, wb_mpqlinkflg=?, wb_mpblinkflg=?, wb_mpgrosswt=?, wb_linklogid=? where wb_id=?  ");
							$stmt60->bind_param("sssiiissi", $trtyp, $trtyp, $trtyp, $zero, $zero, $zero, $trtyp, $trtyp, $wb_id);
							$result60 = $stmt60->execute();
							$stmt60->close();
	...
	public function GetLotList($scode, $mobile1, $cropname, $varietyname, $ups, $packtype) {
		...
		if ($stmt_2->num_rows > 0) {
			...
			while($stmt_2->fetch())
			{
				...
				$wbqty=0;
				/*$stmt_wb = $this->conn_ps->prepare("SELECT wb_nop, wb_qty FROM tbl_wbqrcode WHERE wb_mpqlinkflg=0 and wb_lotno = ? ");
				$stmt_wb->bind_param("s", $lotno3);
				$result_ver=$stmt_wb->execute();
				$stmt_wb->store_result();
				if ($stmt_wb->num_rows > 0) {
					$stmt_wb->bind_result($wb_nop, $wb_qty);
					//looping through all the records 
					while($stmt_wb->fetch())
					{
						$wbqty=$wbqty+$wb_qty;
	...
	public function GetLblNoUpdate($scode, $trid, $lbltype, $lblno, $macoprname, $dopdate, $shiftcode, $lblno2) {
		$plantcode = $this->getPlantcode($scode);
		$flg=0; $lblchar1=''; $lbln1=''; $lblchar2=''; $lbln2=''; $proopr_id=0; $proopr_fname=''; $proopr_lname=''; $plcode1=''; $plcode2='';
		...
		$stmt_ups->close();*/
		$totnop=0;
		$stmt_pnpbar = $this->conn_ps->prepare("SELECT sum(wb_nop) FROM tbl_wbqrcode WHERE wb_pnptrid = ? ");
		...
		if($lbltype=="start")
		{
			...
			$stmt6 = $this->conn_ps->prepare("Update tbl_pnpslipsub SET pnpslipsub_slabelno=?, pnpslipsub_lblflg=1, pnpslipsub_shifchar=?, pnpslipsub_lblchar1=?, pnpslipsub_slblno=?, pnpslipsub_lblchar2=?, pnpslipsub_slblno2=?, pnpslipsub_plcode1=?, pnpslipsub_plcode2=? where pnpslipmain_id = ? ");
			$stmt6->bind_param("ssssssssi", $slblno, $shifchar, $lblchar1, $lbln1, $lblchar2, $lbln2, $plcode1, $plcode2, $trid);
		}
		else
		{
			...
			$stmt6 = $this->conn_ps->prepare("Update tbl_pnpslipsub SET pnpslipsub_elabelno=?, pnpslipsub_lblflg=0, pnpslipsub_shifchar=?, pnpslipsub_lblchar1=?, pnpslipsub_elblno=?, pnpslipsub_lblchar2=?, pnpslipsub_elblno2=?, pnpslipsub_totbags=?, pnpslipsub_shiftbags=? where pnpslipmain_id = ? ");
			$stmt6->bind_param("ssssssssi", $elblno, $shifchar, $lblchar1, $lbln1, $lblchar2, $lbln2, $totnop, $pnpslipsub_shiftbags, $trid);
		}
	...
	$plantcode = $this->getPlantcode($scode);
		...
		$stmt = $this->conn_ps->prepare("SELECT pnpslipmain_id, pnpslipmain_date, pnpslipmain_crop, pnpslipmain_variety, pnpslipmain_promachcode, pnpslipmain_proopr, pnpslipmain_treattype, pnpslipmain_trtype, pnpslipmain_dop, pnpslipmain_proopr, pnpslipmain_proslipno FROM tbl_pnpslipmain WHERE pnpslipmain_id = ?");
        $stmt->bind_param("i", $trid);
        $stmt->execute();
        $stmt->store_result();
		$arrivalcode=0;
		$pnpslipmain_id=''; $pnpslipmain_date=''; $pnpslipmain_crop=''; $pnpslipmain_variety=''; $pnpslipmain_promachcode=''; $pnpslipmain_proopr=''; $pnpslipmain_treattype=''; $pnpslipmain_ttype=''; $pnpslipmain_dop=''; $pnpslipmain_proopr=''; $pnpslipmain_proslipno=''; $pnpslipmain_packlinetype=''; $pnpslipmain_printflg=0; $pnpslipmain_packlinecode=''; $pnpslipmain_manualtype='';
        if ($stmt->num_rows > 0) {
            // user existed 
			...
			if ($stmt_2->num_rows == 0) {
				...
				if(($pnpslipsub_qcdot=="" || $pnpslipsub_qcdot=="0000-00-00") && $pnpslipsub_qcdttype!='DoT' ){
					...
					$stmt_srsub = $this->conn_ps->prepare("SELECT softr_id FROM tbl_softr_sub WHERE softrsub_lotno = ? order by softr_id desc");
					...
					if ($stmt_srsub->num_rows > 0) {
						...
						$stmt_srmain = $this->conn_ps->prepare("SELECT softr_date FROM tbl_softr WHERE softr_id = ?  order by softr_id desc");
					...
					$stmt_srsub2 = $this->conn_ps->prepare("SELECT softr_id FROM tbl_softr_sub2 WHERE softrsub_lotno = ?  order by softr_id desc");
					...
					if ($stmt_srsub2->num_rows > 0) {
						...
						$stmt_srmain2 = $this->conn_ps->prepare("SELECT softr_date FROM tbl_softr2 WHERE softr_id = ?  order by softr_id desc");
				...
				$stmt_dommac = $this->conn_ps->prepare("SELECT plflg FROM tbl_rm_packmac where plantcode='$plantcode' and packlinecode='".$pnpslipsub_plcode1."' order by packlinecode ASC ");
				...
				{
					$stmt_dommac = $this->conn_ps->prepare("SELECT plflg FROM tbl_rm_packmac where plantcode='$plantcode' and packlinecode='".$pnpslipsub_plcode2."' order by packlinecode ASC ");
					//$stmt_dommac->bind_param("i", $pnpslipmain_variety);
					$result_dommac=$stmt_dommac->execute();
					$stmt_dommac->store_result();
					if ($stmt_dommac->num_rows > 0) {
	...
	public function GetQRcodeUpdate($scode, $trid, $scantype, $qrcode) {
		...
		if($trid>0)
		{
			$stmt_chk = $this->conn_ps->prepare("SELECT wb_extqrcode FROM tbl_wbqrcode WHERE wb_pnptrid = ? and wb_mpqlinkflg=0 and wb_actflg=1 ");
			
			$stmt_chk->bind_param("i", $trid);
			$result2=$stmt_chk->execute();
			$stmt_chk->store_result();
			if ($stmt_chk->num_rows>=$pnpslipsub_wbinmp) {
				$wbflg++;
			}
			$stmt_chk->close();
		}
		...
		else if($scantype=="mpqrcode")
		{
			...
			if ($stmt_2->num_rows == 0) {
				...
				$stmt_wbmpq = $this->conn_ps->prepare("SELECT wbm_mpqrcode FROM tbl_wbmpqrcode WHERE wbm_mpqrcode = ? ");
				$stmt_wbmpq->bind_param("s", $qrcode);
				$resultwbmpq=$stmt_wbmpq->execute();
				$stmt_wbmpq->store_result();
				if ($stmt_wbmpq->num_rows == 0) {
...
//return "SELECT packtype, lotldg_crop, lotldg_variety FROM tbl_lot_ldg_pack WHERE lotno = '$sval' order by lotdgp_id DESC LIMIT 0,1"; 							
	...
	$plantcode = $this->getPlantcode($scode);
		...
		$stmtm = $this->conn_ps->prepare("SELECT pnpslipmain_id FROM tbl_pnpslipmain WHERE pnpslipmain_tflag = 2 and pnpslipmain_trtype='wb' and pnpslipmain_wbactflag=0");
		//$stmtm->bind_param("i", $trid);
		$stmtm->execute();
		$stmtm->store_result();
		if ($stmtm->num_rows > 0) {
	...
	function GetQRScanningFinalize($scode, $trid, $loosepouches, $endlabelno)
	{
		$plantcode = $this->getPlantcode($scode);
		$flg=0; $dt=date("Y-m-d"); $one=1; $two=2; $zero=0;
		...
		if ($stmt->num_rows > 0) {
			...
			$stmt->fetch();
				...
				if($endlabelno!='')
				{
					$stmt60 = $this->conn_ps->prepare("UPDATE tbl_pnpslipsub SET pnpslipsub_loosepouches=?, pnpslipsub_lblflg=?, pnpslipsub_elabelno=?  where pnpslipmain_id=?  ");
					$stmt60->bind_param("sisi", $loosepouches, $zero, $endlabelno, $trid);
				}
				else
				{
...
//return "SELECT  wb_extqrcode, wb_type, wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_mptype, wb_mpqrcode, wb_mpbarcode, wb_mpwt, wb_mpgrosswt, wb_id  FROM tbl_wbqrcode WHERE wb_mpqrcode = '$qrcode'";		
		...
		if($wbids2!="")
		{ //return "SELECT DISTINCT(wb_lotno) FROM tbl_wbqrcode_sub WHERE wb_id IN ($wbids2)";
			$stmt_wbsub = $this->conn_ps->prepare("SELECT DISTINCT(wb_lotno) FROM tbl_wbqrcode_sub WHERE wb_id IN ($wbids2)");
		...
		if($wbids2!="")
		{		
			$stmt_wbs = $this->conn_ps->prepare("SELECT DISTINCT wb_lotno FROM tbl_wbqrcode_sub WHERE wb_id IN ($wbids2)");
			//$stmt_wbs->bind_param("i", $wb_id);
			$result_wbs=$stmt_wbs->execute();
			$stmt_wbs->store_result();
			if ($stmt_wbs->num_rows > 0) {
				...
				while($stmt_wbs->fetch())
				{
					...
					$nop=0; $qty=0;
						$stmt_wbs2 = $this->conn_ps->prepare("SELECT wb_extqrcode, wb_type, wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_mptype FROM tbl_wbqrcode_sub WHERE wb_lotno=? AND wb_id IN ($wbids2)");
					$stmt_wbs2->bind_param("s", $wb_lotno);
					$result_wbs2=$stmt_wbs2->execute();
					$stmt_wbs2->store_result();
					if ($stmt_wbs2->num_rows > 0) {
	...
	public function GetQRcodeDelete($scode, $trid, $scantype, $qrcode, $trtype) {
		...
		if($trid>0)
		{
			if($scantype=="wbqrcode")
			{
				$stmt_2 = $this->conn_ps->prepare("SELECT wb_extqrcode FROM tbl_wbqrcode WHERE wb_extqrcode = ? and wb_mpqlinkflg!=1 and wb_pnptrid=? ");
			
				$stmt_2->bind_param("si", $qrcode, $trid);
				$result2=$stmt_2->execute();
				$stmt_2->store_result();
//return $stmt_2->num_rows;
				if ($stmt_2->num_rows > 0) {
					...
					$stmtqrchk = $this->conn_ps->prepare("SELECT wb_crop, wb_variety, wb_ups, wb_id FROM tbl_wbqrcode WHERE wb_extqrcode = ? and wb_actflg>0 and wb_mpqlinkflg!=1 and wb_pnptrid=? ");
					$stmtqrchk->bind_param("si", $qrcode, $trid);
					$result_qrchk=$stmtqrchk->execute();
					$stmtqrchk->store_result();
			...
			{
				$stmt_2 = $this->conn_ps->prepare("SELECT wb_crop, wb_variety, wb_ups, wb_id, wb_pnptrid FROM tbl_wbqrcode WHERE wb_mpqrcode = ?  and wb_mpblinkflg!=1 and wb_pnptrid=? ");
				$stmt_2->bind_param("si", $qrcode, $trid);
				$result2=$stmt_2->execute();
				$stmt_2->store_result();
				if ($stmt_2->num_rows > 0) {
					$stmt_2->bind_result($cropname, $varietyname, $ups, $wb_id, $wb_pnptrid);
					$trtyp=NULL;
		...
		else 
		{
			...
			{
				$stmt_2 = $this->conn_ps->prepare("SELECT wb_crop, wb_variety, wb_ups, wb_id, wb_pnptrid FROM tbl_wbqrcode WHERE wb_mpqrcode = ? and wb_mpblinkflg!=1 ");
				$stmt_2->bind_param("s", $qrcode);
				$result2=$stmt_2->execute();
				$stmt_2->store_result();
				if ($stmt_2->num_rows > 0) {
...
//return "SELECT wb_crop, wb_variety, wb_ups, wb_lotno, wb_nop, wb_qty, wb_totwt, wb_type FROM tbl_wbqrcode WHERE wb_linklogid='$scode' and wb_mpblinkflg!=1 and wb_mptype='$mptype' and wb_lotno='$wblotno' ";
	...
	public function GetRPSTrList($scode, $mobile1) {
		$plantcode = $this->getPlantcode($scode);
         $stmt = $this->conn_ps->prepare("SELECT  packaging_id, packaging_type, packaging_tdate, packaging_tcode, packaging_slipno,  packaging_crop, packaging_variety, packaging_lotno, packaging_upssize, packaging_remarks, packaging_mptype, packaging_packlinetype, packaging_manualtype, packaging_printflg  FROM tbl_rpspackaging WHERE packaging_tflg=2 and packaging_wbactflag=0 and plantcode=? ORDER BY packaging_id ASC");
		  
        $stmt->bind_param("s", $plantcode);
        $stmt->execute();
        $stmt->store_result();
		$userSR = array(); $user24=array();
		$packaging_id=''; $packaging_type=''; $packaging_tdate=''; $packaging_tcode=''; $packaging_slipno='';  $packaging_crop=''; $packaging_variety=''; $packaging_lotno=''; $packaging_upssize=''; $packaging_remarks=''; $packaging_mptype=''; $packaging_packlinetype=''; $packaging_manualtype=''; $packaging_printflg=0;
        if ($stmt->num_rows > 0) {
            // user existed 
			$stmt->bind_result($packaging_id, $packaging_type, $packaging_tdate, $packaging_tcode, $packaging_slipno,  $packaging_crop, $packaging_variety, $packaging_lotno, $packaging_upssize, $packaging_remarks, $packaging_mptype, $packaging_packlinetype, $packaging_manualtype, $packaging_printflg);
			while($stmt->fetch())
			{
				if($packaging_tdate==NULL){$packaging_tdate='';} if($packaging_crop==NULL){$packaging_crop='';} if($packaging_variety==NULL){$packaging_variety='';} if($packaging_lotno==NULL){$packaging_lotno='';} if($packaging_upssize==NULL){$packaging_upssize='';} if($packaging_type==NULL){$packaging_type='';} if($packaging_mptype==NULL){$packaging_mptype='';} if($packaging_packlinetype==NULL){$packaging_packlinetype='';} if($packaging_manualtype==NULL){$packaging_manualtype='';}
				...
				$userSR["variety"] = $popularname;
				$userSR["trtype"] = $trtyp;
				$userSR["packaging_packlinetype"] = $packaging_packlinetype;
				$userSR["packaging_manualtype"] = $packaging_manualtype;
				...
				if ($stmt_2->num_rows == 0) {
					...
					$userSR["wbwt"] = $pnpslipsub_wbwt;
					$userSR["wbinmp"] = $pnpslipsub_wbinmp;
					$userSR["packaging_printflg"] = $packaging_printflg;
					$userSR["pnpslipsub_plcode1"] = $pnpslipsub_plcode1;
					$userSR["pnpslipsub_slabelno"] = $pnpslipsub_plcode1;
					$stmt_2->close();
	...
	$plantcode = $this->getPlantcode($scode);
		...
		$stmt = $this->conn_ps->prepare("SELECT  packaging_id, packaging_type, packaging_tdate, packaging_tcode, packaging_slipno,  packaging_crop, packaging_variety, packaging_lotno, packaging_upssize, packaging_remarks, packaging_mptype, promachcode, macoprname, proopr, packaging_indentdate, packaging_dop, packaging_packlinetype, packaging_manualtype, packaging_printflg  FROM tbl_rpspackaging WHERE packaging_tflg=2 and packaging_id=? ORDER BY packaging_id ASC");
        $stmt->bind_param("i", $trid);
        $stmt->execute();
        $stmt->store_result();
		$userSR = array(); $user24=array();
		$packaging_id=''; $packaging_type=''; $packaging_tdate=''; $packaging_tcode=''; $packaging_slipno='';  $packaging_crop=''; $packaging_variety=''; $packaging_lotno=''; $packaging_upssize=''; $packaging_remarks=''; $packaging_mptype='';  $packaging_date=''; $packaging_dop='';  $packaging_packlinetype=''; $packaging_manualtype=''; $packaging_printflg=0; 
        if ($stmt->num_rows > 0) {
            // user existed 
			$stmt->bind_result($packaging_id, $packaging_type, $packaging_tdate, $packaging_tcode, $packaging_slipno,  $packaging_crop, $packaging_variety, $packaging_lotno, $packaging_upssize, $packaging_remarks, $packaging_mptype, $promachcode, $macoprname, $proopr, $packaging_date, $packaging_dop, $packaging_packlinetype, $packaging_manualtype, $packaging_printflg );
			while($stmt->fetch())
			{
				if($packaging_tdate==NULL){$packaging_tdate='';} if($packaging_crop==NULL){$packaging_crop='';} if($packaging_variety==NULL){$packaging_variety='';} if($packaging_lotno==NULL){$packaging_lotno='';} if($packaging_upssize==NULL){$packaging_upssize='';} if($packaging_type==NULL){$packaging_type='';} if($packaging_mptype==NULL){$packaging_mptype='';} if($promachcode==NULL){$promachcode='';}  if($macoprname==NULL){$macoprname='';}  if($proopr==NULL){$proopr='';} if($packaging_date==NULL){$packaging_date='';}   if($packaging_dop==NULL){$packaging_dop='00-00-0000';}  if($packaging_packlinetype==NULL){$packaging_packlinetype='';} if($packaging_manualtype==NULL){$packaging_manualtype='';}
				...
				$packagingsub_lotno=''; $packagingsub_upssize=''; $packagingsub_wtmp='0.000'; $packagingsub_wtnop='0'; $packagingsub_extwh=''; $packagingsub_extbin=''; $packagingsub_extsubbin=''; $packagingsub_extnop='0'; $packagingsub_extqty='0.000'; $packagingsub_slabelno=''; $packagingsub_elabelnop=''; $packagingsub_wbnop='0'; $packagingsub_wbwt='0.000'; $packagingsub_wbinmp='0'; $packagingsub_remarks=''; $packagingsub_slabelno=''; $packagingsub_elabelno=''; $packagingsub_qcdot=''; $packagingsub_dop=''; $packagingsub_valupto=''; $packagingsub_pchmrp=''; $packagingsub_grmmrp=''; $packagingsub_shiftflg=0; $pnpslipsub_verifyflg=0; $pnpslipsub_plcode1=''; $pnpslipsub_plcode2=''; $plflg1=0; $plflg2=0; $packagingsub_shifchar=''; $pnpslipsub_printopr=''; $pnpslipsub_printstartflg=0; $pnpslipsub_printendflg=0; $pnpslipsub_printstartlabel=''; $pnpslipsub_printendlabel=''; $pnpslipsub_printstartshiftflg=0; $pnpslipsub_printendshiftflg=0; $pnpslipsub_minlabel=0; $pnpslipsub_maxlabel=0; $pnpslipsub_printshiftchar='';
				$stmt_2 = $this->conn_ps->prepare("SELECT  packagingsub_lotno, packagingsub_upssize, packagingsub_wtmp, packagingsub_wtnop, packagingsub_extwh, packagingsub_extbin, packagingsub_extsubbin, packagingsub_extnop, packagingsub_extqty, packagingsub_slabelno, packagingsub_elabelno,  packagingsub_wbnop, packagingsub_wbwt, packagingsub_wbinmp, packagingsub_remarks, packagingsub_qcdot, packagingsub_dop, packagingsub_valupto, packagingsub_pchmrp, packagingsub_grmmrp, packagingsub_shiftflg, pnpslipsub_verifyflg, pnpslipsub_plcode1, pnpslipsub_plcode2, packagingsub_shifchar, pnpslipsub_printopr, pnpslipsub_printstartflg, pnpslipsub_printendflg, pnpslipsub_printstartlabel, pnpslipsub_printendlabel, pnpslipsub_printstartshiftflg, pnpslipsub_printendshiftflg, pnpslipsub_minlabel, pnpslipsub_maxlabel, pnpslipsub_printshiftchar   FROM tbl_rpspackaging_sub WHERE packaging_id = ? ");
				$stmt_2->bind_param("i", $packaging_id);
				$result2=$stmt_2->execute();
				$stmt_2->store_result();
				if ($stmt_2->num_rows == 0) {
					...
					$userSR["pnpslipsub_verifyflg"] = $pnpslipsub_verifyflg;
					$userSR["pnpslipsub_shifchar"] = $packagingsub_shifchar;
					$userSR["plflg1"] = $plflg1;
					$userSR["plflg2"] = $plflg2;
					$userSR["pnpslipmain_packlinetype"] = $packaging_packlinetype;
					$userSR["pnpslipmain_manualtype"] = $packaging_manualtype;
					$userSR["packaging_printflg"] = $packaging_printflg;
					$userSR["pnpslipsub_plcode1"] = $pnpslipsub_plcode1;
					$userSR["pnpslipsub_slabelno"] = $pnpslipsub_plcode1;
					$userSR["pnpslipsub_printshiftchar"] = $pnpslipsub_printshiftchar;
					$userSR["pnpslipsub_printopr"] = $pnpslipsub_printopr;
					$userSR["pnpslipsub_printstartflg"] = $pnpslipsub_printstartflg;
					$userSR["pnpslipsub_printendflg"] = $pnpslipsub_printendflg;
					$userSR["pnpslipsub_printstartlabel"] = $pnpslipsub_printstartlabel;
					$userSR["pnpslipsub_printendlabel"] = $pnpslipsub_printendlabel;
					$userSR["pnpslipsub_printstartshiftflg"] = $pnpslipsub_printstartshiftflg;
					$userSR["pnpslipsub_printendshiftflg"] = $pnpslipsub_printendshiftflg;
					$userSR["pnpslipsub_minlabel"] = $pnpslipsub_minlabel;
					$userSR["pnpslipsub_maxlabel"] = $pnpslipsub_maxlabel;
					$stmt_2->close();
	
				} else {
					$stmt_2->bind_result($packagingsub_lotno, $packagingsub_upssize, $packagingsub_wtmp, $packagingsub_wtnop, $packagingsub_extwh, $packagingsub_extbin, $packagingsub_extsubbin, $packagingsub_extnop, $packagingsub_extqty, $packagingsub_slabelno, $packagingsub_elabelno,  $packagingsub_wbnop, $packagingsub_wbwt, $packagingsub_wbinmp, $packagingsub_remarks,  $packagingsub_qcdot, $packagingsub_dop, $packagingsub_valupto, $packagingsub_pchmrp, $packagingsub_grmmrp, $packagingsub_shiftflg, $pnpslipsub_verifyflg, $pnpslipsub_plcode1, $pnpslipsub_plcode2, $packagingsub_shifchar, $pnpslipsub_printopr, $pnpslipsub_printstartflg, $pnpslipsub_printendflg, $pnpslipsub_printstartlabel, $pnpslipsub_printendlabel, $pnpslipsub_printstartshiftflg, $pnpslipsub_printendshiftflg, $pnpslipsub_minlabel, $pnpslipsub_maxlabel, $pnpslipsub_printshiftchar);
					//looping through all the records
					$stmt_2->fetch();
					if($packagingsub_lotno==NULL){$packagingsub_lotno='';} if($packagingsub_extqty==NULL){$packagingsub_extqty='0.00';} if($packagingsub_upssize==NULL){$packagingsub_upssize='';} if($packagingsub_wtmp==NULL){$packagingsub_wtmp='0';} if($packagingsub_extnop==NULL){$packagingsub_extnop='0';} if($packagingsub_wbnop==NULL){$packagingsub_wbnop='0';} if($packagingsub_wbwt==NULL){$packagingsub_wbwt='0';}  if($packagingsub_wbinmp==NULL){$packagingsub_wbinmp='0';}   if($packagingsub_shifchar==NULL){$packagingsub_shifchar='';}  if($packagingsub_slabelno==NULL){$packagingsub_slabelno='';}  if($packagingsub_elabelno==NULL){$packagingsub_elabelno='';}  if($pnpslipsub_printopr==NULL){$pnpslipsub_printopr='';} if($pnpslipsub_printstartlabel==NULL){$pnpslipsub_printstartlabel='';} if($pnpslipsub_printendlabel==NULL){$pnpslipsub_printendlabel='';}  if($pnpslipsub_printshiftchar==NULL){$pnpslipsub_printshiftchar='';}  
	...
	public function GetRPSLblNoUpdate($scode, $trid, $lbltype, $lblno, $macoprname, $dopdate, $shiftcode, $lblno2) {
		$plantcode = $this->getPlantcode($scode);
		$flg=0; $lblchar1=''; $lbln1=''; $lblchar2=''; $lbln2=''; $proopr_id=0; $proopr_fname=''; $proopr_lname=''; $plcode1=''; $plcode2='';
...
//return $stmt_2->num_rows;
		...
		$stmt_pnps = $this->conn_ps->prepare("SELECT packagingsub_id, packagingsub_slabelno, packagingsub_elabelno, packagingsub_shifchar, packagingsub_shiftbags FROM tbl_rpspackaging_sub WHERE packaging_id = ? ");
		$stmt_pnps->bind_param("i", $trid);
		$result_pnps=$stmt_pnps->execute();
		$stmt_pnps->store_result();
		if ($stmt_pnps->num_rows > 0) {
			$stmt_pnps->bind_result($pnpslipsub_id, $pnpslipsub_slabelno, $pnpslipsub_elabelno, $packagingsub_shifchar, $packagingsub_shiftbags);
			//looping through all the records 
			$stmt_pnps->fetch();
			$stmt_pnps->close();
		}
		$totnop=0;
		$stmt_pnpbar = $this->conn_ps->prepare("SELECT sum(wb_nop) FROM tbl_wbqrcode WHERE wb_rpstrid = ? ");
		$stmt_pnpbar->bind_param("i", $trid);
		$result_pnpbar=$stmt_pnpbar->execute();
		$stmt_pnpbar->store_result();
		$totrows=$stmt_pnpbar->num_rows;
		...
		$shiftbval=0; $balshiftnop=0;
		if($packagingsub_shiftbags!='' && $packagingsub_shiftbags!=NULL)
		{
			$shiftb=explode(",",$packagingsub_shiftbags);
			foreach($shiftb as $shiftb2)
			{
				if($shiftb2<>"")
				{
		...
		if($packagingsub_shiftbags!=""){
			$packagingsub_shiftbags=$packagingsub_shiftbags.",".$balshiftnop;
		}else{
			$packagingsub_shiftbags=$balshiftnop;
		}
		//$pnpslipsub_shiftbags=$pnpslipsub_shiftbags.",".$balshiftnop;
		if($packagingsub_shifchar!=""){
			$shifchar=$packagingsub_shifchar.",".$shiftcode;
		}else{
			$shifchar=$shiftcode;
		}
		//$shifchar=$pnpslipsub_shifchar.",".$shiftcode;
		if($lbltype=="start")
		{
			...
			{$slblno=$lblno;}
			$stmt6 = $this->conn_ps->prepare("Update tbl_rpspackaging_sub SET packagingsub_slabelno=?, packagingsub_shiftflg=1, packagingsub_shifchar=?, packagingsub_lblchar1=?, packagingsub_slblno=?, pnpslipsub_lblchar2=?, pnpslipsub_slblno2=?, pnpslipsub_plcode1=?, pnpslipsub_plcode2=?  where packaging_id = ? ");
			$stmt6->bind_param("ssssssssi", $slblno, $shifchar, $lblchar1, $lbln1, $lblchar2, $lbln2, $plcode1, $plcode2, $trid);
		}
		else
		{
			...
			{$elblno=$lblno;}
			$stmt6 = $this->conn_ps->prepare("Update tbl_rpspackaging_sub SET packagingsub_elabelno=?, packagingsub_shiftflg=0, packagingsub_shifchar=?, packagingsub_lblchar1=?, packagingsub_elblno=?, pnpslipsub_lblchar2=?, pnpslipsub_elblno2=?, packagingsub_totbags=?, packagingsub_shiftbags=? where packaging_id = ? ");
			$stmt6->bind_param("ssssssssi", $elblno, $shifchar, $lblchar1, $lbln1, $lblchar2, $lbln2, $totnop, $pnpslipsub_shiftbags, $trid);
		}
	...
	public function GetMacOprDetails() {
	
		$user10=array(); 
		$stmt_dommac = $this->conn_ps->prepare("SELECT proopr_fname, proopr_lname FROM tbl_rm_proopr where proopr_status='Active' order by proopr_fname, proopr_lname ASC ");
		//$stmt_dommac->bind_param("i", $pnpslipmain_variety);
		$result_dommac=$stmt_dommac->execute();
		$stmt_dommac->store_result();
		if ($stmt_dommac->num_rows > 0) {
...
//return $wb_mpbarcode." = ".$wb_pnptrid." = ".$wb_rpstrid;
			if($wb_pnptrid>0)
			{
				...
				if ($stmtm->num_rows > 0) {
					...
					$stmtm->fetch(); 
					if($pnpslipmain_tflag==0) { $flg=1; }
					if($pnpslipmain_tflag==1) { $flg=2; }
					if($pnpslipmain_tflag==2) { $flg=1; }
				}
				$stmtm->close();
			}
			if($wb_rpstrid>0)
			{
				$stmtm = $this->conn_ps->prepare("SELECT  packaging_tflg WHERE packaging_id=?");
				$stmtm->bind_param("i", $wb_rpstrid);
				$stmtm->execute();
				$stmtm->store_result();
				$arrivalcode=0;
				if ($stmtm->num_rows > 0) {
					$stmtm->bind_result($packaging_tflg);
					$stmtm->fetch();
					if($packaging_tflg==0) { $flg=1; }
					if($packaging_tflg==1) { $flg=2; }
					if($packaging_tflg==2) { $flg=1; }
				}
				$stmtm->close();
			}
		}
...
//return $wb_mpbarcode." = ".$wb_pnptrid." = ".$wb_rpstrid;
	...
	public function GetBarcodeUpdate($scode, $extqrcode, $newqrcode) {
		...
		$stmt_2->store_result();
		if($stmt_2->num_rows == 0) {
				$stmt60 = $this->conn_ps->prepare("UPDATE tbl_wbqrcode SET wb_mpbarcode=? where wb_mpbarcode=? ");
	...
	public function GetBarCodeDetails($qrcode) {
		...
		$stmt_2 = $this->conn_ps->prepare("SELECT wb_mpbarcode, wb_pnptrid, wb_rpstrid, wb_crop, wb_variety, wb_ups, wb_lotno FROM tbl_wbqrcode WHERE wb_mpbarcode = ? ");
		$stmt_2->bind_param("s", $qrcode);
		$result2=$stmt_2->execute();
		$stmt_2->store_result();
		if ($stmt_2->num_rows > 0) {
		
			$stmt_2->bind_result($wb_mpbarcode, $wb_pnptrid, $wb_rpstrid, $wb_crop, $wb_variety, $wb_ups, $wb_lotno);
			$stmt_2->fetch();
			$cropname=''; $popularname='';
			if($wb_ups=="" || $wb_ups==NULL) {$wb_ups='';}
			if($wb_lotno=="" || $wb_lotno==NULL) {$wb_lotno='';}
			if($wb_crop!=''){
				$stmt_crop = $this->conn_ps->prepare("SELECT cropid, cropname FROM tblcrop WHERE cropid = ? ");
				$stmt_crop->bind_param("i", $wb_crop);
				$result_crop=$stmt_crop->execute();
...
public function GetRPSExtLabelDetails($scode, $packmachcode, $shifttype, $trid) {
	$pcode=''; $plcode='';
	$stmt_plant = $this->conn_ps->prepare("SELECT plantcode, plantcode1  FROM tbluser WHERE scode=? ");
	$stmt_plant->bind_param("s", $scode);
	$result_plant=$stmt_plant->execute();
	...
	if($shifttype=="start")
	{
		...
		$stmt_sub = $this->conn_ps->prepare("SELECT MAX(CAST(SUBSTRING(label, 4) AS UNSIGNED)) AS max_number
FROM (
    SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(packagingsub_elabelno, ',', 1), ',', -1)) AS label FROM tbl_rpspackaging_sub
    UNION ALL
    SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(packagingsub_elabelno, ',', 2), ',', -1)) FROM tbl_rpspackaging_sub
    UNION ALL
    SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(packagingsub_elabelno, ',', 3), ',', -1)) FROM tbl_rpspackaging_sub
    UNION ALL
    SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(packagingsub_elabelno, ',', 4), ',', -1)) FROM tbl_rpspackaging_sub
    UNION ALL
    SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(packagingsub_elabelno, ',', 5), ',', -1)) FROM tbl_rpspackaging_sub
    UNION ALL
    SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(packagingsub_elabelno, ',', 6), ',', -1)) FROM tbl_rpspackaging_sub
    UNION ALL
    SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(packagingsub_elabelno, ',', 7), ',', -1)) FROM tbl_rpspackaging_sub
    UNION ALL
    SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(packagingsub_elabelno, ',', 8), ',', -1)) FROM tbl_rpspackaging_sub
    UNION ALL
    SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(packagingsub_elabelno, ',', 9), ',', -1)) FROM tbl_rpspackaging_sub
    UNION ALL
    SELECT TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(packagingsub_elabelno, ',', 10), ',', -1)) FROM tbl_rpspackaging_sub
) AS split_labels
WHERE label LIKE '$lblchar%'");
		//$stmt_sub->bind_param("s", $lblchar);
		$result_sub=$stmt_sub->execute();
...
public function GetRPSTotPouches($scode, $trid) {	
	
	$stmt_pnps = $this->conn_ps->prepare("SELECT packagingsub_totbags FROM tbl_rpspackaging_sub WHERE packaging_id = ? ");
	$stmt_pnps->bind_param("i", $trid);
	$result_pnps=$stmt_pnps->execute();
	$stmt_pnps->store_result();
	if ($stmt_pnps->num_rows > 0) {
		$stmt_pnps->bind_result($packagingsub_totbags);
		//looping through all the records 
		$stmt_pnps->fetch();
		$stmt_pnps->close();
	}
	...
	$balshiftnop=$totnop-$packagingsub_totbags;
...
public function updateDataVerification($scode, $trid) {
   ...
   // $stmt->close();
	...
	public function GetMPTrList($scode, $mobile1) {
		$plantcode = $this->getPlantcode($scode);
         $stmt = $this->conn_ps->prepare("SELECT pnpslipmain_id, pnpslipmain_date, pnpslipmain_crop, pnpslipmain_variety, pnpslipmain_promachcode, pnpslipmain_proopr, pnpslipmain_treattype, pnpslipmain_ttype, pnpslipmain_packlinetype, pnpslipmain_manualtype, pnpslipmain_printflg FROM tbl_pnpslipmain WHERE pnpslipmain_tflag=2 and pnpslipmain_trtype='wb' AND pnpslipmain_wbactflag=0 AND plantcode=? AND pnpslipmain_packlinetype='Manual' ORDER BY pnpslipmain_id ASC");
        $stmt->bind_param("s", $plantcode);
        $stmt->execute();
	...
	public function GetMPTranDetails ($scode,$trid, $mobile1) {
	$plantcode = $this->getPlantcode($scode);
		$userSR=array(); $arrival_id=0; $trtype='Fresh Seed with PDN'; $stage='Raw'; $user24=array();
		$stmt = $this->conn_ps->prepare("SELECT pnpslipmain_id, pnpslipmain_date, pnpslipmain_crop, pnpslipmain_variety, pnpslipmain_promachcode, pnpslipmain_proopr, pnpslipmain_treattype, pnpslipmain_trtype, pnpslipmain_dop, pnpslipmain_proopr, pnpslipmain_proslipno, pnpslipmain_packlinetype, pnpslipmain_manualtype, pnpslipmain_printflg FROM tbl_pnpslipmain WHERE pnpslipmain_id = ?");
        $stmt->bind_param("i", $trid);
        $stmt->execute();
        $stmt->store_result();
		$arrivalcode=0;
        ...
        if ($stmt->num_rows > 0) {
            // user existed 
			$stmt->bind_result($pnpslipmain_id, $pnpslipmain_date, $pnpslipmain_crop, $pnpslipmain_variety, $pnpslipmain_promachcode, $pnpslipmain_proopr, $pnpslipmain_treattype, $pnpslipmain_ttype, $pnpslipmain_dop, $pnpslipmain_proopr, $pnpslipmain_proslipno, $pnpslipmain_packlinetype, $pnpslipmain_manualtype, $pnpslipmain_printflg);
			$stmt->fetch();
			if($pnpslipmain_date==NULL){$pnpslipmain_date='';} if($pnpslipmain_crop==NULL){$pnpslipmain_crop='';} if($pnpslipmain_variety==NULL){$pnpslipmain_variety='';} if($pnpslipmain_promachcode==NULL){$pnpslipmain_promachcode='';} if($pnpslipmain_proopr==NULL){$pnpslipmain_proopr='';} if($pnpslipmain_treattype==NULL){$pnpslipmain_treattype='';} if($pnpslipmain_ttype==NULL){$pnpslipmain_ttype='';}  if($pnpslipmain_dop==NULL){$pnpslipmain_dop='';}   if($pnpslipmain_proopr==NULL){$pnpslipmain_proopr='';} if($pnpslipmain_packlinetype==NULL){$pnpslipmain_packlinetype='';} if($pnpslipmain_packlinecode==NULL){$pnpslipmain_packlinecode='';} if($pnpslipmain_manualtype==NULL){$pnpslipmain_manualtype='';} 
			if($pnpslipmain_date!='' && $pnpslipmain_date!=NULL)
			{
			...
			$pnpslipsub_lotno=''; $pnpslipsub_pickpqty=''; $pnpslipsub_ups=''; $pnpslipsub_wtmp=''; $pnpslipsub_nop=''; $pnpslipsub_wbnop=''; $pnpslipsub_wbwt='';  $pnpslipsub_wbinmp=''; $pnpslipsub_remarks=''; $pnpslipsub_qcdot=''; $pnpslipsub_valupto=''; $pnpslipsub_pouchccqty=0; $pnpslipsub_pouchmrp=0.00;  $pnpslipsub_gmmrp=0.00; $pnpslipsub_lblflg=0; $pnpslipsub_slabelno=''; $pnpslipsub_elabelno=''; $pnpslipsub_plotno=''; $pnpslipsub_availpqty='0.000'; $pnpslipsub_packtype=''; $pnpslipsub_availpnob=0; $pnpslipsub_qcdttype=''; $pnpslipsub_shifchar=''; $pnpslipsub_verifyflg=0; $pnpslipsub_plcode1=''; $pnpslipsub_plcode2=''; $plflg1=0; $plflg2=0; $pnpslipsub_printshiftchar=''; $pnpslipsub_printopr=''; $pnpslipsub_printstartflg=0; $pnpslipsub_printendflg=0; $pnpslipsub_printstartlabel=''; $pnpslipsub_printendlabel=''; $pnpslipsub_printstartshiftflg=0; $pnpslipsub_printendshiftflg=0; $pnpslipsub_minlabel=0; $pnpslipsub_maxlabel=0;
			$stmt_2 = $this->conn_ps->prepare("SELECT pnpslipsub_lotno, pnpslipsub_pickpqty, pnpslipsub_ups, pnpslipsub_wtmp, pnpslipsub_nop, pnpslipsub_wbnop, pnpslipsub_wbwt, pnpslipsub_wbinmp, pnpslipsub_remarks, pnpslipsub_qcdot, pnpslipsub_valupto, pnpslipsub_pouchccqty, pnpslipsub_pouchmrp, pnpslipsub_gmmrp, pnpslipsub_lblflg, pnpslipsub_slabelno, pnpslipsub_elabelno, pnpslipsub_plotno, pnpslipsub_oqty, pnpslipsub_packtype, pnpslipsub_onob, pnpslipsub_qcdttype, pnpslipsub_shifchar, pnpslipsub_verifyflg, pnpslipsub_plcode1, pnpslipsub_plcode2,  pnpslipsub_printshiftchar, pnpslipsub_printopr, pnpslipsub_printstartflg, pnpslipsub_printendflg, pnpslipsub_printstartlabel, pnpslipsub_printendlabel, pnpslipsub_printstartshiftflg, pnpslipsub_printendshiftflg, pnpslipsub_minlabel, pnpslipsub_maxlabel   FROM tbl_pnpslipsub WHERE pnpslipmain_id = ? ");
			$stmt_2->bind_param("i", $pnpslipmain_id);
			$result2=$stmt_2->execute();
			$stmt_2->store_result();
			if ($stmt_2->num_rows == 0) {
				...
				$stmt_2->bind_result($pnpslipsub_lotno, $pnpslipsub_pickpqty, $pnpslipsub_ups, $pnpslipsub_wtmp, $pnpslipsub_nop, $pnpslipsub_wbnop, $pnpslipsub_wbwt, $pnpslipsub_wbinmp, $pnpslipsub_remarks, $pnpslipsub_qcdot, $pnpslipsub_valupto, $pnpslipsub_pouchccqty, $pnpslipsub_pouchmrp, $pnpslipsub_gmmrp, $pnpslipsub_lblflg,  $pnpslipsub_slabelno, $pnpslipsub_elabelno, $pnpslipsub_plotno, $pnpslipsub_availpqty, $pnpslipsub_packtype, $pnpslipsub_availpnob, $pnpslipsub_qcdttype, $pnpslipsub_shifchar, $pnpslipsub_verifyflg, $pnpslipsub_plcode1, $pnpslipsub_plcode2,  $pnpslipsub_printshiftchar, $pnpslipsub_printopr, $pnpslipsub_printstartflg, $pnpslipsub_printendflg, $pnpslipsub_printstartlabel, $pnpslipsub_printendlabel, $pnpslipsub_printstartshiftflg, $pnpslipsub_printendshiftflg, $pnpslipsub_minlabel, $pnpslipsub_maxlabel);
				//looping through all the records
				$stmt_2->fetch();
				if($pnpslipsub_lotno==NULL){$pnpslipsub_lotno='';} if($pnpslipsub_pickpqty==NULL){$pnpslipsub_pickpqty='';} if($pnpslipsub_ups==NULL){$pnpslipsub_ups='';} if($pnpslipsub_wtmp==NULL){$pnpslipsub_wtmp='';} if($pnpslipsub_nop==NULL){$pnpslipsub_nop='';} if($pnpslipsub_wbnop==NULL){$pnpslipsub_wbnop='';} if($pnpslipsub_wbwt==NULL){$pnpslipsub_wbwt='';}  if($pnpslipsub_wbinmp==NULL){$pnpslipsub_wbinmp='';}  if($pnpslipsub_remarks==NULL){$pnpslipsub_remarks='';}  if($pnpslipsub_qcdot==NULL){$pnpslipsub_qcdot='';}  if($pnpslipsub_valupto==NULL){$pnpslipsub_valupto='';}  if($pnpslipsub_slabelno==NULL){$pnpslipsub_slabelno='';}  if($pnpslipsub_elabelno==NULL){$pnpslipsub_elabelno='';}  if($pnpslipsub_plotno==NULL){$pnpslipsub_plotno='';} if($pnpslipsub_shifchar==NULL){$pnpslipsub_shifchar='';} if($pnpslipsub_printshiftchar==NULL){$pnpslipsub_printshiftchar='';} if($pnpslipsub_printopr==NULL){$pnpslipsub_printopr='';} if($pnpslipsub_printstartlabel==NULL){$pnpslipsub_printstartlabel='';} if($pnpslipsub_printendlabel==NULL){$pnpslipsub_printendlabel='';}  
				...
				if($pnpslipmain_proopr!=''){
					$stmt_proopr = $this->conn_ps->prepare("SELECT proopr_fname, proopr_lname FROM tbl_rm_proopr WHERE proopr_id = ? ");
					$stmt_proopr->bind_param("i", $pnpslipsub_printopr);
					$result_proopr=$stmt_proopr->execute();
					$stmt_proopr->store_result();
					if ($stmt_proopr->num_rows > 0) {
						$stmt_proopr->bind_result($proopr_fname, $proopr_lname);
				...
				$userSR["pnpslipsub_slabelno"] = $pnpslipsub_slabelno;
				$userSR["pnpslipsub_printshiftchar"] = $pnpslipsub_printshiftchar;
				$userSR["pnpslipsub_printopr"] = $macoprname;
				$userSR["pnpslipsub_printstartflg"] = $pnpslipsub_printstartflg;
				$userSR["pnpslipsub_printendflg"] = $pnpslipsub_printendflg;
				$userSR["pnpslipsub_printstartlabel"] = $pnpslipsub_printstartlabel;
				$userSR["pnpslipsub_printendlabel"] = $pnpslipsub_printendlabel;
...
public function GetPrintLabelChk ($scode, $prtype, $trid) {
	$plantcode = $this->getPlantcode($scode);
	$dt=date("Y-m-d");
	$stmt_insert = $this->conn_ps->prepare("UPDATE tbl_pnpslipmain SET pnpslipmain_printflg=2 WHERE pnpslipmain_id=?");
	$stmt_insert->bind_param("i", $trid);
	$result_insert = $stmt_insert->execute();
	if ($result_insert) {
		if($prtype=='start')
		{
			$stmt_insert2 = $this->conn_ps->prepare("UPDATE tbl_pnpslipsub SET pnpslipsub_printstartflg=1, pnpslipsub_startprintdt='$dt'  WHERE pnpslipmain_id=?");
		}
		else
		{
			$stmt_insert2 = $this->conn_ps->prepare("UPDATE tbl_pnpslipsub SET pnpslipsub_printendflg=1 WHERE pnpslipmain_id=?");
		}
		$stmt_insert2->bind_param("i", $trid);
		$result_insert2 = $stmt_insert2->execute();
		if ($result_insert2) {
			return true;
		}
		$stmt_insert2->close();
...
public function GetPrintLabelUpd ($scode, $labelno, $printshifttype, $trid, $dopdate, $printshiftchar, $printopr) {
	$plantcode = $this->getPlantcode($scode);
	
	$abc=str_split($labelno);
	$lblchar1=$abc[0].$abc[1].$abc[2];
	$plcode1=$abc[1];
	$lbln1=$abc[3].$abc[4].$abc[5].$abc[6].$abc[7].$abc[8].$abc[9];
	$slabel=''; $elabel='';
	...
	$stmt_pnpss = $this->conn_ps->prepare("SELECT pnpslipsub_printstartlabel, pnpslipsub_printendlabel FROM tbl_pnpslipsub WHERE pnpslipmain_id = ? ");
	$stmt_pnpss->bind_param("i", $pnpslipmain_id);
	$result_pnpss=$stmt_pnpss->execute();
	$stmt_pnpss->store_result();
	if ($stmt_pnpss->num_rows > 0) {
		$stmt_pnpss->bind_result($pnpslipsub_printstartlabel, $pnpslipsub_printendlabel);
		//looping through all the records 
		while($stmt_pnpss->fetch())
		{
			$slabel=$pnpslipsub_printstartlabel;
			$elabel=$pnpslipsub_printendlabel;
		}
	}	
	$proopr_id=0;
	if($printopr!=''){
		$stmt_proopr = $this->conn_ps->prepare("SELECT proopr_id, proopr_fname, proopr_lname FROM tbl_rm_proopr WHERE CONCAT_WS(' ', proopr_fname, proopr_lname)=? ");
		$stmt_proopr->bind_param("s", $printopr);
		$result_proopr=$stmt_proopr->execute();
		$stmt_proopr->store_result();
		if ($stmt_proopr->num_rows > 0) {
			$stmt_proopr->bind_result($proopr_id, $proopr_fname, $proopr_lname);
	...
	if($printshifttype=='start') { if($slabel!='') { $slabel=$slabel.",".$labelno; } else { $slabel=$labelno; } } else { if($elabel!='') { $elabel=$elabel.",".$labelno; } else { $elabel=$labelno; } }
	if($printshifttype=='start')
	{
		$stmt_insert = $this->conn_ps->prepare("UPDATE tbl_pnpslipsub SET pnpslipsub_printstartlabel='$labelno', pnpslipsub_printstartshiftflg=1, pnpslipsub_printendshiftflg=0, pnpslipsub_printshiftchar='$printshiftchar', pnpslipsub_printopr='$proopr_id' WHERE pnpslipmain_id=?");
	}
	else
	{
		$stmt_insert = $this->conn_ps->prepare("UPDATE tbl_pnpslipsub SET pnpslipsub_printendlabel='$labelno', pnpslipsub_printendshiftflg=1, pnpslipsub_printstartshiftflg=0, pnpslipsub_printstartflg=0, pnpslipsub_printendflg=0 WHERE pnpslipmain_id=?");
	}
	$stmt_insert->bind_param("i", $trid);
	$result_insert = $stmt_insert->execute();
	if ($result_insert) {
		...
		$stmt_insert2 = $this->conn_ps->prepare("UPDATE tbl_pnpslipmain SET pnpslipmain_printflg=2, pnpslipmain_dop=? WHERE pnpslipmain_id=?");
		$stmt_insert2->bind_param("si", $dopdt, $trid);
		$result_insert2 = $stmt_insert2->execute();
		if ($result_insert2) {
			return true;
		}
...
public function GetPrintLabelFinalSubmit ($scode, $trid) {
	$plantcode = $this->getPlantcode($scode);
	$pnpslipsub_printendshiftflg=0;
	$stmt_pnps = $this->conn_ps->prepare("SELECT pnpslipsub_printendshiftflg FROM tbl_pnpslipsub WHERE pnpslipmain_id = ? ");
	$stmt_pnps->bind_param("i", $trid);
	$result_pnps=$stmt_pnps->execute();
	$stmt_pnps->store_result();
	if ($stmt_pnps->num_rows > 0) {
		$stmt_pnps->bind_result($pnpslipsub_printendshiftflg);
		//looping through all the records 
		$stmt_pnps->fetch();
		$stmt_pnps->close();
	}
	if($pnpslipsub_printendshiftflg>0)
	{
		$stmt_insert = $this->conn_ps->prepare("UPDATE tbl_pnpslipmain SET pnpslipmain_printflg=1 WHERE pnpslipmain_id=?");
		$stmt_insert->bind_param("i", $trid);
		$result_insert = $stmt_insert->execute();
		if ($result_insert) {
			return true;
	...
	public function GetMLLblNoUpdate($scode, $trid, $lbltype, $lblno, $macoprname, $dopdate, $shiftcode, $lblno2) {
		$plantcode = $this->getPlantcode($scode);
		$flg=0; $lblchar1=''; $lbln1=''; $lblchar2=''; $lbln2=''; $proopr_id=0; $proopr_fname=''; $proopr_lname=''; $plcode1=''; $plcode2='';
		...
		if($lbltype=="start")
		{
			$stmt6 = $this->conn_ps->prepare("Update tbl_pnpslipsub SET pnpslipsub_lblflg=1, pnpslipsub_shifchar=? where pnpslipmain_id = ? ");
			$stmt6->bind_param("si", $shifchar, $trid);
		}
		else
		{
			$stmt6 = $this->conn_ps->prepare("Update tbl_pnpslipsub SET pnpslipsub_lblflg=0, pnpslipsub_shifchar=?, pnpslipsub_totbags=?, pnpslipsub_shiftbags=? where pnpslipmain_id = ? ");
			$stmt6->bind_param("sssi", $shifchar, $totnop, $pnpslipsub_shiftbags, $trid);
		}
...
public function GetPrintLabelDetails($scode, $packmachcode, $shifttype, $trid) {
	$pcode=''; $plcode='';
	$stmt_plant = $this->conn_ps->prepare("SELECT plantcode, plantcode1  FROM tbluser WHERE scode=? ");
	$stmt_plant->bind_param("s", $scode);
	$result_plant=$stmt_plant->execute();
...
WHERE label LIKE '$lblchar%' ");
		...
		$stmt_23 = $this->conn_ps->prepare("SELECT MAX(pnpslipsub_maxlabel) FROM tbl_pnpslipsub WHERE pnpslipsub_lblchar1 = ? ");
		$stmt_23->bind_param("s", $lblchar);
		$result23=$stmt_23->execute();
		$stmt_23->store_result();
		if ($stmt_23->num_rows == 0) {
			$stmt_23->close();
		} else {
			$stmt_23->bind_result($pnpslipsub_maxlabel);
			$stmt_23->fetch();
		}	
		 if($pnpslipsub_maxlabel>0 && $pnpslipsub_maxlabel > $pnpslipsub_lblno)
		 {
		 	$pnpslipsub_lblno=$pnpslipsub_maxlabel;
		 }
		if($ycode=='R')
		{
			if($pnpslipsub_lblno<=1000000)
		...
		{
			if($pnpslipsub_lblno==0){$pnpslipsub_lblno=1;} else {$pnpslipsub_lblno=$pnpslipsub_lblno+1;}
			$lblnm=$pnpslipsub_lblno;
			$pnpslipsub_lblno=sprintf('%07d', $pnpslipsub_lblno);
			$labelno=$lblchar.$pnpslipsub_lblno;
			//$labelno=$pnpslipsub_lblno;
			
		}
		$pnpslipsub_ups=''; $pnpslipsub_upsid=0; $pnpslipsub_pickpqty=0; $pnpslipsubmaxlabel=0; $pnpslipsubslabelno='';			
		$stmt_2 = $this->conn_ps->prepare("SELECT pnpslipsub_ups, pnpslipsub_upsid, pnpslipsub_pickpqty, pnpslipsub_maxlabel, pnpslipsub_slabelno FROM tbl_pnpslipsub WHERE pnpslipmain_id = ? ");
		$stmt_2->bind_param("i", $trid);
		$result2=$stmt_2->execute();
		$stmt_2->store_result();
		if ($stmt_2->num_rows == 0) {
			...
			$stmt_2->bind_result($pnpslipsub_ups, $pnpslipsub_upsid, $pnpslipsub_pickpqty, $pnpslipsubmaxlabel, $pnpslipsubslabelno);
			$stmt_2->fetch();
		}	
		
		$upval=explode(" ",$pnpslipsub_ups);
		
		$upswt=$upval[1]; $ups=$upval[0]; $mx=0; $ptp=0; $ptp1=0; $mxlm=0; $minlblno=0; $maxlblno=0; $per=0;
		/*$stmt_ups = $this->conn_ps->prepare("SELECT wt, ups  FROM tblups WHERE uid =? ");
		$stmt_ups->bind_param("i", $pnpslipsub_upsid);
		$result_yer=$stmt_ups->execute();
		$stmt_ups->store_result();
		if ($stmt_ups->num_rows > 0) {
			$stmt_ups->bind_result($upswt, $ups);
			//looping through all the records 
			$stmt_ups->fetch();*/
	...
	if($pnpslipsubmaxlabel==0)
	{
		$stmt_insert = $this->conn_ps->prepare("UPDATE tbl_pnpslipsub SET pnpslipsub_printstartlabel='$labelno', pnpslipsub_slabelno='$labelno', pnpslipsub_lblchar1='$lblchar', pnpslipsub_slblno='$lblnm', pnpslipsub_plcode1='$plcode', pnpslipsub_minlabel='$minlblno', pnpslipsub_maxlabel='$maxlblno' WHERE pnpslipmain_id=?");
		$stmt_insert->bind_param("i", $trid);
		$result_insert = $stmt_insert->execute();
		if ($result_insert) {
