...
if(isset($_REQUEST['scode'])) {
	...
	if($trtype=="nmc" || $trtype=="Nmc" || $trtype=="NMC"){$trtype="NMC";}
	if($trtype=="nlc" || $trtype=="Nlc" || $trtype=="NLC"){$trtype="NLC";}
...
//echo $trtype;	exit;
	if($flg==0)
	{
		...
		{
			// user is found
			$user_trlist = $db->GetQRcodeDelete($scode, $trid, $scantype, $qrcode, $trtype);
			...
			if ($user_trlist != false) 
			{
				...
				{				
					...
					{ //echo $trtype." - ".$scode." - ".$cropname." - ".$varietyname." - ".$ups;
