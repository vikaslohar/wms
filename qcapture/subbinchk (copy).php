...
if (isset($_REQUEST['scode'])) {
	...
	$subbinid = $_REQUEST['subbinid'];
	$variety = $_REQUEST['varietyname'];	
	...
	{
		// user is found
		$user_trlist = $db->GetSubBinChk($scode, $mobile1, $whid, $binid, $subbinid, $variety);
		...
		else
		{
			...
			$response["data"]=array();
			$response["msg"] = "Crop/Variety/Stage mismatch for selected SubBin. Please Try again.";
			echo json_encode($response);
		}
	}
	else
