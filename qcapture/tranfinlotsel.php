...
if (isset($_REQUEST['scode'])) {
	...
	{
		// user is found
		$user_trlist = $db->GetTranFinLotSel($scode,$trid,$lotno);
		...
		if ($user_trlist != false) 
		{
			...
			$response["user"]["ledurationsel"] = $user_trlist['ledurationsel'];
			...
			$response["user"]["wharray1"] = $user_trlist['wharray1'];
			$response["user"]["binarray1"] = $user_trlist['binarray1'];
			$response["user"]["subbinarray1"] = $user_trlist['subbinarray1'];
