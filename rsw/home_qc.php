...
$sql_tbl=mysqli_query($link,"select * from tbl_rsw_main where arr_role='".$logid."' and arrtrflag=0 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	 $arrival_id=$row_tbl['arrival_id'];	
	
	$s_sub="delete from tbl_rsw where arrival_id='".$arrival_id."' and plantcode='$plantcode'";
	...
	*/
	...
	$s_sub="delete from tbl_rsw_main where arr_role='".$logid."' and plantcode='$plantcode' and arrtrflag=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_qc1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
...
<title>RSW - Transaction - QC Sampling Request</title>
...
<table width="1003" height="600" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#e48324" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#e48324" style="border-bottom:solid; border-bottom-color:#e48324" >
	    <tr >
	      <td width="820" height="25" class="Mainheading">&nbsp;Transaction - QC Sampling Request </td>
	    </tr></table></td>
	  <td width="80" height="25" align="right" class="submenufont" >
	  <table border="3" align="right" bordercolor="#e48324" bordercolordark="#e48324" cellspacing="0" cellpadding="0" width="116" style="border-collapse:collapse;">

<tr height="15" class="tbltitle" >
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="home_qc2.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
...
$sql_arr_home=mysqli_query($link,"select * from tbl_rsw_main where arr_role='$logid' and arrtrflag=1 and plantcode='$plantcode'") or die(mysqli_error($link));
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_rsw_main where arr_role='$logid' and plantcode='$plantcode' order by arr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_rsw_main where arr_role='$logid' and plantcode='$plantcode' order by arr_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_rsw_main where arr_role='$logid' order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_rsw_main where arr_role='$logid' and arrtrflag=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="850" bordercolor="#e48324" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
              <td width="6%"align="center" valign="middle" class="tblheading">#</td> 
			   <td width="8%" align="center" valign="middle" class="tblheading">DOSC</td>
              ...
              <td width="18%" align="center" valign="middle" class="tblheading">Lot No.</td>
			   <td width="16%" align="center" valign="middle" class="tblheading">QC Tests</td>
			    <td width="9%" align="center" valign="middle" class="tblheading">QC Status </td>
			      <td width="12%" align="center" valign="middle" class="tblheading">GOT Status </td>
              </tr>
<?php
//$srno=1;
if($tot_arr_home > 0)
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$crop=""; $variety=""; $lotno="";  $moist=""; $got=""; $gemp="";$pp="";$qc="";
	...
	while($row_tbl_sub=mysqli_fetch_array($sql_tbl_sub))
	{
		...
		$sql_variety=mysqli_query($link,"select * from tblvariety where varietyid='".$row_tbl_sub['variety']."'  and vertype='PV'") or die(mysqli_error($link));
...
$sql_arhome=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_lotno='".$row_tbl_sub['lotno']."' and lotldg_balqty > 0 and plantcode='$plantcode' order by lotldg_id desc") or die(mysqli_error($link));
while($row_arhome=mysqli_fetch_array($sql_arhome))
{  
$qc=$row_arhome['lotldg_qc'];
$zc=explode(" ", $row_arhome['lotldg_got1']);
$got=$zc[0]." ".$row_arhome['lotldg_got'];
}
...
<tr class="Light">
         ...
         <td width="18%" align="center" valign="middle" class="tblheading"><?php echo $lotno?></td>
           ...
           <td width="12%" align="center" valign="middle" class="tblheading"><?php echo $got;?></td>
         </tr>
<?php
}
else
