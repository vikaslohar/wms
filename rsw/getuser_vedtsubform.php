...
$sql_tbl_sub=mysqli_query($link,"select * from tbl_rsw where arrsub_id='".$a."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_qc=mysqli_query($link,"SELECT distinct(variety) FROM tbl_rsw WHERE crop='".$row_tbl_sub['crop']."' and variety NOT RLIKE '^[-+0-9.E]+$' and plantcode='$plantcode'") or die(mysqli_error($link)); 
$tt=mysqli_num_rows($sql_qc);

$sql_month=mysqli_query($link,"select varietyid, popularname from tblvariety where cropname='".$row_tbl_sub['crop']."' and actstatus='Active' and vertype='PV' order by popularname Asc")or die(mysqli_error($link));
?>
