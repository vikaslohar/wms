...
$adjacents = 3;
 	...
 	$query = "SELECT COUNT(*) as num FROM tbl_sloc where supflg=1 and (stage='Raw' or stage='Raw Seed') and plantcode='$plantcode'";
	...
	/* Get data. */
	$sql = "select * from tbl_sloc where supflg=1 and (stage='Raw' or stage='Raw Seed') and plantcode='$plantcode' order by slid desc LIMIT $start, $limit";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_sloc where supflg=1 and stage='Raw' order by scode desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
