...
<?php	
	...
	if($itemid!="ALL")
	{
		...
		$qrycr="select distinct lotldg_crop from tbl_lot_ldg where lotldg_qctestdate<='".$edate."' and lotldg_crop='".$itemid."' and plantcode='$plantcode' ";
	}
	else
	{
		$qrycr="select distinct lotldg_crop from tbl_lot_ldg where lotldg_qctestdate<='".$edate."' and plantcode='$plantcode' ";
	}
	
	if($varietyo!="ALL")
	{
		$qrycr.=" and lotldg_variety='".$varietyo."' ";
	}
	if($result!="ALL")	
	{
		$qrycr.=" and lotldg_qc='".$result."' ";
	}
	if($seedstage!="ALL")
	{
		$qrycr.=" and lotldg_sstage='".$seedstage."' ";
	}
	$qrycr.=" order by lotldg_crop asc,lotldg_qctestdate asc ";
	//echo $qry;
	$sql_arr_homecr1=mysqli_query($link,$qrycr) or die(mysqli_error($link));
	$tot_arr_homecr=mysqli_num_rows($sql_arr_homecr1);
...
if($tot_arr_homecr > 0)
{
while($row_arr_homecr1=mysqli_fetch_array($sql_arr_homecr1))
{

		$sql_class=mysqli_query($link,"select * from tblcrop where cropid='".$row_arr_homecr1['lotldg_crop']."'") or die(mysqli_error($link));
		$row_class=mysqli_fetch_array($sql_class);
		$crop=$row_class['cropname'];	 
		
		$qry="select distinct lotldg_variety from tbl_lot_ldg where lotldg_qctestdate<='".$edate."' and lotldg_crop='".$row_arr_homecr1['lotldg_crop']."' and plantcode='$plantcode' ";
...
while($row_arr_home1=mysqli_fetch_array($sql_arr_home1))
{
	...
	$quer2=mysqli_query($link,"SELECT  cropname,cropid FROM tblcrop where cropid='".$row_arr_homecr1['lotldg_crop']."'"); 
	$row_dept=mysqli_fetch_array($quer2);
	$cropname=$row_dept['cropname'];
	$quer3=mysqli_query($link,"SELECT * FROM tblvariety where varietyid ='".$row_arr_home1['lotldg_variety']."'  and vertype='PV'"); 
	...
	$cont=0;
	$sql_arr_home244=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_qctestdate<='$edate' and lotldg_crop='".$row_arr_homecr1['lotldg_crop']."'  and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and plantcode='$plantcode' $reslt order by lotldg_variety asc, lotldg_qctestdate asc ") or die(mysqli_error($link));
...
<table  border="1" cellspacing="0" cellpadding="0" width="970" bordercolor="#e48324" style="border-collapse:collapse" align="center">
...
<?php
$sql_arr_home2=mysqli_query($link,"select distinct lotldg_lotno from tbl_lot_ldg where lotldg_qctestdate<='$edate' and lotldg_crop='".$row_arr_homecr1['lotldg_crop']."'  and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and plantcode='$plantcode' $reslt order by lotldg_variety asc, lotldg_qctestdate asc ") or die(mysqli_error($link));
$t=mysqli_num_rows($sql_arr_home2);
while($row_arr_home2=mysqli_fetch_array($sql_arr_home2))
{

$sql_arr_home3=mysqli_query($link,"select MAX(lotldg_id) from tbl_lot_ldg where lotldg_qctestdate<='".$edate."' and lotldg_crop='".$row_arr_homecr1['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and lotldg_lotno='".$row_arr_home2['lotldg_lotno']."' and plantcode='$plantcode' $reslt order by lotldg_variety asc, lotldg_qctestdate asc  ") or die(mysqli_error($link));
...
//echo $slups;
	...
	$quer3=mysqli_query($link,"SELECT * FROM tblvariety where varietyid ='".$row_arr_home['lotldg_variety']."'  and vertype='PV'"); 
