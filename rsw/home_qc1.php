...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_rsw_main  where arrival_date >='$tdate' and arrival_date <='$pdate' and arrtrflag=1 and plantcode='$plantcode' order by arr_code desc LIMIT $start, $limit") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);
...
$query = "select * from tbl_rsw_main  where arrival_date >='$tdate' and arrival_date <='$pdate' and arrtrflag=1 and plantcode='$plantcode' order by arr_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_rsw_main  where arrival_date >='$tdate' and arrival_date <='$pdate' and arrtrflag=1 order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_rsw_main where arrival_date >='$tdate' and arrival_date <='$pdate'  and arrtrflag=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
