...
$sql_is=mysqli_query($link,"select distinct lotldg_subbinid, lotldg_whid, lotldg_binid from tbl_lot_ldg where  lotldg_lotno='".$row_arr_home['lotldg_lotno']."' and plantcode='$plantcode' and lotldg_sstage='Raw' and lotldg_variety='".$variety."' group by lotldg_subbinid order by lotldg_id asc") or die(mysqli_error($link));

 while($row_is=mysqli_fetch_array($sql_is))
 { 
$sql_is1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_is['lotldg_subbinid']."' and lotldg_binid='".$row_is['lotldg_binid']."' and plantcode='$plantcode' and lotldg_whid='".$row_is['lotldg_whid']."' and lotldg_lotno='".$row_arr_home['lotldg_lotno']."' and lotldg_sstage='Raw' and lotldg_variety='".$variety."' order by lotldg_id asc ") or die(mysqli_error($link));
...
$sqlistbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_lotno='".$row_arr_home['lotldg_lotno']."' and plantcode='$plantcode' and lotldg_sstage='Raw' and lotldg_variety='".$variety."' order by lotldg_id asc") or die(mysqli_error($link)); 
$t=mysqli_num_rows($sqlistbl);
if($t > 0)
{
$rowistbl=mysqli_fetch_array($sqlistbl);

	$trdate=$rowistbl['lotldg_trdate'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;
