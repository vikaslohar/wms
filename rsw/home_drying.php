...
$sql_tbl=mysqli_query($link,"select * from tbl_drying where stage='RSW' and arrflg=0 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	*/
	...
	$s_sub="delete from tbl_drying where stage='RSW' and  arrflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	/**/
...
<title>QC - Transaction - QC Sampling Request</title>
...
$sql_arr_home=mysqli_query($link,"select * from tbl_drying where stage='RSW'and arrflg=1 and plantcode='$plantcode' order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

 $total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_drying where stage='RSW' and arrflg=1");
