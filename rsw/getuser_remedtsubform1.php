...
$sql_tbl_sub=mysqli_query($link,"select * from tbl_rswrem_sub where rswremsub_id='".$a."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_tbl_subsub=mysqli_query($link,"select * from tbl_rswremsub_sub where rswremsub_id='".$a."' and rswrem_id='".$tid."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_subsub=mysqli_fetch_array($sql_tbl_subsub);

$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_binid, lotldg_subbinid  from tbl_lot_ldg where lotldg_crop ='".$row['crop']."' and lotldg_variety='".$row['variety']."' and lotldg_sstage='Raw' and lotldg_lotno='".$row['lotnumber']."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
<?php
$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$row['variety']."' and actstatus='Active' and vertype='PV' order by popularname Asc"); 
...
$sql_binn=mysqli_query($link,"select binname from tbl_bin where binid='".$row_issue['lotldg_binid']."' and whid='".$row_issue['lotldg_whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
?>
<input type="hidden" name="txtlot1" value="<?php echo $row['lotnumber'];?>" />
</table>
<br />
