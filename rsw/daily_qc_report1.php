...
<title>RSW-Report Daily QC Result Report</title>
...
<?php	
	...
	$sql_arr_home=mysqli_query($link,"select distinct(sampleno) from tbl_qctest where testdate='$sdate' and trstage='Raw' and plantcode='$plantcode' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
...
<table align="center" border="0" cellspacing="0" cellpadding="0" width="850" style="border-collapse:collapse">
  ...
  <table  border="1" cellspacing="0" cellpadding="0" width="850" bordercolor="#e48324" style="border-collapse:collapse" align="center">
...
while($row_arr_home2=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_arr_home2=mysqli_query($link,"select MAX(tid) from tbl_qctest where sampleno='".$row_arr_home2['sampleno']."' and trstage='Raw' and testdate='$sdate' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
	$row_arr_home3=mysqli_fetch_array($sql_arr_home2);

$sql_arr_home3=mysqli_query($link,"select * from tbl_qctest where tid='".$row_arr_home3[0]."' and sampleno='".$row_arr_home2['sampleno']."' and trstage='Raw' and plantcode='$plantcode' order by crop asc, variety asc, oldlot asc") or die(mysqli_error($link));
