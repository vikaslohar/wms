...
{	
	...
	window.location='getuser_drying_lotno.php?crop='+val3+'&variety='+val4+'&sqq='+txtlot1;
	return true;	
	}
}	
...
$arrlots="";
/**/$sql_arrival=mysqli_query($link,"select trid from tbl_drying where crop='".$crop."' and variety='".$variety."' and plantcode='$plantcode'") or die(mysqli_error($link));
$tot_arrival=mysqli_num_rows($sql_arrival);
if($tot_arrival)
{
	while($row_arrival=mysqli_fetch_array($sql_arrival))
	{$row_arrival['trid'];
		$sql_tbl_sub=mysqli_query($link,"select * from tbl_dryingsub where   trid='".$row_arrival['trid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_tbl_sub=mysqli_query($link,"select distinct(lotldg_lotno)  from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$tp."' and lotldg_mergerflg=0 and plantcode='$plantcode' $sss")or die(mysqli_error($link));
