...
session_start();
	...
	$crp="ALL"; $ver="ALL";
	$qry="select Distinct lotldg_crop, lotldg_variety from tbl_lot_ldg where lotldg_sstage='Raw'";
	...
	$qry.=" and plantcode='$plantcode' group by lotldg_crop, lotldg_variety";

	$sql_arr_home1=mysqli_query($link,$qry) or die(mysqli_error($link));
 	$tot_arr_home=mysqli_num_rows($sql_arr_home1);
	$dat=date("d-m-Y");		
	
	$dh="Crop_Variety_wise_Raw_Seed_Report_".$dat;
	$datahead = array("Crop Variety wise Raw Seed Report  ".$dat);
	$filename=$dh.".xls";  
...
$sql_rr=mysqli_query($link,"select distinct lotldg_variety from tbl_lot_ldg where  lotldg_crop='".$row_arr_home1['lotldg_crop']."' and lotldg_variety='".$row_arr_home1['lotldg_variety']."' and lotldg_sstage='Raw' and plantcode='$plantcode'") or die(mysqli_error($link));
...
while($row_rr=mysqli_fetch_array($sql_rr))
{
	...
	$sql_crop=mysqli_query($link,"select * from tblcrop where cropid='".$row_arr_home1['lotldg_crop']."'") or die(mysqli_error($link));
		$row31=mysqli_fetch_array($sql_crop);
		 $crop=$row31['cropname'];		
		$sql_variety=mysqli_query($link,"select * from tblvariety where actstatus='Active' and vertype='PV' and varietyid='".$row_rr['lotldg_variety']."' ") or die(mysqli_error($link));
...
$totalbags=0; $totalqty=0;
	...
	$datahead2[$cnt] = array("#","Lot No.","NoB","Qty","Qc Status","Moisture %","Germination %","DOT","GOT Status","Production Location","State","Seed Status","Production Grade","GOT Grade"); 
...
$datahead2[$cnt] = array("#","Lot No.","NoB","Qty","SLOC","Qc Status","Moisture %","Germination %","DOT","GOT Status","Production Location","State","Seed Status"); 
if($slchk2=="yes") 
	{	
	array_push($datahead2[$cnt],"Arrival Date","Harvest Date"); 
	}
...
array_push($data1[$cnt][$d],$totqc,$totmost,$totgemp,$txtdot,$totgot,$ploc,$pstate,$totsst,$prodgrade,$gotgrade);
if($slchk2=="yes") 
{
