...
<title>RSW- Transaction-Status Updation</title>
...
?>
 <select class="tbltext" name="pcode" style="width:40px;">
  ...
  isNumberKey(event)"  onchange="lot2chk();"  /> <font size="+1"><b>/</b></font> <input name="stcode" type="text" size="4" class="tbltext" tabindex="0" maxlength="5" onkeypress="return isNumberKey(event)"  value="00000" onchange="slocshow();" /> <font size="+1"><b>/</b></font> <input name="stcode2" type="text" size="2" class="tbltext" tabindex="0" maxlength="2" onkeypress="return isNumberKey(event)"  value="00"  />
	  <b>R</b>&nbsp;<font color="#FF0000">*</font>&nbsp;&nbsp;<img src="../images/search.gif" border="0"  class="tbltext" name="sub" value="Search" style="display:inline;cursor:pointer; vertical-align:middle" onclick="lotchk();" /><!--<input type="image" src="../images/search.gif" border="0"  class="tbltext" name="sub" value="Search" style="display:inline;cursor:pointer; vertical-align:middle" onclick="lotchk();"> -->
    </form>
 </td></tr></table>
   <tr>
  <td valign="top">
...
<?php 
$lotchk=array('DR20063/00000/00R','DR15080/00000/00R','DR15079/00000/00R');
if($sqq!="")
{
...
$sss="";
 $qr="select distinct lotldg_lotno, lotldg_sstatus, lotldg_got1, lotldg_got, lotldg_srflg, lotldg_qc from tbl_lot_ldg where lotldg_crop='".$crop."' and lotldg_variety='".$variety."' and lotldg_sstage='".$tp."' and lotldg_mergerflg=0 and plantcode='$plantcode' $sss ";
...
$sqltbl=mysqli_query($link,"select * from tbl_rswrem where rswrem_id='".$trid."' and plantcode='$plantcode'") or die(mysqli_error($link));
$rowtbl=mysqli_fetch_array($sqltbl);
  $tot=mysqli_num_rows($sqltbl);		
$arrival_id=$rowtbl['rswrem_id'];

$sqltblsub=mysqli_query($link,"select * from tbl_rswrem_sub where rswrem_id='".$arrival_id."' and plantcode='$plantcode'") or die(mysqli_error($link));
$subtbltot=mysqli_num_rows($sqltblsub);
while($rowtblsub=mysqli_fetch_array($sqltblsub))
{
//echo $rowtblsub['lotnumber']."-".$row['lotldg_lotno'];
if($rowtblsub['lotnumber']==$row['lotldg_lotno'])
{
$flg++;
}
}
...
<td align="right"  valign="middle" class="tbltext"><input type="radio" name="loc" value="<?php echo $row['lotldg_lotno'];?>" onclick="clk(this.value,'<?php echo $row['lotldg_lotno'];?>','<?php echo $flg1?>');" />&nbsp;</td>
