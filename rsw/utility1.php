...
?>
<table align="center" border="1" bordercolor="#e48324" cellspacing="0" cellpadding="0" width="550" style="border-collapse:collapse">
...
<td width="199" align="right"  valign="middle" class="tblheading">Stage:&nbsp;Raw&nbsp;&nbsp;</td>
</tr>
  </table>
<?php
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where  lotldg_lotno='".$txtlot1."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and plantcode='$plantcode' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_lotno='".$txtlot1."'") or die(mysqli_error($link));
$row_issue1=mysqli_fetch_array($sql_issue1); 

$sql_arr_home=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$row_issue1[0]."' and plantcode='$plantcode'") or die(mysqli_error($link)); 
...
$sql_arr_sub=mysqli_query($link,"select * from tblarrival_sub where lotno='".$txtlot1."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_arr_sub=mysqli_fetch_array($sql_arr_sub);
//$tot=mysqli_num_rows($sql_tbl);		

$sql_arr=mysqli_query($link,"select * from tblarrival where arrival_id='".$row_arr_sub['arrival_id']."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_arr=mysqli_fetch_array($sql_arr);
	...
	$crop=$row31['cropname']; 
	$quer3=mysqli_query($link,"SELECT * FROM tblvariety where varietyid ='".$row_tbl_sub1['lotldg_variety']."'  and vertype='PV'"); 
...
$aq=explode(".",$row_tbl_sub1['lotldg_balbags']);
if($aq[1]==000){$ac=$aq[0];}else{$ac=$row_tbl_sub1['lotldg_balbags'];}

$an=explode(".",$row_tbl_sub1['lotldg_balqty']);
if($an[1]==000){$acn=$an[0];}else{$acn=$row_tbl_sub1['lotldg_balqty'];}
...
$sql_binn=mysqli_query($link,"select binname from tbl_bin where binid='".$row_tbl_sub1['lotldg_binid']."' and plantcode='$plantcode' and whid='".$row_tbl_sub1['lotldg_whid']."'") or die(mysqli_error($link));
...
$slups=$slups+$row_tbl_sub1['lotldg_balbags'];
 $slqty=$slqty+$row_tbl_sub1['lotldg_balqty'];
if($slocs!="")
...
$sql_arr_home=mysqli_query($link,"select * from tbl_qctest where oldlot='$newlot1' and plantcode='$plantcode' order by oldlot,tid desc limit 0,1") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);
...
if($acn==0)$slocs="";
?>   
<table align="center" border="1" cellspacing="0" cellpadding="0" width="550" bordercolor="#e48324" style="border-collapse:collapse">
