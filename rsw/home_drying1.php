<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='home_drying.php?sdate=$sdate&edate=$edate'</script>";
		}
		else
		 {?>
		 <script>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#e48324" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#e48324" style="border-bottom:solid; border-bottom-color:#e48324" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Drying Slip </td>
	    </tr></table></td>
...
/**/$sql_arr_home=mysqli_query($link,"select * from tbl_drying  where stage='RSW' and arrflg=1 and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_arr_home=mysqli_query($link,"select * from tbl_drying  where dryingdate >='$tdate' and dryingdate <='$pdate' and arrflg=1 and stage='RSW' and plantcode='$plantcode' order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
 $tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_drying where dryingdate >='$tdate' and stage='RSW' and dryingdate <='$pdate'  and arrflg=1 and plantcode='$plantcode'");
...
<table align="center" border="0" width="650" cellspacing="0" cellpadding="0" bordercolor="#e48324" style="border-collapse:collapse" > 
...
while($row_arr_home=mysqli_fetch_array($sql_arr_home))
{
	...
	$sql_crop=mysqli_query($link,"select * from tblcrop where cropid='".$row_arr_home['crop']."'") or die(mysqli_error($link));
		...
		$sql_variety=mysqli_query($link,"select * from tblvariety where varietyid='".$row_arr_home['variety']."'  and vertype='PV'") or die(mysqli_error($link));
...
<tr class="Light">
         <td width="6%" align="center" valign="middle" class="tblheading"><?php echo $srno;?></td>
		   <td width="8%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
		     <td width="9%" align="center" valign="middle" class="tblheading"><a href="Javascript:void(0)" onClick="openslocpopprint('<?php echo $row_arr_home['trid'];?>');"><?php echo "TQS".$row_arr_home['arr_code']."/".$yearid_id."/".$lgnid;?></a></td>
