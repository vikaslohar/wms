...
$sql_qc=mysqli_query($link,"SELECT distinct(variety) FROM tbl_rsw_main WHERE crop='".$row_tbl['crop']."' and variety NOT RLIKE '^[-+0-9.E]+$'") or die(mysqli_error($link)); 
$tt1=mysqli_num_rows($sql_qc);
$vv1 = mysqli_fetch_array($sql_qc);
echo $pp=$vv1['variety'];
$sql_month=mysqli_query($link,"select varietyid, popularname from tblvariety where cropname='".$row_tbl['crop']."' and actstatus='Active' and vertype='PV' order by popularname Asc")or die(mysqli_error($link));
