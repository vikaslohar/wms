...
$sql_in1=mysqli_query($link,"select * from tbl_sloc where slid=$trid and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_subtbl=mysqli_query($link,"select * from tbl_sloc_sub where slocid='".$trid."' and rowid='".$stldgid."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
<?php
$whi=0;$bni=0;$sbni=0;
/*$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$c."' and lotldg_variety='".$b."'") or die(mysqli_error($link));
...
*/$otups=0; $otqty=0;
$sql_issuetbl=mysqli_query($link,"select * from tbl_lot_ldg where lotldg_id='".$stldgid."' and lotldg_balqty > 0 and plantcode='$plantcode'") or die(mysqli_error($link)); 
...
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$c."' and lotldg_variety='".$b."' and lotldg_lotno='".$txtlot1."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$totBags=0; $totqty=0; $cnt=0; $whid="";$binid="";$subbinid="";
 ...
 $sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_variety='".$b."' and lotldg_lotno='".$txtlot1."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
if($trid > 0)
{
$sql_subtb=mysqli_query($link,"select * from tbl_sloc_sub where slocid='".$trid."' and subbinid='".$row_issue['lotldg_subbinid']."' and  binid='".$row_issue['lotldg_binid']."' and  whid='".$row_issue['lotldg_whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_subtb=mysqli_num_rows($sql_subtb);
}
else
{
//$sql_subtb=mysqli_query($link,"select * from tbl_sloc_sub where gid='".$trid."' and rowid!='".$row_issue1[0]."'") or die(mysqli_error($link));
$row_subtb=0;
}
//echo $row_subtb;
...
// echo $row_issuetbl['stld_whid']."/".$row_issuetbl['stld_binid']."/".$row_issuetbl['stld_subbinid']."<br>";
 ...
 /*$sql_subtb=mysqli_query($link,"select * from tbl_sloc_sub where gid='".$trid."' and rowid='".$stldgid."'") or die(mysqli_error($link));
...
?>
  <tr class="Light" height="30" >
    ...
    <?php
$bing1_query=mysqli_query($link,"select binid, binname from tbl_bin where whid='".$row_issuetbl['lotldg_whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
?>
    ...
    <?php
$subbing1_query=mysqli_query($link,"select sid, sname from tbl_subbin where whid='".$row_issuetbl['lotldg_whid']."' and binid='".$row_issuetbl['lotldg_binid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
?>
...
$qt=$t_gdsum[1];*/
//echo $p1_array[$ct];
/*$sql_iss=mysqli_query($link,"select max(slocsubid) from tbl_sloc_sub where gid='".$trid."' and whid='".$t_gddist['whid']."' and binid='".$t_gddist['binid']."' and subbinid='".$t_gddist['subbinid']."'") or die(mysqli_error($link));
 $t=mysqli_fetch_array($sql_iss);*/
//$srno=1;
//echo $t[0];
$sql_issue=mysqli_query($link,"select * from tbl_sloc_sub where slocsubid='".$p1_array[$ct]."' and plantcode='$plantcode'") or die(mysqli_error($link));
$totBags=0; $totqty=0; 
 while($row_issue=mysqli_fetch_array($sql_issue))
 { 
  $up=$row_issue['opbags'];
...
?>
  <tr class="Light" height="30" >
    ...
    <?php
$bing1_query=mysqli_query($link,"select binid, binname from tbl_bin where whid='".$row_issue['whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
?>
    <td width="93" align="center"  valign="middle" class="tbltext" id="bing<?php echo $srno?>">&nbsp;<select class="tbltext" name="txtslbing<?php echo $srno?>" style="width:60px;" onchange="bin<?php echo $srno?>(this.value);" >
          <option value="" selected>--Bin--</option>
		  <?php while($noticia_bing1 = mysqli_fetch_array($bing1_query)) { ?>
    ...
    <?php
$subbing1_query=mysqli_query($link,"select sid, sname from tbl_subbin where whid='".$row_issue['whid']."' and binid='".$row_issue['binid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
?>
    <td width="113" align="center"  valign="middle" class="tbltext" id="sbing<?php echo $srno?>">&nbsp;<select class="tbltext" name="txtslsubbg<?php echo $srno?>" style="width:80px;" onchange="subbin<?php echo $srno?>(this.value);"  >
          <option value="" selected>--Sub Bin--</option>
		  <?php while($noticia_subbing1 = mysqli_fetch_array($subbing1_query)) { ?>
