...
if(isset($_GET['txtlot1']))
	...
	if(isset($_GET['sloctyp']))
	{
	$sloctype = $_GET['sloctyp'];	 
	}
	
	if($sloctype=='Regular Warehouse')$ocnt=2;
	if($sloctype=='Cold Storage')$ocnt=10;
	if($sloctype=='Both')$ocnt=10;
...
if($trid == 0)
{
 $sql_in1="insert into tbl_sloc (code, sldate, crop, variety, lotno, yearcode, surole,stage, plantcode) values ('$code', '$tdate', '$txtcrop', '$txtvariety', '$txtlot1', '$yearid_id', '$lgnid','$tp', '$plantcode')";
...
if(mysqli_query($link,$sql_in1) or die(mysqli_error($link)))
{
	...
	for($j=1; $j<=$ocnt; $j++)
	{
		...
		if(isset($_GET[$dorowigx])) { $dorowig= $_GET[$dorowigx]; }
		if(isset($_GET[$rowidx])) { $rowid= $_GET[$rowidx]; }
		
		if($txtslqtyg!="" && $txtsubbg!="")
		{
			$sql_in="insert into tbl_sloc_sub(slocid, crop, variety, lotno, whid, binid, subbinid, opbags, opqty, balbags, balqty, bags, qty, rowid, plantcode) values('$mainid', '$txtcrop', '$txtvariety', '$txtlot1', '$txtwhg', '$txtbing', '$txtsubbg', '$exBagsg', '$exqtyg', '$balBagsg', '$balqtyg', '$txtslBagsg', '$txtslqtyg', '$rowid', '$plantcode')";
...
{
$sql_in1="update tbl_sloc set crop='$txtcrop', variety='$txtvariety', lotno='$txtlot1', yearcode='$yearid_id', stage='$tp',surole='$lgnid' where slid='".$trid."'";
...
if(mysqli_query($link,$sql_in1) or die(mysqli_error($link)))
{
	...
	for($j=1; $j<=$ocnt; $j++)
	{
		...
		if($txtslqtyg!="" && $txtsubbg!="")
		{
			$sql_in="insert into tbl_sloc_sub(slocid, crop, variety, lotno, whid, binid, subbinid, opbags, opqty, balbags, balqty, bags, qty, rowid, plantcode) values('$mainid', '$txtcrop', '$txtvariety', '$txtlot1', '$txtwhg', '$txtbing', '$txtsubbg', '$exBagsg', '$exqtyg', '$balBagsg', '$balqtyg', '$txtslBagsg', '$txtslqtyg', '$rowidx', '$plantcode')";
			mysqli_query($link,$sql_in) or die(mysqli_error($link));
		}
	}
/* if($god1==1)
{
$sql_in="insert into tbl_sloc_sub(slocid, crop, variety, lotno, whid, binid, subbinid, opbags, opqty, balbags, balqty, bags, qty, rowid) values('$mainid', '$txtcrop', '$txtvariety', '$txtlot1', '$y', '$z', '$a1', '$exBags1', '$exqty1', '$balup1', '$balqt1', '$c1', '$b1', '$orwoid')";
mysqli_query($link,$sql_in) or die(mysqli_error($link));
}
if($god2==1)
{
$sql_in="insert into tbl_sloc_sub(slocid, crop, variety, lotno, whid, binid, subbinid, opbags, opqty, balbags, balqty, bags, qty, rowid) values('$mainid', '$txtcrop', '$txtvariety', '$txtlot1', '$d1', '$e1', '$f1', '$exBags2', '$exqty2', '$balup2', '$balqt2', '$h1', '$g1', '$orwoid')";
mysqli_query($link,$sql_in) or die(mysqli_error($link));
}
*/
}
...
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_subbinid, lotldg_binid from tbl_lot_ldg where lotldg_crop='".$txtcrop."' and lotldg_variety='".$txtvariety."' and lotldg_lotno='".$txtlot1."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_issue1=mysqli_query($link,"select max(lotldg_id) from tbl_lot_ldg where lotldg_subbinid='".$row_issue['lotldg_subbinid']."' and lotldg_binid='".$row_issue['lotldg_binid']."' and lotldg_whid='".$row_issue['lotldg_whid']."' and lotldg_variety='".$txtvariety."' and lotldg_lotno='".$txtlot1."' and plantcode='$plantcode'") or die(mysqli_error($link));
...
$sql_sloc=mysqli_query($link,"select * from tbl_sloc_sub where slocid='".$trid."' and rowid='".$row_issue1[0]."'") or die(mysqli_error($link));
...
$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$row_sloc['subbinid']."' and binid='".$row_sloc['binid']."' and whid='".$row_sloc['whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
$row_subbinn=mysqli_fetch_array($sql_subbinn);
/*if($subbinn!="")
