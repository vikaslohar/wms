...
$sql_tbl=mysqli_query($link,"select * from tbl_rswrem where logid='$logid' and  rswrem_tflg=0 and plantcode='$plantcode'") or die(mysqli_error($link));
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	 $arrival_id=$row_tbl['rswrem_id'];	
	
	$s_sub="delete from tbl_rswrem_sub where rswrem_id='".$arrival_id."' ";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));
	
	$s_sub_sub="delete from tbl_rswremsub_sub where rswrem_id='".$arrival_id."'";
	mysqli_query($link,$s_sub_sub) or die(mysqli_error($link));
	
}

	$s_sub="delete from tbl_rswrem where logid='$logid' and  rswrem_tflg=0";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	
...
<title>QC - Transaction - RSW - Raw Seed Release</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#e48324" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#e48324" style="border-bottom:solid; border-bottom-color:#e48324" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Raw Seed Release</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:Pointer;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="trn_qty_removal.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
...
$targetpage = $_SERVER['PHP_SELF']; 
  ...
  $sql_arr_home=mysqli_query($link,"select * from tbl_rswrem where rswrem_tflg=1 and plantcode='$plantcode' order by rswrem_code desc LIMIT $start, $limit") or die(mysqli_error($link));
...
$query = "select * from tbl_rswrem where rswrem_tflg=1 and plantcode='$plantcode' order by rswrem_code desc";
...
$sql_arr_home=mysqli_query($link,"select * from tbl_rswrem where rswrem_tflg=1  order by rswrem_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

 $total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_rswrem where rswrem_tflg=1");
$total_results2 = mysqli_fetch_array($total_results3);
$total_results = $total_results2[0]; */
...
<table align="center" border="0" width="950" cellspacing="0" cellpadding="0" bordercolor="#e48324" style="border-collapse:collapse" > 
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="950" bordercolor="#e48324" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
              ...
              <td width="13%" align="center" valign="middle" class="tblheading"> Lot Number</td>
			   <td width="7%" align="center" valign="middle" class="tblheading">Released NoB</td>
			   <td width="8%" align="center" valign="middle" class="tblheading">Released Qty</td>
               <td width="7%" align="center" valign="middle" class="tblheading">Balance NoB</td>
...
{
	$trdate=$row_arr_home['rswrem_date'];
	...
	$arrival_id=$row_arr_home['rswrem_id'];

	$crop=""; $variety=""; $nob1=""; $qty1=""; $balnob1=""; $balqty1=""; $lot="";
	...
	$nob=0; $qty=0; $balnob=0; $balqty=0;
	$sql_tblsub_sub=mysqli_query($link,"select * from tbl_rswremsub_sub where rswrem_id='".$arrival_id."' and rswremsub_id='".$row_tbl_sub['rswremsub_id']."' and plantcode='$plantcode'") or die(mysqli_error($link));
	$subsubtbltot=mysqli_num_rows($sql_tblsub_sub);
	while($row_tblsub_sub=mysqli_fetch_array($sql_tblsub_sub))
	{
	$nob=$nob+$row_tblsub_sub['remnob'];
	$qty=$qty+$row_tblsub_sub['remqty'];
	$balnob=$balnob+$row_tblsub_sub['balnob'];
	$balqty=$balqty+$row_tblsub_sub['balqty'];
	}
	if($balqty==0)$balnob=0;
...
<tr class="Light">
         <td width="3%" align="center" valign="middle" class="tblheading"><?php echo $srno;?></td>
		   <td width="8%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
		     <td width="13%" align="center" valign="middle" class="tblheading"><?php echo "RR".$row_arr_home['rswrem_code']."/".$yearid_id."/".$lgnid;?></td>
