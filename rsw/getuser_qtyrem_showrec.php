...
if($h > 0)
{
	$sql_tbl_sub=mysqli_query($link,"select * from tbl_rswrem_sub where rswrem_id='".$h."' and lotnumber='$a' and plantcode='$plantcode'") or die(mysqli_error($link));
	$subtbltot=mysqli_num_rows($sql_tbl_sub);
	if($subtbltot > 0) $tttt=1;
}
$sql_issue=mysqli_query($link,"select distinct lotldg_whid, lotldg_binid, lotldg_subbinid  from tbl_lot_ldg where lotldg_crop ='".$b."' and lotldg_variety='".$c."' and lotldg_sstage='Raw' and lotldg_lotno='$a' and plantcode='$plantcode'") or die(mysqli_error($link));
...
<?php
$verty="";
$quer4=mysqli_query($link,"SELECT varietyid, popularname FROM tblvariety where varietyid='".$c."' and actstatus='Active' and vertype='PV' order by popularname Asc"); 
$tt=mysqli_num_rows($quer4);
if($tt>0)
{
$noticia = mysqli_fetch_array($quer4);
$verty=$noticia['popularname'];
}
else
{
$verty=$c;
...
?>
    ...
    <td align="left"  valign="middle" class="tbltext" id="vitem" >&nbsp;<input type="text" name="txtvariety1" readonly="true" style="background-color:#CCCCCC;" value="<?php echo $verty;?>" size="32"  />&nbsp;<font color="#FF0000">*</font>&nbsp;<input type="hidden" name="txtvariety" value="<?php echo $c;?>" /></td>
           </tr>
</table>		   
<table align="center" border="1" width="850" cellspacing="0" cellpadding="0" bordercolor="#e48324" style="border-collapse:collapse" >
  <tr class="tblsubtitle" height="20">
...
$sql_whouse=mysqli_query($link,"select perticulars from tbl_warehouse where whid='".$row_issue['lotldg_whid']."' and plantcode='$plantcode' order by perticulars") or die(mysqli_error($link));
$row_whouse=mysqli_fetch_array($sql_whouse);
$wareh=$row_whouse['perticulars']."/";

$sql_binn=mysqli_query($link,"select binname from tbl_bin where binid='".$row_issue['lotldg_binid']."' and whid='".$row_issue['lotldg_whid']."'") or die(mysqli_error($link));
$row_binn=mysqli_fetch_array($sql_binn);
$binn=$row_binn['binname']."/";

$sql_subbinn=mysqli_query($link,"select sname from tbl_subbin where sid='".$row_issue['lotldg_subbinid']."' and binid='".$row_issue['lotldg_binid']."' and whid='".$row_issue['lotldg_whid']."' and plantcode='$plantcode'") or die(mysqli_error($link));
	...
	<?php while($noticia_item = mysqli_fetch_array($sql_month)) { ?>
		...
		<?php } ?></select>&nbsp;<font color="#FF0000">*</font>&nbsp;</td>
         ...
         <tr class="Light" height="30" id="vitem">
           ...
           <td align="left" width="272" valign="middle" class="tbltext" style="border-color:#F1B01E" >&nbsp;<input name="txtlot1" type="text" size="15" class="tbltext" tabindex=""  maxlength="14"  value="" onchange="ltchk(this.value);"  onBlur="javascript:this.value=this.value.toUpperCase();" >&nbsp;<font color="#FF0000">*</font>&nbsp;<a href="Javascript:void(0);" onclick="openslocpop();">Select</a><input type="hidden" name="txtlotnoid" /></td>
