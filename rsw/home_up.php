...
while($row_tbl=mysqli_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tbl_rsw where arrival_id='".$arrival_id."'";
	...
	/*$s_sub="delete from tbl_rsw where arr_role='".$logid."' arrtrflag=1";
	mysqli_query($link,$s_sub) or die(mysqli_error($link));	*/
...
$sql_arr_home=mysqli_query($link,"select * from tbl_rsw_main where arr_role='$logid' and arrtrflag=1 and plantcode='$plantcode' order by arr_code desc LIMIT $from, $max_results") or die(mysqli_error($link));
$tot_arr_home=mysqli_num_rows($sql_arr_home);

$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tbl_rsw_main where arr_role='$logid' and arrtrflag=1 and plantcode='$plantcode'");
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="650" bordercolor="#F1B01E" style="border-collapse:collapse">
