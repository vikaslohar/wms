...
<!-- actual page start--->	
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="5398ee" >
	...
	<table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
                <tr >
                  <td width="810" height="25">&nbsp; Sales Return - PV SLOC Master - Warehouse</td>
                </tr>
            </table></td>
  ...
  <td align="center" colspan="4" >
	...
	$sql_sel="select * from tblpvwarehouse order by perticulars LIMIT $from, $max_results";
	$res=mysqli_query($link,$sql_sel) or die (mysqli_error($link));
	
	$total=mysqli_num_rows($res);
	$total_results3 =mysqli_query($link,"SELECT COUNT(*)  FROM tblpvwarehouse");
...
$srno=1;
	...
	*/
	 $sql_p=mysqli_query($link,"select * from tblpvbin where whid='".$row['whid']."' and  plantcode='".$row['plantcode']."'")or die(mysqli_error($link));
  	 $row_p=mysqli_fetch_array($sql_p);
	$num_of_records_target_set=mysqli_num_rows($sql_p);
	 $bin_no=$num_of_records_target_set;
	 $row['whid'];
	$sql_v=mysqli_query($link,"select * from tblpvsubbin where whid='".$row['whid']."' and  plantcode='".$row['plantcode']."'")or die(mysqli_error($link));
...
?>
                  <tr class="Light" height="25">
                    ...
                    <td valign="middle" class="tbltext" align="center"><a href="bin_home3.php?whid=<?php echo $row['whid'];?>&plantid=<?php echo $row['plantcode']?>"><?php echo $bin_no;?></a> </td>
