<?php
	 ...
	 if(isset($_POST['frm_action'])=='submit')
	{
		...
		//exit;
		 ...
		 {
			$sql_in="insert into tblvariety(cropname, cropid, popularname, opt, vt, gm, wbtype, wtnop, wtnopkg, mtyp, mtype, wtmp, nowb,gsdis, expdt, moinlors, mptnop, expdtt, dorduration, stlduration, twgptsf, twgptst, actstatus, ppt, vertype, leduration) values('$cropname', '$cname',  '$vname', '$opt', '$type', '$gm', '$wb', '$nop', '$wtwb', '$mp', '$mpytype', '$wtmp', '$nowb','$txtdsp', '$expdtg', '$moilars', '$mptnop', '$expdtt', '$txtdordurtion', '$txtstduration', '$txttwtpgmsf', '$txttwtpgmst', '$actsts', '$txtppt', '$vertyp', '$txtleduration')";
			if(mysqli_query($link,$sql_in)or die(mysqli_error($link)))
			{
				$id=mysqli_insert_id($link);
				$sqlin="update tblvariety set pvverid='$tid' where varietyid='$id' and actstatus='Active'";
				mysqli_query($link,$sqlin)or die(mysqli_error($link));
				
				if($trtyp==0)
				echo "<script>window.location='home_variety.php?page=$pageid'</script>";
				else
				echo "<script>window.location='add_cvvariety.php?page=$pageid&varietyid=$tid'</script>";	
			}
		}
}
?>
...
<table align="center" border="1" width="950" cellspacing="0" cellpadding="0" bordercolor="#4ea1e1" style="border-collapse:collapse"  vspace=""> 
	...
	<tr class="Light" height="25">
		...
		<td width="165"  align="left"  valign="middle" class="smalltbltext">&nbsp;<?php echo $row_main['expdtt'];?>&nbsp;days<input name="expdtt" type="hidden" size="1" class="smalltbltext" tabindex="0" onChange="ddperiod(this.value);" onKeyPress="return isNumberKey(event)"  maxlength="2" value="<?php echo $row_main['expdtt'];?>" /></td>
	</tr>
	<tr class="Dark" height="25">
...
<?php
$srn=1; $verlimit=10;
	$sql_v=mysqli_query($link,"select * from tblvariety where pvverid='".$row_main['varietyid']."' and actstatus='Active' order by vertype desc");
